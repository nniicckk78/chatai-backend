...
async function runPlanningStep(customerMessage, detectedSituations, allRules, conversationHistory = '', alreadyIntroducedSelfInChat = false, currentMessageNotClearlySexual = false, currentMessageHasImage = false, customerClaimsWeAgreedOrWantsAddress = false, lastPreviousCustomerMessageAgeMs = null, customerOccupation = null, isPureGreetingFromInterpreter = null, fakeGenderForPlan = null, customerGenderForPlan = null) {
  ...
  const hasUndDuAlsNamensfrage = hasUndDuRueckfrage && /\b(ich\s+)?hei[ßs]e\s+[\wäöüß-]+|(ich\s+bin\s+)[\wäöüß-]+|(name|hei[ßs]e)\s+[\wäöüß-]+/i.test(customerSnippet);
...
async function runMistralCorrector({ customerMessage = '', context = {}, grokText = '', learningContext = '', exampleSnippet = '', planSnippet = '', conversationSnippet = '' }) {
  ...
  const hasUndDuAlsNamensfrageMistral = hasUndDuRueckfrageMistral && /\b(ich\s+)?hei[ßs]e\s+[\wäöüß-]+|(ich\s+bin\s+)[\wäöüß-]+|(name|hei[ßs]e)\s+[\wäöüß-]+/i.test(fullCustomerMsg);
...
async function runOpenAIFullCorrector({ customerMessage = '', context = {}, grokText = '', learningContext = '', exampleSnippet = '', planSnippet = '', conversationSnippet = '' }) {
  ...
  const hasUndDuAlsNamensfrageOpenAI = hasUndDuRueckfrageOpenAI && /\b(ich\s+)?hei[ßs]e\s+[\wäöüß-]+|(ich\s+bin\s+)[\wäöüß-]+|(name|hei[ßs]e)\s+[\wäöüß-]+/i.test(fullCustomerMsg);
  ...
  const answerDirectQuestionRuleOpenAI = hasDirectQuestionToPersonaOpenAI
    ? ' PFLICHT: Die AKTUELLE Kundennachricht enthaelt eine direkte Frage an die Persona (z.B. wie geht es dir, wie heisst du, was machst du – oder "was machst du heute?", "was hast du vor?" – oder "was soll ich mit dir machen?", "sag mal was ich tun soll", "was wuerdest du dir wuenschen?" – oder Treffen). Beantwortet die Antwort diese Frage? Bei "und du?" / "und du" am Ende: Kunde stellt Rueckfrage (z.B. nach deinem Namen wenn er seinen genannt hat, nach deinen Vorlieben wenn es um Vorlieben ging). Enthaelt die Antwort KEINE Antwort auf diese Rueckfrage (z.B. keinen Namen wenn er seinen genannt hat)? Dann AM ANFANG oder passend einbauen: Name nennen bzw. Vorlieben/Tabus nennen. Bei "was soll ich mit dir machen" / "was wuerdest du dir wuenschen": ZUERST inhaltlich beantworten (z.B. 1–2 konkrete Vorschlaege/Wuensche: lecken, kuessen, verwöhnen), NICHT nur zurueckfragen oder Meta-Kommentar. Bei "was machst du heute"/"was hast du vor": kurz etwas Passendes nennen. Bei Treffen-Frage: kurz unverbindlich. Sonst: zuerst Frage beantworten, dann Gegenfrage.'
    : '';
  const treffenFrageIgnoredRuleOpenAI = hasTreffenFrageOpenAI
    ? ' TREFFEN-FRAGE IGNORIERT: Kunde fragt nach Treffen (z.B. "Treffen wir uns heute?", "Wann sehen wir uns?") und die Antwort geht NICHT darauf ein (kein "heute klappt nicht", kein "muss schauen", kein "passt heute nicht", kein unverbindliches Abblocken)? Dann AM ANFANG der Antwort einen kurzen Satz einfuegen: z.B. "Heute klappt es leider nicht, aber " oder "Muss schauen wann es passt – " dann den bisherigen Antworttext. Die Treffen-Frage darf nie ignoriert werden.'
    : '';
  const hasNameOrWohnortRequestOpenAI = context.askForNameOrWohnort === true || hasUndDuAlsNamensfrageOpenAI || /\b(vornamen?|name)\s+und\s+wohnort\b|\bwohnort\s+und\s+(vornamen?|name)\b|\b(deinen?|dein)\s+(vornamen?|wohnort)\b|\bdenkst\s+du\s+noch\s+an\s+(deinen?\s+)?(vornamen?|wohnort)\b/i.test(fullCustomerMsg);
