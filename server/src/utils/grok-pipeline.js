...
const EXTRA_RULES = {
  ...
  guteSchlussfrage: 'GUTE SCHLUSSFRAGE: Die Frage am Ende muss sich auf das beziehen, was der Kunde gesagt hat, oder auf das aktuelle Gespraechsthema – oder passende Nebenthemen ansprechen (Thema sinnvoll erweitern, z.B. Stimmung, Vorlieben, naechster Schritt). VERBOTEN: vage/generische Fragen wie "Was denkst du darüber?", "was trinkst du gerne dazu?" / "was magst du daran?" wenn "dazu"/"daran" unklar ist oder die Frage vom Thema abgekoppelt wirkt. Stattdessen: konkrete, thematisch passende Frage (z.B. nach Stimmung/Gefuehl, "wuerde dir das gefallen?", "magst du das?"; nie "Was denkst du darüber?"). Nie dieselben Begriffe in Frageform zurueckgeben; nie unklare "dazu/daran/damit"-Fragen. Die Frage soll dem Kunden einen Grund geben, wieder zu antworten – also keine Ja/Nein-Killer, lieber nach Erlebnissen, Meinungen, Fantasien oder "und du?" fragen.\n',
  schlussfrageImSzenario: 'SCHLUSSFRAGE – IMMER IM THEMA/SZENARIO BLEIBEN (gilt fuer alle Themen): Die Frage am Ende fragt nach dem, was im Gespraech passiert, wie etwas ist, was als naechstes kommt, oder nach konkreten Vorlieben/Details – sie bleibt im Szenario. Sie fragt NICHT nach Erwartungen, Bewertung oder Reflexion (keine Fragen nach "was erhoffst/erwartest du", "wie intensiv", "was wuerdest du dir wuenschen", "was denkst du wie das waere"). Reflexionsfragen wirken unnatuerlich; Fragen im Szenario (Ablauf, Wie, Als-naechstes, Vorliebe, Stimmung) sind die richtige Art.\n',
  besteFrageAusKontext: 'BESTE FRAGE AUS DEM KONTEXT: Ueberlege bei jeder Antwort: Was waere in DIESEM Kontext die naheliegendste oder beste naechste Frage? Die Schlussfrage soll direkt an das anknuepfen, was der Kunde gesagt hat oder was im Szenario/Thema passiert (z.B. was als naechstes, wie, was magst du mehr, woran denkst du gerade, wie stellst du dir X vor). Wenn sich eine konkrete, passende Frage anbietet – diese stellen. Keine generische oder reflexhafte Frage waehlen, wenn der Kontext eine bessere, naheliegendere Frage hergibt.\n',
  schlussfrageStilWieBeispiele: 'SCHLUSSFRAGE – STIL WIE TRAININGSBEISPIELE: Die Frage am Ende soll klingen wie in den Trainingsbeispielen: EINE kurze, direkte, persoenliche Frage – umgangssprachlich, an das Gesagte oder an deine letzte Frage anknuepfend. Gute Beispiele: "und du?", "oder?", "oder irre ich mich?", "was machst du am Wochenende?", "was magst du mehr?", "hast du schon Ideen?", "wie lange machst du das schon? Gefaellt dir?", "was suchst du denn hier?". NICHT: "Was denkst du darueber?", "Was interessiert dich an X?", "Was sind Dinge die du erzaehlen moechtest?", "Was schaetzt du besonders an X?" – das wirkt analytisch und unpersoenlich. Lieber eine einfache Frage, die er direkt beantworten kann.\n',
...
async function interpretCustomerMessage(customerMessage, conversationHistorySnippet = '') {
  ...
  const systemContent = `Du liest eine Kundennachricht aus einem Chat (Dating/Flirt). Deine einzige Aufgabe: die Nachricht so verstehen, wie ein Mensch sie meint.
...
/** Alter aus ISO-Datum (YYYY-MM-DD). Fallback wenn FPC/Extension nur birthDate als Datum liefert. */
...
PFLICHT: Nur eine Nachricht ausgeben; Frage am Ende (im Stil der Beispiele); keine Meta-Kommentare; KEIN ECHO – nicht Name, Alter, Ort oder Vorlieben des Kunden zurueckspielen, eigenstaendig formulieren.`;
  ...
  if (conversationHistory && conversationHistory.trim()) {
    ...
    const lines = historySnippet.split(/\n/).filter(Boolean);
    let lastPersonaLine = '';
    for (let i = lines.length - 1; i >= 0; i--) {
      if (/^Fake:\s*/i.test(lines[i])) {
        lastPersonaLine = lines[i].replace(/^Fake:\s*/i, '').trim();
        break;
      }
    }
    userContent += `PFLICHT – DIESE NACHRICHT IST DIE AKTUELLE (LETZTE) KUNDENNACHRICHT. Antworte PRIMAER auf DIESE Nachricht – nicht auf ein frueheres Thema im Verlauf.\n\nAktuelle Kundennachricht:\n"${sanitizeForApiContent(customerMessage || '')}"\n\n`;
    userContent += `Chat-Verlauf (nur Kontext – zum Verstehen, worauf sich "es"/"das" bezieht; Antwort bezieht sich auf die obige aktuelle Nachricht):\n${sanitizeForApiContent(historySnippet)}\n\n`;
    if (lastPersonaLine) {
      userContent += `Deine letzte Nachricht im Verlauf: "${sanitizeForApiContent(lastPersonaLine.substring(0, 400))}${lastPersonaLine.length > 400 ? '…' : ''}"\n\n`;
    }
    userContent += `Generiere deine nächste Nachricht in diesem Dialog. Gehe auf die AKTUELLE Kundennachricht (die erste Box oben) ein – wenn sie ein anderes Thema oder einen anderen Ton hat als früher im Verlauf, antworte auf DIESE aktuelle Nachricht.\n\n`;
  } else {
  ...
  if (examples && examples.length > 0) {
    ...
    userContent += '\nDie Schlussfrage in diesen Beispielen ist immer kurz, eine klare Frage, thematisch passend und umgangssprachlich (z.B. "und du?", "was machst du am Wochenende?", "oder?"). Genau DIESE Art von Schlussfrage übernehmen – nicht analytisch ("Was denkst du darüber?"), nicht generisch. Antworte wie in den Beispielen an den Verlauf anknüpfend. Generiere eine Antwort mit Gegenfrage im Stil der Beispiele oben (thematisch, konkret, menschlich).\n\n';
  }
...
async function runMistralCorrector({ customerMessage = '', context = {}, grokText = '', learningContext = '', exampleSnippet = '', planSnippet = '', conversationSnippet = '' }) {
  ...
  // Antwort darf nicht auf ein frueheres Thema eingehen, wenn die AKTUELLE Kundennachricht ein anderes hat (z.B. Kunde schreibt jetzt explizit/erste Nacht, Antwort redet von Unsicherheit/Vertrauen).
  const replyMustAddressCurrentMessageRule = ' AKTUELLE NACHRICHT IGNORIERT: Enthaelt die AKTUELLE (letzte) Kundennachricht ein klares anderes Thema oder einen anderen Ton (z.B. explizit, erste Nacht, lecken, Szenario, "dich auspacken", "richtig knallen", "wenn du dich traust") – und die zu korrigierende Antwort geht stattdessen auf ein frueheres Thema ein (z.B. "unsicher", "Vertrauen braucht Zeit", "Intimitaet ist etwas Besonderes", "braucht Zeit")? Wenn ja → Antwort umschreiben: primaer auf DIESE aktuelle Kundennachricht eingehen (auf das was er JETZT geschrieben hat), nicht auf ein frueheres Thema. Ton und Thema der Antwort muessen zur aktuellen Nachricht passen.';
  ...
  const schlussfrageImSzenarioRuleMistral = ' SCHLUSSFRAGE IM SZENARIO: Die letzte Frage muss im Thema/Szenario bleiben (nach Ablauf, Wie, Vorliebe, Stimmung, was als naechstes). Ist die Schlussfrage eine Reflexionsfrage (z.B. nach Erwartungen, Erlebnis, Intensitaet – "was erhoffst/erwartest du", "wie intensiv", "was wuerdest du dir wuenschen")? Wenn ja → ersetzen durch eine konkrete Frage zum Geschehen oder zum Thema (Ablauf, Wie, Vorliebe, Stimmung), nicht nach Reflexion fragen.';
    const besteFrageAusKontextRuleMistral = ' BESTE FRAGE AUS KONTEXT: Gibt der Kontext (Kundennachricht, Szenario, Thema) eine naheliegendere naechste Frage her (z.B. was als naechstes, wie, was magst du mehr, woran denkst du gerade)? Ist die aktuelle Schlussfrage dagegen generisch oder reflexiv? Wenn ja → Schlussfrage ersetzen durch die konkrete, naheliegende Frage die direkt am Gesagten/Szenario anknuepft.';
    ...
    const noGenericOpenAIPhrasesRule = ' GENERISCHER KI-TON/FLOSKEL-EINSTIEG: Beginnt die Antwort mit Floskeln wie "Das klingt spannend", "Klingt spannend", "Das klingt gut", "Klingt gut", "klingt echt spannend", "ich stell mir vor wie wir uns näherkommen", "du mich richtig verwöhnst", "was würdest du denn als erstes mit mir anstellen?" (formelhaft)? Wenn ja → diesen Einstieg entfernen oder ersetzen: direkt auf den Inhalt der Kundennachricht eingehen, keine formelhaften Floskeln am Anfang.';
...
async function runOpenAIFullCorrector({ customerMessage = '', context = {}, grokText = '', learningContext = '', exampleSnippet = '', planSnippet = '', conversationSnippet = '' }) {
  ...
  const besteFrageAusKontextRuleOpenAI = ' BESTE FRAGE AUS KONTEXT: Gibt der Kontext (Kundennachricht, Szenario, Thema) eine naheliegendere naechste Frage her (z.B. was als naechstes, wie, was magst du mehr, woran denkst du gerade)? Ist die aktuelle Schlussfrage dagegen generisch oder reflexiv? Wenn ja → Schlussfrage ersetzen durch die konkrete, naheliegende Frage die direkt am Gesagten/Szenario anknuepft.';
  ...
  const noGenericOpenAIPhrasesRuleOpenAI = ' PFLICHT – VERBOTENE EINSTIEGE (NIEMALS stehen lassen, NIEMALS hinzufuegen): "Das klingt spannend", "Oh wow, das klingt ja spannend", "Klingt spannend", "Das klingt gut", "Klingt gut", "Das klingt super", "Klingt super", "Klingt interessant", "klingt echt spannend", "ich stell mir vor wie wir uns näherkommen", "du mich richtig verwöhnst" – diese und aehnliche Meta-Einstiege sind VERBOTEN. Beginnt die Antwort damit? → diesen kompletten Einstieg ENTFERNEN, die Nachricht beginnt mit dem naechsten inhaltlichen Satz (direkte Reaktion, Aussage oder Frage zur Kundennachricht). Wenn du die Nachricht korrigierst oder erweiterst: solche Floskeln NIEMALS einbauen. Stil der Vorlage beibehalten, keine "verbessernden" Meta-Kommentare hinzufuegen.';
  ...
  const replyMustAddressCurrentMessageRuleOpenAI = ' AKTUELLE NACHRICHT IGNORIERT: Enthaelt die AKTUELLE (letzte) Kundennachricht ein klares anderes Thema oder einen anderen Ton (z.B. explizit, erste Nacht, lecken, Szenario, "dich auspacken", "richtig knallen", "wenn du dich traust") – und die zu korrigierende Antwort geht stattdessen auf ein frueheres Thema ein (z.B. "unsicher", "Vertrauen braucht Zeit", "Intimitaet ist etwas Besonderes", "braucht Zeit")? Wenn ja → Antwort umschreiben: primaer auf DIESE aktuelle Kundennachricht eingehen (auf das was er JETZT geschrieben hat), nicht auf ein frueheres Thema. Ton und Thema der Antwort muessen zur aktuellen Nachricht passen.';
...
async function runGrokPipeline(opts) {
  ...
  if (vectorDbFunc && typeof vectorDbFunc === 'function') {
    try {
      examples = await vectorDbFunc(messageForPipeline, { topK: exampleTopK, situation: situationsForExamples, conversationHistory, includeSexual: true }) || [];
    } catch (e) {
      // ignore
    }
  }
  ...
  if (examples.length === 0 && messageForPipeline) {
    try {
      examples = await vectorDbFunc(messageForPipeline, { topK: exampleTopK, situation: null, conversationHistory, includeSexual: true }) || [];
    } catch (e) {
      // ignore
    }
  }
  if (examples.length === 0 && messageForPipeline) {
    try {
      examples = await selectSmartExamples(messageForPipeline, { topK: exampleTopK, conversationHistory, includeSexual: true }) || [];
    } catch (e) {
      // ignore
    }
  }
  ...
  let plan = '';
  try {
    plan = await runPlanningStep(messageForPipeline, detectedSituations, allRules, conversationHistory);
