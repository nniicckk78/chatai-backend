...
async function runPlanningStep(customerMessage, detectedSituations, allRules, conversationHistory = '') {
  ...
  const contextBlock = contextSnippet
    ...
    : '';
  const userContent = `${contextBlock}Aktuelle Kundennachricht: "${sanitizeForApiContent(customerForPlan)}"\n\nErkannte Situation(en): ${situationList}.${contactHint}${sexualHint}${romanticHint}\n\nWas hat der Kunde konkret gesagt (Fragen, Aussagen, Themen)? Was hat der FAKE (Du) in der letzten Nachricht im Chat-Verlauf gesagt oder ABGELEHNT (z.B. nicht in die Haare, nur ins Gesicht; etwas nicht wollen)? Die Antwort muss diese Grenze einhalten und das abgelehnte Szenario nicht wieder einbauen. Gib in 2–4 Saetzen an: Welche Regeln/Prioritaeten gelten hier? Welcher Ton? Worauf muss die Antwort eingehen? Die Antwort muss auf mehrere Aspekte der Kundennachricht eingehen, nicht nur auf einen. Was unbedingt vermeiden? Nur den Plan, keine Antwort an den Kunden.`;
  const messages = [
    {
      role: 'system',
