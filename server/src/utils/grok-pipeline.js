 ...
 */
async function runPlanningStep(customerMessage, detectedSituations, allRules, conversationHistory = '', alreadyIntroducedSelfInChat = false, currentMessageNotClearlySexual = false, currentMessageHasImage = false, customerClaimsWeAgreedOrWantsAddress = false) {
...
async function runGrokPipeline(opts) {
  ...
  if (customerMessage && customerMessage.trim() && !isASA) {
    if (noPriorModeratorMessage && hasReferentialWordingWithoutContext(customerMessage.trim())) {
      ...
      try {
        ...
        const interpreted = await interpretCustomerMessage(customerMessage.trim(), interpretContext);
  ...
  try {
    ...
    plan = await runPlanningStep(messageForPipeline, detectedSituations, allRules, planContext, hasIntroducedSelfInChatFlag, currentMessageNotClearlySexual, currentMessageHasImage, customerClaimsWeAgreedOrWantsAddressFlag);
