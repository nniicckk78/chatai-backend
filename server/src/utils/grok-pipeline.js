...
PFLICHT: Nur eine Nachricht ausgeben; mindestens eine Frage in der Nachricht (wo auch immer); keine Meta-Kommentare; KEIN ECHO – nicht Name, Alter, Ort oder Vorlieben des Kunden zurueckspielen, eigenstaendig formulieren.`;
  ...
  if (conversationHistory && conversationHistory.trim()) {
    ...
    userContent += `PFLICHT – DIESE NACHRICHT IST DIE AKTUELLE (LETZTE) KUNDENNACHRICHT. Antworte PRIMAER auf DIESE Nachricht – nicht auf ein frueheres Thema im Verlauf. Enthaelt sie mehrere Teile (z.B. Double-Text gebuendelt): als EINEN Turn behandeln; NICHT zuerst Teil 1 beantworten, dann Teil 2 – Fokus auf das NEUE oder Betonte. Was du in DEINER letzten Nachricht schon gesagt hast (z.B. wie gehts dir, was du machst), NICHT nochmal beantworten.\n\nAktuelle Kundennachricht:\n"${sanitizeForApiContent(customerMessage || '')}"\n\nKontextgewichtung: Der Chat-Verlauf unten dient NUR zum Verstehen (Bezug von "es"/"das", was du zuvor gefragt hast). Deine Antwort bezieht sich AUSSCHLIESSLICH auf die obige aktuelle Kundennachricht. Aeltere Kundenaussagen (z.B. lange ohne Beziehung/Sex) NICHT in der Antwort aufgreifen, es sei denn die aktuelle Nachricht thematisiert sie.\n\n`;
