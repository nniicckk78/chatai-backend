...
Antworte NUR mit der einen Nachricht, keine Erklaerungen.`;
...
const SITUATION_DETECT_TIMEOUT_MS = 8000;
const SITUATION_DETECT_MAX_TOKENS = 120;
...
Antworte NUR mit einem JSON-Array der zutreffenden Situationsnamen, z.B. ["Bilder Anfrage", "Sexuelle Themen"]. Kein anderer Text, keine Erklärung.`
  ...
  try {
    const raw = await callGrok(messages, {
      timeoutMs: SITUATION_DETECT_TIMEOUT_MS,
      max_tokens: SITUATION_DETECT_MAX_TOKENS,
      temperature: 0.2
    });
    ...
    console.warn('⚠️ LLM-Situationserkennung fehlgeschlagen, Fallback auf Stichwörter:', err.message);
    return null;
  }
}
...
async function runGrokPipeline(opts) {
  ...
  try {
    ...
    let finalMessage = await callGrok(messages);
    finalMessage = postProcessMessage(finalMessage);
    // ========== KORREKTOR-LORA: nur bei normaler Reply, nur wenn USE_GROK_CORRECTOR_LORA=true + CORRECTOR_LORA_MODEL_ID gesetzt ==========
    ...
    if (useCorrectorEnv && correctorModelId) {
    ...
    if (corrected && corrected.trim()) {
      ...
      if (lenNew >= minLen) {
        ...
        console.log('ℹ️ Grok-Pipeline: Korrektor-Ergebnis verworfen (zu kurz: ' + lenNew + ' < ' + minLen + ', behalte Original)');
      }
    }
    const MAX_FINAL = 250;
