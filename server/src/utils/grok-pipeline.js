...
async function ensureQuestionInMessage(message, opts = {}) {
  ...
  const systemRules = [
    ...
    'VERBOTEN: Reflexionsfragen wie "Was reizt dich (am meisten/dabei/daran)?", "Was denkst du dazu/darueber?", "Was ist dir dabei am wichtigsten?", "Wie intensiv koennte das sein?" – nie einbauen. Nur konkrete Fragen, auf die man mit Handlung/Vorliebe/Ja-Nein antworten kann.',
...
async function runMistralCorrector({ customerMessage = '', context = {}, grokText = '', learningContext = '', exampleSnippet = '', planSnippet = '', conversationSnippet = '' }) {
  ...
  const schlussfrageImSzenarioRuleMistral = ' SCHLUSSFRAGE IM SZENARIO: Die letzte Frage muss im Thema/Szenario bleiben (nach Ablauf, Wie, Vorliebe, Stimmung, was als naechstes). Ist die Schlussfrage eine Reflexionsfrage (z.B. nach Erwartungen, Erlebnis, Intensitaet – "was erhoffst/erwartest du", "wie intensiv", "was wuerdest du dir wuenschen")? Wenn ja → ersetzen durch eine konkrete Frage zum Geschehen oder zum Thema (Ablauf, Wie, Vorliebe, Stimmung), nicht nach Reflexion fragen.';
    const noReiztAmMeistenRuleMistral = ' REFLEXIONSFRAGEN "REIZT/AM MEISTEN/DENKST DU DAZU" VERBOTEN: Enthaelt die Antwort eine Frage mit "reizt dich", "was reizt", "am meisten dabei/daran?", "was denkst du dazu/darueber?" oder aehnlicher Reflexion? Wenn ja → diese Frage ERSETZEN durch eine konkrete Frage (Ablauf, naechster Schritt, "magst du X?", "und dann?", konkrete Vorliebe). Der ERSATZ darf NICHT "reizt", "am meisten" (im Sinne von was reizt dich) oder "was denkst du dazu/darueber?" enthalten – nur handlungsnahe, konkrete Fragen.';
...
async function runOpenAIFullCorrector({ customerMessage = '', context = {}, grokText = '', learningContext = '', exampleSnippet = '', planSnippet = '', conversationSnippet = '' }) {
  ...
  const noReiztAmMeistenRuleOpenAI = ' REFLEXIONSFRAGEN "REIZT/AM MEISTEN/DENKST DU DAZU" VERBOTEN: Enthaelt die Antwort eine Frage mit "reizt dich", "was reizt", "am meisten dabei/daran?", "was denkst du dazu/darueber?" oder aehnlicher Reflexion? Wenn ja → diese Frage ERSETZEN durch eine konkrete Frage (Ablauf, naechster Schritt, "magst du X?", "und dann?", konkrete Vorliebe). Der ERSATZ darf NICHT "reizt", "am meisten" (im Sinne von was reizt dich) oder "was denkst du dazu/darueber?" enthalten – nur handlungsnahe, konkrete Fragen.';
...
PHRASEN STRIKT VERBOTEN (sofort ersetzen, nie stehen lassen): "Was reizt dich...", "reizt dich am meisten", "was reizt dich dabei/daran", "was denkst du dazu/darueber?", "was ist dir dabei am wichtigsten?" – durch konkrete, gespraechsweiterfuehrende Frage ersetzen (z.B. "Und dann?", "Gefaellt dir das?", "Magst du X?", naechster Schritt, Vorliebe). Reflexionsfragen verboten.
