...
/**
 * Entfernt offensichtliche Wiederholungen desselben Begriffs/Phrase (z.B. "MST Service" mehrfach).
 * Behaelt nur die erste Erwaehnung, ersetzt weitere durch "…" und raeumt auf.
 * @param {string} replyText - Moderatoren-Antwort
 * @returns {string}
 */
function removeRepeatedPhrases(replyText) {
  if (!replyText || typeof replyText !== 'string') return replyText;
  const text = replyText.trim().replace(/\s+/g, ' ');
  if (text.length < 50) return replyText;
  // Finde Woerter/Phrasen die 2+ mal vorkommen (2-5 Woerter, mind. 4 Zeichen pro Wort)
  ...
  for (let len = 5; len >= 2; len--) {
    for (let i = 0; i <= words.length - len; i++) {
      ...
      if (matches && matches.length >= 2) {
        result = result.replace(regex, (m, offset) => (offset === 0 ? m : '…'));
        result = result.replace(/\s*…\s*…\s*/g, ' … ').replace(/\s+/g, ' ').trim();
        result = result.replace(/^\s*…\s*[,.]?\s*/i, '').replace(/\s*[,.]?\s*…\s*$/i, '').trim();
...
async function runGrokPipeline(opts) {
  ...
  try {
    ...
    finalMessage = removeCustomerEcho(customerMessage, finalMessage);
    finalMessage = removeRepeatedPhrases(finalMessage);
