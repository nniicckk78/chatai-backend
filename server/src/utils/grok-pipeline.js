...
// Allgemein: Thema halten, nicht zu schlau, Schreibfehler verstehen, natürlich wirken
...
function buildASAPrompt({ allRules, asaConversationContext, asaExample }) {
  ...
  const systemContent = `${MODERATOR_PERSONA}${GENERAL_BEHAVIOR}Du antwortest auf eine System-Nachricht (Kuss oder Like) – der Kunde hat dich geliked oder einen Kuss geschickt, du schreibst die ERSTE Antwort.
${rulesBlock}
...
function buildFirstMessagePrompt({ allRules, firstMessageInstructions, profileInfo, extractedUserInfo }) {
  ...
  const systemContent = `${MODERATOR_PERSONA}${GENERAL_BEHAVIOR}Du antwortest als FAKE-PROFIL namens ${fakeName}${fakeCity ? ` aus ${fakeCity}` : ''}.
${rulesBlock}
...
function buildCityPrompt({ allRules, cityInstructions, customerMessage }) {
  const rulesBlock = buildRulesBlock(allRules);
  const systemContent = `${MODERATOR_PERSONA}${GENERAL_BEHAVIOR}${rulesBlock}
...
function buildNormalPrompt({
  ...
  systemContent += `LOGIK: Gehe auf die Kundennachricht ein. Stelle eine passende Frage. Mindestens 150 Zeichen. Natürlich und locker.
