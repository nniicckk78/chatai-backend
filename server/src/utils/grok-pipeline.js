...
async function runMistralCorrector({ customerMessage = '', context = {}, grokText = '', learningContext = '', exampleSnippet = '', planSnippet = '', conversationSnippet = '' }) {
  ...
  const meetingTreffenAussichtRule = (context.isMeetingSituation === true)
    ...
    : '';
  // Korrektor soll keine neuen Inhalte erzeugen oder kuenstlich verlaengern.
  const minLengthRule = ' KEINE KUENSTLICHE VERLAENGERUNG: Falls die Antwort kurz ist, NICHT mit neuen Inhalten oder neuen Themen erweitern. Nur vorhandenen Inhalt korrigieren und bei Bedarf kompakt umformulieren.';
...
async function runOpenAIFullCorrector({ customerMessage = '', context = {}, grokText = '', learningContext = '', exampleSnippet = '', planSnippet = '', conversationSnippet = '' }) {
  ...
  const minLengthRule = ' Keine kuenstliche Verlaengerung: Ist die Antwort kurz, nicht mit neuen Themen erweitern; nur korrigieren und bei Bedarf kompakt umformulieren.';
...
async function runGrokPipeline(opts) {
  ...
  try {
    ...
    plan = await runPlanningStep(messageForPipeline, detectedSituations, allRules, planContext, hasIntroducedSelfInChatFlag, currentMessageNotClearlySexual, currentMessageHasImage, customerClaimsWeAgreedOrWantsAddressFlag, opts.lastPreviousCustomerMessageAgeMs, customerOccupationForPlan, isPureGreetingFromInterpreter);
