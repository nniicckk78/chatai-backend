...
async function runPlanningStep(customerMessage, detectedSituations, allRules, conversationHistory = '', alreadyIntroducedSelfInChat = false, currentMessageNotClearlySexual = false, currentMessageHasImage = false, customerClaimsWeAgreedOrWantsAddress = false, lastPreviousCustomerMessageAgeMs = null, customerOccupation = null) {
  ...
  const hasDirectQuestionToPersona = hasNameQuestionInclUndDu || hasWieGehtEsDir || hasTreffenFrage || hasWasMachstDuHeute || hasWasSollIchMitDirPlan || hasUndDuRueckfrage || /\b(für dich|für dich\?|was macht .* für dich|was suchst du|wie oft|was wäre für dich|was findest du|was ist für dich|besonders schön für dich)\b/i.test(customerSnippet) || (/\?/.test(customerSnippet) && /\b(dich|dir|du)\b/i.test(customerSnippet));
