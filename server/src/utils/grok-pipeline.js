...
async function runOpenAIFullCorrector({ customerMessage = '', context = {}, grokText = '', learningContext = '', exampleSnippet = '', planSnippet = '', conversationSnippet = '' }) {
  ...
  const noEchoRule = ' Gib NUR die Moderatoren-Antwort zurueck. Kundennachricht weder woertlich noch sinngemaess nachplappern.';

  const systemContent = `PFLICHT: Nur die fertige korrigierte Nachricht ausgeben, keine Erklaerungen.

Du bist ein Korrektor für Chat-Moderator-Antworten. Entscheide anhand des gesamten Kontexts und der Kundennachricht. ${sexualRule} ${contactIrritatedRule}${meetingRule} ${multiThemeRuleOpenAI} ${noFalseSingleRule} ${noTelefonsexPhotoRule} ${ruckfrageCallbackRule} ${noWiedersehenRule} ${noSharedPastRule} ${noTimeAgreementRule} ${boundaryConsistencyRule} ${selfConsistencyRule} ${noEchoOwnModeratorStatementRule} ${censorshipRule} ${noOftTreffenRule} ${limitationFollowUpRule} ${engageOnSuggestionsRule} ${metaRule} ${noEchoRule}${questionAndWholeRule}${minLengthRule}${sameActivityPlaceRule}${noParaphraseRule}${customerSingularRule}${singleProfileNoWirRule} Stil und Wortschatz der ursprünglichen Antwort möglichst beibehalten. Jede Nachricht muss eine Frage enthalten; mindestens 120 Zeichen.${humanTyposHint} ${imageOnlyAnnouncedRule} ${noPlaceClaimRule} ${noStrongHopeRule} ${neutralMessageNoSexRule}
