...
async function ensureQuestionInMessage(message, opts = {}) {
  ...
  // Post-Processing soll die gleichen Kernregeln wie der Generator nutzen, aber NUR eine Frage einfuegen (sonst nichts umschreiben).
  const systemRules = [
    // Generator-Kern (Stil/Grundhaltung)
    MODERATOR_PERSONA,
    GENERAL_BEHAVIOR,
    ONE_TASK_PER_MESSAGE,
    '',
    // Strikter Auftrag fuers Einfuegen
    'AUFGABE (NUR EINBAUEN, NICHT UMSCHREIBEN): Die folgende Nachricht enthaelt keine Frage. Fuege GENAU EINE kurze, thematisch passende Frage ein (irgendwo in der Nachricht, z.B. am Ende oder mitten drin).',
    'Du darfst NICHT umformulieren, NICHT neu strukturieren, NICHT kuerzen und keine zusaetzlichen Fakten erfinden. Der bestehende Text bleibt inhaltlich gleich; du fuegst nur eine Frage ein.',
    'Gib NUR die komplette Nachricht inkl. der eingefuegten Frage zurueck. Keine Erklaerungen.',
    '',
    // Frage-Regeln wie im Generator (kompakt gebuendelt)
  ...
  try {
    ...
    const result = await callGenerator([
      ...
      { role: 'user', content: userContent }
    ], { temperature: 0.2, max_tokens: 180, timeoutMs: 12000 });
    const out = (result || '').trim();
    if (out && out.includes('?')) {
      console.log('✅ Grok-Pipeline: Frage in Nachricht ergaenzt (Post-Processing, Generator)');
