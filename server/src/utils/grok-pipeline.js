...
async function runGrokPipeline(opts) {
  ...
  try {
    ...
    const fakeFirstName = (profileInfo?.moderatorInfo?.firstName || profileInfo?.moderatorInfo?.name || moderatorName || extractedUserInfo?.assistant?.Name || '').toString().trim().split(/\s+/)[0] || '';
