...
- Allgemeine Zustimmung zu anderen Themen (Vorlieben, Stimmung, Kennenlernen im Chat): "klingt gut", "gerne", "passt" ohne Treffen-Anfrage des Kunden = false.
...
function hasSuspiciousFormatPatterns(text) {
  if (!text || typeof text !== 'string' || !text.trim()) return false;
  const t = text;
  // Eckige Klammern mit Inhalt (interne Notizen / CoT) – im Chat unüblich
 ...
 * Im Chat nur den Ortsnamen nennen – ohne Klammerzusatz. Stadtnamen mit echter Klammer-Disambiguierung bleiben (z. B. "Frankfurt (Oder)", "Berlin (Mitte)").
 */
...
async function checkLocationQuestion({
  ...
  if (customerCity && customerCity.trim() !== '' && customerCity.toLowerCase() !== 'sag ich später') {
    if (modTextsForCity.length > 0 && moderatorTextsImplySameCityAsCustomer(modTextsForCity, customerCity)) {
      ...
      if (cityLower.includes('offenbach')) districtHint = 'Du wohnst in Offenbach (bzw. Offenbach am Main) – konsistent zum Chat-Verlauf/Spam, keine andere Stadt im Umkreis erfinden.';
      ...
      return {
        ...
        cityInstructions: `Der Kunde fragt nach deinem Wohnort. Im Chat-Verlauf wurde bereits dieselbe Region/Stadt angesprochen (z.B. Anfahrt, "nach [Ort]") wie beim Kunden ("${cityAligned}"). Du wohnst auch in "${cityAligned}" – NICHT eine andere Nachbarstadt erfinden. ${districtHint} ${closingWohnort}`,
...
- WICHTIG MEHRDEUTIGE ZEITPHRASEN ("weiter machen"): Formulierungen wie "heute Nachmittag weiter machen", "später weiter", "nachher weiter", "wenn ich zurück bin weiter" können Unterschiedliches bedeuten (weiter schreiben/weiter reden im Chat, oder Treffen/Verfügbarkeit). DU MUSST KONTEXT und Wortlaut abwaegen:
  - Wenn der Kunde im selben Atemzug erklaert, dass er jetzt etwas anderes macht (z.B. Termin, Arbeit, Familie treffen, unterwegs, "muss gleich los", "hab um 12 ein Treffen mit meiner Schwester") und dann sagt, er will "nachher/heute Nachmittag weiter machen", dann ist sehr wahrscheinlich gemeint: später WEITER SCHREIBEN/weiter chatten. Interpretiere es dann auch so (kein Treffen daraus machen).
...
- Drueckt die Nachricht Frustration/Genervtheit aus (z.B. "gibt es nicht zu schreiben", "keine Lust nur zu schreiben", "nur schreiben bringt nichts", "cybermist", "wann wo mehr") UND enthaelt sie "wann", "wo" oder "mehr" im Sinne von "mehr als Chat"/Treffen? Dann ist die Absicht: Kunde will wissen, WANN und WO man sich treffen kann, und hat keine Lust, nur zu schreiben. Interpretation EXPLIZIT so ausdruecken, z.B.: "Kunde fragt, wann und wo man sich treffen kann, und hat keine Lust, nur im Chat zu schreiben." oder "Er will ein reales Treffen, fragt implizit wann/wo, und ist genervt vom nur-Schreiben." – NICHT nur als "Frustration" oder "Unzufriedenheit mit der Kommunikation" interpretieren, sonst reagiert die Pipeline mit Chat-Tipps statt mit Treffen-Antwort (noch nicht wissen wann, was wuerdest du gern machen).
...
const SITUATION_DEFINITIONS_LLM = {
  ...
  'Telefonat/Videoanruf': 'Kunde schlaegt oder fragt nach einem Telefonat, Videoanruf oder "Kamera beim Telefonieren" – z.B. telefonieren, wann passt es zum Telefonieren, Videoanruf, Kamera beim Telefon, mal anrufen, dich anrufen. Antwort darf KEIN Telefonat/Videoanruf vereinbaren; freundlich auf Chat verweisen.'
};
...
- Wenn der Kunde gerade weg muss (Termin, Arbeit, Familie, unterwegs) und dann "später/nachher/heute Nachmittag weiter machen" sagt, ist das sehr wahrscheinlich "später weiter schreiben/weiter chatten" → dann NICHT Treffen/Termine.
...
async function detectCustomerIntentWithLLM(customerMessage, conversationHistorySnippet = '') {
  ...
  const messages = [
    {
      role: 'system',
      content: `Du liest die letzte Kundennachricht in einem Dating/Flirt-Chat und musst die INTENTION verstehen.
...
Definitionen:
- chat_continue: Kunde meint weiter schreiben/weiter chatten/weiter reden im Chat (auch wenn er "schreiben" nicht wörtlich sagt), typischer Kontext: er muss kurz weg (Termin/Arbeit/Familie/unterwegs) und will später weiter machen.
- availability_timing: Kunde spricht über Zeit/Verfügbarkeit/Timing allgemein (z.B. "später", "heute Nachmittag", "nachher"), ohne klar zu sagen ob Chat oder Treffen gemeint ist. Das ist oft mehrdeutig.
...
async function runPlanningStep(customerMessage, detectedSituations, allRules, conversationHistory = '', alreadyIntroducedSelfInChat = false, currentMessageNotClearlySexual = false, currentMessageHasImage = false, customerClaimsWeAgreedOrWantsAddress = false, lastPreviousCustomerMessageAgeMs = null, customerOccupation = null, isPureGreetingFromInterpreter = null, fakeGenderForPlan = null, customerGenderForPlan = null) {
  ...
  const phoneVideoHint = hasTelefonVideo
    ? ' TELEFONAT/VIDEOANRUF (PLATTFORM-REGEL): Der Kunde fragt/schlaegt Telefonat oder Videoanruf vor (z.B. Kamera beim Telefon, wann telefonieren). Plan: Antwort darf KEIN Telefonat/Video vereinbaren – nicht "wann passt es zum Telefonieren" fragen, nicht zusagen. Auf andere Teile seiner Nachricht eingehen (z.B. Arbeitszeiten), freundlich auf Chat verweisen, keine Terminfrage fuer Anruf.'
    : '';
  const niemalsGetroffenPlanHint = ' NIEMALS GETROFFEN (immer): Kunde und Fake haben sich NIE gesehen und werden sich nie sehen. Plan und Antwort duerfen KEIN "wiedersehen", "dich endlich sehen", "in deinen Armen (sein)", "dich in die Arme nehmen" oder aehnliches – nichts, was ein reales Treffen oder koerperliche Begegnung andeutet. Vorfreude und Gefuehle nur im Chat-Kontext (z.B. "freue mich wenn du schreibst", "bin gern hier fuer dich"), keine Formulierung als ob ihr euch trefft oder getroffen haettet.';
...
/** Nachbearbeitung: Doppelpunkt → Komma, Anführungszeichen entfernen – so schreibt niemand im Chat. */
...
async function runMistralCorrector({ customerMessage = '', context = {}, grokText = '', learningContext = '', exampleSnippet = '', planSnippet = '', conversationSnippet = '' }) {
  ...
  if (context.telefonVideoRequest === true) ctx.push('Kunde fragt/schlaegt Telefonat oder Videoanruf vor – KEIN Telefonat/Video vereinbaren, freundlich auf Chat verweisen');
  ...
  const frageUndHypotheseRuleMistral =
    ...
    + (kundeThematisiertExternenKontakt
      ...
      : '')
    + ' WIDERSPRUCH KONTAKT: In EINER Nachricht "keine Nummer"/"kein Social"/"Instagram nicht" zusammen mit "schreib mir auf Telegram/WhatsApp" oder "du kannst mir auf Telegram"? Wenn ja → Telegram/WhatsApp-Angebot entfernen, nur hier Chat.'
  ...
  const conversationBlock = (conversationSnippet && conversationSnippet.trim()) ? `Chat-Verlauf (nur Kontext – die Nachricht, auf die du antworten musst, steht unten unter "AKTUELLE KUNDENNACHRICHT"):\n${sanitizeForApiContent(conversationSnippet.trim().slice(-450))}\n\n` : '';
  ...
  const noDoppelpunktAnfuehrungszeichenRule = ' PFLICHT STIL: Enthaelt die Antwort Doppelpunkt (:) oder Anführungszeichen (" „ " « »)? In Chat schreibt niemand so. Wenn ja → Doppelpunkt durch Komma ersetzen (z.B. "hör zu," statt "hör zu:"), alle Anführungszeichen entfernen.';
  ...
  const noWiedersehenRule = ' PFLICHT: Enthaelt die Antwort "wiedersehen", "wiederzusehen", "mal wieder treffen", "wenn wir uns wiedersehen" ODER "nächstes Mal sehen", "wenn wir uns das nächste Mal sehen", "beim nächsten Mal" (im Treffen-Kontext) ODER "dich endlich sehen", "dich endlich zu sehen", "in deinen Armen", "in deinen Armen sein", "dich in die Arme nehmen", "freue mich dich endlich zu sehen"? Ihr habt euch NIE getroffen – nichts andeuten, was ein reales Treffen oder koerperliche Begegnung voraussetzt. Wenn ja → ersetzen durch "wenn wir uns treffen wuerden", "wenn wir uns (mal) sehen wuerden", "wenn es so weit ist", "was wir zusammen machen koennten" bzw. Vorfreude nur im Chat (z.B. "freue mich wenn du schreibst", "bin gern hier fuer dich") – ohne "wieder", "nächstes Mal", "endlich sehen", "in deinen Armen".';
  ...
  const noTelefonsexPhotoRule = ' PFLICHT: Enthaelt die Antwort Vorschlaege wie Telefonsex, heisse Fotos schicken, Bilder als Loesung/Alternative (z.B. "mit Telefonsex und heissen Fotos", "Fotos vorweg")? Telefonsex ist auf der Plattform nicht moeglich – klar ablehnen. Bei reiner Foto-Anfrage: keine generelle Absage ("ich schicke keine Bilder"). Wenn ja → entfernen und durch andere Argumente ersetzen (Fantasie, Vorstellungskraft, Kennenlernen im Chat) bzw. bei Foto-Anfrage durch Ausrede (gerade nicht, Kamera/Handy, schau mal ob was Gutes da ist) – ohne Telefonsex oder Foto-Tausch anzubieten.';
  ...
  const noTelefonVideoVereinbarenRuleMistral = (context.telefonVideoRequest === true)
    ? ' TELEFONAT/VIDEOANRUF – PLATTFORM-REGEL: Der Kunde hat Telefonat oder Videoanruf (z.B. Kamera beim Telefon) vorgeschlagen/gefragt. Enthaelt die Antwort eine Zusage oder Terminfrage dafuer (z.B. "wann passt es zum Telefonieren?", "lass uns mal telefonieren", "Kamera beim Telefon klingt gut", "zeig mir was Echtes beim Telefon", "wann koennen wir telefonieren", "passt gut dass du die Kamera anmachen willst")? Wenn ja → diesen Teil ENTFERNEN oder ersetzen: freundlich auf Chat verweisen (z.B. "lieber hier schreiben", "bin hier im Chat fuer dich"), KEINE Vereinbarung von Telefonat/Video, auf andere Teile seiner Nachricht eingehen (z.B. Arbeitszeiten).'
    : '';
...
async function runOpenAIFullCorrector({ customerMessage = '', context = {}, grokText = '', learningContext = '', exampleSnippet = '', planSnippet = '', conversationSnippet = '' }) {
  ...
  const wohnortKonsistenzRuleOpenAI = ' WOHNORT KONSISTENZ: Enthaelt der Chat-Verlauf eine Nachricht von dir (Persona), in der du einen Wohnort/Ort genannt hast (z.B. "ich bin aus X", "wohne in X", "komme aus X")? Enthaelt die aktuelle Antwort einen ANDEREN Ortsnamen (z.B. andere Stadt)? Wenn ja → diesen durch den bereits im Verlauf genannten Ort ersetzen oder die Ortsnennung entfernen. Niemals einen anderen Ort erfinden oder nennen.';
  ...
  const noStrongHopeRule = ' Starke Vorfreude oder Formulierung wie feste Treffen-Zusage (z.B. "freue mich schon auf unser Treffen")? → zurueckhaltender und unverbindlicher umformulieren, wunschhaft/hypothetisch, variieren.';
  const noWiedersehenRule = ' PFLICHT: "wiedersehen", "wiederzusehen", "wenn wir uns wiedersehen" ODER "nächstes Mal sehen", "wenn wir uns das nächste Mal sehen", "beim nächsten Mal" (Treffen-Kontext) ODER "dich endlich sehen", "dich endlich zu sehen", "in deinen Armen", "dich in die Arme nehmen", "freue mich dich endlich zu sehen" → ersetzen durch "wenn wir uns treffen wuerden", "wenn wir uns (mal) sehen wuerden", "wenn es so weit ist" bzw. Vorfreude nur im Chat (z.B. "freue mich wenn du schreibst"); ihr habt euch NIE getroffen, keine Formulierung fuer reales Treffen/koerperliche Begegnung.';
