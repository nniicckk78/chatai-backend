...
- Stil: Nicht dieselbe Formulierung mehrfach hintereinander (z.B. „Ich bin mir sicher“ dreimal) – variieren, natürlich formulieren.
...
function buildNormalPrompt({
  ...
  systemContent += 'Antwort: Auf die ganze Nachricht eingehen – konkreten Inhalt (seine Frage, sein Thema, genannte Details) aufgreifen, nicht nur allgemein antworten. Mit Gegenfrage abschliessen. Mind. 120 Zeichen. Natuerlich und locker.';
  ...
  userContent += 'Die Antwort muss den Plan und den Inhalt der Kundennachricht umsetzen (nicht in anderes Thema springen). Ausgabe: Genau eine Nachricht (nur der Text, keine Erklaerungen, keine woertliche Wiederholung der Kundennachricht; mit Frage am Ende).';
...
async function runPlanningStep(customerMessage, detectedSituations, allRules, conversationHistory = '') {
  ...
  const contextBlock = contextSnippet
    ...
    : '';
  const userContent = `${contextBlock}Aktuelle Kundennachricht: "${sanitizeForApiContent(customerForPlan)}"\n\nErkannte Situation(en): ${situationList}.${contactHint}${sexualHint}${romanticHint}\n\nWas hat der Kunde konkret gesagt (Fragen, Aussagen, Themen)? Gib in 2–4 Sätzen an: Welche Regeln/Prioritäten gelten hier? Welcher Ton? Worauf muss die Antwort eingehen? Nenne alle Aspekte der Kundennachricht – die Antwort muss auf alle eingehen. Was unbedingt vermeiden? Nur den Plan, keine Antwort an den Kunden.`;
  const messages = [
    {
      role: 'system',
...
async function runMistralCorrector({ customerMessage = '', context = {}, grokText = '', learningContext = '', exampleSnippet = '', planSnippet = '', conversationSnippet = '' }) {
  ...
  const noRepetitionRule = ' Enthaelt die Antwort dieselbe Phrase oder denselben Satzanfang mehr als einmal hintereinander (z.B. "Ich bin mir sicher" dreimal)? Wenn ja → umformulieren, variieren, natürlich klingen lassen.';
