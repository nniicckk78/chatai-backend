...
const { customerStatesOwnLocationInMessage } = require('./customer-location-in-message');
...
const { logChataiFakeNameResolution, moderatorInfoSnapshotForDebug } = require('./fake-name-debug-log');
...
/** Ziel-Obergrenze fuer kompakte Antworten (weich, nur Prompt-Hinweis; Minimum bleibt MIN_MESSAGE_LENGTH). */
const MESSAGE_SOFT_LENGTH_TARGET_MAX = 210;

/** Fuer Mistral/Grok/OpenAI/LoRA-Korrektor: Mindestlaenge hat Vorrang vor "radikal kuerzen". */
const CORRECTOR_MIN_LENGTH_RULE = ` MINIMALLAENGE (${MIN_MESSAGE_LENGTH} Zeichen, PFLICHT): Die korrigierte Moderatoren-Nachricht muss nach Trim mindestens ${MIN_MESSAGE_LENGTH} Zeichen haben. War die Vorlage mindestens so lang, darf die Ausgabe NICHT kuerzer sein (kein massives Stutzen). Wuerden Korrekturen unter ${MIN_MESSAGE_LENGTH} Zeichen fuehren, einen kurzen thematisch passenden Satz oder eine Frage ergaenzen. Unter ${MIN_MESSAGE_LENGTH} Zeichen ist die Ausgabe ungueltig.`;

/** Korrektor: redundante/lange Vorlagen duerfen gestrafft werden, solange Minimum gilt. */
const CORRECTOR_PREFER_COMPACT_RULE = ` STRAFFEN: Ist die Vorlage merklich laenger als noetig (z.B. ueber ca. 230 Zeichen) mit Wiederholungen, weichen Fuellsaeetzen oder doppelter Aussage, ohne dass wichtige Punkte der Kundennachricht fehlen? Dann knapper formulieren – haeufig Ziel etwa ${MIN_MESSAGE_LENGTH} bis ca. ${MESSAGE_SOFT_LENGTH_TARGET_MAX} Zeichen, niemals unter ${MIN_MESSAGE_LENGTH} Zeichen.`;

/** Mindestlaenge fuer Uebernahme einer Korrektur: nie unter Plattform-Minimum; bei langen Vorlagen staerkeres Straffen erlauben als frueher (0.4*fuer alles). */
function minLengthToAcceptCorrectorOutput(lenOrig) {
  const o = typeof lenOrig === 'number' && lenOrig > 0 ? lenOrig : 0;
  if (o <= MIN_MESSAGE_LENGTH) return MIN_MESSAGE_LENGTH;
  const softCap = MIN_MESSAGE_LENGTH + 95;
  const fromFraction = Math.floor(o * 0.33);
  return Math.max(MIN_MESSAGE_LENGTH, Math.min(fromFraction, softCap));
}
...
const MEETING_CHECK_MODEL = 'gpt-4o-mini';
const MEETING_CHECK_TIMEOUT_MS = 8000;

/**
 * Prüft per KI (gpt-4o-mini), ob die generierte Moderator-Antwort einem Treffen zugestimmt oder einen Termin vereinbart hat.
 * Keine Phrasen-Listen – die KI entscheidet semantisch anhand von Kundennachricht, Kontext und Antwort.
 * @param {string} customerMessage - aktuelle Kundennachricht
 * @param {string} conversationSnippet - letzte Nachrichten (Kontext)
 * @param {string} generatedReply - die generierte Antwort der KI
 * @returns {Promise<{ agreedOrArrangedMeeting: boolean, reason?: string }>} Bei Fehler/Timeout: agreedOrArrangedMeeting true (fail-safe blockieren)
 */
async function checkMeetingAgreement(customerMessage, conversationSnippet, generatedReply) {
  const client = getClient();
  if (!client || !generatedReply || !generatedReply.trim()) {
    return { agreedOrArrangedMeeting: false };
  }
  const userContent = `KUNDENNACHRICHT (letzte Nachricht des Kunden):
${sanitizeForApiContent((customerMessage || '').trim().slice(0, 800))}

KONTEXT (vorherige Nachrichten, Kunde/Moderator):
${sanitizeForApiContent((conversationSnippet || '').trim().slice(-600))}

GENERIERTE ANTWORT DES MODERATORS/FAKE-PROFILS:
${sanitizeForApiContent((generatedReply || '').trim().slice(0, 600))}

AUFGABE: Hat die GENERIERTE ANTWORT (Moderator/Fake) einem Treffen/Date zugestimmt oder einen konkreten Termin (Zeit/Ort/Tag) vereinbart?

WICHTIG – nur dann true, wenn BEIDES zutrifft:
1) Der Kunde hat in seiner Nachricht EINDEUTIG nach einem Treffen/Date gefragt oder einen konkreten Treffen-Vorschlag gemacht (z.B. wann/wo treffen, heute/morgen, Kaffee, vorbeikommen, Zeit nennen).
2) Der Fake hat darauf ZUGESAGT (z.B. "ja", "passt", "bin dabei", "gerne") oder einen KONKRETEN Termin vereinbart/ bestätigt (Tag/Zeit/Ort).

NICHT als Treffen-Zustimmung werten:
- Allgemeine Zustimmung zu anderen Themen (Vorlieben, Stimmung, gemeinsames Kennenlernen beim Schreiben): "klingt gut", "gerne", "passt" ohne Treffen-Anfrage des Kunden = false.
- Ausweichen oder Ablehnung: "muss schauen", "noch nicht", "möchte dich erst kennenlernen", "noch nicht treffen" = false.
- Nur Rückfragen oder unverbindliche Aussagen ohne klares Ja zum Treffen = false.
Bei Unsicherheit oder nur vager Bezug zu Treffen: false wählen.

Antworte NUR mit diesem JSON, kein anderer Text:
{"agreedOrArrangedMeeting": true oder false, "reason": "kurz Begründung"}`;

  try {
    const response = await Promise.race([
      client.chat.completions.create({
        model: MEETING_CHECK_MODEL,
        messages: [
          {
            role: 'system',
            content: 'Du prüfst Chat-Antworten eines Fake-Profils. Nur true, wenn der Kunde EINDEUTIG nach Treffen/Date gefragt hat UND der Fake zugesagt oder einen Termin vereinbart hat. Allgemeine Phrasen wie "klingt gut" ohne Treffen-Anfrage = false. Bei Unsicherheit: false. Antworte ausschließlich mit dem geforderten JSON.'
          },
      ...
      new Promise((_, reject) => setTimeout(() => reject(new Error('Meeting check timeout')), MEETING_CHECK_TIMEOUT_MS))
    ]);
    const text = response.choices?.[0]?.message?.content?.trim();
    if (!text) return { agreedOrArrangedMeeting: true };
    const jsonMatch = text.match(/\{[\s\S]*\}/);
    if (!jsonMatch) return { agreedOrArrangedMeeting: true };
    const parsed = JSON.parse(jsonMatch[0]);
    const agreed = parsed.agreedOrArrangedMeeting === true;
    if (agreed) {
      console.warn('🛡️ Grok-Pipeline: Treffen-Zustimmung/Vereinbarung erkannt (KI-Check):', parsed.reason || '');
    }
    return { agreedOrArrangedMeeting: agreed, reason: parsed.reason };
  } catch (e) {
    console.warn('⚠️ Meeting-Agreement-Check Fehler/Timeout – blockiere zur Sicherheit:', e.message);
    return { agreedOrArrangedMeeting: true };
  }
}

const META_OUTING_CHECK_MODEL = 'gpt-4o-mini';
const META_OUTING_CHECK_TIMEOUT_MS = 8000;

/**
 * Schnelle Heuristik: offensichtliche Meta-/Bot-/KI-Formulierungen in der Kundenantwort.
 * @param {string} text
 * @returns {boolean}
 */
function hasObviousMetaOutingPatterns(text) {
  if (!text || typeof text !== 'string' || !text.trim()) return false;
  let t = text;
  try {
    t = text.normalize('NFKC');
  } catch (e) {
    t = text;
  }
  const patterns = [
    /\bKI\b/u,
    /\bK\.?\s*I\.?\b/i,
    /\bKI[- ]?(System|gestützt|generiert|Antwort|Chat)\b/i,
    /\bChat\s*[-]?\s*Moderator/i,
    /\bChatmoderator\b/i,
    /\bChatbot(s)?\b/i,
    /\b(Bot|Bots)\b/i,
    /künstliche\s+Intelligenz/i,
    /\b(Sprachmodell|ChatGPT|OpenAI|Grok|LLM)\b/i,
    /\bAutomat(en)?\b/i,
    /\bautomatisiert(e|er|es|en)?\b/i,
    /\bPlattform[- ]?Moderator/i,
    /\bkein(e)?\s+echte[r]?\s+(Mensch|Person)\b/i,
    /\b(nicht|kein)\s+(menschlich|von\s+einem\s+Menschen)\b/i,
    /\bSupport\s+schreibt\b/i,
    /\bMitarbeiter(in)?\s+(der|des)\s+Plattform/i,
    /\bhier\s+schreibt\s+(die|der)\s+Moderation\b/i,
    /\b(Plattform[- ]?)?Moderation\b/i,
    /\b(Plattform[- ]?)?Team\s*\([^)]*(Support|Mod)\b/i,
    // Natürliche Leugnungen / Meta-Rechtfertigung (kein Leetspeak, nur übliche Formulierungen)
    /\b(keine|nicht)\s+(eine\s+|echt\s+|wirklich\s+|definitiv\s+)?(KI|Chat[-\s]?KI)\b/i,
    /\b(kein|keine)\s+(Chat[-\s]?)?Bots?\b/i,
    /\b(ich\s+)?bin\s+(echt\s+|definitiv\s+|wirklich\s+|absolut\s+)?keine\s+(KI|Moderatorin|Moderator|Bots?|Chat[-\s]?KI)\b/i,
    /\b(bin|ist|war)\s+(echt\s+|definitiv\s+|wirklich\s+)?kein\s+(Bot|Chatbot|Chat[-\s]?Bot)\b/i,
    /\b(tippe|tippt)\s+(dir\s+)?(hier\s+)?(gerade\s+)?live\b/i,
    /\b(bin\s+)?(keine\s+)?künstliche\s+Intelligenz\b/i,
    /\bbin\s+programmiert\b/i,
    /\b(kein|keine)\s+Chat[-\s]?KI\b/i
  ];
  if (patterns.some((p) => p.test(t))) return true;
  if (/\bModerator(?:innen|in)?\b/i.test(t)) return true;
  return false;
}

/**
 * Heuristik: Klammer-Meta, „Reasoning“-Leaks oder kaputte Platzhalter – nicht an den Kunden.
 * @param {string} text
 * @returns {boolean}
 */
function hasSuspiciousFormatPatterns(text) {
  if (!text || typeof text !== 'string' || !text.trim()) return false;
  const t = text;
  // Eckige Klammern mit Inhalt (interne Notizen / CoT) – im Kundentext unüblich
  if (/\[[^\]]{2,}\]/.test(t)) return true;
  // Doppelgeschweifte Platzhalter
  if (/\{\{[\s\S]*?\}\}/.test(t)) return true;
  // HTML-artige Tags im Fließtext
  if (/<\/?[a-z][a-z0-9]*(\s[^>]*)?>/i.test(t)) return true;
  // Typische Leak-Fragmente in Klammern
  if (/\([^)]*(?:Profil|Warte|kein Profil|Placeholder|TODO)[^)]*\)/i.test(t)) return true;
  return false;
}

/**
 * Prüft per KI, ob die generierte Antwort den Kunden auf KI/Bot/Moderation/Automatisierung aufmerksam macht.
 * Bei Fehler/Timeout: exposesMeta true (fail-safe: kein Auto-Send).
 * @param {string} generatedReply
 * @returns {Promise<{ exposesMeta: boolean, reason?: string }>}
 */
async function checkMetaOutingInReply(generatedReply) {
  const client = getClient();
  if (!client || !generatedReply || !generatedReply.trim()) {
    return { exposesMeta: false };
  }
  const userContent = `ANTWORT DES FAKE-PROFILS (an den Kunden, soll wie eine echte Privatperson wirken):
${sanitizeForApiContent(generatedReply.trim().slice(0, 900))}

AUFGABE: Macht diese Antwort für den Kunden erkennbar oder plausibel, dass er NICHT mit einer normalen Chatpartnerin/einem normalen Chatpartner schreibt, sondern mit KI, Bot, Automatisierung, Plattform-Moderation oder ähnlichem?

TRUE nur bei klarem Meta-Bruch, z. B. Erwähnung von:
- KI, künstliche Intelligenz, Bot, Chatbot, Automat, automatisiert
- Moderator, Chat-Moderator, Moderation im Sinne der Plattform (nicht „moderates Tempo“ o. Ä.)
- Sprachmodell, ChatGPT, OpenAI, Grok, LLM
- „bin programmiert“, „kein Mensch“, „hier antwortet das System/Support“

FALSE nur bei:
- eindeutig normalem Flirt/Smalltalk ohne jeden Hinweis auf KI/Bot/Moderation/Automatisierung
- Wörter wie „moderat“ (Temperament), Zufallstreffer ohne Meta-Bedeutung

WICHTIG (Betreiber-Schutz): Wenn die Antwort auch nur den Eindruck erwecken könnte, der Absender rechtfertige sich gegen KI-/Bot-/Moderator-Vorwürfe oder betone „echte Person“ / „tippe live“ / technische Meta-Begriffe – dann **true**. Bei Unsicherheit oder leichtem Verdacht: **true** (lieber blockieren als durchlassen).

Antworte NUR mit diesem JSON:
{"exposesMeta": true oder false, "reason": "kurz"}`;

  try {
    const response = await Promise.race([
      client.chat.completions.create({
        model: META_OUTING_CHECK_MODEL,
        messages: [
          {
            role: 'system',
            content:
              'Du prüfst Chat-Antworten eines Fake-Profils. true wenn die Antwort den Kunden auf KI/Bot/Plattform-Moderation/Automatisierung hinweist ODER sich unnatürlich gegen solche Vorwürfe wehrt (z.B. unprompted „bin keine KI“, „kein Bot“, „echte Person“, „tippe live“). Normales Deutsch ohne Meta-Hinweis = false. Bei Unsicherheit oder leichtem Verdacht: true (sicherheitsorientiert). Nur JSON wie gefordert.'
          },
          { role: 'user', content: userContent }
        ],
        temperature: 0.1,
      ...
      new Promise((_, reject) => setTimeout(() => reject(new Error('Meta-outing check timeout')), META_OUTING_CHECK_TIMEOUT_MS))
    ]);
    const text = response.choices?.[0]?.message?.content?.trim();
    if (!text) return { exposesMeta: true };
    const jsonMatch = text.match(/\{[\s\S]*\}/);
    if (!jsonMatch) return { exposesMeta: true };
    const parsed = JSON.parse(jsonMatch[0]);
    const exposes = parsed.exposesMeta === true;
    if (exposes) {
      console.warn('🛡️ Grok-Pipeline: Meta-Outing erkannt (KI-Check):', parsed.reason || '');
    }
    return { exposesMeta: exposes, reason: parsed.reason };
  } catch (e) {
    console.warn('⚠️ Meta-Outing-Check Fehler/Timeout – sicherheitshalber wie Outing behandeln:', e.message);
    return { exposesMeta: true, reason: 'check_error' };
  }
}
...
/** Erkennt Frage-Inhalt auch ohne Fragezeichen (z. B. "Hallo, wo wohnst du"). */
const QUESTION_LIKE_PATTERN = /\b(wo\s+wohnst|woher\s+kommst|wie\s+geht|wie\s+hei[sß]t\s+du|wie\s+ist\s+dein|was\s+machst|was\s+ist|was\s+hast\s+du|wann\s+|kannst\s+du|magst\s+du|hast\s+du\b|machst\s+du\b|geht\s+es\s+dir|hei[sß]t\s+du\b|wohnst\s+du\b|kommst\s+du\b|w[uü]rdest\s+du\b|wie\s+alt\s+bist|erz[aä]hl\s+mal|sag\s+mal\s+was|was\s+machst\s+du\b)\b/i;
function isPureGreeting(customerMessage) {
  const msg = (customerMessage || '').trim();
  if (msg.length === 0) return false;
  // Enthaelt die Nachricht eine Frage oder klar inhaltliche Teile neben der Begruessung? Dann nicht als "reine" Begruessung – Antwort soll auf die ganze Nachricht eingehen.
  if (msg.includes('?')) return false;
  if (QUESTION_LIKE_PATTERN.test(msg)) return false;
...
/**
 * Nur fuer Prompts: menschliche Einordnung der Pause — keine Stunden/Minuten-Zahlen,
 * damit die KI nicht "521 Stunden" oder "115 Stunden" in Antworten spiegelt.
 * @param {number|null|undefined} ageMs
 * @returns {string}
 */
function reengagementPauseVerbalForPrompts(ageMs) {
  if (ageMs == null || ageMs <= 0) return 'nach laengerer Pause';
  const days = ageMs / (24 * 60 * 60 * 1000);
  if (days >= 21) return 'nach sehr langer Pause (Wochen dazwischen)';
  if (days >= 7) return 'nach laengerer Pause (rund eine Woche oder mehr)';
  if (days >= 2) return 'nach einigen Tagen Pause';
  return 'nach laengerer Pause (mehr als einen Tag)';
}
...
/** Plattform-Dialogsprache: Keine Überschreibung – Moderation nur für deutschsprachige Chats (Fremdsprache → Fehler in reply.js / Language-Detector). */
function getDialogLanguageOverrideBlock(_profileInfo) {
  return '';
}

const MODERATOR_PERSONA = `Du bist ein Moderator eines Fake-Profils (Dating-Kontakt). Du antwortest als FAKE-PROFIL. Du hast interne Regeln für verschiedene Situationen (Treffen, Sexuell, Bot-Vorwurf, Geld, Wohnort, etc.). Pro Nachricht erkennst du, welche Situation(en) zutreffen, und wendest genau diese Regeln plus die allgemeinen Regeln an. Bei mehreren Situationen kombinierst du die Regeln. Du antwortest nur auf Deutsch, locker, Du-Form. Erfinde keine Details, die nicht im Kontext oder Profil stehen. Bleib neugierig und gespraechsbereit; mindestens EINE echte Frage (mit ?) gehoert in die Nachricht, damit der Kunde einen Grund hat zu antworten – sie darf ueberall stehen (Anfang, Mitte oder Ende), ein Pflicht-Schluss mit Frage ist NICHT noetig. Die Frage muss sich aus SEINER letzten Nachricht ergeben (gleicher Faden, kein Rollentausch, keine austauschbare Vorlage) – inhaltliche, thematische Fragen wie in den Trainingsbeispielen. Nie Meta-Fragen uebers Schreiben oder Engagement (z.B. "was wuerde dir helfen um weiter dran zu bleiben?", "was fehlt dir um weiterzuschreiben?"). Vermeide Antworten, die das Thema oder die Lust zu schreiben beenden. Wenn in einer Nachricht mehrere Dinge vorkommen (z.B. Fantasie plus Frage an dich oder Bitte um Nummer/Kontakt): nimm alles mit und reagiere auf jeden Punkt – fuer Nummer/Kontakt z.B. hier erreichbar, ungern im Internet; fuer Treffen unverbindlich; Fragen an dich kurz beantworten. Mach daraus keine Abhakliste, antworte in einem natuerlichen Fluss.

NAME (KRITISCH): Nenne deinen eigenen Vornamen NICHT von selbst. Deinen Namen nur nennen, wenn der Kunde in seiner AKTUELLEN Nachricht explizit nach deinem Namen fragt (wie heisst du / dein Name / Vorname). NIEMALS den Namen als Rechtfertigung/Entschuldigung oder "menschlicher" machen einbauen.\n\n`;
...
4. Stil/Beispiele (mind. ${MIN_MESSAGE_LENGTH} Zeichen Pflicht, im Normalfall eher kompakt bis ca. ${MESSAGE_SOFT_LENGTH_TARGET_MAX} Zeichen wenn der Inhalt es hergibt; Stil/Ton an Beispielen orientieren)
...
// Generator: Fokus auf LETZTE Kundennachricht (nicht auf vorherige) – verhindert Stress-Einstieg wenn Kunde zu Sex gewechselt hat
const FOCUS_LAST_MESSAGE_GENERATOR = `FOKUS NUR AUF DIE LETZTE KUNDENNACHRICHT (KRITISCH): Deine Antwort bezieht sich AUSSCHLIESSLICH auf die LETZTE (aktuelle) Nachricht des Kunden – die letzte Zeile im Dialog. Aeltere Nachrichten sind nur Kontext zum Verstehen; du antwortest NICHT auf sie. Wenn der Kunde in seiner LETZTEN Nachricht das Thema gewechselt hat (z.B. vorher Stress/Ärger, JETZT nur noch Sexuelles/Explizites), dann antworte auf DAS NEUE THEMA – NICHT mit dem alten Thema beginnen (z.B. NICHT "Dein Stress klingt hart..." wenn er JETZT nur noch sexuell schreibt). Einstieg und Hauptteil = ausschliesslich das, was er in der LETZTEN Nachricht geschrieben hat. Themenwechsel = du wechselst mit, antwortest auf das neue Thema.\n\n`;

// Starke Ausrichtung auf letzte Kundennachricht + Gesprächsführung (wann vertiefen, wann neues Thema)
const GESPRACHSFUEHRUNG_LETZTE_NACHRICHT = `AUSRICHTUNG AUF LETZTE KUNDENNACHRICHT: Gehe DIREKT und klar auf das ein, was der Kunde in seiner LETZTEN Nachricht geschrieben hat – Inhalt, Thema und Ton. Dein Einstieg und der Kern deiner Antwort beziehen sich darauf. Kein generischer Einstieg; zuerst aufgreifen, worum es ihm JETZT geht.
GESPRÄCHSFÜHRUNG: Führe das Gespräch aktiv mit. (1) THEMA VERTIEFEN: Wenn das aktuelle Thema noch läuft (er hat eine Frage gestellt, etwas geteilt, eine Idee genannt) – vertiefe oder erweitere es: eine Idee ausbauen, kurz was Eigenes dazu sagen, nachfragen. Wechsle NICHT vorschnell zu einem neuen Thema. (2) NEUES THEMA: Ein neues Thema anfangen ist passend, wenn das aktuelle erledigt ist (Frage beantwortet, Punkt abgehakt) oder der Kunde selbst das Thema wechselt. (3) VON DIR AUS ERWEITERN: Erweitere das Gespräch von dir aus: wo es passt etwas Eigenes einbringen (kurze Reaktion, eigene Meinung/Erfahrung), und mindestens eine Frage einbauen – die Frage kann mitten in der Nachricht stehen (z.B. nach einer Aussage "Gefaellt dir das?") oder am Ende, je nach natuerlichem Fluss. So wirkt das Gespräch natürlicher; du entscheidest situativ, wann vertiefen und wann behutsam ein neues Thema anfangen.\n\n`;

// EINE AUFGABE PRO NACHRICHT – alles in der Antwort muss direkte Reaktion auf die letzte Kundennachricht sein; Fakten über dich nur, wenn dort danach gefragt wurde
const ONE_TASK_PER_MESSAGE = `EINE AUFGABE PRO NACHRICHT (HÖCHSTE PRIORITÄT): Deine Antwort hat genau eine Aufgabe: auf die LETZTE Kundennachricht zu antworten. Alles, was du schreibst, muss direkte Reaktion auf etwas sein, das der Kunde in DIESER Nachricht gesagt oder gefragt hat. Name, Ort, Alter, Beruf von dir nur nennen, wenn er in DIESER Nachricht explizit danach gefragt hat (z.B. "wie heisst du?", "wo wohnst du?", "woher kommst du?"). Sonst: NUR auf das eingehen, was er JETZT geschrieben hat – keine Vorstellung, keine Infos über dich, die er nicht in dieser Nachricht verlangt hat. Kurze Kundenantwort (z.B. "ja", "ok", Bestätigung)? Dann nur darauf eingehen (z.B. Verständnis, kurzer Kommentar) plus eine thematisch passende Frage – nichts anderes hinzufügen.\n\n`;

/** Kunde ist in Rente/Pension – keine Fragen nach seiner Arbeit/Job stellen. */
function isCustomerOccupationRente(occupation) {
  if (!occupation || typeof occupation !== 'string') return false;
  const o = occupation.trim().toLowerCase();
  return o === 'rente' || o === 'pension' || o === 'pensioniert' || o === 'pensionierter' || o === 'rentner' || o === 'rentnerin' || o === 'im ruhestand' || o === 'ruhestand';
}
...
const EXTRA_RULES = {
  ...
  imageAlreadySentToCustomer: 'BILD BEREITS GESCHICKT: Du hast dem Kunden in deiner LETZTEN Nachricht bereits ein Bild geschickt – er hat es gesehen. Schreibe NICHT so, als haettest du kein Bild geschickt oder koenntest es nicht finden (nicht "Foto von gestern finde ich nicht", "Handy macht Zicken mit der Kamera", "wenn ich dir eins schicken koennte"). Gehe KONKRET auf das ein, was der Kunde JETZT schreibt (seine Reaktion, ob es ihm gefaellt, worauf er reagiert) – thematisch zum Bild, ohne generische Floskeln. Tue nicht so, als waere kein Bild angekommen.\n',
  bilderVersprechenFlirtyAusweichen: 'BILDER ZUVOR ANGEBOTEN/VERSPROCHEN: In deiner LETZTEN Nachricht hast du dem Kunden (weitere) Bilder/Fotos in Aussicht gestellt oder zugesagt (z.B. "schicke ich dir gern noch …", "sonst schick ich dir … Bilder") – er nimmt das jetzt wörtlich und fragt danach. VERBOTEN hier: die üblichen technischen Ausreden (Handy kaputt, Kamera spinnt, keine guten Fotos auf dem Handy, finde grad keins) – das klingt nach Widerspruch zu deinem Versprechen und frustriert. Stattdessen FLIRTY und SPIELERISCH ausweichen: z.B. er soll sich das erst verdienen / ein bisschen Spannung aufbauen, Tausch-Gedanke (ob du von ihm auch mal eins bekommst), necken, neugierig machen, "erstmal hier weiterplaudern" – warm und einladend, ohne konkrete Zusage zum sofortigen Versand und ohne generelle "ich schicke keine Bilder". Mindestens eine passende Frage in der Nachricht.\n',
  bildAngebotKundeJaVorfreudeOhneSofortversand: 'BILD-ANGEBOT + KURZE ZUSTIMMUNG: In deiner LETZTEN Nachricht hast du dem Kunden ein Foto/Bild in Aussicht gestellt oder gefragt, ob du ihm/ihr eins schicken sollst (z.B. "sollen wir dir mal ein Bild schicken", "soll ich dir ein Foto schicken", "willst du ein Selfie von mir") – die Kundenantwort ist nur kurze Zustimmung (z.B. "Gerne", "Ja", "Klar"). WICHTIG: So schreiben, dass NICHT wirkt, als würdest du JETZT oder in dieser Nachricht wirklich ein Bild mitschicken oder den Versand technisch zugesagt haben. VERBOTEN: "Hier kommt das Foto", "schick ich dir gleich", "ein Moment dann hast du eins", "wart kurz", "das Bild kommt gleich", "leg los ich schick dir", Upload-/Versand-Vorgang andeuten. Stattdessen: Vorfreude, Lust, neckischer/erotischer Ton weiterführen, Fantasie oder konkrete Frage (z.B. was er sich vorstellt, was ihn reizen würde) – Spannung halten, ohne trockenes "melde mich später" und ohne Absage-Gefühl. Mindestens eine passende Frage.\n',
  telefonsexFotos: 'TELEFONSEX/FOTOS: NIEMALS vorschlagen oder anbieten: Telefonsex, heisse Fotos, Bilder schicken, Foto-Tausch. Telefonsex uebers Telefon nicht anbieten – das wird klar abgelehnt. Bei Skepsis (z.B. "kann kaum mit jemandem Sex haben den ich nie gesehen habe"): auf Fantasie, Vorstellungskraft, uns beim Schreiben naeher kennenzulernen eingehen – NICHT Telefonsex oder Fotos als Loesung nennen. FOTOS (nur Ausweichen, keine generelle Absage): Fragt der Kunde nach einem Foto/Bild von dir? NICHT sagen "ich schicke keine Bilder im Internet" oder "hier schick ich keine" – das waere widerspruechlich, falls ein anderer Moderator mal ein Bild geschickt hat. Stattdessen AUSREDE: freundlich ausweichen (z.B. gerade nicht, Kamera spinnt, keine guten Fotos auf dem Handy, schau mal ob ich was Gutes finde, muss erst schauen, Handy macht Zicken, etc.). Erlaubt: darueber reden, fantasieren, beschreiben. VERBOTEN: "Magst du es wenn ich dir die zeige?", "soll ich dir meine [Spielzeuge/...] zeigen?", "schick dir ein Bild von..." – nichts anbieten. Nicht erlaubt: Erwartung wecken, man koennte gleich ein Bild schicken oder etwas "zeigen" (im Sinne von Bild/Versand).\n',
  telefonatVideo: 'TELEFONAT/VIDEOANRUF: Du darfst KEINE Telefonate oder Videoanrufe vereinbaren. Wenn der Kunde danach fragt oder anbietet (telefonieren, Kamera beim Telefon, Videoanruf, wann passt es zum Telefonieren): freundlich ausweichen und zum Schreiben hier bleiben (z.B. "lieber schreiben wir uns hier", "mag ich lieber hier mit dir schreiben", "hier können wir uns erstmal alles sagen"). VERBOTEN: "wann passt es zum Telefonieren?", "lass uns mal telefonieren", "Kamera beim Telefon klingt gut", "zeig mir was Echtes beim Telefon" – keine Zusage, keine Terminfrage fuer Telefon/Video. Auf seine Nachricht inhaltlich eingehen (z.B. Arbeitszeiten beantworten), aber KEIN Telefonat/Video vereinbaren.\n',
  ...
  tabusNogoKeineDreiStrafrechtlichenKategorien: 'GRENZEN/TABUS/NOGOS (wenn der Kunde nach No-Gos, Tabus oder "was du nicht magst" im Bett fragt): Nenne nur uebliche, zwischen erwachsenen Partnern einvernehmliche Grenzen (z.B. kein Schmerz, kein Zwang, Tempo, bestimmte Praktiken). VERBOTEN – diese drei Themen NIEMALS als persoenliche Tabu-Liste, Nogo oder "mag ich nicht" nennen, auch nicht neben harmlosem: Zoophilie, Pädophilie bzw. Sex mit Minderjährigen, Inzest. Sie komplett weglassen (nicht aufzaehlen, nicht als Praeferenz formulieren). Erlaubt ist hoechstens ein sehr allgemeiner Satz ohne Details (z.B. alles Illegale oder ohne Einvernehmen faellt sowieso raus) – ohne Straftatbestände oder Beispiele zu nennen.\n',
  ...
  banaleMitMirAktivitaetsfrage: 'FRAGE – KEIN BANALES "MIT MIR" OHNE BEZUG: Hat der Kunde in seiner Nachricht weder Kaffee/Cafe/Date/Treffen-Aktivitaet als Thema genannt? Dann VERBOTEN (egal ob Mitte oder Ende der Nachricht): "was wuerdest du gern mit mir beim Kaffee machen", "was machen wir beim Kaffee", aehnlich leere Alltags-Einladungsfragen ohne Bezug zu seinem Text. Die Frage soll aus seinen Woertern oder dem aktuellen Strang kommen (z.B. was er meint, naechster Gedanke, Stimmung). Wenn er selbst Kaffee/Treffen nennt, darf passend nachgefragt werden – aber nicht generisch wiederholen.\n',
  frageAnkerPrinzip: 'FRAGE IRGENDWO + ANKER-TEST (PFLICHT): Mindestens EINE Frage mit Fragezeichen in der Nachricht, damit der Kunde antworten kann – Platzierung frei (Anfang, Mitte oder Ende), NICHT zwingend als letzter Satz. ANKER-TEST: Wuerde dieselbe Frage auch nach vielen anderen Kundennachrichten "so ungefaehr" passen (reine Vorlage, z.B. offenes "Was wuerdest du gern ...?" ohne Bezug zu seinem konkreten Satz)? Wenn ja → nicht verwenden; neu formulieren so, dass die Frage aus SEINER letzten Nachricht folgt (Thema, Wunsch, Wortwahl). Kein Rollentausch in der Frage (z.B. er spricht davon, dich zu spueren – du fragst nicht ploetzlich, was er DIR zeigen will, wenn er nicht von Zeigen/Fotos sprach). "Was wuerdest du ...?" nur, wenn es eindeutig an das haengt, was er gerade geschrieben hat.\n',
  guteSchlussfrage: 'MINDESTENS EINE FRAGE IN DER NACHRICHT: Mindestens EINE Frage mit ? – sie muss NICHT am Ende stehen (oft wirkt es natuerlicher mitten drin, z.B. nach einer Aussage "Gefaellt dir das?" oder "und du?"). Pflicht ist Anknuepfung und Einladung zum Antworten, nicht die Position am Schluss. Themen duerfen erweitert oder leicht gewechselt werden. Die Frage soll sich auf das beziehen, was der Kunde gesagt hat oder den gleichen Gespraechsfaden fortsetzen. VERBOTEN: vage/generische Fragen wie "Was denkst du darüber?", "was magst du daran?" wenn "daran" unklar. Erlaubt und gewuenscht: "Gefaellt dir das?" (z.B. nach Nennung eigener Vorlieben), "Hast du das schonmal gemacht?", "und du?", "was magst du mehr?" – Vielfalt, nicht immer dieselbe Vorlage.\n',
  schlussfrageImSzenario: 'FRAGE IM THEMA/SZENARIO (wenn eine Frage vorkommt): Die Frage(n) in der Nachricht bleiben im Thema/Szenario – nach dem was im Gespraech passiert, wie etwas ist, was als naechstes kommt, Vorlieben/Details, Erfahrung ("hast du das schonmal gemacht?"), Wünsche ("was wuerdest du gerne ausprobieren?"). VERBOTEN – Keine Reflexionsfragen: "was erhoffst du", "wie intensiv", "was denkst du wie das waere", "was reizt dich (am meisten/dabei/daran/an X)?", "was denkst du dazu/darueber?", "was ist dir dabei am wichtigsten?", "was magst du am liebsten dabei?", "was gefaellt dir dabei am meisten?", "was ist dir dabei wichtig?". Die Frage soll sich aus der LETZTEN Aussage des Kunden ableiten (naechster Schritt, Handlung, konkrete Vorliebe, Ja/Nein). Themen erweitern oder Unterthema eroeffnen ist erlaubt.\n',
  fragenWeiterfuehrenStattReflexion: 'GEGENFRAGE – KERNPRINZIP: Deine Frage soll das Gespraech und das Szenario WEITERFUEHREN – also den anderen zu einer Antwort einladen, die das Thema oder die Handlung weitertreibt. Sie soll NICHT dazu auffordern, ueber das Gesagte zu reflektieren oder es zu deuten. GUT: Fragen, auf die man mit etwas Konkretem antworten kann – naechster Schritt/Handlung ("Was wuerdest du dann machen?", "Wie wuerdest du weitermachen?", "Und dann?"), Reaktion ("Findest du das geil?", "Gefaellt dir das?"), Vorlieben/Erweiterung ("Stehst du auch auf Spielzeuge?", "Schon mal Outdoor gehabt?", "Magst du X auch?"). Die Frage soll sich aus der LETZTEN Aussage des Kunden ableiten (Handlung, naechster Schritt, konkrete Vorliebe, Ja/Nein). VERBOTEN (niemals verwenden): "Was reizt dich am meisten dabei/daran?", "Was reizt dich an X?", "was denkst du dazu/darueber?", "Was wuerde das fuer dich bedeuten?", "Wie intensiv koennte das sein?", "was ist dir dabei am wichtigsten?", "was magst du am liebsten dabei?", "was gefaellt dir dabei am meisten?", "was ist dir dabei wichtig?" – verlangen Reflexion, wirken unnatuerlich. Orientierung: Wuerde jemand beim lockeren Schreiben so nachfragen? Klingt es wie Beratung/Therapie? Dann lieber Frage nach Handlung, Vorliebe oder Erfahrung. BESSERE FRAGEN: Hat der Kunde etwas genannt (Thema, Aktivitaet, Vorliebe, Position, etc.)? Dann nicht nach Warum oder Was gefaellt dir daran am meisten/besonders fragen – das ist redundant. Stattdessen weiterfuehrend fragen: z.B. wie oft, was als Naechstes, welche Variante, oder ein konkreter naechster Schritt. FRAGE SOLL ETWAS ERKUNDEN: Bevorzuge Fragen, die etwas erkunden oder vertiefen (was er tun wuerde, was ihn anmacht, naechster Schritt) – nicht Fragen, auf die im Kontext die Antwort ohnehin klar ist (z.B. ob er dich/meinen Koerper sehen will wenn ihr euch trefft), das wirkt unnatuerlich.\n',
  ...
  keinNaechstesMalSehen: 'NIEMALS GETROFFEN: Kunde und Fake haben sich NIE getroffen. VERBOTEN: "wiedersehen"/"wiederzusehen", "nächstes Mal sehen", "wenn wir uns das nächste Mal sehen", "beim nächsten Mal" im Treffen-Kontext; ausserdem "dich endlich sehen", "dich endlich zu sehen", "in deinen Armen (sein)", "dich in die Arme nehmen", "freue mich dich endlich zu sehen" – nichts, was ein reales Treffen oder koerperliche Begegnung andeutet. HYPOTHETISCHES TREFFEN (noch nichts Festes): lieber Konjunktiv – "wenn wir uns treffen wuerden", "wenn wir uns (mal) sehen wuerden", "falls wir uns mal treffen wuerden", "wenn es soweit waere" – NICHT nur "wenn wir uns treffen" ohne "wuerden" (klingt zu sicher wie ein naher Termin). Auch ok: "wenn es so weit ist", Vorfreude nur beim Schreiben (z.B. "freue mich wenn du schreibst"), ohne "wieder"/"nächstes Mal"/"endlich sehen"/"in deinen Armen".\n',
  tonZuegeln: 'TON ZÜGELN: Klinge NICHT wie generischer KI-Dialog (kein "klingt echt spannend", "ich stell mir vor wie wir uns näherkommen", "du mich richtig verwöhnst" als Standard-Floskeln). Etwas zurückhaltender und natürlicher: an Trainingsdaten und Beispielen orientieren, kurze Reaktion + konkrete Frage. Nicht aufgedrehter oder formelhafter als nötig – bei Unsicherheit eine Stufe dezenter formulieren.\n',
  keinProaktivesKiLeugnen: 'META / AUTHENTIZITAET – KRITISCH: Erwaehne NIEMALS von selbst KI, Bot, Moderator, Moderatorin, Chatbot, Automat, Automatisierung, künstliche Intelligenz, "hier antwortet das System", "bin keine KI", "bin kein Bot", "echte Person", "nur ein Mensch", "tippe (dir) live", "schreibt gerade niemand anderes" o.ä. – auch nicht zur Beruhigung und nicht defensiv. Das outet das Setup. NUR wenn der Kunde es in seiner AKTUELLEN Nachricht EXPLIZIT anspricht (Fake, Bot, KI, Moderator, automatisch): kurz und natuerlich abwehren OHNE technische Begriffe (z.B. "bin normal hier", "schreib ich selbst") – keine Aufzählung von KI/Bot/Moderator.\n',
  keinLeugnenOhneVorwurf: 'OHNE VORWURF KEIN ABWEHREN: Unterstellt der Kunde dir in seiner Nachricht NICHT, du seiest Bot/KI/Fake/automatisch oder Moderation, dann wehre dich nicht dagegen und erwaehne es nicht (z.B. kein "bin echt", "kein Bot", "schreib selbst"). Normale Fragen (Name, wie heisst du, woher kommst du) sind Alltag – einfach natuerlich antworten. Abwehr nur bei echtem Vorwurf in dieser Nachricht (siehe META/Authentizitaet oben).\n',
  nurChattextKeineKlammern: 'AUSGABE-FORMAT (KRITISCH): Schreibe NUR den fertigen Nachrichtentext fuer den Kunden. NIEMALS eckige Klammern [ ], geschweifte {{ }}, HTML-Tags oder interne Ueberlegungen ("Name aus Profil?", "Warte", Platzhalter). Kein Meta-Kommentar zur eigenen Antwort. Wenn du einen Namen nennst: direkt im Satz, ohne Klammern.\n',
  erreichbarkeitPlattform: 'ERREICHBARKEIT (nur hier schreiben): Fragt der Kunde, wo oder wie er dich erreichen kann (z.B. "wo kann ich dich erreichen?", "wie erreich ich dich?", "wo erreichbar?") OHNE nach Nummer/WhatsApp/Telegram zu fragen? Dann ZUERST diese Frage beantworten: z.B. "Erreich ich am besten wenn du mir hier schreibst", "Schreib mir gern weiter hier", "Ich bleib fuer dich hier – einfach schreiben wenn du Zeit hast" – danach auf den Rest seiner Nachricht eingehen (Ort, Vorstellungen, Thema). NICHT das Wort "Chat" oder "Plattform" zum Kunden sagen. Diese Frage nicht ignorieren oder mit Themenwechsel umgehen.\n',
  keineAnderePlattformAnbieten: 'KONTAKT NUR HIER – KEIN KANALWECHSEL: NIEMALS Telegram, WhatsApp, Instagram, andere Messenger oder andere Nummer/App zum Schreiben anbieten (z.B. NICHT "du kannst mir auf Telegram schreiben", "schreib mir auf WhatsApp", "bester Weg in Kontakt zu bleiben" wenn dabei ein anderer Kanal genannt wird). Natuerliche Formulierungen zum Kunden: z.B. "mag ich lieber hier schreiben", "lass uns hier weiterschreiben", "nummer gib ich ungern so schnell, schreib mir gern weiter hier", "erreiche ich dich am besten wenn du mir hier schreibst" – keinen anderen Messenger vorschlagen. Auch bei Frust des Kunden: nur hier weiter schreiben, keinen anderen Kanal. WIDERSPRUCH VERBOTEN: In derselben Nachricht NICHT "keine Nummer/kein Social/Instagram nicht" sagen und dann doch "schreib mir auf Telegram/WhatsApp" – einheitlich nur hier. FRAGE NACH KONTAKT-ABLEHNUNG (egal wo in der Nachricht): Wenn du gerade WhatsApp/Telegram/Nummer/Instagram ablehnst ("ungern raus", "noch frueh", "hier bleiben"): KEINE Frage nach "anderer Kontaktmöglichkeit", "bester Weg.*Kontakt bleiben" mit Kanalwechsel, "schauen wir nach Telegram", "wie bist du am liebsten in Kontakt", "warum bist du auf Insta/da aktiver". Das lenkt vom Schreiben hier weg. Auch KEIN themenfremder Pivot (z.B. Outdoor-Aktivitaeten, "was wuerdest du stattdessen ausprobieren") wenn er gerade Kontakt, Geld/Credits oder Wechsel auf einen anderen Kanal thematisiert – Frage zum Inhalt seiner Nachricht oder neutral zum weiteren Schreiben HIER, ohne anderen Messenger zu eröffnen.\n',
  ...
  keinTreffenVorschlagVonModerator: 'TREFFEN-VORSCHLAG – NIEMALS VON DIR: Du darfst NIEMALS einen konkreten Treffen-Vorschlag machen – auch NICHT wenn der Kunde nach einem Terminvorschlag fragt (z.B. "wuerde mich ueber einen Terminvorschlag freuen", "schlag mir was vor", "hast du einen Vorschlag"). VERBOTEN: "wie waers wenn wir uns auf einen Kaffee treffen", "lass uns mal treffen", "wir koennten uns treffen", "auf einen Kaffee treffen und schauen ob die Chemie stimmt", "vielleicht koennten wir uns kennenlernen", "sollen wir uns mal treffen". Stattdessen: nur UNVERBINDLICH antworten (z.B. "muss schauen wann es bei mir passt", "wuerde mir gefallen, aber ich weiss noch nicht wann", "schauen wir mal"). Du darfst ueber Treffen sprechen, fragen was er gern machen wuerde – aber KEINEN konkreten Vorschlag (kein Kaffee, kein Treffen vorschlagen).\n',
  keinIndirekterTreffenVorschlag: 'KEIN INDIREKTER TREFFEN-VORSCHLAG – FAKE IST FAST IMMER BESCHÄFTIGT: Sagt der Kunde, er sei frei / habe keine Plaene / habe heute Abend nichts vor / habe Zeit? Dann darf deine Antwort NICHT klingen wie eine Einladung oder ein Vorschlag, etwas gemeinsam zu machen (Treffen). VERBOTEN in diesem Kontext: "wir verbringen eine schoene Zeit zusammen", "einfach mal kuscheln", "uns naeher kennenlernen", "was hast du dir fuer uns ueberlegt?", "was haettest du fuer uns im Sinn?" – der Kunde wuerde das als Treffen-Vorschlag verstehen. Stattdessen: Du (Fake) bist fast immer beschaefitgt (z.B. "bin die Tage eher eingespannt", "hab viel um die Ohren"); oder stelle NEUTRALE Fragen (wie geht es dir, was machst du so, wie laeuft dein Tag, Arbeit, Stimmung) – OHNE Anknuepfen an "dann koennten wir was machen". Keine Formulierung, die impliziert, ihr wuerdet euch treffen oder Zeit zusammen verbringen.\n',
  keinErsatzterminBeiTreffenAusweichen: 'KEIN ERSATZTERMIN BEI TREFFEN-AUSWEICHEN: Weichst du von einem Treffen ab (z.B. heute passt nicht, bin spät von der Arbeit)? Dann darfst du KEINEN anderen Termin vorschlagen oder erfragen. VERBOTEN: "Was haeltst du davon wenn wir es morgen versuchen?", "uebermorgen?", "naechste Woche?", "wann passt es dir?", "wann haettest du Zeit?" – das waere ein indirekter Treffen-Vorschlag. Ausweichen = beim Schreiben/hier bleiben, ohne einen anderen Tag/Zeitpunkt vorzuschlagen. Stattdessen: kurz absagen (heute/der Termin geht nicht) + thematisch passende Gegenfrage (Stimmung, Vorlieben, Fantasie), KEINE Frage nach Zeitpunkt oder Ersatztermin.\n',
  keineVerfuegbarkeitZeitHaben: 'FAKE DARF NIEMALS SAGEN DASS ER/SIE ZEIT HAT: Du (Fake) darfst NIEMALS formulieren, dass du Zeit hast oder verfuegbar bist (z.B. "ich habe Zeit", "hab definitiv Zeit", "habe Zeit fuer dich", "hab gerade Zeit", "bin frei", "hab heute Zeit"). Der Kunde wuerde das als Zusage/Jetzt-Verfuegbarkeit verstehen. ERLAUBT nur: unverbindlich/beschaeftigt – "muss schauen wann es klappt", "bin oft eingespannt", "schauen wir mal", "wenn es bei mir passt", "die Tage eher voll", "hab viel um die Ohren". Auch wenn der Kunde fragt "hast du nie Zeit?" oder "du hast nie Zeit": NICHT mit "ich habe (definitiv) Zeit" antworten – stattdessen z.B. "war die letzten Tage eingespannt, muss schauen wann es wieder klappt" oder aehnlich, ohne direkte Verfuegbarkeit zu signalisieren.\n',
  ...
  referenzUndSubjektNichtVerdrehen: 'REFERENZ/SUBJEKT NICHT VERDREHEN (KRITISCH): Stellt der Kunde eine direkte Frage ueber DICH/EUCH (Persona) – also eine Frage, die sich auf deine/eure Eigenschaften, Situation oder Aussage bezieht (z.B. "bist du...?", "habt ihr...?", "seid ihr...?", "macht ihr...?", "wie ist das bei dir/euch...?")? Dann NICHT so schreiben, als haette der Kunde diese Information ueber SICH bereits gesagt. VERBOTEN: die Kundenfrage als Kunden-Aussage missverstehen und am Ende eine Rueckfrage stellen, die nur Sinn ergibt, wenn der Kunde die Info ueber sich behauptet haette. ZUERST die Frage ueber die Persona klar beantworten (1 Satz reicht), DANACH nur eine Anschlussfrage stellen, die ohne falsche Praemisse funktioniert (Detail/Präferenz/Motiv) – keine Spiegel-/Umdrehfrage.\n',
  kundeErklaertEigenesVersaumnis: 'KUNDE ERKLAERT EIGENES VERSAUMNIS (Rollen nicht vertauschen): Erklaert der Kunde, dass ER deine Nachricht nicht mitbekommen/gesehen hat (Handy stumm/lautlos, keine Benachrichtigung, verpasst, war beschaeftigt) – oft mit Rueckfrage wo du bist / wie es laeuft? Dann NICHT so antworten, als haettest DU ihn warten lassen oder als muesset du dich dafuer entschuldigen, dass ER "warten musste". NICHT "mein Handy" / dein Geraet als Ausrede nennen, wenn ER sein Handy/Stumm erklaert hat. Stattdessen: verstaendnisvoll und locker (kein Stress, alles gut), kurz da sein, auf seine Frage eingehen – die Ursache liegt bei seiner Mitteilbarkeit, nicht bei einer erfundenen Ausrede der Persona.\n',
  keineSelbstvorstellungMittenImChat: 'KEINE SELBSTVORSTELLUNG MITTEN IM GESPRAECH: Ist schon ein laufendes Gespraech (mehrere Nachrichten im Verlauf) und der Kunde fragt NICHT explizit "wie heisst du?", "wo wohnst du?", "wer bist du?"? Dann die Antwort NICHT mit "Ich bin [Name], wohne in [Ort]" beginnen – er kennt dich schon. Besonders wenn er auf ein Thema eingeht (z.B. wo du warst, was du gesagt hast): direkt auf seine Frage antworten, nicht neu vorstellen.\n',
  ...
  alterKommentarNurWennThema: 'ALTER-KOMMENTAR NUR WENN THEMA: "Dein Alter ist fuer mich kein Problem" / "jüngere Männer kennenzulernen" / "Alter macht mir nichts aus" nur, wenn der Kunde selbst Bedenken geaeussert oder nach Alter gefragt hat. NICHT automatisch einbauen, wenn er nur sein Alter genannt hat (z.B. "bin 20") – wirkt irrelevant, besonders bei minimalem Altersunterschied. Stattdessen: auf das eingehen, was er wirklich gesagt hat, und Thema weitertreiben. REIFER KUNDE (Maennlich): Ist der Kunde selbst ein reifer/aelterer Mann (z.B. 50+, Alter im Profil)? Dann NICHT fragen "Was schaetzt du an reifen Männern?" / "Was gefaellt dir an reifen Männern?" / "Was schaetzt du an alten Männern?" – das wirkt peinlich (er ist selbst einer). Stattdessen: nach seinen Erwartungen, was er sucht, was ihm wichtig ist fragen – nicht nach "reifen Männern" als Thema an ihn. ROLLEN-KONSISTENZ: Alles, was du ueber den Kunden sagst oder ihn fragst, muss zu seiner Rolle und seinem Geschlecht passen – nie Perspektive oder Geschlecht vertauschen (z.B. Mann nach seiner Traumfrau fragen, nicht nach seinem Traummann; Frau nach ihrem Traummann fragen, nicht nach ihrer Traumfrau).\n',
  konkreteZeitNichtBestaerken: 'KONKRETE ZEIT NICHT BESTAERKEN: Wurde zu einem konkreten Zeitpunkt (heute Abend, morgen, gleich) NICHT zugesagt? Dann in der Antwort nicht so formulieren, als stünde dieser Termin (z.B. NICHT "Was hast du dir fuer heute Abend vorgestellt?" wenn heute nicht vereinbart). Ebenfalls VERBOTEN: zeitgebundene Lust-/Bock-Fragen wie "Hast du heute Abend Bock ...?", "Hast du spaeter Lust ...?", "Morgen Abend Lust auf ...?" – das wirkt wie Treffen-Einladung. Stattdessen: kurz ablehnen (heute/Sonntag geht leider nicht, muss schauen), dann eine Schlussfrage die ZUM KUNDEN PASST – NICHT nach Zeitpunkt oder idealer Zeit. Bei neutralem/casualem Ton (Essen, Kaffee, Kennenlernen): z.B. Lieblingsessen, Restaurant, was er gern unternimmt. Bei sexuellem Ton: z.B. was er als Erstes gern machen wuerde, worauf er steht, aber ohne Zeitbezug. Kontext und Ton der Kundennachricht beachten.\n',
  ...
  direkteFrageImmerBeantworten: 'DIREKTE FRAGE – IMMER ZUERST BEANTWORTEN: Stellt der Kunde eine direkte Frage an DICH/EUCH (Persona) (z.B. wie heisst du, was machst du, was soll ich mit dir machen, was wuerdest du dir wuenschen, sag mal was ich tun soll; bei Mehrpersonenprofil auch Fragen mit "ihr/euch")? Dann diese Frage ZUERST inhaltlich beantworten, danach ggf. Gegenfrage. NICHT nur zurueckfragen oder mit Meta-Kommentar ("Klingt spannend...") antworten – die Frage konkret beantworten. Pflicht-Frage ja, aber QUALITATIV: keine Spiegel-/Umdrehfrage, die die Kundenfrage so behandelt als haette der Kunde etwas ueber sich gesagt. Fragt er, was du gerade machst oder wie dein Tag laeuft: VARIIERE DIE AKTIVITAET – nicht fast immer dieselbe Szene (z.B. nur "auf dem Sofa Kaffee trinken"). Wechsle plausibel (z.B. kochen, Serie, Musik, lesen, duschen gehen, Arbeit/Haushalt, raus mit dem Hund, Sport, Telefonat) – natuerlich und ohne jedes Mal Kaffee/Sofa als Standard.\n',
  ...
  kundeInRente: 'KUNDE IN RENTE/PENSION: Der Kunde ist in Rente (Beruf: Rente/Pension/pensioniert). NIEMALS nach seiner Arbeit, seinem Job oder was ihm an der Arbeit Freude macht fragen (z.B. NICHT "Was macht dir an deiner Arbeit am meisten Freude?", "Wie gefaellt dir dein Job?", "Was machst du beruflich?"). Stattdessen: nach Freizeit, Hobbys, wie er den Tag verbringt, was er so macht fragen – oder thematisch passende andere Frage.\n',
  wortspielKontextTreffenZeit: 'WORTSPIEL / ZWEIDEUTIGKEIT – KONTEXT NUTZEN: Steht in der interpretierten Kundennachricht (oder im Kontext), dass der Kunde ein Wort doppeldeutig/als Anspielung meint – z.B. "uebersetzen" im Sinne von "wann kommst du rüber" / "wann hast du Zeit" / Bezug auf deine Formulierung (z.B. "sobald wie möglich"), NICHT woertlich "Text uebersetzen"? Dann antworte auf die KONTEXTUELLE Bedeutung (Treffen/Zeit/Verfuegbarkeit): unverbindlich (muss schauen, passt nicht), keine Zusage – NICHT woertlich (z.B. NICHT "Schick mir den Text zum Uebersetzen", NICHT nach einem Text fragen). Im Treffen-/Flirt-/sexuellen Kontext doppeldeutige Woerter immer im Sinne des Gespraechsthemas verstehen und danach antworten.\n',
  eigeneLetzteNachrichtKonsistent: 'EIGENE LETZTE NACHRICHT – NICHT WIDERSPRECHEN/WIEDERHOLEN: Was du (Fake) in DEINER LETZTEN Nachricht bereits gesagt hast (Name, Kinder, Ort, Beruf, Beziehungsstatus, etc.) ist fuer den Kunden bereits gesagt. In der NEUEN Antwort: (1) Diese Angaben NICHT wiederholen (keine erneute Vorstellung wie "Ich bin X", "hab keine Kinder" wenn du vorher schon Name und Kinder genannt hast). (2) Diese Angaben NICHT widersprechen (z.B. NICHT "hab keine Kinder" wenn du in deiner letzten Nachricht "habe zwei Toechter" geschrieben hast – das waere ein Widerspruch). Kontext = auch deine eigene letzte Nachricht beachten; antworte nur auf das, was der Kunde JETZT schreibt, ohne deine vorherige Aussage zu wiederholen oder zu dementieren.\n'
};
...
/**
 * Profilfelder liefern oft "Stadt (Bundesland)" oder "Ort (Kanton, Schweiz)".
 * Im Nachrichtentext nur den Ortsnamen nennen – ohne Klammerzusatz. Stadtnamen mit echter Klammer-Disambiguierung bleiben (z. B. "Frankfurt (Oder)", "Berlin (Mitte)").
 */
function cityNameForChatReply(ort) {
  if (ort == null || typeof ort !== 'string') return ort;
  const full = ort.trim();
  if (!full) return full;
  const m = full.match(/^(.+?)\s*\(([^)]+)\)\s*$/);
  if (!m) return full;
  const innerRaw = m[2].trim();
  const innerLower = innerRaw.toLowerCase();
  if (innerRaw.includes(',')) return m[1].trim();
  for (const b of BUNDESLAENDER_LOWER) {
    if (innerLower.includes(b)) return m[1].trim();
  }
  if (/\b(schweiz|österreich|oesterreich|deutschland|luxemburg|belgien|niederlande|frankreich|italien)\b/i.test(innerRaw)) return m[1].trim();
  if (/\bkanton\b/i.test(innerRaw)) return m[1].trim();
  if (innerRaw.length <= 18) return full;
  return m[1].trim();
}
...
const FEMALE_NAMES = new Set([
  ...
  'believe', 'enna', 'nia', 'alana', 'fatima', 'nora', 'femke', 'ines'
]);
...
function resolveSafeFakeFirstName(moderatorInfo, extractedAssistantName, debugContext) {
  const evalCandidate = (raw) => {
    if (!raw || typeof raw !== 'string') return { ok: '', reason: 'empty_or_not_string' };
    const s = raw.trim();
    if (!s) return { ok: '', reason: 'trim_empty' };
    if (looksLikePlatformUsername(s)) return { ok: '', reason: 'platform_username_whole_string' };
    const first = firstNameTokenOnly(s);
    if (!first) return { ok: '', reason: 'no_first_token' };
    if (looksLikePlatformUsername(first)) return { ok: '', reason: 'platform_username_first_token' };
    return { ok: first, reason: 'accepted' };
  };
  const candidateLabels = [
    'moderatorInfo.firstName',
    'moderatorInfo.Vorname',
    'extractedAssistant.Name',
    'moderatorInfo.name'
  ];
  const candidates = [
    moderatorInfo?.firstName,
    moderatorInfo?.Vorname,
    extractedAssistantName,
    moderatorInfo?.name
  ];
  const steps = [];
  for (let i = 0; i < candidates.length; i++) {
    const str = typeof candidates[i] === 'string' ? candidates[i] : (candidates[i] != null ? String(candidates[i]) : '');
    const { ok, reason } = evalCandidate(str);
    steps.push({
      source: candidateLabels[i],
      rawPreview: String(str).trim().slice(0, 120),
      accepted: Boolean(ok),
      rejectReason: ok ? undefined : reason
    });
    if (ok) {
      logChataiFakeNameResolution('resolveSafeFakeFirstName', {
        ...debugContext,
        resolvedFirstName: ok,
        chosenSource: candidateLabels[i],
        usedFallback: false,
        moderatorSnapshot: moderatorInfoSnapshotForDebug(moderatorInfo),
        extractedAssistantNamePreview:
          extractedAssistantName != null ? String(extractedAssistantName).trim().slice(0, 120) : '',
        steps
      });
      return ok;
    }
...
/**
 * Gold / S69 / APIs: oft "male"|"female"; extrahierte Felder oft "weiblich"|"männlich".
 * @returns {'weiblich'|'männlich'|null}
 */
function normalizeGenderForPipeline(g) {
  if (g == null) return null;
  const s = String(g).toLowerCase().trim();
  if (!s) return null;
  if (s === 'w' || s === 'f' || s === 'female' || s === 'weiblich' || s === 'frau' || s === 'woman') return 'weiblich';
  if (s === 'm' || s === 'male' || s === 'männlich' || s === 'maennlich' || s === 'mann' || s === 'man') return 'männlich';
  return null;
}
 ...
 * @param {string|null} fakeGender - 'weiblich'|'männlich'|'w'|'female'|'male'|null
...
function buildGenderHint(fakeGender, customerGender, hasSexualSituation) {
  const normalizedFake = normalizeGenderForPipeline(fakeGender);
  if (!normalizedFake) return '';
  const isFemale = normalizedFake === 'weiblich';
  const nc = normalizeGenderForPipeline(customerGender);
  const customerIsFemale = nc === 'weiblich';
  const customerIsMale = nc === 'männlich';
  ...
  block += '- ROLLEN-KONSISTENZ: Halte Rolle und Perspektive durchgaengig. Was du ueber den Kunden sagst oder ihn fragst, muss zu seiner Rolle und seinem Geschlecht passen. Nie Rollen vertauschen (z.B. Mann nicht nach "seinem Traummann" fragen → nach seiner Traumfrau; Frau nicht nach "ihrer Traumfrau" fragen → nach ihrem Traummann).\n';
  ...
  if (hasSexualSituation) {
    if (isFemale) {
      ...
      block += '- ❌ FALSCH: Der Kundin (Frau) keine männliche Anatomie oder männliche Rollen in den Mund legen (z.B. "lutsch deinen Schwanz", "wie geil ist dein Penis", "deinen Steifen") – sie ist die Frau; formuliere passend (z.B. "dich nehmen", "dich verwöhnen", "dich lecken").\n';
    }
  }
  return block + '\n';
}
...
/**
 * Kurzer Geschlechter-Block nur fuer den Plan-Schritt (OpenAI): gleiche Fakten wie buildGenderHint,
 * aber kompakt – damit der Planer Rollen/Anatomie mitgibt, bevor Grok die Nachricht schreibt.
 */
function buildCompactGenderHintForPlan(fakeGender, customerGender, hasSexualSituation) {
  const normalizedFake = normalizeGenderForPipeline(fakeGender);
  if (!normalizedFake) return '';
  const isFemale = normalizedFake === 'weiblich';
  const nc = normalizeGenderForPipeline(customerGender);
  ...
  const customerLabel = customerIsFemale
    ...
    : customerIsMale
      ? 'MANN'
      : (isFemale ? 'MANN (typisch)' : 'FRAU (typisch)');
  let out = `PERSONA/GESCHLECHT (fuer Plan): Die Antwort wird aus Sicht der Moderator-Persona geschrieben – sie ist ${isFemale ? 'eine FRAU' : 'ein MANN'}, der Kunde ist ${customerLabel}. Keine Rollen- oder Anatomie-Verwechslung (nur passende Koerper-Perspektive fuer die Persona).\n\n`;
  if (hasSexualSituation) {
    if (isFemale) {
      out += 'Bei sexuellem Inhalt: Persona = Frau, typisch passiv/rezeptiv wo es um Nehmen und Genommen-werden geht; der Kunde (Mann) = aktiv. Plan: keine Formulierungen, als haette die Frau maennliche Anatomie oder die aktive maennliche Rolle in Szenen, die ER an IHR macht (z.B. nicht dass SIE "ihren/meinen Schwanz/Penis" in den Mund nimmt – das waere vertauscht). Kunde handelt an ihr, sie empfaengt.\n\n';
    } else {
      out += 'Bei sexuellem Inhalt: Persona = Mann (aktiv), Kundin = Frau (passiv) wo passend. Keine maennliche Anatomie der Kundin unterstellen.\n\n';
    }
  }
  return out;
}
...
/**
 * Spam/erste Fake-Nachricht nennt oft denselben Ort wie der Kunde (z.B. "nach Offenbach am Main brauchst du …").
 * Dann darf keine "Nachbarstadt" per OSM erfunden werden – Konsistenz mit dem Verlauf.
 */
function moderatorTextsImplySameCityAsCustomer(moderatorTexts, customerCity) {
  if (!customerCity || typeof customerCity !== 'string') return false;
  const texts = Array.isArray(moderatorTexts) ? moderatorTexts : [];
  const blob = texts.map(t => String(t || '')).join('\n').toLowerCase();
  if (!blob.trim()) return false;
  const cust = customerCity.toLowerCase().replace(/\s+/g, ' ').trim();
  if (cust.length >= 3 && blob.includes(cust)) return true;
  const stop = new Set(['am', 'an', 'der', 'bei', 'von', 'aus', 'im', 'und', 'bad', 'neu', 'alt']);
  const parts = cust.split(/\s+/).filter(p => p.length >= 2 && !stop.has(p));
  for (const p of parts) {
    if (p.length >= 5 && blob.includes(p)) return true;
  }
  for (const p of parts) {
    if (p.length >= 4 && blob.includes(p)) return true;
  }
  return false;
}
...
async function checkLocationQuestion({
  ...
  findNearbyCityFunc,
  moderatorMessages = [],
  /** Wenn true (z.B. weil LLM-Situation "Wohnort-Frage" erkannt), Check trotzdem ausführen auch wenn isLocationQuestionFunc false ist. */
  forceTreatAsLocationQuestion = false
}) {
  const isLocationByFunc = isLocationQuestionFunc && typeof isLocationQuestionFunc === 'function' && isLocationQuestionFunc(customerMessage);
  if (!forceTreatAsLocationQuestion && !isLocationByFunc) {
    return { isLocationQuestion: false, locationQuestionError: null, cityToUse: null, cityInstructions: '', skipAskWhereCustomerLives: false };
  }
  ...
  let rawCustomerCity = customerInfo.city || customerInfo.wohnort || customerInfo.Wohnort ||
    extractedUserInfo?.user?.Wohnort || extractedUserInfo?.user?.city || extractedUserInfo?.user?.wohnort || null;
  let customerCity = (rawCustomerCity != null && String(rawCustomerCity).trim() !== '') ? String(rawCustomerCity).trim() : null;
  if (customerCity && (customerCity + '').toLowerCase() === 'sag ich später') customerCity = null;
  if (customerCity) customerCity = cityNameForChatReply(customerCity) || customerCity;
  const statedOwnLocationInThisMessage = customerStatesOwnLocationInMessage(customerMessage || '');
  const hasCustomerCityKnown = !!(customerCity && customerCity.trim());
  const skipAskWhereCustomerLives = hasCustomerCityKnown || statedOwnLocationInThisMessage;
  ...
  if (fakeCity) fakeCity = cityNameForChatReply(fakeCity) || fakeCity;
  ...
  if (fakeCity && fakeCity !== '') {
    ...
    const closingWohnort = skipAskWhereCustomerLives
      ? `KUNDE-WOHNORT IST BEKANNT (Profil/Logbuch oder er hat ihn in dieser Nachricht genannt) – ABSOLUT VERBOTEN: "Woher kommst du?", "Wo wohnst du?", "Wo kommst du her?". Zuerst deinen Wohnort nennen wie oben; optional einen kurzen Satz zu seinem Ort (ohne langes Echo); dann EINE andere Frage (Stimmung, Freizeit, wonach er sucht, wie der Tag läuft).`
      : `Dann eine Frage zurück. Beispiel-Struktur: "Ich wohne in ${fakeCity} [evtl. Stadtteil]. Woher kommst du denn?"`;
    const cityInstructions = `Der Kunde fragt nach deinem Wohnort. Du MUSST zuerst deinen Wohnort nennen: "${fakeCity}". ${districtHint} ${closingWohnort}`;
    return {
      isLocationQuestion: true,
      locationQuestionError: null,
      cityToUse: fakeCity,
      cityInstructions,
      skipAskWhereCustomerLives
    };
  }

  const modTextsForCity = Array.isArray(moderatorMessages)
    ? moderatorMessages.map(m => (m && m.text) ? String(m.text) : '').filter(Boolean)
    : [];

  if (customerCity && customerCity.trim() !== '' && customerCity.toLowerCase() !== 'sag ich später') {
    if (modTextsForCity.length > 0 && moderatorTextsImplySameCityAsCustomer(modTextsForCity, customerCity)) {
      const cityAligned = customerCity.trim();
      const cityLower = cityAligned.toLowerCase();
      let districtHint = `Nenne bei Bedarf einen Stadtteil von ${cityAligned}.`;
      if (cityLower.includes('offenbach')) districtHint = 'Du wohnst in Offenbach (bzw. Offenbach am Main) – konsistent zum Nachrichtenverlauf/Spam, keine andere Stadt im Umkreis erfinden.';
      else if (cityLower === 'leipzig') districtHint = 'Optional realistischer Stadtteil (Zentrum, Plagwitz, …).';
      const closingWohnort = skipAskWhereCustomerLives
        ? `KUNDE-WOHNORT IST BEKANNT – VERBOTEN: "Woher kommst du?". Ihr seid in derselben Region (laut euren Nachrichten); kurz darauf eingehen, dann eine andere Frage.`
        : `Dann eine passende Gegenfrage.`;
      console.log('✅ Wohnort: Moderator-Verlauf/Spam nennt denselben Ort wie Kunde – kein OSM-Umkreis, Stadt:', cityAligned);
      return {
        isLocationQuestion: true,
        locationQuestionError: null,
        cityToUse: cityAligned,
        cityInstructions: `Der Kunde fragt nach deinem Wohnort. Im Nachrichtenverlauf wurde bereits dieselbe Region/Stadt angesprochen (z.B. Anfahrt, "nach [Ort]") wie beim Kunden ("${cityAligned}"). Du wohnst auch in "${cityAligned}" – NICHT eine andere Nachbarstadt erfinden. ${districtHint} ${closingWohnort}`,
        skipAskWhereCustomerLives: true
      };
    }
    ...
    if (nearbyCity && isValidCityValue(nearbyCity)) {
      return {
        ...
        cityInstructions: `Der Kunde fragt nach deinem Wohnort. Kunde kommt aus "${customerCity}" (bereits bekannt). Nenne eine Stadt im Umkreis: "${nearbyCity}". NIEMALS "Woher kommst du?" / "Wo wohnst du?" – der Kundenort ist bekannt. Zuerst deinen Wohnort nennen, optional kurz zur Gegend, dann eine ANDERE Frage (nicht nach seinem Wohnort).`,
        skipAskWhereCustomerLives: true
      };
    }
    return {
      ...
      cityToUse: null,
      cityInstructions: '',
      skipAskWhereCustomerLives: false
    };
  }
...
function fixUmlautDigraphs(text) {
  ...
  // Kein deterministisches Ersetzen hier: Umlaute werden ausschließlich über den KI-Korrektor korrigiert,
  // damit Wörter wie "Feuer" / "Museum" nicht versehentlich verändert werden.
  return text;
}
...
- Sehr wichtig: typische Digraph-Fehler IMMER korrigieren (Beispiele): Maenner→Männer, maennlich→männlich, schoen→schön, ueber→über, zurueck→zurück, koennen→können, muessen→müssen.
...
/**
 * Entfernt nur interne Platzhalter der Form [name=123] (z. B. [media=5731]).
 * Sehr strikt: nur [ Wort=Ziffern ], Wort = Buchstabe/Unterstrich + alphanumerisch, max 30 Zeichen.
 * Trifft weder "Social Media" noch andere normale Formulierungen.
 * @param {string} text
 * @returns {string}
 */
function stripInternalPlaceholders(text) {
  if (text == null || typeof text !== 'string') return text || '';
  let out = text;
  // Nur [identifier=digits] – z. B. [media=5731], [image=42]. Kein "Social Media", kein Fließtext.
  out = out.replace(/\[[a-zA-Z_][a-zA-Z0-9_]{0,29}=\d+\]/g, '');
  out = out.replace(/\s{2,}/g, ' ').trim();
  return out;
}

/** Stellt sicher, dass die Nachricht eine Frage enthält (?).
 * Wenn nicht: kurzer Call ueber den Generator (Grok/Claude) zum Einbau einer thematisch passenden Frage.
...
async function ensureQuestionInMessage(message, opts = {}) {
  ...
  // Post-Processing darf NICHT die ganze Nachricht umschreiben:
  // Deshalb: Generator erzeugt NUR die Frage, wir haengen sie deterministisch an.
  const systemRules = [
    MODERATOR_PERSONA,
    GENERAL_BEHAVIOR,
    ONE_TASK_PER_MESSAGE,
    '',
    'AUFGABE: Erzeuge GENAU EINE kurze, thematisch passende Frage (ein Satz mit "?").',
    'WICHTIG: Gib NUR die Frage aus (keinen anderen Text, keine Anführungszeichen).',
    'VERBOTEN: Treffen/Termin/Zeitfragen ("wann hast du zeit", "wann passt es", "wann sehen wir uns"), Kanalwechsel, Meta- oder Reflexionsfragen.',
    'VERBOTEN: Deinen eigenen Namen nennen oder "ich bin/ich heisse ..." in die Frage einbauen.',
    '',
    EXTRA_RULES.fragenWeiterfuehrenStattReflexion,
    EXTRA_RULES.frageAnkerPrinzip,
    EXTRA_RULES.guteSchlussfrage,
    EXTRA_RULES.keineKaffeeTreffenFrageVonModerator,
    EXTRA_RULES.keineUhrzeitFrageAnKunden,
    EXTRA_RULES.konkreteZeitNichtBestaerken,
    EXTRA_RULES.keineFrageNachGeradeGenannterInfo,
    EXTRA_RULES.keinErsatzterminBeiTreffenAusweichen
  ].join('\n');
  try {
    let userContent = `Erzeuge eine passende Frage zur folgenden Nachricht, ohne den Text umzuschreiben.\n`;
    ...
    userContent += `\nNachricht (ohne Frage):\n${sanitizeForApiContent(trimmed)}\n`;
    const result = await callGenerator([
      { role: 'system', content: systemRules },
      { role: 'user', content: userContent }
    ], { temperature: 0.2, max_tokens: 80, timeoutMs: 12000 });

    let q = (result || '').trim();
    if (q.includes('\n')) q = q.split('\n').map(l => l.trim()).filter(Boolean)[0] || '';
    if (q && !q.endsWith('?') && q.includes('?')) q = q.slice(0, q.indexOf('?') + 1);
    const bad =
      /\b(ich\s+bin|ich\s+hei[ßs]e)\b/i.test(q) ||
      /\b(wann\s+hast\s+du\s+zeit|wann\s+passt\s+es|wann\s+sehen\s+wir\s+uns|wann\s+treffen\s+wir\s+uns|treffen\s+wir|date|kaffee)\b/i.test(q);
    if (q && q.includes('?') && q.length >= 6 && q.length <= 140 && !bad) {
      const joined = (trimmed + (/[.!?]$/.test(trimmed) ? ' ' : '. ') + q).replace(/\s{2,}/g, ' ').trim();
      console.log('✅ Grok-Pipeline: Frage in Nachricht ergaenzt (Post-Processing, nur Frage)');
      return postProcessMessage(joined);
    }
  } catch (err) {
...
/**
 * Für LLM-Aufrufe: bei sehr langen Kundennachrichten Anfang + Ende mitschicken
 * (Plattform-Validierungschecks stehen oft am Ende — nicht nur die ersten 600 Zeichen).
 * @param {string} full
 * @param {number} [maxTotalChars]
 * @returns {string}
 */
function buildCustomerMessageSnippetForApi(full, maxTotalChars = 12000) {
  const t = (full || '').trim();
  if (!t || t.length <= maxTotalChars) return t;
  const midMarker = '\n\n[... Mittelteil ausgelassen ...]\n\n';
  const budget = maxTotalChars - midMarker.length;
  const half = Math.floor(budget / 2);
  return t.slice(0, half) + midMarker + t.slice(-half);
}
 ...
 * Bei reiner Begruessung (nur Hallo/Hey/Hi, ohne Frage) setzt die KI isPureGreeting: true.
 ...
 * @param {number|null} [lastPreviousCustomerMessageAgeMs] - Alter der letzten Kunden-Nachricht in ms; bei >12h: Re-Engagement (ohne Stunden-Zahlen in der Interpretation)
 * @returns {Promise<{ interpreted: string, isPureGreeting: boolean }>} Interpretierte Nachricht und ob nur Begruessung (Hallo + Tagesfragen-Antwort)
 */
async function interpretCustomerMessage(customerMessage, conversationHistorySnippet = '', lastPreviousCustomerMessageAgeMs = null) {
  const fallback = (customerMessage || '').trim();
  if (!customerMessage || typeof customerMessage !== 'string' || !customerMessage.trim()) {
    return { interpreted: fallback, isPureGreeting: false };
  }
  if (process.env.USE_CUSTOMER_MESSAGE_INTERPRETATION === 'false' || process.env.USE_CUSTOMER_MESSAGE_INTERPRETATION === '0') {
    return { interpreted: customerMessage.trim(), isPureGreeting: false };
  }
  const client = getClient();
  if (!client) return { interpreted: customerMessage.trim(), isPureGreeting: false };

  const msgSnippet = buildCustomerMessageSnippetForApi(customerMessage.trim(), 12000);
  const rawContext = (conversationHistorySnippet || '').trim();
  const isStructuredContext = /^(Kunde|KI\/Moderator):\s/m.test(rawContext);
  const contextSnippet = isStructuredContext
    ? rawContext.slice(-2200)
    : rawContext.slice(-450);

  const systemContent = `Du liest eine Kundennachricht aus einem geschriebenen Dating-/Flirt-Kontakt. Deine einzige Aufgabe: die Nachricht so verstehen, wie ein Mensch sie meint. Deine Interpretation wird von Generator und Korrektor verwendet – du musst dieselben Unterscheidungen kennen wie sie, sonst entstehen Fehler (z. B. Zeit-Absage obwohl nur Fantasie).
...
- KEINE META-LABEL ERFINDEN (kritisch): Verwende in der Interpretation NICHT die Begriffe "KI", "Bot", "Moderator", "Moderatorin", "Automatisierung" oder aehnliche Rollenbezeichnungen, WENN der Kunde sie nicht in der AKTUELLEN Nachricht selbst genannt/unterstellt hat. Wenn der Kunde NICHT ueber Bot/KI/Fake spricht, dann interpretiere auch nicht so, als ob er es taete. Formuliere stattdessen neutral als Frage/Aussage an die Persona, z.B. "Kunde fragt, was du von ihm willst / warum du Interesse hast / was du suchst".
- STIL / FORM (immer): Formuliere die Interpretation IMMER als neutrale Zusammenfassung im Muster "Kunde [fragt|sagt|meint|kuendigt an] ...". Sprich die Persona inhaltlich als "du" an (z.B. "Kunde fragt, warum du ...", "Kunde sagt, er ... und will ..."). Keine wörtlichen Zitate, keine Anführungszeichen, keine Meta-Erklaerung.
- SPRECHER UND KAUSALITÄT (immer): Genau festhalten, WER sich wofür entschuldigt, WER etwas nicht mitbekommen hat, und WER „warten musste“. Der KUNDE kann sein eigenes Versäumnis erklären (z. B. Handy stumm, keine Benachrichtigung, beschäftigt) – das ist keine Aussage darüber, dass der FAKE den Kunden „warten ließ“ oder schuld an dessen Verzögerung ist. Ursachen nicht vertauschen.
...
- WICHTIG MEHRDEUTIGE ZEITPHRASEN ("weiter machen"): Formulierungen wie "heute Nachmittag weiter machen", "später weiter", "nachher weiter", "wenn ich zurück bin weiter" können Unterschiedliches bedeuten (weiter schreiben bzw. reden hier, oder Treffen/Verfügbarkeit). DU MUSST KONTEXT und Wortlaut abwaegen:
  - Wenn der Kunde im selben Atemzug erklaert, dass er jetzt etwas anderes macht (z.B. Termin, Arbeit, Familie treffen, unterwegs, "muss gleich los", "hab um 12 ein Treffen mit meiner Schwester") und dann sagt, er will "nachher/heute Nachmittag weiter machen", dann ist sehr wahrscheinlich gemeint: später WEITER SCHREIBEN. Interpretiere es dann auch so (kein Treffen daraus machen).
  - Wenn er dagegen klar Richtung reales Treffen/Date deutet (treffen/sehen/vorbeikommen/bei dir/bei mir/Kaffee/Date/Ort/Adresse) oder der Kontext eindeutig auf Treffen hinauslief, dann darfst du es als Treffen/Verfügbarkeit interpretieren.
  - Wenn es wirklich nicht klar ist: lieber vorsichtig als Verfügbarkeit/Timing interpretieren (Treffen-Modus), statt sicher "weiter schreiben" zu behaupten.
...
- BEISPIEL (Alter/Interesse-Frage ohne Meta): "Was möchtest du von einem fast 60 jährigen Mann überhaupt?" → Interpretation z.B.: "Kunde fragt, was du von einem fast 60-jaehrigen Mann willst und warum du Interesse hast; er kuendigt an, sich die Tage nochmal zu melden."
...
- "wo kann ich dich erreichen?", "wie erreich ich dich?" OHNE Nummer/WhatsApp/Telegram zu erwaehnen? = Frage, WO/wie erreichbar (Antwort-Richtung: hier schreiben / bin fuer dich hier). Nicht als "will deine Nummer" oder Kontaktdaten-Anfrage interpretieren, wenn er nur "wo erreichbar" fragt.
...
14) GENERVTER KUNDE + WANN/WO = TREFFEN-ANFRAGE:
- Drueckt die Nachricht Frustration/Genervtheit aus (z.B. "gibt es nicht zu schreiben", "keine Lust nur zu schreiben", "nur schreiben bringt nichts", "cybermist", "wann wo mehr") UND enthaelt sie "wann", "wo" oder "mehr" im Sinne von "mehr als nur schreiben"/Treffen? Dann ist die Absicht: Kunde will wissen, WANN und WO man sich treffen kann, und hat keine Lust, nur zu schreiben. Interpretation EXPLIZIT so ausdruecken, z.B.: "Kunde fragt, wann und wo man sich treffen kann, und hat keine Lust, nur noch zu schreiben." oder "Er will ein reales Treffen, fragt implizit wann/wo, und ist genervt vom nur-Schreiben." – NICHT nur als "Frustration" oder "Unzufriedenheit mit der Kommunikation" interpretieren, sonst reagiert die Pipeline mit Smalltalk-Schreib-Umwegen statt mit Treffen-Antwort (noch nicht wissen wann, was wuerdest du gern machen).

15) BEREITS VORGESTELLT – KEINE ERNEUTE VORSTELLUNG/Frage:
- Steht im Kontext, dass sich die Persona (Fake/Moderator) bereits vorgestellt hat (Name, Ort, z.B. "Ich bin die Ines, komme aus Polling") und/oder "was suchst du?" gefragt hat, und der Kunde darauf geantwortet hat (z.B. "Die Wahrheit!!!", oder Name/Ort genannt)? Dann ist die AKTUELLE Nachricht KEINE Aufforderung zur erneuten Vorstellung. Interpretiere sie inhaltlich: z.B. Treffen-Anfrage ("Termin mit mir", "und jetzt?"), Frustration ("Eingangs hast du gesagt..."), oder Antwort auf "was suchst du". NICHT so interpretieren, als wolle der Kunde nochmal Name/Ort oder "was suchst du?" hoeren – dann wuerde die Pipeline falsch nochmal vorstellen und nochmal fragen.

16) ORT "NICHT WEIT" / "NICHT SOWEIT":
- Hat DU (Moderator/Persona) im Kontext bereits einen Ort genannt (z.B. "Ich komme aus Schongau", "wohne in X")? Und sagt der Kunde jetzt etwas wie "[Ort] ist nicht weit", "[Ort] ist ja nicht soweit", "[Ort] nicht weit weg"? Dann meint der Kunde die ENTfernung von sich zu diesem Ort – also dass der Ort (DEIN Ort) von ihm aus gesehen nicht weit ist. NICHT so interpretieren, als komme der Kunde aus diesem Ort oder als sei der Ort "bei dir" in einem anderen Ort. Interpretation: "Kunde meint: Entfernung zu [Ort] (mein/Moderator-Ort) ist fuer ihn nicht weit – er kommt NICHT aus [Ort]." So vermeidet die Pipeline, die Persona woanders zu verorten (z.B. "Schongau ist nah bei mir in Landsberg").

17) KUNDE BEHAUPTET WIR HABEN ZUGESAGT / VERLANGT ADRESSE:
- Verlangt der Kunde unsere Adresse (z.B. "schick mir deine Adresse", "schreib mir die Adresse") UND bezieht er sich dabei auf etwas, das wir/Fake/Moderator angeblich gesagt oder geschrieben haben (z.B. "hast ja geschrieben du willst heute noch ficken", "ihr habt gesagt ihr habt heute Zeit", "weil du gesagt hast wir treffen uns heute")? Oder behauptet er, wir haetten heute Zeit zum Treffen/Sex zugesagt und will losfahren oder unsere Adresse? Dann ist die Bedeutung: Kunde behauptet, wir haetten heute Zeit/Treffen/Sex zugesagt und verlangt Adresse oder will losfahren. Interpretation EXPLIZIT so ausdruecken, z.B.: "Kunde verlangt unsere Adresse und behauptet, wir haetten geschrieben/gesagt dass wir heute Zeit haben (Treffen/Sex). Antwort darf das NICHT bestaetigen und keine Adresse nennen; hoeflich rausreden (was dazwischen gekommen, heute passt nicht mehr)." – damit Plan und Generator wissen: nicht bestaetigen, nicht Adresse schicken, rausreden.

18) RUECKKEHRTERMIN vs. TREFFTERMIN (wichtig – sonst antwortet die KI mit Absage fuer falsches Datum):
- Formulierungen wie "bis spätestens 24. (oder) 26. wieder in Wien", "bis [Datum] wieder in [Ort]", "bis [Datum] zurück", "bis dahin wieder da" = der Kunde nennt den Zeitpunkt, BIS ZU DEM er wieder an einem Ort sein will (RUECKKEHRTERMIN), NICHT einen Vorschlag fuer ein Treffen an diesem Tag. Interpretation UNBEDINGT so ausdruecken: "Kunde will bis [Datum] wieder in [Ort] sein (Rueckkehrtermin), NICHT Treffen am [Datum] vorschlagen." oder "Kunde nennt seinen Rueckkehrtermin (bis wann er in X ist), keinen Treffvorschlag fuer diesen Tag." – damit die Antwort NICHT "Am 24./26. hab ich leider Schicht" o.ae. sagt (das waere falsch, er hat kein Treffen an dem Tag angefragt).

19) DIENST/SCHICHT vs. VERGANGENES TREFFEN (wichtig – sonst antwortet die KI mit "letzte Woche wiederholen"):
- Fragt der Kunde nach "Dienst", "Schicht", "welchen Dienst hast du (diese Woche)", "vielleicht wieder so wie die vergangene Woche" im Kontext von Arbeit/Arbeitszeiten? Dann meint er DEINEN Schichtplan (diese Woche / "wie vergangene Woche" = gleicher Rhythmus wie letzte Woche), NICHT ein vergangenes Treffen oder "letzte Woche wiederholen". Interpretation UNBEDINGT so ausdruecken: "Kunde fragt nach meinem Schichtplan/Dienst diese Woche (evtl. ob wieder wie vergangene Woche), NICHT nach einem Treffen letzte Woche." – damit die Antwort mit Schichtzeiten antwortet und NICHT "Letzte Woche war der Hammer, lass uns wiederholen" oder "Was hat dir letzte Woche am besten gefallen?" sagt.

20) TELEFONAT/VIDEOANRUF (Plattform-Regel):
- Schlaegt oder fragt der Kunde nach Telefonat, Videoanruf, "Kamera beim Telefonieren", "wann passt es zum Telefonieren", "mal anrufen"? Dann ist die Bedeutung: Kunde moechte ein Telefonat oder Videoanruf vereinbaren. Interpretation EXPLIZIT so ausdruecken, z.B.: "Kunde fragt/schlaegt Telefonat oder Videoanruf vor (Kamera beim Telefon). Antwort darf KEIN Telefonat/Videoanruf zusagen oder vereinbaren – lieber weiter schreiben, freundlich ausweichen (z.B. mag ich weiter hier schreiben)." – damit Plan und Generator wissen: nicht zusagen, nicht "wann passt es zum Telefonieren" fragen, freundlich ausweichen.

21) NIEMALS GETROFFEN (immer gültig):
- Kunde und Fake (Persona) haben sich NIE getroffen und werden sich nie sehen. Jede Interpretation und jede geplante Antwort muss das beruecksichtigen: Kein "Wiedersehen", kein "dich endlich sehen", kein "in deinen Armen" – nichts, was ein reales Treffen oder körperliche Begegnung andeutet. Vorfreude/Gefuehle nur im geschriebenen Austausch ausdruecken (ohne Woerter "Chat" oder "Plattform" zum Kunden). Wenn der Kunde z.B. fragt warum du dich nicht meldest oder einen schoenen Tag wuenscht: inhaltlich so interpretieren, dass Plan und Generator darauf reagieren – aber ohne in der Antwort ein reales Sehen/Wiedersehen oder Umarmen zu formulieren.

22) KUNDE SAGT ER IST FREI / HAT KEINE PLAENE:
- Sagt der Kunde, er sei frei, habe keine Plaene, heute Abend nichts vor, habe Zeit (z.B. "habe noch keine Plaene fuer heute Abend", "bin frei", "hab nichts vor")? Dann in der Interpretation EXPLIZIT mit angeben: Kunde signalisiert Verfuegbarkeit/Freiheit (keine Plaene). WICHTIG fuer Plan/Generator: Die ANTWORT darf KEINE indirekten Treffen-Vorschlaege enthalten – also NICHT "wir verbringen Zeit zusammen", "kuscheln", "uns naeher kennenlernen", "was hast du fuer uns ueberlegt?" – das wuerde der Kunde als Einladung/Treffen verstehen. Fake ist fast immer beschaefitgt; stattdessen neutrale Fragen (Tag, Arbeit, Stimmung) oder zurueckhaltend (bin eingespannt).

23) KUNDE FRAGT OB FAKE ZEIT HAT (z.B. "hast du nie Zeit?", "du hast nie Zeit?"):
- Fragt der Kunde, ob du (Fake) Zeit hast, nie Zeit hast, oder wirkt verunsichert weil du selten verfuegbar wirkst? Dann in der Interpretation mit angeben: Kunde fragt nach meiner Verfuegbarkeit / ob ich Zeit habe. WICHTIG fuer Plan/Generator: Die ANTWORT darf NICHT behaupten, du haettest Zeit oder seist jetzt verfuegbar (z.B. NICHT "ich habe definitiv Zeit", "habe Zeit fuer dich"). Nur unverbindlich/beschaeftigt antworten (muss schauen wann es klappt, bin oft eingespannt, war die letzten Tage voll).

24) SEXUELLES SZENARIO MIT KLAREN ROLLEN (wer macht was):
- Beschreibt der Kunde ein sexuelles Szenario, in dem klar ist WER was tut (z.B. er wichst / laesst sich einen wichsen, du schaust zu; oder er macht X an dir, du machst Y an ihm; oder du ziehst seinen Schwanz in seinen Mund, er schmeckt seine Wichse)? Dann in der Interpretation EXPLIZIT festhalten: Wer ist Akteur welcher Handlung, wer schaut zu? Z.B. "Kunde beschreibt: er wichst/lässt sich wichsen, Fake schaut zu" oder "Kunde beschreibt: er will sich einen wichsen lassen, Fake soll zuschauen und ihm den Schwanz in den Mund ziehen". WICHTIG fuer Plan/Generator: Die ANTWORT darf diese Rollen NICHT vertauschen (nicht so antworten, als wuerde Fake wichsen und Kunde zuschauen, wenn der Kunde das Gegenteil beschrieben hat).
- Beschreibt der Kunde was ER an DIR (Fake) macht (z.B. deinen Nacken, deine Brüste, deine Beine erkunden, deine Klitoris massieren – "dein/deine" = dein Koerper)? Dann in der Interpretation angeben: Kunde beschreibt, dass er meinen Koerper erkundet/massiert (Nacken, Brüste, Beine, Klitoris o.ae.). WICHTIG fuer Plan/Generator: Antwort aus Fake-Sicht – von MEINEM Koerper sprechen (mein Nacken, meine Brüste), ER (Kunde) ist der Handelnde (du erkundest, du massierst). NICHT umdrehen: nicht "ich erkunde deinen Nacken/deine Brüste" – das wuerde heissen du handelst an ihm.
- DU = PERSONA (sexueller Kontext): Spricht der Kunde DICH (Fake/Persona) mit "du" an und beschreibt, was "du" tust oder was mit "dir" passiert (z.B. "wenn du schluckst", "wenn du es liebst bekommst du alle 3 Loecher gefickt", "dann bekommst du X")? Dann meint "du" IMMER die PERSONA/Fake (die Angesprochene), NICHT den Kunden. Interpretation explizit so: z.B. "Kunde beschreibt: Persona/Fake soll schlucken; wenn sie es mag, bekommt sie alle 3 Loecher." FALSCH: "Kunde beschreibt, dass er bereit ist zu schlucken" oder "Kunde schluckt" – das vertauscht die Rolle (Kunde redet ueber die Persona, nicht ueber sich).

25) KUNDE HAT BEREITS ETWAS GENANNT ODER BEANTWORTET (allgemein):
- Hat der Kunde in DIESER Nachricht bereits etwas genannt, beantwortet oder mitgeteilt (z.B. Hobbys, Beruf, Ort, Vorlieben, was er sucht, Nummer/Kontakt, Alter, Familie – egal was)? Dann in der Interpretation EXPLIZIT angeben: Kunde hat bereits [kurz was] genannt/beantwortet. WICHTIG fuer Plan/Generator: Die Antwort darf NICHT erneut nach genau dieser Sache fragen (nicht nochmal "was machst du in deiner Freizeit?" wenn er Hobbys genannt hat, nicht nochmal "woher kommst du?" wenn er den Ort genannt hat, usw.) – stattdessen darauf eingehen und andere Frage oder anderes Thema. Bei Nummer/Kontaktdaten: darauf eingehen (z.B. Policy: erst beim Treffen, lieber weiter hier schreiben), nicht ignorieren.

26) LANGE NACHRICHT MIT MEHREREN THEMEN:
- Enthaelt die Kundennachricht mehrere klar erkennbare Punkte (z.B. wenig Zeit zum Schreiben, zärtliche Wünsche wie drücken/streicheln/küssen, Wünsche erfüllen, Distanz/Fernbeziehung, Erwartungen, ob Alter eine Rolle spielt)? Dann in der Interpretation ALLE Hauptpunkte stichwortartig aufführen (nicht nur einen reduzieren wie "Kunde fragt nach Distanz"). Z.B. "Kunde erwaehnt wenig Zeit, moechte zärtlich sein (druecken, streicheln, küssen), wuerde Wünsche erfuellen, fragt nach Distanz, nach deinen Erwartungen, ob sein Alter eine Rolle spielt." WICHTIG fuer Plan/Generator: Antwort soll auf alle Punkte eingehen (kurz), nicht nur auf einen – und zärtliche/romantische Teile duerfen warm anerkannt werden, ohne zu sexualisieren.

27) KUNDE BITTET UM TERMINVORSCHLAG / TREFFEN-VORSCHLAG bzw. FRAGT NACH UNSERER ZEIT:
- Bittet der Kunde um einen Terminvorschlag, Treffenvorschlag oder fragt ob wir Zeit haben (z.B. "wuerde mich ueber einen Terminvorschlag freuen", "schlag mir was vor", "hast du am Sonntag Zeit?", "hast du Zeit um uns zu treffen?")? Dann in der Interpretation angeben: Kunde fragt nach Treffen/Zeit. WICHTIG fuer Plan/Generator: Antwort darf KEINEN konkreten Treffen-Vorschlag machen – nur UNVERBINDLICH (passt nicht, muss schauen). Schlussfrage NICHT nach Zeitpunkt/idealer Zeit. Stattdessen zum Kunden/Kontext passend: bei neutralem Ton (Essen, Kaffee) z.B. Lieblingsessen, Restaurant; bei sexuellem Ton z.B. was er als Erstes gern machen wuerde. Plan darf nicht anweisen, nach idealem Zeitpunkt zu fragen.

28) KEIN ECHO – GESPRÄCH WEITERFUEHREN (immer relevant): Die Interpretation wird von Plan und Generator genutzt. WICHTIG: Plan und Antwort duerfen das bereits im Kontext Offensichtliche NICHT wiederholen oder ausformulieren (z.B. nicht das Thema Hotel/Nacht/Lautstaerke nochmal bestaetigen oder ausschreiben, nicht "Hotel klingt super" und dann das Gleiche in anderen Worten). Gespraech WEITERFUEHREN – kurz anknuepfen, dann NEUES (eigene Reaktion, naechster Schritt, Frage). Kein Nachplappern, kein Echo des Besprochenen. In der Interpretation nicht anweisen, das Thema zu "aufgreifen" im Sinne von wiederholen; stattdessen: Antwort soll weitertreiben.

29) BEREITS GEANTWORTET – GESPRÄCH WEITERFUEHREN (generell): Hat der Kunde in DIESER Nachricht bereits auf eine Frage oder ein Thema geantwortet (z.B. etwas genannt, mitgeteilt, "und du?" gestellt – Vorlieben, Hobbys, Beruf, Ort, was auch immer)? Dann in der Interpretation angeben: Kunde hat bereits [kurz was] genannt/beantwortet. WICHTIG fuer Plan/Generator: Nicht nach "weiteren" oder "mehr" vom Gleichen fragen (nicht gleiche Art Frage nochmal – Thema nicht ausschlachten). Stattdessen Gespraech WEITERFUEHREN: darauf eingehen, dann neuer Aspekt / eigene Aussage + weiterfuehrende Frage. Gilt generell, nicht nur bei Vorlieben/Favoriten.

30) ANTWORT AUF UNSERE LETZTE FRAGE: Bezieht sich die Kundennachricht auf DEINE (Moderator/KI) letzte Nachricht im Kontext und antwortet er darauf (z.B. auf "wie lange bist du schon alleine?" mit "seit 5 Jahren Single", auf Berufsfrage mit seinem Beruf, auf Ortsfrage mit einem Ort)? Dann in der Interpretation EXPLIZIT angeben: Kunde antwortet auf unsere letzte Frage – [kurz was er geantwortet hat, z.B. Single-Dauer, Beruf, Ort]. WICHTIG fuer Plan/Generator: Diese Frage (die wir gestellt haben) NICHT nochmal stellen. Darauf eingehen, andere Schlussfrage.

31) NUR BEGRUESSUNG (wichtig fuer Antwort-Steuerung): Ist die Kundennachricht NUR eine Begruessung – z.B. "Hallo", "Hey", "Hi", "Moin", "Hallo Merle", "Hey [Name]", evtl. mit Emoji – OHNE weitere Frage, Bitte oder inhaltliche Aussage? Dann ist es eine "reine Begruessung". Sobald ZUSÄTZLICH eine Frage oder Bitte dabei ist (z.B. "Hallo, wo wohnst du?", "Hey, was machst du?", "Hi, wie heisst du?"), ist es KEINE reine Begruessung – gib die inhaltliche Bedeutung wieder und gib NICHT "NUR BEGRUESSUNG" aus. Ausgabe: Erste Zeile = interpretierte Bedeutung (z.B. "Kunde grüsst nur (Hallo Merle)."). Wenn und NUR wenn es eine reine Begruessung ist, gib in der ZWEITEN Zeile genau an: NUR BEGRUESSUNG (nur diese zwei Woerter). Bei jeder Frage oder inhaltlichen Bitte in der Nachricht – auch ohne Fragezeichen – keine zweite Zeile / kein NUR BEGRUESSUNG.

32) WORTSPIEL / ANSpielung / ZWEIDEUTIGKEIT (Kontext entscheidet): Steht im Kontext, dass DU (Moderator/Fake) zuletzt etwas zu Treffen/Zeit/Sex gesagt hast (z.B. "sobald wie möglich", "keine Lust mehr selbst Hand anzulegen", wann du Zeit hast, ob ihr euch trefft)? Und enthaelt die Kundennachricht ein Wort, das WOERTLICH etwas anderes heissen koennte (z.B. "uebersetzen" = Text uebersetzen), aber im Kontext wie eine ANSpielung auf DEINE Nachricht klingt (z.B. "wann kommst du rüber" / "wann hast du Zeit" / "was meinst du mit sobald wie möglich")? Dann ist die Bedeutung die KONTEXTUELLE: Kunde fragt nach Zeit/Treffen/Verfuegbarkeit oder spielt auf deine Formulierung an – NICHT die woertliche (z.B. NICHT "er will einen Text uebersetzen lassen"). Interpretation EXPLIZIT so ausdruecken, z.B.: "Kunde fragt, wann du Zeit hast / was du mit 'sobald wie möglich' meinst – bezieht sich auf deine letzte Nachricht (Treffen/Zeit), nicht um Text-Uebersetzung." So reagiert die Pipeline auf die richtige Bedeutung (wann Zeit, unverbindlich antworten) statt woertlich (Text schicken). Gilt generell: Bei Treffen-/Flirt-/sexuellem Kontext doppeldeutige Woerter im Sinne des Gespraechsthemas verstehen.

33) KUNDE ERKLAERT EIGENES VERSAUMNIS (Handy stumm, nicht mitbekommen, verpasst) vs. FAKE HAT GEWARTET:
- Sagt der Kunde, er habe deine Nachricht nicht mitbekommen/gesehen (Handy auf stumm/lautlos, keine Benachrichtigung, verpasst, war beschaeftigt) und meldet sich dazu (oft mit "wo bist du", "Guten Tag", Entschuldigung)? Dann bedeutet das: **Er erklaert sein eigenes Versäumnis** beim Lesen/Antworten – **nicht**, dass du ihn absichtlich warten ließest oder dass **du** dich fuer eine Wartezeit auf **seiner** Seite entschuldigen musst.
- Interpretation EXPLIZIT so ausdruecken, z.B.: "Kunde erklaert, er hat unsere Nachricht nicht mitbekommen (z.B. Handy stumm) und meldet sich wieder; fragt ggf. wo wir sind / wie es laeuft. **Er** rechtfertigt **sein** spaetes Lesen – **keine** Situation, in der die Persona sich fuer 'dass er warten musste' entschuldigen oder ein 'mein Handy hatte Schuld' der **Persona** erfinden soll."
- ANDERS: Wenn der Kunde **dir** vorwirft, du meldest dich nicht / du laesst ihn warten / du antwortest nicht → dann ist die Absicht Vorwurf an den Fake; das getrennt behandeln (nicht mit Eigen-Versaumnis verwechseln).

34) HOTEL / ZIMMER / UNTERKUNFT (gemeinsam, Kosten, buchen):
- Diskutiert oder plant der Kunde ein Hotel, Doppelzimmer, Suite, Ferienwohnung, Airbnb, Unterkunft, gemeinsam übernachten, Kosten teilen, buchen, reservieren oder ähnliches – auch nur hypothetisch, ohne "soll ich buchen?"? Dann in der Interpretation EXPLIZIT z.B.: "Kunde spricht über Hotel/Zimmer/Unterkunft (gemeinsam), ggf. Kosten/Buchung/Reservierung." WICHTIG für Plan/Generator: Antwort soll klarmachen, er soll NOCH NICHTS buchen und noch kein Geld ausgeben – erst klären ob und wann es bei beiden passt; reinschauen/Recherchieren ok. Nicht nur Suite/Hotel loben oder "klingt gut" ohne diesen Hinweis; keine Zustimmung zur Buchung oder zu bezahlten Schritten.`;
  ...
  if (contextSnippet) {
    ...
    if (isStructuredContext) {
      userContent += '\n\nHinweis: Die Zeilen oben sind mit Kunde bzw. KI/Moderator gekennzeichnet. Die Kundennachricht bezieht sich oft DIREKT auf die letzte Nachricht von KI/Moderator (z.B. "verrat es mir" = Ja, erzaehl mir davon / vom Traum; "ja" = Zustimmung zu dem, was du angeboten hast). Prüfe das zuerst – worauf antwortet er?';
    }
  }
  const isReengagement = lastPreviousCustomerMessageAgeMs != null && lastPreviousCustomerMessageAgeMs > REENGAGEMENT_THRESHOLD_MS;
  if (isReengagement) {
    const verbalPause = reengagementPauseVerbalForPrompts(lastPreviousCustomerMessageAgeMs);
    userContent += `\n\nWICHTIG – RE-ENGAGEMENT: Zwischen der vorherigen und dieser Kundennachricht lag eine laengere Pause (interne Einordnung nur: ${verbalPause} — diese Formulierung NICHT woertlich wiederholen, keine Stunden/Minuten/Tage-Zahlen). Wenn die aktuelle Nachricht dazu passt (z.B. Begruessung, Entschuldigung, Guthaben, vermisst, Zuneigung), gib in EINER Zeile den Sinn wieder, z.B. dass sich der Kunde nach laengerer Zeit wieder meldet und [kurz Inhalt]. VERBOTEN in der Interpretation: konkrete Zahlen (Stunden, Minuten, Tage), "X Stunden Pause", "521 Stunden".`;
  }
  userContent += '\n\nInterpretierte Bedeutung in der ersten Zeile ausgeben. Wenn die Nachricht NUR eine Begruessung ist (Regel 31), in der zweiten Zeile genau: NUR BEGRUESSUNG';
  ...
  try {
    ...
    const hasPureGreetingMarker = /^\s*NUR\s+BEGRUESSUNG\s*$/im.test(raw) || raw.endsWith('\nNUR BEGRUESSUNG') || raw.endsWith('\nNUR BEGRUESSUNG\n');
    let interpreted = raw
      .replace(/\n\s*NUR\s+BEGRUESSUNG\s*$/im, '')
      .replace(/^\s*NUR\s+BEGRUESSUNG\s*\n/im, '')
      .trim();
    if (interpreted && interpreted.length >= 2) {
      if (process.env.NODE_ENV !== 'production') {
        console.log('✅ Kundennachricht interpretiert:', (customerMessage.trim().slice(0, 50) + (customerMessage.length > 50 ? '…' : '')) + ' → ' + (interpreted.slice(0, 60) + (interpreted.length > 60 ? '…' : '')) + (hasPureGreetingMarker ? ' [NUR BEGRUESSUNG]' : ''));
      }
      return { interpreted, isPureGreeting: !!hasPureGreetingMarker };
    }
  } catch (err) {
...
async function classifyCurrentMessageSexuality(customerMessage, conversationHistorySnippet = '') {
  ...
  const msgSnippet = buildCustomerMessageSnippetForApi(msg, 8000);
  ...
  const systemContent = `Du bewertest die AKTUELLE (letzte) Kundennachricht in einem Dating-/Schreib-Kontakt. Kontext (vorherige Nachrichten) kann mitgegeben sein.
...
/**
 * Wählt aus ASA-Beispielen eine diverse Untermenge (verschiedene Einstiege/Formulierungen),
 * damit die KI nicht 2–3 fast identische Vorlagen sieht und die Variation besser wird.
 * Hinweis: In reply.js wird bei ASA-Fällen die Grok-Pipeline nicht aufgerufen
 * (USE_GROK_PIPELINE && !isASACalculated) – ASA läuft über multi-agent.js.
 * @param {Array<{ asaMessage?: string, moderatorResponse?: string }>} examples
 * @param {number} n - Anzahl (2 oder 3)
 * @returns {Array} n möglichst unterschiedliche Beispiele
 */
function selectDiverseASAExamples(examples, n) {
  if (!Array.isArray(examples) || examples.length === 0) return [];
  if (examples.length <= n) return [...examples];

  const getSignature = (ex) => {
    const text = (ex.asaMessage || ex.moderatorResponse || '').trim().slice(0, 120);
    return text.toLowerCase().replace(/\s+/g, ' ').trim();
  };
  const wordSet = (s) => new Set((s || '').split(/\s+/).filter(Boolean));

  const dist = (a, b) => {
    const wa = wordSet(getSignature(a));
    const wb = wordSet(getSignature(b));
  ...
  while (selected.length < n && pool.length > 0) {
    ...
    for (let i = 0; i < pool.length; i++) {
      const minDist = Math.min(...selected.map((s) => dist(s, pool[i])));
...
/**
 * Wählt ein ASA-Beispiel, das möglichst unterschiedlich zum zuletzt verwendeten ist (für multi-agent.js).
 * @param {Array<{ asaMessage?: string, moderatorResponse?: string }>} examples
 * @param {string|null} lastUsedText - Erste 120 Zeichen der zuletzt verwendeten ASA-Nachricht (oder null)
 * @returns {{ chosen: object, signatureText: string }|null}
 */
function selectOneASADiverseFromLast(examples, lastUsedText) {
  ...
  const wordSet = (s) => new Set((s || '').split(/\s+/).filter(Boolean));
  const dist = (sigA, sigB) => {
    const wa = wordSet(sigA);
    const wb = wordSet(sigB);
    if (wa.size === 0 && wb.size === 0) return 1;
    let inter = 0;
    for (const w of wa) { if (wb.has(w)) inter++; }
    const union = wa.size + wb.size - inter;
  ...
  if (!lastUsedText || !String(lastUsedText).trim()) {
    const chosen = examples[Math.floor(Math.random() * examples.length)];
    return { chosen, signatureText: getSignature(chosen) };
  }
  const lastSig = String(lastUsedText).toLowerCase().replace(/\s+/g, ' ').trim().slice(0, 120);
  let best = examples[0];
  let bestDist = -1;
  for (const ex of examples) {
    const d = dist(getSignature(ex), lastSig);
    if (d > bestDist) { bestDist = d; best = ex; }
  }
  return { chosen: best, signatureText: getSignature(best) };
}
...
function buildASAPrompt({ allRules, asaConversationContext, asaExample, asaExamples = [], doubleProfileHint = '', customerHasProfilePic = false, profileInfo = {}, extractedUserInfo = {}, customerFirstNameForAddress }) {
  ...
  const fakeName = resolveSafeFakeFirstName(moderatorInfo, extractedUserInfo?.assistant?.Name, {
    pipeline: 'buildASAPrompt'
  });
  ...
  let systemContent = MODERATOR_PERSONA + getDialogLanguageOverrideBlock(profileInfo || {}) + GENERAL_BEHAVIOR;
  ...
  systemContent += `Du antwortest auf eine System-Nachricht (Like, Kuss oder Reaktivierung). Lies den Kontext: Steht dort ausdrücklich LIKE/geliked? Dann bedanke dich NUR für den Like – niemals „Kuss“ oder „küssen“. Steht dort KUSS/geküsst? Dann bedanke dich für den Kuss – nicht für einen Like. Wenn der Kontext den Auslöser klar nennt, exakt so spiegeln.
${rulesBlock}

WICHTIG – VARIATION: Antworte jedes Mal ANDERS. Kopiere keine Saetze aus den Beispielen. Variiere den Einstieg (z.B. mal "Hey", mal "Oh", mal "Super", mal direkt Frage), die Danke-Formulierung und die Fragen (wie geht es dir / was machst du / Tag / Arbeit / Stimmung). Mindestens ${MIN_MESSAGE_LENGTH} Zeichen, wenn möglich eher knapp (ca. ${MIN_MESSAGE_LENGTH}–${MESSAGE_SOFT_LENGTH_TARGET_MAX}), nicht weitschweifig. Schreibe mit ä, ö, ü (Umlaute), z.B. wäre, möchte, für. Immer ss, nie ß. KEINE Anführungszeichen am Anfang/Ende. KEINE Bindestriche.`;
...
function buildFirstMessagePrompt({ allRules, firstMessageInstructions, profileInfo, extractedUserInfo, doubleProfileHint = '', customerFirstNameForAddress }) {
  const rulesBlock = buildRulesBlock(allRules);
  const fakeName =
    resolveSafeFakeFirstName(profileInfo?.moderatorInfo || {}, extractedUserInfo?.assistant?.Name, {
      pipeline: 'buildFirstMessagePrompt'
    }) || 'Sandy';
...
(1) Like/Kuss: Kunde hat geliked oder Kuss geschickt – er hat KEINEN Text geschrieben. NICHT "dass du dich meldest" / "dass du dich gemeldet hast". Stattdessen: kurzes Danke fuer den Like/Kuss (z.B. Das freut mich, Oh wie suess, Schoen danke, Nice danke) + 1–2 Fragen (wie geht es dir, was machst du, was gefaellt dir an mir).
...
WICHTIG: Keine Vorstellung. Schreibe mit ä, ö, ü (Umlaute), z.B. wäre, möchte, für. Immer ss, nie ß. KEINE Bindestriche. KEINE Anführungszeichen am Anfang/Ende. Nutze Zeitkontext (${weekday}, ${timePhase}). Antworte natürlich, mindestens ${MIN_MESSAGE_LENGTH} Zeichen, wenn möglich eher knapp (ca. ${MIN_MESSAGE_LENGTH}–${MESSAGE_SOFT_LENGTH_TARGET_MAX}), nicht weitschweifig.`;
...
function buildCityPrompt({ allRules, cityInstructions, customerMessage, doubleProfileHint = '', cityToUse = '', skipAskWhereCustomerLives = false, profileInfo = {} }) {
  const rulesBlock = buildRulesBlock(allRules);
  let systemContent = MODERATOR_PERSONA + getDialogLanguageOverrideBlock(profileInfo || {}) + GENERAL_BEHAVIOR;
  if (doubleProfileHint && doubleProfileHint.trim()) systemContent += doubleProfileHint.trim() + '\n\n';
  const cityRule = cityToUse && cityToUse.trim()
    ...
    : '';
  const noRedundantOrtFrage = skipAskWhereCustomerLives
    ? '\n\n🚨 HÖCHSTE PRIORITÄT: Der Kunde hat seinen Wohnort in dieser Nachricht oder im Profil bereits genannt – schließe NIEMALS mit "Woher kommst du?", "Wo wohnst du?" oder "Wo kommst du her?" ab (auch nicht als „Beispiel“). Schlussfrage = anderes Thema.\n'
    : '';
  systemContent += rulesBlock + noRedundantOrtFrage + `

WOHNORT-FRAGE: ${cityInstructions}
${cityRule}Antworte kurz (1–2 Sätze), nenne den Wohnort nur mit dem Ortsnamen – ohne Bundesland/Kanton in Klammern (z.B. "ich komme aus Rahden", nicht "Rahden (Nordrhein-Westfalen)"). Der genaue Name steht in der Anweisung. Stelle eine passende Frage zurück (siehe oben: ggf. nicht nach Kunden-Wohnort). Keine Anführungszeichen am Anfang/Ende. KEINE Bindestriche. Immer ss, nie ß.`;
...
/** Parst ein Geburtsdatum (Tag/Monat/Jahr) aus verschiedenen Formaten. */
function parseBirthDateDMY(val) {
  if (val == null) return null;
  if (typeof val === 'object') {
    const dateLike = val.date || val.birthDate || val.dob || val.value || null;
    if (dateLike && dateLike !== val) return parseBirthDateDMY(dateLike);
  }
  ...
  if (m) {
    ...
    if (year >= 1900 && year <= 2030 && month >= 1 && month <= 12 && day >= 1 && day <= 31) return { day, month, year };
  }
  // DD.MM.YYYY / DD-MM-YYYY / DD/MM/YYYY
...
function formatBirthDateDMY(dmy) {
  if (!dmy) return '';
...
function zodiacFromDayMonth(day, month) {
  const d = Number(day), m = Number(month);
  if (!d || !m) return '';
  // Sternzeichen (tropisch)
  if ((m === 3 && d >= 21) || (m === 4 && d <= 20)) return 'Widder';
  if ((m === 4 && d >= 21) || (m === 5 && d <= 20)) return 'Stier';
  if ((m === 5 && d >= 21) || (m === 6 && d <= 21)) return 'Zwillinge';
  if ((m === 6 && d >= 22) || (m === 7 && d <= 22)) return 'Krebs';
  if ((m === 7 && d >= 23) || (m === 8 && d <= 23)) return 'Löwe';
  if ((m === 8 && d >= 24) || (m === 9 && d <= 23)) return 'Jungfrau';
  if ((m === 9 && d >= 24) || (m === 10 && d <= 23)) return 'Waage';
  if ((m === 10 && d >= 24) || (m === 11 && d <= 22)) return 'Skorpion';
  if ((m === 11 && d >= 23) || (m === 12 && d <= 21)) return 'Schütze';
  if ((m === 12 && d >= 22) || (m === 1 && d <= 20)) return 'Steinbock';
  if ((m === 1 && d >= 21) || (m === 2 && d <= 19)) return 'Wassermann';
  if ((m === 2 && d >= 20) || (m === 3 && d <= 20)) return 'Fische';
  return '';
}
...
function getBerlinTimeContext() {
  ...
  const clockShort = `${String(hour).padStart(2, '0')}:${String(minute).padStart(2, '0')}`;
  const timePhase = hour >= 22 || hour < 6 ? 'Nacht' : hour >= 18 ? 'Abend' : hour >= 12 ? 'Nachmittag' : hour >= 6 ? 'Vormittag' : 'Nacht';
  return { weekday, timePhase, hour, minute, clockShort };
}
...
/** Prüft einen Text auf typische Vorstellungs-Formulierungen (Name/Ort). */
function textContainsIntroduction(text) {
...
/**
 * Erkennt, ob die Persona sich im Nachrichtenverlauf bereits vorgestellt hat (Name und/oder Ort).
 * Prüft Fake:- und KI/Moderator:-Zeilen sowie den Block "Letzte Nachricht von Fake (du):".
 * Wenn ja, sollen Interpreter, Plan, Generator und Korrektor keine erneute Vorstellung erzeugen.
...
function hasIntroducedSelfInChat(conversationHistory) {
  ...
  const moderatorPrefix = /^(Fake|KI\/Moderator):\s*/i;
  for (const line of lines) {
    if (!moderatorPrefix.test(line)) continue;
    const content = line.replace(moderatorPrefix, '').trim();
    if (textContainsIntroduction(content)) return true;
  }
  const marker = 'Letzte Nachricht von Fake (du):';
  const idx = conversationHistory.indexOf(marker);
  if (idx >= 0) {
    const after = conversationHistory.substring(idx + marker.length).trim();
    const end = after.indexOf('\n\n');
    const lastFake = (end >= 0 ? after.substring(0, end) : after).trim();
    if (lastFake.length > 0 && lastFake !== '(keine)' && textContainsIntroduction(lastFake)) return true;
  }
  return false;
}

/**
 * Extrahiert den in der letzten Moderatoren-Nachricht genannten Namen (z. B. "Ich bin die Ines" → "Ines").
 * Für AVZ: Antwort soll diesen Namen verwenden, nicht den Plattform-Username.
 * @param {string} conversationHistory
 * @returns {string|null}
 */
function extractNameFromLastModeratorMessage(conversationHistory) {
  if (!conversationHistory || typeof conversationHistory !== 'string' || !conversationHistory.trim()) return null;
  const lines = conversationHistory.trim().split(/\n/);
  const moderatorTexts = [];
  for (const line of lines) {
    if (/^(Fake|KI\/Moderator):\s*/i.test(line)) {
      moderatorTexts.push(line.replace(/^(Fake|KI\/Moderator):\s*/i, '').trim());
    }
  }
  const marker = 'Letzte Nachricht von Fake (du):';
  const idx = conversationHistory.indexOf(marker);
  if (idx >= 0) {
    ...
    const end = after.indexOf('\n\n');
    const lastFake = (end >= 0 ? after.substring(0, end) : after).trim();
    if (lastFake.length > 0 && lastFake !== '(keine)') moderatorTexts.push(lastFake);
  }
  const combined = moderatorTexts.join(' ');
  if (!combined || combined.length < 5) return null;
  const m1 = combined.match(/\bich\s+bin\s+(?:die|der)\s+([\wäöüß\-]+)/i);
  if (m1 && m1[1] && m1[1].length > 1) return m1[1].trim();
  const m2 = combined.match(/\b(?:ich\s+)?hei[ßs]e\s+([\wäöüß\-]+)/i);
  if (m2 && m2[1] && m2[1].length > 1) return m2[1].trim();
  const m3 = combined.match(/\bich\s+bin\s+([\wäöüß\-]+)(?:\s+und|\s*,|\.|$)/i);
  if (m3 && m3[1] && m3[1].length > 1 && !/^(der|die|das|ein|eine|essen|the)$/i.test(m3[1])) return m3[1].trim();
  return null;
}

/**
 * Letzter Text einer Moderatoren-/Fake-Zeile im Verlauf (Fake:, KI/Moderator:, oder Marker "Letzte Nachricht von Fake").
 * @param {string} conversationHistory
 * @returns {string}
 */
function getLastModeratorMessageText(conversationHistory) {
  if (!conversationHistory || typeof conversationHistory !== 'string' || !conversationHistory.trim()) return '';
  const lines = conversationHistory.trim().split(/\n/);
  for (let i = lines.length - 1; i >= 0; i--) {
    if (/^(Fake|KI\/Moderator):\s*/i.test(lines[i])) {
      const t = lines[i].replace(/^(Fake|KI\/Moderator):\s*/i, '').trim();
      if (t && t !== '(keine)') return t;
      break;
    }
  }
  const marker = 'Letzte Nachricht von Fake (du):';
  ...
  if (idx >= 0) {
    ...
    const end = after.indexOf('\n\n');
    const lastFake = (end >= 0 ? after.substring(0, end) : after).trim();
    if (lastFake && lastFake !== '(keine)') return lastFake;
  }
  return '';
}

/**
 * True, wenn die letzte Moderatoren-Nachricht "was suchst du" / "und was suchst du?" o. ä. enthält.
 * Dann soll die Antwort diese Frage nicht nochmal stellen (Kunde hat ggf. schon geantwortet).
 * @param {string} conversationHistory
 * @returns {boolean}
 */
function lastModeratorAskedWhatYouSearch(conversationHistory) {
  const lastPersona = getLastModeratorMessageText(conversationHistory);
  if (!lastPersona) return false;
  return /\b(und\s+)?was\s+suchst\s+du\b/i.test(lastPersona) || /\bwas\s+suchst\s+du\s+denn\b/i.test(lastPersona) || /\bwas\s+suchst\s+du\s+eigentlich\b/i.test(lastPersona);
}

/**
 * True, wenn in der letzten Fake-Nachricht Bilder/Fotos dem Kunden gegenüber angeboten oder zugesagt wurden
 * (z.B. "schicke ich dir gern noch Bilder") – dann keine technische Handy/Kamera-Ausrede bei Nachfrage.
 * @param {string} conversationHistory
 * @returns {boolean}
 */
function lastModeratorPromisedCustomerMoreImages(conversationHistory) {
  const last = getLastModeratorMessageText(conversationHistory);
  if (!last || last.length < 12) return false;
  const lower = last.toLowerCase();
  if (!/\b(bild|bilder|foto|fotos|selfie|selfies|pics?)\b/i.test(last)) return false;
  if (/\b(schick|schicke|sende|senden)\s+(mir|uns)\b/i.test(last) && /(bild|foto|selfie|pic)/i.test(last)) return false;
  if (/\b(zeig|zeige)\s+mir\b/i.test(last) && /(bild|foto|selfie)/i.test(last)) return false;
  if (/\bwillst\s+du\s+mir\s+(ein|e\s|mal)?\s*(bild|foto|selfie)/i.test(last)) return false;
  if (/\bdir\s+keine\s+(bild|foto)|schick(e|en)?\s+dir\s+keine|keine\s+bilder\s*(mehr\s+)?schick|schick(e|en)?\s+dir\s+nichts|versprech\w*\s+dir\s+keine/i.test(lower)) return false;
  return (
    /\b(ich|wir)\s+(schick|schicke|schicken|sende|senden)\s+dir\b/i.test(last) ||
    /\b(schick|schicke|schicken|sende)\s+(ich\s+)?dir\b/i.test(last) ||
    /\bdir\s+(auch\s+)?(gern(e)?\s+)?(ein\s+)?(paar\s+)?(noch\s+)?(mehr\s+)?(bild|bilder|foto|fotos|selfie|pics)\b/i.test(last) ||
    (/\bsonst\s+.{0,50}\b(schick|schicke|schicken)\b/i.test(lower) && /(bild|foto|selfie)/i.test(last)) ||
    /\b(kann|könnt|koennt|wuerde|würde|wuerd|könnte|koennte)\s+ich\s+dir\s+.{0,55}(bild|foto|selfie|was\s+sch[oö]nes|eins)\b/i.test(lower) ||
    /\bhol\s+(ich\s+)?dir\s+.{0,35}(bild|foto|selfie)/i.test(lower) ||
    (/\b(schick|schicke)\s+dir\s+.{0,45}(gern|gerne|rüber|rueber|auch|noch)\b/i.test(lower) && /(bild|foto|selfie)/i.test(last))
  );
}

/**
 * Letzte Fake-Nachricht: Foto/Bild dem Kunden anbieten oder fragen, ob eins geschickt werden soll
 * (z.B. "sollen wir dir mal ein Bild schicken", "soll ich dir ein Foto schicken", "willst du ein Selfie von mir").
 * Nicht: Kunde soll dir schicken (willst du mir ein Bild schicken).
 * @param {string} conversationHistory
 * @returns {boolean}
 */
function lastModeratorOfferedOrAskedSendPictureToCustomer(conversationHistory) {
  const last = getLastModeratorMessageText(conversationHistory);
  if (!last || last.length < 15) return false;
  if (!/\b(bild|bilder|foto|fotos|selfie|selfies|pics?)\b/i.test(last)) return false;
  if (/\b(schick|schicke|sende|senden)\s+(mir|uns)\b/i.test(last)) return false;
  if (/\b(zeig|zeige)\s+mir\b/i.test(last) && /(bild|foto|selfie)/i.test(last)) return false;
  if (/\bwillst\s+du\s+mir\s+(ein|e\s|mal)?\s*(bild|foto|selfie)/i.test(last)) return false;
  if (/\bdir\s+keine\s+(bild|foto)|schick(e|en)?\s+dir\s+keine|keine\s+bilder\s*(mehr\s+)?schick|schick(e|en)?\s+dir\s+nichts/i.test(last.toLowerCase())) return false;
  const lower = last.toLowerCase();
  if (/\b(ich|wir)\s+(schick|schicke|schicken|sende|senden)\s+dir\b/i.test(last)) return true;
  if (/\b(schick|schicke|schicken|sende|senden)\s+(ich\s+)?dir\b/i.test(last)) return true;
  if (/\b(dir|dich)\b/i.test(last) && /\b(bild|foto|selfie|pics?)\b/i.test(last) && /\b(schick|schicken|senden|zeig|zeigen|hol\s+dir|rüber|rueber)\b/i.test(lower)) return true;
  if (/\b(sollen|soll|darf|wollen|wollt)\s+(wir|ich)\b/i.test(last) && /\b(bild|foto|selfie)\b/i.test(last) && /\b(schick|schicken|senden|zeig|schicke|rüber|rueber)\b/i.test(lower)) return true;
  if (/\b(sollen|soll)\b/i.test(last) && /\b(dir|dich)\b/i.test(last) && /\b(bild|foto|selfie)\b/i.test(last)) return true;
  if (/\b(willst\s+du|willt\s+ihr)\s+.{0,45}\b(bild|foto|selfie)\b/i.test(last) && !/\bwillst\s+du\s+mir\b/i.test(lower)) return true;
  if (/\b(magst|möcht|moecht)(est|et)?\s+du\s+.{0,45}\b(bild|foto|selfie)\b/i.test(last)) return true;
  return false;
}

/**
 * Kurze Zustimmung auf ein Bild-Angebot (kein langer eigener Text).
 * @param {string} customerMessage
 * @returns {boolean}
 */
function isShortCustomerAgreementForImage(customerMessage) {
  const t = String(customerMessage || '').replace(/\s+/g, ' ').trim();
  if (!t || t.length > 100) return false;
  const lower = t.toLowerCase();
  if (/^(nein|nee|nö|nope|no|lieber\s+nicht|kein\s+interesse|lass\s+mal)\b/i.test(lower)) return false;
  if (/^(ja|jap|jup|joo|jo|ok|okay|okey|klar|mach|bitte|na\s+klar|sehr\s+gern|gern|gerne|gerne\s+doch|ja\s+gerne|jawohl|tu\s+ich|mach\s+ich|mach\s+doch|sowas\s+von|auf\s+jeden|jaa+|jaaa+)\b/i.test(lower)) return true;
  if (/^(jaa+|jaaa+)\.?$/i.test(lower)) return true;
  if (t.length <= 35 && /^(ja|ok|klar)\s+/i.test(lower) && !/\b(nein|nicht|kein)\b/i.test(lower)) return true;
  if (t.length <= 40 && /\b(gerne|gern|klar|ja|ok|mach|bitte|doch)\b/i.test(lower) && !/\b(nein|nicht|kein|keine)\b/i.test(lower)) return true;
  return false;
}

/**
 * Extrahiert den in Moderatoren-Nachrichten genannten Ort/Herkunft der Persona (z. B. "Ich komme aus Schongau" → "Schongau").
 * Verhindert, dass die KI einen anderen Ort für die Persona erfindet oder "X ist nah bei mir in Y" schreibt, obwohl die Persona aus X kommt.
 * @param {string} conversationHistory
 * @returns {string|null}
 */
function extractOrtFromVerlauf(conversationHistory) {
  if (!conversationHistory || typeof conversationHistory !== 'string' || !conversationHistory.trim()) return null;
  const lines = conversationHistory.trim().split(/\n/);
  const moderatorTexts = [];
  for (const line of lines) {
  ...
  const combined = moderatorTexts.join(' ');
  if (!combined || combined.length < 5) return null;
  const m1 = combined.match(/\b(?:ich\s+)?komme\s+aus\s+([\wäöüß\-]+)/i);
  if (m1 && m1[1] && m1[1].length > 1) return m1[1].trim();
  const m2 = combined.match(/\b(?:ich\s+)?wohne\s+in\s+([\wäöüß\-]+)/i);
  if (m2 && m2[1] && m2[1].length > 1) return m2[1].trim();
  const m3 = combined.match(/\b(?:ich\s+)?bin\s+aus\s+([\wäöüß\-]+)/i);
  if (m3 && m3[1] && m3[1].length > 1) return m3[1].trim();
  const m4 = combined.match(/\baus\s+([\wäöüß\-]+)\s+(?:komme|bin)\b/i);
  if (m4 && m4[1] && m4[1].length > 1) return m4[1].trim();
  return null;
}
...
/**
 * Extrahiert aus dem Verlauf Besitz-Fakten der Persona (Fake): Dinge, die du (Moderator) als "mein/meine" bezeichnet hast.
 * Verhindert, dass die KI in der Antwort "dein Apartment" sagt, wenn im Verlauf "mein Apartment" stand.
 * @param {string} conversationHistory - Format mit "Fake:", "KI/Moderator:" oder "Letzte Nachricht von Fake (du):"
 * @returns {string[]} Liste kurzer Phrasen, z.B. ["Apartment in Spandau", "meine Wohnung"]
 */
function extractBesitzFaktenFromVerlauf(conversationHistory) {
  if (!conversationHistory || typeof conversationHistory !== 'string' || !conversationHistory.trim()) return [];
  const lines = conversationHistory.trim().split(/\n/);
  const moderatorTexts = [];
  for (const line of lines) {
  ...
  if (idx >= 0) {
    ...
    const end = after.indexOf('\n\n');
    const lastFake = (end >= 0 ? after.substring(0, end) : after).trim();
    if (lastFake.length > 0) moderatorTexts.push(lastFake);
  }
  const combined = moderatorTexts.join(' ');
  if (!combined || combined.length < 5) return [];
  const facts = new Set();
  const meinRe = /\bmein[e]?\s+([\wäöüß\-]+)(?:\s+(?:ist\s+)?in\s+(?:der\s+)?([\wäöüß\-]+))?/gi;
  let match;
  while ((match = meinRe.exec(combined)) !== null) {
    const thing = match[1];
    const ort = match[2] || null;
    if (/^(apartment|wohnung|haus|auto|ort|zimmer|bude)$/i.test(thing) || thing.length > 3) {
      facts.add(ort ? thing + ' in ' + ort : thing);
    }
  }
  const ichHabeRe = /\b(?:ich\s+)?(?:habe|besitze)\s+(?:ein[e]?\s+)?([\wäöüß\-]+)(?:\s+in\s+([\wäöüß\-]+))?/gi;
  while ((match = ichHabeRe.exec(combined)) !== null) {
    const thing = match[1];
    const ort = match[2] || null;
    if (/^(apartment|wohnung|haus|auto|ort|zimmer)$/i.test(thing) || thing.length > 4) {
      facts.add(ort ? thing + ' in ' + ort : thing);
    }
  }
  const apartmentInRe = /\b(apartment|wohnung|haus)\s+(?:ist\s+)?(?:in\s+(?:der\s+)?)([\wäöüß\-]+)/gi;
  while ((match = apartmentInRe.exec(combined)) !== null) {
    if (match[1] && match[2]) facts.add(match[1] + ' in ' + match[2]);
  }
...
/**
 * Extrahiert den Fake-Wohnort aus dem Logbuch-Text (z.B. "Wohnort: Frankenberg", "Ort: X", "Locality: X").
 * Fallback wenn moderatorInfo.city / assistant.Wohnort nicht gesetzt ist (z.B. AvZ) – verhindert, dass die KI einen neuen Ort erfindet.
 * @param {string} logbookHint - Aus buildFakeLogbookHint
 * @returns {string} Stadt/Ort oder ''
 */
function extractWohnortFromLogbookHint(logbookHint) {
  if (!logbookHint || typeof logbookHint !== 'string') return '';
  const m = logbookHint.match(/(?:Wohnort|Ort|Locality)\s*[:\-]\s*([^\n]+?)(?=\n|$|Eintrag vom)/i);
  if (m && m[1]) {
    const city = m[1].trim().replace(/\s+/g, ' ').slice(0, 80);
    if (city.length >= 2 && isValidCityValue(city)) return city;
  }
  return '';
}

/**
 * True, wenn der String wie ein Plattform-Benutzername wirkt (z. B. Baldzuzweit, Spritzigmag), nicht wie ein Vorname.
 * Dann in Nachrichten nur Vornamen/Anzeigenamen verwenden, nie den Username.
 */
function looksLikePlatformUsername(name) {
  if (!name || typeof name !== 'string') return false;
  const s = name.trim();
  ...
  if (s.length >= 10 && /ig|tz|mag|heim|heit|ung|lich|keit/i.test(s)) return true;
  return false;
}

/** Wie reply.parseGoldNotesLabelValue: "- Name: Renate" aus Mod-/Kunden-Notizen. */
function parseNotesLabelValueForPipeline(notesRaw, label) {
  if (!notesRaw || typeof notesRaw !== 'string') return '';
  const labelNorm = String(label || '').toLowerCase().replace(/\s+/g, '');
  if (!labelNorm) return '';
  const lines = notesRaw.split(/\r?\n/);
  for (const line of lines) {
    const m = line.match(/^\s*-\s*([^:]+):\s*(.*)$/);
    if (!m) continue;
    const key = m[1].trim().toLowerCase().replace(/\s+/g, '');
    if (key !== labelNorm) continue;
    const v = (m[2] || '').trim();
    if (v) return v;
  }
  return '';
...
function firstNameTokenOnly(full) {
 ...
 * Doppelprofil bei: "X und Y", "X, Y", "X u. Y" (FPC), "X & Y", oder 3+ Namen.
 * "Angela, 56" oder "Nickname, Angela" = EINE Person, kein Doppelprofil.
 * @param {string} nameStr - name oder Vorname (roh, z.B. "Finja u. Larissa")
...
function parseMultiFakeNames(nameStr) {
  ...
  const hasUnd = /\s+und\s+/i.test(nameStr) || /\s+u\.\s+/i.test(nameStr) || /\s+&\s+/.test(nameStr);
  const parts = nameStr.split(/\s*,\s*|\s+und\s+|\s+u\.\s+|\s+&\s+/i).map(p => p.trim()).filter(Boolean);
...
/** Kunde hat sich vorgestellt und will dieselbe Info (Name/Ort/…) von der Persona – z.B. "selbe Frage zurück". */
function customerAsksSameIntroductionBack(customerMsg) {
  const t = (customerMsg || '').trim();
  if (t.length < 12) return false;
  const hasSelfIntro =
    /\b(ich\s+)?hei(ss|ß)e\s+[\wäöüß-]+/i.test(t) ||
    /\bmein\s+name\s+ist\s+[\wäöüß-]+/i.test(t) ||
    (/\bich\s+komm(e)?\s+aus\b/i.test(t) && /\b(name|hei(ss|ß)e)\s+ist\s+[\wäöüß-]+/i.test(t)) ||
    (/\bich\s+komm(e)?\s+aus\b/i.test(t) && /\bbin\s+\d{1,3}\s*jahr/i.test(t));
  const asksMirror =
    /\b(selbe|gleiche)\s+frage\s+zur(u|ü)ck/i.test(t) ||
    /\bfrage\s+zur(u|ü)ck\b/i.test(t) ||
    /\b(dasselbe|das\s+gleiche)\s+für\s+dich\b/i.test(t) ||
    /\bnow\s+your\s+turn\b/i.test(t);
  return hasSelfIntro && asksMirror;
}

/**
 * Wenn der Generator einen falschen Vornamen in Vorstellungsformeln nutzt (z.B. Halluzination aus Beispielen),
 * durch den Profil-Vornamen ersetzen. Nur typische Muster, keine Sätze wie "ich bin müde".
 */
function correctWrongFakeIntroName(message, fakeFirstName, fakeNamesList) {
  if (!message || typeof message !== 'string' || !fakeFirstName || typeof fakeFirstName !== 'string') return message || '';
  const canon = fakeFirstName.trim();
  if (canon.length < 2) return message;
  const allowedLower = new Set(
    [canon, ...(Array.isArray(fakeNamesList) ? fakeNamesList : [])]
      .map(s => String(s || '').trim())
      .filter(Boolean)
      .map(s => s.toLowerCase())
  );
  const skipWords = /^(single|frei|hier|da|auch|noch|so|total|sehr|gerade|echt|leider|happy|super|jung|alt|dabei|dankbar|neugierig|gespannt|bereit)$/i;
  const cap = (w) => w.charAt(0).toUpperCase() + w.slice(1).toLowerCase();
  const good = cap(canon);

  let out = message;
  const isNameLikeToken = (tok) => {
    if (!tok || typeof tok !== 'string') return false;
    const t = tok.trim().toLowerCase();
    if (!t) return false;
    if (skipWords.test(t)) return false;
    // Sehr strikt: nur Tokens, die wie Vornamen aussehen (aus unseren Namens-Sets oder aus erlaubten Profilnamen).
    // Damit ersetzen wir niemals Adjektive/Normalwörter, selbst wenn sie groß/klein durcheinander sind.
    if (allowedLower.has(t)) return true;
    if (FEMALE_NAMES.has(t) || MALE_NAMES.has(t)) return true;
    return false;
  };
  // Nur "Vorstellung" anfassen: "Ich bin X," / "Ich bin X." / "Ich bin X!" / "Ich bin X?" / "Ich bin X und ..."
  // Nicht anfassen bei normalen Sätzen wie "Ich bin am Wochenende ...", "Ich bin schon verplant", "Ich bin total müde".
  out = out.replace(/\b(ich|wir)\s+(bin|sind)\s+([A-ZÄÖÜa-zäöüß]{2,23})(?=\s*(?:[,.!?]|$|\bund\b))/gi, (full, subj, cop, name) => {
    const nl = name.toLowerCase();
    if (!isNameLikeToken(name)) return full;
    if (allowedLower.has(nl)) return full;
    return `${subj} ${cop} ${good}`;
  });
  out = out.replace(/\b(ich\s+)?hei(ss|ß)e\s+([A-ZÄÖÜa-zäöüß]{2,23})(?=\s*(?:[,.!?]|$|\bund\b))/gi, (full, ich, _ss, name) => {
    const nl = name.toLowerCase();
    if (!isNameLikeToken(name)) return full;
    if (allowedLower.has(nl)) return full;
    const prefix = ich ? `${ich.trimEnd()} ` : '';
    return `${prefix}heisse ${good}`;
  });
  return out;
}
...
function buildDoubleProfileHint(fakeIsPlural, fakeNamesList) {
  ...
  hint += `IDENTITAETEN: Die Namen [${namesStr}] sind EUERE Vornamen in diesem Profil – in jedem Kontext (Anrede, Frage, Scherz, Story, expliziter Text). Wenn der Kunde einen oder mehrere dieser Namen nennt oder "euch" sagt, meint er fast immer EUCH; antworte ALS diese Person(en), nicht ueber euch in der dritten Person, und verwechsle das nicht mit einer Selbstvorstellung DES Kunden.\n`;
  hint += `EIGENE NAMEN NICHT WIE FREMDE FORMULIEREN: Wenn der Kunde z.B. einen Dreier oder ein Szenario mit euch anspricht und dabei [${namesStr}] nennt, seid ihr das – nicht zwei andere Leute. VERBOTEN: eure Namen in der gleichen Weise wie der Kunde als Dritte zu formulieren (z.B. „Ein Dreier mit ${namesStr} stelle ich mir auch geil vor") – dann klingen ${namesStr} wie Fremde, obwohl ihr das seid. Richtig aus EUCHER Sicht: „mit uns", „mit uns beiden", „dafuer mit uns", „stell wir uns das auch geil vor", „darauf mit uns" – immer klar: ihr seid die Gemeinten.\n`;
  hint += `NAMEN ≠ KUNDE: Der Kunde heisst nicht [${namesStr}] – das seid ihr. Auch wenn er eure Namen im gleichen Satz mit Orten, Handlungen oder Andeutungen nennt: das bezieht sich auf euch oder auf ein Gespraechsthema MIT euch, solange er nicht ausdruecklich sich selbst mit anderem Namen vorstellt (z.B. ich heisse …, ich bin … aus …). NIEMALS so antworten, als haette er sich mit euren Namen „vorgestellt" oder als waeren eure Namen seine Identitaet; keine Kennlern-Reaktion („Hallo, schoen dass du dich vorstellst … ich aus … du aus …") auf eure eigenen Namen im Kundentext biegen – das ist Rollenverwechslung.\n`;
  const firstN = fakeNamesList[0] || namesStr.split(',')[0].trim();
  hint += `VORSTELLUNG / NAMENSFRAGE: Fragt der Kunde nach eurem Namen oder will dieselbe Kennenlern-Runde (Name, Alter, Ort, Beruf wie er)? Nutzt NUR die Profil-Vornamen [${namesStr}] – niemals einen erfundenen Namen. Ein Vorname reicht (z.B. "${firstN}"). Alter und Beruf nur aus Profil/Logbuch, nichts erfinden.\n\n`;
  return hint;
}
...
function buildNormalPrompt({
  ...
  customerClaimsWeAgreedOrWantsAddress = false,
  ...
  besitzFaktenHint = [],
  fakeOrtFromVerlauf = null,
  customerAsksForFakeAge = false,
  customerAsksForFakeBirthday = false,
  customerAsksForFakeZodiac = false,
  fakeBirthdayDate = '',
  fakeZodiac = '',
  customerOccupationInRente = false,
  customerHasNoProfilePic = false,
  fakeCityFromProfile = null,
  moderatorPromisedMoreImages = false,
  profileInfo = null
}) {
  const situationHasBilderAnfrage = !!(situationRulesBlock && situationRulesBlock.includes('Bilder Anfrage'));
  const bildAngebotKundeJaVorfreude = lastModeratorOfferedOrAskedSendPictureToCustomer(conversationHistory || '') && isShortCustomerAgreementForImage(customerMessage);
  const lengthGuidanceModerator = `LAENGE: Mindestens ${MIN_MESSAGE_LENGTH} Zeichen bleiben Pflicht. Schreibe im Normalfall kompakt, Zielgroesse oft etwa ${MIN_MESSAGE_LENGTH} bis ca. ${MESSAGE_SOFT_LENGTH_TARGET_MAX} Zeichen, wenn die Kundennachricht es hergibt. Keine Fuellschleifen, keine doppelte Aussage desselben. Nur deutlich laenger, wenn mehrere wichtige Punkte oder Fragen der Kundennachricht kurz bedient werden muessen (natuerlich im Fluss, keine Abhakliste).\n\n`;
  let systemContent = MODERATOR_PERSONA + getDialogLanguageOverrideBlock(profileInfo || {}) + GENERAL_BEHAVIOR + lengthGuidanceModerator + PRIORITY_NOTE + ONE_TASK_PER_MESSAGE + FOCUS_LAST_MESSAGE_GENERATOR + GESPRACHSFUEHRUNG_LETZTE_NACHRICHT;
  systemContent += 'ALTE THEMEN NICHT RECYCLEN: Fuege KEINE Themen aus frueheren Nachrichten neu ein, wenn sie in der AKTUELLEN Kundennachricht nicht vorkommen. Beispiel: Hat ein frueherer Moderator schon zum Geburtstag gratuliert und die aktuelle Nachricht spricht den Geburtstag nicht an, dann NICHT nochmal gratulieren. Kontext dient nur zum Verstehen, nicht zum Wiederaufwaermen alter Punkte.\n\n';
  if (customerHasNoProfilePic) {
    systemContent += 'PROFILBILD KUNDE: Der Kunde hat KEIN Profilbild. Keine Aussagen die implizieren du findest ihn optisch ansprechend (z.B. "gefaellst mir auch", "siehst gut aus", "mag wie du aussiehst"). Bei Sympathie/Kompliment NUR auf Art, Ehrlichkeit, Schreibstil beziehen (z.B. "deine Ehrlichkeit gefaellt mir", "deine Art mag ich", "wie du schreibst finde ich sympathisch").\n\n';
  }
  ...
  if (fakeAge != null && Number(fakeAge) >= 18 && Number(fakeAge) <= 120) {
    ...
    if (customerAsksForFakeAge) {
      systemContent += `ALTERSFRAGE (PFLICHT): Der Kunde fragt nach DEINEM Alter. Du MUSST mit dem Profil-Alter antworten (z.B. "ich bin ${Number(fakeAge)} Jahre"), NICHT ausweichen oder verraten/neugierig zurueckfragen – die Frage direkt beantworten.\n\n`;
    }
  } else {
  ...
  // Geburtstag / Sternzeichen: nur nennen, wenn der Kunde explizit danach fragt.
  if (customerAsksForFakeBirthday && fakeBirthdayDate) {
    systemContent += `GEBURTSTAG (Fake-Profil, PFLICHT BEI FRAGE): Dein Geburtstag/Geburtsdatum im Profil ist ${sanitizeForApiContent(String(fakeBirthdayDate))}. Wenn der Kunde nach deinem Geburtstag/Geburtsdatum fragt: GENAU dieses Datum nennen, nichts anderes erfinden.\n\n`;
  }
  if (customerAsksForFakeZodiac && fakeZodiac) {
    systemContent += `STERNZEICHEN (Fake-Profil, PFLICHT BEI FRAGE): Dein Sternzeichen ist ${sanitizeForApiContent(String(fakeZodiac))}. Wenn der Kunde nach Sternzeichen/Horoskop fragt: GENAU dieses Sternzeichen nennen.\n\n`;
  }
  const customerAsksForFakeName =
    /\b(wie\s+hei[ßs]t\s+du|wie\s+ist\s+dein\s+name|was\s+ist\s+dein\s+name|dein\s+name\??|vorname\??|wie\s+hei[ßs]en\s+sie|wer\s+bist\s+du)\b/i.test(customerMessage || '') ||
    customerAsksSameIntroductionBack(customerMessage || '');
  if (nameStr) {
    // Root-Cause Fix: Den Fake-Namen NICHT in den Prompt injizieren, ausser wenn der Kunde explizit danach fragt.
    // Sonst nutzen Modelle den Namen als Schablone fuer "Ich bin <Name> ..." selbst wenn keiner danach gefragt hat.
    if (customerAsksForFakeName) {
      systemContent += `DEIN NAME (Fake-Profil): ${sanitizeForApiContent(nameStr)}. Wenn der Kunde nach deinem Namen fragt, NUR diesen Namen nennen – keinen anderen.\n\n`;
      systemContent += 'NAME IN NACHRICHTEN (WICHTIG): Wenn du deinen Namen nennst, dann NUR den Vornamen/Anzeigenamen (z. B. ' + sanitizeForApiContent(nameStr) + ') – niemals den Plattform-Login/Username.\n\n';
      if (!(doubleProfileHint && doubleProfileHint.trim())) {
        systemContent += `NAMEN-ROLLE (EINZELPROFIL): Wenn der Kunde deinen Vornamen "${sanitizeForApiContent(nameStr)}" in seiner Nachricht nutzt, meint er in der Regel DICH – unabhaengig davon, ob es locker, neckisch, sachlich oder explizit klingt. Nur bei klarer Selbstvorstellung des Kunden (z.B. ich heisse …, ich bin … aus … mit anderem Namen) ist das nicht dein Name.\n\n`;
      }
    } else {
      systemContent += 'NAME (WICHTIG): Deinen Namen NICHT von selbst nennen. Nur wenn der Kunde in der AKTUELLEN Nachricht explizit nach deinem Namen fragt.\n\n';
    }
  }
  ...
  const fakeCityStr = (typeof fakeCityFromProfile === 'string' && fakeCityFromProfile.trim()) ? fakeCityFromProfile.trim() : '';
  if (fakeCityStr && fakeCityStr.toLowerCase() !== 'sag ich später') {
    systemContent += `DEIN WOHNORT (Fake-Profil): ${sanitizeForApiContent(fakeCityStr)}. Wenn du in der Nachricht einen Ort nennst (wo du wohnst, woher du kommst), NUR diesen Ort nennen – NIEMALS einen anderen erfinden oder suchen. Kein anderer Stadtname.\n\n`;
  }
  ...
  const besitzFakten = Array.isArray(besitzFaktenHint) ? besitzFaktenHint : (besitzFaktenHint ? [besitzFaktenHint] : []);
  if (besitzFakten.length > 0) {
    const list = besitzFakten.map(f => (typeof f === 'string' && f.trim()) ? f.trim() : null).filter(Boolean);
    if (list.length > 0) {
      systemContent += `BESITZ-FAKTEN (PFLICHT): Folgende Dinge gehoeren DIR (der Persona/Fake), NICHT dem Kunden: ${list.join('; ')}. In deiner Antwort fuer diese Dinge NIEMALS "dein/deine" verwenden – immer "mein/meine" (z.B. "mein Apartment in Spandau", nicht "dein Apartment"). Der Kunde hat diese Dinge nicht; du hast sie im Verlauf bereits als deine bezeichnet.\n\n`;
    }
  }
  const ortFromVerlauf = (typeof fakeOrtFromVerlauf === 'string' && fakeOrtFromVerlauf.trim()) ? fakeOrtFromVerlauf.trim() : null;
  if (ortFromVerlauf) {
    systemContent += `DEIN ORT/HERKUNFT AUS DEM VERLAUF (PFLICHT): Du hast im Gespraech bereits gesagt, dass du aus ${sanitizeForApiContent(ortFromVerlauf)} kommst bzw. dort wohnst. In der Antwort NICHT behaupten, du wohnst woanders (z.B. anderer Ort) oder "${sanitizeForApiContent(ortFromVerlauf)} sei nah bei dir in [anderer Ort]" – du kommst aus ${sanitizeForApiContent(ortFromVerlauf)}. Bleib bei diesem Ort.\n\n`;
  }
  ...
  if (isReengagementGreeting) {
    systemContent += 'REINE BEGRUESSUNG / NEUES GESPRÄCH (HÖCHSTE PRIORITÄT): Der Kunde schreibt nur eine Begruessung (z.B. Hey, Hi, Hallo, Hallo [dein Name]). Reagiere NUR mit kurzer Begruessung zurueck (Hallo, Hey) und 1–2 normalen Tagesfragen (wie gehts dir, was machst du gerade, wie ist dein Tag). VERBOTEN: "freut mich dass du [Name] sagst", "gut dass du Merle sagst" oder aehnliche Meta-Kommentare zum Namen – der Kunde gruesst nur, nicht kommentieren. VERBOTEN: Themen aus dem Verlauf aufgreifen (Abenteuer, Kuss, vorherige Fragen, was du zuvor geschrieben hast) – bei reiner Begruessung NUR Hallo + Tagesfragen, nichts aus aelteren Nachrichten.\n\n';
  } else if (isPureGreetingMessage) {
    systemContent += 'REINE BEGRUESSUNG: Die Kundennachricht ist nur eine Begruessung (Hey, Hi, Hallo, Hallo [dein Name] o.ae.). Reagiere NUR mit kurzer Begruessung zurueck (Hallo, Hey) und 1–2 normalen Tagesfragen (wie gehts dir, was machst du gerade). VERBOTEN: "freut mich dass du [Name] sagst", "gut dass du Merle sagst" oder Kommentare zum Namen – nur zurueckgruessen. VERBOTEN: Etwas aus dem Nachrichtenverlauf aufgreifen (Abenteuer, Kuss, Bock auf, vorherige Themen) – die AKTUELLE Nachricht ist nur Hallo, also NUR Hallo + Tagesfragen, kein Bezug auf aeltere Nachrichten.\n\n';
  } else if (noPriorModeratorMessage) {
    systemContent += 'ERSTE NACHRICHT DES KUNDEN: Der Kunde hat dir die erste Nachricht geschrieben (es gab noch keine vorherige Nachricht von dir vom Profil aus). Begruessung am Anfang (Hey, Hallo, Hi) ist angebracht – du darfst mit Hey oder Hallo beginnen.\n\n';
  }
  ...
  if (isContactRequest) {
    systemContent += 'KONTAKTDATEN: An Trainingsdaten orientieren (dort ist gezeigt, wie ausweichen). Kunde fragt nach Nummer/WhatsApp/Telegram etc. – persoenlich ausweichen (z.B. Nummer nicht so schnell rausgeben, mag ich lieber hier schreiben, schreib mir gern weiter hier). VERBOTEN an den Kunden: "nicht erlaubt", "Regeln", "Plattform", "echte Kontaktdaten verboten/duerfen nicht" – wirkt wie System-Regel (Wort "Plattform" gegenueber Kunden meiden). VERBOTEN: Telegram, WhatsApp, Instagram oder anderen Messenger anbieten (z.B. NICHT "du kannst mir auf Telegram schreiben") – bleib natuerlich beim Schreiben hier, ohne "Chat" oder "Plattform" zu sagen. Nicht sexualisieren, mindestens eine thematisch passende Frage in der Nachricht.\n\n';
  }
  if (customerClaimsWeAgreedOrWantsAddress) {
    systemContent += 'KUNDE BEHAUPTET WIR HABEN ZUGESAGT / WILL ADRESSE: Der Kunde behauptet, wir haetten gesagt oder geschrieben dass wir heute Zeit haben (Treffen/Sex) oder verlangt unsere Adresse mit diesem Verweis (z.B. weil ein anderer Moderator/Betreiber etwas Falsches geschrieben hat). NICHT bestaetigen, NICHT "ja ich schick dir die Adresse", NICHT so tun als staende das Treffen. Stattdessen hoeflich rausreden: z.B. mir ist was dazwischen gekommen, heute passt es leider nicht mehr, muss was Dringendes erledigen, kurzfristig hat sich was geaendert – in eigenen Worten variieren. NIEMALS Adresse nennen oder zusagen. Auf seinen Wunsch kurz eingehen, dann absagen und thematisch passende Frage.\n\n';
  }
  ...
  if (imageAlreadySentToCustomer) {
    ...
    if (moderatorPromisedMoreImages && situationHasBilderAnfrage) {
      systemContent += 'ZUSATZ (Bild schon da + du hattest mehr angeboten): Der Kunde will jetzt (weitere) Bilder – obiger Absatz verbietet nur, so zu tun als waere kein Bild angekommen. Fuer die Bitte nach MEHR gilt trotzdem BILDER ZUVOR ANGEBOTEN: flirty ausweichen, KEINE Handy/Kamera-Ausrede.\n\n';
    }
  }
  if (moderatorSentImageDescription && moderatorSentImageDescription.trim()) {
    systemContent += 'DU HAST DEM KUNDEN FOLGENDES BILD GESCHICKT: ' + sanitizeForApiContent(moderatorSentImageDescription.trim()) + '. Beziehe dich in deiner Antwort darauf.\n\n';
  }
  if (customerOccupationInRente) {
    systemContent += EXTRA_RULES.kundeInRente;
  }
  if (situationRulesBlock && situationRulesBlock.trim()) {
    ...
    if (situationRulesBlock.includes('Bilder Anfrage')) {
      systemContent += moderatorPromisedMoreImages
        ? 'HINWEIS Bilder Anfrage (du hattest Bilder angeboten): Der Kunde fragt nach Fotos/Bildern – du hattest in deiner LETZTEN Nachricht (weitere) Bilder in Aussicht gestellt. KEINE technische Ausrede (Handy, Kamera, nichts Schoenes auf dem Handy). Stattdessen FLIRTY/SPIELERISCH: z.B. erst verlockend hinhalten, Tausch-Gedanke, er soll sich was einfallen lassen, ob du von ihm auch mal eins bekommst – warm, ohne Frust und ohne Zusage zum sofortigen Versand. Thematisch passende FRAGE. NICHT auf Treffen ausweichen.\n\n'
        : 'HINWEIS Bilder Anfrage: Der Kunde fragt nach Fotos/Bildern von dir. Antworte wie in den TRAININGSBEISPIELEN: freundlich AUSWEICHEN mit AUSREDE (z.B. gerade nicht, Kamera spinnt, keine guten Fotos auf dem Handy, schau mal ob ich was Gutes finde) – KEINE generelle Absage ("ich schicke keine Bilder"). Dann thematisch passende FRAGE (z.B. ob du auch eins von ihm bekämest, was er damit machen würde). NICHT auf Treffen ausweichen. Stil und Art der Gegenfrage aus den Beispielen uebernehmen.\n\n';
    }
  }
  ...
  if (locationContext && (locationContext.fakeCity || locationContext.customerCity)) {
    ...
    if (locationContext.customerCity) {
      ...
      systemContent += `\n🚨 ENTFERNUNG/WOHNORT – KEINE NEGATIVE BEWERTUNG: Sage NIEMALS, dass der Wohnort des Kunden "zu weit", "weit weg", "zu weit entfernt" oder "ungünstig" sei. Keine Beschwerden/Absagen wegen Entfernung. Wenn du Entfernung überhaupt erwähnst, dann neutral/positiv (z.B. "klingt gut", "passt doch") oder lass es komplett unkommentiert.`;
    }
    systemContent += '\n\n';
  }
  ...
  if (isReengagementGreeting) {
    ...
    systemContent += 'PLAN "Gespraech weiterfuehren": Steht im Plan nur "Reaktion: Gespraech weiterfuehren" (oder aehnlich allgemein)? Dann heisst das: ZUERST auf die LETZTE (aktuelle) Kundennachricht eingehen und davon aus weiterspinnen. NICHT das thematisch letzte Thema aus dem Verlauf (z.B. Fantasien, Treffen) unveraendert fortsetzen – die Antwort bezieht sich auf das, was der Kunde JETZT geschrieben hat.\n\n';
  }
  ...
  if (unclearReferentAskBack) {
    systemContent += 'UNKLARER BEZUG (erste Nachricht oder ohne Kontext): Die Kundennachricht enthaelt einen Bezug (z.B. "dagegen", "dafuer", "das", "es"), aber im Nachrichtenverlauf gibt es keinen klaren Bezugspunkt – es ist die erste Nachricht oder der Verlauf liefert keinen Referenten. Reagiere wie ein Mensch: freundlich nachfragen, womit genau er/sie meint (z.B. "Ich verstehe nicht ganz, gegen was hast du nichts dagegen?", "Wobei meinst du genau?", "Worauf beziehst du dich?"). NICHT raten oder deuten – eine kurze, lockere Nachfrage stellen.\n\n';
  }
  if (alreadyIntroducedSelfInChat) {
    systemContent += 'BEREITS VORGESTELLT: Du hast dich in diesem Gespraech bereits mit Name und/oder Ort vorgestellt. Stelle dich NICHT erneut vor (kein erneutes "ich bin X", "wohne in Y", "komme aus Z"). Die Frage "was suchst du?" / "und was suchst du?" hast du ggf. bereits gestellt – nicht nochmal stellen; der Kunde hat ggf. schon geantwortet. Antworte inhaltlich auf die AKTUELLE Kundennachricht (z.B. Termin-Anfrage, Frustration, "und jetzt?"); hoechstens kurzer Verweis ("wie ich schon sagte").\n\n';
    systemContent += 'WOHNORT KONSISTENZ: Hast du im Verlauf bereits einen Wohnort/Ort genannt (z.B. "ich bin aus X", "wohne in X", "komme aus X")? Dann in dieser Antwort NIEMALS einen ANDEREN Ort nennen – nur denselben Ort wie im Verlauf verwenden oder die Ortsnennung weglassen. Einen neuen Ort erfinden ist verboten.\n\n';
  }
  ...
  if (hasConversationContext) {
    systemContent += 'WOHNORT AUS VERLAUF: Enthaelt der Nachrichtenverlauf eine Nachricht von dir (Fake/Persona), in der du einen Wohnort/Ort genannt hast (z.B. "ich bin aus X", "wohne in X", "komme aus X")? Dann in deiner Antwort DENSELBEN Ort verwenden oder die Ortsnennung weglassen. NIEMALS einen anderen Ort erfinden oder nennen – das waere widersprüchlich.\n\n';
    systemContent += `KONTEXT ANWENDEN (aus dem Kernprinzip): Bevor du antwortest – worauf bezieht sich die aktuelle Nachricht im Verlauf? (1) Antwort auf DEINE letzte Nachricht oder das gerade besprochene Thema? (2) Klarstellung/Abschwaechung ("nur X", "nichts Schlimmes")? → passend reagieren, kein Trost wo keiner noetig ist. (3) Kurze Nachricht ("ja", "ok", "gut")? → fast immer Bezug auf das zuletzt Gesagte, im selben Kontext antworten. (4) Wunsch/Hoffnung (z.B. "gehofft du waerst dagegen", "dass wir X haetten")? → darauf eingehen, nicht generisch ausweichen. (5) Fragt der Kunde danach, wo/wann/was DU gesagt oder getan hast (z.B. "wo warst du?", "wo bist du an mir vorbei gejoggt?", "du hast gesagt...")? → Im Verlauf steht, was du zuvor gesagt hast. Aus DEINER Sicht antworten (ich war..., ich habe..., ich jogge oft in X), bestaetigen oder mitspielen, dann Gegenfrage. NIEMALS die Rolle vertauschen (nicht ihn fragen "wo warst du?" wenn er dich gefragt hat wo du warst). RICHTUNG WER WAS GESAGT HAT: Hast DU (Fake) im Verlauf einen Wunsch oder eine Aussage geaeussert (z.B. Sex/Naehe, Einladung), und der Kunde geht darauf ein oder bestaetigt? Dann in der Antwort NICHT dem Kunden diesen Wunsch/diese Aussage zuschreiben (nicht "dein Wunsch", "dass du X willst" wenn du es warst) – stattdessen "meinen Wunsch", "dass du darauf eingehst". Umgekehrt: Sagt der Kunde, was ER mit dir/von dir will (z.B. deine Pisse trinken, dich lecken, deinen Koerper), dann bezieht sich "dein/deine" in seiner Nachricht auf DICH (Persona). In deiner Antwort diese Richtung NICHT umdrehen – nicht so antworten, als wolltest du dasselbe mit seinem Koerper/seinem (z.B. nicht "deine Pisse trinken waere geil" wenn er meinte er will DEINE Pisse trinken). Kontext ignorieren = falsche Reaktion.\n\n`;
...
PFLICHT: Nur eine Nachricht ausgeben; mindestens eine Frage in der Nachricht (wo auch immer); keine Meta-Kommentare; KEIN ECHO – nicht Name, Alter, Ort oder Vorlieben des Kunden zurueckspielen, eigenstaendig formulieren.`;
  ...
  if (conversationHistory && conversationHistory.trim()) {
    ...
    userContent += `AKTUELLE KUNDENNACHRICHT (NUR DARAUF ANTWORTEN): "${sanitizeForApiContent(currentMsg)}"\n\n`;
    userContent += `Deine Antwort bezieht sich NUR auf diese eine Nachricht oben. Fruehere Kunden-Nachrichten im Verlauf (z.B. Wohnort, Tagesfreizeit) sind bereits beantwortet – NICHT darauf antworten.\n\n`;
    userContent += `Generiere deine nächste Nachricht in diesem Dialog. Die letzte Zeile im Dialog und die obige AKTUELLE KUNDENNACHRICHT sind identisch – antworte ausschliesslich darauf.\n\n`;
  } else {
    const currentMsgOnly = (customerMessage || '').trim() || '(leer)';
    userContent += `AKTUELLE KUNDENNACHRICHT (NUR DARAUF ANTWORTEN): "${sanitizeForApiContent(currentMsgOnly)}"\n\n`;
    userContent += `Generiere deine nächste Nachricht (Fake). Antworte ausschliesslich auf diese eine Kundennachricht.\n\n`;
  }
  if (plan && String(plan).trim()) {
    ...
    userContent += `Die Antwort MUSS diesem Plan entsprechen. Kernthema = nur das, was im Plan steht (z.B. Bereitschaft, Zeit, wo?). Kein anderes Thema (Wohnort, Fotos, etc.) zum Hauptthema machen.\n\n`;
    if (/RICHTUNG\s*:/i.test(planStr)) {
      userContent += `PFLICHT RICHTUNG AUS PLAN: Der Plan enthaelt eine Zeile "RICHTUNG:". Die Antwort muss diese Richtung EINHALTEN – dem Kunden NICHT zuschreiben, was im Plan als DEINE (Fake) Aussage oder DEIN Wunsch steht (z.B. nicht "dein Wunsch nach Sex" wenn du den Wunsch geaeussert hast und er darauf eingeht – stattdessen "meinen Wunsch", "dass du darauf eingehst"). Keine Rollen vertauschen.\n\n`;
    }
  }
  ...
  if (asksProductLikeOnImageUser) {
    userContent += 'Der Kunde fragt nach etwas WIE AUF DEM BILD. Deine Antwort muss sich auf den BILDINHALT (siehe Beschreibung oben) beziehen, NICHT auf andere im Gespraech erwaehnte Produkte oder Themen.\n\n';
  }
  ...
  userContent += `KRITISCH – KONSISTENZ: Lies den Nachrichtenverlauf oben – was hast DU (Fake/Moderator) bereits ueber dich gesagt (z.B. was du isst, machst, wo du bist)? Bleib dabei – wechsle nicht zu etwas anderem. Wenn du "Nudeln mit Pesto" gesagt hast, sag nicht "Omelett" oder "Spaghetti Bolognese".\n\n`;
...
const SITUATION_DEFINITIONS_LLM = {
  'Treffen/Termine': 'Wählen, wenn der Kunde KONKRET ein reales Treffen/Date/gemeinsame Zeit meint (wann/wo treffen, sehen, vorbeikommen, bei dir/bei mir, Date, Café, Ort/Adresse). Bei mehrdeutigen Phrasen wie "heute Nachmittag weiter machen": Nutze Kontext. Wenn der Kunde gerade erkennbar nur weg muss (Termin/Arbeit/Familie/unterwegs) und dann "später/nachher/heute Nachmittag weiter machen" sagt, ist das sehr wahrscheinlich "später weiter schreiben" → dann NICHT Treffen/Termine. Nur wenn Treffen-Signale da sind oder der Kontext klar Richtung reales Treffen ging, Treffen/Termine wählen. Bei echter Unklarheit lieber Treffen/Termine (sicherer Modus: nicht zusagen) als sicher "weiter schreiben" zu behaupten.',
  ...
  'Moderator/Outing': 'Kunde fragt ob du Moderator(in) eines Profils oder aehnlich bist.',
  ...
  'Telefonat/Videoanruf': 'Kunde schlaegt oder fragt nach einem Telefonat, Videoanruf oder "Kamera beim Telefonieren" – z.B. telefonieren, wann passt es zum Telefonieren, Videoanruf, Kamera beim Telefon, mal anrufen, dich anrufen. Antwort darf KEIN Telefonat/Videoanruf vereinbaren; freundlich ausweichen (z.B. lieber weiter schreiben, bin hier fuer dich beim Schreiben).'
};
...
async function detectSituationsWithLLM(customerMessage, conversationHistorySnippet = '') {
  ...
  const messageSnippet = buildCustomerMessageSnippetForApi((customerMessage || '').trim(), 8000);
  const userContent = contextSnippet
...
WICHTIG MEHRDEUTIG "weiter machen": "heute Nachmittag weiter machen" / "später weiter" / "nachher weiter" kann beides sein. Nutze Kontext:
- Wenn der Kunde gerade weg muss (Termin, Arbeit, Familie, unterwegs) und dann "später/nachher/heute Nachmittag weiter machen" sagt, ist das sehr wahrscheinlich "später weiter schreiben" → dann NICHT Treffen/Termine.
- Nur wenn Treffen-Signale (treffen/sehen/vorbeikommen/bei dir/bei mir/Date/Kaffee/Ort/Adresse) oder klarer Treffen-Kontext vorhanden ist → Treffen/Termine.
- Bei echter Unklarheit lieber Treffen/Termine (sicherer Modus: nicht zusagen) statt sicher "weiter schreiben" zu behaupten.
...
WICHTIG Telefonat/Videoanruf: Wenn der Kunde ein Telefonat, Videoanruf oder "Kamera beim Telefonieren" vorschlaegt oder danach fragt (z.B. telefonieren, wann passt es zum Telefonieren, Videoanruf, Kamera beim Telefon, mal anrufen) → IMMER "Telefonat/Videoanruf" waehlen, damit die Antwort KEIN Telefonat/Video vereinbart.
...
// ========== Intent-Disambiguierung (LLM, feiner als Situationen) ==========

/**
 * Entscheidet die Hauptintention der letzten Kundennachricht, v.a. bei mehrdeutigen Zeitphrasen.
 * Gibt JSON zurück: { intent, confidence, evidence }.
 * intent ∈ ["chat_continue","availability_timing","real_meetup","unclear"]
 */
async function detectCustomerIntentWithLLM(customerMessage, conversationHistorySnippet = '') {
  if (!customerMessage || typeof customerMessage !== 'string' || !customerMessage.trim()) {
    return { intent: 'unclear', confidence: 0, evidence: 'empty' };
  }
  const contextSnippet = (conversationHistorySnippet || '').slice(-900).trim();
  const messageSnippet = buildCustomerMessageSnippetForApi((customerMessage || '').trim(), 8000);
  const userContent = contextSnippet
    ? `Kontext (Verlauf – Moderator und Kunde, letzte Nachrichten):\n${sanitizeForApiContent(contextSnippet)}\n\nAktuelle Kundennachricht:\n${sanitizeForApiContent(messageSnippet)}`
    : `Aktuelle Kundennachricht:\n${sanitizeForApiContent(messageSnippet)}`;
  const messages = [
    {
      role: 'system',
      content: `Du liest die letzte Kundennachricht in einem Dating/Flirt-Schreibkontakt und musst die INTENTION verstehen.

Gib NUR ein JSON-Objekt im Format:
{"intent":"chat_continue|availability_timing|real_meetup|unclear","confidence":0.0,"evidence":"kurzer Grund"}

Definitionen:
- chat_continue: Kunde meint weiter schreiben/weiter hier schreiben oder ähnlich (auch wenn er "schreiben" nicht wörtlich sagt), typischer Kontext: er muss kurz weg (Termin/Arbeit/Familie/unterwegs) und will später weiter machen.
- availability_timing: Kunde spricht über Zeit/Verfügbarkeit/Timing allgemein (z.B. "später", "heute Nachmittag", "nachher"), ohne klar zu sagen ob weiter schreiben oder Treffen gemeint ist. Das ist oft mehrdeutig.
- real_meetup: Kunde deutet klar auf reales Treffen/Date/zusammen etwas machen/bei dir/bei mir/Adresse/Ort/Café hin oder fragt explizit nach wann/wo treffen.
- unclear: nicht sicher einordenbar.

WICHTIG:
- Wenn echte Treffen-Signale vorkommen (treffen/sehen/vorbeikommen/bei dir/bei mir/Date/Kaffee/Ort/Adresse), dann eher real_meetup.
- Wenn der Kunde weg muss (z.B. Schwester treffen, Arzttermin, Arbeit) und dann "nachher/später/heute Nachmittag weiter machen" sagt, ist oft chat_continue.
- Wenn du nicht sicher bist: availability_timing oder unclear (nicht raten).
Antworte nur mit JSON.`
  ...
  try {
    ...
    const trimmed = (raw || '').trim();
    const jsonMatch = trimmed.match(/\{[\s\S]*\}/);
    const obj = jsonMatch ? JSON.parse(jsonMatch[0]) : JSON.parse(trimmed);
    ...
    const allowed = new Set(['chat_continue', 'availability_timing', 'real_meetup', 'unclear']);
    return {
    ...
    if (process.env.NODE_ENV !== 'production') console.warn('⚠️ Intent-Detection (OpenAI) fehlgeschlagen:', err.message);
    return { intent: 'unclear', confidence: 0, evidence: 'error' };
  }
}
...
function getDetectedSituations(customerMessage, allRules) {
  ...
  if (/\b(telefon|telefonieren|videoanruf|kamera\s+beim\s+telefon|mal\s+anrufen|dich\s+anrufen|wann\s+passt\s+es\s+(zum\s+)?telefon)\b/i.test(lower)) {
    out.push('Telefonat/Videoanruf');
  }
...
/**
 * Kunde behauptet, wir/Fake haetten heute Zeit/Treffen/Sex zugesagt oder verlangt unsere Adresse mit Verweis darauf
 * (z.B. Betreiber/dummer Moderator hat geschrieben "heute Zeit zum Ficken" oder "wir haben heute Zeit").
 * Dann darf die Antwort das NICHT bestaetigen und keine Adresse nennen – hoeflich rausreden.
 * @param {string} customerMessage - aktuelle Kundennachricht (Original)
 * @returns {boolean}
 */
function customerClaimsWeAgreedTodayOrWantsAddress(customerMessage) {
  if (!customerMessage || typeof customerMessage !== 'string') return false;
  const lower = customerMessage.trim().toLowerCase();
  const hasAdresse = /\badresse\b/i.test(lower);
  const hasRefToUsSaid = /\b(geschrieben|gesagt|ihr habt|du hast|hast ja|habt ja|weil (wir|du|ihr)|dass (wir|du|ihr)|das (wir|du|ihr))\b/i.test(lower);
  const hasTodayTimeFicken = /\b(heute|zeit|ficken|fick|treffen|vorbeikommen|komm(e)?\s+(vorbei|zu\s+dir)|fahr(e)?\s+los)\b/i.test(lower);
  if (hasAdresse && (hasRefToUsSaid || hasTodayTimeFicken)) return true;
  if (hasRefToUsSaid && (/\bheute\b.*\b(zeit|ficken|treffen|vorbei)\b/i.test(lower) || /\b(zeit|ficken|treffen|vorbei)\b.*\bheute\b/i.test(lower))) return true;
  if (/\b(schick|schreib)(\s+mir)?\s+(die\s+)?(deine?|eure?|unsere?)\s+adresse\b/i.test(lower) && hasRefToUsSaid) return true;
  return false;
}
 ...
 * @param {number|null} [lastPreviousCustomerMessageAgeMs] - Alter der letzten Kunden-Nachricht in ms; bei >12h: Re-Engagement-Hint (Gespraech von heute weiterfuehren, nicht an gestriges Thema anknuepfen)
 * @param {boolean|null} [isPureGreetingFromInterpreter] - Von Interpreter: true = nur Begruessung, false = nicht, null = Fallback Heuristik
 * @param {string|null} [fakeGenderForPlan] - Profil/Geschlecht Fake (wie buildGenderHint), fuer kompakten Plan-Hinweis
 * @param {string|null} [customerGenderForPlan] - Profil/Geschlecht Kunde
 */
async function runPlanningStep(customerMessage, detectedSituations, allRules, conversationHistory = '', alreadyIntroducedSelfInChat = false, currentMessageNotClearlySexual = false, currentMessageHasImage = false, customerClaimsWeAgreedOrWantsAddress = false, lastPreviousCustomerMessageAgeMs = null, customerOccupation = null, isPureGreetingFromInterpreter = null, fakeGenderForPlan = null, customerGenderForPlan = null) {
  ...
  const sexualHint = Array.isArray(detectedSituations) && detectedSituations.some(s => (s || '').toLowerCase().includes('sexuell'))
    ? ' Bei Sexuelle Themen: auf sexuelle Inhalte und Fragen eingehen, nicht ausweichen – Ton und Regeln der Situation Sexuelle Themen beachten. RICHTUNG AUS KUNDENTEXT: Beschreibt der Kunde was ER an DIR (Fake) macht (z.B. deinen Nacken, deine Brüste, deine Klitoris erkunden/massieren)? Dann im Plan angeben: Antwort aus Fake-Sicht – mein/meine für meinen Koerper (mein Nacken, meine Brüste), Kunde = Handelnder (du erkundest, du massierst). NICHT umdrehen (nicht "ich erkunde deinen Nacken/deine Brüste" – das waere Rolle vertauscht).'
    : '';
  ...
  const romanticHint = hasRomantik && !hasSexuell
    ? ' Bei Romantik/Verliebtheit: warm, romantisch, flirty antworten – keine expliziten sexuellen Formulierungen. Enthaelt die Kundennachricht mehrere Punkte (z.B. Zeit, Zuneigung/Druecken/Streicheln/Küssen, Distanz, Erwartungen, Alter)? Plan: Auf ALLE eingehen (kurz), nicht nur auf Distanz oder Alter. Zärtliche Wünsche (drücken, Wangen streicheln, küssen) duerfen warm anerkannt werden (z.B. "das klingt schoen"), ohne zu sexualisieren – dann seine Fragen (Distanz, Erwartungen, Alter) und passende Gegenfrage.'
    : '';
  ...
  const moderatorPromisedMoreImagesForPlan = lastModeratorPromisedCustomerMoreImages(historyStr);
  const hasBilderAnfrageSituationPlan = Array.isArray(detectedSituations) && detectedSituations.some(s => (s || '').includes('Bilder'));
  const bilderVersprechenPlanHint = moderatorPromisedMoreImagesForPlan && hasBilderAnfrageSituationPlan
    ? ' BILDER-VERSPRECHEN (PFLICHT): In deiner LETZTEN Nachricht hast du dem Kunden (weitere) Bilder/Fotos angeboten; er fragt jetzt danach (Situation Bilder Anfrage). Plan: Reaktion = weder technische Ausrede (Handy kaputt, Kamera spinnt, nichts Schoenes auf dem Handy) noch konkrete Zusage zum sofortigen Versand ("schick dir gleich", "klar mache ich", "kann dir welche schicken"). Stattdessen flirty/spielerisch verzoegern (Spannung, erst verlocken, Tausch-Gedanke – ob du von ihm auch mal eins bekommst) – warm, nicht frustrierend. Unter "Vermeiden": Zusage jetzt/sofort mehrere Bilder zu schicken.'
    : '';
  const isStructuredPlanContext = /^(Kunde|KI\/Moderator):\s/m.test(historyStr);
  const contextSnippet = isStructuredPlanContext
    ? historyStr.slice(-2200)
    : historyStr.slice(-700);
  const historyLines = historyStr.split(/\n/).filter(Boolean);
  let lastPersonaLineForPlan = '';
  for (let i = historyLines.length - 1; i >= 0; i--) {
    if (/^(Fake|KI\/Moderator):\s*/i.test(historyLines[i])) {
      lastPersonaLineForPlan = historyLines[i].replace(/^(Fake|KI\/Moderator):\s*/i, '').trim();
      break;
    }
  }
  if (!lastPersonaLineForPlan) {
    const markerPlan = 'Letzte Nachricht von Fake (du):';
    const idxPlan = historyStr.indexOf(markerPlan);
    if (idxPlan >= 0) {
      const afterPlan = historyStr.substring(idxPlan + markerPlan.length).trim();
      const endPlan = afterPlan.indexOf('\n\n');
      lastPersonaLineForPlan = (endPlan >= 0 ? afterPlan.substring(0, endPlan) : afterPlan).trim();
      if (lastPersonaLineForPlan === '(keine)') lastPersonaLineForPlan = '';
    }
  }
  ...
  const hasUndDuRueckfrage = /\bund\s+du\??\b/i.test(customerSnippet);
  const hasUndDuAlsNamensfrage = (hasUndDuRueckfrage && /\b(ich\s+)?hei[ßs]e\s+[\wäöüß-]+|(ich\s+bin\s+)[\wäöüß-]+|(name|hei[ßs]e)\s+[\wäöüß-]+/i.test(customerSnippet)) || customerAsksSameIntroductionBack(customerSnippet);
  const hasNameQuestionInclUndDu = hasNameQuestion || hasUndDuAlsNamensfrage;
  ...
  const hasErwartungsOrMetaFrage = /\b(was\s+erwartet\s+(mich|mir)|was\s+habe\s+ich\s+(davon|dabei)|was\s+bekomme\s+ich\s+(davon)?|wie\s+meinst\s+du\s+(das)?|was\s+hei[sß]t\s+(das|denn)\s+(für mich|eigentlich)?|was\s+bedeutet\s+(das|denn)|was\s+heisst\s+(das|denn)|was\s+soll\s+ich\s+mir\s+davon\s+halten|worauf\s+läuft\s+das\s+raus)\b/i.test(customerSnippet);
  const hasDirectQuestionToPersona = hasNameQuestionInclUndDu || hasWieGehtEsDir || hasTreffenFrage || hasWasMachstDuHeute || hasWasSollIchMitDirPlan || hasUndDuRueckfrage || hasErwartungsOrMetaFrage || /\b(für dich|für dich\?|was macht .* für dich|was suchst du|wie oft|was wäre für dich|was findest du|was ist für dich|besonders schön für dich)\b/i.test(customerSnippet) || (/\?/.test(customerSnippet) && /\b(dich|dir|du)\b/i.test(customerSnippet));
  ...
  const directQuestionHint = hasDirectQuestionToPersona
    ? ' PFLICHT – FRAGE ZUERST BEANTWORTEN: Die aktuelle Kundennachricht enthaelt eine direkte Frage an die Persona (z.B. wie geht es dir, wie heisst du, was machst du / was machst du heute – oder "was erwartet mich eigentlich?", "was habe ich davon?", "wie meinst du das?" – oder "was soll ich mit dir machen?", Treffen). Plan: Die Antwort muss ZUERST mit 1–2 Saetzen diese Frage beantworten (Einstieg = Beantwortung der Frage, kein Einstieg auf anderes Thema). Danach ggf. Gegenfrage oder Weiterfuehrung. Bei "und du?" / "und du": Rueckfrage BEANTWORTEN (Name/Vorlieben nennen). Bei "was soll ich mit dir machen" / "was wuerdest du dir wuenschen": ZUERST inhaltlich beantworten (1–2 konkrete Vorschlaege), NICHT nur zurueckfragen oder Meta-Kommentar. Bei "was machst du heute"/"was hast du vor": kurz etwas Passendes nennen; bei Treffen-Frage unverbindlich. KEIN Einstieg auf altes Thema (z.B. tabulos, vorherige Nachricht), wenn er JETZT etwas anderes fragt.'
    : '';
  ...
  const meetingHint = hasTreffen && hasConcreteOrtZeit
    ? ' Bei Treffen: Kunde hat konkreten Ort und/oder Zeit genannt (z.B. morgen 09:00, Aldi Parkplatz). Plan darf NICHT vorschlagen "Klingt gut" oder "Morgen klingt gut" am Anfang – das wirkt wie Zusage. Stattdessen: Einstieg mit Abschwaechung (wuerde mir gefallen, aber... / haette Lust, aber...), dann unverbindlich (muss schauen ob es klappt). Schlussfrage nach Absage: NICHT nach Zeitpunkt/idealer Zeit. Stattdessen zum Kunden passend: bei neutralem Ton (Essen, Kaffee) z.B. Lieblingsessen, Restaurant; bei sexuellem Ton z.B. was er als Erstes gern machen wuerde. Im Plan angeben: welche Art Schlussfrage (kontextangemessen).'
    : '';
  const hasHotelUnterkunftDiskussion =
    /\b(hotel|motel|hostel|pension|airbnb|unterkunft|doppelzimmer|doppel\s*zimmer|hotelzimmer|fewo|ferienwohnung|übernachtung|uebernachtung)\b/i.test(customerSnippet) ||
    (/\b(zimmer|suite)\b/i.test(customerSnippet) &&
      /\b(buchen|reservier|gebucht|kosten\s*teilen|hälfte|haelfte|wir\s+beide|mit\s+dir|mit\s+mir\s+zusammen|zusammen\s+.*\b(bezahl|kost))\b/i.test(customerSnippet));
  const hotelUnterkunftPlanHint = hasHotelUnterkunftDiskussion
    ? ' HOTEL/UNTERKUNFT – NOCH NICHT BUCHEN (PFLICHT): Kunde spricht über Hotel/Zimmer/Suite/Unterkunft für ein gemeinsames Treffen, Kosten teilen, Buchen oder Reservieren. Plan: Antwort MUSS klar machen: noch NICHTS buchen, noch kein Geld ausgeben – erst klären ob und wann es bei beiden passt. Reinschauen/Recherchieren ok, keine Buchung/Reservierung. NICHT nur "klingt gut" oder Suite loben ohne diesen Hinweis. VERMEIDEN: Zustimmung zur Buchung, zu konkreten Kosten, oder so tun als wäre Buchen der nächste Schritt.'
    : '';
  ...
  const imageProductHint = hasImageProductQuestion
    ? ' Kunde fragt nach etwas wie auf dem Bild / so ein Teil: Plan muss angeben, dass die Antwort sich auf den BILDINHALT (was auf dem Bild zu sehen ist) beziehen muss, NICHT auf andere im Gespraech erwaehnte Produkte oder Themen.'
    : '';
  ...
  const alreadyIntroducedHint = alreadyIntroducedSelfInChat
    ? ' WICHTIG – BEREITS VORGESTELLT: Du (die Persona) hast dich in diesem Gespraech bereits vorgestellt (Name und/oder Ort). Vermeiden: Keine erneute Vorstellung – nicht nochmal "ich bin X", "wohne in Y", "komme aus Z" sagen. Inhaltlich antworten, hoechstens kurz verweisen ("wie ich schon sagte").'
    : '';
  const lastModeratorAskedWhatSearch = lastModeratorAskedWhatYouSearch(conversationHistory || '');
  const wasSuchstDuBereitsHint = (alreadyIntroducedSelfInChat || lastModeratorAskedWhatSearch)
    ? ' WICHTIG – "WAS SUCHST DU?" BEREITS GESTELLT: Du hast in deiner letzten Nachricht bereits "was suchst du?" / "und was suchst du?" gefragt und der Kunde hat darauf geantwortet (z.B. "Die Wahrheit!!!", oder inhaltlich). Plan: Diese Frage NICHT nochmal stellen. Auf die AKTUELLE Kundennachricht eingehen (z.B. Termin-Anfrage, Frustration "und jetzt?", Treffen) – keine erneute Vorstellung, keine Wiederholung der Frage was er sucht.'
    : '';
  ...
  const notClearlySexualHint = currentMessageNotClearlySexual
    ...
    : '';
  const currentMessageAsksForFakeAge = /\b(wie\s+alt\s+bist\s+du|wie\s+alt\s+bist\s+du\s+eigentlich|dein\s+alter|wie\s+alt\s+du\s+bist|wie\s+alt\s+seid\s+ihr)\b/i.test((customerMessage || '').trim());
  const altersfragePlanHint = currentMessageAsksForFakeAge
    ? ' ALTERSFRAGE (PFLICHT): Der Kunde fragt nach DEINEM Alter (wie alt bist du, etc.). Reaktion = Alter NENNEN (Profil-Alter verwenden, z.B. "ich bin X Jahre"), NICHT ausweichen, NICHT nur Neugier zeigen oder zurueckfragen – die Antwort muss die Frage direkt beantworten.'
    : '';
  const hasReturnDateNotMeeting = /\bbis\s+(spätestens\s+)?(\d{1,2}\.?\s*)(oder\s*)?\d{0,2}\.?\s*wieder\s+in\s+\w+/i.test(customerSnippet) || /\bbis\s+.*?(wieder\s+in|zurück|wieder\s+da)\b/i.test(customerSnippet) || /\bwieder\s+in\s+\w+\s+(sein|will|zu\s+sein)\b/i.test(customerSnippet);
  const returnDatePlanHint = hasReturnDateNotMeeting
    ? ' RUECKKEHRTERMIN (PFLICHT): Der Kunde nennt einen Zeitpunkt, BIS ZU DEM er wieder an einem Ort sein will (Rueckkehrtermin), NICHT einen Treffvorschlag fuer diesen Tag. Plan: Antwort darf NICHT so tun, als haette er ein Treffen am 24./26./o.ae. angefragt (z.B. NICHT "Am 24./26. hab ich leider Schicht"). Stattdessen: auf seinen Rueckkehrtermin eingehen (z.B. Starthilfe/Auto, Treffpunkt, Einladung) und DEINEN Schichtplan/Dienst antworten, wenn er danach fragt.'
    : '';
  const hasShiftQuestionNotPastMeeting = /\b(dienst|schicht|welchen\s+dienst|welche\s+schicht)\b/i.test(customerSnippet) && /\b(diese\s+woche|vergangen[e]?\s+woche|letzte\s+woche|wie\s+(die\s+)?vergangene)\b/i.test(customerSnippet);
  const shiftQuestionPlanHint = hasShiftQuestionNotPastMeeting
    ? ' SCHICHTFRAGE (PFLICHT): Der Kunde fragt nach DEINEM Dienst/Schicht diese Woche (evtl. ob wieder wie vergangene Woche). Das ist eine Frage nach deinem ARBEITSPLAN/Schichtplan, NICHT nach einem Treffen letzte Woche. Plan: Antwort mit Schichtzeiten/Dienst antworten. VERMEIDEN: "Letzte Woche war der Hammer, lass uns wiederholen", "Was hat dir letzte Woche am besten gefallen?" – er hat nicht nach einem vergangenen Treffen gefragt.'
    : '';
  const kundeInRentePlanHint = isCustomerOccupationRente(customerOccupation)
    ? ' KUNDE IN RENTE (PFLICHT): Kundenberuf ist Rente/Pension. Plan: Antwort darf NICHT nach seiner Arbeit, seinem Job oder was ihm an der Arbeit Freude macht fragen (nicht "Was macht dir an deiner Arbeit Freude?", "Was machst du beruflich?"). Stattdessen: Frage nach Freizeit, Hobbys, wie er den Tag verbringt – oder andere thematisch passende Frage.'
    : '';
  const hasVerabschiedungAufgeben = Array.isArray(detectedSituations) && detectedSituations.some(s => (s || '').includes('Verabschiedung'));
  const verabschiedungHint = hasVerabschiedungAufgeben
    ? ' WICHTIG – VERABSCHIEDUNG/AUFGABEN (REGEL): Der Kunde verabschiedet sich oder gibt auf. Du darfst dich NICHT verabschieden und den Kunden NICHT gehen lassen. Reaktion im Plan: freundlich und einladend bleiben, signalisieren dass du ihn nicht so schnell aufgeben willst. Ggf. auf Abschiedsgrund eingehen (z.B. Alter: "ach komm, ist dir das wirklich so wichtig?"). VERBOTEN im Plan: "keine weiteren Versuche", "Absage" als Reaktion, "Kunde zieht sich zurück" mit Reaktion aufgeben – stattdessen: einladend reagieren, nicht verabschieden.'
    : '';
  // Allgemein: Wer hat was geäußert (Wunsch, Aussage) – Richtung im Plan festhalten, damit der Generator Rollen nicht vertauscht
  const richtungPlanHint = ' RICHTUNG (PFLICHT wenn relevant): Wenn aus dem Verlauf klar ist, WER was geaeussert hat (z.B. DU/Fake hast einen Wunsch oder eine Aussage gemacht – z.B. nach Sex/Naehe, Einladung, Angebot – und der Kunde geht darauf ein oder bestaetigt): Gib im Plan eine Zeile "RICHTUNG: [Wer = Subjekt des Wunsches/der Aussage], [wer = reagiert darauf]. In der Antwort: [nicht dem Kunden zuschreiben was dein Wunsch/deine Aussage war]." – z.B. "RICHTUNG: Ich (Fake) habe Wunsch/Naehe geaeussert, Kunde geht darauf ein. In der Antwort: nicht dem Kunden diesen Wunsch zuschreiben (nicht dein Wunsch), sondern meinen Wunsch / dass er auf mich eingeht." Diese Richtung muss die Antwort einhalten – keine Rollen vertauschen.';
  const attributionPlanHint = ' ATTRIBUTION / SPRECHER (immer pruefen): Wer entschuldigt sich wofuer, wer hat was nicht mitbekommen? Erklaert der Kunde in der AKTUELLEN Nachricht, dass ER deine Nachricht nicht mitbekommen/gelesen hat (Handy stumm/lautlos, keine Benachrichtigung, verpasst, beschaeftigt) – ggf. mit Rueckfrage "wo bist du"? Dann im Plan ausdruecklich: Antwort **nicht** so, als muesse die Persona sich fuer "dass **du** (Kunde) warten musstest" entschuldigen; **nicht** die Schuld auf **unser** Handy/Geraet legen, wenn **er** sein Geraet/Stumm nennt. Stattdessen: verstaendnisvoll (kein Stress), kurz da, auf seine Frage/Thema eingehen – **keine** falsche Rollenumkehr. Wenn der Kunde uns vorwirft zu spaet/nicht zu schreiben: das ist andere Lage (nicht mit Eigen-Versaumnis verwechseln). Bei Unsicherheit im Plan kurz festhalten: wer hat wem welches Verhalten erklaert.';
  const hasTelefonVideo = Array.isArray(detectedSituations) && detectedSituations.some(s => (s || '').includes('Telefonat/Videoanruf'));
  const phoneVideoHint = hasTelefonVideo
    ? ' TELEFONAT/VIDEOANRUF (KEIN ANRUF ZUSAGEN): Der Kunde fragt/schlaegt Telefonat oder Videoanruf vor (z.B. Kamera beim Telefon, wann telefonieren). Plan: Antwort darf KEIN Telefonat/Video vereinbaren – nicht "wann passt es zum Telefonieren" fragen, nicht zusagen. Auf andere Teile seiner Nachricht eingehen (z.B. Arbeitszeiten), freundlich ausweichen (weiter schreiben, ohne "Chat"/"Plattform" zum Kunden), keine Terminfrage fuer Anruf.'
    : '';
  const niemalsGetroffenPlanHint = ' NIEMALS GETROFFEN (immer): Kunde und Fake haben sich NIE gesehen und werden sich nie sehen. Plan und Antwort duerfen KEIN "wiedersehen", "dich endlich sehen", "in deinen Armen (sein)", "dich in die Arme nehmen" oder aehnliches – nichts, was ein reales Treffen oder koerperliche Begegnung andeutet. Vorfreude und Gefuehle nur beim geschriebenen Austausch formulieren (z.B. "freue mich wenn du schreibst", "bin gern hier fuer dich"), ohne Woerter "Chat" oder "Plattform" zum Kunden, keine Formulierung als ob ihr euch trefft oder getroffen haettet.';
  const keinEchoPlanHint = ' KEIN ECHO – GESPRÄCH WEITERFUEHREN (immer): Die Antwort soll das Gespraech WEITERFUEHREN, nicht das bereits Gesagte oder aus dem Kontext Offensichtliche wiederholen/ausformulieren. Kurz anknuepfen, dann eigene Reaktion / naechster Schritt / Frage – kein Nachplappern des Themas, kein Bestaetigen von Dingen die schon klar sind (z.B. nicht "Hotel klingt super" und dann dasselbe Thema nochmal ausschreiben). Im Plan angeben: Antwort weitertreiben, kein Echo.';
  const hasKundeFreiKeinePlaene = /\b(frei|keine\s+pl[aä]ne|nichts\s+vor|habe?\s+(heute\s+)?(abend\s+)?(noch\s+)?(keine\s+)?pl[aä]ne|habe?\s+nichts\s+vor|bin\s+frei|habe?\s+zeit|hatte?\s+noch\s+keine\s+pl[aä]ne)\b/i.test(customerSnippet);
  const keinIndirekterTreffenPlanHint = hasKundeFreiKeinePlaene
    ? ' KUNDE IST FREI / HAT KEINE PLAENE (PFLICHT): Der Kunde signalisiert, er sei frei oder habe keine Plaene (z.B. heute Abend). Plan: Antwort darf KEINE indirekten Treffen-Vorschlaege enthalten – NICHT "wir verbringen eine schoene Zeit zusammen", "kuscheln", "uns naeher kennenlernen", "was hast du dir fuer uns ueberlegt?" – das wuerde der Kunde als Einladung/Treffen verstehen. Fake ist fast immer beschaefitgt (z.B. bin eingespannt, hab viel um die Ohren) ODER neutrale Fragen stellen (wie geht es dir, was machst du so, Tag, Arbeit) – OHNE Anknuepfen an gemeinsame Verabredung.'
    : '';
  const hasKundeBittetUmTerminvorschlag = /\b(terminvorschlag|termin\s*vorschlag|schlag\s+mir\s+(was\s+)?vor|vorschlag\s+f[uü]r\s+(ein\s+)?treffen|w[uü]rde\s+mich\s+(ueber|über)\s+(einen\s+)?termin\s+freuen|hast\s+du\s+(einen\s+)?vorschlag|vorschl[aä]ge?\s+(von\s+dir|fuer\s+uns)|im\s+ruhestand.*tags[uü]ber)\b/i.test(customerSnippet);
  const keinTerminvorschlagPlanHint = hasKundeBittetUmTerminvorschlag
    ? ' KUNDE BITTET UM TERMINVORSCHLAG (PFLICHT): Der Kunde bittet um einen Termin-/Treffen-Vorschlag (z.B. "schlag mir was vor", "Terminvorschlag", "wuerde mich ueber einen Termin freuen"). Plan: Antwort darf KEINEN konkreten Treffen-Vorschlag vorgeben – NICHT "Kaffee treffen vorschlagen", NICHT "wie waere es wenn wir uns treffen" – nur UNVERBINDLICH (z.B. muss schauen wann es passt, wuerde mir gefallen aber …). Generator-Anweisung im Plan darf nicht lauten, einen Treffen-Vorschlag zu machen.'
    : '';
  const bereitsGenanntPlanHint = ' BEREITS GENANNTES NICHT ERNEUT ERFRAGEN (immer): Hat der Kunde in seiner AKTUELLEN Nachricht bereits etwas genannt oder beantwortet (z.B. Hobbys, Beruf, Ort, Vorlieben, Nummer/Kontakt, was er sucht, Alter – egal was)? Dann im Plan angeben: nicht nochmal danach fragen – darauf eingehen, andere Frage oder anderes Thema. Keine Doppelfrage (nicht "was machst du in deiner Freizeit?" wenn er gerade Hobbys genannt hat, nicht "woher kommst du?" wenn er den Ort genannt hat, usw.).';
  const bereitsBeantwortetWeiterfuehrenPlanHint = ' BEREITS GEANTWORTET – GESPRÄCH WEITERFUEHREN (immer, generell): Hat der Kunde in seiner Nachricht bereits etwas genannt oder eine Frage beantwortet (egal welches Thema – Vorlieben, Hobbys, Beruf, Ort, etc.)? Plan: Antwort NICHT nach "mehr" oder "weiteren" vom Gleichen fragen (nicht gleiche Art Frage nochmal – Thema nicht ausschlachten). Stattdessen Gespraech WEITERFUEHREN: darauf eingehen, dann neuer Aspekt / eigene Aussage + weiterfuehrende Frage. Schlussfrage: weiterfuehrend, nicht dasselbe Thema nochmal abfragen. Gilt generell.';
  const frageAusEigenerLetzterNichtWiederholenPlanHint = ' FRAGE AUS EIGENER LETZTER NACHRICHT NICHT WIEDERHOLEN (immer): Enthielt DEINE (Moderator) letzte Nachricht im Verlauf eine konkrete Frage an den Kunden (z.B. wie lange alleine, Beruf, Ort, was machst du)? Dann antwortet der Kunde in seiner AKTUELLEN Nachricht oft genau darauf. Plan: Diese Frage NICHT nochmal in der Antwort stellen – darauf eingehen, andere Schlussfrage. Keine Doppelfrage (nicht "wie lange bist du schon alleine?" wenn er gerade "seit X Jahren Single" gesagt hat, nicht dieselbe Frage wie in unserer letzten Nachricht).';
  const isReengagementPlan = lastPreviousCustomerMessageAgeMs != null && lastPreviousCustomerMessageAgeMs > REENGAGEMENT_THRESHOLD_MS;
  const reengagementPlanHint = isReengagementPlan
    ? (() => {
        const verbalPause = reengagementPauseVerbalForPrompts(lastPreviousCustomerMessageAgeMs);
        return ` RE-ENGAGEMENT (PFLICHT): Kunde meldet sich nach laengerer Pause wieder (intern: ${verbalPause} — in der geplanten Antwort KEINE Stunden-, Minuten- oder exakten Tageszahlen; hoechstens nebenbei freuen dass er wieder schreibt, nicht die Dauer breit ausrollen). Plan: Reaktion = kurz positiv auf sein Wieder-Melden; Gespraech von HEUTE weiterfuehren (z.B. wie gehts dir, was machst du heute, wie war dein Tag) – NICHT an letzte Moderator-Nachricht (gestriges Thema, z.B. Sofa/Handy/Pool) anknuepfen. SCHLUSSFRAGE: Nicht zu einem Thema aus unserer letzten Nachricht (gestriges Thema wie Pool, Sofa, Handy). Stattdessen: zu HEUTE oder zu dem was der Kunde JETZT will (z.B. Treffen/Zeit: was haettest du vor, was stellst du dir vor). VERBOTEN im Plan: Formulierungen wie "nach X Stunden" oder konkrete Zeitmengen.`;
      })()
    : '';
  const isPureGreetingForPlan = isPureGreetingFromInterpreter === true || (isPureGreetingFromInterpreter === false ? false : isPureGreeting(customerSnippet));
  const pureGreetingPlanHint = isPureGreetingForPlan
    ? ' REINE BEGRUESSUNG (PFLICHT): Die Kundennachricht ist nur eine Begruessung (z.B. Hallo, Hey, Hallo [Name]). Plan: Antwort = NUR kurze Begruessung zurueck (Hallo, Hey) + 1–2 Tagesfragen (wie gehts dir, was machst du gerade). VERMEIDEN: "freut mich dass du [Name] sagst", Kommentare zum Namen, und JEGLICHES Aufgreifen aus dem Verlauf (Abenteuer, Kuss, Bock auf, vorherige Themen) – bei nur Hallo NUR Hallo + Tagesfragen.'
    : '';
  const berlinForPlan = getBerlinTimeContext();
  const timeCoherencePlanHint =
    berlinForPlan.hour >= 12
      ? ` TAGESZEIT-KOHÄRENZ (PFLICHT): Referenz Europe/Berlin: ${berlinForPlan.weekday}, ${berlinForPlan.timePhase}, ${berlinForPlan.clockShort} Uhr. Die Antwort darf NICHT so formulieren, als waere oder kaeme GERADE der Morgen (z.B. "mein Morgen ist gleich viel besser", "der Morgen ist schoener") – ab Mittag ist das widerspruechlich. Plan: "mein Tag", "heute", "Nachmittag/Abend" passend, oder neutral.`
      : berlinForPlan.hour < 6
        ? ` TAGESZEIT: Nacht/frueh (${berlinForPlan.clockShort} Uhr). Kein "schoener Morgen"/Tagesstart als JETZT formulieren.`
        : '';
  const schlussfrageHypothesePlanHint =
    ' FRAGE & ANKER & TREFFEN-HYPOTHESE (PFLICHT): (1) Mindestens eine konkrete Frage fuer die Antwort planen – sie muss NICHT die letzte Phrase sein (darf in der Mitte stehen). Frage muss an die LETZTE Kundennachricht anknuipfen (ANKER-TEST: nicht austauschbare Vorlage); keine banale Fuellfrage (z.B. nicht "was beim Kaffee mit mir" wenn er nicht von Kaffee/Date spricht), kein themenfremder Sprung zu Outdoor/Hobbys wenn er gerade Kontakt/WhatsApp/Telegram/Nummer/Geld/Plattform thematisiert. (2) Nach Ablehnung von WhatsApp/Telegram/Nummer/Instagram: keine Frage nach "anderer Kontaktmöglichkeit", anderem Kanal oder "bester Weg in Kontakt" ausserhalb des Chats – Gespraech bleibt hier. (3) Noch nicht vereinbartes Treffen: Konjunktiv bevorzugen ("wenn wir uns treffen wuerden", "falls wir uns mal sehen wuerden"), nicht nur "wenn wir uns treffen" ohne wuerde – wirkt zu sicher.';
  const planGenderBlock = buildCompactGenderHintForPlan(fakeGenderForPlan, customerGenderForPlan, hasSexuell);
  let contextBlock = planGenderBlock + (contextSnippet
    ? `Kontext (Auszug aus dem Gespräch – beachten für Ton und Thema):\n${sanitizeForApiContent(contextSnippet)}\n\n`
    : '');
  if (isStructuredPlanContext && contextSnippet) {
    contextBlock += 'Hinweis: Der Kunde reagiert oft DIREKT auf unsere letzte Nachricht (z.B. "verrat es mir" nach unserem Angebot vom Traum = er will, dass du vom Traum erzaehlst; "ja" = Zustimmung zu dem, was wir angeboten haben). Bezug und Hauptthema daraus ableiten – Antwort muss auf DAS eingehen.\n\n';
  }
  if (lastPersonaLineForPlan) {
    ...
    contextBlock += 'PFLICHT – EIGENE LETZTE NACHRICHT KONSISTENT: In dieser deiner letzten Nachricht hast du bereits Angaben gemacht (z.B. Name, Kinder, Ort, Beruf). Diese duerfen in der geplanten Antwort NICHT wiederholt werden (keine erneute Vorstellung) und NICHT widersprochen werden (z.B. nicht "hab keine Kinder" wenn du dort "zwei Toechter" o.ae. gesagt hast). Plan: Antworte NUR auf das, was der Kunde in seiner AKTUELLEN Nachricht schreibt; im Plan unter "Vermeiden" angeben: keine Wiederholung/Widerspruch zu dem, was wir in unserer letzten Nachricht gesagt haben.\n\n';
  }
  const besitzFaktenForPlan = extractBesitzFaktenFromVerlauf(conversationHistory || '');
  if (besitzFaktenForPlan.length > 0) {
    contextBlock += `BESITZ (PFLICHT im Plan): Du (Fake/Moderator) hast im Verlauf bereits Besitz genannt: ${besitzFaktenForPlan.join('; ')}. Diese Dinge gehoeren DIR, nicht dem Kunden. Im Plan UNBEDINGT angeben: "Besitz: [diese Dinge] gehoert MIR (Fake). In der Antwort nur mein/meine verwenden, NIEMALS dein/deine dafuer." So weiss der Generator, die Perspektive nicht zu vertauschen.\n\n`;
  }
  const ortFromVerlaufForPlan = extractOrtFromVerlauf(conversationHistory || '');
  const customerSaysOrtNotFar = ortFromVerlaufForPlan && customerSnippet && (
    new RegExp(ortFromVerlaufForPlan.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + '\\s+ist\\s+(ja\\s+)?nicht\\s+(weit|soweit)', 'i').test(customerSnippet) ||
    /\bist\s+(ja\s+)?nicht\s+(weit|soweit)\b/i.test(customerSnippet) && customerSnippet.toLowerCase().includes(ortFromVerlaufForPlan.toLowerCase())
  );
  if (ortFromVerlaufForPlan && customerSaysOrtNotFar) {
    contextBlock += `ORT-BEZUG (PFLICHT im Plan): Du hast im Verlauf gesagt, du kommst aus ${sanitizeForApiContent(ortFromVerlaufForPlan)}. Der Kunde sagt "${sanitizeForApiContent(ortFromVerlaufForPlan)} ist nicht weit" / "nicht soweit" = er meint die ENTfernung zu DEINEM Ort (von ihm aus gesehen), NICHT dass er aus ${sanitizeForApiContent(ortFromVerlaufForPlan)} kommt. Plan: Antwort aus meiner Sicht (ich = ${sanitizeForApiContent(ortFromVerlaufForPlan)}). KEINEN anderen Ort fuer mich erfinden (z.B. nicht "${sanitizeForApiContent(ortFromVerlaufForPlan)} ist nah bei mir in [anderer Ort]").\n\n`;
  }
  ...
  const irritatedHint = (contextSnippet && customerSeemsIrritated)
    ...
    : '';
  const customerClaimsHint = customerClaimsWeAgreedOrWantsAddress
    ? ' WICHTIG – KUNDE BEHAUPTET WIR HABEN ZUGESAGT / WILL ADRESSE: Der Kunde behauptet, wir haetten gesagt dass wir heute Zeit haben (Treffen/Sex) oder verlangt unsere Adresse mit diesem Verweis. Plan: NICHT bestaetigen, NICHT Adresse zusagen oder schicken. Reaktion: hoeflich rausreden (z.B. was dazwischen gekommen, heute passt nicht mehr, kurzfristig geaendert) – NIEMALS Adresse schicken oder zusagen.\n\n'
    : '';
...
/** Nachbearbeitung: Doppelpunkt → Komma, Anführungszeichen entfernen – so schreibt kaum jemand beim schnellen Tippen. */
function applyChatStyleFixes(text) {
  if (!text || typeof text !== 'string') return text;
  let t = text
    .replace(/["„""«»]/g, '')
    .replace(/([a-zA-ZäöüÄÖÜß])\s*:\s*(?=\s|$)/g, '$1, ');
...
function buildStrictCorrectorConversationSnippet(rawConversation = '', lastMessagesForContext = '') {
  const src = String((lastMessagesForContext || rawConversation || '')).trim();
  if (!src) return '';
  ...
  const normalized = [];
  for (const line of lines) {
    if (/^(kunde|fake|ki\/moderator)\s*:/i.test(line)) {
      normalized.push(line);
      continue;
    }
    const m = line.match(/^letzte nachricht von fake \(du\)\s*:\s*(.+)$/i);
    if (m && m[1]) normalized.push('Fake: ' + m[1].trim());
  }

  const relevant = (normalized.length > 0 ? normalized : lines).slice(-2).join('\n').trim();
  return relevant.length > 380 ? relevant.slice(-380) : relevant;
}
...
async function runMistralCorrector({ customerMessage = '', context = {}, grokText = '', learningContext = '', exampleSnippet = '', planSnippet = '', conversationSnippet = '' }) {
  ...
  if (context.telefonVideoRequest === true) ctx.push('Kunde fragt/schlaegt Telefonat oder Videoanruf vor – KEIN Telefonat/Video vereinbaren, freundlich ausweichen (weiter schreiben, ohne Chat/Plattform-Woerter zum Kunden)');
  ...
  if (context.berlinHour != null && Number(context.berlinHour) >= 12) {
    ctx.push(`Referenzzeit Europe/Berlin: ${context.berlinClockShort || ''} (${context.berlinTimePhase || ''}) – keine JETZT-Formulierung mit "mein Morgen"/"der Morgen ist jetzt schoener"`);
  } else if (context.berlinHour != null && Number(context.berlinHour) < 6) {
    ctx.push(`Referenzzeit Europe/Berlin: Nacht/frueh ${context.berlinClockShort || ''} – kein Tages-/Morgen-Start als JETZT`);
  }
  const contextLine = ctx.length > 0 ? `Kontext: ${ctx.join(', ')}\n\n` : '';
  const timeCoherenceBerlinRuleMistral =
    context.berlinHour != null && Number(context.berlinHour) >= 12
      ? ` TAGESZEIT (Europe/Berlin ${context.berlinClockShort || ''}, ${context.berlinTimePhase || ''}): Enthaelt die Antwort "mein Morgen", "der Morgen ist (jetzt|gleich)", "Morgen ist/wird schoener/besser" im Sinne vom aktuellen Moment (nicht Begruessung "guten Morgen" aus anderem Grund)? Wenn ja → umschreiben: z.B. "mein Tag", "macht meinen Tag/Nachmittag besser", "freut mich fuer heute".`
      : context.berlinHour != null && Number(context.berlinHour) < 6
        ? ` TAGESZEIT (Nacht/frueh ${context.berlinClockShort || ''}): Enthaelt die Antwort Vormittags-/Morgen-Floskeln als JETZT (z.B. "schoener Start in den Tag")? Wenn ja → an Nacht/frueh anpassen oder neutral.`
        : '';
  const custLowForHyp = (fullCustomerMsg || '').toLowerCase();
  const kundeThematisiertExternenKontakt = /\b(whatsapp|telegram|instagram|insta|nummer|handy|telefon|kontakt|coins?|geld|credit|bonus|coils?|mail|email|signal)\b/i.test(fullCustomerMsg || '');
  const frageUndHypotheseRuleMistral =
    ' HYPOTHETISCHES TREFFEN: Enthaelt die Antwort "wenn wir uns treffen" oder "wenn wir uns mal treffen" ohne "wuerde"/"wuerden" im gleichen Satz bzw. unmittelbar dabei, und es geht um ein noch NICHT fest vereinbartes Treffen (nicht reine Begruessung "guten Tag wenn wir uns treffen" ohne Fantasie)? Wenn ja → umschreiben: "wenn wir uns treffen wuerden", "falls wir uns mal treffen wuerden", "wenn es soweit waere".'
    + (kundeThematisiertExternenKontakt
      ? ' KONTAKT-ABLEHNUNG + SCHLUSSFRAGE: Die Kundennachricht thematisiert WhatsApp/Telegram/Nummer/Instagram/Geld/Credits/Kontakt. Lehnt die Antwort externen Kontakt ab ("ungern raus", "noch frueh", "hier bleiben", "nicht im Internet") und endet mit Frage nach "anderer Kontaktmöglichkeit", "bester Weg.*Kontakt", "auf Telegram", "WhatsApp schreiben", "wie bist du.*in Kontakt", "warum.*(insta|instagram|da).*aktiver", oder themenfremd Outdoor/"was.*stattdessen.*ausprobieren"? Wenn ja → Schlussfrage ersetzen durch eine zum INHALT seiner Nachricht oder zum Weiterschreiben HIER (ohne anderen Kanal).'
      : '')
    + ' WIDERSPRUCH KONTAKT: In EINER Nachricht "keine Nummer"/"kein Social"/"Instagram nicht" zusammen mit "schreib mir auf Telegram/WhatsApp" oder "du kannst mir auf Telegram"? Wenn ja → Telegram/WhatsApp-Angebot entfernen, nur weiter hier schreiben.'
    + (!/\b(kaffee|café|cafe|coffee|date|treffen|essen gehen|spazier)\b/i.test(custLowForHyp)
      ? ' BANALE KAFFEE-MIT-MIR-FRAGE: Kundennachricht ohne Kaffee/Date/Treffen als Thema – enthaelt die Antwort eine Frage mit "beim Kaffee"/"Kaffee trinken" + "mit mir"? Wenn ja → Frage ersetzen durch eine zum aktuellen Kundentext passende Frage.'
      : '');
  ...
  const conversationBlock = (conversationSnippet && conversationSnippet.trim()) ? `Nachrichtenverlauf (nur Kontext – die Nachricht, auf die du antworten musst, steht unten unter "AKTUELLE KUNDENNACHRICHT"):\n${sanitizeForApiContent(conversationSnippet.trim().slice(-450))}\n\n` : '';
  const customerForCorrector = fullCustomerMsg.length > 800 ? fullCustomerMsg.slice(0, 800) + '…' : fullCustomerMsg;
  const currentMessageBlock = `AKTUELLE KUNDENNACHRICHT (nur darauf antworten – Verlauf oben nur Kontext):\n"${sanitizeForApiContent(customerForCorrector)}"\n\n`;
  ...
  const customerClarificationRuleMistral = hasClarificationNurMistral
    ...
    : '';
  const customerNotMyRoleRuleMistral = ' NICHT-ICH-BIN-X (KRITISCH): Wenn der Kunde klar abgrenzt "du bist nicht mein X" (z.B. nicht mein Seelenklempner, nicht mein Therapeut, nicht mein Kumpel) oder "du bist kein X": Dann NIEMALS als Persona bestaetigen "ich bin X". Richtig ist zustimmend/locker: "bin ich auch nicht" / "will ich auch nicht sein" + auf seine Aussage eingehen. Achte besonders auf Verneinungen ("nicht", "kein") – nicht ins Gegenteil drehen.';
  ...
  const hasUndDuRueckfrageMistral = /\bund\s+du\??\b/i.test(fullCustomerMsg);
  const hasUndDuAlsNamensfrageMistral = (hasUndDuRueckfrageMistral && /\b(ich\s+)?hei[ßs]e\s+[\wäöüß-]+|(ich\s+bin\s+)[\wäöüß-]+|(name|hei[ßs]e)\s+[\wäöüß-]+/i.test(fullCustomerMsg)) || customerAsksSameIntroductionBack(fullCustomerMsg);
  const hasNameQuestionInclUndDuMistral = hasNameQuestionMistral || hasUndDuAlsNamensfrageMistral;
  ...
  const hasDirectQuestionToPersonaMistral =
    hasNameQuestionInclUndDuMistral ||
    hasWieGehtEsDirMistral ||
    hasTreffenFrageMistral ||
    hasWasMachstDuHeuteMistral ||
    hasWasSollIchMitDirMistral ||
    hasUndDuRueckfrageMistral ||
    /\b(für dich|für dich\?|was macht .* für dich|was suchst du|wie oft|was wäre für dich|besonders schön für dich)\b/i.test(fullCustomerMsg) ||
    // Mehrpersonenprofil: Fragen können "ihr/euch" enthalten (nicht nur du/dich/dir)
    (context.fakeIsPlural === true && /\?/.test(fullCustomerMsg) && /\b(ihr|euch|bei euch|für euch)\b/i.test(fullCustomerMsg)) ||
    (/\?/.test(fullCustomerMsg) && /\b(dich|dir|du)\b/i.test(fullCustomerMsg));
  const answerDirectQuestionRuleMistral = hasDirectQuestionToPersonaMistral
    ? ' PFLICHT: Die AKTUELLE Kundennachricht enthaelt eine direkte Frage an die Persona (z.B. wie geht es dir, wie heisst du, was machst du – oder "was machst du heute?", "was hast du vor?" – oder "was soll ich mit dir machen?", "sag mal was ich tun soll", "was wuerdest du dir wuenschen?" – oder Treffen). Beantwortet die Antwort diese Frage oder ignoriert sie? Bei "und du?" / "und du" am Ende: Kunde stellt Rueckfrage (z.B. Kennenlern, Vorlieben). Enthaelt die Antwort KEINE Antwort auf diese Rueckfrage? Dann inhaltlich nachziehen (AM ANFANG oder passend) – ohne Persona-Vornamen im Text zu ersetzen oder neu zu erfinden (Namens-Fix ausserhalb des Korrektors). Bei "was soll ich mit dir machen" / "was wuerdest du dir wuenschen": ZUERST inhaltlich beantworten (z.B. 1–2 konkrete Vorschlaege/Wuensche nennen: lecken, kuessen, verwöhnen, was die Persona sich wuenscht), NICHT nur zurueckfragen oder Meta-Kommentar ("Klingt spannend..."). Bei "was machst du heute"/"was hast du vor": kurz etwas Passendes nennen. Bei Treffen-Frage: kurz unverbindlich. Sonst: zuerst Frage beantworten, dann Gegenfrage.'
    : '';
  const treffenFrageIgnoredRuleMistral = hasTreffenFrageMistral
    ...
    : '';
  const hasNameOrWohnortRequestMistral = context.askForNameOrWohnort === true || customerAsksSameIntroductionBack(fullCustomerMsg) || /\b(vornamen?|name)\s+und\s+wohnort\b|\bwohnort\s+und\s+(vornamen?|name)\b|\b(deinen?|dein)\s+(vornamen?|wohnort)\b|\bdenkst\s+du\s+noch\s+an\s+(deinen?\s+)?(vornamen?|wohnort)\b/i.test(fullCustomerMsg);
  const fc = (context.fakeCityForAnswer && String(context.fakeCityForAnswer).trim()) || '';
  /** Korrektor soll Persona-Vornamen nicht ersetzen/„korrigieren“ – Namens-Fix passiert deterministisch nach dem Korrektor (correctWrongFakeIntroName). */
  const personaFirstNamesDoNotEditRuleMistral = ' PERSONA-VORNAMEN NICHT ANFASSEN: Vornamen der Persona und konkrete Personen-Namen in der Moderatoren-Antwort NICHT ersetzen, nicht umbenennen und nicht "korrigieren" oder "vereinheitlichen". Rechtschreibung und Umlaute nur ausserhalb von Namen im Fliesstext. Ausnahme: Platzhalter wie [Vorname] oder [dein Name] duerfen entfernt werden.';
  const nameAndWohnortRuleMistral = (hasNameOrWohnortRequestMistral && fc)
    ? ' PFLICHT WOHNORT: Der Kunde hat nach Wohnort/Ort gefragt oder eine Kennenlern-Runde inkl. Ort. Wenn die Antwort den vereinbarten Fake-Wohnort aus dem Kontext nennen soll: Ort "' + String(fc).trim() + '" verwenden; einen anderen (falschen) Ortsnamen in der Antwort durch diesen ersetzen oder die falsche Ortsnennung weglassen – keine Fantasie-Staedte. Persona-Vornamen dabei NICHT aendern (sie PERSONA-VORNAMEN).'
    : '';
  const erreichbarkeitRuleMistral = (context.askForErreichbarkeit === true)
    ? ' ERREICHBARKEIT: Der Kunde hat gefragt, wo/wie er dich erreichen kann (z.B. "wo kann ich dich erreichen?"). Die Antwort MUSS diese Frage beantworten (z.B. "Du kannst mich hier erreichen", "Schreib mir gern weiter hier", "Bin fuer dich hier erreichbar"). Enthaelt die Antwort KEINEN solchen Bezug (hier, schreiben, erreichbar)? Dann AM ANFANG der Antwort einen kurzen Satz einfuegen: z.B. "Du kannst mich hier erreichen. " oder "Schreib mir gern hier, bin fuer dich da. " – ohne Woerter "Chat" oder "Plattform". Dann den bisherigen Antworttext. Danach auf den Rest der Kundennachricht eingehen.'
    : '';
  const focusCurrentMessageRuleMistral = ' FOKUS AKTUELLE KUNDENNACHRICHT: Die Antwort muss sich auf die AKTUELLE (letzte) Kundennachricht konzentrieren. Der Nachrichtenverlauf dient nur zum Verstaendnis und fuer Bezuege (es, das, worauf er sich bezieht). Themen aus frueheren Nachrichten (z.B. Foto-Anfrage) hat der letzte Moderator bereits beantwortet – nicht erneut darauf antworten oder das zum Hauptthema machen. Wenn die aktuelle Nachricht z.B. nach Kennenlern-Infos (Name, Ort) fragt, muss die Antwort inhaltlich darauf eingehen; Wohnort siehe WOHNORT-Regel; Persona-Vornamen im Text nicht vom Korrektor ersetzen. Andere Themen aus dem Verlauf (z.B. Fotos) sind sekundaer oder schon beantwortet. Kontext = Verstehen; Antwort = auf die aktuelle Nachricht.';
  const answerLastMessageQuestionNotPreviousRuleMistral = ' LETZTE NACHRICHT ZUERST: Enthaelt die LETZTE Kundennachricht eine Bestaetigung/Begeisterung plus eine NEUE Frage (z.B. "Oh ja, X klingt toll. Was denkst du, wie lange...?", "Klingt gut. Wie lange koennten wir...?")? Dann muss die Antwort VOR ALLEM diese letzte Frage beantworten und auf DIESE letzte Nachricht reagieren. Die Antwort darf sich NICHT vor allem auf eine vorherige, detailliertere Kundennachricht beziehen (z.B. "Beides?" als Einstieg oder Ausarbeitung des aelteren Vorschlags) – zuerst die Frage aus der LETZTEN Nachricht beantworten, dann ggf. kurz auf das Thema eingehen.';
  // Name nicht wiederholen, wenn letzter Moderator/Kunde ihn schon kennt; nur Vorname (nie Benutzername) nennen
  const noRepeatNameIfAlreadySaidRuleMistral = ' Hat die LETZTE Moderator-Nachricht im Nachrichtenverlauf bereits den Vornamen der Persona genannt (z.B. "Ich bin Justine", "Ich heisse X") oder spricht der Kunde die Persona mit diesem Namen an (z.B. "liebe Justine", "Hey Justine")? Enthaelt die zu korrigierende Antwort trotzdem eine erneute Vorstellung mit "Ich heisse [Name]" oder "Ich bin [Name]"? Wenn ja → diesen redundanten Vorstellungs-Teil entfernen oder durch kurze Bestaetigung ersetzen (z.B. "Ja, genau.", "Freut mich.") – dabei die genannten Vornamen im Wortlaut NICHT ersetzen oder umbenennen (nur Satz weglassen oder kuerzen).';
  ...
  const neutralMessageNoSexRule = (context.allowSexualContent !== true)
    ? ' WICHTIG: Wenn die AKTUELLE Kundennachricht keine sexuellen Themen enthaelt, entferne oder entschaerfe explizite sexuelle Formulierungen in der Antwort. Die Antwort muss zum Ton der aktuellen Nachricht passen – nicht zum Nachrichtenverlauf.'
    : '';
  ...
  const noDoppelpunktAnfuehrungszeichenRule = ' PFLICHT STIL: Enthaelt die Antwort Doppelpunkt (:) oder Anführungszeichen (" „ " « »)? So schreibt kaum jemand in kurzen Nachrichten. Wenn ja → Doppelpunkt durch Komma ersetzen (z.B. "hör zu," statt "hör zu:"), alle Anführungszeichen entfernen.';
  const umlautAndSsRule = ' PFLICHT UMLAUTE: Enthaelt die Antwort ae, oe oder ue anstelle von ä, ö, ü (z.B. Maenner→Männer, fuer→für, moechte→möchte, Koerper→Körper, ueber→über, kuessend→küssend, spueren→spüren, schoen→schön, waere→wäre, naechstes→nächstes)? Dann durch echte Umlaute ersetzen. NICHT ersetzen in Woertern wo ue/oe/ae zwei getrennte Buchstaben sind (Feuer, Museum, Schuhe, Abenteuer, Poesie). Immer ss statt ß.';
  ...
  const wrongApologyAttributionRule = ' FALSCHES ENTSCHULDIGUNGS-ROLLENBILD: Sagt der Kunde, er habe deine Nachricht nicht mitbekommen (Handy stumm/lautlos, keine Benachrichtigung, verpasst, beschaeftigt)? Enthaelt die Antwort eine Entschuldigung der Persona, als haette DER KUNDE warten muessen wegen DIR, oder dass DEIN Handy/Geraet schuld war (z.B. "tut mir leid dass du warten musstest", "mein Handy hatte eigene Plaene"), obwohl ER sein Geraet/Stumm erklaert hat? Wenn ja → umschreiben: locker verstaendnisvoll (kein Stress, alles gut), ohne falsche Schuldzuweisung an die Persona, ohne Entschuldigung fuer sein Versaumnis als waere die Persona schuld – kurz da + auf seine Frage/Thema.';
  // Plan enthaelt RICHTUNG (Fake hat Wunsch geaeussert, Kunde geht darauf ein) – Antwort darf dem Kunden diesen Wunsch nicht zuschreiben
  const richtungAusPlanRuleMistral = (planSnippet && /RICHTUNG\s*:/i.test(planSnippet))
    ? ' RICHTUNG AUS PLAN: Der Plan besagt, dass DU (Fake) den Wunsch/die Aussage geaeussert hast und der Kunde darauf eingeht. Enthaelt die Antwort "dein Wunsch", "dass du den Wunsch hast", "dass du X willst", "dass du deinen Wunsch ernst nimmst" (als ob der Kunde den Wunsch geaeussert haette)? Wenn ja → ersetzen durch "meinen Wunsch", "dass du darauf eingehst", "dass du meinen Wunsch ernst nimmst" o.ae. – keine Rollen vertauschen.'
    : '';
  ...
  const sexualRoleReversalRuleMistral = (context.allowSexualContent === true && context.fakeIsFemale === true)
    ? ' SEXUELLE ROLLE (FRAU = EMPFAENGERIN): Enthaelt die Antwort Formulierungen, in denen die Persona den KUNDEN aktiv "nimmt" oder "fickt" (z.B. "ich stecke sie dir rein", "ich nehme dich", "ich ficke dich", "ich nehme dich doggy", "dich doggy nehmen", "dich nehmen", "dich ficken", "wuerd dich doggy nehmen", "würd dich nehmen")? Die Persona ist die FRAU – sie EMPFAENGT (wird genommen, ihr wird etwas reingesteckt), der Kunde ist der AKTIVE. Wenn ja → umschreiben: als Empfaengerin formulieren (z.B. "wenn du mich nimmst", "wenn du mich doggy nimmst", "dass du mich nimmst", "ich laes mich von dir doggy nehmen", "bei mir geht sie so tief rein"). Niemals die Persona als die Handelnde, die den Kunden nimmt oder ihm etwas reinsteckt.'
    : '';
  // Kunde vs. Persona: keine weibliche Anatomie am "du" wenn Kunde Mann (und umgekehrt)
  const customerAnatomyMaleRuleMistral = (context.allowSexualContent === true && context.fakeIsFemale === true && context.customerIsMale === true)
    ? ' ANATOMIE KUNDE = MANN (Persona = Frau): Beim Ansprechen des Kunden (du, dein, deine, dir, dich) duerfen KEINE weiblichen Koerperteile oder typisch weibliche Erregung fuer IHN stehen. VERBOTEN z.B.: "deine Muschi", "deine Spalte", "deinen Kitzler", "deine Klitoris", "deine Brust/deine Brüste" als sein Koerper, "du bist (ganz) feucht", "du wirst feucht", "bei dir wird es feucht" (im Sinne Vulva). Wenn ja → umschreiben: maennlich oder neutral (Schwanz, Penis, steif, hart, kommst, geil, heiss); "feucht"/Muschi/Kitzler nur bei ich/mir/mich/meine (Persona).'
    : '';
  const customerAnatomyFemaleRuleMistral = (context.allowSexualContent === true && context.fakeIsFemale === false && context.customerIsFemale === true)
    ? ' ANATOMIE KUNDE = FRAU (Persona = Mann): Beim Ansprechen der Kundin (du, dein, deine) duerfen KEINE offensichtlich maennlichen Koerperteile als IHR Koerper stehen. VERBOTEN z.B.: "deinen Schwanz", "dein Penis", "deine Eichel" gemeint als ihr Koerper. Wenn ja → umschreiben: weiblich/neutral fuer sie; Schwanz/Penis nur bei ich/mir/meinem (Persona).'
    : '';
  const customerKoseAnredeRuleMistral = context.customerIsMale === true
    ? ' KUNDEN-KOSE-ANREDE: Der Kunde ist ein Mann. Enthaelt die Antwort "süße", "meine Süße", "liebe [Name]" oder aehnliche weiblich markierte Kose-Anreden fuer IHN (du/dich)? Wenn ja → korrigieren zu "süßer", "mein Süßer", "lieber" o.ae.'
    : context.customerIsFemale === true
      ? ' KUNDEN-KOSE-ANREDE: Die Kundin ist eine Frau. Enthaelt die Antwort "süßer", "mein Süßer", "lieber [Name]" oder aehnliche maennlich markierte Kose-Anreden fuer SIE (du/dich)? Wenn ja → korrigieren zu "süße", "meine Süße", "liebe" o.ae.'
      : '';
  ...
  const pureGreetingAnswerRule = (context.isPureGreetingMessage === true)
    ? ' BEGRUESSUNG ALS KUNDENNACHRICHT: Die Kundennachricht ist nur eine Begruessung (Hey, Hi, Hallo, Hallo [Name] o.ae.). Enthaelt die Antwort "freut mich dass du [Name] sagst", "gut dass du Merle sagst", "passt super" zum Namen oder aehnlichen Meta-Kommentar? Wenn ja → diesen Teil ENTFERNEN, nur Hallo/Hey zurueck + Tagesfragen. Enthaelt die Antwort Bezug auf den Verlauf (Abenteuer, Kuss, Bock auf, was du vorher gefragt hast, vorherige Themen)? Wenn ja → ersetzen durch NUR kurze Begruessung (Hallo, Hey) + 1–2 Tagesfragen (wie gehts dir, was machst du gerade). Keine romantischen/emotionalen Formulierungen, kein Aufgreifen alter Themen.'
    : '';
  ...
  const noTimeRefusalWithoutMeetingRequestRule = (context.isDirectMeetingRequest !== true)
    ...
    : '';
  // Kein eigener Treffen-Vorschlag der Persona – auch wenn Kunde um Terminvorschlag bittet
  const noMeetingSuggestionFromModeratorRule = ' KEIN EIGENER TREFFEN-VORSCHLAG (IMMER): Enthaelt die Antwort einen VORSCHLAG der Persona zum Treffen (z.B. "wir koennten uns treffen", "lass uns mal treffen", "wie waers wenn wir uns treffen", "wie waere es wenn wir uns auf einen Kaffee treffen", "auf einen Kaffee treffen und schauen ob die Chemie stimmt", "sollen wir uns mal treffen")? Auch wenn der Kunde nach einem Terminvorschlag gefragt hat – keinen konkreten Treffen-Vorschlag. Wenn ja → diesen Satz/Teil entfernen oder ersetzen durch unverbindliche Formulierung (z.B. muss schauen wann es passt, wuerde mir gefallen aber …). Rest der Nachricht bleibt.';
  const noErsatzterminVorschlagRuleMistral = ' ERSATZTERMIN VERBOTEN: Enthaelt die Antwort einen Vorschlag oder eine Frage nach einem ANDEREN Termin (z.B. "Was haeltst du davon wenn wir es morgen versuchen?", "uebermorgen versuchen?", "naechste Woche?", "wann passt es dir?", "wann haettest du Zeit?")? Das waere ein indirekter Treffen-Vorschlag. Wenn ja → diesen Satz/Teil ENTFERNEN und durch eine thematisch passende Gegenfrage ersetzen (Stimmung, Vorlieben, Fantasie – keine Zeit/Ersatztermin).';
  ...
  const imageAlreadySentToCustomerRule = (context.imageAlreadySentToCustomer === true)
    ? (
        context.isBilderAnfrageSituation === true && context.moderatorPromisedMoreImages === true
          ? ' BILD BEREITS GESCHICKT + WEITERE BILDER ANGEBOTEN: Du hast ein Bild geschickt und (weitere) Bilder in Aussicht gestellt. NICHT so tun als waere kein Bild angekommen. KEINE technische Ausrede (Handy, Kamera, keine Fotos auf dem Handy). Bei Bitte nach mehr: flirty verzoegern, nicht "Foto finde ich nicht" fuer das bereits Geschickte.'
    ...
    : '';
  const bildAngebotKundeJaOhneSofortversandRuleMistral = (context.bildAngebotKundeJaVorfreude === true)
    ? ' BILD-ANGEBOT + KUNDE JA: Letzte Moderatoren-Nachricht bot ein Foto/Bild an oder fragte ob geschickt werden soll; Kundenantwort ist kurze Zustimmung. Enthaelt die Antwort Formulierungen als wuerde die Persona JETZT/GLEICH wirklich ein Bild mitschicken oder den Versand zugesagt haben (z.B. "hier kommt", "hier ist", "schick ich dir gleich", "schick dir gleich", "ein Moment", "wart kurz", "gleich hast du eins", "das Foto kommt", "leg los", "upload", "schick es dir jetzt")? Wenn ja → ersetzen durch Vorfreude/Lust, flirty weiterreden, Fantasie oder Frage – ohne technische Sofort-Zusage des Versands; nicht wie Abwimmeln oder "melde mich spaeter" ohne Spielfreude.'
    : '';
  // Persona hat sich im Verlauf bereits vorgestellt (Name/Ort) – keine erneute Vorstellung
  const alreadyIntroducedSelfRuleMistral = (context.alreadyIntroducedSelfInChat === true)
    ? ' BEREITS VORGESTELLT: Persona hat sich in diesem Gespraech bereits vorgestellt (Name und/oder Ort). Enthaelt die Antwort erneute Vorstellung ("ich bin X", "wohne in Y", "komme aus Z", "ich heisse X")? Wenn ja → diesen Teil entfernen, inhaltlich antworten, hoechstens kurzer Verweis ("wie ich schon sagte").'
    : '';
  ...
  const noStrongHopeRule = ' Formuliert die Antwort feste Zusage oder starke Vorfreude (z.B. Vorfreude auf ein konkretes Treffen, "freue mich schon auf unser Treffen"/"aufs Treffen")? Wenn ja → zurueckhaltender und unverbindlicher umformulieren, wunschhaft/hypothetisch, in eigenen Worten variieren.';
  ...
  const noWiedersehenRule = ' PFLICHT: Enthaelt die Antwort "wiedersehen", "wiederzusehen", "mal wieder treffen", "wenn wir uns wiedersehen" ODER "nächstes Mal sehen", "wenn wir uns das nächste Mal sehen", "beim nächsten Mal" (im Treffen-Kontext) ODER "dich endlich sehen", "dich endlich zu sehen", "in deinen Armen", "in deinen Armen sein", "dich in die Arme nehmen", "freue mich dich endlich zu sehen"? Ihr habt euch NIE getroffen – nichts andeuten, was ein reales Treffen oder koerperliche Begegnung voraussetzt. Wenn ja → ersetzen durch "wenn wir uns treffen wuerden", "wenn wir uns (mal) sehen wuerden", "wenn es so weit ist", "was wir zusammen machen koennten" bzw. nur Vorfreude beim Schreiben (z.B. "freue mich wenn du schreibst", "bin gern hier fuer dich") – ohne "wieder", "nächstes Mal", "endlich sehen", "in deinen Armen".';
  ...
  const selfConsistencyRule = ' Enthaelt der Nachrichtenverlauf eine vorherige Moderator-Nachricht, in der die Persona etwas ueber SICH gesagt hat (z.B. was sie isst, macht, wo sie ist – Nudeln mit Pesto, Omelett, etc.)? Widerspricht die zu korrigierende Antwort dem (z.B. anderes Essen, andere Aktivitaet)? Wenn ja → umschreiben: konsistent mit der vorherigen Aussage bleiben, NICHT das des Kunden echoen oder etwas Neues erfinden.';
  ...
  const noTelefonsexPhotoRule = ' PFLICHT: Enthaelt die Antwort Vorschlaege wie Telefonsex, heisse Fotos schicken, Bilder als Loesung/Alternative (z.B. "mit Telefonsex und heissen Fotos", "Fotos vorweg")? Telefonsex geht fuer uns so nicht – klar ablehnen (nicht als System-/Plattform-Regel zum Kunden formulieren). Bei reiner Foto-Anfrage: keine generelle Absage ("ich schicke keine Bilder"). Wenn ja → entfernen und durch andere Argumente ersetzen (Fantasie, Vorstellungskraft, erst mal kennenlernen beim Schreiben) bzw. bei Foto-Anfrage durch Ausrede (gerade nicht, Kamera/Handy, schau mal ob was Gutes da ist) – ohne Telefonsex oder Foto-Tausch anzubieten.';
  const noTelefonsexPhotoRuleEffective = (context.isBilderAnfrageSituation === true && context.moderatorPromisedMoreImages === true)
    ? ' PFLICHT: Telefonsex/heisse Fotos als Loesung → entfernen wie ueblich. BILDER ZUVOR ANGEBOTEN: Letzte Moderator-Nachricht bot dem Kunden (weitere) Bilder; er fragt danach. KEINE technische Foto-Ausrede (Kamera spinnt, Handy kaputt, keine guten Fotos auf dem Handy) in der Antwort – weder stehen lassen noch einfuegen; stattdessen flirty verzoegernd formulieren. Keine generelle "ich schicke keine Bilder". Keine konkrete Zusage sofort/mehr zu schicken ("schick dir gleich").'
  ...
  const imageRequestDirectionRule = (context.isBilderAnfrageSituation === true && context.moderatorPromisedMoreImages === true)
    ? ' BILDER-ANFRAGE NACH EIGENEM ANGEBOT: Du hattest (weitere) Bilder angeboten; der Kunde bittet darum. Richtungsvertauschung pruefen (Antwort klingt als wuerde ER DIR schicken statt umgekehrt)? Wenn ja → korrigieren. KEINE technische Ausrede (Kamera, Handy, keine Fotos auf dem Handy). Stattdessen flirty verzoegern (Spannung, verdienen, Tausch-Gedanke). KEINE sofortige Zusage zum Versand.'
    : ' BILDER-ANFRAGE RICHTUNG: Bittet die Kundennachricht DICH (Persona), ein Foto/Selfie zu schicken (z.B. "willst du mir ein Selfie schicken", "schick mir ein Foto", "kannst du mir ein Bild schicken", "selfie von der Arbeit schicken")? Enthaelt die Antwort trotzdem etwas wie "wuerde mich freuen dein Selfie zu sehen", "freue mich auf dein Bild", "wuerde dein Selfie gern sehen" – also als wuerde der Kunde DIR ein Bild schicken? Das ist Richtungsvertauschung. Wenn ja → ersetzen durch freundliches Ausweichen mit AUSREDE (z.B. gerade nicht, Kamera spinnt, keine guten Fotos auf dem Handy, schau mal ob ich was Gutes finde, muss erst schauen – locker im Ton). KEINE Phrase wie "schick keine Fotos im Internet" oder "hier schick ich keine". Auf den Rest der Nachricht eingehen.';
  ...
  const meetingTreffenAussichtRule = (context.isMeetingSituation === true)
    ...
    : '';
  const minLengthRule = CORRECTOR_MIN_LENGTH_RULE + CORRECTOR_PREFER_COMPACT_RULE + ' Keine themenfremden neuen Gross-Themen erfinden; Ergaenzungen nur passend zur Kundennachricht und zur bestehenden Antwort.';
  ...
  const kaufenBestellenBuchenRule = ' Kunde will etwas FUER DICH (Fake) kaufen, bestellen oder buchen (z.B. "soll ich buchen?", "soll ich dir was bestellen?", "soll ich das Hotel buchen?")? KAUFEN/BESTELLEN: Antwort muss klar sagen, dass er es (noch) nicht braucht / nicht bestellen/kaufen soll – nicht nur weich ausweichen. BUCHEN mit konkretem Zeitraum (z.B. naechstes Wochenende, 27.02): Antwort muss DEUTLICH fuer diesen Zeitraum absagen (da habe ich keine Zeit, da passt nicht, wann anders besser) – nicht nur "muss schauen". Wenn ja → entsprechend umschreiben. HOTEL/ZIMMER/SUITE/UNTERKUNFT (gemeinsam, Kosten teilen, Doppelzimmer, Airbnb, reservieren – auch ohne "soll ich buchen?"): Fehlt ein klarer Hinweis NOCH NICHTS zu buchen / kein Geld / erst Termin bei beiden klären? Wenn ja → ergaenzen; nicht nur Hotel/Suite loben oder "klingt gut" ohne diesen Hinweis.';
  ...
  const noTelefonVideoVereinbarenRuleMistral = (context.telefonVideoRequest === true)
    ? ' TELEFONAT/VIDEOANRUF (KEIN ANRUF ZUSAGEN): Der Kunde hat Telefonat oder Videoanruf (z.B. Kamera beim Telefon) vorgeschlagen/gefragt. Enthaelt die Antwort eine Zusage oder Terminfrage dafuer (z.B. "wann passt es zum Telefonieren?", "lass uns mal telefonieren", "Kamera beim Telefon klingt gut", "zeig mir was Echtes beim Telefon", "wann koennen wir telefonieren", "passt gut dass du die Kamera anmachen willst")? Wenn ja → diesen Teil ENTFERNEN oder ersetzen: freundlich ausweichen (z.B. "lieber weiter schreiben", "mag ich wenn du mir schreibst"), KEINE Vereinbarung von Telefonat/Video, auf andere Teile seiner Nachricht eingehen (z.B. Arbeitszeiten). Gegenueber dem Kunden nicht "Chat"/"Plattform" sagen.'
    : '';
  ...
  if (MISTRAL_USE_MINIMAL_PROMPT) {
    ...
    const contactRuleMinimal = (context.contactRequest === true)
      ? ' Bei Kontaktdaten-Anfrage: Enthaelt die Antwort "nicht erlaubt", "Regeln", das Wort "Plattform" zum Kunden, "echte Kontaktdaten verboten" oder "duerfen nicht"? Wenn ja → diese Formulierungen entfernen und persoenlich ausweichend ersetzen. Enthaelt die Antwort ein Angebot fuer Telegram, WhatsApp, Instagram oder anderen Messenger zum Wechseln (z.B. "auf Telegram schreiben", "schreib mir auf WhatsApp")? Wenn ja → diesen Teil ENTFERNEN, nur "schreib mir gern hier" / "hier erreichbar" lassen (ohne "Chat"). Sonst Stil beibehalten.'
      : '';
  const noOtherPlatformRuleMistral = ' ANDERER MESSENGER / KANAL ANGEBOTEN: Enthaelt die Antwort ein Angebot oder einen Vorschlag, auf Telegram, WhatsApp, Instagram, Social Media oder einen anderen separaten Kanal zu wechseln bzw. dort zu schreiben (z.B. "du kannst mir auf Telegram schreiben", "schreib mir auf WhatsApp")? Wenn ja → diesen Teil KOMPLETT entfernen und durch kurze Hinweise wie "schreib mir gern hier" / "bin hier fuer dich erreichbar" ersetzen oder weglassen. NIEMALS einen anderen Messenger als diesen Kontakt zum Kunden anbieten. Gegenueber dem Kunden das Wort "Plattform" vermeiden.';
    ...
    systemContent = 'PFLICHT: Nur die fertige korrigierte Nachricht zurueckgeben, keine Erklaerungen.\n\nDu bist ein Korrektor für Chat-Moderator-Antworten. Gib nur die fertige korrigierte Nachricht zurück, keine Erklärungen, keine Meta-Kommentare.' + toneMinimal + ' Stil und Wortschatz der ursprünglichen Antwort möglichst beibehalten, nur klare Fehler korrigieren. Jede Nachricht muss eine Frage enthalten; maximal ein bis zwei Fragen, keine Frage-Kaskade.' + CORRECTOR_MIN_LENGTH_RULE + CORRECTOR_PREFER_COMPACT_RULE + ' Keine themenfremden Einschuebe aus aelteren Nachrichten.' + noEchoMinimal + contactRuleMinimal + noOtherPlatformRuleMistral + focusCurrentMessageRuleMistral + answerLastMessageQuestionNotPreviousRuleMistral + nameAndWohnortRuleMistral + personaFirstNamesDoNotEditRuleMistral + erreichbarkeitRuleMistral + profileDumpRule + expectedCityWohnortRule + expectedJobRule + wohnortConcreteCityRule + customerClarificationRuleMistral + customerNotMyRoleRuleMistral + dagegenSexHopeRuleMistral + meldeDichVerfuegbarkeitRuleMistral + answerDirectQuestionRuleMistral + treffenFrageIgnoredRuleMistral + naturalFormulationRule + noFalseSingleRule + noTelefonsexPhotoRuleEffective + imageRequestDirectionRule + noRepeatHereKennenlernenWhenGenervtRule + ruckfrageCallbackRule + answerOurQuestionStayOnTopicRule + ambiguousContextStayOnTopicRule + noWiedersehenRule + noSharedPastRule + noTimeAgreementRule + noAbholenRule + kaufenBestellenBuchenRule + konkreterSexuellerPlanRuleMistral + deineVsMeineRuleMistral + deineVsMeineKontaktRuleMistral + customerAnatomyMaleRuleMistral + customerAnatomyFemaleRuleMistral + mehrereFragenBesuchbarRuleMistral + kontextgebundenRuleMistral + treffenRichtungRuleMistral + verlaufBezugRuleMistral + noFreeRelaxAfterMeetingDeclineRule + replyMustAddressCurrentMessageRule + imageCurrentMessageRuleMistral + noFlexibelFloskelRuleMistral + noTelefonVideoVereinbarenRuleMistral + bilderAnfrageNoTreffenRuleMistral + imageProductLikeRuleMistral + noUnaskedAgeTypeComplimentRule + noHastDuLustRule + boundaryConsistencyRule + selfConsistencyRule + noEchoOwnModeratorStatementRule + noRepeatOwnLastMessageRule + censorshipRule + noOftTreffenRule + limitationFollowUpRule + engageOnSuggestionsRule + neutralMessageNoSexRule + meetingRule + meetingTreffenAussichtRule + noTagRaussuchenRule + multiThemeRule + multiPointNaturalRule + questionAndWholeRule + minLengthRule + sameActivityPlaceRule + noParaphraseRule + noVorliebenTabusEchoRule + noPerfekteMomentFrageRule + noParaphraseSchlussfrageRule + noMetaQuestionAboutCustomerActivityRule + noQuestionAlreadyAnsweredRule + noRedundantFactQuestionRule + noKaffeeTreffenFrageRule + noVagueDazuSchlussfrageRule + pureGreetingAnswerRule + noTimeRefusalWithoutMeetingRequestRule + noMeetingSuggestionFromModeratorRule + noRedundantScenarioQuestionRule + noRoleReversalRule + richtungAusPlanRuleMistral + noRepeatNameIfAlreadySaidRuleMistral + noAddressCustomerWithPlatformUsernameRuleMistral + noMmmOpenerRuleMistral + massageNotMasseurinRuleMistral + logbookNoContradictRuleMistral + imageOnlyAnnouncedRule + imageClaimedButNotPresentRule + imageAlreadySentToCustomerRule + bildAngebotKundeJaOhneSofortversandRuleMistral + alreadyIntroducedSelfRuleMistral + notClearlySexualRuleMistral + noPlaceClaimRule + noStrongHopeRule + customerSingularRule + timeCoherenceBerlinRuleMistral + frageUndHypotheseRuleMistral + noDoppelpunktAnfuehrungszeichenRule + umlautAndSsRule + humanTyposHint;
    userContent = `${contextLine}${planBlock}${conversationBlock}${learningBlock}${exampleBlock}${currentMessageBlock}Zu korrigierende Moderatoren-Antwort:\n\n${sanitizeForApiContent(grokText.trim())}\n\n${CORRECTOR_MIN_LENGTH_RULE}${CORRECTOR_PREFER_COMPACT_RULE}\n\nGib NUR die korrigierte Moderatoren-Antwort aus. Niemals die Kundennachricht oder eine Paraphrase davon zurueckgeben.`;
    ...
    userContent = `${contextLine}${planBlock}${conversationBlock}${learningBlock}${exampleBlock}${currentMessageBlock}Zu korrigierende Moderatoren-Antwort:\n\n${sanitizeForApiContent(grokText.trim())}\n\nKontext nutzen: Nachrichtenverlauf zeigt, worauf die Kundennachricht sich bezieht. Ist sie eine Klarstellung/Abschwaechung (z.B. \"nur X\", \"nichts Schlimmes\")? Enthaelt die Antwort Trost oder \"nicht dein Tag\"? → ersetzen durch positive Reaktion. Stellt der Kunde eine direkte Frage an die Persona (z.B. was für dich schön ist – oder nach dem Namen: wie heißt du, wie ist dein Name)? Beantwortet die Antwort sie oder gibt sie sie nur zurück? Wenn nicht → zuerst inhaltlich auf die Frage eingehen; Persona-Vornamen im Text nicht ersetzen oder neu erfinden (stehen wie vom Generator), dann Gegenfrage.\n\nPrüfe die obige Moderatoren-Antwort und korrigiere/verbessere sie:\n(1) PFLICHT: Geht die Antwort auf die GESAMTE Kundennachricht ein – jede Frage, jedes Thema, jede Aussage? Enthaelt die Kundennachricht mehrere Fragen/Themen? Geht die Antwort auf JEDE ein? Wenn eine Frage oder ein Thema fehlt → kurzen Bezug ergaenzen (1 Satz oder Teilsatz), nichts auslassen.\n(2) Enthält die Nachricht eine Frage? Wenn nein → Frage einbauen.\n(3) Umlaute (ä,ö,ü) und ss statt ß? Stil/Bindestriche?\n(4) Enthält die Antwort Meta-Kommentare, Echo/Paraphrase der Kundennachricht oder Meta-Fragen wie "Ich finde es spannend was du dir wünschst", "Was wäre dein perfektes Szenario?"? Wenn ja → entfernen bzw. durch konkrete thematische Frage ersetzen, eigenständig formulieren (Kundentext nicht nachplappern).\n(5) Kunde will treffen: Enthält die Antwort "lass uns erstmal noch schreiben", "erst kennenlernen", "noch ein bisschen schreiben" oder Meta-Fragen wie "Was denkst du darüber?"? Wenn ja → umschreiben: auf Treffenwunsch eingehen (z.B. will auch nicht ewig nur schreiben, schaue wie es passt) + konkrete Frage zum moeglichen Treffen (Café, Essen, Vorlieben – variieren), nicht immer dieselbe Formulierung.${contactCheck}\n(6) KEINE neuen Themen einbauen: Nichts aus aelteren Nachrichten reinziehen, was in der AKTUELLEN Kundennachricht nicht vorkommt (z.B. kein erneuter Geburtstagsbezug ohne aktuellen Anlass).\n(7) ${(CORRECTOR_MIN_LENGTH_RULE + CORRECTOR_PREFER_COMPACT_RULE).trim()}\nGib NUR den fertigen korrigierten Text zurück, keine Erklärungen. Niemals die Kundennachricht oder eine Paraphrase davon zurueckgeben.`;
    ...
    const noStrongHopeRule = ' Starke Vorfreude oder Formulierung wie feste Treffen-Zusage (z.B. "freue mich schon auf unser Treffen")? → zurueckhaltender und unverbindlicher umformulieren, wunschhaft/hypothetisch, variieren.';
    const sexualRule = context.allowSexualContent === true
      ...
      : 'Wenn die AKTUELLE Kundennachricht keine sexuellen Themen enthaelt, entferne oder entschaerfe explizite sexuelle Formulierungen in der Antwort – die Antwort muss zum Ton der aktuellen Nachricht passen (nicht zum Nachrichtenverlauf). Ansonsten: sexuelle Formulierungen nur entfernen, wenn im Kontext "Kunde moechte nicht ueber Sex schreiben" steht.';
  ...
  const schlussfrageImSzenarioRuleMistral = ' SCHLUSSFRAGE IM SZENARIO: Die letzte Frage muss im Thema/Szenario bleiben (nach Ablauf, Wie, Vorliebe, Stimmung, was als naechstes). Ist die Schlussfrage eine Reflexionsfrage (z.B. nach Erwartungen, Erlebnis, Intensitaet – "was erhoffst/erwartest du", "wie intensiv", "was wuerdest du dir wuenschen")? Wenn ja → ersetzen durch eine konkrete Frage zum Geschehen oder zum Thema (Ablauf, Wie, Vorliebe, Stimmung), nicht nach Reflexion fragen.';
    const noReiztAmMeistenRuleMistral = ' REFLEXIONSFRAGEN VERBOTEN: Enthaelt die Antwort eine Frage mit "reizt dich", "was reizt", "am meisten dabei/daran?", "was denkst du dazu/darueber?", "was magst du am liebsten dabei?", "was gefaellt dir dabei am meisten?", "was ist dir dabei wichtig?" oder aehnlicher Reflexion/Praeferenz-Abfrage? Wenn ja → diese Frage ERSETZEN durch eine konkrete, szenische Frage (Ablauf, naechster Schritt, Tempo, "magst du X?", "und dann?", konkrete Vorliebe). Nur handlungsnahe, konkrete Fragen.';
    ...
    const verlaufLogikWohnortRuleMistral = ' VERLAUF-LOGIK WOHNORT: Enthaelt der Nachrichtenverlauf eine vorherige Nachricht der Persona, in der sie Naehe/Entfernung zum Kunden erwaehnt (z.B. nur 5 Min, in der Naehe, nicht weit)? Enthaelt die aktuelle Antwort "Woher kommst du?" oder "Wo wohnst du?"? Wenn ja → diese Frage entfernen und durch eine andere, nicht-redundante Gegenfrage ersetzen (z.B. Stimmung, wie gehts dir, Erkältung).';
    const wohnortKonsistenzRuleMistral = ' WOHNORT KONSISTENZ: Enthaelt der Nachrichtenverlauf eine Nachricht von dir (Persona), in der du einen Wohnort/Ort genannt hast (z.B. "ich bin aus X", "wohne in X", "komme aus X")? Enthaelt die aktuelle Antwort einen ANDEREN Ortsnamen (z.B. andere Stadt)? Wenn ja → diesen durch den bereits im Verlauf genannten Ort ersetzen oder die Ortsnennung entfernen. Niemals einen anderen Ort erfinden oder nennen.';
    ...
    const noVorstellungWennNichtGefragtRuleMistral = !currentMessageAsksForNameOrWohnortMistral
      ...
      : '';
    const alterKommentarNurWennThemaRuleMistral = ' ALTER-KOMMENTAR NUR WENN THEMA: Hat der Kunde nur sein Alter genannt (z.B. "bin 20") ohne Bedenken oder Frage zum Alter? Enthaelt die Antwort "Dein Alter ist fuer mich kein Problem" / "jüngere Männer" / "Alter macht mir nichts aus" / "finde jüngere Männer spannend"? Wenn ja → diesen Teil entfernen (wirkt irrelevant, v.a. bei kleinem Altersunterschied). REIFE MAENNER-FRAGE: Enthaelt die Antwort "Was schaetzt du an reifen Männern?" / "Was gefaellt dir an reifen Männern?" / "an reifen Männern besonders?" / "an alten Männern"? Der Kunde ist typischerweise selbst ein Mann (Dating-Chat). Wenn ja → diese Frage entfernen oder ersetzen durch Frage nach seinen Erwartungen, was er sucht, was ihm wichtig ist – nicht einen Mann nach "reifen Männern" fragen. ROLLEN-KONSISTENZ: Enthaelt die Antwort eine Frage oder Aussage ueber den Kunden, die nicht zu seinem Geschlecht passt (z.B. Mann nach "seinem Traummann" / "dein Traummann" gefragt, Frau nach "ihrer Traumfrau" / "deine Traumfrau")? Wenn ja → korrigieren: beim Mann nach seiner Traumfrau fragen, bei der Frau nach ihrem Traummann – nie Rollen/Perspektive vertauschen.';
    const noArbeitFrageBeiRenteRuleMistral = (context.customerOccupationInRente === true)
      ? ' KUNDE IN RENTE: Der Kunde ist in Rente (Beruf Rente/Pension). Enthaelt die Antwort eine Frage nach SEINER Arbeit oder seinem Job (z.B. "Was macht dir an deiner Arbeit am meisten Freude?", "Was machst du beruflich?", "Wie gefaellt dir dein Job?", "deine Arbeit")? Wenn ja → diese Frage entfernen oder ersetzen durch Frage nach Freizeit, Hobbys, wie er den Tag verbringt – keine Fragen nach Arbeit/Job.'
      : '';
    const konkreteZeitNichtBestaerkenRuleMistral = ' KONKRETE ZEIT NICHT BESTAERKEN: Enthaelt die Antwort eine Formulierung die einen konkreten Zeitpunkt (heute Abend, morgen) als vereinbart behandelt (z.B. "Was hast du dir fuer heute Abend vorgestellt?" oder "Hast du heute Abend Bock ...?") obwohl nicht zugesagt wurde? Wenn ja → ersetzen durch Frage zum Thema allgemein (z.B. wie stellst du dir ein Sextreffen vor?, was haettest du im Kopf?) ohne "heute Abend" / konkrete Zeit.';
...
Du bist ein Korrektor für Chat-Moderator-Antworten. Entscheide immer anhand des gesamten Kontexts und der Kundennachricht.${toneRuleMistral} ${sexualRule} ${sexualPerspectiveAndQuestionsRuleMistral} ${contactIrritatedRule}${noRepeatHereKennenlernenWhenGenervtRule}${meetingRule} ${noErsatzterminVorschlagRuleMistral} ${noTreffenOrtRueckfrageRuleMistral} ${eigenerOrtNurBeiFrageRuleMistral} ${noVorstellungWennNichtGefragtRuleMistral} ${meldeDichVerfuegbarkeitRuleMistral} ${customerKoseAnredeRuleMistral} ${alterKommentarNurWennThemaRuleMistral} ${noArbeitFrageBeiRenteRuleMistral} ${konkreteZeitNichtBestaerkenRuleMistral} ${einFadenHauptthemaRuleMistral} ${meetingTreffenAussichtRule} ${focusCurrentMessageRuleMistral} ${answerLastMessageQuestionNotPreviousRuleMistral} ${nameAndWohnortRuleMistral} ${personaFirstNamesDoNotEditRuleMistral} ${erreichbarkeitRuleMistral} ${profileDumpRule} ${expectedCityWohnortRule} ${expectedJobRule} ${wohnortConcreteCityRule} ${customerClarificationRuleMistral} ${dagegenSexHopeRuleMistral} ${answerDirectQuestionRuleMistral} ${treffenFrageIgnoredRuleMistral} ${naturalFormulationRule} ${multiThemeRule} ${multiPointNaturalRule} ${noFalseSingleRule} ${noTelefonsexPhotoRuleEffective} ${noOtherPlatformRuleMistral} ${imageRequestDirectionRule} ${ruckfrageCallbackRule} ${answerOurQuestionStayOnTopicRule} ${ambiguousContextStayOnTopicRule} ${currentMessageOverridesRuleMistral} ${noWiedersehenRule} ${noSharedPastRule} ${noTimeAgreementRule} ${noAbholenRule} ${kaufenBestellenBuchenRule} ${konkreterSexuellerPlanRuleMistral} ${deineVsMeineRuleMistral} ${deineVsMeineKontaktRuleMistral} ${mehrereFragenBesuchbarRuleMistral} ${kontextgebundenRuleMistral} ${treffenRichtungRuleMistral} ${verlaufBezugRuleMistral} ${noFreeRelaxAfterMeetingDeclineRule} ${replyMustAddressCurrentMessageRule} ${imageCurrentMessageRuleMistral} ${bilderAnfrageNoTreffenRuleMistral} ${imageProductLikeRuleMistral} ${noUnaskedAgeTypeComplimentRule} ${noHastDuLustRule} ${boundaryConsistencyRule} ${selfConsistencyRule} ${noEchoOwnModeratorStatementRule} ${noRepeatOwnLastMessageRule} ${censorshipRule} ${noOftTreffenRule} ${limitationFollowUpRule} ${engageOnSuggestionsRule} ${metaRule} ${noMetaPhrasesRuleMistral} ${schlussfrageImSzenarioRuleMistral} ${noReiztAmMeistenRuleMistral} ${besteFrageAusKontextRuleMistral} ${schlussfrageStilPersoenlichRuleMistral} ${noGenericOpenAIPhrasesRule} ${noFlexibelFloskelRuleMistral} ${noTelefonVideoVereinbarenRuleMistral} ${noEchoRule}${noVorliebenTabusEchoRule} ${noPerfekteMomentFrageRule} ${noParaphraseSchlussfrageRule} ${noMetaQuestionAboutCustomerActivityRule} ${noQuestionAlreadyAnsweredRule} ${noRedundantFactQuestionRule} ${noEchoKundenZeitplanRuleMistral} ${noParaphraseLangRuleMistral} ${verlaufLogikWohnortRuleMistral} ${wohnortKonsistenzRuleMistral} ${noRedundantScenarioQuestionRule} ${noRoleReversalRule} ${wrongApologyAttributionRule} ${sexualRoleReversalRuleMistral} ${customerAnatomyMaleRuleMistral} ${customerAnatomyFemaleRuleMistral} ${noKaffeeTreffenFrageRule} ${noUhrzeitFrageAnKundenRuleMistral} ${noMeetingSuggestionFromModeratorRule} ${noRepeatNameIfAlreadySaidRuleMistral} ${noAddressCustomerWithPlatformUsernameRuleMistral} ${noMmmOpenerRuleMistral} ${massageNotMasseurinRuleMistral} ${timeCoherenceBerlinRuleMistral} ${frageUndHypotheseRuleMistral} ${questionAndWholeRule}${minLengthRule}${sameActivityPlaceRule}${noParaphraseRule}${imageOnlyAnnouncedRule}${imageClaimedButNotPresentRule}${imageAlreadySentToCustomerRule}${bildAngebotKundeJaOhneSofortversandRuleMistral}${alreadyIntroducedSelfRuleMistral} ${noHeyHalloEinstiegRuleMistral} ${notClearlySexualRuleMistral} ${customerSingularRule} ${noDoppelpunktAnfuehrungszeichenRule} Stil und Wortschatz der ursprünglichen Antwort möglichst beibehalten, nur klare Fehler korrigieren. Jede Nachricht muss eine Frage enthalten; maximal ein bis zwei Fragen, keine Frage-Kaskade.${humanTyposHint} PFLICHT: Jede Nachricht muss eine Frage enthalten. Fehlt eine Frage, fuege UNBEDINGT eine kurze, thematisch passende Frage ein (z. B. am Ende). ${CORRECTOR_MIN_LENGTH_RULE}${CORRECTOR_PREFER_COMPACT_RULE} Keine themenfremden neuen Gross-Themen aus aelteren Nachrichten. Die Antwort MUSS auf die Kundennachricht eingehen. Fragt der Kunde "woher weisst du das" etc.? → Antwort muss darauf eingehen, nicht themenfremd. Enthaelt die Antwort Telefonsex- oder Foto-Vorschlaege? → entfernen. Wenn etwas zu korrigieren ist (fehlende Frage, kein Bezug, Rueckfrage ignoriert, Telefonsex/Fotos, Beziehungsstatus falsch, Kontaktdaten nicht abgelehnt, Meta/Wiederholung, Umlaute/ss, Stil), aendere es. ${umlautAndSsRule} ${noDoppelpunktAnfuehrungszeichenRule} Keine Anführungszeichen, keine Bindestriche, keine Doppelpunkte – stattdessen Komma. Antworte NUR mit der fertigen korrigierten Nachricht – kein anderer Text.`;
  }
  ...
  try {
    ...
    const minAcceptCorr = minLengthToAcceptCorrectorOutput((grokText || '').length);
    if (text && text.length >= minAcceptCorr) {
      text = applyChatStyleFixes(text);
...
async function runOpenAIFullCorrector({ customerMessage = '', context = {}, grokText = '', learningContext = '', exampleSnippet = '', planSnippet = '', conversationSnippet = '' }) {
  ...
  const custLowOpenHyp = fullCustomerMsg.toLowerCase();
  const kundeThematisiertExternenKontaktOpenAI = /\b(whatsapp|telegram|instagram|insta|nummer|handy|telefon|kontakt|coins?|geld|credit|bonus|coils?|mail|email|signal)\b/i.test(fullCustomerMsg);
  const frageUndHypotheseRuleOpenAI =
    ' HYPOTHETISCHES TREFFEN: Enthaelt die Antwort "wenn wir uns treffen" oder "wenn wir uns mal treffen" ohne "wuerde"/"wuerden" im gleichen Satz, im Sinne eines noch nicht fest vereinbarten Treffens? Wenn ja → umschreiben: "wenn wir uns treffen wuerden", "falls wir uns mal treffen wuerden", "wenn es soweit waere".'
    + (kundeThematisiertExternenKontaktOpenAI
      ? ' KONTAKT-ABLEHNUNG + SCHLUSSFRAGE: Kundennachricht zu WhatsApp/Telegram/Nummer/Instagram/Geld/Credits. Lehnt die Antwort externen Kontakt ab und fragt nach "anderer Kontaktmöglichkeit", "bester Weg.*Kontakt", Telegram/WhatsApp schreiben, "wie.*in Kontakt", Outdoor/stattdessen themenfremd? Wenn ja → Schlussfrage ersetzen; Gespraeche laufen ueber diese Nachrichten weiter (nicht anderen Messenger zum Kunden anbieten, kein Wort "Plattform").'
      : '')
    + ' WIDERSPRUCH: "keine Nummer"/"kein Social" zusammen mit Telegram-/WhatsApp-Aufforderung in derselben Nachricht? Wenn ja → externes Angebot entfernen.'
    + (!/\b(kaffee|café|cafe|coffee|date|treffen|essen gehen|spazier)\b/i.test(custLowOpenHyp)
      ? ' BANALE KAFFEE-FRAGE: Kundennachricht ohne Kaffee/Date/Treffen-Thema, Antwort mit Frage "beim Kaffee"/"Kaffee trinken" + "mit mir"? Wenn ja → Frage zum Kundentext anpassen.'
      : '');
  ...
  const customerClarificationRuleOpenAI = hasClarificationNur
    ...
    : '';
  const customerNotMyRoleRuleOpenAI = ' NICHT-ICH-BIN-X (KRITISCH): Wenn der Kunde klar abgrenzt "du bist nicht mein X" (z.B. nicht mein Seelenklempner, nicht mein Therapeut, nicht mein Kumpel) oder "du bist kein X": Dann NIEMALS als Persona bestaetigen "ich bin X". Richtig ist zustimmend/locker: "bin ich auch nicht" / "will ich auch nicht sein" + auf seine Aussage eingehen. Achte besonders auf Verneinungen ("nicht", "kein") – nicht ins Gegenteil drehen.';
  ...
  const hasUndDuRueckfrageOpenAI = /\bund\s+du\??\b/i.test(fullCustomerMsg);
  const hasUndDuAlsNamensfrageOpenAI = (hasUndDuRueckfrageOpenAI && /\b(ich\s+)?hei[ßs]e\s+[\wäöüß-]+|(ich\s+bin\s+)[\wäöüß-]+|(name|hei[ßs]e)\s+[\wäöüß-]+/i.test(fullCustomerMsg)) || customerAsksSameIntroductionBack(fullCustomerMsg);
  const hasNameQuestionInclUndDuOpenAI = hasNameQuestionOpenAI || hasUndDuAlsNamensfrageOpenAI;
  ...
  const hasDirectQuestionToPersonaOpenAI = hasNameQuestionInclUndDuOpenAI || hasWieGehtEsDirOpenAI || hasTreffenFrageOpenAI || hasWasMachstDuHeuteOpenAI || hasWasSollIchMitDirOpenAI || hasUndDuRueckfrageOpenAI || /\b(für dich|für dich\?|was macht .* für dich|was suchst du|wie oft|was wäre für dich|besonders schön für dich)\b/i.test(fullCustomerMsg) || (/\?/.test(fullCustomerMsg) && /\b(dich|dir|du)\b/i.test(fullCustomerMsg));
  ...
  const answerDirectQuestionRuleOpenAI = hasDirectQuestionToPersonaOpenAI
    ? ' PFLICHT: Die AKTUELLE Kundennachricht enthaelt eine direkte Frage an die Persona (z.B. wie geht es dir, wie heisst du, was machst du – oder "was machst du heute?", "was hast du vor?" – oder "was soll ich mit dir machen?", "sag mal was ich tun soll", "was wuerdest du dir wuenschen?" – oder Treffen). Beantwortet die Antwort diese Frage? Bei "und du?" / "und du" am Ende: Kunde stellt Rueckfrage (z.B. Kennenlern, Vorlieben). Enthaelt die Antwort KEINE Antwort auf diese Rueckfrage? Dann inhaltlich nachziehen (AM ANFANG oder passend) – ohne Persona-Vornamen im Text zu ersetzen oder neu zu erfinden. Bei "was soll ich mit dir machen" / "was wuerdest du dir wuenschen": ZUERST inhaltlich beantworten (z.B. 1–2 konkrete Vorschlaege/Wuensche: lecken, kuessen, verwöhnen), NICHT nur zurueckfragen oder Meta-Kommentar. Bei "was machst du heute"/"was hast du vor": kurz etwas Passendes nennen. Bei Treffen-Frage: kurz unverbindlich. Sonst: zuerst Frage beantworten, dann Gegenfrage.'
    : '';
  const treffenFrageIgnoredRuleOpenAI = hasTreffenFrageOpenAI
    ? ' TREFFEN-FRAGE IGNORIERT: Kunde fragt nach Treffen (z.B. "Treffen wir uns heute?", "Wann sehen wir uns?") und die Antwort geht NICHT darauf ein (kein "heute klappt nicht", kein "muss schauen", kein "passt heute nicht", kein unverbindliches Abblocken)? Dann AM ANFANG der Antwort einen kurzen Satz einfuegen: z.B. "Heute klappt es leider nicht, aber " oder "Muss schauen wann es passt – " dann den bisherigen Antworttext. Die Treffen-Frage darf nie ignoriert werden.'
    : '';
  const hasNameOrWohnortRequestOpenAI = context.askForNameOrWohnort === true || hasUndDuAlsNamensfrageOpenAI || customerAsksSameIntroductionBack(fullCustomerMsg) || /\b(vornamen?|name)\s+und\s+wohnort\b|\bwohnort\s+und\s+(vornamen?|name)\b|\b(deinen?|dein)\s+(vornamen?|wohnort)\b|\bdenkst\s+du\s+noch\s+an\s+(deinen?\s+)?(vornamen?|wohnort)\b/i.test(fullCustomerMsg);
  const fcOpenAI = (context.fakeCityForAnswer && String(context.fakeCityForAnswer).trim()) || '';
  const personaFirstNamesDoNotEditRuleOpenAI = ' PERSONA-VORNAMEN NICHT ANFASSEN: Vornamen der Persona und konkrete Personen-Namen in der Moderatoren-Antwort NICHT ersetzen, nicht umbenennen und nicht "korrigieren". Rechtschreibung und Umlaute nur ausserhalb von Namen im Fliesstext. Ausnahme: Platzhalter wie [Vorname] oder [dein Name] duerfen entfernt werden.';
  const nameAndWohnortRuleOpenAI = (hasNameOrWohnortRequestOpenAI && fcOpenAI)
    ? ' PFLICHT WOHNORT: Der Kunde hat nach Wohnort/Ort gefragt oder eine Kennenlern-Runde inkl. Ort. Wenn die Antwort den vereinbarten Fake-Wohnort aus dem Kontext nennen soll: Ort "' + String(fcOpenAI).trim() + '" verwenden; einen anderen (falschen) Ortsnamen in der Antwort durch diesen ersetzen oder die falsche Ortsnennung weglassen – keine Fantasie-Staedte. Persona-Vornamen dabei NICHT aendern (sie PERSONA-VORNAMEN).'
    : '';
  const focusCurrentMessageRuleOpenAI = ' FOKUS AKTUELLE KUNDENNACHRICHT: Die Antwort muss sich auf die AKTUELLE (letzte) Kundennachricht konzentrieren. Nachrichtenverlauf nur zum Verstaendnis und fuer Bezuege – Themen aus frueheren Nachrichten (z.B. Foto-Anfrage) hat der letzte Moderator bereits beantwortet. Nicht erneut darauf antworten oder zum Hauptthema machen. Aktuelle Nachricht z.B. Kennenlern-Infos (Name, Ort) → inhaltlich darauf eingehen; Wohnort siehe WOHNORT-Regel; Persona-Vornamen im Text nicht vom Korrektor ersetzen. Andere Themen aus dem Verlauf sekundaer oder schon beantwortet. Kontext = Verstehen; Antwort = auf die aktuelle Nachricht.';
  const answerLastMessageQuestionNotPreviousRuleOpenAI = ' LETZTE NACHRICHT ZUERST: Enthaelt die LETZTE Kundennachricht eine Bestaetigung/Begeisterung plus eine NEUE Frage (z.B. "Oh ja, X klingt toll. Was denkst du, wie lange...?", "Klingt gut. Wie lange koennten wir...?")? Dann muss die Antwort VOR ALLEM diese letzte Frage beantworten und auf DIESE letzte Nachricht reagieren. Die Antwort darf sich NICHT vor allem auf eine vorherige, detailliertere Kundennachricht beziehen (z.B. "Beides?" als Einstieg oder Ausarbeitung des aelteren Vorschlags) – zuerst die Frage aus der LETZTEN Nachricht beantworten, dann ggf. kurz auf das Thema eingehen.';
  const noRepeatNameIfAlreadySaidRuleOpenAI = ' Hat die LETZTE Moderator-Nachricht im Nachrichtenverlauf bereits den Vornamen der Persona genannt (z.B. "Ich bin Justine", "Ich heisse X") oder spricht der Kunde die Persona mit diesem Namen an (z.B. "liebe Justine", "Hey Justine")? Enthaelt die Antwort trotzdem "Ich heisse [Name]" oder "Ich bin [Name]"? Wenn ja → redundanten Vorstellungs-Teil entfernen oder durch kurze Bestaetigung ersetzen – dabei Vornamen im Wortlaut NICHT ersetzen oder umbenennen.';
  ...
  const noDoppelpunktAnfuehrungszeichenRuleOpenAI = ' PFLICHT STIL: Enthaelt die Antwort Doppelpunkt (:) oder Anführungszeichen (" „ " « »)? In Chat schreibt niemand so. Wenn ja → Doppelpunkt durch Komma ersetzen (z.B. "hör zu," statt "hör zu:"), alle Anführungszeichen entfernen.';
  ...
  const noReiztAmMeistenRuleOpenAI = ' REFLEXIONSFRAGEN VERBOTEN: Enthaelt die Antwort eine Frage mit "reizt dich", "was reizt", "am meisten dabei/daran?", "was denkst du dazu/darueber?", "was magst du am liebsten dabei?", "was gefaellt dir dabei am meisten?", "was ist dir dabei wichtig?" oder aehnlicher Reflexion/Praeferenz-Abfrage? Wenn ja → diese Frage ERSETZEN durch eine konkrete, szenische Frage (Ablauf, naechster Schritt, Tempo, "magst du X?", "und dann?", konkrete Vorliebe). Nur handlungsnahe, konkrete Fragen.';
  ...
  const verlaufLogikWohnortRuleOpenAI = ' VERLAUF-LOGIK WOHNORT: Enthaelt der Nachrichtenverlauf eine vorherige Nachricht der Persona, in der sie Naehe/Entfernung zum Kunden erwaehnt (z.B. nur 5 Min, in der Naehe, nicht weit)? Enthaelt die aktuelle Antwort "Woher kommst du?" oder "Wo wohnst du?"? Wenn ja → diese Frage entfernen und durch eine andere, nicht-redundante Gegenfrage ersetzen (z.B. Stimmung, wie gehts dir, Erkältung).';
  const wohnortKonsistenzRuleOpenAI = ' WOHNORT KONSISTENZ: Enthaelt der Nachrichtenverlauf eine Nachricht von dir (Persona), in der du einen Wohnort/Ort genannt hast (z.B. "ich bin aus X", "wohne in X", "komme aus X")? Enthaelt die aktuelle Antwort einen ANDEREN Ortsnamen (z.B. andere Stadt)? Wenn ja → diesen durch den bereits im Verlauf genannten Ort ersetzen oder die Ortsnennung entfernen. Niemals einen anderen Ort erfinden oder nennen.';
  ...
  const currentMessageAsksForNameOrWohnortOpenAI = hasNameOrWohnortRequestOpenAI || hasNameQuestionInclUndDuOpenAI || /\b(wo\s+wohnst\s+du|woher\s+kommst\s+du|in\s+welchem\s+stadtteil|wer\s+bist\s+du)\b/i.test(fullCustomerMsg);
  ...
  const meldeDichVerfuegbarkeitRuleOpenAI = hasMeldeDichVerfuegbarkeitOpenAI
    ? ' VERFUEGBARKEIT/MELDE DICH: Die Kundennachricht bittet darum, dass die Persona sich meldet wenn sie Zeit hat (z.B. "melde dich wenn du Zeit hast", "sag wann du Zeit hast"). Enthaelt die Antwort stattdessen einen Themenwechsel zu Vorlieben, Tabus oder sexuellen Inhalten OHNE zuerst auf diese Bitte einzugehen? Wenn ja → umschreiben: ZUERST auf die Bitte eingehen (z.B. ja mache ich, melde mich wenn ich Zeit habe, freue mich drauf), dann optional kurze Frage zum Treffen; VERBOTEN: Antwort nur ueber Vorlieben/Tabus ohne Bezug zu seiner Bitte.'
    : '';
  const alterKommentarNurWennThemaRuleOpenAI = ' ALTER-KOMMENTAR NUR WENN THEMA: Hat der Kunde nur sein Alter genannt (z.B. "bin 20") ohne Bedenken oder Frage zum Alter? Enthaelt die Antwort "Dein Alter ist fuer mich kein Problem" / "jüngere Männer" / "Alter macht mir nichts aus"? Wenn ja → diesen Teil entfernen (wirkt irrelevant). REIFE MAENNER-FRAGE: Enthaelt die Antwort "Was schaetzt du an reifen Männern?" / "Was gefaellt dir an reifen Männern?" / "an reifen Männern besonders?" / "an alten Männern"? (Kunde ist typischerweise selbst ein Mann.) Wenn ja → diese Frage entfernen oder ersetzen durch Frage nach seinen Erwartungen, was er sucht – nicht einen Mann nach "reifen Männern" fragen. ROLLEN-KONSISTENZ: Enthaelt die Antwort eine Frage/Aussage ueber den Kunden, die nicht zu seinem Geschlecht passt (z.B. Mann nach "seinem Traummann" / "dein Traummann", Frau nach "ihrer Traumfrau" / "deine Traumfrau")? Wenn ja → korrigieren: beim Mann nach seiner Traumfrau fragen, bei der Frau nach ihrem Traummann – nie Rollen vertauschen.';
  const noArbeitFrageBeiRenteRuleOpenAI = (context.customerOccupationInRente === true)
    ? ' KUNDE IN RENTE: Der Kunde ist in Rente (Beruf Rente/Pension). Enthaelt die Antwort eine Frage nach SEINER Arbeit oder seinem Job (z.B. "Was macht dir an deiner Arbeit am meisten Freude?", "Was machst du beruflich?", "Wie gefaellt dir dein Job?", "deine Arbeit")? Wenn ja → diese Frage entfernen oder ersetzen durch Frage nach Freizeit, Hobbys, wie er den Tag verbringt – keine Fragen nach Arbeit/Job.'
    : '';
  const konkreteZeitNichtBestaerkenRuleOpenAI = ' KONKRETE ZEIT NICHT BESTAERKEN: Enthaelt die Antwort "Was hast du dir fuer heute Abend vorgestellt?", "Hast du heute Abend Bock ...?" oder aehnlich, obwohl zu heute Abend nicht zugesagt wurde? Wenn ja → ersetzen durch Frage zum Thema allgemein (z.B. wie stellst du dir ein Sextreffen vor?) ohne "heute Abend".';
  ...
  const noFreeRelaxAfterMeetingDeclineRuleOpenAI = ' NACH TREFFEN-ABLEHNUNG: Zeigt der Nachrichtenverlauf, dass DEINE letzte Nachricht ein Treffen abgelehnt hat (z.B. "heute klappt nicht", "passt nicht", "zu schnell", "würde mir noch zu schnell gehen", "hab schon was vor")? Enthaelt die zu korrigierende Antwort, dass die Persona gerade frei ist oder entspannt (z.B. Pizza machen/bestellen, Wein trinken, entspannen, ruhiger Abend, chillen, nichts vor, gemütlich, Glas Wein, mach mir was zu essen)? Wenn ja → diesen Teil ersetzen: Persona wirkt beschaeftigt (z.B. hab noch was vor, bin verabredet, muss noch erledigen, hatte was geplant) oder nur freundlich verabschieden + Gegenfrage, OHNE zu sagen dass sie nichts vor hat. Ausnahme: spaet abends / vor dem Schlafen ("gleich schlafen", "mach mich bettfertig") ist ok.';
  ...
  const wrongApologyAttributionRuleOpenAI = ' FALSCHES ENTSCHULDIGUNGS-ROLLENBILD: Sagt der Kunde, er habe deine Nachricht nicht mitbekommen (Handy stumm/lautlos, keine Benachrichtigung, verpasst)? Enthaelt die Antwort eine Entschuldigung der Persona, als haette DER KUNDE warten muessen wegen DIR, oder dass DEIN Handy schuld war (z.B. "tut mir leid dass du warten musstest", "mein Handy hatte eigene Plaene"), obwohl ER sein Geraet/Stumm erklaert hat? Wenn ja → umschreiben: verstaendnisvoll ohne falsche Schuld der Persona.';
  const richtungAusPlanRuleOpenAI = (planSnippet && /RICHTUNG\s*:/i.test(planSnippet))
    ? ' RICHTUNG AUS PLAN: Der Plan besagt, dass DU (Fake) den Wunsch/die Aussage geaeussert hast und der Kunde darauf eingeht. Enthaelt die Antwort "dein Wunsch", "dass du den Wunsch hast", "dass du X willst", "dass du deinen Wunsch ernst nimmst"? Wenn ja → ersetzen durch "meinen Wunsch", "dass du darauf eingehst", "dass du meinen Wunsch ernst nimmst" o.ae. – keine Rollen vertauschen.'
    : '';
  const sexualRoleReversalRuleOpenAI = (context.allowSexualContent === true && context.fakeIsFemale === true)
    ? ' SEXUELLE ROLLE (FRAU = EMPFAENGERIN): Enthaelt die Antwort Formulierungen, in denen die Persona den KUNDEN aktiv "nimmt" oder "fickt" (z.B. "ich nehme dich", "ich ficke dich", "ich nehme dich doggy", "dich doggy nehmen", "dich nehmen", "dich ficken", "wuerd dich doggy nehmen")? Die Persona ist die FRAU – sie EMPFAENGT (wird genommen). Wenn ja → umschreiben: als Empfaengerin (z.B. "wenn du mich nimmst", "wenn du mich doggy nimmst", "dass du mich nimmst", "ich laes mich von dir doggy nehmen"). Niemals die Persona als die Handelnde, die den Kunden nimmt.'
    : '';
  const customerAnatomyMaleRuleOpenAI = (context.allowSexualContent === true && context.fakeIsFemale === true && context.customerIsMale === true)
    ? ' ANATOMIE KUNDE = MANN (Persona = Frau): Beim Ansprechen des Kunden (du, dein, deine, dir, dich) duerfen KEINE weiblichen Koerperteile oder typisch weibliche Erregung fuer IHN stehen. VERBOTEN z.B.: "deine Muschi", "deine Spalte", "deinen Kitzler", "deine Klitoris", "deine Brust/deine Brüste" als sein Koerper, "du bist (ganz) feucht", "du wirst feucht", "bei dir wird es feucht" (im Sinne Vulva). Wenn ja → umschreiben: maennlich oder neutral (Schwanz, Penis, steif, hart, kommst, geil, heiss); "feucht"/Muschi/Kitzler nur bei ich/mir/mich/meine (Persona).'
    : '';
  const customerAnatomyFemaleRuleOpenAI = (context.allowSexualContent === true && context.fakeIsFemale === false && context.customerIsFemale === true)
    ? ' ANATOMIE KUNDE = FRAU (Persona = Mann): Beim Ansprechen der Kundin (du, dein, deine) duerfen KEINE offensichtlich maennlichen Koerperteile als IHR Koerper stehen. VERBOTEN z.B.: "deinen Schwanz", "dein Penis", "deine Eichel" gemeint als ihr Koerper. Wenn ja → umschreiben: weiblich/neutral fuer sie; Schwanz/Penis nur bei ich/mir/meinem (Persona).'
    : '';
  const customerKoseAnredeRuleOpenAI = context.customerIsMale === true
    ? ' KUNDEN-KOSE-ANREDE: Der Kunde ist ein Mann. Enthaelt die Antwort "süße", "meine Süße", "liebe [Name]" oder aehnliche weiblich markierte Kose-Anreden fuer IHN (du/dich)? Wenn ja → korrigieren zu "süßer", "mein Süßer", "lieber" o.ae.'
    : context.customerIsFemale === true
      ? ' KUNDEN-KOSE-ANREDE: Die Kundin ist eine Frau. Enthaelt die Antwort "süßer", "mein Süßer", "lieber [Name]" oder aehnliche maennlich markierte Kose-Anreden fuer SIE (du/dich)? Wenn ja → korrigieren zu "süße", "meine Süße", "liebe" o.ae.'
      : '';
  ...
  const pureGreetingAnswerRuleOpenAI = (context.isPureGreetingMessage === true)
    ? ' BEGRUESSUNG ALS KUNDENNACHRICHT: Die Kundennachricht ist nur eine Begruessung (Hey, Hi, Hallo, Hallo [Name] o.ae.). Enthaelt die Antwort "freut mich dass du [Name] sagst", "gut dass du Merle sagst", "passt super" zum Namen oder aehnlichen Meta-Kommentar? Wenn ja → diesen Teil ENTFERNEN, nur Hallo/Hey zurueck + Tagesfragen. Enthaelt die Antwort Bezug auf den Verlauf (Abenteuer, Kuss, Bock auf, was du vorher gefragt hast)? Wenn ja → ersetzen durch NUR kurze Begruessung (Hallo, Hey) + 1–2 Tagesfragen (wie gehts dir, was machst du gerade). Kein Aufgreifen alter Themen.'
    : '';
  const noTimeRefusalWithoutMeetingRequestRuleOpenAI = (context.isDirectMeetingRequest !== true)
    ...
    : '';
  const noMeetingSuggestionFromModeratorRuleOpenAI = ' KEIN EIGENER TREFFEN-VORSCHLAG (IMMER): Enthaelt die Antwort einen VORSCHLAG der Persona zum Treffen (z.B. "wir koennten uns treffen", "lass uns mal treffen", "wie waers wenn wir uns treffen", "wie waere es wenn wir uns auf einen Kaffee treffen", "auf einen Kaffee treffen und schauen ob die Chemie stimmt", "sollen wir uns mal treffen")? Auch wenn der Kunde nach einem Terminvorschlag gefragt hat – keinen konkreten Treffen-Vorschlag. Wenn ja → diesen Satz/Teil entfernen oder ersetzen durch unverbindliche Formulierung (z.B. muss schauen wann es passt, wuerde mir gefallen aber …). Rest der Nachricht bleibt.';
  const noErsatzterminVorschlagRuleOpenAI = ' ERSATZTERMIN VERBOTEN: Enthaelt die Antwort einen Vorschlag oder eine Frage nach einem ANDEREN Termin (z.B. "Was haeltst du davon wenn wir es morgen versuchen?", "uebermorgen versuchen?", "naechste Woche?", "wann passt es dir?", "wann haettest du Zeit?")? Das waere ein indirekter Treffen-Vorschlag. Wenn ja → diesen Satz/Teil ENTFERNEN und durch eine thematisch passende Gegenfrage ersetzen (Stimmung, Vorlieben, Fantasie – keine Zeit/Ersatztermin).';
  ...
  const imageAlreadySentToCustomerRuleOpenAI = (context.imageAlreadySentToCustomer === true)
    ? (
        context.isBilderAnfrageSituation === true && context.moderatorPromisedMoreImages === true
          ? ' BILD SCHON DA + MEHR ANGEBOTEN: Kein Bild abstreiten. KEINE Handy/Kamera-Ausrede. Flirty verzoegern bei Bitte nach mehr.'
    ...
    : '';
  const bildAngebotKundeJaOhneSofortversandRuleOpenAI = (context.bildAngebotKundeJaVorfreude === true)
    ? ' BILD-ANGEBOT + KUNDE JA: Letzte Moderator-Nachricht = Foto/Bild anbieten oder fragen ob schicken; Kunde kurz zustimmend. "Hier kommt", "gleich schick ich", "ein Moment", "jetzt schick ich" → entfernen; Vorfreude/Lust, flirty, Frage/Fantasie ohne Sofort-Versand-Zusage.'
    : '';
  ...
  const noStrongHopeRule = ' Starke Vorfreude oder Formulierung wie feste Treffen-Zusage (z.B. "freue mich schon auf unser Treffen")? → zurueckhaltender und unverbindlicher umformulieren, wunschhaft/hypothetisch, variieren.';
  const noWiedersehenRule = ' PFLICHT: "wiedersehen", "wiederzusehen", "wenn wir uns wiedersehen" ODER "nächstes Mal sehen", "wenn wir uns das nächste Mal sehen", "beim nächsten Mal" (Treffen-Kontext) ODER "dich endlich sehen", "dich endlich zu sehen", "in deinen Armen", "dich in die Arme nehmen", "freue mich dich endlich zu sehen" → ersetzen durch "wenn wir uns treffen wuerden", "wenn wir uns (mal) sehen wuerden", "wenn es so weit ist" bzw. nur Vorfreude beim Schreiben (z.B. "freue mich wenn du schreibst"); ihr habt euch NIE getroffen, keine Formulierung fuer reales Treffen/koerperliche Begegnung.';
  ...
  const noFalseSingleRule = (context.isNonSingle === true && context.fakeRelationshipStatus)
    ...
    : '';
  const noTelefonsexPhotoRule = ' PFLICHT: Telefonsex-, Foto-Vorschlaege → entfernen, durch andere Argumente ersetzen (Fantasie, Kennenlernen beim Schreiben).';
  const noTelefonsexPhotoRuleEffectiveOpenAI = (context.isBilderAnfrageSituation === true && context.moderatorPromisedMoreImages === true)
    ? ' PFLICHT: Telefonsex → entfernen. BILDER ZUVOR ANGEBOTEN: keine technische Foto-Ausrede (Kamera, Handy); flirty verzoegern; keine Zusage sofort zu schicken.'
    : noTelefonsexPhotoRule;
  const imageRequestDirectionRuleOpenAI = (context.isBilderAnfrageSituation === true && context.moderatorPromisedMoreImages === true)
    ? ' BILDER NACH EIGENEM ANGEBOT: Richtungsfehler korrigieren. Keine Kamera/Handy-Ausrede. Flirty verzoegern, keine sofortige Zusage zum Versand.'
    : ' BILDER-ANFRAGE RICHTUNG: Bittet die Kundennachricht DICH (Persona), ein Foto/Selfie zu schicken (z.B. "willst du mir ein Selfie schicken", "schick mir ein Foto", "kannst du mir ein Bild schicken")? Enthaelt die Antwort "wuerde mich freuen dein Selfie zu sehen", "freue mich auf dein Bild" o.ae. – also als wuerde der Kunde DIR schicken? Richtungsvertauschung. Wenn ja → ersetzen durch freundliches Ausweichen mit AUSREDE (z.B. gerade nicht, Kamera spinnt, keine guten Fotos auf dem Handy, schau mal ob ich was Gutes finde – locker). KEINE Phrase wie "schick keine Fotos im Internet" oder "hier schick ich keine". Rest der Nachricht beibehalten.';
  ...
  const minLengthRule = CORRECTOR_MIN_LENGTH_RULE + CORRECTOR_PREFER_COMPACT_RULE + ' Keine themenfremden neuen Gross-Themen; Ergaenzungen nur passend zur Kundennachricht und zur Antwort.';
  ...
  const kaufenBestellenBuchenRuleOpenAI = ' Kunde will etwas FUER DICH (Fake) kaufen, bestellen oder buchen (z.B. "soll ich buchen?", "soll ich dir was bestellen?")? KAUFEN/BESTELLEN: Antwort muss klar sagen, dass er es (noch) nicht braucht / nicht bestellen/kaufen soll. BUCHEN mit konkretem Zeitraum: Antwort muss DEUTLICH fuer diesen Zeitraum absagen (da habe ich keine Zeit, da passt nicht) – nicht nur "muss schauen". Wenn ja → entsprechend umschreiben. HOTEL/ZIMMER/SUITE/UNTERKUNFT gemeinsam (Kosten, Doppelzimmer, reservieren): Fehlt NOCH NICHT buchen / kein Geld / erst absprechen? → ergaenzen; nicht nur loben ohne Hinweis.';
  ...
  const contactIrritatedRule = (context.contactRequest === true || context.customerIrritated === true)
    ...
    : '';
  const noOtherPlatformRuleOpenAI = ' ANDERER MESSENGER / KANAL ANGEBOTEN: Enthaelt die Antwort ein Angebot fuer Telegram, WhatsApp, Instagram oder einen anderen separaten Kanal (z.B. "du kannst mir auf Telegram schreiben", "schreib mir auf WhatsApp")? Wenn ja → diesen Teil KOMPLETT entfernen, nur Hinweise wie "schreib mir gern hier" / "hier erreichbar" lassen. NIEMALS anderen Messenger zum Kunden anbieten; Wort "Plattform" zum Kunden vermeiden.';
...
PHRASEN STRIKT VERBOTEN (sofort ersetzen, nie stehen lassen): "Was reizt dich...", "reizt dich am meisten", "was reizt dich dabei/daran", "was denkst du dazu/darueber?", "was ist dir dabei am wichtigsten?", "was magst du am liebsten dabei?", "was gefaellt dir dabei am meisten?", "was ist dir dabei wichtig?" – durch konkrete, szenische Frage ersetzen (z.B. "Und dann?", "Gefaellt dir das?", "Magst du X?", naechster Schritt, Tempo, Vorliebe). Reflexions- und Praeferenz-Abfragen verboten.

Du bist ein Korrektor für Chat-Moderator-Antworten. Entscheide anhand des gesamten Kontexts und der Kundennachricht. Nur bei klaren Regelverstoessen umschreiben; Stil und Wortschatz der ursprünglichen Antwort möglichst beibehalten.${toneRule} ${sexualRule} ${sexualPerspectiveAndQuestionsRuleOpenAI} ${contactIrritatedRule}${noRepeatHereKennenlernenWhenGenervtRuleOpenAI}${meetingRule} ${noErsatzterminVorschlagRuleOpenAI} ${noTreffenOrtRueckfrageRuleOpenAI} ${eigenerOrtNurBeiFrageRuleOpenAI} ${noVorstellungWennNichtGefragtRuleOpenAI} ${meldeDichVerfuegbarkeitRuleOpenAI} ${customerKoseAnredeRuleOpenAI} ${alterKommentarNurWennThemaRuleOpenAI} ${noArbeitFrageBeiRenteRuleOpenAI} ${konkreteZeitNichtBestaerkenRuleOpenAI} ${einFadenHauptthemaRuleOpenAI} ${customerClarificationRuleOpenAI} ${customerNotMyRoleRuleOpenAI} ${dagegenSexHopeRuleOpenAI} ${answerDirectQuestionRuleOpenAI} ${erreichbarkeitRuleOpenAI} ${treffenFrageIgnoredRuleOpenAI} ${naturalFormulationRuleOpenAI} ${multiThemeRuleOpenAI} ${noFalseSingleRule} ${noTelefonsexPhotoRuleEffectiveOpenAI} ${noOtherPlatformRuleOpenAI} ${frageUndHypotheseRuleOpenAI} ${imageRequestDirectionRuleOpenAI} ${ruckfrageCallbackRule} ${answerOurQuestionStayOnTopicRuleOpenAI} ${ambiguousContextStayOnTopicRuleOpenAI} ${currentMessageOverridesRuleOpenAI} ${focusCurrentMessageRuleOpenAI} ${answerLastMessageQuestionNotPreviousRuleOpenAI} ${nameAndWohnortRuleOpenAI} ${personaFirstNamesDoNotEditRuleOpenAI} ${noRepeatNameIfAlreadySaidRuleOpenAI} ${noAddressCustomerWithPlatformUsernameRuleOpenAI} ${noMmmOpenerRuleOpenAI} ${massageNotMasseurinRuleOpenAI} ${logbookNoContradictRuleOpenAI} ${profileDumpRuleOpenAI} ${expectedCityWohnortRuleOpenAI} ${expectedJobRuleOpenAI} ${wohnortConcreteCityRuleOpenAI} ${noWiedersehenRule} ${noSharedPastRule} ${noTimeAgreementRule} ${noAbholenRuleOpenAI} ${kaufenBestellenBuchenRuleOpenAI} ${konkreterSexuellerPlanRuleOpenAI} ${deineVsMeineRuleOpenAI} ${deineVsMeineKontaktRuleOpenAI} ${mehrereFragenBesuchbarRuleOpenAI} ${kontextgebundenRuleOpenAI} ${treffenRichtungRuleOpenAI} ${verlaufBezugRuleOpenAI} ${noFreeRelaxAfterMeetingDeclineRuleOpenAI} ${replyMustAddressCurrentMessageRuleOpenAI} ${imageCurrentMessageRuleOpenAI} ${bilderAnfrageNoTreffenRuleOpenAI} ${imageProductLikeRuleOpenAI} ${noUnaskedAgeTypeComplimentRuleOpenAI} ${noHastDuLustRuleOpenAI} ${boundaryConsistencyRule} ${selfConsistencyRule} ${noEchoOwnModeratorStatementRule} ${noRepeatOwnLastMessageRuleOpenAI} ${censorshipRule} ${noOftTreffenRule} ${limitationFollowUpRule} ${engageOnSuggestionsRule} ${metaRule} ${noMetaPhrasesRule} ${schlussfrageImSzenarioRuleOpenAI} ${noReiztAmMeistenRuleOpenAI} ${besteFrageAusKontextRuleOpenAI} ${schlussfrageStilPersoenlichRuleOpenAI} ${noGenericOpenAIPhrasesRuleOpenAI} ${noFlexibelFloskelRuleOpenAI} ${noEchoRule} ${echoReplaceRule}${noVorliebenTabusEchoRuleOpenAI} ${noPerfekteMomentFrageRuleOpenAI} ${noParaphraseSchlussfrageRuleOpenAI}${noMetaQuestionAboutCustomerActivityRuleOpenAI} ${noRedundantFactQuestionRuleOpenAI} ${noEchoKundenZeitplanRuleOpenAI} ${noParaphraseLangRuleOpenAI} ${verlaufLogikWohnortRuleOpenAI} ${wohnortKonsistenzRuleOpenAI} ${noRedundantScenarioQuestionRuleOpenAI} ${noRoleReversalRuleOpenAI} ${wrongApologyAttributionRuleOpenAI} ${richtungAusPlanRuleOpenAI} ${sexualRoleReversalRuleOpenAI} ${customerAnatomyMaleRuleOpenAI} ${customerAnatomyFemaleRuleOpenAI} ${noVagueDazuSchlussfrageRuleOpenAI} ${pureGreetingAnswerRuleOpenAI} ${noTimeRefusalWithoutMeetingRequestRuleOpenAI} ${noMeetingSuggestionFromModeratorRuleOpenAI} ${noUhrzeitFrageAnKundenRuleOpenAI} ${noQuestionAlreadyAnsweredRule}${questionAndWholeRule}${minLengthRule}${sameActivityPlaceRule}${noParaphraseRule}${customerSingularRule}${singleProfileNoWirRule} ${noDoppelpunktAnfuehrungszeichenRuleOpenAI} Jede Nachricht muss eine Frage enthalten. Keine themenfremden neuen Gross-Themen.${humanTyposHint} ${imageOnlyAnnouncedRule} ${imageClaimedButNotPresentRule} ${imageAlreadySentToCustomerRuleOpenAI} ${bildAngebotKundeJaOhneSofortversandRuleOpenAI} ${alreadyIntroducedSelfRuleOpenAI} ${noHeyHalloEinstiegRuleOpenAI} ${notClearlySexualRuleOpenAI} ${noPlaceClaimRule} ${noStrongHopeRule} ${neutralMessageNoSexRule}

Außerdem: Umlaute korrigieren (ae→ä, oe→ö, ue→ü wo es Umlaut ist; nicht in Feuer, Museum, etc.). Immer ss statt ß. Keine Anführungszeichen, keine Bindestriche, keine Doppelpunkte – stattdessen Komma. Gib NUR die fertige korrigierte Nachricht zurück – kein anderer Text.`;

  const userContent = `${contextLine}${planBlock}${conversationBlock}${learningBlock}${exampleBlock}${currentMessageBlock}Zu korrigierende Moderatoren-Antwort:\n\n${sanitizeForApiContent(grokText.trim())}\n\n${CORRECTOR_MIN_LENGTH_RULE}${CORRECTOR_PREFER_COMPACT_RULE}\n\nKontext nutzen: Der Nachrichtenverlauf zeigt, worauf sich die Kundennachricht bezieht. Ist sie eine Klarstellung oder Abschwaechung (z.B. \"nur X\", \"nichts Schlimmes\")? Enthaelt die Antwort trotzdem Trost oder \"nicht dein Tag\"? → ersetzen durch passende, positive Reaktion auf die Klarstellung. Stellt der Kunde eine direkte Frage an die Persona (z.B. was für dich schön ist – oder nach dem Namen: wie heißt du, wie ist dein Name)? Beantwortet die Antwort diese Frage oder gibt sie sie nur zurück? Wenn nicht beantwortet → zuerst inhaltlich auf die Frage eingehen; Persona-Vornamen im Text nicht ersetzen oder neu erfinden, dann Gegenfrage.\n\nPrüfe und korrigiere: Geht die Antwort auf die GESAMTE Kundennachricht ein? Enthaelt die Kundennachricht mehrere Fragen oder Themen (z.B. mehrere Saetze/Fragen)? Geht die Antwort auf JEDE ein? Wenn eine Frage oder ein Thema fehlt → kurzen Bezug ergaenzen (1 Satz oder Teilsatz). Enthaelt sie eine Frage? PFLICHT: Ausgabe mindestens ${MIN_MESSAGE_LENGTH} Zeichen (nach trim); war die Vorlage schon so lang, Ausgabe nicht kuerzer. Umlaute und ss statt ß? Beginnt die Antwort mit einer Paraphrase/Aufzaehlung der Kundennachricht (z. B. "Ah, du machst X und Y und Z...")? Wenn ja → ersetzen: nur kurze Reaktion (z. B. Das klingt flexibel bei dir) + eigene Aussage + Frage. Echo/Paraphrase: Gibt die Antwort den Kundentext (oder sein Szenario/Wunsch) woertlich oder sinngemaess zurueck? Wenn ja → ersetzen durch eigenstaendige Reaktion. Meta: Enthaelt die Antwort "Ich finde es spannend was du dir wünschst", "Was wäre dein perfektes Szenario?" oder aehnliche Meta-Fragen? Wenn ja → durch konkrete thematische Frage ersetzen. Kunde will treffen: Enthaelt die Antwort "erstmal noch schreiben", "erst kennenlernen" oder "Was denkst du darüber?"? Wenn ja → umschreiben: auf Treffenwunsch eingehen; NICHT aktiv nach Ort oder Ideen fragen ("Wo koennten wir treffen?", "Was fuer Ideen?" – verboten); andeuten/vage Vorlagen, Initiative beim Kunden. Enthaelt die Antwort "einen Tag raussuchen", "wenn wir uns einen Tag raussuchen" oder "Was haeltst du davon wenn wir uns einen Tag raussuchen"? Wenn ja → ersetzen durch unverbindliche Formulierung (z.B. "muessen mal schauen wann es bei uns klappt", "schaue mal wann es passt"). Gib NUR den fertigen korrigierten Text zurück.`;
  ...
  try {
    ...
    let corrected = (out || '').trim();
    const minOut = grokText.trim().length >= MIN_MESSAGE_LENGTH ? MIN_MESSAGE_LENGTH : 20;
    if (corrected && corrected.length >= minOut && corrected.length <= grokText.length * 1.8) {
      corrected = applyChatStyleFixes(corrected);
 ...
 * @param {string} [opts.lastMessagesForContext] - Letzte 5–6 Nachrichten mit Kennzeichnung Kunde/KI/Moderator (für Interpreteur & Plan)
...
async function runGrokPipeline(opts) {
  ...
  // —— 1.5 Sprache: Kundennachricht nur Deutsch (wie Multi-Agent-Pipeline) ——
  if (customerMessage && String(customerMessage).trim()) {
    const { agentLanguageDetector } = require('./multi-agent');
    const langRes = await agentLanguageDetector(customerMessage);
    if (langRes.needsGermanResponse && langRes.response) {
      console.log(`🌍 Grok-Pipeline: Fremdsprache erkannt (${langRes.detectedLanguage}) – Hinweisantwort`);
      return emptyResult({
        needsGermanResponse: true,
        germanResponse: langRes.response,
        finalMessage: '',
        blocked: false
      });
    }
  ...
  const moderatorUsername = (profileInfo?.moderatorInfo?.username || '').toString().trim();
  const moderatorName = (profileInfo?.moderatorInfo?.name || extractedUserInfo?.assistant?.Name || '').toString().trim();
  const moderatorFirstNameField = (profileInfo?.moderatorInfo?.firstName || profileInfo?.moderatorInfo?.Vorname || '').toString().trim();
  const modNotesStr = typeof profileInfo?.moderatorNotes === 'string' ? profileInfo.moderatorNotes : '';
  const nameFromModNotes = firstNameTokenOnly(parseNotesLabelValueForPipeline(modNotesStr, 'Name'));
  const assistantFirst = firstNameTokenOnly((extractedUserInfo?.assistant?.Name || '').toString());
  const mnFirst = moderatorName ? firstNameTokenOnly(moderatorName) : '';

  let nameSource = firstNameTokenOnly(moderatorFirstNameField) || nameFromModNotes || assistantFirst;
  if (!nameSource && mnFirst) {
    const mu = moderatorUsername.toLowerCase();
    const mnLow = mnFirst.toLowerCase();
    if (mu && mnLow === mu) {
      nameSource = nameFromModNotes || assistantFirst;
    } else if (mu && mnLow.length >= 2 && mu.includes(mnLow) && mu.length > mnLow.length) {
      nameSource = mnFirst;
    } else if (!looksLikePlatformUsername(moderatorName)) {
      nameSource = mnFirst;
    } else {
      nameSource = nameFromModNotes || assistantFirst;
    }
  }
  if (!nameSource) nameSource = assistantFirst || mnFirst || '';
  if (process.env.NODE_ENV !== 'production' && nameSource && (moderatorUsername || moderatorName)) {
    console.log('ℹ️ Fake-Anrede-Name (nur Vorname):', nameSource, moderatorUsername ? `(Login: ${moderatorUsername})` : '');
  }
  const modRawForMultiParse = (() => {
    const fnf = (moderatorFirstNameField || '').toString().trim();
    const mnom = (moderatorName || '').toString().trim();
    const multiSep = /\s+(u\.|und|&)\s+|,\s*[A-Za-zÄÖÜäöüß]/;
    if (multiSep.test(fnf)) return fnf;
    if (multiSep.test(mnom)) return mnom;
    return fnf || mnom;
  })();
  const { fakeIsPlural, fakeNamesList } = parseMultiFakeNames(modRawForMultiParse);
  ...
  /** Von der KI gesetzt, wenn Interpreter lief; null = Interpreter nicht genutzt → Fallback isPureGreeting(). */
  let isPureGreetingFromInterpreter = null;
  const customerClaimsWeAgreedOrWantsAddressFlag = customerClaimsWeAgreedTodayOrWantsAddress(customerMessage || '');
  let unclearReferentAskBack = false;
  const isOnlyImagePlaceholder = /^der\s+kunde\s+hat\s+ein\s+bild\s+geschickt\.*\s*$/i.test((customerMessage || '').trim().replace(/\s+/g, ' '));
  const isSelfieOrIntimateImageType = ['face', 'body', 'penis', 'dildo', 'nude'].includes((imageType || '').toLowerCase().trim());
  if (customerMessage && customerMessage.trim() && !isASA) {
    ...
    } else if (isOnlyImagePlaceholder && (imageDescription || '').trim() && !isSelfieOrIntimateImageType) {
      // Reine Bildnachricht ohne Selfie/Porträt: Konkrete Bildbeschreibung nutzen statt pauschal "Bild von sich selbst" zu unterstellen.
      messageForPipeline = (imageDescription || '').trim();
      if (process.env.NODE_ENV !== 'production') console.log('✅ Kundennachricht interpretiert (Bildinhalt): Platzhalter → konkrete Bildbeschreibung (Typ: ' + (imageType || 'other') + ')');
    } else {
      try {
        const interpretContext = (lastMessagesForContext || '').trim()
          || (conversationHistory || '').trim().slice(-1200);
        const interpretResult = await interpretCustomerMessage(customerMessage.trim(), interpretContext, opts.lastPreviousCustomerMessageAgeMs);
        if (interpretResult.interpreted && interpretResult.interpreted.trim()) {
          messageForPipeline = interpretResult.interpreted.trim();
          isPureGreetingFromInterpreter = interpretResult.isPureGreeting;
        }
      } catch (e) {
  ...
  // Auch ausführen, wenn LLM-Situation "Wohnort-Frage" erkannt wurde (z. B. bei interpretierter Nachricht, wo isLocationQuestionFunc sonst nicht greift)
  const hasWohnortSituation = Array.isArray(detectedSituationsFromReply) && detectedSituationsFromReply.some(s => s === 'Wohnort-Frage');
  if (!isFirstMessage && !isASA) {
    const loc = await checkLocationQuestion({
      ...
      findNearbyCityFunc,
      moderatorMessages,
      forceTreatAsLocationQuestion: hasWohnortSituation
    });
    ...
    if (loc.isLocationQuestion && loc.cityInstructions) {
      try {
        const messages = buildCityPrompt({
          ...
          skipAskWhereCustomerLives: !!loc.skipAskWhereCustomerLives,
        ...
        const cityPlanSnippet = (loc.skipAskWhereCustomerLives
          ? `Wohnort-Frage: Antwort nennt den eigenen (Fake-)Wohnort. Kunde hat seinen Ort schon genannt oder Profil – KEIN Woher-kommst-du / Wo-wohnst-du. Andere Schlussfrage. Mindestens ${MIN_MESSAGE_LENGTH} Zeichen, wenn möglich knapp (ca. ${MIN_MESSAGE_LENGTH}–${MESSAGE_SOFT_LENGTH_TARGET_MAX}). Umlaute (ä, ö, ü), ss statt ß.`
          : `Wohnort-Frage: Antwort nennt den Wohnort (Stadt/Stadtteil) und stellt eine Frage zurück. Mindestens ${MIN_MESSAGE_LENGTH} Zeichen, wenn möglich knapp (ca. ${MIN_MESSAGE_LENGTH}–${MESSAGE_SOFT_LENGTH_TARGET_MAX}). Umlaute (ä, ö, ü), ss statt ß.`)
          + ' Nur den Ortsnamen – kein Bundesland/Kanton in Klammern (nicht „Stadt (Nordrhein-Westfalen)“).';
        let corrected = null;
        ...
        if (corrected != null && corrected.trim()) {
          ...
          const minLen = minLengthToAcceptCorrectorOutput(lenOrig);
        ...
        finalMessage = stripInternalPlaceholders(finalMessage);
        const safeCityWohnort = (loc.cityToUse && isValidCityValue(loc.cityToUse)) ? String(loc.cityToUse).trim() : null;
        const custCityLog = profileInfo?.customerInfo?.city || profileInfo?.customerInfo?.wohnort || profileInfo?.customerInfo?.Wohnort
          || extractedUserInfo?.user?.Wohnort || extractedUserInfo?.user?.city || null;
        return emptyResult({
          safety: safetyCheck,
          finalMessage,
          stage2Examples: [],
          interpretedCustomerMessage: messageForPipeline !== (customerMessage || '').trim() ? messageForPipeline : undefined,
          locationContext: safeCityWohnort ? { fakeCity: safeCityWohnort, customerCity: custCityLog ? String(custCityLog).trim() : null } : null
        });
      } catch (err) {
  ...
  if (isASA) {
    ...
    // 2–3 möglichst unterschiedliche Beispiele (diverse Auswahl statt Zufall), damit die KI variierte Vorlagen sieht
    const numExamples = Math.min(3, Math.max(2, asaExamples.length));
    const selectedASAs = selectDiverseASAExamples(asaExamples, numExamples);
    try {
  ...
  let detectedSituations = Array.isArray(detectedSituationsFromReply) && detectedSituationsFromReply.length > 0
    ...
    : [];
  // Intent-Disambiguierung (feiner als Situationen): kann Treffen/Termine bei mehrdeutigem Timing sinnvoll an-/abschalten
  let intentSignal = null;
  try {
    const intentContext = (lastMessagesForContext || '').trim() || (conversationHistory || '').trim().slice(-1200);
    intentSignal = await detectCustomerIntentWithLLM(messageForPipeline, intentContext);
    if (intentSignal && process.env.NODE_ENV !== 'production') {
      console.log('🧭 Intent:', intentSignal.intent, 'conf=' + Math.round((intentSignal.confidence || 0) * 100) + '%', intentSignal.evidence ? ('reason=' + intentSignal.evidence.slice(0, 80)) : '');
    }
  } catch (e) {
    // ignore
  }
  ...
  // Intent-gesteuerte Feinjustierung (ohne Hardrule, aber mit Confidence):
  // - chat_continue mit hoher Sicherheit → Treffen/Termine entfernen (sonst wirkt "kann heute nicht" komisch)
  // - real_meetup mit hoher Sicherheit → Treffen/Termine hinzufügen (sicherer Modus: ausweichen, nicht zusagen)
  if (intentSignal && intentSignal.intent === 'chat_continue' && (intentSignal.confidence || 0) >= 0.78) {
    detectedSituations = detectedSituations.filter(s => s !== 'Treffen/Termine');
  } else if (intentSignal && intentSignal.intent === 'real_meetup' && (intentSignal.confidence || 0) >= 0.6) {
    if (!detectedSituations.includes('Treffen/Termine')) detectedSituations = [...detectedSituations, 'Treffen/Termine'];
  }
  const hasTelefonVideoInMessage = /\b(telefon|telefonieren|videoanruf|kamera\s+beim\s+telefon|mal\s+anrufen|dich\s+anrufen|wann\s+passt\s+es\s+(zum\s+)?telefon)\b/i.test(messageForPipeline || '');
  if (hasTelefonVideoInMessage && !detectedSituations.includes('Telefonat/Videoanruf')) {
    detectedSituations = [...detectedSituations, 'Telefonat/Videoanruf'];
    if (process.env.NODE_ENV !== 'production') console.log('🔍 Situationen: Telefonat/Videoanruf ergänzt');
  }
  ...
  // Ausnahme: Kunde hat in diesem Turn ein Bild geschickt (imageDescription) – dann greift diese Text-Absicherung nicht, sonst wertet „Der Kunde hat ein Bild geschickt.“ fälschlich als nicht-sexuell.
  ...
  const customerTurnHasImage = !!(imageDescription && (typeof imageDescription === 'string' ? imageDescription.trim() : imageDescription));
  if (customerTurnHasImage) {
    currentMessageNotClearlySexual = false;
    if (process.env.NODE_ENV !== 'production') {
      console.log('ℹ️ Kunde hat Bild geschickt – „nicht eindeutig sexuell“ wird nicht angewendet (nur für reinen Kundentext).');
    }
  }
  ...
  if (situationsForRulesBlock.includes('Telefonat/Videoanruf') && EXTRA_RULES.telefonatVideo) {
    situationRulesBlock += '\n\n' + EXTRA_RULES.telefonatVideo;
  }
  ...
  let plan = '';
  try {
    const currentMessageHasImage = customerTurnHasImage;
    const planContext = (lastMessagesForContext || '').trim() || (conversationHistory || '').trim();
        const customerOccupationForPlan = (profileInfo?.customerInfo?.occupation || '').toString().trim();
    plan = await runPlanningStep(messageForPipeline, detectedSituations, allRules, planContext, hasIntroducedSelfInChatFlag, currentMessageNotClearlySexual, currentMessageHasImage, customerClaimsWeAgreedOrWantsAddressFlag, opts.lastPreviousCustomerMessageAgeMs, customerOccupationForPlan, isPureGreetingFromInterpreter, fakeGender, customerGender);
  ...
  if (hasWohnortInLogbook && (!fakeCity || !fakeCity.trim())) {
    const logbookText = buildFakeLogbookHint(profileInfo);
    const cityFromLogbook = extractWohnortFromLogbookHint(logbookText);
    if (cityFromLogbook && isValidCityValue(cityFromLogbook)) fakeCity = cityFromLogbook;
  }
  ...
  if (!isWohnortFrage) {
    ...
    console.log('✅ Fake-Wohnort aus Profil (keine Suche):', (fakeCity || '').trim());
    ...
    const modTextsLoc = (opts.moderatorMessages || []).map(m => (m && m.text) ? String(m.text) : '').filter(Boolean);
    if (inputForResolve && customerCity && modTextsLoc.length > 0 && moderatorTextsImplySameCityAsCustomer(modTextsLoc, customerCity)) {
      resolvedFakeCity = customerCity.trim();
      console.log('✅ Wohnort-Kontext (Plan): Verlauf nennt gleichen Ort wie Kunde – kein Umkreis:', resolvedFakeCity);
    } else if (inputForResolve && (!resolvedFakeCity || isBundesland(fakeCity))) {
      try {
        const nearbyCity = await findNearbyCityFunc(inputForResolve);
        if (nearbyCity && nearbyCity.trim()) {
          resolvedFakeCity = nearbyCity.trim();
          console.log('🔍 Wohnort-Suche (Umkreis):', (inputForResolve || '').trim(), '→', resolvedFakeCity);
        }
      } catch (e) {
  ...
  if (fakeHasWohnortAlready && fakeCity && !isWohnortFrage) {
    console.log('✅ Fake-Wohnort aus Profil (keine Suche, keine Wohnort-Frage):', (fakeCity || '').trim());
  }
  ...
  const imageContextHint = (imageDescription && imageDescription.trim())
    ...
    : null;
  const { weekday, timePhase, hour, clockShort } = getBerlinTimeContext();
  const timeFormulationRule =
    hour >= 12
      ? ` Formulierungen zur JETZT-Zeit: Es ist ${timePhase} (${clockShort} Uhr, Europe/Berlin). NICHT schreiben, dass "der Morgen" oder "mein Morgen" "jetzt" oder "gleich" schoener/besser waere – der Morgen ist zeitlich nicht mehr der aktuelle Abschnitt. Stattdessen z.B. "mein Tag", "der Tag", "mein Nachmittag/Abend" passend zur Phase, oder neutral "freut mich fuer heute".`
      : hour < 6
        ? ` Es ist Nacht/frueh (${clockShort} Uhr). Keine Formulierungen als waere es gerade Vormittag/Mittag (z.B. "schoener Morgen", "Start in den Tag") – eher Nacht/Frueh oder neutral.`
        : '';
  const timeContextHint = `Heute ${weekday}, ${timePhase} (${clockShort} Uhr, Europe/Berlin). Nur Aktivitaeten nennen, die dazu passen (z.B. Sonntag kein Einkaufen, nachts keine Arbeit).${timeFormulationRule}`;
  ...
  // Fake-Geburtstag / Sternzeichen (aus Profil-Geburtsdatum ableiten; nur nennen wenn Kunde explizit fragt)
  const fakeBirthDmy = parseBirthDateDMY(profileInfo?.moderatorInfo?.birthDate);
  const fakeBirthdayDateStr = fakeBirthDmy ? formatBirthDateDMY(fakeBirthDmy) : '';
  const fakeZodiac = fakeBirthDmy ? zodiacFromDayMonth(fakeBirthDmy.day, fakeBirthDmy.month) : '';
  ...
  // KI (Interpreter) entscheidet ueber "nur Begruessung"; nur wenn Interpreter nicht lief, Fallback auf Heuristik.
  ...
  const isPureGreetingMsg = isPureGreetingFromInterpreter === true || (isPureGreetingFromInterpreter === false ? false : isPureGreeting(messageForPipeline));
  ...
  const besitzFakten = extractBesitzFaktenFromVerlauf(conversationHistory || '');
  const fakeOrtFromVerlauf = extractOrtFromVerlauf(conversationHistory || '');
  const moderatorPromisedMoreImages = lastModeratorPromisedCustomerMoreImages(conversationHistory || '');
  const bildAngebotKundeJaVorfreude = lastModeratorOfferedOrAskedSendPictureToCustomer(conversationHistory || '') && isShortCustomerAgreementForImage(messageForPipeline);
  try {
    const messages = buildNormalPrompt({
      ...
      customerClaimsWeAgreedOrWantsAddress: customerClaimsWeAgreedOrWantsAddressFlag,
      emotionalToneHint,
      imageContextHint,
      ...
      moderatorSentImageDescription: opts.moderatorSentImageDescription && String(opts.moderatorSentImageDescription).trim() ? String(opts.moderatorSentImageDescription).trim() : null,
      fakeProfession,
      isReallyUnclearMessage: isReallyUnclear,
      fakeLogbookHint,
      fakeName: (nameSource || mnFirst || assistantFirst || '').toString().trim() || '',
      ...
      besitzFaktenHint: besitzFakten,
      fakeOrtFromVerlauf: fakeOrtFromVerlauf || undefined,
      customerAsksForFakeAge: /\b(wie\s+alt\s+bist\s+du|wie\s+alt\s+bist\s+du\s+eigentlich|dein\s+alter|wie\s+alt\s+du\s+bist|wie\s+alt\s+seid\s+ihr)\b/i.test((messageForPipeline || '').trim()),
      customerAsksForFakeBirthday: /\b(geburtstag|geburtsdatum|wann\s+hast\s+du\s+geburtstag|wann\s+hast\s+du\s+geburts(tag|datum)|birthday|birth\s*date|dob)\b/i.test((messageForPipeline || '').trim()),
      customerAsksForFakeZodiac: /\b(sternzeichen|zodiac|horoskop|welches\s+sternzeichen)\b/i.test((messageForPipeline || '').trim()),
      fakeBirthdayDate: fakeBirthdayDateStr,
      fakeZodiac,
      customerOccupationInRente: isCustomerOccupationRente(profileInfo?.customerInfo?.occupation),
      customerHasNoProfilePic: profileInfo?.customerInfo?.hasProfilePic === false,
      fakeCityFromProfile: (fakeCity && String(fakeCity).trim() && !isBundesland(fakeCity)) ? String(fakeCity).trim() : null,
      moderatorPromisedMoreImages: !!moderatorPromisedMoreImages,
      profileInfo: profileInfo || {}
    });
    let finalMessage = '';
    ...
    const fakeFirstName = (nameSource || '').toString().trim() || '';
    ...
    const asksForNameOrWohnort = isWohnortFrage || customerAsksSameIntroductionBack(messageForPipeline || '') || /\b(vornamen?|name)\s+und\s+wohnort\b|\bwohnort\s+und\s+(vornamen?|name)\b|\b(deinen?|dein)\s+(vornamen?|wohnort)\b|\bdenkst\s+du\s+noch\s+an\s+(deinen?\s+)?(vornamen?|wohnort)\b|\b(richtigen?\s+)?vornamen?\b.*\b(schatzi|erfahren|sagen|nennen|verraten|denkst)\b|\b(wo\s+wohnst|woher\s+kommst)\b|(\b(ich\s+)?hei[ßs]e\s+[\wäöüß-]+|\bich\s+bin\s+[\wäöüß-]+)\s+und\s+du\??\b/i.test((messageForPipeline || '').trim());
    ...
    const correctorContext = {
      ...
      telefonVideoRequest: detectedSituations && detectedSituations.some(s => (s || '').includes('Telefonat/Videoanruf')),
      ...
      customerIsFemale: customerGender === 'weiblich' || customerGender === 'w' || customerGender === 'female',
      customerIsMale: customerGender === 'männlich' || customerGender === 'male',
      ...
      customerOccupationInRente: isCustomerOccupationRente(profileInfo?.customerInfo?.occupation),
      berlinHour: hour,
      berlinClockShort: clockShort,
      berlinTimePhase: timePhase,
      moderatorPromisedMoreImages: !!(moderatorPromisedMoreImages && primarySituation === 'Bilder Anfrage'),
      bildAngebotKundeJaVorfreude: !!bildAngebotKundeJaVorfreude
    };
    const correctorPlanSnippet = (plan || '').trim();
    const correctorConversationSnippet = buildStrictCorrectorConversationSnippet(
      (conversationHistory || '').trim(),
      (lastMessagesForContext || '').trim()
    );
    ...
    for (let questionAttempt = 1; questionAttempt <= 2; questionAttempt++) {
      ...
      if (fakeFirstName) {
        finalMessage = correctWrongFakeIntroName(finalMessage, fakeFirstName, fakeNamesList);
      }
    ...
    if (finalMessage.length > MAX_FINAL) {
      ...
      // Satzgrenze nur nutzen, wenn genug Inhalt bleibt (Plattform-Minimum)
      const minLengthAtSentence = MIN_MESSAGE_LENGTH - 1;
    ...
    if (openAICorrected && openAICorrected.trim()) {
      ...
      if (lenNew >= minLengthToAcceptCorrectorOutput(finalMessage.length)) {
    ...
    finalMessage = await ensureMinimumLength(finalMessage, messageForPipeline);
    // Plattform-Login des Fakes (z. B. Wildrenate) → nur Vorname (z. B. Renate)
    if (moderatorUsername && nameSource && moderatorUsername.toLowerCase() !== nameSource.toLowerCase() && finalMessage && finalMessage.length > 0) {
      const modUserRe = new RegExp(escapeRegex(moderatorUsername), 'gi');
      if (modUserRe.test(finalMessage)) {
        finalMessage = finalMessage.replace(modUserRe, nameSource);
        if (process.env.NODE_ENV !== 'production') console.log('✅ Fake-Login in Nachricht durch Vornamen ersetzt:', moderatorUsername, '→', nameSource);
      }
    }
    // Profilfeld name falls es vom Login abweicht (z. B. noch alter Nick in der Antwort)
    if (moderatorName && nameSource && moderatorName !== nameSource && moderatorName.toLowerCase() !== moderatorUsername.toLowerCase() && finalMessage && finalMessage.length > 0) {
      const usernameRe = new RegExp(escapeRegex(moderatorName), 'gi');
      if (usernameRe.test(finalMessage)) {
        finalMessage = finalMessage.replace(usernameRe, nameSource);
        if (process.env.NODE_ENV !== 'production') console.log('✅ Fake-Profilname in Nachricht durch Anrede-Namen ersetzt:', moderatorName, '→', nameSource);
      }
    }
    const custLogin = (profileInfo?.customerInfo?.username || '').toString().trim();
    const custAddr = (opts.customerFirstNameForAddress || '').toString().trim();
    if (custLogin && custAddr && custLogin.toLowerCase() !== custAddr.toLowerCase() && finalMessage && finalMessage.length > 0) {
      const custRe = new RegExp(escapeRegex(custLogin), 'gi');
      if (custRe.test(finalMessage)) {
        finalMessage = finalMessage.replace(custRe, custAddr);
        if (process.env.NODE_ENV !== 'production') console.log('✅ Kunden-Login in Nachricht durch Vornamen ersetzt:', custLogin, '→', custAddr);
      }
    }
    // AVZ: Wenn im Verlauf die Persona sich mit einem anderen Namen vorgestellt hat (z. B. "Ich bin die Ines"), diesen statt Username verwenden
    const nameFromVerlauf = extractNameFromLastModeratorMessage(conversationHistory || '');
    if (nameFromVerlauf && finalMessage && finalMessage.length > 0) {
      const ichBinMatch = finalMessage.match(/\bich\s+bin\s+(?:die|der\s+)?([\wäöüß\-]+)/i);
      if (ichBinMatch && ichBinMatch[1]) {
        const genannt = ichBinMatch[1].trim();
        const siehtAusWieUsername = looksLikePlatformUsername(genannt) || (moderatorName && genannt.toLowerCase() === moderatorName.toLowerCase());
        if (genannt.toLowerCase() !== nameFromVerlauf.toLowerCase() && (siehtAusWieUsername || genannt === moderatorName)) {
          finalMessage = finalMessage.replace(new RegExp('(ich\\s+bin\\s+(?:die|der\\s+)?)' + escapeRegex(genannt), 'gi'), '$1' + nameFromVerlauf);
          if (process.env.NODE_ENV !== 'production') console.log('✅ Name in Nachricht aus Verlauf ersetzt:', genannt, '→', nameFromVerlauf);
        }
      }
    }
    finalMessage = stripInternalPlaceholders(finalMessage);
    // Schnell: Klammer-/Format-Leaks (CoT, Platzhalter) – gleiche Extension-Logik wie Meta-Outing: einfügen, kein Auto-Send
    if (hasSuspiciousFormatPatterns(finalMessage)) {
      console.error('🛡️ Grok-Pipeline: Auffällige Formatierung (Klammern/Platzhalter) – Antwort wird ausgeliefert, Extension sendet nicht automatisch');
      return emptyResult({
        safety: safetyCheck,
        blocked: false,
        finalMessage,
        formatAnomalyDoNotSend: true,
    ...
    // Semantische Prüfung: Hat die KI einem Treffen zugestimmt oder Termin vereinbart? (gpt-4o-mini, kontextbasiert)
    const conversationSnippetForMeeting = (conversationHistory || '').trim().slice(-600);
    const meetingCheck = await checkMeetingAgreement(messageForPipeline, conversationSnippetForMeeting, finalMessage);
    if (meetingCheck.agreedOrArrangedMeeting) {
      const errMsg = 'Die KI hat versucht, einem Treffen zuzustimmen oder einen Termin auszumachen. Das ist nicht erlaubt.';
      console.error('🛡️ Grok-Pipeline: Treffen-Zustimmung blockiert –', meetingCheck.reason || errMsg);
      return emptyResult({
        safety: { isBlocked: true, reason: 'meeting_agreement', errorMessage: errMsg },
        blocked: true,
        finalMessage: '',
        error: errMsg
      });
    }
    let metaOutingDoNotSend = hasObviousMetaOutingPatterns(finalMessage);
    let metaOutingReason = metaOutingDoNotSend ? 'keyword_pattern' : '';
    if (!metaOutingDoNotSend) {
      const metaCheck = await checkMetaOutingInReply(finalMessage);
      metaOutingDoNotSend = !!metaCheck.exposesMeta;
      metaOutingReason = metaCheck.reason || '';
    }
    if (metaOutingDoNotSend) {
      console.error('🛡️ Grok-Pipeline: Meta-Outing (KI/Bot/Moderator) – Antwort wird ausgeliefert, Extension sendet nicht automatisch:', metaOutingReason || '');
      return emptyResult({
        safety: safetyCheck,
        blocked: false,
        finalMessage,
...
module.exports = {
  ...
  selectOneASADiverseFromLast,
  hasObviousMetaOutingPatterns,
  hasSuspiciousFormatPatterns
};
