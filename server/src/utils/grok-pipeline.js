...
async function ensureQuestionInMessage(message) {
  ...
  try {
    const userContent = `Die folgende Chat-Nachricht enthaelt keine Frage. Fuege genau eine kurze, thematisch passende Frage ein (z. B. am Ende). Gib NUR die komplette Nachricht inkl. Frage zurueck, keine Erklaerungen.\n\nNachricht:\n\n${sanitizeForApiContent(trimmed)}`;
    const result = await callOpenAI([
      { role: 'system', content: 'Du fuegst einer Chat-Nachricht genau eine kurze, thematisch passende Frage ein (z. B. am Ende). Gib NUR die komplette Nachricht mit eingebauter Frage zurueck. Keine Anführungszeichen, kein anderer Text.' },
...
function buildNormalPrompt({
  ...
  if (isMeetingSituation) {
    if (isDirectMeetingRequest) {
      ...
      systemContent += 'TREFFEN/VORFREUDE: Der Kunde aeussert nur Vorfreude oder Hoffnung auf ein Treffen (z.B. "hoffe wir sehen uns bald"), keine konkrete Treffen-Anfrage. Antworte NICHT mit "erst kennenlernen" oder "erst quatschen" – nutze die Vielfalt aus den Trainingsdaten (z.B. positiv darauf eingehen, Gegenfrage stellen). Keine Zusage zu Ort/Zeit. Beende mit einer klaren Gegenfrage.\n\n';
    }
  }
  if (isContactRequest) {
...
async function runGrokCorrector({ customerMessage = '', context = {}, grokText = '', learningContext = '', exampleSnippet = '', planSnippet = '', conversationSnippet = '' }) {
  ...
  const meetingRule = (context.isMeetingSituation && context.isDirectMeetingRequest !== true)
    ? ' TREFFEN/VORFREUDE: Kunde hat nur Vorfreude geaeussert. Enthaelt die Antwort "kennenlernen" oder "erst quatschen" als Ablehnung? Wenn ja → umformulieren: positiv auf die Vorfreude eingehen oder Gegenfrage, wie in Trainingsdaten. Keine Zusage zu Ort/Zeit.'
    : (context.isMeetingSituation ? ' TREFFEN: Keine Zusage zu Tag/Ort/Uhrzeit. Keine Saetze wie "Sonntag klingt gut", "komm vorbei".' : '');
  ...
  const contactIrritatedRule = (context.contactRequest === true || context.customerIrritated === true)
    ...
    : '';
  const customerSingularRuleGrok = (context.fakeIsPlural === true)
...
async function runMistralCorrector({ customerMessage = '', context = {}, grokText = '', learningContext = '', exampleSnippet = '', planSnippet = '', conversationSnippet = '' }) {
  ...
  const meetingRule = (context.isMeetingSituation === true)
    ? (context.isDirectMeetingRequest === true
      ? ' PFLICHT TREFFEN: Wenn Kontext "Thema Treffen/Bar/Zeit" enthaelt: Die Antwort darf NIEMALS einem konkreten Tag (z.B. Sonntag, Montag), einer Uhrzeit oder einem Ort zustimmen oder diese nennen. Saetze wie "Sonntag klingt gut", "dann passt mir", "komm vorbei" muessen entfernt oder umformuliert werden – keine Zusage.'
  ...
  if (MISTRAL_USE_MINIMAL_PROMPT) {
    ...
    systemContent = `Du bist ein Korrektor für Chat-Moderator-Antworten. Entscheide immer anhand des gesamten Kontexts und der Kundennachricht. ${sexualRule} ${contactIrritatedRule}${meetingRule} ${metaRule} ${noEchoRule}${questionAndWholeRule}${sameActivityPlaceRule}${noParaphraseRule}${customerSingularRule} Stil und Wortschatz der ursprünglichen Antwort möglichst beibehalten, nur klare Fehler korrigieren. Jede Nachricht muss eine Frage enthalten; maximal ein bis zwei Fragen, keine Frage-Kaskade.${humanTyposHint} PFLICHT: Jede Nachricht muss eine Frage enthalten. Fehlt eine Frage, fuege UNBEDINGT eine kurze, thematisch passende Frage ein (z. B. am Ende). Die Antwort MUSS auf die Kundennachricht eingehen. Wenn etwas zu korrigieren ist (fehlende Frage, kein Bezug, Kontaktdaten nicht abgelehnt, Meta/Wiederholung, Umlaute/ss, Stil), aendere es. Schreibe mit ä, ö, ü. Immer ss, nie ß. Keine Anführungszeichen. Keine Bindestriche. Antworte NUR mit der fertigen korrigierten Nachricht – kein anderer Text.`;
  }
...
async function runOpenAICorrector({ customerMessage = '', context = {}, grokText = '', learningContext = '', exampleSnippet = '', planSnippet = '', conversationSnippet = '' }) {
  ...
  const metaRule = 'KEINE Meta-Kommentare, keine internen Notizen, keine Erklaerungen – ausschliesslich die eine Chat-Nachricht ausgeben.';
  const noEchoRule = 'Wiederhole die Kundennachricht NICHT woertlich oder fast woertlich. Formuliere eigenstaendig; gehe inhaltlich darauf ein, ohne seine Formulierungen zu echoen (z.B. nicht "dass du mich so X findest" wenn der Kunde "du bist so X" schrieb).';
  const systemContent = `Du bist ein Korrektor für Chat-Moderator-Antworten. Entscheide immer anhand des gesamten Kontexts und der Kundennachricht. ${sexualRule} ${contactIrritatedRule}${meetingRule} ${metaRule} ${noEchoRule}${customerSingularRule} PFLICHT: Jede Nachricht muss eine Frage enthalten. Fehlt eine Frage, fuege UNBEDINGT eine kurze, thematisch passende Frage ein (z. B. am Ende). Die Antwort MUSS auf die Kundennachricht eingehen. Wenn etwas zu korrigieren ist (fehlende Frage, kein Bezug, Kontaktdaten nicht abgelehnt, Meta/Wiederholung, Umlaute/ss, Stil), aendere es. Schreibe mit ä, ö, ü. Immer ss, nie ß. Keine Anführungszeichen. Keine Bindestriche. Antworte NUR mit der fertigen korrigierten Nachricht – kein anderer Text.`;
...
async function runGrokPipeline(opts) {
  ...
  const fakeIsPlural = moderatorName.includes(' und ');
  const doubleProfileHint = fakeIsPlural
    ? 'DOPPELPROFIL: Du schreibst fuer 2 Personen (z.B. zwei Namen im Profil). Nutze "wir" statt "ich", "uns" statt "mich", "unsere" statt "meine" – als waeren zwei Personen am Schreiben.\nANREDE KUNDE: Der Kunde ist immer EINE Person. Sprich ihn/sie mit du, dich, dir an – niemals ihr, euch. (Du = wir/uns, Kunde = du/dich/dir.)\n\n'
    : '';
  ...
  // Alle erkannten Situationen für Beispielsuche (Beispiele mit mehr passenden Situationen werden bevorzugt)
  const situationsForExamples = detectedSituations.length > 0 ? detectedSituations : null;
  let examples = [];
  ...
  try {
    ...
    // Post-Processing: Sicherstellen, dass die Nachricht eine Frage enthaelt
