...
function removeCustomerEcho(customerMessage, replyText) {
  ...
  if (customerWords.length < 5) return reply;
  let result = reply;
  const MIN_PHRASE_WORDS = 5;
  const MIN_PHRASE_LEN = 30;
...
/** Ersetzt Umlaut-Digraphen ae/ue/oe durch ä/ü/ö (zwei Zeichen -> ein Umlaut). Entfernt überzähliges "e" falls wo üe/äe/öe steht (z.B. füer -> für). */
function fixUmlautDigraphs(text) {
  ...
  return text
    ...
    .replace(/üe/g, 'ü').replace(/äe/g, 'ä').replace(/öe/g, 'ö')
    .replace(/Üe/g, 'Ü').replace(/Äe/g, 'Ä').replace(/Öe/g, 'Ö');
}
...
function buildNormalPrompt({
  ...
  systemContent += EXTRA_RULES.orte + EXTRA_RULES.vorfreude + EXTRA_RULES.telefonsexFotos + EXTRA_RULES.ruckfrageCallback + EXTRA_RULES.flirtyKontinuitaet + EXTRA_RULES.keinEcho + EXTRA_RULES.beziehungszieleVsTreffen;
...
PFLICHT: Nur eine Nachricht ausgeben; Frage am Ende; keine Meta-Kommentare; KEIN ECHO – nicht Name, Alter, Ort oder Vorlieben des Kunden zurueckspielen, eigenstaendig formulieren.`;
...
function postProcessMessage(msg) {
  ...
  // Allgemeine Umlaut-Digraph-Ersetzung: ae->ä, ue->ü, oe->ö (ersetzt immer die ZWEI Zeichen durch EINEN Umlaut)
  m = m.replace(/AE/g, 'Ä').replace(/Ae/g, 'Ä').replace(/ae/g, 'ä');
  m = m.replace(/OE/g, 'Ö').replace(/Oe/g, 'Ö').replace(/oe/g, 'ö');
  m = m.replace(/UE/g, 'Ü').replace(/Ue/g, 'Ü').replace(/ue/g, 'ü');
  // Bereinigung: Falls wo nur u/a/o durch Umlaut ersetzt wurde und "e" stehen blieb (z.B. füer, schäen) -> überzähliges e entfernen
  m = m.replace(/üe/g, 'ü').replace(/äe/g, 'ä').replace(/öe/g, 'ö');
  m = m.replace(/Üe/g, 'Ü').replace(/Äe/g, 'Ä').replace(/Öe/g, 'Ö');
  return m;
}
