...
async function runGrokPipeline(opts) {
  ...
  let nameSource = moderatorFirstName || moderatorName;
  if (!moderatorFirstName && moderatorName && looksLikePlatformUsername(moderatorName)) {
    const fromLogbook = (extractedUserInfo?.assistant?.Name || '').toString().trim();
    if (fromLogbook && !looksLikePlatformUsername(fromLogbook)) {
      nameSource = fromLogbook;
      if (process.env.NODE_ENV !== 'production') console.log('ℹ️ Fake-Name: Username ignoriert, Vorname aus Logbuch verwendet:', fromLogbook);
    }
  }
  ...
  try {
    const messages = buildNormalPrompt({
      ...
      fakeLogbookHint,
      fakeName: (nameSource || moderatorFirstName || moderatorName || extractedUserInfo?.assistant?.Name || '').trim() || '',
    ...
    // Vorname für Chat: nameSource (bereits Username bereinigt), dann firstName/Vorname, nie Benutzername
    const fakeFirstName = (nameSource || profileInfo?.moderatorInfo?.firstName || profileInfo?.moderatorInfo?.Vorname || extractedUserInfo?.assistant?.Name || moderatorName || profileInfo?.moderatorInfo?.name || '').toString().trim().split(/\s+/)[0] || '';
    ...
    // Username in Nachricht durch Anzeigenamen ersetzen (AVZ/FPC: nie "Ich bin Baldzuzweit", sondern "Ich bin Nora")
    if (moderatorName && nameSource && moderatorName !== nameSource && finalMessage && finalMessage.length > 0) {
      ...
      if (usernameRe.test(finalMessage)) {
        ...
        if (process.env.NODE_ENV !== 'production') console.log('✅ Username in Nachricht durch Anzeigenamen ersetzt:', moderatorName, '→', nameSource);
      }
    }
