...
const EXTRA_RULES = {
  ...
  guteSchlussfrage: 'GUTE SCHLUSSFRAGE: Die Frage am Ende darf NICHT nur das wiederfragen, was der Kunde gerade gesagt hat (z.B. nicht "Was machst du mit den Huehnern und den Kleinigkeiten?" wenn er Huehner und Kleinigkeiten nannte – wirkt abgehakt). Stattdessen: thematisch passend, aber EIGENSTAENDIG. Moeglichkeiten: nach Stimmung/Gefuehl fragen ("Wie gehts dir so mit dem langen Tag?", "Freust du dich schon auf Abend?"), nach Vorfreude oder naechstem Schritt ("Wann seid ihr wieder unterwegs?", "Dann kannst du bald abschalten?"), oder das Thema leicht erweitern – nie dieselben Begriffe in Frageform zurueckgeben.\n',
...
PFLICHT: Nur eine Nachricht ausgeben; Frage am Ende (im Stil der Beispiele); keine Meta-Kommentare; KEIN ECHO – nicht Name, Alter, Ort oder Vorlieben des Kunden zurueckspielen, eigenstaendig formulieren.`;
  ...
  if (conversationHistory && conversationHistory.trim()) {
    ...
    userContent += `LAUFENDES GESPRÄCH: Die folgende Kundennachricht ist eine ANTWORT auf den Chat-Verlauf. Verstehe sie NUR im Bezug dazu – nie isoliert.\n\nChat-Verlauf:\n${sanitizeForApiContent(historySnippet)}\n\nNeue Kundennachricht (bezieht sich auf obigen Verlauf – worauf genau? Erst einordnen, dann passend antworten):\n"${sanitizeForApiContent(customerMessage || '')}"\n\n`;
  } else {
  ...
  userContent += `FRAGE BEANTWORTEN: Stellt der Kunde eine direkte Frage an DICH (z.B. was macht X für dich schön, was suchst du, wie oft – oder nach deinem Namen: wie heißt du, wie ist dein Name)? → ZUERST diese Frage beantworten (aus deiner Sicht; bei Namensfrage: deinen Namen nennen), nicht die Frage nur zurückgeben oder umlenken. Danach Gegenfrage.\n\n`;
...
async function runMistralCorrector({ customerMessage = '', context = {}, grokText = '', learningContext = '', exampleSnippet = '', planSnippet = '', conversationSnippet = '' }) {
  ...
  const hasDirectQuestionToPersonaMistral = hasNameQuestionMistral || /\b(für dich|für dich\?|was macht .* für dich|was suchst du|wie oft|was wäre für dich|besonders schön für dich)\b/i.test(fullCustomerMsg) || (/\?/.test(fullCustomerMsg) && /\b(dich|dir|du)\b/i.test(fullCustomerMsg));
  const answerDirectQuestionRuleMistral = hasDirectQuestionToPersonaMistral
    ? ' PFLICHT: Kunde fragt die Persona direkt (z.B. was für dich schön ist – oder nach dem Namen: wie heißt du, wie ist dein Name). Beantwortet die Antwort die Frage oder gibt sie sie nur zurück? Wenn nicht → zuerst Frage beantworten (bei Namensfrage: Persona-Namen nennen), dann Gegenfrage.'
    : '';
  ...
  if (MISTRAL_USE_MINIMAL_PROMPT) {
    ...
    const sexualPerspectiveAndQuestionsRuleMistral = (context.allowSexualContent === true && context.customerTalkingAboutSexWithFake === true)
      ? ' SEX KONTEXT: Echo der Kundenszenario-Formulierung (z.B. "wenn du mir den Ruecken zudrehst") kann missverstaendlich sein → aus Persona-Sicht formulieren (z.B. "wenn ich dir den Ruecken zudrehe"). Schlussfrage bei Sex: Keine generischen/schwachen Fragen ("Was denkst du wie intensiv das sein koennte?", "Pizza oder Brüste", "Wie heftig pulsiert...") → ersetzen durch Frage die das Szenario erweitert (Stellungen, Spielzeug, was Neues ausprobieren, Tempo, Haerter/Langsamer).'
      : '';
    ...
    const noMetaPhrasesRuleMistral = ' Formulierungen wie "Ich finde es spannend was du dir wünschst", "Was wäre dein perfektes Szenario?", "Was waere fuer dich der perfekte Moment fuer Doggy/Reiterstellung/..." sind verboten – durch konkrete thematische Fragen ersetzen.';
...
async function runOpenAIFullCorrector({ customerMessage = '', context = {}, grokText = '', learningContext = '', exampleSnippet = '', planSnippet = '', conversationSnippet = '' }) {
  ...
  const hasDirectQuestionToPersonaOpenAI = hasNameQuestionOpenAI || /\b(für dich|für dich\?|was macht .* für dich|was suchst du|wie oft|was wäre für dich|besonders schön für dich)\b/i.test(fullCustomerMsg) || (/\?/.test(fullCustomerMsg) && /\b(dich|dir|du)\b/i.test(fullCustomerMsg));
  const answerDirectQuestionRuleOpenAI = hasDirectQuestionToPersonaOpenAI
    ? ' PFLICHT: Der Kunde stellt eine direkte Frage an die Persona (z.B. was für dich schön ist, was du suchst – oder nach dem Namen: wie heißt du, wie ist dein Name). Beantwortet die Antwort diese Frage aus Persona-Sicht oder gibt sie sie nur zurück / lenkt um? Wenn nicht beantwortet → zuerst die Frage beantworten (bei Namensfrage: Persona-Namen nennen; sonst z.B. was für mich/dich schön ist), dann Gegenfrage.'
    : '';
  ...
  const noMetaPhrasesRule = ' VERBOTEN: Meta-Kommentare wie "Ich finde es spannend was du dir wünschst", "Was wäre dein perfektes Szenario?", "Was waere fuer dich der perfekte Moment fuer Doggy/Reiterstellung/..." oder vergleichbare Meta-/Position-Fragen. Stattdessen: konkrete thematische Fragen (z.B. zum Treffen, zu Vorlieben, zur Situation).';
  ...
  const meetingRule = (context.isMeetingSituation === true)
    ? (context.isDirectMeetingRequest === true
      ? ' PFLICHT TREFFEN: Keine Zusage zu Tag/Uhrzeit/Ort. "Ich schaue mal wann es klappt", "muss mal schauen wann es bei mir passt", "muessen mal schauen wann es bei uns klappt" sind ERLAUBT. Hat der Kunde eine konkrete Orts- oder Zeitangabe gemacht (z.B. "hab frei am Sonntag", "Kaffee treffen", "Hast du Zeit?")? Dann soll die Nachricht AM ANFANG einen kurzen unverbindlichen Einstieg haben (z.B. "Ich geb dir noch Bescheid ob ich da kann, " oder "Ich schau mal ob es da bei mir klappt, " oder "Da bin ich eigentlich schon verplant aber ich schau mal ob ich da kann, ") – wenn nicht vorhanden, am Anfang ergaenzen. VERBOTEN zu direkt: "einen Tag raussuchen", "wenn wir uns einen Tag raussuchen" – ersetzen durch unverbindlich. Kunde will treffen: VERBOTEN "lass uns erstmal noch schreiben", "erst kennenlernen" – ersetzen durch: auf Wunsch eingehen. NICHT aktiv nach Ort/Ideen fragen; andeuten oder vage Vorlagen. VERBOTEN "Was denkst du darüber?" – durch thematische Frage zum Treffen ersetzen. Ein Ausweichgrund reicht.'
  ...
  const sexualPerspectiveAndQuestionsRuleOpenAI = (context.allowSexualContent === true && context.customerTalkingAboutSexWithFake === true)
    ? ' SEX KONTEXT: Echo der Kundenszenario-Formulierung (z.B. "wenn du mir den Ruecken zudrehst") kann missverstaendlich sein → aus Persona-Sicht formulieren (z.B. "wenn ich dir den Ruecken zudrehe"). Schlussfrage bei Sex: Keine generischen/schwachen Fragen ("Was denkst du wie intensiv...", "Pizza oder Brüste", "Wie heftig pulsiert...") → ersetzen durch Frage die das Szenario erweitert (Stellungen, Spielzeug, was Neues ausprobieren, Tempo).'
    : '';
  const contactIrritatedRule = (context.contactRequest === true || context.customerIrritated === true)
    ? (context.contactRequest === true
