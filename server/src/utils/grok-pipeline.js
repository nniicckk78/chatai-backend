...
function buildNormalPrompt({
  ...
  if (alreadyStatedNoRepeat && alreadyStatedNoRepeat.trim()) {
    systemContent += `BEREITS IN DER AKTUELLEN KUNDENNACHRICHT GENANNT: ${sanitizeForApiContent(alreadyStatedNoRepeat.trim())}. Stelle KEINE Rueckfrage nach diesen Punkten (z.B. nicht "woher kommst du?" wenn er schon Wohnort gesagt hat, nicht "wie heisst du?" wenn er den Namen genannt hat). Reagiere darauf, frage nicht erneut.\n\n`;
  }
...
async function runMistralCorrector({ customerMessage = '', context = {}, grokText = '', learningContext = '', exampleSnippet = '', planSnippet = '', conversationSnippet = '' }) {
  ...
  // Keine Fragen nach Dingen, die der Kunde im Chat / in der letzten Nachricht bereits gesagt hat
  const noAlreadyAnsweredRule = ' BEREITS BEANTWORTET: Enthaelt die Antwort eine Frage nach etwas (Name, Wohnort, Alter, Beruf, woher er kommt, was er sucht), das der Kunde in seiner letzten Nachricht oder im Chat-Verlauf bereits gesagt hat? Wenn ja → diese Frage entfernen oder durch eine andere passende Frage ersetzen. Nicht nach bereits Genanntem fragen. Nutze den Chat-Verlauf fuer Kontext.';
