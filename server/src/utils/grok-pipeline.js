...
async function runMistralCorrector({ customerMessage = '', context = {}, grokText = '', learningContext = '', exampleSnippet = '', planSnippet = '', conversationSnippet = '' }) {
  ...
  const hasDirectQuestionToPersonaMistral = hasNameQuestionInclUndDuMistral || hasWieGehtEsDirMistral || hasTreffenFrageMistral || hasWasMachstDuHeuteMistral || hasWasSollIchMitDirMistral || hasUndDuRueckfrageMistral || /\b(für dich|für dich\?|was macht .* für dich|was suchst du|wie oft|was wäre für dich|besonders schön für dich)\b/i.test(fullCustomerMsg) || (/\?/.test(fullCustomerMsg) && /\b(dich|dir|du)\b/i.test(fullCustomerMsg));
