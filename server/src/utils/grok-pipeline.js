...
async function runPlanningStep(customerMessage, detectedSituations, allRules, conversationHistory = '') {
  ...
  const contextBlock = contextSnippet
    ...
    : '';
  const userContent = `${contextBlock}Aktuelle Kundennachricht: "${sanitizeForApiContent(customerForPlan)}"\n\nErkannte Situation(en): ${situationList}.${contactHint}${sexualHint}${romanticHint}${wasWillstDuWissenHint}\n\nGib in 2–4 Sätzen an: Welche Regeln/Prioritäten gelten hier? Welcher Ton? WICHTIG: Nenne explizit ALLE Themen/Beduerfnisse in der Nachricht (nicht nur die erkannten Situationen), z.B. Sex, gutes Gespraech wichtig, nicht ewig nur schreiben wollen, absprechen/Anruf/Simsen, Treffen-Vorfreude – die Antwort muss auf JEDES davon mindestens kurz eingehen, nichts im Raum stehen lassen. Nenne alle Aspekte der Kundennachricht (Fragen, Aussagen, Themen). Fragt der Kunde auf die letzte Moderatoren-Nachricht zurueck (z.B. "woher weisst du das")? Dann: explizit darauf eingehen. Was unbedingt vermeiden? Nur den Plan, keine Antwort an den Kunden.`;
  const messages = [
    {
      role: 'system',
...
async function runOpenAIFullCorrector({ customerMessage = '', context = {}, grokText = '', learningContext = '', exampleSnippet = '', planSnippet = '', conversationSnippet = '' }) {
  ...
  const meetingRule = (context.isMeetingSituation === true)
    ? (context.isDirectMeetingRequest === true
      ? ' PFLICHT TREFFEN: Keine Zusage zu Tag/Uhrzeit/Ort. "Ich schaue mal wann es klappt", "muss mal schauen wann es bei mir passt" sind ERLAUBT. Kunde will treffen: VERBOTEN "lass uns erstmal noch schreiben", "erst kennenlernen", "noch ein bisschen schreiben" – ersetzen durch: auf Wunsch eingehen (z.B. will auch nicht ewig nur schreiben, schaue wie es passt) + konkrete Frage UEBER Treffen (Café, Essen, Vorlieben – variieren). VERBOTEN "Was denkst du darüber?" – durch thematische Frage zum Treffen ersetzen. Ein Ausweichgrund reicht.'
...
Du bist ein Korrektor für Chat-Moderator-Antworten. Entscheide anhand des gesamten Kontexts und der Kundennachricht. Nur bei klaren Regelverstoessen umschreiben; Stil und Wortschatz der ursprünglichen Antwort möglichst beibehalten. ${sexualRule} ${contactIrritatedRule}${meetingRule} ${multiThemeRuleOpenAI} ${noFalseSingleRule} ${noTelefonsexPhotoRule} ${ruckfrageCallbackRule} ${noWiedersehenRule} ${noSharedPastRule} ${noTimeAgreementRule} ${boundaryConsistencyRule} ${selfConsistencyRule} ${noEchoOwnModeratorStatementRule} ${censorshipRule} ${noOftTreffenRule} ${limitationFollowUpRule} ${engageOnSuggestionsRule} ${metaRule} ${noMetaPhrasesRule} ${noEchoRule} ${echoReplaceRule}${questionAndWholeRule}${minLengthRule}${sameActivityPlaceRule}${noParaphraseRule}${customerSingularRule}${singleProfileNoWirRule} Jede Nachricht muss eine Frage enthalten; mindestens 120 Zeichen.${humanTyposHint} ${imageOnlyAnnouncedRule} ${noPlaceClaimRule} ${noStrongHopeRule} ${neutralMessageNoSexRule}
...
Außerdem: Umlaute korrigieren (ae→ä, oe→ö, ue→ü wo es Umlaut ist; nicht in Feuer, Museum, etc.). Immer ss statt ß. Keine Anführungszeichen, keine Bindestriche. Gib NUR die fertige korrigierte Nachricht zurück – kein anderer Text.`;
  ...
  const userContent = `${contextLine}${planBlock}${conversationBlock}${learningBlock}${exampleBlock}Kundennachricht (nur Kontext – nicht ausgeben):\n"${sanitizeForApiContent(customerForCorrector)}"\n\nZu korrigierende Moderatoren-Antwort:\n\n${sanitizeForApiContent(grokText.trim())}\n\nPrüfe und korrigiere: Geht die Antwort auf die GESAMTE Kundennachricht ein? Enthaelt sie eine Frage? Mindestens 120 Zeichen? Umlaute und ss statt ß? Echo/Paraphrase: Gibt die Antwort den Kundentext (oder sein Szenario/Wunsch) woertlich oder sinngemaess zurueck? Wenn ja → ersetzen durch eigenstaendige Reaktion. Meta: Enthaelt die Antwort "Ich finde es spannend was du dir wünschst", "Was wäre dein perfektes Szenario?" oder aehnliche Meta-Fragen? Wenn ja → durch konkrete thematische Frage ersetzen. Kunde will treffen: Enthaelt die Antwort "erstmal noch schreiben", "erst kennenlernen" oder "Was denkst du darüber?"? Wenn ja → umschreiben: auf Treffenwunsch eingehen + konkrete Frage zum moeglichen Treffen (Café, Essen, Vorlieben – variieren). Gib NUR den fertigen korrigierten Text zurück.`;
