 ...
 * @param {{ minLength?: number }} [options] - minLength für Wohnort-Pfad etc. (Standard: MIN_MESSAGE_LENGTH)
 */
async function ensureMinimumLength(msg, customerMessage, options) {
  if (!msg || typeof msg !== 'string') return msg || '';
  const minLen = (options && typeof options.minLength === 'number') ? options.minLength : MIN_MESSAGE_LENGTH;
  const m = msg.trim();
  if (m.length >= minLen) return m;
  if (minLen !== MIN_MESSAGE_LENGTH) return m;
...
const EXTRA_RULES = {
  ...
  keinBestellenKaufen: 'BESTELLEN/KAUFEN/BUCHEN: NIEMALS den Kunden auffordern oder dazu ermuntern, etwas zu bestellen, zu kaufen oder zu buchen (z.B. "Bestell die schonmal", "kauf mal", "bestell mal", "buch das", "bestell das", "kannst du das bestellen"). Du darfst auf seine Angebote eingehen (z.B. "würde mich freuen", "klingt gut") – aber KEINE Aufforderung an ihn, etwas zu ordern/buchen/kaufen. Wenn er sagt, er bestelle etwas: freuen, kommentieren, NICHT "bestell schonmal" o.ae.\n'
};
...
/** Kurzer Regeln-Auszug für die Sinnprüfung (max. 500 Zeichen). */
function buildRulesSnippetForSenseCheck(allRules, maxLen = 500) {
  if (!allRules || typeof allRules !== 'object') return '';
  const parts = [];
  if (allRules.situationalResponses && typeof allRules.situationalResponses === 'object') {
    const names = Object.keys(allRules.situationalResponses).slice(0, 12);
    if (names.length) parts.push('Situationen: ' + names.join(', '));
  }
  if (allRules.generalRules && String(allRules.generalRules).trim()) {
    parts.push('Allgemein: ' + String(allRules.generalRules).trim().slice(0, 200));
  }
  const snippet = parts.join(' | ');
  return snippet.length > maxLen ? snippet.slice(0, maxLen) + '...' : snippet;
}

/** Rechtschreibung, Umlaute und optional Sinnprüfung per KI. Bei Kontext: prüft auch, ob die Nachricht logisch/grammatisch sinnvoll ist (z.B. keine "einzige Stellungen", keine Vermischung Person/Sache); korrigiert nur die fehlerhaften Stellen, nicht die ganze Nachricht. */
async function correctSpellingAndUmlautsWithOpenAI(message, context = {}) {
  ...
  const hasContext = !!(context && (context.customerMessage || context.rulesSnippet));
  try {
    const client = getClient();
    if (!client) return message;
    let system = `Du bist ein Korrektor für deutsche Chat-Moderator-Nachrichten. Zwei Schritte:

1) RECHTSCHREIBUNG UND UMLAUTE (immer):
- Umlaute: ae/oe/ue nur dann durch ä/ö/ü ersetzen, wenn es der Umlaut ist (z.B. fuer→für, moechte→möchte). NICHT in echten Wörtern wie Feuer, Museum, Schuhe, Abenteuer, Poesie.
- Falsche Mischformen korrigieren: z.B. "teür"→"teuer", "Füer"→"Feuer". Immer ss statt ß. Keine Anführungszeichen oder Bindestriche einfügen.

2) SINNPRÜFUNG (nur wenn Kontext gegeben): Prüfe, ob die Nachricht logisch und grammatisch sinnvoll ist – besonders im Bezug auf die Kundennachricht. Typische Fehler: "einzige" mit Plural ("einzige Stellungen" → "einzige Stellung"); Vermischung von Person und Sache ("wäre ich die einzige Stellung" – Person ist keine Stellung, Formulierung anpassen); unklare oder wirre Fragen. Wenn du solche Stellen findest: korrigiere NUR die fehlerhafte Formulierung, den Rest der Nachricht lasse unverändert. Wenn alles sinnvoll ist, ändere am Inhalt nichts.

Ausgabe: Gib NUR die vollständige, korrigierte Nachricht zurück, keine Erklärungen.`;
    let user = '';
    if (hasContext && context.customerMessage) {
      user += `Kundennachricht (Kontext):\n${sanitizeForApiContent(String(context.customerMessage).trim().slice(0, 500))}\n\n`;
    }
    if (hasContext && context.rulesSnippet) {
      user += `Regeln (Auszug):\n${sanitizeForApiContent(String(context.rulesSnippet).slice(0, 400))}\n\n`;
    }
    user += `Zu korrigierende Moderatoren-Nachricht:\n\n${sanitizeForApiContent(message.trim())}`;
    const out = await callOpenAI(
      [{ role: 'system', content: system }, { role: 'user', content: user }],
      { temperature: 0.1, max_tokens: 600, timeoutMs: 12000 }
    );
    const corrected = (out || '').trim();
    if (corrected && corrected.length >= 15 && corrected.length <= message.length * 1.8) {
      console.log(hasContext ? '✅ KI-Rechtschreib- und Sinnprüfung angewendet' : '✅ KI-Rechtschreibkorrektur angewendet');
      return corrected;
    }
  } catch (err) {
...
/** Wendet optional OpenAI-Rechtschreib- und ggf. Sinnprüfung auf die finale Nachricht an (wenn API-Key gesetzt). context: { customerMessage?, conversationSnippet?, rulesSnippet? } für Sinnprüfung. */
async function applySpellingCorrectionIfAvailable(finalMessage, context = {}) {
  ...
  return correctSpellingAndUmlautsWithOpenAI(finalMessage, context);
}
...
Antworte kurz: Nenne deinen Wohnort genau wie angegeben (z. B. "Ich wohne in Magdeburg"), nicht umschreiben. Dann natürlich weiterschreiben – z. B. eine Gegenfrage wie "Woher kommst du?" oder thematisch passend. Keine Anführungszeichen am Anfang/Ende. KEINE Bindestriche. Immer ss, nie ß.`;
...
/** Extrahiert die letzte Fake-Nachricht aus dem strukturierten Kontext (reply.js: buildStructuredConversationContext). */
function extractLastFakeMessageFromStructuredContext(conversationHistory) {
  if (!conversationHistory || typeof conversationHistory !== 'string') return '';
  const m = conversationHistory.match(/Letzte Nachricht von Fake \(du\):\s*([\s\S]*?)(?=\n\nAktuelle Kundennachricht|\n\nÄlterer Verlauf|$)/i);
  if (!m) return '';
  const text = (m[1] || '').trim();
  return text === '(keine)' ? '' : text;
}

/** Prüft ob im Text bereits "Guten Morgen" oder "Guten Tag" vorkommt (Begrüßung schon getauscht). */
function hasGreetingInLastExchange(text) {
  if (!text || typeof text !== 'string') return false;
  const t = text.slice(-600).toLowerCase();
  return /\b(guten\s+morgen|guten\s+tag|guten\s+abend)\b/.test(t);
}
...
function buildNormalPrompt({
  ...
  const lastExchangeText = [conversationHistory || '', customerMessage || ''].join('\n');
  if (hasGreetingInLastExchange(lastExchangeText)) {
    systemContent += 'BEGRUESSUNG: Bereits "Guten Morgen"/"Guten Tag" im letzten Austausch – Antwort NICHT erneut damit beginnen, inhaltlich einsteigen.\n\n';
  }
  if (isMeetingSituation) {
    if (isDirectMeetingRequest) {
      systemContent += 'TREFFEN/BAR-FRAGEN: NIEMALS Bar, Ort oder Uhrzeit nennen. Wenn der Kunde bereits Ideen genannt hat (z.B. Kino, Kaffee, kochen): Gehe darauf ein – sag was dir gefaellt, statt dieselben Ideen als neue Aufzaehlung zurueckzugeben. Keine konkrete Planung (Ort, Zeit, Zusage). VERBOTEN auch weiche Zusagen wie "am Abend koennten wir uns Zeit nehmen", "dann koennten wir", "am Abend passt", "Abend waere gut", "dann haetten wir Zeit" – klingt nach Zusage. Nur hypothetisch (wie toll es waere), keine Andeutung dass eine Zeit "passt" oder "wir uns Zeit nehmen". VERBOTEN: "wiedersehen", "wiederzusehen", "mal wieder treffen", "wenn wir uns wiedersehen" (auch in Fragen) – ihr habt euch NIE getroffen, "wieder" impliziert ein bereits stattgefundenes Treffen. Stattdessen z.B. "wenn wir uns (mal) sehen würden" oder Gegenfrage ohne "wieder". Wenn der Kunde Terminvorschlaege oder Zeitfenster nennt: NICHT schreiben, dass die Zeiten dir passen oder du flexibel bist – unverbindlich bleiben. NIEMALS begruenden, dass du dich im Chat wohler fuehlst. Zeitangabe (morgen Abend, Wochenende) nur mit „im Chat“ oder „wenn wir schreiben“ – sonst wirkt es wie Treffen-Zusage. AUSWEICHEN: NICHT standardmaessig "erst kennenlernen" oder "besser kennenlernen" – wirkt zu abweisend. Bevorzuge weichere Alternativen: "muss schauen wann es bei mir klappt", "schauen ob wir das hinbekommen", konkrete Gruende (Arbeit, Wochenende, zu spontan, muss noch erledigen). NICHT DOPPELN: Wenn du bereits einen konkreten Grund nennst, schreibe NICHT zusaetzlich "besser kennenlernen" – ein Grund reicht. Beende mit einer klaren Gegenfrage.\n\n';
    } else {
      systemContent += 'TREFFEN/VORFREUDE: Der Kunde aeussert nur Vorfreude oder Hoffnung auf ein Treffen (z.B. "hoffe wir sehen uns bald"), keine konkrete Treffen-Anfrage. VERBOTEN: "wiedersehen", "wiederzusehen", "wenn wir uns wiedersehen" – ihr habt euch NIE getroffen. Stattdessen "wenn wir uns sehen würden" o.ae., nie "wieder". Antworte NICHT mit "erst kennenlernen" oder "erst quatschen" – nutze die Vielfalt aus den Trainingsdaten (z.B. positiv darauf eingehen, Gegenfrage stellen). Keine Zusage zu Ort/Zeit. Beende mit einer klaren Gegenfrage.\n\n';
    }
  }
  if (isContactRequest) {
  ...
  systemContent += EXTRA_RULES.orte + EXTRA_RULES.vorfreude + EXTRA_RULES.telefonsexFotos + EXTRA_RULES.ruckfrageCallback + EXTRA_RULES.flirtyKontinuitaet + EXTRA_RULES.keinEcho + EXTRA_RULES.beziehungszieleVsTreffen + EXTRA_RULES.keinBestellenKaufen;
  ...
  if (situationRulesBlock && situationRulesBlock.trim()) {
    ...
    if (situationRulesBlock.includes('Sexuell')) {
      systemContent += 'HINWEIS Sexuelle Themen: Orientiere dich an der Explizitheit der Kundennachricht – nicht ueberbieten. Schreibt der Kunde zurueckhaltend/andeutend, antworte ebenfalls zurueckhaltender; wird er expliziter, kannst du mitgehen. Nicht von dir aus eine Stufe drauflegen. Wenn der Kunde ein sexuelles Szenario beschreibt: NICHT dasselbe Szenario Schritt fuer Schritt zurueckspielen – nur kurz reagieren (Gefuehl/Erregung) und mit einer Frage fortfuehren. Keine Meta-Fragen wie "Was erregt dich am meisten dabei?" – lieber z.B. "Und dann?", "Was als naechstes?", "Haerter oder langsamer?"\n\n';
    }
  }
...
FRAGEN: Die Gegenfrage am Ende MUSS wie in den Trainingsbeispielen sein – konkret, thematisch, zur Kundennachricht passend. VERBOTEN: generische Meta-Fragen wie "Was interessiert dich (noch/denn) an unseren Gesprächen?", "Was magst du an mir?", "Was gefällt dir an unserem Chat?" – solche Fragen stehen NICHT in den Trainingsdaten. Nutze stattdessen Fragen wie in den Beispielen (z.B. nachfragen zum Thema, zu dem was der Kunde gesagt hat, oder thematisch erweiternd).
...
PFLICHT: Nur eine Nachricht ausgeben; Frage am Ende (im Stil der Beispiele); keine Meta-Kommentare; KEIN ECHO – nicht Name, Alter, Ort oder Vorlieben des Kunden zurueckspielen, eigenstaendig formulieren.`;
  ...
  let userContent = '';
  const lastFakeMsg = extractLastFakeMessageFromStructuredContext(conversationHistory || '');
  if (conversationHistory && conversationHistory.trim()) {
    const hist = conversationHistory.trim();
    const maxLen = 800;
    const historySnippet = hist.length <= maxLen
      ? hist
      : hist.substring(0, 480) + '\n[...]\n' + hist.substring(hist.length - 300);
    userContent += `Chat-Verlauf (Auszug):\n${sanitizeForApiContent(historySnippet)}\n\n`;
  }
  if (lastFakeMsg) {
    userContent += `DEINE LETZTE NACHRICHT (daran festhalten, nicht widersprechen): "${sanitizeForApiContent(lastFakeMsg.slice(0, 400))}"\n\n`;
  }
  userContent += `PFLICHT: Deine Antwort muss auf ALLE Punkte dieser Kundennachricht eingehen (jede Frage, jedes Thema, jede Aussage). Ignoriere nichts.\n\n`;
  userContent += `KRITISCH – KONSISTENZ: Was hast DU (Fake/Moderator) bereits ueber dich gesagt (z.B. was du isst, machst, wo du bist)? Bleib dabei – wechsle nicht zu etwas anderem. Wenn du "Lasagne" gesagt hast, sag nicht "Nudeln" oder "Omelett".\n\n`;
...
async function runPlanningStep(customerMessage, detectedSituations, allRules, conversationHistory = '') {
  ...
  const contextBlock = contextSnippet
    ...
    : '';
  const userContent = `${contextBlock}Aktuelle Kundennachricht: "${sanitizeForApiContent(customerForPlan)}"\n\nErkannte Situation(en): ${situationList}.${contactHint}${sexualHint}${romanticHint}\n\nGib in 2–4 Sätzen an: Welche Regeln/Prioritäten gelten hier? Welcher Ton? Worauf muss die Antwort eingehen? Nenne alle Aspekte der Kundennachricht (Fragen, Aussagen, Themen) – die Antwort muss auf alle eingehen. Fragt der Kunde auf die letzte Moderatoren-Nachricht zurueck (z.B. "woher weisst du das", "wie meinst du das", "wer weiss")? Dann: explizit darauf eingehen, Rueckbezug herstellen, Flirt-Ton beibehalten – nicht themenfremd werden. Was unbedingt vermeiden? Nur den Plan, keine Antwort an den Kunden.`;
  const messages = [
    {
      role: 'system',
...
async function runMistralCorrector({ customerMessage = '', context = {}, grokText = '', learningContext = '', exampleSnippet = '', planSnippet = '', conversationSnippet = '' }) {
  ...
  // Bestellen/Kaufen/Buchen: NIEMALS den Kunden auffordern – Korrektor entfernt/ersetzt solche Formulierungen
  const noBestellenKaufenRule = ' Enthaelt die Antwort eine Aufforderung an den Kunden, etwas zu bestellen, zu kaufen oder zu buchen (z.B. "Bestell die schonmal", "bestell mal", "kauf mal", "buch das", "bestell das", "kannst du das bestellen")? Wenn ja → diesen Satz/Teil entfernen oder umformulieren: nur freuen/kommentieren (z.B. "würde mich freuen", "klingt gut"), KEINE Aufforderung an ihn zu ordern/buchen/kaufen.';
...
async function runGrokPipeline(opts) {
  ...
  if (!isFirstMessage && !isASA) {
    ...
    if (loc.isLocationQuestion && loc.cityInstructions) {
      try {
        ...
        const cityPlanSnippet = 'Wohnort-Frage: Antwort kurz (1–2 Sätze). Wohnort nennen + eine natürliche Gegenfrage (z.B. Woher kommst du? Was hast du heute vor?). Keine zweite generische Frage. Nicht auf 120 Zeichen strecken. Umlaute (ä, ö, ü), ss statt ß.';
        let corrected = null;
        ...
        finalMessage = await ensureMinimumLength(finalMessage, customerMessage, { minLength: 50 });
        finalMessage = await applySpellingCorrectionIfAvailable(finalMessage, {
          customerMessage,
          conversationSnippet: (conversationHistory || '').trim().slice(-400),
          rulesSnippet: buildRulesSnippetForSenseCheck(allRules)
        });
        return emptyResult({
          safety: safetyCheck,
          finalMessage,
  ...
  if (isASA) {
    ...
    try {
      ...
      finalMessage = await applySpellingCorrectionIfAvailable(finalMessage, {
        customerMessage: (asaConversationContext || '').trim().slice(0, 400),
        rulesSnippet: buildRulesSnippetForSenseCheck(allRules)
      });
      return emptyResult({
        safety: safetyCheck,
