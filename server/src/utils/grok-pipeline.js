...
WICHTIG: Schreibe NIEMALS deinen eigenen Namen. Schreibe mit ä, ö, ü (Umlaute), z.B. wäre, möchte, für. Immer ss, nie ß. KEINE Bindestriche. KEINE Anführungszeichen am Anfang/Ende. Nutze Zeitkontext (${weekday}, ${timePhase}). Antworte natürlich, mindestens 120 Zeichen.`;
...
function buildNormalPrompt({
  ...
  if (isMeetingSituation) {
    if (isDirectMeetingRequest) {
      systemContent += 'TREFFEN/BAR-FRAGEN: NIEMALS Bar, Ort oder Uhrzeit nennen. Keine Vorschlaege machen, nicht auf Vorschlaege eingehen. Wenn der Kunde Terminvorschlaege oder Zeitfenster nennt (z.B. naechste 1–2 Wochen, Vormittag/Nachmittag): NICHT schreiben, dass die Zeiten dir passen oder du flexibel bist – unverbindlich bleiben, wie in den Trainingsdaten. Nur darueber schreiben, was man "wuerde" machen, wie toll es "waere" – mehr nicht. Orientiere dich streng an den Trainingsdaten, dort ist es gezeigt. NIEMALS begruenden, dass du dich im Chat wohler fuehlst. Beende mit einer klaren Gegenfrage.\n\n';
    } else {
...
Rechtschreibung: Umlaute (ä, ö, ü), ss statt ß, keine Anführungszeichen, keine Bindestriche.
...
PFLICHT: Nur eine Nachricht ausgeben; Frage am Ende; keine Meta-Kommentare, keine woertliche Wiederholung der Kundennachricht.`;
...
async function runMistralCorrector({ customerMessage = '', context = {}, grokText = '', learningContext = '', exampleSnippet = '', planSnippet = '', conversationSnippet = '' }) {
  ...
  if (MISTRAL_USE_MINIMAL_PROMPT) {
    ...
    const contactCheck = context.contactRequest === true ? '\n(6) Hat der Kunde nach Kontaktdaten/Telegram/WhatsApp gefragt und die Antwort lehnt das nicht ab? Wenn ja → freundliche Ablehnung (keine Nummer/Telegram) + Verweis auf Plattform einbauen.' : '';
