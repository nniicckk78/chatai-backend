...
async function runMistralCorrector({ customerMessage = '', context = {}, grokText = '', learningContext = '', exampleSnippet = '', planSnippet = '', conversationSnippet = '' }) {
  ...
  const umlautAndSsRule = ' PFLICHT UMLAUTE: Enthaelt die Antwort ae, oe oder ue anstelle von ä, ö, ü (z.B. fuer→für, moechte→möchte, Koerper→Körper, ueber→über, kuessend→küssend, spueren→spüren, schoen→schön, waere→wäre, naechstes→nächstes)? Dann durch echte Umlaute ersetzen. NICHT ersetzen in Woertern wo ue/oe/ae zwei getrennte Buchstaben sind (Feuer, Museum, Schuhe, Abenteuer, Poesie). Immer ss statt ß.';
