...
/** Mindestlaenge fuer Uebernahme einer Korrektur: nie unter Plattform-Minimum, zusaetzlich Schutz vor starkem Schrumpfen. */
function minLengthToAcceptCorrectorOutput(lenOrig) {
  const o = typeof lenOrig === 'number' && lenOrig > 0 ? lenOrig : 0;
  return Math.max(MIN_MESSAGE_LENGTH, Math.max(30, Math.floor(o * 0.4)));
}
...
function buildNormalPrompt({
  ...
  let systemContent = MODERATOR_PERSONA + getDialogLanguageOverrideBlock(profileInfo || {}) + GENERAL_BEHAVIOR + PRIORITY_NOTE + ONE_TASK_PER_MESSAGE + FOCUS_LAST_MESSAGE_GENERATOR + GESPRACHSFUEHRUNG_LETZTE_NACHRICHT;
...
async function runMistralCorrector({ customerMessage = '', context = {}, grokText = '', learningContext = '', exampleSnippet = '', planSnippet = '', conversationSnippet = '' }) {
  ...
  const meetingTreffenAussichtRule = (context.isMeetingSituation === true)
    ...
    : '';
  const minLengthRule = CORRECTOR_MIN_LENGTH_RULE + ' Keine themenfremden neuen Gross-Themen erfinden; Ergaenzungen nur passend zur Kundennachricht und zur bestehenden Antwort.';
...
async function runOpenAIFullCorrector({ customerMessage = '', context = {}, grokText = '', learningContext = '', exampleSnippet = '', planSnippet = '', conversationSnippet = '' }) {
  ...
  const minLengthRule = CORRECTOR_MIN_LENGTH_RULE + ' Keine themenfremden neuen Gross-Themen; Ergaenzungen nur passend zur Kundennachricht und zur Antwort.';
    ...
    if (loc.isLocationQuestion && loc.cityInstructions) {
      try {
        ...
        const cityPlanSnippet = (loc.skipAskWhereCustomerLives
          ? 'Wohnort-Frage: Antwort nennt den eigenen (Fake-)Wohnort. Kunde hat seinen Ort schon genannt oder Profil – KEIN Woher-kommst-du / Wo-wohnst-du. Andere Schlussfrage. Mindestens 120 Zeichen. Umlaute (ä, ö, ü), ss statt ß.'
          : 'Wohnort-Frage: Antwort nennt den Wohnort (Stadt/Stadtteil) und stellt eine Frage zurück. Mindestens 120 Zeichen. Umlaute (ä, ö, ü), ss statt ß.')
