...
async function runGrokPipeline(opts) {
  ...
  try {
    ...
    let finalMessage = await callGrok(messages);
    finalMessage = postProcessMessage(finalMessage);
    // ========== KORREKTOR: LoRA (USE_GROK_CORRECTOR_LORA) | Grok (USE_GROK_AS_CORRECTOR) | OpenAI (wenn OPENAI_API_KEY gesetzt) ==========
    ...
    const useOpenAICorrector = hasOpenAI && !useGrokAsCorrector && !(useCorrectorEnv && correctorModelId);
