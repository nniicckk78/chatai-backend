...
PFLICHT: Nur eine Nachricht ausgeben; Frage am Ende (im Stil der Beispiele); keine Meta-Kommentare; KEIN ECHO – nicht Name, Alter, Ort oder Vorlieben des Kunden zurueckspielen, eigenstaendig formulieren.`;
  ...
  userContent += `FRAGE BEANTWORTEN: Stellt der Kunde eine direkte Frage an DICH (z.B. was macht X für dich schön, was suchst du, wie oft)? → ZUERST diese Frage beantworten (aus deiner Sicht), nicht die Frage nur zurückgeben oder umlenken. Danach Gegenfrage.\n\n`;
...
async function runPlanningStep(customerMessage, detectedSituations, allRules, conversationHistory = '') {
  ...
  const unclearHint = isUnclear
    ...
    : '';
  const hasDirectQuestionToPersona = /\b(für dich|für dich\?|was macht .* für dich|was suchst du|wie oft|was wäre für dich|was findest du|was ist für dich|besonders schön für dich)\b/i.test(customerSnippet) || (/\?/.test(customerSnippet) && /\b(dich|dir|du)\b/i.test(customerSnippet));
  const directQuestionHint = hasDirectQuestionToPersona
    ? ' PFLICHT: Der Kunde stellt eine direkte Frage an die Persona (z.B. was für dich schön ist, was du suchst). Plan muss enthalten: ZUERST diese Frage beantworten (aus Persona-Sicht), dann thematische Gegenfrage – nicht die Frage nur zurückgeben oder umlenken.'
    : '';
...
async function runMistralCorrector({ customerMessage = '', context = {}, grokText = '', learningContext = '', exampleSnippet = '', planSnippet = '', conversationSnippet = '' }) {
  ...
  const dagegenSexHopeRuleMistral = hasDagegenSexHopeMistral
    ...
    : '';
  const hasDirectQuestionToPersonaMistral = /\b(für dich|für dich\?|was macht .* für dich|was suchst du|wie oft|was wäre für dich|besonders schön für dich)\b/i.test(fullCustomerMsg) || (/\?/.test(fullCustomerMsg) && /\b(dich|dir|du)\b/i.test(fullCustomerMsg));
  const answerDirectQuestionRuleMistral = hasDirectQuestionToPersonaMistral
    ? ' PFLICHT: Kunde fragt die Persona direkt (z.B. was für dich schön ist). Beantwortet die Antwort die Frage oder gibt sie sie nur zurück? Wenn nicht → zuerst Frage beantworten, dann Gegenfrage.'
    : '';
...
async function runOpenAIFullCorrector({ customerMessage = '', context = {}, grokText = '', learningContext = '', exampleSnippet = '', planSnippet = '', conversationSnippet = '' }) {
  ...
  const dagegenSexHopeRuleOpenAI = hasDagegenSexHope
    ...
    : '';
  const hasDirectQuestionToPersonaOpenAI = /\b(für dich|für dich\?|was macht .* für dich|was suchst du|wie oft|was wäre für dich|besonders schön für dich)\b/i.test(fullCustomerMsg) || (/\?/.test(fullCustomerMsg) && /\b(dich|dir|du)\b/i.test(fullCustomerMsg));
  const answerDirectQuestionRuleOpenAI = hasDirectQuestionToPersonaOpenAI
    ? ' PFLICHT: Der Kunde stellt eine direkte Frage an die Persona (z.B. was für dich schön ist, was du suchst). Beantwortet die Antwort diese Frage aus Persona-Sicht oder gibt sie sie nur zurück / lenkt um? Wenn nicht beantwortet → zuerst die Frage beantworten (was für mich/dich schön ist, was ich suche, etc.), dann Gegenfrage.'
    : '';
