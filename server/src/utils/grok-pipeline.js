...
async function runMistralCorrector({ customerMessage = '', context = {}, grokText = '', learningContext = '', exampleSnippet = '', planSnippet = '', conversationSnippet = '' }) {
  ...
  const noStandardSexQuestionRule = ' STANDARD-SEX-FRAGE: Enthaelt die Antwort am Ende eine generische Sex-Frage wie "Was magst du denn so?", "Was magst du?", "Wuerdest du mich doggy nehmen?", "Hast du schonmal Doggy genommen?", "Doggy?", "Was magst du beim Sex?" obwohl der Kunde ein konkretes Thema genannt hat (Spielzeuge, Peitsche, Fesseln, Fantasie, etc.) oder detailliert sexuell geantwortet hat? Wenn ja → diese Frage durch eine thematisch passende Nachfrage zum genannten Inhalt ersetzen (zum gerade Besprochenen), keine generische Sex-Frage.';
