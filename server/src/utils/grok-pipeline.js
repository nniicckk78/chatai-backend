...
AUFBAU: Beginne NICHT mit einer Zusammenfassung oder Paraphrase der Kundennachricht (z.B. nicht "Ah, du fährst nachts und morgens bis mittags, Dienstag frei..."). Start mit kurzer Reaktion oder direkt mit deiner Aussage/Frage.
...
PFLICHT: Nur eine Nachricht ausgeben; Frage am Ende (im Stil der Beispiele); keine Meta-Kommentare; KEIN ECHO – nicht Name, Alter, Ort oder Vorlieben des Kunden zurueckspielen, eigenstaendig formulieren.`;
  ...
  if (examples && examples.length > 0) {
    ...
    userContent += '\nDiese Beispiele zeigen Antworten im Dialogverlauf. Antworte wie in den Beispielen an den Verlauf anknüpfend. Generiere eine Antwort mit Gegenfrage im Stil der Beispiele oben (thematisch, konkret).\n\n';
  }
...
async function runGrokPipeline(opts) {
  ...
  if (isMeetingSituation && isOnlyAskingAboutFakePlans(customerMessage)) {
    isMeetingSituation = false;
