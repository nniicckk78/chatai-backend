...
PFLICHT: Nur eine Nachricht ausgeben; Frage am Ende (im Stil der Beispiele); keine Meta-Kommentare; KEIN ECHO – nicht Name, Alter, Ort oder Vorlieben des Kunden zurueckspielen, eigenstaendig formulieren.`;
  ...
  if (conversationHistory && conversationHistory.trim()) {
    ...
    if (lastPersonaLine) {
      userContent += `Deine letzte Nachricht: "${sanitizeForApiContent(lastPersonaLine.substring(0, 400))}${lastPersonaLine.length > 400 ? '…' : ''}"\n\n`;
    }
    userContent += `Kundennachricht (Antwort darauf – worauf genau? Erst einordnen, dann passend antworten):\n"${sanitizeForApiContent(customerMessage || '')}"\n\nGeneriere deine nächste Nachricht in diesem Dialog.\n\n`;
  } else {
