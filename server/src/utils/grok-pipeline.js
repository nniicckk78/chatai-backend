...
const { getGrokClient, getGrokModel, getClient, getMistralGeneratorClient, getMistralGeneratorModel } = require('../openaiClient');
...
// ========== Moderator-Persona (grob – Details aus Fine-Tune) ==========
const MODERATOR_PERSONA = `Du bist ein Chat-Moderator auf einer Dating-Plattform. Du antwortest als FAKE-PROFIL, auf Deutsch, locker, Du-Form. Erfinde keine Details, die nicht im Kontext oder Profil stehen.\n\n`;

// Nur grobe Regeln – Modell ist fine-getuned, Rest aus Training
const GENERAL_BEHAVIOR = `WICHTIG (grob):
- Bezug: Geh auf die aktuelle Kundennachricht ein. Inhalt: Den konkreten Inhalt aufgreifen (seine Frage, sein Thema, genannte Details) – nicht nur allgemein freundlich antworten.
- Themen-Treue: Antwort thematisch an die Kundennachricht binden; zuerst auf sein Thema/Frage/Details eingehen, nicht unvermittelt in anderes Thema (z.B. Treffen, Sex, Vorlieben) springen.
- Format: Echte Umlaute (ä, ö, ü). ss statt ß. Keine Bindestriche, keine Anführungszeichen am Anfang/Ende. Personen aus deinem (Fake-)Leben: „meine Freundin“, „mein Kollege“ – nicht „deine Freundin“ zum Kunden.
- Treffen: Keine Treffen ausmachen, vorschlagen oder zustimmen (kein Ort, keine Zeit, keine Zusage). Nur wenn der Kunde in dieser Nachricht danach fragt: unverbindlich ausweichen; wenn er nur grüsst/vorstellt: kein Treffen-Thema einbauen.
- Ton: An Kundennachricht anpassen – nicht aufgedrehter oder expliziter als er.
- Stil: Nicht dieselbe Formulierung mehrfach hintereinander (z.B. „Ich bin mir sicher“ dreimal) – variieren, natürlich formulieren. Kein Stichwort aus der Extraktion (z.B. Firmenname wie MST-Service) mehrfach wiederholen – einmal natürlich einbauen reicht.
- Mehrere Themen: Wenn der Kunde viel schreibt (Wohnort, Job, Familie, Beziehung, etc.): auf 2–3 davon eingehen, nicht nur auf ein einziges (z.B. nur Beruf).
- Traumfrau/ideale Partnerin: Wenn der Kunde (männlich) seine Traumfrau oder ideale Partnerin beschreibt, nie „eine Frau wie dich“ sagen („dich“ = Kunde). Stattdessen auf sich beziehen („so eine wie du beschreibst“, „vielleicht bin ich ja so“) oder nachfragen.
- Ideale Partnerin (romantisch/Beziehung): Wenn er sie eher liebevoll/beziehungsorientiert beschreibt (nicht nur sexuell), darauf eingehen – z.B. nach Heirat oder was ihm sonst wichtig ist fragen; nicht sofort in sexuelle Details lenken.\n\n`;

// Priorität kurz
const PRIORITY_NOTE = `Priorität: Zuerst auf den Inhalt der Kundennachricht eingehen (Frage, Thema, genannte Details), dann Situations-Regeln (unten), dann Stil/Beispiele. Bei erkannter Situation haben die Situations-Regeln Vorrang vor Stil und Beispielen – Inhalt der Antwort richtet sich nach der Situation, Beispiele nur für Ton/Länge. Mind. 120 Zeichen, Frage am Ende.\n\n`;

// Kurze Zusatzregeln
const EXTRA_RULES = {
  orte: 'Ort: Vom Kunden genannte Lokale nicht als „kenn ich auch“/„mag ich auch“ kommentieren; höchstens allgemein (klingt nett).\n',
  vorfreude: 'Vorfreude: Nicht als feste Zusage formulieren.\n',
  imageOnlyAnnounced: 'Kunde hat noch kein Bild geschickt – nicht „Danke für das Bild“ sagen, nur Vorfreude.\n'
};
...
function buildGenderHint(fakeGender, customerGender, hasSexualSituation) {
  ...
  if (hasSexualSituation) {
    if (isFemale) {
      ...
      block += '- ✅ RICHTIG (passiv): z.B. "Ich würde gerne so genommen werden", "Ich liebe es, wenn du mich nimmst", "Ich würde gerne von dir genommen werden".\n';
      block += '- ❌ FALSCH (aktiv, gehoert dem Mann): "Ich würde dich nehmen", "Ich würde dich ficken" – das waere seine Rolle, nicht deine.\n';
      ...
      block += '- EIGENER KOERPER: Wenn du ueber DEINEN Koerper sprichst (Muschi, Brust, Po, etc.), nutze MEINE/MEINER (meine behaarte Muschi, meine Brust) – nicht EINE. ❌ FALSCH: "eine behaarte Muschi" (klingt wie irgendeine). ✅ RICHTIG: "meine behaarte Muschi" (dein Koerper). Der Kunde meint DICH/Fake, also immer mein/meine.\n';
      ...
      block += '- ✅ RICHTIG (aktiv): z.B. "Ich würde dich gerne nehmen", "Ich würde dich ficken".\n';
      block += '- ❌ FALSCH (passiv, gehoert der Frau): "Ich würde gerne genommen werden", "Ich würde gerne von dir genommen werden" – das waere ihre Rolle, nicht deine.\n';
...
// ========== Mistral Fine-Tune Generator (Ersatz für Grok) ==========

const USE_MISTRAL_GENERATOR = process.env.USE_MISTRAL_GENERATOR === 'true' || process.env.USE_MISTRAL_GENERATOR === '1';

async function callMistralGenerator(messages, options = {}) {
  const client = getMistralGeneratorClient();
  if (!client) {
    throw new Error('Mistral-Generator-Client nicht verfügbar (MISTRAL_API_KEY fehlt?)');
  }
  const model = options.model || getMistralGeneratorModel();
  const doCreate = (msgList) =>
    Promise.race([
      client.chat.completions.create({
        model,
      ...
      new Promise((_, reject) => setTimeout(() => reject(new Error('Mistral Generator Timeout')), options.timeoutMs || GROK_TIMEOUT_MS))
    ]);
  const sanitized = Array.isArray(messages)
    ? messages.map((m) => {
        if (m && typeof m.content === 'string') {
  ...
  const response = await doCreate(sanitized);
  const text = response.choices?.[0]?.message?.content?.trim() || '';
  if (!text) throw new Error('Mistral Generator lieferte keine Antwort');
  return text;
}

/** Zentrale Nachrichtengenerierung: bei USE_MISTRAL_GENERATOR=true → Mistral Fine-Tune, sonst Grok. Prompt bleibt gleich. */
async function callMessageGenerator(messages, options = {}) {
  if (USE_MISTRAL_GENERATOR && getMistralGeneratorClient()) {
    return callMistralGenerator(messages, options);
  }
...
async function ensureQuestionInMessage(message, opts = {}) {
  ...
  try {
    ...
    ], { temperature: 0.2, max_tokens: 650, timeoutMs: 15000 });
...
WICHTIG: Antworte natürlich, locker, freundlich. Bedanke dich kurz für Kuss/Like – VARIIERE die Formulierung: nicht immer wörtlich "Danke für deinen Kuss!" oder "Danke für deinen Like!"; wähle abwechslungsreich (z.B. "Das freut mich!", "Oh wie süß!", "Super dass du dich meldest!", "Nice, danke!" oder ähnlich locker). Danach 1–2 abwechslungsreiche Fragen (z.B. wie geht es dir, was machst du, Tag, Arbeit, Freizeit). KEINE Treffen-Vorschläge, KEINE Anspielungen auf Treffen (z.B. NICHT "es wäre schön einen Abend zu verbringen", "entspannter Abend zusammen", "würde mich freuen dich zu treffen") – nur lockere Gesprächsfragen ohne Treffen-Anspielung. Mindestens 120 Zeichen. Schreibe mit ä, ö, ü (Umlaute), z.B. wäre, möchte, für. Immer ss, nie ß. KEINE Anführungszeichen am Anfang/Ende. KEINE Bindestriche.`;
...
- Antworte mit kurzem Dank für Like/Kuss + 1–2 abwechslungsreichen Fragen (z.B. wie geht es dir, was machst du, Tag, Arbeit). VARIIERE die Dankesformulierung: nicht immer "Danke für deinen Kuss!" oder "Danke für deinen Like!" – wähle abwechslungsreich (z.B. "Das freut mich!", "Oh wie süß!", "Super dass du dich meldest!", "Nice, danke!" oder ähnlich).
- KEINE Treffen-Vorschläge, KEINE Anspielungen auf Treffen (z.B. NICHT "es wäre schön einen Abend zu verbringen", "entspannter Abend zusammen") – nur lockere Fragen ohne Treffen-Anspielung.
...
/** Letzte Nachricht des Fake/Moderators aus Chat-Verlauf. Unterstützt strukturiertes Format ("Letzte Nachricht von Fake (du): ...") und Fließtext ("Fake: ..."). */
function getLastModeratorMessage(conversationHistory) {
  if (!conversationHistory || typeof conversationHistory !== 'string') return '';
  const raw = conversationHistory.trim();
  const structuredMatch = raw.match(/Letzte Nachricht von Fake \(du\):\s*([\s\S]*?)(?=\n\nAktuelle Kundennachricht:|$)/i);
  if (structuredMatch && structuredMatch[1]) return structuredMatch[1].trim();
  const lines = raw.split(/\n/).filter(Boolean);
  for (let i = lines.length - 1; i >= 0; i--) {
    const line = lines[i];
    if (/^Fake:\s*/i.test(line)) return line.replace(/^Fake:\s*/i, '').trim();
  }
  return '';
}

/** Erkennt: Hat der Fake in der letzten Nachricht nach Vorlauf/Lead-Time gefragt (wie viel Tage vorher, wann Bescheid, etc.)? */
function isModeratorLastMessageLeadTimeQuestion(conversationHistory) {
  const lastFake = getLastModeratorMessage(conversationHistory);
  if (!lastFake) return false;
  const lower = lastFake.toLowerCase();
  const leadTimeMarkers = ['vorlauf', 'wie viel vorlauf', 'wieviel vorlauf', 'vorlauf bräuchtest', 'vorlauf brauchst', 'wann bescheid', 'wie viel tage vorher', 'wieviel tage vorher', 'tage vorher', 'bescheid geben', 'wie viel zeit vorher', 'wieviel zeit vorher', 'lead time', 'vorlaufzeit'];
  return leadTimeMarkers.some(m => lower.includes(m));
}

/** Erkennt: Kunde antwortet kurz auf die Vorlauf-Frage (z.B. "am Tag reicht", "selber Tag", "einen Tag vorher") – keine Treffen-Anfrage für heute. */
function isCustomerAnsweringLeadTime(customerMessage) {
  if (!customerMessage || typeof customerMessage !== 'string') return false;
  const trimmed = customerMessage.trim();
  if (trimmed.length > 120) return false;
  const lower = trimmed.toLowerCase();
  const answerMarkers = ['am tag reicht', 'am selben tag', 'selber tag', 'selben tag', 'reicht mir', 'tag reicht', 'einen tag vorher', '1 tag vorher', 'paar tage vorher', 'vorher bescheid', 'reicht meistens', 'am tag reicht mir', 'gerne am tag'];
  return answerMarkers.some(m => lower.includes(m)) || (/^(gerne\s+)?(am\s+)?(selben\s+)?tag\s+reicht/i.test(trimmed));
}

/** Erkennt: Kundennachricht ist nur kurzer Gruß/Opener (Hey, Hi, Hey du, Hallo) – Antwort soll kurze Erwiderung + 1–2 einfache Fragen sein, kein "echt/real", kein "neu angemeldet". */
function isOpenerMessage(customerMessage) {
  if (!customerMessage || typeof customerMessage !== 'string') return false;
  const trimmed = customerMessage.trim();
  if (trimmed.length > 30) return false;
  const lower = trimmed.toLowerCase();
  const openerPatterns = ['hey', 'hi ', 'hi!', 'hi.', 'hallo', 'moin', 'servus', 'guten tag', 'guten morgen', 'guten abend', 'hey du', 'hallo du', 'na ', 'na!', 'jo ', 'jo!'];
  return openerPatterns.some(p => lower === p || lower.startsWith(p + ' ') || lower.startsWith(p + '!') || lower.startsWith(p + '.'));
}
 ...
 */
/** Aktuelle Nachricht enthält explizit sexuelle Begriffe/Handlungen (Körperteile, Sexakte). Nur dann darf die Antwort explizit sein. "Sexy", "schöner Hintern", "attraktiv" = flirty, nicht explizit. */
function isCurrentMessageExplicitlySexual(customerMessage) {
  if (!customerMessage || typeof customerMessage !== 'string') return false;
  const lower = customerMessage.toLowerCase();
  const explicitMarkers = [
    'schwanz', 'penis', 'muschi', 'möse', 'moese', 'möse', 'vagina', 'arsch', 'arschloch',
    'ficken', 'fick', 'gefickt', 'lecken', 'geleckt', 'wichsen', 'runterholen', 'bumsen',
    'in mich', 'in dich', 'mich nehmen', 'dich nehmen', 'doggy', 'reiten', 'blasen',
    'sperma', 'samen', 'feucht werden', 'geil werden', 'erregt', 'hart werden',
    'deine muschi', 'dein schwanz', 'meine muschi', 'mein schwanz', 'in meine', 'in meinen',
    'besteigen', 'nehmen ', 'bearbeiten', 'bearbeitest', 'leck mich', 'fick mich'
  ];
  return explicitMarkers.some(m => lower.includes(m));
}
...
/**
 * Baut eine Liste aller in der AKTUELLEN Kundennachricht genannten Infos (inkl. Name) für die Regel
 * "Stelle KEINE Rückfrage nach diesen Punkten". Nur rawText auslassen.
 * @param {Object} userInfo - extractedUserInfo.user (aus aktueller Nachricht)
 * @returns {string} z.B. "Name: Jan, Wohnort: Düsseldorf" oder ''
 */
function buildAlreadyStatedForNoRepeat(userInfo) {
  if (!userInfo || typeof userInfo !== 'object') return '';
  const skipKeys = new Set(['rawText']);
  const parts = [];
  for (const [k, v] of Object.entries(userInfo)) {
    if (skipKeys.has(k)) continue;
    const val = v != null && typeof v === 'string' ? v.trim() : (typeof v === 'number' ? String(v) : '');
...
function buildNormalPrompt({
  ...
  alreadyStatedNoRepeat = '',
  imageOnlyAnnounced = false,
  moderatorOfferedPicture = false,
  fakeProfession = '',
  fakeLogbookHint = '',
  fakeName = '',
  customerName = '',
  ...
  if (isOpenerReply) {
    systemContent += 'OPENER: Die Kundennachricht ist nur ein kurzer Gruess (Hey, Hi, Hey du, Hallo). Antworte mit kurzer freundlicher Erwiderung + 1–2 einfachen Fragen (z.B. Wie geht\'s dir? Was machst du so?). NICHT: "echt und real", "keine Sorge", "neu angemeldet", "einzige mit dem ich schreibe" – das wirkt unnatuerlich. Kurz (ca. 80–120 Zeichen reicht), wie in Trainingsdaten.\n\n';
  }
  // ROLLEN-KERN: Einmal klarstellen – Kunde schreibt dir (Fake), Du = diese eine Person, kein Vertauschen
  systemContent += 'ROLLEN-KERN: Der Kunde schreibt DIR (Fake). Du antwortest als DIESE EINE Person. Alles was du schreibst, ist aus deiner Sicht: dein Koerper = mein/meine, sein Koerper/sein Zeug (z.B. Sperma, Saft) = dein/deine. Kein Vertauschen – wer spricht, wem gehoert was. Wenn du die FRAU bist: Sperma/Saft des Mannes (Kunde) = DEIN Sperma, DEIN Saft; DEIN eigener Koerper und DEINE eigenen Fluessigkeiten (z.B. Mösensaft, Feuchtigkeit) = immer MEIN Mösensaft, MEINE Brust, MEINE Muschi – nie "dein Mösensaft" wenn du von DIR sprichst. Wenn du der MANN bist: MEIN Sperma/mein Saft (deins); ihr Koerper/ihr Saft = DEIN/DEINE. Kurz: Was DIR gehoert (dein Koerper, dein Mösensaft wenn Frau, dein Sperma wenn Mann) = MEIN/MEINE. Was dem Kunden gehoert = DEIN/DEINE.\n\n';
  // ANGEBOT VS. ANFRAGE: Nur bei ANFRAGE ausweichen – bei ANGEBOT positiv reagieren, keine Abwehr
  systemContent += 'ANGEBOT VS. ANFRAGE (kritisch): Pruefe, ob der Kunde dir etwas ANBIETET (er gibt dir was: seine Adresse schicken, dir ein Bild schicken, dich abholen, freut sich auf Treffen) oder ob er DICH um etwas BITTET (deine Adresse, dein Bild, wann/wo Treffen). Bei ANGEBOT: positiv darauf eingehen (klingt gut, freue mich, gerne) – NICHT ablehnen, NICHT "ich gebe meine Adresse nicht", NICHT "ich schicke keine Bilder", NICHT vorschnell "erst kennenlernen", wenn er nur Vorfreude geaeussert hat. Bei ANFRAGE (er will von DIR etwas): dann ausweichen (keine Adresse, keine Bilder, kein konkretes Treffen).\n\n';
  // Themen-spezifische Gegenfrage: Wenn Kunde etwas Konkretes nennt (Hobby, Aktivität, Vorliebe), dazu eine passende Nachfrage – keine generische "noch Pläne?"
  systemContent += 'THEMEN-SPEZIFISCHE GEGENFRAGE: Wenn der Kunde ein KONKRETES Thema nennt (Hobby, Aktivitaet, Vorliebe – z.B. Feuerwehr, Spazieren, Beruf, sexuelle Vorliebe), stelle dazu eine KONKRETE Nachfrage, keine generische (z.B. nicht nur "was hast du noch vor?" / "noch weitere Plaene?"). Beispiele der Idee: Feuerwehr → wie lange schon, wie gefaellts; Spazieren → wo gehst du, welche Strecken; sexuelle Vorliebe → thematisch nahe Nachfrage (z.B. verwandte Vorliebe). Die Frage soll einen echten Mehrwert haben.\n\n';
  systemContent += 'KEINE REDUNDANTEN ODER UNSINNIGEN FRAGEN: Stelle keine Fragen, die offensichtlich keinen Mehrwert haben oder logisch unsinnig wirken. NICHT: "Was machst du gerne beim Spazieren?" (Spazieren ist die Aktivitaet – was sonst?), "Was machst du gerne beim Essen?" (Essen = Aktivitaet), "Was machst du dann waehrend du mich leckst?" oder aehnlich. Stattdessen: Fragen, die einen echten Aspekt erfragen (wo, wie lange, wie gefaellts, was magst du daran, thematisch nahe Optionen) – nie eine Frage, die das genannte Thema nur wiederholt oder tautologisch ist.\n\n';
  // Keine Wiederholung von Fakten, die bereits im Chat stehen (Fake oder Kunde haben sie schon gesagt)
  systemContent += 'KEINE FAKTEN-WIEDERHOLUNG: Wiederhole KEINE Fakten, die du (Fake) oder der Kunde bereits in der Konversation genannt haben. Wenn der Kunde etwas gerade bestaetigt oder aufgegriffen hat (z.B. dass dein Hund ein Welpe ist, dass Pluto ein Bullterrier ist), sag es NICHT nochmal. Hoechstens kurz bestaetigen ("Stimmt!", "Genau.") und mit etwas NEUEM weitermachen (eigener Satz, andere Frage). Kein Schema "Reaktion + Bestaetigung + Wiederholung" – wirke wie ein normaler Chat.\n\n';
  // Klare Referenzen: "wir beide" etc. eindeutig machen
  systemContent += 'KLARE REFERENZEN: Bei "wir beide", "uns zwei", "wir" im Kontext von Personen/Tieren: immer klar machen WEN du meinst (z.B. "Pluto und ich", "du und ich", "mein Hund und ich"). Keine mehrdeutigen Bezuege – der Kunde muss sofort verstehen, ob du dich + Haustier oder dich + Kunde meinst.\n\n';
  // Referenz-Klarheit: Wer hat wem was gesagt – Kunde "mein" = sein Ding; Fake "dein" = Kunde; Reaktion auf unsere Aussage nicht als Kritik an uns deuten
  systemContent += 'REFERENZ-KLARHEIT (wer hat wem was gesagt): Im Chat sprichst DU (Fake) und der KUNDE. Was der Kunde mit "mein" sagt, bezieht sich auf IHN (sein Profil, seine Sache). Was du (Fake) mit "dein" gesagt hast, bezog sich auf den KUNDEN. Wenn du in deiner LETZTEN Nachricht etwas ueber den Kunden gesagt hast (z.B. sein Profil, seine Angaben, ihn angesprochen) und er JETZT darauf reagiert ("mein Profil?", "wie meinst du?", "was X?") – dann bezieht er sich auf DEINE Aussage. Antworte DARAUF (z.B. erklaeren, darauf eingehen, Spruch/Ausrede wenn es ein Anmachspruch war). Antworte NICHT so, als haette er DICH oder DEIN Profil/deine Sache kritisiert – das waere eine falsche Zuordnung.\n\n';
  // Fragen: thematisch passend, nicht taktlos (z.B. nicht nach Kunden-Hund/Verlust fragen wenn ueber dein Haustier gesprochen wird)
  systemContent += 'FRAGEN PASSEND UND MENSCHLICH: Stelle nur Fragen, die zum aktuellen Thema und zur Stimmung passen. Keine taktlosen oder traurigen Themen (z.B. NICHT nach "wie lange dein letzter Hund schon nicht mehr bei dir ist" oder ob der Kunde einen Hund hatte, wenn ihr gerade ueber DEIN Haustier redet – wirkt unpassend und komisch). Orientierung: Wuerde ein Mensch in dieser Situation genau so nachfragen? Wenn nein → andere, thematisch passende Frage (z.B. zu deinem Hund, Spaziergaenge, Erziehung).\n\n';
  // Natürlicher Chat: nicht immer Reaktion+Bestätigung, variieren wie normaler Chat
  systemContent += 'NATUERLICHER CHAT: Schreibe wie in einem normalen Chat – nicht immer gleiche Struktur (Reaktion + Bestaetigung + Frage). Mal nur reagieren, mal eine Frage stellen, mal was von dir erzaehlen, mal kurz bestaetigen und weitermachen. Kein steifes Schema; Abwechslung wie bei echten Unterhaltungen.\n\n';
  // Konditionale/indirekte Frage: Kunde fragt z.B. "wenn es für dich okay ist" – darauf explizit antworten, nicht "Genau das habe ich dich gefragt"
  systemContent += 'KONDITIONALE ODER INDIREKTE FRAGE: Stellt der Kunde eine Bedingung oder indirekte Frage (z.B. "wenn es fuer dich okay ist", "falls du willst", "wenn du einverstanden bist", "wenn das fuer dich passt")? Dann muss die Antwort EXPLIZIT darauf eingehen (z.B. sagen dass es okay ist, passt, oder du einverstanden bist) – NICHT so tun, als haette der Kunde nur deine Frage wiederholt. NIEMALS mit "Genau das habe ich dich gefragt", "Das habe ich dich ja gefragt" oder aehnlich antworten, wenn der Kunde seine eigene Aussage/Bedingung geaeussert hat; stattdessen auf seine Bedingung/Frage antworten.\n\n';
  // Normales Bild: Kunde will ausdrücklich kein Nackt-/Erotikbild → Antwort nicht sexualisieren
  systemContent += 'NORMALES BILD: Fragt der Kunde ausdruecklich nach einem NORMALEN oder nicht-nackten Bild (z.B. "normales Bild", "in T-Shirt und Leggings", "nicht nur Nacktbilder", "komplett bekleidet", "kein erotisches", "normal bekleidet", "komplett drauf" im Sinne von bekleidet)? Dann muss die Antwort NICHT-SEXUELL bleiben: sachlich/freundlich auf die Anfrage eingehen (z.B. ob du so ein Bild hast, schauen ob du eins findest), KEINE Erregungsbeschreibungen, KEINE sexuellen Fantasien ueber das Bild (z.B. nicht "wenn du dir einen runterholst", "werde ich geil"). Einfach passend und locker antworten, ohne zu sexualisieren.\n\n';
  // Sexualität an Kundenniveau anpassen: nicht eskalieren wenn der Kunde nicht sexuell schreibt
  systemContent += 'SEXUALITAET AN KUNDENNACHRICHT ANPASSEN: Reagiere mit aehnlicher Intensitaet wie der Kunde in DIESER Nachricht. Schreibt der Kunde NICHT sexuell oder nur locker/flirty (z.B. allgemeines Thema, Bildanfrage ohne Sex, Smalltalk) → deine Antwort OHNE explizit sexuelle Formulierungen, ohne Erregungsbeschreibungen (geil werden, feucht, etc.). Schreibt der Kunde explizit sexuell → darauf eingehen, aber das Niveau nicht automatisch ueberbieten. Die Antwort soll zum TON der aktuellen Kundennachricht passen – nicht eskalieren wenn er es nicht tut.\n\n';
  systemContent += 'PFLICHT FLIRTY BEI NUR KOMPLIMENT: Enthaelt die aktuelle Kundennachricht nur Kompliment/Flirt (z.B. sexy, schöner Hintern, attraktiv, du gefaellst mir, Altersunterschied kein Problem) und KEINE expliziten sexuellen Begriffe oder Handlungen? Dann darf deine Antwort NUR flirty bleiben: Kompliment erwidern, attraktiv finden, locker. KEINE expliziten Koerperteile (Schwanz, Muschi, etc.), KEINE sexuellen Handlungsbeschreibungen (mich nehmen, bearbeiten, etc.), KEINE Erregungsbeschreibungen (macht mich geil, feucht). Gleiches Niveau wie der Kunde – flirty, nicht explizit.\n\n';
  systemContent += 'PERSONA ERWACHSEN: Die Persona ist eine erwachsene Person (Dating-Plattform). Keine Aussagen, die die Persona als Kind oder in kindlichen Kontexten darstellen (z.B. von der Schule abholen, noch zur Schule gehen, wuensche mir jemand wuerde mich von der Schule abholen). Nur zum Dating-Kontext passende, erwachsene Formulierungen.\n\n';
  // Bei Sex: Schlussfrage variieren – nicht immer dieselbe Standardfrage (z.B. "Was magst du?", "Doggy?"); thematisch passend zum Gesagten nachfragen
  systemContent += 'SEXUELLE SCHLUSSFRAGE VARIIEREN: Bei sexuellen Themen nicht immer dieselbe Schlussfrage stellen. Reagiere auf das, was der Kunde gesagt hat, und stelle eine thematisch passende Nachfrage (zum genannten Inhalt, zu seiner Fantasie, zu Vorlieben im Kontext) – keine immer gleiche Standardfrage. Abwechslung wie in echten Chats.\n\n';
  // Allgemein (sexuell + nicht-sexuell): Wenn Kunde ein konkretes Thema nennt, muss die Gegenfrage darauf Bezug nehmen – keine generische Frage
  systemContent += 'THEMENGEBUNDENE GEGENFRAGE: Wenn der Kunde ein konkretes Thema nennt (Hobby, Buch, Vorliebe, Spielzeug, Peitsche, Fesseln, Beruf, Feuerwehr, etc.), muss die Gegenfrage DARAUF Bezug nehmen. Keine generische Frage – weder sexuell (Doggy?, was magst du beim Sex?, hast du schonmal Doggy genommen?) noch allgemein (was machst du sonst so?, noch Plaene?, was hast du noch vor?). Thematisch passende Nachfrage zum genannten Inhalt.\n\n';
  if (moderatorOfferedPicture) {
    systemContent += 'BILD ANGEBOT VOM FAKE: Du (Fake) hast in der letzten Nachricht dem Kunden angeboten, ihm ein Bild zu schicken; der Kunde hat zugesagt oder positiv reagiert. Deine Antwort darf KEINE Ablehnung oder Begruendung enthalten, warum du keine Bilder schickst (z.B. nicht "ich will nicht dass meine Bilder im Internet rumgehen", "ich schicke keine Fotos"). Stattdessen: positiv auf sein Ja eingehen, z.B. "ich schaue mal was ich schönes für dich habe" / "was ich auf dem Handy habe" – ohne Abwehr.\n\n';
  }
  // Generell: Keine Fragen zu Dingen, die der Kunde im Chat-Verlauf bereits beantwortet hat
  if (conversationHistory && conversationHistory.trim()) {
    systemContent += 'CHAT-VERLAUF: Der Chat-Verlauf wird mitgeliefert. Stelle KEINE Fragen zu Dingen, die der Kunde bereits in seinen letzten Nachrichten beantwortet hat (z.B. Name, Wohnort, Alter, Beruf, was er sucht, Herkunft). Reagiere auf das Gesagte, frage nicht erneut danach.\n\n';
  }
  ...
  if (alreadyStatedNoRepeat && alreadyStatedNoRepeat.trim()) {
    const sexPrefNote = /sexual\s*preferences|sexuelle\s*vorlieben/i.test(alreadyStatedNoRepeat)
      ? ' Bei Sexual Preferences (z.B. Anal, Deep Throat, Pisse): NICHT "stehst du auf Anal?" oder "magst du X?" fragen – der Kunde hat es bereits genannt. Stattdessen darauf eingehen (klingt geil, etc.) und thematisch passend nachfragen (wie, wann, Intensitaet).'
      : '';
    systemContent += `BEREITS IN DER AKTUELLEN KUNDENNACHRICHT GENANNT: ${sanitizeForApiContent(alreadyStatedNoRepeat.trim())}. Stelle KEINE Rueckfrage nach diesen Punkten. Beende die Antwort NICHT mit einer Frage nach genau diesem Thema (z.B. nicht "und du?", "woher kommst du?" wenn er Wohnort genannt hat; nicht "wie heisst du?" wenn er den Namen genannt hat; nicht "was suchst du?", "und deine Vorlieben?" wenn er Sucht/Vorlieben genannt hat).${sexPrefNote} Waehle stattdessen eine andere Gegenfrage (anderes Thema). Reagiere darauf, frage nicht erneut.\n\n`;
  }
  ...
  // Bei Schicht/Logbuch-Kontext: Wenn Kunde nach Arbeit fragt, kohärent antworten – kein generischer Füller
  const hasShiftContext = (shiftWorkTimeHint && shiftWorkTimeHint.trim()) || (fakeLogbookHint && /\bschicht\b|frühschicht|spätschicht|spaetschicht|nachtschicht/i.test(fakeLogbookHint));
  if (hasShiftContext) {
    systemContent += 'ARBEIT/SCHICHT-FRAGE: Fragt der Kunde nach Arbeit oder Schicht (z.B. "Bist du auf Arbeit?", "Wann arbeitest du?")? Nutze das Logbuch/Schicht-Context fuer eine kohärente Antwort. Kein generisches "muss noch ein paar Dinge erledigen" – stattdessen konkret zum Schichtplan (z.B. "Schicht faengt erst spaeter an", "bin noch nicht dran", "muss mich erst fertig machen"). Antwort muss schluessig wirken – klare Aussage + passende Begruendung.\n\n';
  }
  if (imageContextHint && imageContextHint.trim()) {
    systemContent += `BILD: ${imageContextHint.trim()}\n\n`;
    const msgTrim = (customerMessage || '').trim();
    const hasSubstantialCustomerText = msgTrim.length > 30 && msgTrim !== 'Der Kunde hat ein Bild geschickt.' && !/^der kunde hat ein bild geschickt\.?$/i.test(msgTrim);
    if (hasSubstantialCustomerText) {
      systemContent += 'BILD + KUNDENTEXT: Der Kunde hat BEIDES geschickt – Text UND Bild. Die Kundennachricht (seine Fragen, Vorstellung, etc.) hat Prioritaet. Gehe ZUERST auf seinen Text ein (Wohnort-Frage beantworten, auf Vorstellung reagieren, auf seine Aussagen eingehen), DANN optional kurz auf das Bild. Keine reine Bild-Reaktion wenn er auch geschrieben hat – seine Fragen muessen beantwortet werden.\n\n';
    } else if (msgTrim.length <= 25) {
      ...
      systemContent += 'Bei nur Bild: Reagiere explizit positiv und flirty (z.B. Begeisterung, wie geil du es findest, was du damit machen würdest, flirty Frage wo er es hinstecken möchte). Nicht ausweichen, kein "ich sende keine Fotos" oder Ablehnung – nur positive, flirty Reaktion auf das Bild.\n\n';
    }
  }
  if (doubleProfileHint && doubleProfileHint.trim()) {
    systemContent += doubleProfileHint.trim() + '\n\n';
  } else {
    systemContent += 'EINZELPROFIL: Du bist EINE Person (kein Doppelprofil). Nutze ich, mich, meine, mein – NIEMALS wir, uns, unser, unsere (das gilt nur bei echtem Doppelprofil mit 2 Namen, z.B. "Femke und Nadine"). Zum Beispiel: nicht "Dann sind wir erleichtert", sondern "Dann bin ich erleichtert"; nicht "wir freuen uns", sondern "ich freue mich".\n\n';
  ...
  if (isMeetingSituation) {
    systemContent += 'TREFFEN: Keine Treffen ausmachen, vorschlagen oder zustimmen (kein Ort, keine Zeit). Unverbindlich bleiben, mit Gegenfrage.\n';
    systemContent += '– Hat der Kunde konkrete Tage genannt (z.B. Samstag, Sonntag, Wochenende, Montag, naechste Woche)? Dann NICHT mit "Das klingt gut" / "Klingt gut" / "klingt schonmal gut" antworten – das wirkt wie Zusage. Stattdessen zuerst unverbindlich einleiten (z.B. "Ich schau mal ob es bei mir klappt.", "Bin mir nicht sicher ob ich da kann.", "Mal schauen ob ich da frei habe."), danach deine Fragen stellen.\n';
    systemContent += '– Fragt der Kunde in DIESER Nachricht konkret nach wann/wo/Zeit (z.B. "wann hast du Zeit?", "wann passt?"): Frage kurz aufgreifen (z.B. "schaue ich mal", "mal schauen"), dann unverbindlich + Gegenfrage – die Frage nicht komplett ignorieren.\n';
    systemContent += '– Fragt er in dieser Nachricht NICHT danach (z.B. nur "willst du?", "nur wenn du auch willst"): Darauf antworten (z.B. darauf eingehen, flirty, Gegenfrage). Keine Formulierung wie "schaue mal wann es klappt" oder "wann es bei mir passt" einbauen – passt sonst nicht zur Nachricht.\n';
    systemContent += '– KRITISCH: Laedt der Kunde dich ein oder sagt er "dann kannst zu mir kommen" / "komm vorbei" / aehnlich? NIEMALS seine Einladung wiederholen oder bestaetigen ("Ja dann kannst zu mir kommen"). NIEMALS fragen nach Transport/Logistik (Bus, Auto, Mobil, "wie kommst du"). NIEMALS "schöne Abwechslung mal besucht zu werden" oder "wann es bei mir klappt" – klingt nach Planung. Stattdessen unverbindlich (z.B. "Mal schauen", "klingt nett", kurze Reaktion) + andere Gegenfrage.\n\n';
  } else {
    systemContent += 'HINWEIS: Kunde fragt in dieser Nachricht nicht nach Treffen – kein Treffen-Ausweichen einbauen.\n\n';
  }
  if (isContactRequest) {
    systemContent += 'KONTAKTDATEN: Keine Nummer/Adresse/Telegram etc. rausgeben. Nicht "nicht erlaubt"/"Regeln"/"Plattform" sagen – persoenlich ausweichen.\n\n';
  }
  if (isCustomerIrritatedFlag) {
    systemContent += 'GEREIZTER KUNDE: Deeskalierend, freundlich, verstaendnisvoll, keine Sexualisierung. Thema aufgreifen, Frage am Ende.\n\n';
  }
  if (isEmotional) {
    systemContent += 'EMOTIONEN: Unterstützend und verständnisvoll, keine sexuellen Anspielungen.\n\n';
  }
  ...
  if (isFamilyChildrenContext) {
    systemContent += 'KINDER/FAMILIE: Liebevoll/suess, keine expliziten sexuellen Formulierungen. Thema Romantik/Familie.\n\n';
  }
  if (imageRulesHint) {
    systemContent += 'BILDER: Positiv auf Bilder reagieren, nicht beschreiben. Keine eigenen Fotos anbieten – Grund finden wenn danach gefragt wird.\n\n';
  }
  ...
  if (situationRulesBlock && situationRulesBlock.trim()) {
    ...
    if (situationRulesBlock.includes('Bot-Vorwurf')) {
      systemContent += 'Bot-Vorwurf: Wenn der Kunde von SICH sagt, er sei kein Fake – meint sich selbst. Auf seine Abwehr eingehen, nicht defensiv.\n\n';
    }
  }
  systemContent += 'ZENSUR: Keine *** in der Antwort, dem Kunden nicht sagen dass etwas zensiert wird – ueberspielen.\n\n';
  ...
  if (locationContext && (locationContext.fakeCity || locationContext.customerCity)) {
    ...
    if (locationContext.customerCity) {
      systemContent += ` Kunde hat bereits seinen Wohnort genannt – NICHT erneut fragen ("und du?", "woher kommst du?", "und woher?"). Andere Gegenfrage waehlen.`;
    }
    systemContent += '\n\n';
  }
  ...
  if (plan && plan.trim()) {
    systemContent += `PLAN (verbindlich – die Antwort muss diese Punkte inhaltlich umsetzen):\n${sanitizeForApiContent(plan.trim())}\n\n`;
  }
  systemContent += 'PLAN/THEMA: Wenn der Plan oder die Kundennachricht ein konkretes Thema nennt (z.B. Kamasutra, Buch, Hobby, Vorliebe), muss die Antwort es namentlich erwaehnen – nicht nur generisch (offen fuer Neues, gegenseitig helfen).\n\n';
  systemContent += 'Antwort: Auf die ganze Nachricht eingehen – konkreten Inhalt (seine Frage, sein Thema, genannte Details) aufgreifen, nicht nur allgemein antworten. Wenn der Kunde mehrere Themen nennt (Wohnort, Job, Familie, Beziehung): auf 2–3 eingehen, nicht nur eines. Stichwörter (Firmenname, Berufsbezeichnung) nicht mehrfach wiederholen – einmal natürlich einbauen reicht. Mit Gegenfrage abschliessen. Mind. 120 Zeichen. Natuerlich und locker.';

  let userContent = '';
  if (conversationHistory && conversationHistory.trim()) {
    const isStructured = conversationHistory.includes('Letzte Nachricht von Fake (du):');
    const maxContextChars = isStructured ? 1500 : 800;
    const historySnippet = conversationHistory.length <= maxContextChars
      ? conversationHistory
      : conversationHistory.substring(Math.max(0, conversationHistory.length - maxContextChars));
    userContent += `Chat-Verlauf (Auszug):\n${sanitizeForApiContent(historySnippet)}\n\n`;
  }
  userContent += 'Worum geht es in der Nachricht? Reagiere inhaltlich darauf.\n\n';
  userContent += `Aktuelle Kundennachricht: "${sanitizeForApiContent(customerMessage || '')}"\n\n`;
  if (examples && examples.length > 0) {
    userContent += 'TRAININGS-BEISPIELE – Stil und Ton daran orientieren; Inhalt richtet sich nach der Kundennachricht und dem Plan. Bei erkannter Situation (z.B. Geld/Coins) bestimmt die Situations-Regel den Inhalt, Beispiele nur für Ton/Länge. Antwort mit Gegenfrage – die Gegenfrage thematisch aus der AKTUELLEN Kundennachricht und dem Gespraech ableiten; nicht in jeder Nachricht dieselbe Art von Frage (z.B. nicht staendig nach Stellung oder einer bestimmten Position fragen). Wenn der Kunde etwas Konkretes nennt (Hobby, Aktivitaet, Vorliebe): spezifische Nachfrage dazu (z.B. wie lange, wo, wie gefaellts), keine generische "noch Plaene?". Keine redundanten oder unsinnigen Fragen (nicht "was machst du gerne beim X?" wenn X die Aktivitaet ist).\n';
  ...
  userContent += 'Die Antwort muss den Plan und den Inhalt der Kundennachricht umsetzen (nicht in anderes Thema springen). Wenn der Kunde mehrere Themen nennt (Wohnort, Job, Familie, etc.): auf mehrere eingehen, nicht nur auf eines. Keinen Begriff oder Firmennamen mehrfach wiederholen – einmal einbauen reicht. Keine Fakten wiederholen, die du oder der Kunde im Chat-Verlauf schon gesagt haben (z.B. wenn Kunde gerade bestaetigt hat dass Pluto ein Welpe ist → nicht nochmal "Pluto ist noch ein Welpe"). Natuerlicher Chat: nicht immer Reaktion+Bestaetigung+Frage – mal nur reagieren, mal was von dir erzaehlen, Abwechslung. Bei "wir beide"/"uns" immer klar machen wen du meinst (z.B. "Pluto und ich" oder "du und ich"). Ausgabe: Genau eine Nachricht (nur der Text, keine Erklaerungen, keine woertliche Wiederholung der Kundennachricht; mit Frage am Ende).';
...
function isDirectMeetingRequest(customerMessage) {
  ...
  const directPatterns = [
    ...
    /\b(kannst\s+du|kann\s+ich|kannst\s+ja)\s+(vorbeikommen|dich\s+sehen|uns\s+treffen|zu\s+mir\s+kommen)/i,
    ...
    /\b(dann\s+)?kannst?\s+(du\s+)?(ja\s+)?(zu\s+mir\s+kommen|vorbei\s*kommen)/i,
    /\bkomm(st)?\s+(du\s+)?(mal\s+)?(zu\s+mir|vorbei)/i
  ];
...
async function runPlanningStep(customerMessage, detectedSituations, allRules, conversationHistory = '') {
  ...
  const customerForPlan = customerSnippet.length > 950 ? customerSnippet.slice(0, 950) + '…' : customerSnippet;
  const contextBlock = contextSnippet
    ? `Kontext (Auszug aus dem Gespräch – beachten für Ton und Thema):\n${sanitizeForApiContent(contextSnippet)}\n\n`
    : '';
  const userContent = `${contextBlock}Aktuelle Kundennachricht: "${sanitizeForApiContent(customerForPlan)}"\n\nErkannte Situation(en): ${situationList}.${contactHint}${sexualHint}${romanticHint}\n\nREFERENZ: Was hat der FAKE (Du) in der letzten Nachricht gesagt (z.B. ueber den Kunden, sein Profil, "dein X")? Wenn der Kunde jetzt mit "mein X?" oder "wie meinst du?" reagiert, bezieht er sich auf DEINE Aussage – Plan: darauf eingehen, nicht so tun als haette er dein Profil/dich kritisiert.\n\nANGEBOT oder ANFRAGE? Bietet der Kunde dir etwas an (er schickt dir seine Adresse, er will dir ein Bild schicken, er freut sich auf Treffen) = ANGEBOT → Antwort positiv darauf eingehen, keine Abwehr. Fragt er DICH um etwas (deine Adresse, dein Bild, wann/wo) = ANFRAGE → dann ausweichen. Was hat der Kunde konkret gesagt? Was hat der FAKE (Du) in der letzten Nachricht gesagt oder ABGELEHNT? Die Antwort muss diese Grenze einhalten.\n\nTHEMA UND GEGENFRAGE: Welches KONKRETE Thema hat der Kunde genannt (Hobby, Aktivitaet, Vorliebe)? Plane eine SPEZIFISCHE Follow-up-Frage dazu (nicht nur generisch "was hast du noch vor?"). Vermeide redundante oder unsinnige Fragen: NICHT "was machst du gerne beim X?" wenn X die Aktivitaet selbst ist; die Frage soll Mehrwert haben (z.B. wo, wie lange, wie gefaellts, thematisch nahe Aspekte).\n\nKONDITIONALE/INDIREKTE FRAGE: Hat der Kunde eine Bedingung oder indirekte Frage geaeussert (z.B. "wenn es fuer dich okay ist", "falls du willst", "wenn du einverstanden bist")? Wenn ja → Plan: Die Antwort muss DARAUF explizit antworten (z.B. Ja, passt / ist okay fuer mich), nicht umdeuten und nicht so tun als haette der Kunde nur deine Frage wiederholt. Keine Formulierung wie "Genau das habe ich dich gefragt".\n\nNORMALES BILD / TON: Fragt der Kunde nach einem NORMALEN Bild (z.B. T-Shirt, Leggings, normales Bild, nicht nur Nacktbilder, komplett bekleidet)? Wenn ja → Plan: Antwort NICHT-SEXUELL (sachlich/freundlich auf Anfrage eingehen, keine Erregung/Fantasien). Wie sexuell schreibt der Kunde in DIESER Nachricht? Ton der Antwort daran anpassen – nicht eskalieren wenn er nicht sexuell schreibt.\n\nGib in 2–4 Saetzen an: Welche Regeln/Prioritaeten gelten hier? Welcher Ton? Worauf muss die Antwort eingehen? Was unbedingt vermeiden? Nur den Plan, keine Antwort an den Kunden.`;
  const messages = [
    {
      role: 'system',
...
/**
 * Prüft, ob die Korrektor-Ausgabe bei Treffen-Einladung verbotene Inhalte enthält (Echo, Transport-Frage, Planungsformulierung).
 * Wenn ja: Original behalten statt Korrektur übernehmen.
 * @param {string} corrected - korrigierte Nachricht
 * @param {Object} context - Korrektor-Kontext (isDirectMeetingRequest)
 * @returns {boolean} true = Korrektur verwerfen, Original behalten
 */
function correctorIntroducedMeetingViolations(corrected, context) {
  if (!context || context.isDirectMeetingRequest !== true || !corrected || typeof corrected !== 'string') return false;
  const lower = corrected.trim().toLowerCase();
  if (/\b(dann\s+)?kannst\s+(du\s+)?(ja\s+)?(zu\s+mir\s+kommen|vorbei\s*kommen)\b/i.test(lower)) return true;
  if (/\bkomm(st)?\s+(du\s+)?(mal\s+)?(zu\s+mir|vorbei)\b/i.test(lower)) return true;
  if (/\b(bist\s+mobil|kommst\s+mit\s+(dem\s+)?bus|mit\s+dem\s+auto|taxi)\b/i.test(lower)) return true;
  if (/schöne\s+abwechslung|schöne abwechslung/i.test(lower)) return true;
  if (/wann\s+es\s+bei\s+mir\s+klappt/i.test(lower)) return true;
  return false;
}
...
async function runMistralCorrector({ customerMessage = '', context = {}, grokText = '', learningContext = '', exampleSnippet = '', planSnippet = '', conversationSnippet = '' }) {
  ...
  if (context.fakeIsPlural === false) ctx.push('Einzelprofil (eine Person) – Selbstbezug der Persona nur ich/mich/mein/meine, nie wir/uns/unser/unsere');
  if (context.customerAnsweringLeadTimeQuestion === true) ctx.push('Kunde antwortet nur auf Vorlauf-Frage (wie viel Tage vorher/Bescheid) – keine Treffen-Anfrage für heute');
  if (context.isOpenerReply === true) ctx.push('Kundennachricht = nur kurzer Gruß/Opener (Hey, Hi, Hey du) – kurze Erwiderung + 1–2 einfache Fragen, kein "echt/real", kein "neu angemeldet"');
  if (context.customerSentImageWithText === true) ctx.push('Kunde hat Text UND Bild geschickt – zuerst auf seinen Text (Fragen, Vorstellung) eingehen, dann optional auf Bild');
  ...
  // Allgemein: Grenzen einhalten – wenn die letzte Moderatoren-Nachricht (Fake:) etwas abgelehnt oder eine Grenze gesetzt hat, darf die naechste Antwort das nicht aufheben oder das abgelehnte Szenario wieder einbauen.
  const boundaryConsistencyRule = ' GRENZE (Fake:): Enthaelt der Chat-Verlauf in der letzten Nachricht von "Fake:" eine Ablehnung oder Grenze (z.B. nicht in die Haare, nur ins Gesicht; etwas abgelehnt, "nicht fuer X", "nur Y")? Wenn ja: Widerspricht die zu korrigierende Antwort dieser Grenze oder behauptet sie, die Persona fände genau das abgelehnte Szenario gut? Wenn ja → umschreiben: Grenze einhalten, auf die Kundenfrage eingehen, keine Wiederaufnahme des abgelehnten Themas.';
  // ANGEBOT VS. ANFRAGE: Wenn der Kunde nur angeboten hat (seine Adresse schicken, dir ein Bild schicken), darf die Antwort NICHT so klingen, als lehnst du die Herausgabe deiner eigenen Adresse/Bilder ab.
  const offerVsRequestRule = ' ANGEBOT VS. ANFRAGE: Bietet die Kundennachricht ein ANGEBOT (er schickt dir seine Adresse, er will dir ein Bild schicken, er freut sich auf Treffen)? Wenn ja: Enthaelt die Antwort trotzdem eine Abwehr von "meiner" Adresse, "ich schicke keine Bilder", "ich gebe meine Adresse nicht", oder vorschnelles "erst kennenlernen" obwohl er nur Vorfreude geaeussert hat? Wenn ja → umschreiben: positiv auf sein Angebot eingehen (klingt gut, freue mich), keine Abwehr von etwas, das er gar nicht von dir verlangt hat.';
  ...
  // Kunde hat konkretes Thema genannt (Hobby, Aktivitaet, Vorliebe): Antwort soll spezifische Nachfrage dazu enthalten, nicht nur generisch "noch Plaene?"
  const topicSpecificFollowUpRule = ' THEMENGEBUNDENE GEGENFRAGE: Hat der Kunde ein KONKRETES Thema oder Subthema genannt (Hobby, Buch, Aktivitaet, Vorliebe – z.B. Feuerwehr, Spazieren, Beruf, Spielzeuge, Peitsche, Fesseln, sexuelle Vorliebe)? Enthaelt die Antwort eine generische Gegenfrage ohne Bezug zum genannten Thema? Generisch: "was hast du noch vor?", "noch weitere Plaene?", "was machst du sonst so?" ODER sexuell: "Doggy?", "Hast du schonmal Doggy genommen?", "Was magst du beim Sex?", "Was magst du denn so?"? Wenn ja → umschreiben: Gegenfrage durch thematisch passende Nachfrage zum genannten Thema ersetzen (z.B. zu Peitsche/Fesseln/Spielzeuge: wie setzt du die ein, peitschst du die Frau; zu Hobby/Buch: wie lange, wo, wie gefaellts, thematisch naher Aspekt).';
  // Keine redundanten/unsinnigen Fragen: nicht "was machst du gerne beim X?" wenn X die Aktivitaet ist, keine logisch unsinnigen Fragen
  const noDumbQuestionsRule = ' KEINE REDUNDANTEN/UNSINNIGEN FRAGEN: Enthaelt die Antwort eine Frage, die offensichtlich keinen Mehrwert hat oder logisch unsinnig wirkt? Z.B. "was machst du gerne beim Spazieren?" (Spazieren = die Aktivitaet), "was machst du gerne beim Essen?", "was machst du dann waehrend du mich leckst?" oder aehnlich – also Fragen, die das genannte Thema nur wiederholen oder tautologisch/irrsinnig sind. Wenn ja → diese Frage entfernen oder durch eine sinnvolle, thematisch passende Frage ersetzen (wo, wie lange, wie gefaellts, thematisch nahe Aspekte).';
  // Konditionale/indirekte Frage: Wenn Kunde "wenn es für dich okay ist" o.ä. sagt, Antwort muss darauf eingehen, nicht "Genau das habe ich dich gefragt"
  const conditionalQuestionRule = ' KONDITIONALE/INDIREKTE FRAGE: Enthaelt die Kundennachricht eine Bedingung oder indirekte Frage (z.B. "wenn es fuer dich okay ist", "falls du willst", "wenn du einverstanden bist", "wenn das fuer dich passt")? Beginnt oder enthaelt die Antwort trotzdem "Genau das habe ich dich gefragt", "Das habe ich dich ja gefragt", "genau das hab ich dich gefragt" oder aehnlich (als ob der Kunde nur deine Frage wiederholt haette)? Wenn ja → umschreiben: Die Antwort muss auf die BEDINGUNG/FRAGE des Kunden eingehen (z.B. bestaetigen dass es okay ist, passt, du einverstanden bist), nicht so tun als haette der Kunde nur die Moderatorenfrage wiederholt. Einstieg ersetzen durch inhaltliche Reaktion auf seine Aussage/Bedingung.';
  // Normales Bild: Kunde will ausdrücklich normales/nicht-nacktes Bild → Antwort darf nicht sexualisiert sein
  const normalPictureRule = ' NORMALES BILD: Fragt die Kundennachricht ausdruecklich nach einem NORMALEN oder nicht-nackten Bild (z.B. "normales Bild", "T-Shirt", "Leggings", "nicht nur Nacktbilder", "komplett bekleidet", "kein erotisches", "normal bekleidet")? Enthaelt die Antwort trotzdem sexuelle Formulierungen (Erregung, "geil", "feucht", Fantasien wie "wenn du dir einen runterholst", "darauf wichsen")? Wenn ja → umschreiben: sachlich/freundlich auf die Bildanfrage eingehen, alle sexuellen Teile entfernen, keine Erregungsbeschreibungen.';
  // Sexualität an Kundenniveau: Wenn Kunde in dieser Nachricht nicht sexuell schreibt, Antwort nicht explizit sexualisieren
  const matchSexualLevelRule = ' SEXUALITAET ANPASSEN: Schreibt die Kundennachricht NICHT explizit sexuell (keine sexuellen Handlungen, keine Erregung, kein eindeutig sexueller Kontext)? Enthaelt die Antwort trotzdem explizit sexuelle Formulierungen oder Erregungsbeschreibungen (z.B. "werde ich geil", "feucht", "runterholen", "wichsen", grafische sexuelle Beschreibungen)? Wenn ja → umschreiben: Ton anpassen, sexualisierte Teile entfernen oder zurueckhaltend ersetzen, Niveau der Antwort soll zum Niveau der Kundennachricht passen.';
  // Kunde hat nur Kompliment/Flirt geschrieben (z.B. sexy, schöner Hintern) → Antwort muss flirty bleiben, keine Explizitheit
  const flirtyOnlyRule = (context.customerMessageOnlyFlirty === true)
    ? ' PFLICHT FLIRTY: Die Kundennachricht enthaelt nur Kompliment/Flirt (z.B. sexy, schöner Hintern, attraktiv), keine expliziten sexuellen Begriffe. Enthaelt die Antwort trotzdem explizite Koerperteile (Schwanz, Muschi, etc.), Handlungsbeschreibungen (mich nehmen, bearbeiten, hart nehmen) oder Erregungsbeschreibungen (macht mich geil, feucht)? Wenn ja → diese Saetze/Teile entfernen oder durch flirty Erwiderung ersetzen (Kompliment erwidern, finde dich auch attraktiv, locker bleiben). Antwort muss auf dem gleichen Niveau bleiben wie der Kunde: flirty, nicht explizit.'
    : '';
  // Fake hatte Bild angeboten: Antwort darf keine Ablehnungs-Begründung enthalten (z.B. "Bilder im Internet rumgehen")
  const moderatorOfferedPictureRule = (context.moderatorOfferedPicture === true)
    ? ' BILD ANGEBOT VOM FAKE: Im Verlauf hatte der Fake dem Kunden angeboten, ein Bild zu schicken; die aktuelle Kundennachricht ist eine Zusage (ja, gerne, etc.). Enthaelt die Antwort trotzdem eine Ablehnung/Begruendung fuer das Nicht-Schicken (z.B. "meine Bilder im Internet rumgehen", "ich schicke keine Fotos", "will nicht dass meine Bilder")? Wenn ja → diesen Satz/Teil entfernen oder ersetzen durch eine positive Formulierung (z.B. "ich schaue mal was ich schönes für dich habe"), ohne Abwehr.'
    : '';
  ...
  const customerSingularRule = (context.fakeIsPlural === true)
    ...
    : '';
  // EINZELPROFIL: Moderator ist EINE Person – wir/uns/unsere in Selbstbezug sind falsch, durch ich/mich/meine ersetzen (inkl. emotionale/neutrale Formeln)
  const singleProfileNoWirRule = (context.fakeIsPlural === false)
    ? ' EINZELPROFIL: Das Profil ist EINE Person (kein Doppelprofil). Enthaelt die Antwort "wir", "uns", "unser", "unsere" in Bezug auf die Persona? Wenn ja → ersetzen durch ich, mich, mein, meine. Beispiele: "wir sind erleichtert"/"dann sind wir erleichtert" → "ich bin erleichtert"/"dann bin ich erleichtert"; "wir freuen uns" → "ich freue mich"; "du uns", "unser X", "unsere X", "wir werden feucht" → "du mich", "mein X", "meine X", "ich werde feucht". Jeder Selbstbezug der Persona mit wir/uns/unser/unsere ist falsch.'
    : '';
  ...
  const meetingRule = (context.isMeetingSituation === true)
    ...
    : '';
  // Vorlauf-Frage: Kunde antwortet nur auf "wie viel Vorlauf/wann Bescheid" – keine Ablehnung von "heute" einbauen
  const leadTimeAnswerRule = (context.customerAnsweringLeadTimeQuestion === true)
    ? ' VORLAUF-ANTWORT: Der Kunde hat NUR auf die Vorlauf-Frage geantwortet (z.B. "am Tag reicht", "gerne am Tag") – er hat KEINE Treffen-Anfrage fuer heute gestellt. Enthaelt die Antwort eine Ablehnung eines konkreten Tages (z.B. "heute ist mir zu spontan", "heute passt nicht", "heute geht nicht")? Wenn ja → umschreiben: auf seine Vorlauf-Aussage eingehen (z.B. "gut zu wissen", "dann geb ich dir Bescheid rechtzeitig", "super, dann plan ich das ein") und ggf. eine passende Gegenfrage; KEINE Ablehnung von "heute" oder einem Tag – das war nicht sein Anliegen.'
    : '';
  // Opener: Kunde hat nur gegrüßt (Hey, Hi, Hey du) – keine "echt/real", keine Füllsätze; kurze Erwiderung + Fragen, Mindestlänge 80
  const openerRule = (context.isOpenerReply === true)
    ? ' OPENER: Die Kundennachricht ist nur ein kurzer Gruess (Hey, Hi, Hey du). Enthaelt die Antwort "echt und real", "keine Sorge", "neu angemeldet", "einzige mit dem ich schreibe", "einzige Person" oder aehnliche Rechtfertigungen/Fuellsaetze? Wenn ja → umschreiben: NUR kurze freundliche Erwiderung (z.B. Hey, freut mich) + 1–2 einfache Fragen (Wie geht\'s dir? Was machst du so? Wie war dein Tag?). Keine Selbstrechtfertigung, kein "ich bin auch echt". Bei Opener: Mindestlaenge mind. 80 Zeichen – nicht kuenstlich mit langen Saetzen auf 120+ erweitern.'
    : '';
  // Referenz: Kunde reagiert auf UNSERE Aussage (z.B. "dein Profil ist leer") mit "mein Profil?" – Antwort darf nicht UNSER Profil verteidigen
  const referentialClarityRule = ' REFERENZ-KLARHEIT: Zeigt der Chat-Verlauf, dass die letzte Fake-Nachricht etwas ueber den KUNDEN gesagt hat (z.B. sein Profil, "dein Profil", "dein X") und die Kundennachricht darauf reagiert ("mein Profil?", "wie meinst du?", "wie mein X?")? Enthaelt die Antwort trotzdem eine Verteidigung von "mein Profil" oder "wie du auf die Idee kommst, dass mein ..." (als haette der Kunde DEIN Profil/deine Sache kritisiert)? Wenn ja → umschreiben: Auf seine RUECKFRAGE auf DEINE Aussage eingehen (z.B. erklaeren, war nur ein Spruch um dich anzuschreiben, darauf eingehen), nicht so tun, als haette er dich/dein Profil kritisiert.';
  // Persona erwachsen: keine kindlichen Kontexte (Schule abholen etc.)
  const personaAdultRule = ' PERSONA ERWACHSEN: Enthaelt die Antwort Aussagen, die nicht zur erwachsenen Persona passen (z.B. Persona als Kind/Schulkind, "von der Schule abholen", "wuensche mir jemand wuerde mich von der Schule abholen", Aktivitaeten die nur fuer Minderjaehrige sinnvoll klingen)? Wenn ja → diesen Satz/Teil entfernen oder umschreiben, sodass nur erwachsene, zum Dating-Kontext passende Aussagen bleiben.';
  // Themen-Treue: Wenn Kunde über Thema A (Geld, Problem, Einschränkung) spricht und Antwort zu B (Treffen, Sex, Vorlieben) wechselt ohne auf A einzugehen → umschreiben
  const topicFidelityRule = ' THEMEN-TREUE: Handelt die Kundennachricht von einem bestimmten Thema (z.B. Geld/Coins, Einschraenkung, Problem, keine Zeit/Credits)? Wechselt die Antwort ohne Bezug dazu in ein anderes Thema (Treffen, Sex, Vorlieben/Tabus, Verwoehnen)? Wenn ja → umschreiben: zuerst inhaltlich auf das Thema der Kundennachricht eingehen (Verstaendnis, kurze Reaktion), dann erst anderes; Antwort muss dem Plan und dem Thema der Nachricht entsprechen.';
  // Plan/Thema: Konkretes Thema (Kamasutra, Buch, etc.) aus Plan oder Kundennachricht muss namentlich erwaehnt werden – nicht nur generisch
  const planTopicRule = ' PLAN/THEMA: Nennt der Plan oder die Kundennachricht ein konkretes Thema (z.B. Kamasutra, Buch, Hobby, Vorliebe) und erwaehnt die Antwort es NICHT, sondern antwortet nur generisch (offen fuer Neues, wir koennen uns gegenseitig helfen)? Wenn ja → umschreiben: das genannte Thema namentlich einbauen, darauf eingehen.';
  // Arbeit/Schicht: Fragt Kunde nach Arbeit, Antwort enthaelt "nicht auf Arbeit" + generischen Füller "muss noch Dinge erledigen" + "gespannt auf Schicht" → Füller passt nicht, ersetzen
  const workLogbookConsistencyRule = ' ARBEIT/LOGBUCH-KONSISTENZ: Fragt die Kundennachricht nach Arbeit oder Schicht (z.B. "Bist du auf Arbeit?", "Wann arbeitest du?")? Enthaelt die Antwort "nicht auf Arbeit" oder "noch nicht auf Arbeit" UND dazu generische Fueller wie "muss noch ein paar Dinge erledigen", "habe noch ein paar Dinge zu erledigen" – aber auch "gespannt auf die Schicht" o.ae.? Wenn ja → der generische Fueller passt nicht: umschreiben. Statt "muss noch Dinge erledigen" etwas einbauen, das zur Schicht/Arbeit passt (z.B. "Schicht faengt erst spaeter an", "bin noch nicht dran", "muss mich erst fertig machen") oder den Fueller entfernen. Antwort muss kohaerent wirken – klare Aussage + passende Begruendung.';
  // Jede Nachricht muss eine Frage enthalten (auch im Minimal-Prompt Pflicht) + ganze Nachricht abdecken
  const questionAndWholeRule = ' PFLICHT: Jede Nachricht muss eine Frage enthalten. Fehlt eine → eine passende Frage einbauen (z. B. am Ende). Geht die Antwort auf die ganze Kundennachricht ein (alle Themen/Fragen)? Wenn der Kunde mehrere Themen nennt (Wohnort, Job, Familie, Beziehung) und die Antwort nur eines aufgreift → kurze Bezüge zu weiteren Themen ergänzen. Fehlt ein Teil → kurzen Bezug ergänzen.';
  // Mindestlaenge 120 Zeichen: Korrektor erweitert zu kurze Nachrichten thematisch passend (keine generischen Floskeln)
  const minLengthRule = (context.isOpenerReply === true)
    ? ' OPENER: Mindestlaenge mind. 80 Zeichen (nicht erzwungen auf 120 – kurze Erwiderung + 1–2 Fragen reicht).'
    : ' PFLICHT: Die Nachricht muss mindestens 120 Zeichen haben. Ist sie kuerzer: mit einem thematisch passenden Satz oder einer passenden Frage erweitern (zur Kundennachricht und zur Antwort passend), sodass mindestens 120 Zeichen. Keine generischen Fragen wie "Und bei dir so?" – nur inhaltlich passende Erweiterung.';
  ...
  // Zeit-Zusage immer verboten: Kunde nennt Zeitraum/Tag – Antwort darf NICHT zustimmen; "Das klingt gut" am Anfang ersetzen durch Unverbindlichkeit
  const noTimeAgreementRule = ' Hat der Kunde einen Zeitraum, Tag oder Zeitfenster genannt (z.B. Samstag, Sonntag, Wochenende, 01.-21.06, Juni, nächste Woche)? Enthaelt die Antwort eine Zusage dazu (z.B. "passt perfekt", "passt gut", "klingt gut", "Das klingt gut", "klingt schonmal gut", "zwischen X und Y passt", "die Zeiten passen", "passt mir")? Wenn ja → umschreiben: KEINE Zusage. Beginnt die Antwort mit "Das klingt gut" / "Klingt gut" o.ae.? Dann diesen Einstieg ersetzen durch unverbindliche Formulierung (z.B. "Ich schau mal ob es bei mir klappt." oder "Bin mir nicht sicher ob ich da kann.") und den Rest (Fragen, Inhalt) beibehalten.';
  const noRepetitionRule = ' Enthaelt die Antwort dieselbe Phrase, denselben Satzanfang oder denselben Begriff/Firmennamen (z.B. "MST Service", "MST-Service") mehr als einmal? Wenn ja → umformulieren: nur eine natürliche Erwähnung beibehalten, Rest entfernen oder ersetzen, variieren, natürlich klingen lassen.';
  // Sinn/Vollständigkeit: Korrektor soll auffällig unvollständige oder kaputte Formulierungen glätten (kein Post-Processing mehr mit "…"-Ersetzung).
  const senseCheckRule = ' SINN/VOLLSTAENDIGKEIT: Klingt die Nachricht sinnvoll und vollstaendig? Enthaelt sie Luecken, unsinnige "…"-Stellen mitten im Satz, fehlende Woerter oder extrem Auffaelliges? Wenn ja → natürlich ergaenzen oder glätten, sodass ein flüssiger, vollstaendiger Satz entsteht. Keine abgehackten oder unvollstaendig wirkenden Formulierungen.';
  // Keine Fragen nach Dingen, die der Kunde im Chat / in der letzten Nachricht bereits gesagt hat (explizit typische Phrasen)
  const noAlreadyAnsweredRule = ' BEREITS BEANTWORTET: Steht am Ende der Antwort eine Rueckfrage nach etwas, das der Kunde bereits gesagt hat (Wohnort, Name, Vorlieben, Beruf, was er sucht)? Typische verbotene Schlussfragen: "und du?", "woher kommst du?", "und woher?", "wie heisst du?", "und deine Vorlieben?", "was suchst du?", "und was suchst du?". Wenn ja → diese Schlussfrage entfernen (Satz davor mit Punkt beenden) oder durch eine andere thematische Frage ersetzen. Nutze den Chat-Verlauf.';
  // Keine Wiederholung von Fakten, die bereits im Verlauf stehen (Fake oder Kunde haben sie schon gesagt)
  const noFactRepetitionRule = ' FAKTEN-WIEDERHOLUNG: Enthaelt die Antwort eine Aussage, die wörtlich oder inhaltlich bereits von Fake oder Kunde im Chat-Verlauf gesagt wurde (z.B. "Pluto ist noch ein Welpe, er ist 5 Monate alt", wenn das schon steht oder der Kunde es gerade bestaetigt hat)? Wenn ja → diesen Satz/Teil entfernen oder durch kurze Bestaetigung ersetzen ("Stimmt!", "Genau.") und mit eigenem Satz/Frage weitermachen. Keine redundante Wiederholung von bereits genannten Fakten.';
  // Klare Referenzen: "wir beide" / "uns zwei" eindeutig machen
  const clearReferencesRule = ' KLARE REFERENZEN: Enthaelt die Antwort "wir beide", "uns zwei", "wir werden" o.ae. in einem Kontext, wo unklar ist ob Persona+Haustier oder Persona+Kunde gemeint sind? Wenn ja → umformulieren mit klarer Bezeichnung (z.B. "Pluto und ich werden harmonieren" oder "du und ich werden super harmonieren"), sodass sofort klar ist wen du meinst.';
  // Keine taktlosen oder thematisch falschen Fragen (z.B. nach Kunden-Hund/Verlust wenn ueber Fake-Haustier gesprochen wird)
  const noTactlessQuestionRule = ' TAKTLOSE/UNPASSENDE FRAGE: Enthaelt die Antwort eine Frage, die thematisch oder emotional nicht zum Gespraech passt (z.B. "wie lange ist dein letzter Hund schon nicht mehr bei dir?" wenn ueber das HAUSTIER DES FAKE gesprochen wird – wirkt taktlos und komisch)? Wenn ja → diese Frage entfernen oder durch eine thematisch passende ersetzen (z.B. zu deinem Hund, Spaziergaenge, Erziehung). Nur Fragen, die ein Mensch in dieser Situation natuerlich stellen wuerde.';
  // Eigenkoerper: Wenn Persona (Fake) ueber den EIGENEN Koerper spricht (Muschi, Brust, Po), muss MEINE stehen – nicht EINE (Kunde meint die Persona)
  const ownBodyRule = ' EIGENER KOERPER: Spricht die Antwort ueber den Koerper der Persona (z.B. Muschi, Brust, Po) mit "eine behaarte Muschi", "eine geile Brust" o.ae.? Wenn ja → ersetze durch "meine" (meine behaarte Muschi, meine Brust) – der Kunde meint DICH/Persona, nicht irgendeine.';
  // Eigene Fluessigkeiten: Frau = Mösensaft/Saft gehoert ihr → mein/meine; Mann = Sperma gehoert ihm → mein. Korrektor behebt "dein" wo "mein" gemeint ist.
  const ownBodyFluidsRule = (context.fakeIsFemale === true)
    ? ' EIGENE FLUESSIGKEITEN (Persona WEIBLICH): Enthaelt die Antwort "dein Mösensaft", "dein Saft" oder "deine ..." in einem Kontext, wo die SPRECHERIN (die Persona/Frau) von IHRER EIGENEN Flüssigkeit oder ihrem Koerper spricht (z.B. ihr Saft auf Brüste verteilen – sie ist die Frau)? Wenn ja → ersetze durch "mein Mösensaft", "mein Saft", "meine Brüste" etc. – was der Persona gehoert = mein/meine, nicht dein/deine.'
    : (context.fakeIsFemale === false)
      ? ' EIGENE FLUESSIGKEITEN (Persona MAENNLICH): Enthaelt die Antwort "dein Sperma" in einem Kontext, wo der SPRECHER (die Persona/der Mann) von SEINEM EIGENEN Sperma spricht? Wenn ja → ersetze durch "mein Sperma". Was der Persona gehoert = mein/meine.'
      : ' EIGENE FLUESSIGKEITEN: Spricht die Persona von IHRER EIGENEN Flüssigkeit/Koerper (z.B. Frau: Mösensaft; Mann: Sperma) aber steht "dein/deine" davor? Dann durch "mein/meine" ersetzen – was der Persona gehoert = mein/meine.';
  // Standard-Sex-Frage am Ende: nicht immer "Was magst du?" / "Doggy?" – thematisch passend ersetzen
  const noStandardSexQuestionRule = ' STANDARD-SEX-FRAGE: Enthaelt die Antwort eine Sex-Frage wie "stehst du auf Anal?", "magst du Anal?", "Was magst du denn so?", "Doggy?", "Hast du schonmal Doggy genommen?", "Was magst du beim Sex?" obwohl der Kunde dieses Thema (Anal, Deep Throat, Spielzeuge, Peitsche, Fesseln, etc.) bereits genannt hat? Wenn ja → diese Frage entfernen oder durch thematisch passende Nachfrage ersetzen (darauf eingehen, nicht nachfragen).';
  // Bild + Kundentext: Kunde hat Text UND Bild geschickt – Antwort darf nicht nur auf Bild reagieren, muss auf seine Fragen eingehen
  const imageTextPriorityRule = (context.customerSentImageWithText === true)
    ? ' BILD + KUNDENTEXT: Der Kunde hat Text UND Bild geschickt. Geht die Antwort NUR auf das Bild ein (z.B. "dein Schwanz", "dein Koerper", sexuelle Reaktion) ohne auf seine Fragen/Vorstellung einzugehen (Wohnort, Name, etc.)? Wenn ja → umschreiben: ZUERST auf seine Fragen/Vorstellung eingehen (z.B. Wohnort-Frage beantworten, auf Vorstellung reagieren), DANN optional kurz auf das Bild. Seine Fragen muessen beantwortet werden.'
    : '';

  if (MISTRAL_USE_MINIMAL_PROMPT) {
    // Minimal-Prompt: kontextabhängige Fokus-Regeln + kompakte Basis-Regeln. Eigenes Modell (Fine-Tune) ergänzt durch explizite kritische Regeln.
    const contactRuleMinimal = (context.contactRequest === true)
      ...
      : '';
    // Kontextabhängige Fokus-Regeln (kompakt, nur wenn relevant) – prominent, damit Korrektor sie beachtet
    const contextFocusRules = [];
    if (context.isDirectMeetingRequest === true) {
      contextFocusRules.push('TREFFEN-EINLADUNG: Echo der Einladung VERBOTEN (z.B. "dann kannst zu mir kommen", "komm vorbei"). Keine Fragen zu Transport/Logistik (Bus, Auto, Mobil, "bist mobil", "kommst mit Bus"). Keine "schöne Abwechslung" oder "wann es bei mir klappt". Unverbindlich antworten (z.B. Mal schauen, klingt nett) + Gegenfrage.');
    }
    if (context.customerSentImageWithText === true) {
      contextFocusRules.push('BILD+TEXT: ZUERST auf Kundentext/Fragen eingehen, DANN optional auf Bild. Antwort darf nicht NUR auf Bild reagieren.');
    }
    if (context.contactRequest === true) {
      contextFocusRules.push('KONTAKTDATEN: Persönlich ausweichen, NIEMALS "nicht erlaubt"/"Regeln"/"Plattform".');
    }
    if (context.customerStatedSexPreferences) {
      contextFocusRules.push('SEX-VORLIEBEN BEREITS GENANNT: Kunde hat bereits genannt: ' + sanitizeForApiContent(context.customerStatedSexPreferences.slice(0, 80)) + '. NICHT "stehst du auf Anal?" oder "magst du X?" fragen – darauf eingehen, nicht nachfragen.');
    }
    const contextFocusBlock = contextFocusRules.length > 0
      ? ' KRITISCHE REGELN FÜR DIESEN KONTEXT:\n' + contextFocusRules.join('\n') + '\n\n'
      : '';
    // Nur bei Regelverstoß korrigieren – unnötige "Verbesserungen" vermeiden
    const onlyCorrectWhenNeeded = ' Korrigiere NUR bei Regelverstoessen. Wenn der Entwurf die Regeln einhaelt, gib ihn unveraendert zurueck. Fuege keine zusaetzlichen Fragen oder Formulierungen ein, die Fehler einfuehren koennten.';
    systemContent = 'PFLICHT: Nur die fertige korrigierte Nachricht zurueckgeben, keine Erklaerungen.\n\nDu bist ein Korrektor für Chat-Moderator-Antworten.' + onlyCorrectWhenNeeded + ' Gib nur die fertige korrigierte Nachricht zurück, keine Erklärungen, keine Meta-Kommentare. Stil und Wortschatz der ursprünglichen Antwort möglichst beibehalten, nur klare Fehler korrigieren. Jede Nachricht muss eine Frage enthalten; maximal ein bis zwei Fragen, keine Frage-Kaskade. Mindestens 120 Zeichen – bei kürzerer Nachricht thematisch passend erweitern.\n\n' + contextFocusBlock + senseCheckRule + noAlreadyAnsweredRule + noFactRepetitionRule + clearReferencesRule + noTactlessQuestionRule + ownBodyRule + ownBodyFluidsRule + noStandardSexQuestionRule + offerVsRequestRule + conditionalQuestionRule + moderatorOfferedPictureRule + normalPictureRule + matchSexualLevelRule + flirtyOnlyRule + contactRuleMinimal + topicFidelityRule + planTopicRule + workLogbookConsistencyRule + imageTextPriorityRule + noWiedersehenRule + noSharedPastRule + noTimeAgreementRule + boundaryConsistencyRule + censorshipRule + noOftTreffenRule + noRepetitionRule + limitationFollowUpRule + engageOnSuggestionsRule + topicSpecificFollowUpRule + noDumbQuestionsRule + neutralMessageNoSexRule + meetingRule + leadTimeAnswerRule + openerRule + referentialClarityRule + personaAdultRule + questionAndWholeRule + minLengthRule + sameActivityPlaceRule + noParaphraseRule + imageOnlyAnnouncedRule + noPlaceClaimRule + noStrongHopeRule + customerSingularRule + singleProfileNoWirRule + humanTyposHint;
    ...
    if (process.env.NODE_ENV !== 'production') console.log('🔧 Mistral-Korrektor: Minimal-Prompt (eigenes Modell) + kontextabhängige Regeln');
  } else {
    const contactCheck = context.contactRequest === true ? '\n(7) Kontaktdaten: Enthaelt die Antwort "nicht erlaubt", "Regeln", "Plattform", "echte Kontaktdaten verboten"? → entfernen/umschreiben. Lehnt die Antwort die Kontakt-Anfrage nicht ab? → persoenlich ausweichend einbauen.' : '';
    userContent = `${contextLine}${planBlock}${conversationBlock}${learningBlock}${exampleBlock}Kundennachricht: "${sanitizeForApiContent(customerForCorrector)}"\n\nPrüfe die folgende Moderatoren-Antwort und korrigiere/verbessere sie:\n(1) Entspricht die Antwort dem Plan? Bleibt sie beim Thema der Kundennachricht? Geht sie inhaltlich darauf ein? Wenn die Nachricht z.B. von Geld/Problem/Einschränkung handelt und die Antwort zu Treffen/Sex/Vorlieben wechselt ohne Bezug → umschreiben (zuerst auf Kundenthema eingehen).\n(2) Geht die Antwort auf die Kundennachricht ein? Wenn nein → umschreiben.\n(3) Enthält die Nachricht eine Frage? Wenn nein → Frage einbauen.\n(4) Hat die Nachricht mindestens 120 Zeichen? Wenn nein → thematisch passend erweitern (keine generischen Floskeln).\n(5) Umlaute (ä,ö,ü) und ss statt ß? Stil/Bindestriche?\n(6) Enthält die Antwort Meta-Kommentare oder wörtliche Wiederholung der Kundennachricht? Wenn ja → entfernen, eigenständig formulieren.${contactCheck}\nGib NUR den fertigen korrigierten Text zurück, keine Erklärungen.\n\nZu korrigierende Antwort:\n\n${sanitizeForApiContent(grokText.trim())}`;
...
Du bist ein Korrektor für Chat-Moderator-Antworten. Entscheide immer anhand des gesamten Kontexts und der Kundennachricht (inkl. Chat-Verlauf). ${senseCheckRule} ${noAlreadyAnsweredRule} ${noFactRepetitionRule} ${clearReferencesRule} ${noTactlessQuestionRule} ${ownBodyRule} ${ownBodyFluidsRule} ${noStandardSexQuestionRule} ${offerVsRequestRule} ${conditionalQuestionRule} ${moderatorOfferedPictureRule} ${normalPictureRule} ${matchSexualLevelRule} ${flirtyOnlyRule} ${topicFidelityRule} ${planTopicRule} ${workLogbookConsistencyRule} ${imageTextPriorityRule} ${sexualRule} ${contactIrritatedRule}${meetingRule} ${leadTimeAnswerRule} ${openerRule} ${referentialClarityRule} ${personaAdultRule} ${noWiedersehenRule} ${noSharedPastRule} ${noTimeAgreementRule} ${boundaryConsistencyRule} ${censorshipRule} ${noOftTreffenRule} ${limitationFollowUpRule} ${engageOnSuggestionsRule} ${topicSpecificFollowUpRule} ${noDumbQuestionsRule} ${metaRule} ${noEchoRule}${noRepetitionRule}${questionAndWholeRule}${minLengthRule}${sameActivityPlaceRule}${noParaphraseRule}${customerSingularRule}${singleProfileNoWirRule} Stil und Wortschatz der ursprünglichen Antwort möglichst beibehalten, nur klare Fehler korrigieren. Jede Nachricht muss eine Frage enthalten; maximal ein bis zwei Fragen, keine Frage-Kaskade.${humanTyposHint} PFLICHT: Jede Nachricht muss eine Frage enthalten. Fehlt eine Frage, fuege UNBEDINGT eine kurze, thematisch passende Frage ein (z. B. am Ende). Die Antwort MUSS mindestens 120 Zeichen haben – bei kürzerer Nachricht thematisch passend erweitern (keine generischen Floskeln). Die Antwort MUSS auf die Kundennachricht eingehen. Wenn etwas zu korrigieren ist (fehlende Frage, kein Bezug, Kontaktdaten nicht abgelehnt, Meta/Wiederholung, Umlaute/ss, Stil), aendere es. Schreibe mit ä, ö, ü. Immer ss, nie ß. Keine Anführungszeichen. Keine Bindestriche. Antworte NUR mit der fertigen korrigierten Nachricht – kein anderer Text.`;
  }
 ...
 * @param {boolean} [opts.moderatorOfferedPicture] - Fake hatte in letzter Nachricht angeboten, dem Kunden ein Bild zu schicken → keine Ablehnungs-Begründung in Antwort
...
async function runGrokPipeline(opts) {
  ...
  if (USE_MISTRAL_GENERATOR && getMistralGeneratorClient()) {
    console.log('🚀 Pipeline: Mistral Fine-Tune für Nachrichtengenerierung');
  } else {
    console.log('🚀 Pipeline: Grok (xAI) für Nachrichtengenerierung');
  }
  ...
  if (!isFirstMessage && !isASA) {
    ...
    if (loc.isLocationQuestion && loc.cityInstructions) {
      try {
        ...
        let finalMessage = await callMessageGenerator(messages);
  ...
  if (isASA) {
    ...
    try {
      ...
      const finalMessage = await callMessageGenerator(messages);
      return emptyResult({
        safety: safetyCheck,
        finalMessage: postProcessMessage(finalMessage),
  ...
  if (isFirstMessage && firstMessageInstructions) {
    try {
      ...
      if (finalMessage.length < 150) {
        ...
        finalMessage = (finalMessage + ext).substring(0, 500);
      }
      return emptyResult({
        safety: safetyCheck,
        finalMessage,
  ...
  let plan = '';
  const isOpenerReply = isOpenerMessage(customerMessage || '');
  if (isOpenerReply) {
    plan = 'Opener: Kurz Begruessung erwidern, 1–2 einfache gespraechseroeffnende Fragen (z.B. Wie geht\'s dir? Was machst du so?). Keine Rechtfertigung (echt/real, keine Sorge), kein "neu angemeldet", kein "einzige Person mit der ich schreibe". Kurz und natuerlich (80–120 Zeichen reicht).';
    if (plan) console.log('✅ Grok Plan-Schritt (Opener):', plan.substring(0, 80) + '...');
  } else {
    try {
  ...
  const alreadyStatedNoRepeat = buildAlreadyStatedForNoRepeat(filteredUserInfo);
  ...
  try {
    const messages = buildNormalPrompt({
      ...
      moderatorOfferedPicture: !!opts.moderatorOfferedPicture,
      fakeProfession,
      fakeLogbookHint,
    ...
    for (let questionAttempt = 1; questionAttempt <= 2; questionAttempt++) {
      finalMessage = await callMessageGenerator(messages);
      finalMessage = postProcessMessage(finalMessage);
    ...
    const fakeIsFemale = (fakeGender === 'weiblich' || fakeGender === 'w' || fakeGender === 'female');
    const customerAnsweringLeadTimeQuestion = isModeratorLastMessageLeadTimeQuestion(conversationHistory || '') && isCustomerAnsweringLeadTime(customerMessage || '');
    const hasSubstantialCustomerText = customerMessage && customerMessage.trim().length > 30 &&
      customerMessage.trim() !== 'Der Kunde hat ein Bild geschickt.' &&
      !/^der kunde hat ein bild geschickt\.?$/i.test(customerMessage.trim());
    const customerSentImageWithText = !!(opts.imageDescription && opts.imageDescription.trim()) && hasSubstantialCustomerText;
    const correctorContext = {
      ...
      isDirectMeetingRequest: isDirectMeetingRequestFlag,
      hasProfilePic: profileInfo?.customerInfo?.hasProfilePic === true,
      allowSexualContent: !isMessageAboutFamilyOrChildren(customerMessage) && detectedSituations && detectedSituations.some(s => (s || '').includes('Sexuell')) && !noSexHint && isCurrentMessageExplicitlySexual(customerMessage),
      ...
      fakeIsPlural,
      fakeIsFemale,
      customerAnsweringLeadTimeQuestion,
      isOpenerReply,
      customerMessageOnlyFlirty: !isCurrentMessageExplicitlySexual(customerMessage),
      customerSentImageWithText,
      customerStatedSexPreferences: (extractedUserInfo?.user?.['Sexual Preferences'] || '').trim() || null
    };
    ...
    if (corrected != null && corrected.trim()) {
      ...
      const corrNorm = corrected.trim().toLowerCase().replace(/\s+/g, ' ');
      const isIdentical = origNorm === corrNorm || (origNorm.length > 20 && corrNorm.includes(origNorm) && corrNorm.length - origNorm.length < 15);
      const meetingViolations = correctorIntroducedMeetingViolations(corrected.trim(), correctorContext);
      if (meetingViolations) {
        console.log('ℹ️ Grok-Pipeline: Korrektor-Ergebnis verworfen (Treffen-Regelverstoß: Echo/Transport/schöne Abwechslung)');
      } else if (lenNew >= minLen && !isIdentical) {
        finalMessage = postProcessMessage(corrected);
    ...
    // Post-Processing (kein Echo/Wiederholungs-Ersetzen mehr – Korrektor prüft Sinn/Vollständigkeit)
    ...
    finalMessage = fixKlingtGutWhenCustomerNamedTime(customerMessage, finalMessage);
    finalMessage = stripAlreadyAnsweredEnding(finalMessage, extractedUserInfo?.user);
    finalMessage = fixUnsToMichForSingleProfile(finalMessage, fakeIsPlural);
    finalMessage = postProcessMessage(finalMessage);
    // Sinnvolle Obergrenze (Korrektor-Ergebnis nicht mehr auf 250 kappen – sonst gehen saubere Antworten verloren)
    const MAX_FINAL = 900;
...
/**
 * Ersetzt "uns" durch "mich" bei Einzelprofilen – KI schreibt manchmal "würdest du uns" statt "würdest du mich".
 * @param {string} msg - finale Nachricht
 * @param {boolean} fakeIsPlural - true = Doppelprofil (wir/uns korrekt), false = Einzelprofil (uns → mich)
 * @returns {string}
 */
function fixUnsToMichForSingleProfile(msg, fakeIsPlural) {
  if (!msg || typeof msg !== 'string' || fakeIsPlural === true) return msg;
  let out = msg;
  out = out.replace(/\bwürdest\s+du\s+uns\b/gi, 'würdest du mich');
  out = out.replace(/\bwillst\s+du\s+uns\b/gi, 'willst du mich');
  out = out.replace(/\bkannst\s+du\s+uns\b/gi, 'kannst du mich');
  out = out.replace(/\b([Dd]u)\s+uns\b/g, '$1 mich');
  return out;
}

/**
 * Entfernt Schlussfragen, die nach etwas fragen, das der Kunde bereits in seiner Nachricht genannt hat
 * (Wohnort, Name, Vorlieben, etc.). Allgemein: keine "und du?" / "woher kommst du?" wenn Wohnort
 * schon gesagt; keine "wie heisst du?" wenn Name schon gesagt; etc.
 * @param {string} msg - finale Nachricht
 * @param {Object} userInfo - extractedUserInfo.user (bereits genannte Felder)
 * @returns {string}
 */
function stripAlreadyAnsweredEnding(msg, userInfo) {
  if (!msg || typeof msg !== 'string') return msg;
  if (!userInfo || typeof userInfo !== 'object') return msg;
  const has = (key) => {
    const v = userInfo[key];
    return v != null && String(v).trim() !== '';
  };
  // Welche Themen hat der Kunde bereits genannt? (Keys aus Extraktion)
  const hasWohnort = has('Wohnort') || has('Stadt') || has('city');
  const hasName = has('Name') || has('name');
  const hasVorliebenSucht = has('Other') || has('Beziehungsstatus') || has('Sport and Hobbies');
  const hasBeruf = has('Beruf') || has('Work');
  let out = msg.trim();
  const trimEnd = (s) => s.replace(/\s*[,.]?\s*$/, '').trim();
  // Wohnort bereits genannt → "und du?", "woher kommst du?" etc. am Ende entfernen
  if (hasWohnort) {
    const patterns = [
      /\s*[,.]?\s*und\s+du\s*\.?\s*$/i,
      /\s*[,.]?\s*woher\s+kommst\s+du\s*\.?\s*$/i,
      /\s*[,.]?\s*und\s+woher\s+kommst\s+du\s*\.?\s*$/i,
      /\s*[,.]?\s*und\s+du\s*[,;]\s*wo\s+wohnst\s+du\s*\.?\s*$/i,
      /\s*[,.]?\s*und\s+wo\s+wohnst\s+du\s*\.?\s*$/i
    ];
    for (const re of patterns) {
      if (re.test(out)) {
        out = trimEnd(out.replace(re, ''));
        if (!out.endsWith('.') && !out.endsWith('!')) out = out + '.';
        break;
      }
    }
  }
  // Name bereits genannt → "wie heisst du?" am Ende entfernen
  if (hasName) {
    const re = /\s*[,.]?\s*(und\s+)?wie\s+heisst\s+du\s*\.?\s*$/i;
    if (re.test(out)) {
      out = trimEnd(out.replace(re, ''));
      if (!out.endsWith('.') && !out.endsWith('!')) out = out + '.';
    }
  }
  // Vorlieben/Sucht bereits genannt → "was suchst du?", "und deine Vorlieben?" entfernen
  if (hasVorliebenSucht) {
    const patterns = [
      /\s*[,.]?\s*was\s+suchst\s+du\s*\.?\s*$/i,
      /\s*[,.]?\s*und\s+was\s+suchst\s+du\s*\.?\s*$/i,
      /\s*[,.]?\s*und\s+deine\s+Vorlieben\s*\.?\s*$/i,
      /\s*[,.]?\s*was\s+magst\s+du\s*\.?\s*$/i
    ];
    for (const re of patterns) {
      if (re.test(out)) {
        out = trimEnd(out.replace(re, ''));
  ...
  // Beruf bereits genannt → "was machst du beruflich?" etc. entfernen
  if (hasBeruf) {
    const re = /\s*[,.]?\s*(und\s+)?(was\s+machst\s+du\s+beruflich|was\s+arbeitest\s+du|dein\s+Beruf)\s*\.?\s*$/i;
    if (re.test(out)) {
      out = trimEnd(out.replace(re, ''));
      if (!out.endsWith('.') && !out.endsWith('!')) out = out + '.';
    }
...
/**
 * Wenn der Kunde konkrete Tage/Zeiten genannt hat und die Antwort mit "Das klingt gut" o.ae. beginnt,
 * ersetzt den Einstieg durch eine unverbindliche Formulierung (wirkt sonst wie Zusage).
 * @param {string} customerMessage - aktuelle Kundennachricht
 * @param {string} msg - finale Nachricht
 * @returns {string}
 */
function fixKlingtGutWhenCustomerNamedTime(customerMessage, msg) {
  if (!msg || typeof msg !== 'string') return msg;
  const cust = (customerMessage || '').toLowerCase();
  const hasNamedTime = /\b(samstag|sonntag|montag|dienstag|mittwoch|donnerstag|freitag|wochenende|nächste woche|naechste woche|nächsten?\s+monat|naechsten?\s+monat|\d{1,2}\.\s*(0?[1-9]|1[0-2])\.|januar|februar|märz|april|mai|juni|juli|august|september|oktober|november|dezember)\b/i.test(cust);
  if (!hasNamedTime) return msg;
  const start = msg.slice(0, 80).toLowerCase();
  if (/schau\s+mal|nicht\s+sicher\s+ob\s+ich|mal\s+schauen\s+ob\s+ich/.test(start)) return msg;
  const badStart = /^\s*((Das\s+)?klingt\s+(ja\s+)?(schonmal\s+)?(ganz\s+)?gut\s*[.!]?\s*)/i;
  if (badStart.test(msg)) {
    return msg.replace(badStart, 'Ich schau mal ob es bei mir klappt. ').replace(/\s+/g, ' ').trim();
  }
  return msg;
}
...
function postProcessMessage(msg) {
  ...
  let m = msg.trim();
  // Persona erwachsen: Sätze mit kindlichen Kontexten (z.B. von der Schule abholen) entfernen
  const childContextPattern = /[^.!?]*(?:von der Schule abholen|von der schule abholen|wuensche? (?:mir|mich)[^.!?]*(?:von der )?[Ss]chule[^.!?]*abholen|jemand (?:würde|wuerde) mich von der [Ss]chule abholen)[^.!?]*[.!?]/gi;
  m = m.replace(childContextPattern, '').replace(/\s+/g, ' ').trim();
  // Eigenkoerper: "eine behaarte/geile/... Muschi/Brust" → "meine ..." (Kunde meint die Persona)
  m = m.replace(/\beine\s+(behaarte|geile|feuchte|heisse|schoene|schöne|heiße)\s+(Muschi|Fotze|Brust|Pussy)\b/gi, 'meine $1 $2');
