...
function resolveSafeFakeFirstName(moderatorInfo, extractedAssistantName) {
  const pick = (raw) => {
    if (!raw || typeof raw !== 'string') return '';
    const s = raw.trim();
    if (!s) return '';
    if (looksLikePlatformUsername(s)) return '';
    const first = firstNameTokenOnly(s);
    if (!first) return '';
    if (looksLikePlatformUsername(first)) return '';
    return first;
  };
  const candidates = [
  ...
  for (const c of candidates) {
    const v = pick(typeof c === 'string' ? c : (c != null ? String(c) : ''));
    if (v) return v;
  }
  return '';
}
...
function buildFirstMessagePrompt({ allRules, firstMessageInstructions, profileInfo, extractedUserInfo, doubleProfileHint = '', customerFirstNameForAddress }) {
  const rulesBlock = buildRulesBlock(allRules);
  const fakeName = resolveSafeFakeFirstName(profileInfo?.moderatorInfo || {}, extractedUserInfo?.assistant?.Name) || 'Sandy';
