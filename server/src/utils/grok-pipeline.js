...
function buildNormalPrompt({
  ...
  if (doubleProfileHint && doubleProfileHint.trim()) {
    ...
    systemContent += 'EINZELPROFIL: Du bist EINE Person (kein Doppelprofil). Nutze ich, mich, meine, mein – NIEMALS wir, uns, unser, unsere (das gilt nur bei echtem Doppelprofil mit 2 Namen, z.B. "Femke und Nadine").\n\n';
...
async function runMistralCorrector({ customerMessage = '', context = {}, grokText = '', learningContext = '', exampleSnippet = '', planSnippet = '', conversationSnippet = '' }) {
  ...
  const singleProfileNoWirRule = (context.fakeIsPlural === false)
    ? ' EINZELPROFIL: Das Profil ist EINE Person (kein Doppelprofil). Enthaelt die Antwort "wir", "uns", "unser", "unsere" in Bezug auf die Persona (z.B. "du uns", "unser Arschloch", "unsere Spalte", "wir werden feucht")? Wenn ja → ersetzen durch ich, mich, mein, meine (z.B. "du mich", "mein Arschloch", "meine Spalte", "ich werde feucht").'
    : '';
