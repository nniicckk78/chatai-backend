...
PFLICHT: Nur eine Nachricht ausgeben; Frage am Ende (im Stil der Beispiele); keine Meta-Kommentare; KEIN ECHO – nicht Name, Alter, Ort oder Vorlieben des Kunden zurueckspielen, eigenstaendig formulieren.`;
  ...
  userContent += `FRAGE BEANTWORTEN: Enthaelt die aktuelle Kundennachricht eine direkte Frage an DICH (z.B. Wie geht es dir?, Wie heisst du?, Was machst du?, was suchst du?, was macht X für dich schön)? → ZUERST diese Frage beantworten (aus deiner Sicht; bei Namensfrage: deinen Namen nennen). Nicht die Frage nur zurueckgeben oder auf anderes umlenken. Der Kontext erklaert, warum er das fragt und worauf er sich bezieht – nutze ihn zum Verstehen, antworte aber auf die aktuelle Nachricht. Danach Gegenfrage.\n\n`;
...
async function runPlanningStep(customerMessage, detectedSituations, allRules, conversationHistory = '') {
  ...
  const hasDirectQuestionToPersona = hasNameQuestion || hasWieGehtEsDir || /\b(für dich|für dich\?|was macht .* für dich|was suchst du|wie oft|was wäre für dich|was findest du|was ist für dich|besonders schön für dich)\b/i.test(customerSnippet) || (/\?/.test(customerSnippet) && /\b(dich|dir|du)\b/i.test(customerSnippet));
  const directQuestionHint = hasDirectQuestionToPersona
    ? ' PFLICHT: Die aktuelle Kundennachricht enthaelt eine direkte Frage an die Persona (z.B. wie geht es dir, wie heisst du, was machst du, was suchst du). Plan: ZUERST diese Frage aus Persona-Sicht beantworten (Namensfrage: Persona-Namen), dann thematische Gegenfrage – nicht die Frage nur zurueckgeben oder umlenken. Kontext fuer Verstaendnis nutzen; Antwort auf aktuelle Nachricht.'
    : '';
...
async function runMistralCorrector({ customerMessage = '', context = {}, grokText = '', learningContext = '', exampleSnippet = '', planSnippet = '', conversationSnippet = '' }) {
  ...
  const hasDirectQuestionToPersonaMistral = hasNameQuestionMistral || hasWieGehtEsDirMistral || /\b(für dich|für dich\?|was macht .* für dich|was suchst du|wie oft|was wäre für dich|besonders schön für dich)\b/i.test(fullCustomerMsg) || (/\?/.test(fullCustomerMsg) && /\b(dich|dir|du)\b/i.test(fullCustomerMsg));
  const answerDirectQuestionRuleMistral = hasDirectQuestionToPersonaMistral
    ? ' PFLICHT: Die AKTUELLE Kundennachricht enthaelt eine direkte Frage an die Persona (z.B. wie geht es dir, wie heisst du, was machst du, was suchst du). Beantwortet die Antwort diese Frage aus Persona-Sicht oder lenkt sie nur um / ignoriert sie? Wenn nicht beantwortet → zuerst die Frage beantworten (Namensfrage: Persona-Namen; "wie geht es dir": wie es der Persona geht usw.), dann Gegenfrage. Kontext bleibt wichtig zum Verstehen – die Antwort muss aber auf die aktuelle Nachricht eingehen.'
    : '';
...
async function runOpenAIFullCorrector({ customerMessage = '', context = {}, grokText = '', learningContext = '', exampleSnippet = '', planSnippet = '', conversationSnippet = '' }) {
  ...
  const hasDirectQuestionToPersonaOpenAI = hasNameQuestionOpenAI || hasWieGehtEsDirOpenAI || /\b(für dich|für dich\?|was macht .* für dich|was suchst du|wie oft|was wäre für dich|besonders schön für dich)\b/i.test(fullCustomerMsg) || (/\?/.test(fullCustomerMsg) && /\b(dich|dir|du)\b/i.test(fullCustomerMsg));
  const answerDirectQuestionRuleOpenAI = hasDirectQuestionToPersonaOpenAI
    ? ' PFLICHT: Die AKTUELLE Kundennachricht enthaelt eine direkte Frage an die Persona (z.B. wie geht es dir, wie heisst du, was machst du, was suchst du). Beantwortet die Antwort diese Frage aus Persona-Sicht oder lenkt sie nur um / ignoriert sie? Wenn nicht beantwortet → zuerst die Frage beantworten (Namensfrage: Persona-Namen; "wie geht es dir": wie es der Persona geht usw.), dann Gegenfrage. Kontext bleibt wichtig – die Antwort muss auf die aktuelle Nachricht eingehen.'
    : '';
