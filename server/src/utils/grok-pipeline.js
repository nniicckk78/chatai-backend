...
async function runMistralCorrector({ customerMessage = '', context = {}, grokText = '', learningContext = '', exampleSnippet = '', planSnippet = '', conversationSnippet = '' }) {
  ...
  const topicSpecificFollowUpRule = ' THEMEN-SPEZIFISCHE GEGENFRAGE: Hat der Kunde ein KONKRETES Thema genannt (Hobby, Aktivitaet, Vorliebe – z.B. Feuerwehr, Spazieren, Beruf, sexuelle Vorliebe)? Enthaelt die Antwort nur eine generische Frage ("was hast du noch vor?", "noch weitere Plaene?", "was machst du sonst so?") ohne thematische Nachfrage dazu? Wenn ja → umschreiben: eine spezifische Nachfrage zum genannten Thema einbauen (z.B. wie lange, wo, wie gefaellts, thematisch nahe Aspekt), die einen echten Mehrwert hat.';
  ...
  const noStandardSexQuestionRule = ' STANDARD-SEX-FRAGE: Enthaelt die Antwort am Ende eine immer wieder vorkommende Standardfrage wie "Was magst du denn so?", "Was magst du?", "Würdest du mich doggy nehmen?" obwohl der Kunde bereits detailliert sexuell geantwortet oder eine Fantasie beschrieben hat? Wenn ja → diese Frage durch eine thematisch passende Nachfrage zum genannten Inhalt ersetzen (zum gerade Besprochenen), keine generische Vorlieben-Frage.';
