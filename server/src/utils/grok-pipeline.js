...
function buildNormalPrompt({
  ...
  if (hasConversationContext) {
    systemContent += `KONTEXT VERSTEHEN (PFLICHT): Die aktuelle Kundennachricht gehoert zu einem laufenden Gespraech. Du MUSST den Chat-Verlauf nutzen, um sie richtig einzuordnen.\n(1) Worauf bezieht sich die Nachricht? Oft antwortet der Kunde auf DEINE letzte Nachricht oder ein gerade besprochenes Thema – nicht isoliert lesen.\n(2) Klarstellung/Abschwaechung: Sagt der Kunde etwas wie "nur X", "nur Routine", "nichts Schlimmes", "war nur ...", "just ..." oder korrigiert/praezisiert er etwas Vorheriges? Dann ist es eine Klarstellung – reagiere PASSEND dazu (positiv, locker, Verstaendnis dass es harmlos ist). KEINE Trost- oder Negativ-Formulierung ("nicht dein Tag", "wird schon wieder", "das wird wieder"), sonst ignorierst du den Kontext.\n(3) Kurze Nachrichten ("ja", "ok", "gut", "nur Zahnreinigung", "eh nur Kontrolle") beziehen sich fast immer auf das zuletzt Gesagte – Antworte im selben Kontext, nicht als waere es eine neue schlechte Nachricht.\nKontext ignorieren und nur die letzte Zeile isoliert lesen fuehrt zu falschen Reaktionen (z.B. Trost wo der Kunde nur klargestellt hat).\n\n`;
  }
...
PFLICHT: Nur eine Nachricht ausgeben; Frage am Ende (im Stil der Beispiele); keine Meta-Kommentare; KEIN ECHO – nicht Name, Alter, Ort oder Vorlieben des Kunden zurueckspielen, eigenstaendig formulieren.`;
  ...
  if (conversationHistory && conversationHistory.trim()) {
    ...
    userContent += `Chat-Verlauf (Auszug – nutze ihn, um die folgende Nachricht im Kontext zu verstehen):\n${sanitizeForApiContent(historySnippet)}\n\n`;
    userContent += `Kontext: Interpretiere die folgende Kundennachricht IM KONTEXT des Chat-Verlaufs – worauf bezieht sie sich? Ist es eine Klarstellung oder Abschwaechung (z.B. "nur Routine", "nichts Schlimmes")? Dann antworte passend dazu (kein Trost, keine "nicht dein Tag"-Formulierung).\n\n`;
  }
