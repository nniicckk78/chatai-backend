...
function buildNormalPrompt({
  ...
  systemContent += `PRIORITAET AKTUELLE KUNDENNACHRICHT: Die Nachricht, auf die du JETZT antwortest, ist die aktuelle (letzte) Kundennachricht. Gehe ZUERST auf sie ein – ignoriere sie nicht zugunsten aelterer Themen. Enthaelt sie eine direkte Frage an DICH (z.B. Wie geht es dir?, Wie heisst du?, Was machst du?, was suchst du? – oder Treffen: „Treffen wir uns heute?“, „Wann sehen wir uns?“)? Dann diese Frage ZUERST oder klar im Verlauf beantworten (aus Persona-Sicht). Bei Treffen-Frage: kurz beantworten (z.B. „heute klappt es leider nicht“, „muss schauen wann es passt“) – NICHT ignorieren, auch wenn der Rest der Nachricht sexuell oder lang ist. Der Chat-Verlauf (Kontext) ist wichtig, um zu verstehen WARUM der Kunde das fragt und worauf er sich bezieht – nutze den Kontext zum Verstehen, antworte aber auf die aktuelle Nachricht.\n\n`;
...
PFLICHT: Nur eine Nachricht ausgeben; Frage am Ende (im Stil der Beispiele); keine Meta-Kommentare; KEIN ECHO – nicht Name, Alter, Ort oder Vorlieben des Kunden zurueckspielen, eigenstaendig formulieren.`;
  ...
  userContent += `FRAGE BEANTWORTEN: Enthaelt die aktuelle Kundennachricht eine direkte Frage an DICH (z.B. Wie geht es dir?, Wie heisst du?, Was machst du?, was suchst du? – oder Treffen: „Treffen wir uns heute?“, „Wann sehen wir uns?“)? → Diese Frage beantworten (bei Treffen z.B. „heute klappt es leider nicht“ / unverbindlich; bei Namensfrage: deinen Namen nennen). Keine Frage ignorieren – auch nicht am Ende einer langen Nachricht. Nicht nur zurueckfragen oder umlenken. Danach Gegenfrage.\n\n`;
...
async function runPlanningStep(customerMessage, detectedSituations, allRules, conversationHistory = '') {
  ...
  const hasDirectQuestionToPersona = hasNameQuestion || hasWieGehtEsDir || hasTreffenFrage || /\b(für dich|für dich\?|was macht .* für dich|was suchst du|wie oft|was wäre für dich|was findest du|was ist für dich|besonders schön für dich)\b/i.test(customerSnippet) || (/\?/.test(customerSnippet) && /\b(dich|dir|du)\b/i.test(customerSnippet));
  const directQuestionHint = hasDirectQuestionToPersona
    ? ' PFLICHT: Die aktuelle Kundennachricht enthaelt eine direkte Frage an die Persona (z.B. wie geht es dir, wie heisst du, was machst du, was suchst du – oder Treffen: "Treffen wir uns heute?", "Wann sehen wir uns?"). Plan: Diese Frage NICHT ignorieren – in der Antwort kurz beantworten (bei Treffen-Frage z.B. unverbindlich "heute klappt es leider nicht" / "muss schauen wann es passt"), dann Rest der Nachricht. Kontext nutzen; Antwort auf aktuelle Nachricht.'
    : '';
...
async function runMistralCorrector({ customerMessage = '', context = {}, grokText = '', learningContext = '', exampleSnippet = '', planSnippet = '', conversationSnippet = '' }) {
  ...
  const hasDirectQuestionToPersonaMistral = hasNameQuestionMistral || hasWieGehtEsDirMistral || hasTreffenFrageMistral || /\b(für dich|für dich\?|was macht .* für dich|was suchst du|wie oft|was wäre für dich|besonders schön für dich)\b/i.test(fullCustomerMsg) || (/\?/.test(fullCustomerMsg) && /\b(dich|dir|du)\b/i.test(fullCustomerMsg));
  const answerDirectQuestionRuleMistral = hasDirectQuestionToPersonaMistral
    ? ' PFLICHT: Die AKTUELLE Kundennachricht enthaelt eine direkte Frage an die Persona (z.B. wie geht es dir, wie heisst du, was machst du, was suchst du – oder Treffen: "Treffen wir uns heute?", "Wann sehen wir uns?"). Beantwortet die Antwort diese Frage oder ignoriert sie? Bei Treffen-Frage: kurz beantworten (z.B. "heute klappt es leider nicht" / unverbindlich), NICHT ignorieren. Sonst: zuerst die Frage beantworten, dann Gegenfrage. Kontext bleibt wichtig – Antwort muss auf die aktuelle Nachricht eingehen.'
    : '';
...
async function runOpenAIFullCorrector({ customerMessage = '', context = {}, grokText = '', learningContext = '', exampleSnippet = '', planSnippet = '', conversationSnippet = '' }) {
  ...
  const hasDirectQuestionToPersonaOpenAI = hasNameQuestionOpenAI || hasWieGehtEsDirOpenAI || hasTreffenFrageOpenAI || /\b(für dich|für dich\?|was macht .* für dich|was suchst du|wie oft|was wäre für dich|besonders schön für dich)\b/i.test(fullCustomerMsg) || (/\?/.test(fullCustomerMsg) && /\b(dich|dir|du)\b/i.test(fullCustomerMsg));
  const answerDirectQuestionRuleOpenAI = hasDirectQuestionToPersonaOpenAI
    ? ' PFLICHT: Die AKTUELLE Kundennachricht enthaelt eine direkte Frage an die Persona (z.B. wie geht es dir, wie heisst du, was suchst du – oder Treffen: "Treffen wir uns heute?", "Wann sehen wir uns?"). Beantwortet die Antwort diese Frage oder ignoriert sie? Bei Treffen-Frage: kurz beantworten (z.B. "heute klappt es leider nicht" / unverbindlich), NICHT ignorieren. Sonst: zuerst die Frage beantworten, dann Gegenfrage. Kontext bleibt wichtig – Antwort muss auf die aktuelle Nachricht eingehen.'
    : '';
    ...
    if (loc.isLocationQuestion && loc.cityInstructions) {
      try {
        ...
        const cityCorrectorContext = {
          ...
          expectedCityForWohnort: (loc.cityToUse || '').trim() || null
        };
