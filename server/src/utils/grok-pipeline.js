...
- NICHT SEXUALISIEREN: Enthaelt die Nachricht ein klares nicht-sexuelles Angebot (z. B. Kaffee, Kuchen, Besuch, Treffen, E-Bike, Zeitangabe wie morgen) OHNE sexuelle Formulierung? Gib die Bedeutung GENAU SO wieder – interpretiere es nicht als sexuell oder erotisch.
...
async function classifyCurrentMessageSexuality(customerMessage, conversationHistorySnippet = '') {
  ...
  const systemContent = `Du bewertest NUR die AKTUELLE (letzte) Kundennachricht in einem Dating-Chat.

Frage: Ist diese Nachricht EINDEUTIG SEXUELL? (Ja = Kunde spricht explizit ueber Sex, Koerper, Lust, konkrete sexuelle Handlungen, eindeutige sexuelle Formulierungen. Nein = Smalltalk, Treffen/Kaffee/Kuchen/Besuch/E-Bike ohne sexuelle Formulierung, nur Flirt ohne explizite Sexualitaet, reine Kennenlernen-Fragen.)

WICHTIG: Enthaelt die Nachricht ein klares NICHT-sexuelles Angebot (z.B. Kaffee, Kuchen, Besuch, morgen vorbeikommen, E-Bike-Ausflug) OHNE sexuelle Woerter? Dann ist sie NICHT eindeutig sexuell → Antworte mit "nicht_eindeutig_sexuell".
...
FRAGEN: Mindestens EINE Frage in der Nachricht – konkret, thematisch, zur Kundennachricht passend. Die Frage kann ueberall stehen (z.B. mitten drin: eigene Vorlieben nennen und "Gefaellt dir das?"). Erlaubt: "gefaellt dir das?", "hast du das schonmal gemacht?", "was wuerdest du gerne mal im Bett ausprobieren?", "und du?", "was magst du mehr?" – Vielfalt, nicht nur "was stellst du dir vor?" / "wie machst du weiter". NICHT die letzten Woerter des Kunden einfach in eine Frage packen (nicht "Was machst du mit X und Y?" wenn er X und Y gerade nannte). KRITISCH – KEINE REDUNDANTE FRAGE: Hat der Kunde bereits eine konkrete Information genannt (Wohnort, Film, Beruf)? Dann NICHT nach genau dieser Information fragen. Hat er ein Szenario beschrieben (doggy, lecken)? Dann NICHT dieselbe Handlung als Frage zurueck – stattdessen andere Frage (Tempo, was danach, "hast du das schonmal gemacht?", "was wuerdest du gerne ausprobieren?"). VERBOTEN: "Was interessiert dich an unseren Gesprächen?", "Was denkst du darüber?", Planungs-Fragen ("wie setzen wir das um"). Bei sexuellen Themen: flirty, konkret ("würde dir das gefallen?", "magst du das?", "und was machst du dann für mich?"). Keine vagen "dazu/daran"-Fragen wenn Bezug unklar. Bei Treffen-Wunsch: andeuten, nicht aktiv nach Ort/Ideen fragen.
 ...
 */
async function runPlanningStep(customerMessage, detectedSituations, allRules, conversationHistory = '', alreadyIntroducedSelfInChat = false, currentMessageNotClearlySexual = false) {
...
async function runGrokPipeline(opts) {
  ...
  let plan = '';
  try {
    plan = await runPlanningStep(messageForPipeline, detectedSituations, allRules, conversationHistory, hasIntroducedSelfInChatFlag, currentMessageNotClearlySexual);
