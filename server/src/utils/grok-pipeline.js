...
async function runMistralCorrector({ customerMessage = '', context = {}, grokText = '', learningContext = '', exampleSnippet = '', planSnippet = '', conversationSnippet = '' }) {
  ...
  const meetingRule = (context.isMeetingSituation === true)
    ...
    : '';
  // Frage am Ende (auch im Minimal-Prompt Pflicht) + ganze Nachricht abdecken
  const questionAndWholeRule = ' PFLICHT: Jede Nachricht muss mit einer passenden Frage enden. Fehlt eine → anfügen. Geht die Antwort auf die ganze Kundennachricht ein (alle Themen/Fragen)? Fehlt ein Teil → kurzen Bezug ergänzen.';
