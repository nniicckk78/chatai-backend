...
async function callGrok(messages, options = {}) {
  ...
  let response;
  try {
    response = await doCreate(messages);
  } catch (err) {
    const status = err?.status ?? err?.response?.status;
    const isJsonParseError = status === 400 && err?.message && /hex escape|parse.*json|request body.*json/i.test(String(err.message));
    if (!isJsonParseError || !Array.isArray(messages)) {
      throw err;
    }
    ...
    response = await doCreate(sanitized);
  }
...
async function runGrokPipeline(opts) {
  ...
  // Orts-Kontext für Normal-Reply: Wohnort-Suche NUR wenn (1) Kunde nach Wohnort fragt UND (2) beim Fake noch kein Wohnort im Logbuch steht
  ...
  const fakeHasWohnortAlready = (fakeCity && !isBundesland(fakeCity)) || hasWohnortInLogbook;
  if (!suggestsLocation) {
    locationContext = null;
