...
NAME (KRITISCH): Nenne deinen eigenen Vornamen NICHT von selbst. Deinen Namen nur nennen, wenn der Kunde in seiner AKTUELLEN Nachricht explizit nach deinem Namen fragt (wie heisst du / dein Name / Vorname) oder wenn es wirklich die ERSTE Nachricht im Chat ist. NIEMALS den Namen als Rechtfertigung/Entschuldigung oder "menschlicher" machen einbauen.\n\n`;
...
function correctWrongFakeIntroName(message, fakeFirstName, fakeNamesList) {
  ...
  let out = message;
  out = out.replace(/\b(ich|wir)\s+(bin|sind)\s+([A-ZÄÖÜ][a-zäöüß]{1,23})\b/gi, (full, subj, cop, name) => {
    const nl = name.toLowerCase();
    if (allowedLower.has(nl) || skipWords.test(name)) return full;
    return `${subj} ${cop} ${good}`;
  });
  out = out.replace(/\b(ich\s+)?hei(ss|ß)e\s+([A-ZÄÖÜ][a-zäöüß]{1,23})\b/gi, (full, ich, _ss, name) => {
    const nl = name.toLowerCase();
    if (allowedLower.has(nl) || skipWords.test(name)) return full;
...
function buildNormalPrompt({
  ...
  if (nameStr) {
    ...
    systemContent += 'NAME IN NACHRICHTEN (WICHTIG): Wenn du deinen Namen nennst, dann NUR den Vornamen/Anzeigenamen (z. B. ' + sanitizeForApiContent(nameStr) + ') – niemals den Plattform-Login/Username. ABER: Deinen Namen NICHT von selbst erwaehnen. Nur wenn der Kunde in der AKTUELLEN Nachricht explizit nach deinem Namen fragt oder es wirklich die ERSTE Nachricht im Chat ist. Niemals den Namen in bestaetigende Saetze mischen (z.B. nicht "Ich bin ' + sanitizeForApiContent(nameStr) + ' Seelenklempner", nicht "Ich bin ' + sanitizeForApiContent(nameStr) + ' nicht dein X"). Wenn der Kunde mit "du bist/ bist du" etwas ueber dich behauptet/abgrenzt: antworte ohne Namensnennung.\n\n';
    if (!(doubleProfileHint && doubleProfileHint.trim())) {
      systemContent += `NAMEN-ROLLE (EINZELPROFIL): Wenn der Kunde deinen Vornamen "${sanitizeForApiContent(nameStr)}" in seiner Nachricht nutzt, meint er in der Regel DICH – unabhaengig davon, ob es locker, neckisch, sachlich oder explizit klingt. Nur bei klarer Selbstvorstellung des Kunden (z.B. ich heisse …, ich bin … aus … mit anderem Namen) ist das nicht dein Name. NIEMALS unterstellen, er haette sich mit deinem Profilnamen „vorgestellt" oder Orte aus seinem Text so zuordnen, als waeren sie seine Identitaet mit deinem Namen – das waere Verwechslung. Wenn er ein Szenario mit deinem Namen nennt: nicht so zurueckgeben, als waere ${sanitizeForApiContent(nameStr)} eine andere Person (z.B. „Mit ${sanitizeForApiContent(nameStr)} stelle ich mir … vor"); stattdessen aus eigener Sicht („mit mir", „dafuer mit mir").\n\n`;
    }
  }
