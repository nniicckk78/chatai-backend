    ...
    if (pool) {
      try {
        ...
        if (err.code !== '42P01') { // 42P01 = table does not exist
          ...
          // 🚨 NEU: Prüfe auf Connection-Fehler
          if (err.message && (err.message.includes('Connection terminated') || err.message.includes('connection') || err.message.includes('ECONNRESET'))) {
...
async function initializeVectorDb(trainingData) {
  ...
  const hasTrainingDataChanged = storedHash !== currentTrainingDataHash;
  ...
  if (currentCount > 0 && !hasTrainingDataChanged && !hasMoreTrainingData) {
    console.log(`✅ Vektor-DB bereits initialisiert: ${currentCount} Einträge (Training-Daten: ${trainingDataCount}, Hash: ${storedHash?.substring(0, 8) || 'kein'})`);
    return vectorDb;
  }
