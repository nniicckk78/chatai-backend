...
async function indexTrainingData(trainingData) {
  ...
  for (let i = 0; i < conversations.length; i++) {
    ...
    // 🚨 WICHTIG: Kombiniere Kundennachricht + Moderator-Antwort + Situation für bessere Suche
    ...
    if (conv.situation && conv.situation !== 'allgemein') {
      combinedText = `${conv.situation}: ${combinedText}`;
    }
