...
async function extractDeepPatterns(trainingData, feedbackData, learningStats) {
  ...
  try {
    ...
    for (const [situation, conversations] of topSituations) {
      ...
      // Weniger Beispiele = kleinere Batches, mehr Beispiele = größere Batches (aber max 20)
      const sampleSize = Math.min(Math.max(10, Math.floor(conversations.length / 3)), 20);
...
Antworte NUR als JSON:
{
  ...
  "successfulSequences": [{"sequence": ["..."], "outcome": "..."}]
      ...
      try {
        // Timeout: 12 Sekunden (ausreichend für alle Muster-Typen, aber nicht zu lang)
        const analysis = await Promise.race([
          ...
          new Promise((_, reject) => setTimeout(() => reject(new Error('Timeout')), 12000)) // 12 Sekunden
        ]);
