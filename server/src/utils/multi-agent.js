...
// 🎲 ASA-Auswahl: Round-Robin durch alle Beispiele, optional kombiniert mit „divers zum zuletzt verwendeten“
...
/**
 * Wählt ein ASA-Beispiel per Round-Robin; unter den nächsten Kandidaten wird der am längsten
 * nicht genutzte bevorzugt (least recently used), bei Gleichstand der diverseste zum zuletzt
 * verwendeten Text – mehr Variation, weniger „oft die gleichen“.
...
function selectOneASARoundRobinWithDiversity(pool, lastUsedText) {
  ...
  const CANDIDATES = 8; // Mehr Kandidaten → bessere Chance, einen „lange nicht genutzten“ zu finden
  ...
  for (const { ex, index } of candidates) {
    ...
    // Höher = besser: Frische (lange nicht genutzt) stärker gewichten als Diversität
...
/** Kompatibilität: wie bisher ein ASA auswählen (Round-Robin + optional Diversität). lastUsedText kann null sein. */
...
FORMAT (wie beim normalen Generator): Immer ss, nie ß. KEINE Bindestriche (z. B. Hip Hop, kennenlernen, nicht Hip-Hop oder kennen-lernen). Keine Anführungszeichen am Anfang/Ende.`;
  ...
  const contextRule = `
KONTEXT NUTZEN – SCHLAU REAGIEREN:
- Der Kontext unten ist NUR der letzte Austausch (letzte 1–2 Nachrichtenpaare). Beziehe dich AUSSCHLIESSLICH darauf – keine älteren Themen erfinden.
- Der Kontext kann SEHR KURZ sein (z. B. nur „Kunde: Hallo“ oder „Kunde: Hey“) oder ein längerer Satz. Reagiere passend:
  - Bei nur Gruß (Hallo, Hey, Hi): freundlich zurückgrüßen, 1–2 offene Fragen (wie geht es dir, was machst du), KEIN „unser Gespräch über X“ – es gab keins.
  - Bei echtem letzten Thema: kurz daran anknüpfen (Arbeit, Schlafen, Hobbys, Fantasie im Chat), persönlich und einladend.
- NICHT immer denselben Einstieg (z. B. „Ich habe an unser Gespräch gedacht“). Variiere: mal Hey, mal Oh, mal Na, mal direkt Frage, mal „musste an dich denken“, mal „wo bist du geblieben“ – je nach Kontext.`;

  const variationRule = `
VARIATION: Einstieg jedes Mal ANDERS – NICHT immer „Hey, ich habe gerade an unser Gespräch über X gedacht“. Abwechslung bei Formulierung und Frage.`;

  const lengthHint = `Länge: mindestens ${ASA_MIN_LENGTH} Zeichen, am besten 150–${ASA_IDEAL_MAX_CHARS} Zeichen (kurz und knackig, nicht ausschweifen).`;
  let strongInstructions = `Schreibe EINE kurze Reaktivierungsnachricht, die DIREKT am letzten Thema des Kunden anknüpft (nur an dem, was im Kontext unten steht).
Beispiele: Wenn der Kontext nur „Kunde: Hallo“ ist → freundlich grüßen + 1–2 Fragen, kein „unser Gespräch“. Wenn das letzte Thema Sex war → auf das GESPRÄCH/Fantasie im Chat Bezug nehmen. Wenn der Kunde sagte, er geht arbeiten → "Hoffe bei der Arbeit läuft alles gut, denkst du auch an mich?" Wenn er schlafen geht → "Hoffe du träumst was Schönes, hast du schonmal von mir geträumt?"
WICHTIG: Persönlich, zum KONTEXT passend (nur das, was unten steht), einladend. Keine generischen Floskeln. ${lengthHint} Nur die Nachricht, kein Prefix.`;
  if (noRealConversation) {
    strongInstructions = `WICHTIG: Der Kunde hat hier nur eine Freundschaftsanfrage/Like geschickt – es gab KEIN richtiges Gespräch über ein Thema.
VERBOTEN: „unser Gespräch über X“, „ich habe an unser letztes Gespräch gedacht“, „an unser Gespräch über Musik/Sex gedacht“ – das wäre falsch.
Stattdessen: kurze, freundliche Reaktivierung (z. B. Danke für die Anfrage / dass du dich gemeldet hast), dann 1–2 offene Fragen (wie geht es dir, was machst du, was magst du). Du darfst auf Profilinfos (Hobbys, Musik) eingehen, aber NICHT als „Gespräch“ bezeichnen – eher „ich seh du magst X“ oder „wie geht es dir“. ${lengthHint} Nur die Nachricht, kein Prefix.`;
  }

  let softInstructions = `Schreibe EINE kurze Reaktivierungsnachricht, die zeigt, dass du an ihn denkst/ihn vermisst, mit EIN BISSCHEN Kontext aus dem Verlauf (nur aus dem Kontext unten).
Tonalität: "Wo bist du geblieben?", "Ich musste an dich denken", "Du hast mir gefehlt" – plus leichter Bezug zum letzten Thema im Kontext, aber nicht das genaue Thema wiederholen.
${lengthHint} Nur die Nachricht, kein Prefix.`;
  if (noRealConversation) {
    softInstructions = `Der Kunde hat nur Freundschaftsanfrage/Like geschickt – kein richtiges Gespräch. Schreibe eine freundliche Reaktivierung OHNE „unser Gespräch über X“. Lieber: Danke/Kurz anknüpfen + 1–2 Fragen (wie geht es dir, was machst du). ${lengthHint} Nur die Nachricht, kein Prefix.`;
  }
...
- Stattdessen: konkret und persönlich formulieren, direkte Fragen oder Aussagen zum Thema/Situation (z. B. „Hast du schonmal …?“ / „Wie war’s bei der Arbeit?“ / „Woran hast du gerade gedacht?“), locker und wie im echten Chat.`;
  ...
  const systemContent = `Du bist eine weibliche Chat-Partnerin. Du schreibst eine Reaktivierungsnachricht (ASA), um den Kunden wieder ins Gespräch zu holen.
${personaBlock}
${formatRule}
${contextRule}
${variationRule}
...
Antworte NUR mit der einen Nachricht, nichts anderes.`;
  ...
  if (asaExamplesForPrompt && asaExamplesForPrompt.length > 0) {
    const texts = asaExamplesForPrompt.map((ex, i) => {
      ...
      return msg ? `[${i + 1}] ${msg.substring(0, 280)}${msg.length > 280 ? '...' : ''}` : '';
    }).filter(Boolean);
    if (texts.length > 0) {
      examplesBlock = `\n\nBeispiele für gute Reaktivierungsnachrichten (Stil/Variation – orientiere dich daran, formuliere aber NEU und zum Kontext passend):\n${texts.join('\n')}\n`;
    }
  }

  const userContent = `Letzter Austausch im Chat (NUR diese letzten Nachrichten – darauf beziehen):\n${conversationSnippet}\n${examplesBlock}\nSchreibe die Reaktivierungsnachricht (mind. ${ASA_MIN_LENGTH}, am besten 150–${ASA_IDEAL_MAX_CHARS} Zeichen). Denk an: Ihr habt euch nie getroffen – nur Bezug auf das, was oben steht; nie "unser erstes Mal" o.ä.:`;
    ...
    if (isASA) {
      ...
      try {
        ...
        if (asaExamplesRaw.length > 0) {
          ...
          // 🎲 ASA-Auswahl: Round-Robin + optional divers zum zuletzt verwendeten Text
          ...
          console.log(`✅ Agent: ASA-Beispiel Index ${chosenIdx} von ${pool.length} (Round-Robin${lastUsed ? ' + Diversität' : ''})`);
...
async function runMultiStagePipeline({
  ...
  // 🤖🤖🤖 ASA-FALL - Kontext-ASA (30min–4h stark, 4h–23h weich) oder Pool-ASA (nie geantwortet / 23+ h), mind. 150 Zeichen, ideal 150–250
  // Zeitquelle: 1) Zuverlässig = metaData.lastCustomerMessageAgeMs oder metaData.lastCustomerMessageTimestamp (Extension); 2) Fallback = Alter aus messages (getLast10MessagesAndCustomerAge), ggf. mit Plausibilitäts-Check.
  if (isASA) {
    console.log('🤖 ASA-Fall erkannt - prüfe Kontext vs. Pool');
    let messagesForASA = allMessages || [];
    // Blenny/DF: Falls separate moderatorMessages/customerMessages mitgegeben, als Fallback mergen (gleiche Kontext-Quelle für alle Plattformen)
    if (messagesForASA.length === 0 && (moderatorMessages?.length > 0 || customerMessages?.length > 0)) {
      ...
      if (messagesForASA.length > 0) console.log('🤖 ASA: Kontext aus moderatorMessages + customerMessages (Fallback, ' + messagesForASA.length + ' Nachrichten)');
    }
    ...
    if (lastCustomerMessageAgeMs != null && Number.isFinite(lastCustomerMessageAgeMs) && lastCustomerMessageAgeMs >= 0) {
      ...
      console.log('🤖 ASA: Alter aus Extension (lastCustomerMessageAgeMs):', (ageMinutes | 0) + ' Min');
      ...
      if (Number.isFinite(ts)) {
        ...
        ageMinutes = ageHours * 60;
        ageFromExtension = true;
        console.log('🤖 ASA: Alter aus Extension (lastCustomerMessageTimestamp):', (ageMinutes | 0) + ' Min');
      }
    }
    // Plausibilität nur wenn Alter aus messages kommt (Extension hat kein lastCustomerMessageAgeMs/Timestamp geschickt)
    let forcePoolBecauseUnreliableTimestamp = false;
    if (!ageFromExtension) {
      // Regel 1: Ohne Extension-Timestamp ist „sehr frisch“ (< 30 Min) aus messages unzuverlässig (UI zeigt oft „1 day ago“) → Pool
      const MIN_TRUSTED_AGE_HOURS = 0.5; // 30 Min – darunter nur mit Extension-Wert Kontext-ASA
      if (ageHours != null && ageHours < MIN_TRUSTED_AGE_HOURS) {
        forcePoolBecauseUnreliableTimestamp = true;
        console.log('🤖 ASA: Alter aus Messages < 30 Min, Extension hat kein Alter geschickt – Timestamp unzuverlässig (z.B. UI zeigt 1 Tag), Pool-ASA');
      } else {
        // Regel 2: „0 Min“ + mind. eine ältere Kundenmsg > 12 h → offensichtlich falscher Timestamp → Pool
        ...
        if (ageHours != null && ageHours < SUSPICIOUS_RECENT_MAX_HOURS && Array.isArray(customerAgeHoursAll) && customerAgeHoursAll.length > 1) {
          ...
          if (hasMuchOlder) {
            forcePoolBecauseUnreliableTimestamp = true;
            console.log('🤖 ASA: Letzte Kundenmsg ~0min, aber ältere Kundenmsg > 12h → Timestamp unwahrscheinlich, Pool-ASA');
          }
        }
      }
    }
    // Fallback: Wenn getLast10 aus allMessages keinen Kunden fand, aber wir Fallback-Messages haben → nur Kontext-ASA wenn darunter eine echte Kundennachricht ist (nicht nur Freundschaftsanfrage/Like/Kuss)
    const isFromCustomer = (m) => { const t = (m && (m.type || m.messageType || '')).toString().toLowerCase(); return t === 'received' || t === 'customer' || t === 'incoming' || t === 'user'; };
    if (!hasCustomerInLast10 && messagesForASA.length > 0) {
      const fallbackLast10 = messagesForASA.length <= 10 ? messagesForASA : messagesForASA.slice(-10);
      const hasRealCustomerInFallback = fallbackLast10.some(m => isFromCustomer(m) && isRealCustomerMessage(m));
      if (hasRealCustomerInFallback) {
        last10 = fallbackLast10;
        hasCustomerInLast10 = true;
        if (ageHours == null) ageHours = null;
        console.log('🤖 ASA: Echte Kundenmsg in Fallback-Kontext → Kontext-ASA');
      } else if (fallbackLast10.some(isFromCustomer)) {
        console.log('🤖 ASA: In letzten 10 nur Freundschaftsanfrage/Like/Kuss o.ä. – keine echte Kundennachricht → Pool-ASA');
      }
    }
    ...
    /** Bei Pool-ASA: das gewählte Beispiel (für trainingResult). */
    let selectedPoolChosen = null;

    // Kontext-ASA: Kunde in letzten 10 Nachrichten + Alter < 23 h; nicht wenn Timestamp unplausibel (0min obwohl ältere Kundenmsg > 12h)
    const canUseContext = hasCustomerInLast10 && !forcePoolBecauseUnreliableTimestamp && (ageHours == null || ageHours < ASA_POOL_FROM_HOURS);
    const asaExamplesForPrompt = asaExamples.length >= 2
      ? [...asaExamples].sort(() => Math.random() - 0.5).slice(0, 3)
      : asaExamples.slice(0, 3);
    if (canUseContext) {
      ...
      if (useStrong) {
        asaMessageText = await generateContextASA({ last10Messages: last10, mode: 'strong', profileInfo, extractedUserInfo, noRealConversation: asaNoRealCustomerMessage, asaExamplesForPrompt });
        useContextASA = asaMessageText.length > 0;
        if (useContextASA) console.log(`✅ ASA Kontext (stark, ${ageHours == null ? 'kein Timestamp' : (ageHours < 1 ? '~' + Math.round((ageHours * 60)) + 'min' : '~' + Math.round(ageHours) + 'h')}): ${asaMessageText.length} Zeichen`);
      } else if (useSoft) {
        asaMessageText = await generateContextASA({ last10Messages: last10, mode: 'soft', profileInfo, extractedUserInfo, noRealConversation: asaNoRealCustomerMessage, asaExamplesForPrompt });
        useContextASA = asaMessageText.length > 0;
        if (useContextASA) console.log(`✅ ASA Kontext (weich, ~${Math.round(ageHours)}h): ${asaMessageText.length} Zeichen`);
      }
    }
    if (!hasCustomerInLast10 && isASA) console.log('🤖 ASA: Kein Kunde in letzten 10 Nachrichten → Pool-ASA');
    if (hasCustomerInLast10 && ageHours != null && ageHours >= ASA_POOL_FROM_HOURS) console.log(`🤖 ASA: Alter letzte Kundenmsg ${Math.round(ageHours)}h ≥ 23h → Pool-ASA`);
    ...
    if (!useContextASA && asaExamples.length > 0) {
      ...
      console.log(`🎲 ASA Pool (${!hasCustomerInLast10 ? 'kein Kunde in letzten 10' : (ageHours >= ASA_POOL_FROM_HOURS ? '23+h' : 'Fallback')}): ${asaMessageText.length} Zeichen`);
    }

    if (asaMessageText) {
      asaMessageText = ensureASAMinLength(asaMessageText);
      // Gleiche Format-Regeln wie bei normaler Nachricht: keine Bindestriche, ß → ss (ASA = gleiche Regeln wie Generator)
