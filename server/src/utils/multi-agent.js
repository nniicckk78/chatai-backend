...
async function runStage3Generation({
  ...
  try {
    ...
    if (isTogether) {
      ...
      // 🚀🚀🚀 VEREINFACHT: Fine-Tuned Model hat Situationen bereits gelernt!
      // 🚨 KRITISCH: Wenn Situationen erkannt wurden, füge ALLE Regeln hinzu - keine Filterung!
      // Das Model kann mit mehreren Situationen umgehen und muss auf ALLE eingehen!
      if (situations && situations.length > 0) {
        // 🚨 KRITISCH: Füge Regeln für ALLE erkannten Situationen hinzu - keine Priorisierung!
        ...
        // 🚨 KRITISCH: Wenn "Sexuelle Themen" erkannt wurde, MUSS es in relevantSituations sein!
        const sexualSituation = situations.find(s => s.includes('Sexuell') || s.includes('sexuell'));
        if (sexualSituation && situationalResponses[sexualSituation] && !relevantSituations.includes(sexualSituation)) {
          relevantSituations.push(sexualSituation);
          console.log(`🚨 KRITISCH: "Sexuelle Themen" wurde erkannt und wird zu relevantSituations hinzugefügt!`);
        }
        ...
        if (relevantSituations.length > 0) {
          ...
          situationalRules += `🚨🚨🚨 KRITISCH: Gehe auf ALLE erkannten Situationen ein! Wenn mehrere Situationen erkannt wurden, reagiere auf ALLE! 🚨🚨🚨`;
          console.log(`📋 Situations-spezifische Regeln für Together.ai hinzugefügt: ${relevantSituations.join(', ')} (${relevantSituations.length} Situationen)`);
        } else {
          console.warn(`⚠️ WARNUNG: Situationen erkannt (${situations.join(', ')}), aber keine Regeln gefunden!`);
        }
      }
