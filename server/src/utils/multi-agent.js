 ...
 * Generiert Nachrichten in mehreren Stufen mit verschiedenen Agenten
 * Verwendet createPromptForTogetherAI für korrekten Prompt-Aufbau
 */
async function runMultiStagePipeline({
  ...
  try {
    console.log('🚀 STAGE 1: Initialisiere Multi-Stage Pipeline...');
    
    // Reset Shared Knowledge Base
    const sharedKB = getSharedKnowledgeBase();
    sharedKB.reset();
    
    // STAGE 1: Kontext-Analyse
    console.log('📊 STAGE 1: Kontext-Analyse...');
    const contextAnalysis = await runAgentWithFallback(
      agentContextAnalyst,
      'Context-Analyst',
      { topic: 'allgemein', summary: '', keyPoints: [], contextFlow: 'neutral', success: false },
      8000,
      conversationHistory,
      customerMessage,
      isASA
    ...
    console.log('🔍 STAGE 2: Situation-Detection...');
    const situationResult = await runAgentWithFallback(
      agentSituationDetector,
      'Situation-Detector',
      { situations: situations.length > 0 ? situations : ['allgemein'], success: false },
      15000,
      customerMessage,
      allRules,
      ...
      contextAnalysis,
      isASA,
      null // learningContextResult
    );
    
    const detectedSituations = situationResult.situations || situations;
    
    // STAGE 3: Training-Examples auswählen
    console.log('📚 STAGE 3: Training-Examples auswählen...');
    const trainingResult = await runAgentWithFallback(
      agentTrainingSelector,
      'Training-Selector',
      { selectedExamples: [], trainingExamplesContext: '', success: false },
      15000,
      contextAnalysis,
      customerMessage,
      detectedSituations,
    ...
    // STAGE 4: System-Prompt erstellen
    console.log('📝 STAGE 4: System-Prompt erstellen...');
    // Erstelle System-Prompt direkt (ohne buildSystemPrompt, da es getRules als Parameter erwartet)
...
3. Prägnant und natürlich: 120-250 Zeichen. Keine unnötigen Füllsätze oder Erklärungen.
...
- Sei prägnant - keine unnötigen Details oder Erklärungen`;
    ...
    if (allRules && allRules.forbiddenWords && allRules.forbiddenWords.length > 0) {
      systemPrompt += `\n\nVERBOTENE WÖRTER (NIEMALS verwenden):\n${allRules.forbiddenWords.slice(0, 30).join(', ')}`;
    }
    
    // Füge bevorzugte Wörter hinzu, falls vorhanden
    if (allRules && allRules.preferredWords && allRules.preferredWords.length > 0) {
      systemPrompt += `\n\nBEVORZUGTE WÖRTER (verwende regelmäßig):\n${allRules.preferredWords.slice(0, 30).join(', ')}`;
    }
    
    // STAGE 5: Letzte Moderator-Nachricht extrahieren
    const lastModeratorMessage = moderatorMessages.length > 0 
      ? moderatorMessages[moderatorMessages.length - 1]?.text || ''
      : '';
    
    // STAGE 6: Prompt mit createPromptForTogetherAI erstellen
    console.log('✍️ STAGE 6: Prompt erstellen mit createPromptForTogetherAI...');
    const { systemPrompt: cleanSystemPrompt, userPrompt } = createPromptForTogetherAI({
      conversationHistory: conversationHistory,
      customerMessage: customerMessage,
      lastModeratorMessage: lastModeratorMessage,
      systemPrompt: systemPrompt,
      rules: allRules
    });
    
    // STAGE 7: Nachricht generieren mit Together.ai
    console.log('🤖 STAGE 7: Generiere Nachricht mit Together.ai...');
    const messageClient = getMessageClient();
    const messageModel = getMessageModel();
    
    if (!messageClient) {
      throw new Error('Message Client nicht verfügbar');
    }
    
    // Füge Training-Examples zum User-Prompt hinzu (wenn vorhanden)
    let finalUserPrompt = userPrompt;
    if (trainingResult.trainingExamplesContext) {
      finalUserPrompt += '\n\n' + trainingResult.trainingExamplesContext;
    }
    ...
    let finalMessage = response.choices?.[0]?.message?.content?.trim() || '';
    
    // Post-Processing
    finalMessage = finalMessage.replace(/^["'„""]+/, '').replace(/["'"""]+$/, '').trim();
    finalMessage = finalMessage.replace(/-/g, " ");
    finalMessage = finalMessage.replace(/ß/g, "ss");
    
    if (!finalMessage || finalMessage.trim().length === 0) {
      throw new Error('Keine Nachricht generiert');
    }
    
    console.log(`✅ STAGE 7: Nachricht erfolgreich generiert (${finalMessage.length} Zeichen)`);
    
    return {
      finalMessage: finalMessage,
      stage2Examples: trainingResult,
    ...
    console.error('❌ Fehler in runMultiStagePipeline:', err.message);
    return {
      finalMessage: '',
      blocked: false,
      error: err.message,
...
/**
 * 🎨 Vereinfachte Pipeline (Fallback)
 */
async function runSimplifiedPipeline(params) {
  // Verwende runMultiStagePipeline als Basis
  return await runMultiStagePipeline(params);
}

/**
 * 🔄 Multi-Agent Pipeline (Fallback)
 */
async function runMultiAgentPipeline(params) {
  // Verwende runMultiStagePipeline als Basis
  return await runMultiStagePipeline(params);
}

/**
 * 📝 Agent Message Generator (Fallback für alte Pipelines)
 */
async function agentMessageGenerator(multiAgentResults, params) {
  // Wenn multiAgentResults bereits eine Nachricht hat, verwende sie
  if (multiAgentResults && multiAgentResults.finalMessage) {
    return {
      success: true,
      message: multiAgentResults.finalMessage
    };
  }
  
  // Andernfalls verwende runMultiStagePipeline
  const result = await runMultiStagePipeline({
    conversationHistory: params.conversationHistory || '',
    customerMessage: params.customerMessage || '',
    profileInfo: params.profileInfo || {},
    extractedUserInfo: params.extractedUserInfo || {},
    allRules: params.allRules || {},
    isASA: params.isASA || false,
    asaConversationContext: params.asaConversationContext || '',
    isMeetingRequestFunc: params.isMeetingRequestFunc || null
  });
...
module.exports = {
  ...
  runMultiStagePipeline,
  runSimplifiedPipeline,
  runMultiAgentPipeline,
  agentMessageGenerator,
  // Exportiere auch alle anderen Agent-Funktionen für Kompatibilität
  agentContextAnalyst,
  ...
  agentMultiSituationHandler,
  agentSituationDetector,
  agentFakeContextBuilder,
  agentConversationContextBuilder,
  agentLanguageDetector,
  agentContextConnectionAnalyzer,
  agentAgreementConsensusDetector,
  agentMetaValidator,
