      ...
      try {
        ...
        if (learningDataForScoring) {
          ...
          const processedVariants = variants
            ...
            .map(v => {
              let processed = v.replace(/^["'„""]+/, '').replace(/["'"""]+$/, '').trim();
          ...
          if (processedVariants.length > 0) {
            // Bewerte alle Varianten basierend auf Learning-Daten
            const scoredVariants = await Promise.all(
              processedVariants.map(async (variant) => {
                ...
                // Kombinierter Score: Learning (60%) + Quality (40%)
                const combinedScore = (learningScore * 0.6) + (qualityResultVariant.overallScore * 0.4);
            ...
            console.log(`✅ Multi-Generator: Beste Variante ausgewählt (Learning: ${bestVariant.learningScore.toFixed(1)}%, Quality: ${bestVariant.qualityScore.toFixed(1)}%, Combined: ${bestVariant.combinedScore.toFixed(1)}%)`);
