    ...
    if (isASA) {
      ...
      try {
        ...
        if (asaExamplesRaw.length > 0) {
          ...
          // 🚨 FIX: Bessere zufällige Auswahl - mehr Beispiele (5) für bessere Variation
          // Verwende Fisher-Yates Shuffle für echte Zufälligkeit
          const numToSelect = Math.min(5, Math.max(4, Math.floor(filteredASAExamples.length * 0.4))); // 4-5 Beispiele oder 40% wenn weniger vorhanden
          
          // 🚨 FIX: Fisher-Yates Shuffle für echte Zufälligkeit (besser als sort(() => Math.random()))
          ...
          console.log(`✅ Agent: Training-Selector - ${selectedASAExamples.length} ASA-Beispiele zufällig ausgewählt (von ${filteredASAExamples.length} verfügbar, Fisher-Yates Shuffle)`);
...
function buildSimplifiedPrompt(multiAgentResults, {
  ...
  if (training.selectedExamples && training.selectedExamples.length > 0) {
    ...
    if (isASA) {
      userPrompt += `\n\n📚📚📚📚📚 ASA-BEISPIELE (${training.selectedExamples.length} - KOPIERE FAST 1:1!) 📚📚📚📚📚\n\n`;
      userPrompt += `🚨🚨🚨🚨🚨 ABSOLUT KRITISCH FÜR ASA: KOPIERE FAST 1:1 AUS DEN BEISPIELEN! 🚨🚨🚨🚨🚨\n\n`;
      userPrompt += `🚨🚨🚨 DEINE AUFGABE: KOPIERE DAS BEISPIEL FAST 1:1 - NUR KLEINE VARIATIONEN ERLAUBT! 🚨🚨🚨\n\n`;
      userPrompt += `ANALYSIERE JEDES BEISPIEL GENAU:\n`;
      ...
      userPrompt += `- Wie viele Fragen werden gestellt? (1, 2, 3, 4?) - Verwende GENAU SO VIELE wie in den Beispielen!\n`;
      ...
      training.selectedExamples.slice(0, 5).forEach((ex, idx) => {
        ...
        userPrompt += `BEISPIEL ${idx + 1} (VOLLSTÄNDIG - KOPIERE FAST 1:1):\n`;
        userPrompt += `Kunde: "${customerText}${customerText.length >= 100 ? '...' : ''}"\n`;
        userPrompt += `Antwort: "${responseText}"\n\n`;
      });
      
      userPrompt += `🚨🚨🚨 KRITISCH: KOPIERE ALLES AUS DEN BEISPIELEN FAST 1:1 - NUR KLEINE VARIATIONEN ERLAUBT! 🚨🚨🚨\n`;
  ...
  // 🧠 NEU: Strukturierter Kontext (letzte 3-5 Nachrichten als Liste)
  if (conversationHistory && conversationHistory.trim().length > 0) {
    // Extrahiere letzte Nachrichten aus conversationHistory
    const historyLines = conversationHistory.split('\n').filter(line => line.trim().length > 0);
    const recentMessages = historyLines.slice(-10); // Letzte 10 Zeilen
  ...
  userPrompt += `SCHRITT 1: KONTEXT-ANALYSE\n`;
  userPrompt += `- Lies die letzten Nachrichten oben\n`;
  userPrompt += `- Was wurde bereits besprochen?\n`;
  userPrompt += `- Welche Fragen wurden bereits gestellt?\n`;
  userPrompt += `- Welche Fragen wurden bereits beantwortet?\n\n`;
  
  userPrompt += `SCHRITT 2: LOGIK-PRÜFUNG\n`;
  if (contextConnection.openQuestions && contextConnection.openQuestions.length > 0) {
    userPrompt += `- OFFENE FRAGEN (diese solltest du beantworten):\n`;
    contextConnection.openQuestions.forEach((q, idx) => {
      const questionText = typeof q === 'string' ? q : (q.text || q);
      userPrompt += `  ${idx + 1}. "${questionText}"\n`;
    });
    userPrompt += `\n`;
  }
  ...
  userPrompt += `- Wurde diese Frage bereits gestellt? → NICHT erneut stellen!\n`;
  userPrompt += `- Wurde diese Frage bereits beantwortet? → NICHT erneut stellen!\n`;
  userPrompt += `- Was wurde bereits besprochen? → NICHT wiederholen!\n\n`;
  
  userPrompt += `SCHRITT 3: ANTWORT-GENERIERUNG\n`;
  userPrompt += `- Gehe auf die aktuelle Kundennachricht ein\n`;
  ...
  userPrompt += `1. PRÜFE IMMER: Wurde diese Frage bereits gestellt/beantwortet?\n`;
  userPrompt += `2. VERSTEHE: Was wurde bereits besprochen?\n`;
  userPrompt += `3. DENKE: Was passt logisch als nächste Frage?\n`;
  userPrompt += `4. VERMEIDE: Wiederholungen, doppelte Fragen, bereits besprochene Themen\n`;
  userPrompt += `5. LOGISCH: Wenn Kunde etwas sagt, gehe darauf ein (nicht ignorieren!)\n`;
  userPrompt += `6. LOGISCH: Wenn Kunde eine Frage stellt, beantworte sie (nicht umgehen!)\n`;
  userPrompt += `7. 🚨🚨🚨 NEU: Bei direkten Fragen nach sexuellen Vorlieben → DIREKT antworten, NICHT defensiv ausweichen!\n`;
  userPrompt += `8. 🚨🚨🚨 NEU: Keine Meta-Kommentare (z.B. "Wow, das ist ja eine sehr direkte Frage", "Ich hoffe, du verstehst das?")\n\n`;
  ...
  userPrompt += `1. DENKE ZUERST (Schritte 1-3 oben)\n`;
  userPrompt += `2. PRÜFE LOGIK (keine doppelten Fragen, keine Wiederholungen)\n`;
  userPrompt += `3. GEHE auf die Kundennachricht ein\n`;
  userPrompt += `4. BEANTWORTE offene Fragen\n`;
  userPrompt += `5. STELLE eine NEUE, passende Frage (nicht bereits gestellt/beantwortet)\n`;
  if (contextUnderstanding && contextUnderstanding.shouldBeDirect) {
    userPrompt += `6. 🚨🚨🚨 KRITISCH: DIREKT antworten (nicht defensiv ausweichen, keine Meta-Kommentare!)\n`;
  } else if (contextUnderstanding && contextUnderstanding.shouldBeDefensive) {
    userPrompt += `6. 🚨🚨🚨 KRITISCH: DEFENSIV antworten (nicht zustimmen, aber freundlich!)\n`;
  }
  userPrompt += `\n`;
  ...
  // ============================================
  userPrompt += `\n\n🚨🚨🚨 FINALE ANWEISUNG 🚨🚨🚨\n`;
  userPrompt += `Antworte im extrahierten Stil (${globalStyle ? `${globalStyle.avgSentenceLength} Wörter pro Satz, ${globalStyle.avgSentenceCount} Sätze` : 'natürlich'}), `;
  userPrompt += `inspiriert von ähnlichen Beispielen, beachte Kontext und Regeln, aber schreibe natürlich und flexibel.\n`;
  userPrompt += `Die Nachricht MUSS eine Frage enthalten!\n`;
  userPrompt += `Mindestlänge: 150 Zeichen.\n`;
...
async function runStage3Generation({
  ...
  try {
    ...
    if (isTogether) {
      ...
      // Wenn Situationen erkannt wurden, füge situations-spezifische Regeln hinzu
      // 🚨 WICHTIG: "Sexuelle Themen" hat HÖCHSTE PRIORITÄT!
      if (situations && situations.length > 0) {
        // 🚨 NEU: Priorisiere "Sexuelle Themen" über andere Situationen
        const sexualSituation = situations.find(s => s.includes('Sexuell') || s.includes('sexuell'));
        const primarySituation = sexualSituation || situations[0]; // Nimm "Sexuelle Themen" wenn vorhanden, sonst erste Situation
...
${stage1Plan.isOngoingConversation ? '🚨 KRITISCH: Dies ist ein LAUFENDES Gespräch - KEINE Begrüßung mit "Hey" oder "Hallo"! 🚨' : ''}`;
      ...
      // 🚨 FIX: Erweiterter Kontext für besseres Verständnis
      let contextInfo = '';
      if (conversationHistory && conversationHistory.trim().length > 0) {
        const historyLines = conversationHistory.split('\n').filter(line => line.trim().length > 0);
        // 🚨 FIX: Mehr Kontext (letzte 5-7 Nachrichten, bis 300 Zeichen) für besseres Verständnis
        const recentHistory = historyLines.slice(-7).join(' '); // Letzte 7 Nachrichten
...
${stage1Plan.isOngoingConversation ? '🚨🚨🚨 KRITISCH: Dies ist ein LAUFENDES Gespräch - KEINE Begrüßung mit "Hey" oder "Hallo"! 🚨🚨🚨' : ''}`;
    ...
    // 6. FINALE ANWEISUNG
    userPrompt += `🚨🚨🚨 FINALE ANWEISUNG 🚨🚨🚨\n`;
    userPrompt += `1. FOLGE DEM PLAN (siehe oben)\n`;
    userPrompt += `2. ORIENTIERE DICH STRENG AN DEN BEISPIELEN (Struktur, Wortwahl, Fragen)\n`;
    userPrompt += `3. BEACHTE KRITISCHE ANWEISUNGEN (siehe oben)\n`;
    userPrompt += `4. BEANTWORTE OFFENE FRAGEN (siehe oben)\n`;
    userPrompt += `5. GEHE AUF NEUE INFORMATIONEN EIN (siehe oben)\n`;
...
async function runMultiStagePipeline({
  ...
  if (isASA) {
    ...
    if (asaExamples.length > 0) {
      const shuffled = [...asaExamples].sort(() => Math.random() - 0.5);
      const selectedASAs = shuffled.slice(0, Math.min(5, shuffled.length));
      ...
      console.log(`✅ ${selectedASAs.length} ASA-Beispiele zufällig ausgewählt`);
...
async function runSimplifiedPipeline({
  ...
  if (isASA) {
    ...
    if (asaExamples.length > 0) {
      // Zufällige Auswahl von 3-5 ASA-Beispielen
      const shuffled = [...asaExamples].sort(() => Math.random() - 0.5);
      const selectedASAs = shuffled.slice(0, Math.min(5, shuffled.length));
