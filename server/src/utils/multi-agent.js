...
function buildSimplifiedPrompt(multiAgentResults, {
  ...
  if (isASA) {
    userPrompt += `\n\n🤖🤖🤖 ASA-REAKTIVIERUNG 🤖🤖🤖\n`;
    userPrompt += `Du schreibst eine REAKTIVIERUNGSNACHRICHT (ASA).\n`;
    userPrompt += `- Nutze die Beispiele oben als Inspiration\n`;
    userPrompt += `- Stelle eine Frage, um das Gespräch wieder anzuregen\n`;
    userPrompt += `- Sei natürlich und authentisch\n\n`;
 ...
 */
async function aiSelfValidate(message, customerMessage, context, understoodRules, understoodLearning, contextUnderstanding = null) {
  const client = getClient();
  if (!client) {
 ...
 */
async function aiSelfCorrect(message, issues, customerMessage, context, understoodRules, understoodLearning, contextUnderstanding = null) {
  const client = getClient();
  if (!client) {
