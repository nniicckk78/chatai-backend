...
/**
 * Kombiniert genau 2 ASA-Texte schlau zu EINER kohärenten Nachricht.
 * Analysiere Einstieg / Mittelteil / Schlussfrage und baue daraus eine natürlich fließende Nachricht.
...
async function combineASAExamples(asaTexts) {
  ...
  const systemContent = `Du kombinierst ZWEI Reaktivierungsnachrichten (ASA) zu EINER kohärenten Nachricht.

Schritt 1 – Analysiere beide:
- Nachricht A und B haben jeweils: Einstieg (z.B. Hey/Oh/Na + kurzer Satz), evtl. Mittelteil (Gefühl, Vermissen), Schlussfrage (z.B. wie geht es dir, was machst du).

Schritt 2 – Baue EINE Nachricht:
- Wähle z.B. den Einstieg von Nachricht A und die Schlussfrage von Nachricht B (oder umgekehrt). Oder nimm den ersten Satz von A und den zweiten von B, so dass es natürlich klingt.
- Wichtig: Es muss wie EINE geschriebene Nachricht wirken, nicht wie zwei aneinandergereihte Sätze. Übergänge anpassen (z.B. "und" weglassen wenn doppelt).
- NUR Formulierungen aus den beiden gegebenen Nachrichten verwenden – nichts dazuerfinden.
...
- Ausgabe: NUR die eine kombinierte Nachricht, sonst nichts.`;
  const userContent = `Nachricht A:\n${a}\n\nNachricht B:\n${b}\n\nKombiniere zu EINER flüssigen Nachricht (Teile von A + Teile von B):`;
  try {
