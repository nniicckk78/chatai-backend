...
const { selectOneASADiverseFromLast } = require('./grok-pipeline');

// 🎲 ASA-Auswahl: möglichst unterschiedlich zum zuletzt verwendeten (diverse Variation)
...
let lastUsedASASignature = null;
let lastUsedASAText = null; // Erste 120 Zeichen der zuletzt verwendeten ASA – für diverse Auswahl
    ...
    if (isASA) {
      ...
      try {
        ...
        if (asaExamplesRaw.length > 0) {
          ...
          // 🎲 ASA-Auswahl: möglichst unterschiedlich zum zuletzt verwendeten (diverse Variation, grok-pipeline-Helper)
          const pool = filteredASAExamples;
          const result = selectOneASADiverseFromLast(pool, lastUsedASAText);
          const chosen = result ? result.chosen : pool[Math.floor(Math.random() * pool.length)];
          if (result && result.signatureText) lastUsedASAText = result.signatureText;
          lastUsedASASignature = getASASignature(chosen);
          const selectedASAExamples = [chosen];
          console.log(`✅ Agent: ASA-Beispiel Index ${pool.indexOf(chosen)} von ${pool.length} (diverse Auswahl${lastUsedASAText ? ' vs. zuletzt verwendet' : ''})`);
