...
// 🎲 ASA-Auswahl: Alle Beispiele als Kandidaten, „least recently used“ + Diversität – mehr Variation, keine feste Round-Robin-Fenster
/** Zuletzt verwendete ASA-Indizes (neuestes vorn), für „least recently used“ – weniger oft die gleichen. */
const asaLastUsedIndices = [];
const ASA_LAST_USED_MAX = 40;

/**
 * ASA-Fairness (global, ohne ChatID):
 * - usageCount: wie oft wurde Beispiel i genutzt (RAM, pro Prozess)
 * - recentPicks: zuletzt verwendete Indizes (RAM, pro Prozess) → Recency-Block
 *
 * Ziel: Lieblingsmuster durchbrechen, seltene Beispiele bevorzugen, ohne externe DB.
 */
const asaUsageCount = new Map(); // idx -> count
const asaRecentPicks = []; // newest last (push), contains indices

function getAsaRecentWindowSize(poolSize) {
  const n = Number(poolSize) || 0;
  if (n <= 0) return 0;
  // Bei 140 Beispielen ~56; wächst moderat mit.
  const dyn = Math.floor(n * 0.4);
  return Math.max(20, Math.min(70, dyn));
}

function pickWeightedIndex(indices, weights) {
  if (!indices || indices.length === 0) return -1;
  let total = 0;
  for (let i = 0; i < indices.length; i++) total += Math.max(0, Number(weights[i]) || 0);
...
function buildAsaEligibility(pool, { exclude = new Set(), recentSet = new Set() } = {}) {
  const eligible = [];
  const weights = [];
  for (let i = 0; i < pool.length; i++) {
    if (exclude.has(i)) continue;
    if (recentSet.has(i)) continue;
    const used = asaUsageCount.get(i) || 0;
    // Seltene Beispiele bevorzugen: 1/(1+used)
    const w = 1 / (1 + used);
    eligible.push(i);
    weights.push(w);
  }
...
/** Erste ~120 Zeichen der zuletzt gesendeten Moderator-Nachricht aus conversationHistory (für Diversität). */
...
/**
 * Wählt ein ASA-Beispiel aus dem gesamten Pool: alle N als Kandidaten, bewertet nach
 * „least recently used“ (Frische) und Diversität zum zuletzt verwendeten Text – maximale
 * Streuung über alle ASAs, keine festen Round-Robin-Fenster mehr.
...
function selectOneASARoundRobinWithDiversity(pool, lastUsedText) {
  ...
  const N = pool.length;
  const useDiversity = lastUsedText && String(lastUsedText).trim().length >= 20 && N >= 2;
  const lastSig = useDiversity ? String(lastUsedText).toLowerCase().replace(/\s+/g, ' ').trim().slice(0, 150) : '';

  // Frische: wie lange nicht genutzt (höher = länger nicht genutzt)
  const freshness = (i) => {
    const pos = asaLastUsedIndices.indexOf(i);
    return pos === -1 ? ASA_LAST_USED_MAX + 1 : ASA_LAST_USED_MAX - pos;
  };

  let best = { ex: pool[0], index: 0 };
  let bestScore = -1;
  for (let i = 0; i < N; i++) {
    const ex = pool[i];
    const fresh = freshness(i);
    const sig = (ex.asaMessage || ex.moderatorResponse || '').trim().slice(0, 150).toLowerCase().replace(/\s+/g, ' ');
    const div = useDiversity ? asaTextDistance(sig, lastSig) : 0.6;
    const score = fresh * 1.5 + div * 2.5;
    if (score > bestScore) {
      bestScore = score;
      best = { ex, index: i };
    }
  }
  asaLastUsedIndices.unshift(best.index);
  if (asaLastUsedIndices.length > ASA_LAST_USED_MAX) asaLastUsedIndices.length = ASA_LAST_USED_MAX;
  return { chosen: best.ex, index: best.index };
}

/** Kompatibilität: ein ASA auswählen (LRU + Diversität über ganzen Pool). lastUsedText kann null sein. */
...
/**
 * Wählt genau 2 ASA-Beispiele per Zufall – beide unterschiedlich (nie zweimal dieselbe).
 * Gleichmäßige Streuung über alle ASAs ohne Gedächtnis.
 */
function selectTwoASAs(pool, lastUsedText) {
  if (!pool || pool.length === 0) return [];
  const N = pool.length;
  if (N === 1) return [pool[0]];

  const windowSize = getAsaRecentWindowSize(N);
  const recentSlice = windowSize > 0 ? asaRecentPicks.slice(-windowSize) : [];
  const recentSet = new Set(recentSlice);

  const exclude = new Set();
  let { eligible, weights } = buildAsaEligibility(pool, { exclude, recentSet });
  // Falls zu viel gesperrt ist (z.B. kurz nach Start oder wenige Beispiele), lockern wir Recency.
  if (eligible.length < 2) {
    recentSet.clear();
    ({ eligible, weights } = buildAsaEligibility(pool, { exclude, recentSet }));
  }
  const firstIdx = pickWeightedIndex(eligible, weights);
  exclude.add(firstIdx);

  ({ eligible, weights } = buildAsaEligibility(pool, { exclude, recentSet }));
  if (eligible.length === 0) {
    // Notfall: irgendein anderer Index
    const alt = [];
    for (let i = 0; i < N; i++) if (i !== firstIdx) alt.push(i);
    eligible = alt;
    weights = alt.map(i => 1 / (1 + (asaUsageCount.get(i) || 0)));
  }
  const secondIdx = pickWeightedIndex(eligible, weights);

  // Update RAM-State
  asaUsageCount.set(firstIdx, (asaUsageCount.get(firstIdx) || 0) + 1);
  asaUsageCount.set(secondIdx, (asaUsageCount.get(secondIdx) || 0) + 1);
  asaRecentPicks.push(firstIdx, secondIdx);
  // Nicht unendlich wachsen lassen (nur Sicherheitskappe)
  if (asaRecentPicks.length > 2000) asaRecentPicks.splice(0, asaRecentPicks.length - 2000);

  if (process.env.NODE_ENV !== 'production') {
    const c1 = asaUsageCount.get(firstIdx) || 0;
    const c2 = asaUsageCount.get(secondIdx) || 0;
    console.log(`🎲 ASA Fair-Pick: idxA=${firstIdx} (used=${c1}), idxB=${secondIdx} (used=${c2}), recentWindow=${windowSize}/${N}`);
  }
  return [pool[firstIdx], pool[secondIdx]];
}

// ——— ASA-Kontext (Reaktivierung mit/ohne Kontext) ———
// WANN WELCHE ASA (Kontext-Stärke):
// - Kein Kunde in den letzten 10 Nachrichten (siteInfos.messages) → POOL-ASA.
// - Letzte Kundennachricht ≥ 23 Std → POOL-ASA.
// - Sonst (0 Min bis < 23 Std): KONTEXT-ASA. Stark bis 4 Std, weich 4–23 Std.
// - 0 Min bis < 4 Std → KONTEXT-ASA STARK (direkt am letzten Thema/Satz des Kunden anknüpfen). Auch bei 0min (z. B. Blenny sendet aktuelle Kundenmsg mit „jetzt“).
// - 4 Std ≤ Alter < 23 Std → KONTEXT-ASA WEICH („wo bist du“, „vermisst“, etwas Kontext aus Verlauf).
// - Kein Timestamp an letzter Kundennachricht → KONTEXT-ASA STARK (Fallback).
...
/** Prüft, ob eine Nachricht eine echte Kundennachricht ist (nicht System/Info wie Kuss, Like, Freundschaftsanfrage, Reaktivierungshinweis). Wenn nur solche in den letzten 10 sind → ASA-Pool. */
function isRealCustomerMessage(msg) {
  ...
  // Kuss / Gruß System
  if (/der (benutzer|kunde|user) hat (dich )?geküsst|hat dir (einen |ein )?kuss gesendet|hat (einen |ein )?gruß gesendet|ich habe dir einen kuss geschickt|geküsst.*schreib|schreib(e)? (ihm|ihr) eine nachricht/i.test(lower)) return false;
  if (lower.length < 80 && /kuss|gruß/.test(lower) && /gesendet/.test(lower)) return false;
  // ChatsX/S69 u. ä.: sichtbare Kunden-Bubble „Kuss Nachricht!💋“ (Sticker) — wie reply.js isKussSystemMessage; sonst ASA-Kontext hängt an altem Kuss trotz Fake-Antwort danach
  if (/kuss\s*nachricht/i.test(lower)) return false;
  // Like / geliked
  if (/like erhalten|hat dich gelikt|hat dich geliked|like bekommen|ich habe dir einen like geschickt|geliked/i.test(lower)) return false;
  if (lower.length < 100 && /geliked|like erhalten|hat dich gelikt|like bekommen/.test(lower)) return false;
  // Freundschaftsanfrage
  if (/freundschaftsanfrage|(friend|freund).*request|als freund hinzufügen|möchte dich als freund|benutzer möchte dich als freund/i.test(lower)) return false;
  // Favoriten / Reaktivierung
  if (/bitte motiviere|reaktivierung|motiviere den kunden|motivier den kunden|motivier.*wieder mit dir zu schreiben/i.test(lower)) return false;
  if (/favoriten.*hinzugefügt|zu (seinen|ihren|deinen) favoriten|mitteilen.*dich.*favoriten/i.test(lower)) return false;
  // Bild-System / Sonstige Info
  if (/ein bild wurde übertragen|bild.*gesendet|bild.*übertragen/i.test(lower)) return false;
  if (/^(huhu|hi|hey)[\s,!.]*$/i.test(t)) return true;
  return true;
}

/** ASA: Kontext enthält eindeutige Plattform-Kuss-Systemzeile (nicht beliebiges „kuss“ im Verlauf). */
function asaConversationContextIsKussSystemLine(ctx) {
  if (!ctx || typeof ctx !== 'string') return false;
  const t = ctx.toLowerCase();
  return /kuss\s*nachricht|ich habe dir einen kuss|der benutzer hat dich geküsst|benutzer hat dich geküsst|hat dir (einen|ein) kuss (geschickt|gesendet)|\bhat dich geküsst\b/.test(t);
}

/** ASA: Kontext enthält eindeutige Plattform-Like-Systemzeile. */
function asaConversationContextIsLikeSystemLine(ctx) {
  if (!ctx || typeof ctx !== 'string') return false;
  const t = ctx.toLowerCase();
  return /\bgeliked\b|hat dich geliked|\bgelikt\b|like erhalten|like bekommen|ich habe dir einen like|der benutzer hat dich geliked/.test(t);
}

/**
 * Liefert die letzten 10 Nachrichten (chronologisch, älteste zuerst) und ermittelt die letzte Kundennachricht sowie deren Alter.
 * @param {Array} allMessages - siteInfos.messages (kann neueste-zuerst oder älteste-zuerst sein)
...
function getLast10MessagesAndCustomerAge(allMessages) {
  ...
  // Chronologisch (älteste zuerst); Nachrichten ohne Timestamp ans Ende, dann slice(-10) = letzte 10
  let sorted = [...withTime].sort((a, b) => (a._ts || 0) - (b._ts || 0));
  if (withoutTime.length) sorted = [...sorted, ...withoutTime];
  const last10 = sorted.slice(-10);
  out.last10 = last10;

  // Kunde = received/customer/incoming (Blenny/DF können andere Felder nutzen)
...
/**
 * Stellt sicher, dass eine ASA-Nachricht mindestens ASA_MIN_LENGTH Zeichen hat (z. B. durch Anhängen eines kurzen Satzes).
 */
function ensureASAMinLength(text, minLen = ASA_MIN_LENGTH) {
  const t = (text || '').trim();
  if (t.length >= minLen) return t;
  const add = ' Ich würde mich freuen, von dir zu hören.';
  return (t + add).trim().slice(0, 500);
}

/**
 * ASA: keine Bindestriche/Gedankenstriche, keine Anführungszeichen, keine Doppelpunkte (Plattform-Vorgabe).
 */
function sanitizeASAForbiddenChars(text) {
  if (text == null) return '';
  let t = String(text)
    .replace(/\u2013|\u2014|\u2212/g, ' ')
    .replace(/-/g, ' ')
...
/**
 * Kombiniert genau 2 ASA-Texte zu EINER kohärenten Nachricht (OpenAI).
 * Inhaltliche Ideen aus A+B, aber sprachlich paraphrasiert (andere Satzanfänge/Wortwahl), damit Texte weniger wortgleich zu Vorlagen/History werden.
 * @param {string[]} asaTexts - genau 2 ASA-Nachrichten
 * @returns {Promise<string>} kombinierte Nachricht oder '' bei Fehler
 */
async function combineASAExamples(asaTexts) {
  if (!asaTexts || asaTexts.length < 2) return (asaTexts && asaTexts[0]) ? String(asaTexts[0]).trim() : '';
  const client = getClient();
  if (!client) return '';
  const a = (asaTexts[0] || '').trim();
  const b = (asaTexts[1] || '').trim();
  if (a.length < 30 || b.length < 30) return a || b;
  const systemContent = `Du kombinierst ZWEI Reaktivierungsnachrichten (ASA) zu EINER kohärenten Chat-Nachricht.

Schritt 1 – Verstehen:
- A und B sind Stil-Vorlagen: Einstieg, evtl. Mittelteil (z.B. fehlende Meldung, leichtes Drängeln, flirtig), oft eine kurze Frage am Schluss.
- Übernimm die GESAMTINTENTION (Reaktivierung, Ton, ungefähre Aussage), nicht den Wortlaut wörtlich.

Schritt 2 – EINE flüssige Nachricht bauen:
- Mische sinnvoll aus beiden (z.B. Einstiegsgedanke aus A, Nachfrage aus B, oder umgekehrt), sodass es wie EIN Autor klingt – nicht wie zwei Sätze aneinandergeklebt.
- PARAPHRASIEREN (Pflicht): Andere Satzanfänge als in A und B, andere Wortwahl und Satzbau wo möglich. Keine wortgleichen Kopien langer Phrasen aus nur einer Vorlage; lieber sinngemäß umschreiben.
- Du darfst einzelne Wörter/Kurzphrasen behalten, aber die Nachricht soll sich spürbar von beiden Vorlagen unterscheiden (kein fast identischer Gesamttext).

Schritt 3 – Strikte INHALTS-Grenzen (kein „freies Neuerfinden“):
- KEINE Treffen, Dates, reale Treffpunkte, Uhrzeiten oder Orte vorschlagen oder andeuten (auch nicht „wollen wir uns mal …“, „lass uns treffen“, „wann passt es dir“).
- KEINE neuen konkreten Versprechen (Bilder schicken, Telefonnummer, WhatsApp, draußen sehen, „schreib mir auf …“).
- KEINE neuen Themen oder Storys erfinden, die in A oder B nicht vorkommen; bleib im Rahmen dessen, was die beiden Texte transportieren (nur anders formuliert).
- Chat-only: Ihr habt euch nicht getroffen – nicht so tun, als gäbe es ein echtes Treffen oder ein gemeinsames Erlebnis in der Realität.

Format (Pflicht, sonst ungültig):
- Länge etwa 150 bis 250 Zeichen. Immer ss, nie ß.
- KEINE Bindestriche, KEINE Gedankenstriche und KEIN Minus in der Nachricht (Wörter wie kennenlernen als ein Wort oder mit Leerzeichen).
- KEINE Anführungszeichen irgendwo in der Nachricht (weder " noch ' noch typografische Varianten).
- KEINE Doppelpunkte in der Nachricht, stattdessen Komma (z. B. "Sag mal, wie geht es" statt "Sag mal: wie geht es").
- Ausgabe: NUR die eine kombinierte Nachricht, sonst nichts.`;
  const userContent = `Nachricht A:\n${a}\n\nNachricht B:\n${b}\n\nKombiniere zu EINER flüssigen, paraphrasierten Reaktivierungsnachricht ohne Bindestriche, ohne Anführungszeichen und ohne Doppelpunkte (Ideen aus A und B, deutlich andere Formulierung und Satzanfänge als in den Vorlagen):`;
  try {
    ...
    const cleaned = sanitizeASAForbiddenChars(raw.replace(/^["']|["']$/g, '').trim());
    return cleaned.length >= 80 ? ensureASAMinLength(cleaned) : '';
  } catch (err) {
    console.error('❌ combineASAExamples:', err.message);
    return '';
  }
}

/**
 * Generiert eine kontextbezogene ASA (stark oder weich) per LLM. — NICHT MEHR VERWENDET (ASA = nur Pool + Mix).
 * Kontext = NUR der letzte Austausch (letzte 2–4 Nachrichten).
 * @param {{ last10Messages: Array, mode: 'strong'|'soft', profileInfo: object, extractedUserInfo: object, noRealConversation?: boolean, asaExamplesForPrompt?: Array }} opts
 * @returns {Promise<string>} Nachricht mind. ASA_MIN_LENGTH Zeichen
 */
async function generateContextASA(opts) {
  const { last10Messages, mode, profileInfo = {}, extractedUserInfo = {}, noRealConversation = false, asaExamplesForPrompt = [] } = opts;
  ...
  const lastExchange = (last10Messages || []).slice(-4);
  const lines = lastExchange
...
- Du bist ${fakeName}, eine weibliche Chat-Partnerin. Du schreibst nur im Chat – ihr habt euch NIE getroffen und werdet euch nicht treffen.
- Alles über Treffen, Sex, Dates im Chat ist NUR HYPOTHETISCH („wenn wir uns mal treffen würden“, Fantasie). Es hat kein reales „erstes Mal“, kein reales Treffen, kein „als wir uns gesehen haben“ gegeben.
- VERBOTEN: So tun, als hättet ihr euch schon getroffen oder etwas gemeinsam erlebt. Z. B. NIEMALS: „unser erstes Mal“, „erinnerst du dich an unser Treffen“, „als wir uns getroffen haben“, „beim letzten Mal“, „wie beim letzten Mal“, „wenn wir uns das nächste Mal sehen“ (ihr habt euch nie gesehen). Erlaubt: „unser Gespräch“, „als wir darüber geschrieben haben“, „was wir uns geschrieben haben“, Fantasie über „wenn es mal soweit wäre“.`;

  const formatRule = `
FORMAT (wie beim normalen Generator): Immer ss, nie ß. KEINE Bindestriche (z. B. Hip Hop, kennenlernen, nicht Hip-Hop oder kennen-lernen). Keine Anführungszeichen am Anfang/Ende.`;

  const examplesFirstRule = `
PRIORITÄT – BEISPIELE ZUERST:
- Deine HAUPTVORLAGE sind die ASA-BEISPIELE unten. Stil, Länge (ca. 150–250 Zeichen), Struktur (Einstieg + kurze Reaktion + Frage) und Formulierungen sollen wie in den Beispielen sein.
- Schreibe NICHT frei aus dem Kontext heraus. Nutze die Beispiele als Vorlage: ähnlicher Einstieg (Hey/Oh/Na), ähnliche Satzstruktur, ähnliche Art von Frage. Du darfst nur LEICHT variieren (andere Formulierung, aber gleicher Stil).
- Keine Formulierungen erfinden, die nicht im Stil der Beispiele sind (z. B. kein „Ich habe an unser Gespräch über X gedacht“, wenn die Beispiele so nicht klingen).`;

  const contextWeakRule = `
KONTEXT NUR DEZENT:
- Der „Letzte Austausch“ unten dient nur als leichter Anknüpfungspunkt. Du musst ihn NICHT ausführlich aufgreifen.
- Wenn der Kontext nur „Kunde: Hallo“ oder „Hey“ ist: ignoriere inhaltlich, schreibe einfach im Stil der Beispiele (freundlich + Frage).
- Wenn der Kontext ein Thema hat (Arbeit, Schlafen, Hobbys): du darfst EINEN kurzen Halbsatz oder Satz daran anknüpfen (z. B. „Hoffe bei der Arbeit läuft’s“), der Rest wie in den Beispielen. Nicht mehr als ein leichter Touch Kontext.
- VERBOTEN: lange Kontext-Interpretation, „unser Gespräch über X“, erfundene Themen.`;

  const lengthHint = `Länge: wie in den Beispielen (mind. ${ASA_MIN_LENGTH}, ideal 150–${ASA_IDEAL_MAX_CHARS} Zeichen).`;
  let strongInstructions = `Schreibe EINE Reaktivierungsnachricht: ORIENTIERE DICH AN DEN BEISPIELEN (Stil, Struktur, Länge). Kontext nur dezent einbauen (ein Halbsatz reicht). ${lengthHint} Nur die Nachricht, kein Prefix.`;
  if (noRealConversation) {
    strongInstructions = `Der Kunde hat nur Freundschaftsanfrage/Like geschickt – kein Gespräch. Schreibe NUR im Stil der Beispiele (Danke/Kurz + 1–2 Fragen). KEIN „unser Gespräch über X“. ${lengthHint} Nur die Nachricht, kein Prefix.`;
  }

  let softInstructions = `Schreibe EINE Reaktivierungsnachricht: HAUPTsächlich wie die Beispiele (Stil, Struktur, Länge). Kontext nur ganz leicht einbauen (optional). ${lengthHint} Nur die Nachricht, kein Prefix.`;
  if (noRealConversation) {
    softInstructions = `Kunde hat nur Like/Freundschaftsanfrage. Schreibe im Stil der Beispiele, OHNE „unser Gespräch über X“. ${lengthHint} Nur die Nachricht, kein Prefix.`;
  }

  const treffenRule = `
🚫 TREFFEN-REGEL (gilt auch für ASA): Du darfst NIEMALS ein Treffen vorschlagen oder andeuten.
- VERBOTEN: Treffen vorschlagen/andeuten, zustimmen, Orte oder Aktivitäten für ein Treffen nennen (z. B. Café, Park, Spaziergang, Date, Kaffee, "wann können wir uns treffen", "sollen wir uns mal treffen", "bei dir / bei mir", "kennenlernen" im Sinne von realem Treffen).
- Die Nachricht soll nur reaktivieren (ansprechen, vermissen, am letzten Thema anknüpfen) – KEIN Treffen, KEIN Termin, KEINE Einladung zu einem realen Treffen.`;

  const noMetaPhrasesRule = `
🚫 KEINE META-PHRASEN / KEINE REFLEXIONS-FRAGEN:
- VERBOTEN: Unnatürliche, „reflektierende“ Formulierungen wie „Was reizt dich am meisten dabei?“, „Was findest du daran spannend?“, „Was denkst du darüber?“, „Was gefällt dir daran?“, „Was macht das mit dir?“ – klingen steif und wie eine Umfrage.
- Stattdessen: konkret und persönlich formulieren, direkte Fragen oder Aussagen zum Thema/Situation (z. B. „Hast du schonmal …?“ / „Wie war’s bei der Arbeit?“ / „Woran hast du gerade gedacht?“), locker und wie im echten Chat.`;

  const systemContent = `Du bist eine weibliche Chat-Partnerin. Du schreibst eine Reaktivierungsnachricht (ASA).
${personaBlock}
${formatRule}
${examplesFirstRule}
${contextWeakRule}
...
Antworte NUR mit der einen Nachricht, nichts anderes.`;
  ...
  let examplesBlock = '';
  if (asaExamplesForPrompt && asaExamplesForPrompt.length > 0) {
    const texts = asaExamplesForPrompt.map((ex, i) => {
      const msg = (ex.asaMessage || ex.moderatorResponse || (typeof ex === 'string' ? ex : '')).trim();
      return msg ? `[${i + 1}] ${msg.substring(0, 320)}${msg.length > 320 ? '...' : ''}` : '';
    }).filter(Boolean);
    if (texts.length > 0) {
      examplesBlock = `\n\n--- ASA-BEISPIELE (HAUPTVORLAGE – Stil, Länge und Struktur übernehmen, nur leicht variieren) ---\n${texts.join('\n')}\n`;
    }
  }

  const userContent = `${examplesBlock}\n--- Letzter Austausch (nur optional/dezent einbauen) ---\n${conversationSnippet}\n\nSchreibe EINE Reaktivierungsnachricht: vor allem wie die Beispiele oben, Kontext nur wenn passend mit einem Halbsatz. Länge wie in den Beispielen. Kein "unser erstes Mal", kein Treffen. Nur die Nachricht:`;

  const client = getClient();
  if (!client) {
    console.warn('⚠️ generateContextASA: OpenAI-Client nicht verfügbar, Fallback auf Pool-ASA');
    return '';
  }
  try {
    const res = await client.chat.completions.create({
    ...
    console.error('❌ generateContextASA:', err.message);
    return '';
  }
}
    ...
    if (isASA) {
      ...
      try {
        ...
        if (asaExamplesRaw.length > 0) {
          ...
          // 🎲 ASA-Auswahl: LRU über ganzen Pool + Diversität zum zuletzt verwendeten Text
          ...
          console.log(`✅ Agent: ASA-Beispiel Index ${chosenIdx} von ${pool.length} (LRU${lastUsed ? ' + Diversität' : ''})`);
...
- einen anderen Termin vorschlagen oder erfragen (z. B. "Was hältst du davon, wenn wir es morgen versuchen?", "übermorgen?", "nächste Woche?", "wann passt es dir?" – das wäre ein indirekter Treffen-Vorschlag)
...
- Beim Ausweichen KEINEN Ersatztermin nennen oder erfragen – kein "morgen versuchen", "nächste Woche?", "wann passt es dir?" (das wäre indirekter Treffen-Vorschlag). Ausweichen = im Chat bleiben, ohne anderen Tag vorzuschlagen.
    ...
    if (allRules && allRules.situationalResponses && typeof allRules.situationalResponses === 'object') {
      for (const [situationName, situationResponse] of Object.entries(allRules.situationalResponses)) {
        ...
        if (!matchesSituation) {
          ...
          // False-Positive Schutz: "später/ nachmittag weiter schreiben" ist KEIN Treffen.
          const isJustChatContinue = /\b(weiter\s+schreiben|weiter\s+chatten|später\s+weiter|nachher\s+weiter|am\s+nachmittag\s+weiter|heute\s+nachmittag\s+weiter|später\s+nochmal\s+schreiben)\b/i.test(lowerMessage);
          if (isJustChatContinue) {
            matchesSituation = false;
          } else {
...
- VERBOTEN an den Kunden: "nicht erlaubt", "Regeln", "Plattform", "echte Kontaktdaten verboten/dürfen nicht" – klingt nach System-Regel; nur persönlich ausweichen.`;
    ...
    if (isLocationQuestionFunc && typeof isLocationQuestionFunc === 'function' && isLocationQuestionFunc(customerMessage)) {
      ...
      const customerCity = customerInfo.city || customerInfo.wohnort || extractedUserInfo?.user?.Wohnort || extractedUserInfo?.user?.city || extractedUserInfo?.user?.wohnort || null;
      const statedOwnLocation = customerStatesOwnLocationInMessage(customerMessage || '');
      const skipAskCustomerWhere = (!!customerCity && String(customerCity).trim() !== '' && String(customerCity).toLowerCase() !== 'sag ich später') || statedOwnLocation;
      ...
      if (fakeCity && fakeCity.trim() !== "" && fakeCity.toLowerCase() !== "sag ich später") {
        ...
        if (skipAskCustomerWhere) {
          specificInstructions += `🚨 KUNDE-WOHNORT SCHON BEKANNT (Profil oder diese Nachricht): NIEMALS "Woher kommst du?", "Wo wohnst du?" oder "Wo kommst du her?" – der Kunde hat seinen Ort schon genannt.\n`;
          specificInstructions += `✅ RICHTIG: "Ich wohne in ${fakeCity} …" + andere Frage (Stimmung, Freizeit, wonach er sucht, wie der Tag läuft).\n`;
        } else {
          specificInstructions += `✅ RICHTIGE STRUKTUR (nur wenn Kundenort wirklich unbekannt): "Ich wohne in ${fakeCity}. Woher kommst du denn?" ODER "Ich komme aus ${fakeCity}. Und du?"\n`;
        }
        ...
        if (nearbyCity) {
          ...
          specificInstructions += `🚨 KUNDE KOMMT BEREITS AUS "${customerCity}" – NIEMALS "Woher kommst du?" / "Wo wohnst du?" fragen.\n`;
          specificInstructions += `✅ RICHTIG: "Ich wohne in ${nearbyCity} …" + andere Frage (nicht nach seinem Wohnort).\n`;
          specificInstructions += `❌ FALSCH: "Woher kommst du denn?" OHNE den Wohnort zu nennen!\n`;
          specificInstructions += `🚨🚨🚨 WICHTIG: Der Kunde hat NACH DEM WOHNORT GEFRAGT - du MUSST ihn nennen, nicht zurückfragen ohne ihn zu nennen! 🚨🚨🚨\n`;
        } else {
...
WICHTIG: Wirke NICHT zu schlau! Du bist eine normale Person, keine Wissensdatenbank! Weiche IMMER aus, auch wenn der Kunde explizit danach fragt! NIEMALS sachlich oder wissenschaftlich antworten!`;
    ...
    // Geschlechter-Rollenverständnis (Gold/S69: Gender oft in profileInfo.moderatorInfo.gender als male/female)
    const fakeGender = extractedUserInfo?.assistant?.Gender || profileInfo?.moderatorInfo?.gender || null;
    if (fakeGender) {
      const gLow = String(fakeGender).toLowerCase();
      const isFemale = gLow === "weiblich" || gLow === "w" || gLow === "female" || gLow === "f" || gLow === "frau";
      ...
      if (isFemale) {
        ...
        specificInstructions += `- ❌ FALSCH: Der Kundin (Frau) keine männliche Anatomie in den Mund legen (z.B. "lutsch deinen Schwanz", "dein Penis") – sie ist die Frau, du der Mann.\n`;
      }
    ...
    if (isJobQuestion && !specificInstructions.includes("Beruf") && !specificInstructions.includes("beruf")) {
      ...
      const fakeGenderJob = extractedUserInfo?.assistant?.Gender || profileInfo?.moderatorInfo?.gender || null;
      const gJob = String(fakeGenderJob || "").toLowerCase();
      const isFemale = gJob === "weiblich" || gJob === "w" || gJob === "female" || gJob === "f" || gJob === "frau";
...
async function agentLanguageDetector(customerMessage) {
  try {
    ...
    const languageDetectionPrompt = `Prüfe, ob die folgende Nachricht auf Deutsch geschrieben ist (Inhaltssprache des Kunden).

Nachricht: "${customerMessage.substring(0, 500)}"

Regeln (streng befolgen):
- Im ZWEIFEL IMMER "isGerman": true und "confidence": höchstens 0.5. Lieber jede deutschsprachige Nachricht durchlassen als eine deutsche fälschlich abweisen.
- NUR "isGerman": false, wenn der GESAMTE Text oder klar der überwiegende Teil in EINER anderen Sprache ist (z.B. ganzer Satz/Block auf Englisch, Italienisch, Spanisch, Türkisch, Serbisch/Kroatisch, Russisch – nicht nur ein Wort).
- Denglisch und Alltag: Viele Deutsche schreiben mit englischen Einzelwörtern (cool, sorry, baby, love, online, chat, date, hey, super, crazy, happy, kiss, sweet) – das ist trotzdem DEUTSCH, wenn Satzbau und Rest eindeutig deutsch sind. NIEMALS wegen 1–2 englischen Wörtern in einem sonst deutschen Satz "isGerman": false.
- Einzelne Fremdwörter, Markennamen, Anglizismen, Emoji-Text, "ok", "yes", "no" in einem ansonsten deutschen Kontext = DEUTSCH.
- Deutsch mit Rechtschreibfehlern, Tippfehlern, Umgangssprache, Dialekt, Emojis, kurzen Sätzen = DEUTSCH.
- Derbe, vulgäre oder umgangssprachliche deutsche Formulierungen (auch mit sexualbezogenem Wortschatz) = DEUTSCH.
- Enthält die Nachricht typisch deutsche Wörter oder Grammatik (ich, du, dich, dir, nicht, und, ist, mit, der/die/das, …) oder Umlaute (ä, ö, ü, ß) und wirkt insgesamt deutsch → "isGerman": true.
- Deutsche Abkürzungen/Begriffe wie FKK, Sauna, Strand, nackt = DEUTSCH.
- Nur bei klar erkennbarer Fremdsprache über den ganzen Text: "isGerman": false und "confidence" 0.99 oder höher (nur wenn wirklich sicher).
...
Antworte NUR als JSON, kein zusätzlicher Text.`;
    ...
    // Heuristik: Typisch deutsche Zeichen/Wörter / deutscher Satzbau (inkl. Denglisch) → nicht als "nicht deutsch" werten
    ...
    // Typische deutsche Funktionswörter auch bei vielen Anglizismen (cool, online, sorry, …)
    const coreGermanParticles = /\b(ich|du|dich|dir|mir|mich|er|sie|es|wir|und|oder|aber|nicht|kein|keine|keinen|auch|noch|schon|nur|mal|hier|da|wenn|dass|weil|wie|was|wo|warum|bist|bin|sind|war|habe|hast|hat|haben|kann|kannst|können|muss|musst|will|willst|gibt|geht|machst|mache|kommst|kommen|gehst|sagst|halt|grad|echt|einfach|vielleicht|leider|heute|jetzt|später|mein|dein|sein|ihr)\b/i;
    const hasLikelyGerman =
      hasGermanChars ||
      germanWordPattern.test(msg) ||
      shortGreetingOverride.test(msg) ||
      (msg.length >= 8 && /\b(und|ist|nicht|mit|der|die|das|auf|nur|grad|fkk|was|von|dich|dir|will|geil)\b/i.test(msg)) ||
      (msg.length >= 12 && coreGermanParticles.test(msg));
    ...
    // Nur bei sehr hoher Sicherheit als "nicht deutsch" werten (Denglisch/Anglizismen: eher durchlassen)
    if (!isGerman && confidence > 0.997) {
...
1. Kurz bedanken für den Kuss – VARIIERE die Formulierung: nicht immer wörtlich "Danke für deinen Kuss!"; wähle abwechslungsreich (z.B. "Das freut mich!", "Oh wie süß!", "Schön, danke für deinen Kuss!", "Nice, danke!" oder ähnlich locker). NICHT "dass du dich meldest" / "dass du dich gemeldet hast" – der Kunde hat keinen Text geschrieben, nur einen Kuss geschickt.
...
DEINE ANTWORT – GENAU DIESE STRUKTUR:
1. Kurz bedanken für das Like – VARIIERE die Formulierung: nicht immer wörtlich "Danke für deinen Like!"; wähle abwechslungsreich (z.B. "Das freut mich!", "Oh wie süß!", "Schön, danke für deinen Like!", "Nice, danke!" oder ähnlich locker). NICHT "dass du dich meldest" / "dass du dich gemeldet hast" – der Kunde hat keinen Text geschrieben, nur geliket.
2. Danach 1–2 kurze Fragen stellen, um ein Gespräch zu starten (abwechslungsreich: Wie geht es dir? Was machst du? Tag, Arbeit, Freizeit). KEINE Treffen-Vorschläge, keine Anspielungen (z.B. NICHT "es wäre schön einen Abend zu verbringen", "entspannter Abend zusammen").
   - Wie geht es dir? / Was machst du gerade? / Wie läuft dein Tag? / Bist du noch auf der Arbeit?
...
async function agentMessageGenerator(multiAgentResults, {
  ...
  try {
    ...
    const fakeGender = fakeContext.fakeGender || extractedUserInfo?.assistant?.Gender || profileInfo?.moderatorInfo?.gender || null;
    let genderSpecificNote = "";
    if (fakeGender) {
      const gN = String(fakeGender).toLowerCase();
      const isFemale = gN === "weiblich" || gN === "w" || gN === "female" || gN === "f" || gN === "frau";
      ...
      if (isFemale) {
        ...
        genderSpecificNote += `- ❌ FALSCH: Der Kundin (Frau) keine männliche Anatomie in den Mund legen (z.B. "lutsch deinen Schwanz", "dein Penis") – sie ist die Frau, du der Mann.\n`;
      }
...
${isASA ? '\n🚨🚨🚨🚨🚨🚨🚨🚨🚨 KRITISCH: DIES IST EINE REAKTIVIERUNGSNACHRICHT (ASA)! 🚨🚨🚨🚨🚨🚨🚨🚨🚨\n- Der Kunde hat zuletzt nicht geantwortet.\n- Du hast ~40 ASA-Beispiele zur Verfügung - ein zufälliges wurde ausgewählt.\n- 🚨🚨🚨 DEINE AUFGABE: KOPIERE DAS ASA-BEISPIEL FAST 1:1 - NUR KLEINE VARIATIONEN ERLAUBT! 🚨🚨🚨\n- Ignoriere ALLE Situationen, Kontext, andere Analysen - NUR das ASA-Beispiel zählt!\n- Verwende die GLEICHEN Wörter, Formulierungen, Fragen wie im Beispiel (fast 1:1).\n- Reaktiviere das Gespräch freundlich und interessiert - ORIENTIERE DICH AM BEISPIEL!\n- Stelle die FRAGE aus dem Beispiel (fast 1:1).\n- Sei warmherzig, aber nicht aufdringlich - WIE IM BEISPIEL!\n- 🚨🚨🚨 DEINE NACHRICHT SOLLTE SICH ANFÜHLEN WIE EINE KOPIE DES BEISPIELS! 🚨🚨🚨\n' : ''}`;
    ...
    if (isASA) {
      ...
      if (asaConversationContext && asaConversationContext.trim() !== "") {
        const hasKuss = asaConversationContextIsKussSystemLine(asaConversationContext);
        const hasLike = asaConversationContextIsLikeSystemLine(asaConversationContext);
...
🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨 DEINE NACHRICHT SOLLTE SICH ANFÜHLEN WIE EINE KOPIE DES BEISPIELS! 🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨`;
    ...
    if (message) {
      ...
      if (isASA) {
        message = sanitizeASAForbiddenChars(message);
      }
...
async function runStage1Planning({
  ...
  try {
    ...
    if (isLocationQuestionFunc && typeof isLocationQuestionFunc === 'function' && isLocationQuestionFunc(customerMessage)) {
      ...
      const moderatorInfo = extractedUserInfo?.assistant || profileInfo?.moderatorInfo || {};
      const customerCity = customerInfo.city || customerInfo.wohnort || extractedUserInfo?.user?.Wohnort || extractedUserInfo?.user?.city || extractedUserInfo?.user?.wohnort || null;
      const statedOwnLocationStage1 = customerStatesOwnLocationInMessage(customerMessage || '');
      const skipAskCustomerWhereStage1 = (!!customerCity && String(customerCity).trim() !== '' && String(customerCity).toLowerCase() !== 'sag ich später') || statedOwnLocationStage1;
      const fakeCity = profileInfo?.moderatorInfo?.city || 
                       profileInfo?.moderatorInfo?.Wohnort || 
                       moderatorInfo.Wohnort || 
                       moderatorInfo.city || 
      ...
      if (fakeCity && fakeCity.trim() !== "" && fakeCity.toLowerCase() !== "sag ich später") {
        ...
        if (nearbyCity) {
          ...
          cityContext = `\n\n🚨🚨🚨 KRITISCH: WOHNORT-FRAGE ERKANNT! 🚨🚨🚨\n- Der Kunde fragt, woher du kommst / wo du wohnst.\n- Der Kunde kommt aus: "${customerCity}" (bekannt – nicht erneut fragen!)\n- Nenne eine Stadt/Dorf im 20km Umkreis: "${nearbyCity}"\n- 🚨 NIEMALS "Sag ich später" – nenne immer diesen konkreten Ort!\n- 🚨🚨🚨 ABSOLUT KRITISCH: NENNE DIESEN WOHNORT "${nearbyCity}" ZUERST in deiner Antwort, DANN eine ANDERE Frage (nicht Woher-kommst-du)!\n- ✅ RICHTIG: "Ich wohne in ${nearbyCity} …" + Frage zu anderem Thema.\n- ❌ FALSCH: "Woher kommst du denn?" OHNE den Wohnort zu nennen!\n\n`;
        }
      } else {
  ...
  try {
    ...
    if (isTogether) {
      ...
      if (imageAlreadySentToCustomer) {
        situationalRules += '\n\nBILD BEREITS GESCHICKT: Du hast dem Kunden in deiner letzten Nachricht bereits ein Bild geschickt – er hat es gesehen. Gehe KONKRET auf das ein, was der Kunde JETZT schreibt (seine Reaktion, ob es ihm gefaellt, worauf er reagiert), thematisch zum Bild. Keine generischen Floskeln.\n';
...
async function runMultiStagePipeline({
  ...
  lastCustomerMessageAgeMs = null, // 🚨 Zuverlässig: von Extension (metaData.lastCustomerMessageAgeMs) – Millisekunden seit letzter Kunden-Nachricht
  lastCustomerMessageTimestamp = null, // 🚨 Alternativ: metaData.lastCustomerMessageTimestamp (ISO oder ms) – dann wird Alter daraus berechnet
  asaNoRealCustomerMessage = false // true = Kunde hat nur Freundschaftsanfrage/Like geschickt, kein richtiges Gespräch → ASA ohne "unser Gespräch über X"
}) {
  ...
  // 🤖 ASA-FALL – wie Backup: nur Pool, Round-Robin, 1:1 oder Mix aus 2–3 ASAs
  if (isASA) {
    console.log('🤖 ASA-Fall erkannt – Pool + Round-Robin (ggf. Mix aus 2–3 Beispiele)');
    let asaExamples = [];
    if (trainingData && trainingData.asaExamples && Array.isArray(trainingData.asaExamples)) {
    ...
    if (asaExamples.length > 0) {
      ...
      const selected = selectTwoASAs(pool, lastUsed);
      if (selected.length >= 2) {
        const texts = selected.map(ex => (ex.asaMessage || ex.moderatorResponse || '').trim()).filter(Boolean);
        asaMessageText = await combineASAExamples(texts);
        if (!asaMessageText) {
          asaMessageText = (selected[0].asaMessage || selected[0].moderatorResponse || '').trim();
          selectedPoolChosen = selected[0];
          console.log('🎲 ASA Mix fehlgeschlagen – Fallback 1:1:', asaMessageText.length + ' Zeichen');
      } else {
          selectedPoolChosen = selected;
          console.log(`🎲 ASA Mix (2 Beispiele kombiniert): ${asaMessageText.length} Zeichen`);
        }
      } else {
        selectedPoolChosen = selected[0] || pool[0];
        ...
        console.log(`🎲 ASA 1:1 (Round-Robin): ${asaMessageText.length} Zeichen`);
      }
    }

    if (asaMessageText) {
      asaMessageText = ensureASAMinLength(sanitizeASAForbiddenChars(asaMessageText));
      asaMessageText = ensureASAMinLength(asaMessageText);
      const selectedASAs = selectedPoolChosen
        ? (Array.isArray(selectedPoolChosen) ? selectedPoolChosen : [selectedPoolChosen])
...
async function runSimplifiedPipeline({
  ...
  if (isASA) {
    ...
    if (asaExamples.length > 0) {
      // 🎲 ASA-Auswahl: Round-Robin + optional divers zur letzten Moderator-Nachricht
      const pool = asaExamples;
      const lastUsed = getLastModeratorMessageSnippet(conversationHistory);
      const chosen = selectOneASAWithRotation(pool, lastUsed) || pool[0];
      ...
      console.log(`✅ 1 ASA-Beispiel ausgewählt (Index ${chosenIdx} von ${asaExamples.length}, Round-Robin${lastUsed ? ' + Diversität' : ''})`);
