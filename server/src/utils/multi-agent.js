...
WICHTIG: Du antwortest als FAKE-PROFIL (${fakeName}) auf Nachrichten vom KUNDEN. Der KUNDE schreibt DIR (dem Fake-Profil)!`;
      ...
      // 🚨 FIX: Erweitere Kontext für bessere Antworten (mehr als 373 Zeichen)
      let extendedContext = '';
      
      // Erweitere Kontext mit letzten 2-3 Nachrichten für besseres Verständnis
      if (structuredContext.recentMessages && structuredContext.recentMessages.length > 0) {
        const recentMsgs = structuredContext.recentMessages.slice(-3); // Letzte 3 Nachrichten
        if (recentMsgs.length > 0) {
          extendedContext = '\n\nKontext (letzte Nachrichten):\n';
          recentMsgs.forEach((msg, idx) => {
            const role = msg.type === 'moderator' ? 'Du' : 'Kunde';
            extendedContext += `${role}: "${msg.text.substring(0, 100)}${msg.text.length > 100 ? '...' : ''}"\n`;
          });
          extendedContext += '\n';
        }
      }
      
      userPrompt = `${contextInfo}${extendedContext}Kunde: "${customerMessage}"\n\nAntworte als Chat-Moderator.`;
      ...
      const maxTokens = 150; // 🚨 FIX: Reduziert von 450 auf 150 (~450 Zeichen max statt ~1800)
...
Antworte NUR als JSON:
{
  ...
  "reasoning": "Warum ist die Nachricht gültig/ungültig?"
    ...
    // 🚨🚨🚨 FIX: Prüfe IMMER auf Länge und Wiederholungen, auch bei sexuellen Situationen!
    const MAX_MESSAGE_LENGTH = 300; // Maximale Zeichen
    ...
    if (isSexualSituation && hasMeetingSuggestion) {
      ...
      const removalPrompt = `Entferne den Treffen-Vorschlag aus dieser Nachricht, aber stelle sicher, dass der Satz grammatikalisch korrekt bleibt:

NACHRICHT: "${message}"

AUFGABE:
1. Finde den Treffen-Vorschlag (z.B. "würdest du mich besuchen?", "sollen wir uns treffen?", "kommst du zu mir?")
2. Entferne NUR den Treffen-Vorschlag
3. Formuliere den Rest der Nachricht neu, damit sie grammatikalisch korrekt ist
4. Stelle sicher, dass die Nachricht mit einem vollständigen Satz beginnt
5. Wenn die Nachricht zu kurz wird (< 50 Zeichen), füge eine alternative Frage hinzu (z.B. "Was würdest du denn gerne mit mir machen?")

Antworte NUR als JSON:
{
  "correctedMessage": "Korrigierte Nachricht (grammatikalisch korrekt, vollständiger Satz am Anfang)",
  "removedPart": "Was wurde entfernt?",
  "reasoning": "Warum wurde es so korrigiert?"
}`;
      ...
      try {
        ...
        if (startsWithLowercase || startsWithIncomplete) {
          ...
          // Formuliere den Satz neu, damit er vollständig ist
          const fixPrompt = `Formuliere diese Nachricht neu, damit sie mit einem vollständigen, grammatikalisch korrekten Satz beginnt:

NACHRICHT: "${correctedMessage}"

AUFGABE:
1. Beginne mit einem vollständigen Satz (z.B. "Ich verstehe, dass..." oder "Das klingt interessant, aber...")
2. Stelle sicher, dass die Nachricht grammatikalisch korrekt ist
3. Behalte die ursprüngliche Bedeutung bei

Antworte NUR als JSON:
{
  "correctedMessage": "Korrigierte Nachricht mit vollständigem Satzanfang"
}`;
