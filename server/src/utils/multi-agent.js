...
async function agentFirstMessageDetector(conversationHistory, customerMessage, messages) {
  try {
    ...
    if (isFirstMessage) {
      ...
      const firstMessageInstructions = `
...
1. STELLE DICH VOR:
   - Sage Hallo (z.B. "Hallo", "Hi")
   - Nenne deinen Namen (aus Fake-Profil)
   - Optional: Erwähne dein Alter oder Wohnort (wenn im Fake-Profil vorhanden)

2. ZEIGE INTERESSE:
   - Frage wie es ihm geht (z.B. "Wie geht es dir?", "Wie läuft es bei dir?")
   - Frage was er so macht (z.B. "Was machst du denn so?", "Wie sieht dein Tag aus?")
   - Frage was er hier sucht (z.B. "Was suchst du denn hier?", "Wonach suchst du hier?")
...
3. SEI NATÜRLICH:
   ...
   - Stelle 1-2 natürliche Fragen (nicht mehr!)
...
5. BEISPIEL-STRUKTUR:
   "Hallo, ich bin [Name]. Wie geht es dir denn so? Was machst du denn hier so?"
...
${isASA ? '\n🚨🚨🚨 KRITISCH: DIES IST EINE REAKTIVIERUNGSNACHRICHT (ASA)! 🚨🚨🚨\n- Der Kunde hat zuletzt nicht geantwortet.\n- Reaktiviere das Gespräch freundlich und interessiert.\n- Frage, was den Kunden beschäftigt.\n- Sei warmherzig, aber nicht aufdringlich.\n' : ''}`;
    ...
    if (message) {
      ...
      if (!proactiveCheck || !proactiveCheck.hasOwnExperience) {
