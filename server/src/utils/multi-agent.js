...
async function agentSituationDetector(customerMessage, allRules, isLocationQuestionFunc, findNearbyCityFunc, isMeetingRequestFunc, profileInfo, extractedUserInfo, conversationHistory = "", moderatorMessages = [], customerMessages = [], contextAnalysis = null, isASA = false, learningContextResult = null) {
  try {
    ...
    // 1. AKTUELLE Nachricht indirekt auf Treffen hinweist (Verfügbarkeits-Frage/Antwort ODER Treffen-Keywords ohne Fantasie)
    // 2. NICHT, wenn nur alte Nachrichten Treffen-Keywords enthalten!
    if (hasAvailabilityAnswer || 
        (hasAvailabilityQuestionInMessage) ||
        (hasMeetingKeywordsInMessage && !hasFantasieKeywordsInMessage)) {
        hasMeetingContext = true;
      console.log('🚨 KRITISCH: Treffen-Kontext erkannt in AKTUELLER Nachricht!', 
        hasAvailabilityAnswer ? '(Verfügbarkeits-Antwort gefunden)' : '',
        hasAvailabilityQuestionInMessage ? '(Verfügbarkeits-Frage gefunden)' : '',
        hasMeetingKeywordsInMessage ? '(Treffen-Keywords gefunden)' : '');
    }
...
${isASA ? '\n🚨🚨🚨🚨🚨🚨🚨🚨🚨 KRITISCH: DIES IST EINE REAKTIVIERUNGSNACHRICHT (ASA)! 🚨🚨🚨🚨🚨🚨🚨🚨🚨\n- Der Kunde hat zuletzt nicht geantwortet.\n- Du hast ~40 ASA-Beispiele zur Verfügung - ein zufälliges wurde ausgewählt.\n- 🚨🚨🚨 DEINE AUFGABE: KOPIERE DAS ASA-BEISPIEL FAST 1:1 - NUR KLEINE VARIATIONEN ERLAUBT! 🚨🚨🚨\n- Ignoriere ALLE Situationen, Kontext, andere Analysen - NUR das ASA-Beispiel zählt!\n- Verwende die GLEICHEN Wörter, Formulierungen, Fragen wie im Beispiel (fast 1:1).\n- Reaktiviere das Gespräch freundlich und interessiert - ORIENTIERE DICH AM BEISPIEL!\n- Stelle die FRAGE aus dem Beispiel (fast 1:1).\n- Sei warmherzig, aber nicht aufdringlich - WIE IM BEISPIEL!\n- 🚨🚨🚨 DEINE NACHRICHT SOLLTE SICH ANFÜHLEN WIE EINE KOPIE DES BEISPIELS! 🚨🚨🚨\n' : ''}`;
      ...
      try {
        ...
        if (learningDataForScoring) {
          ...
          if (processedVariants.length > 0) {
            ...
            // 🚨 NEU: Hard-Filter für semantische Validierung
            // Wenn semanticValidation Score < 50% (von 25% = 12.5%), filtere diese Variante raus
            const semanticThreshold = 12.5; // 50% von 25%
            ...
            if (validVariants.length > 0) {
              ...
              if (validVariants.length < scoredVariants.length) {
                console.warn(`⚠️ Multi-Generator: ${scoredVariants.length - validVariants.length} Variante(n) wegen schlechter semantischer Validierung gefiltert`);
              }
            } else {
              // Alle Varianten haben schlechte semantische Validierung - verwende trotzdem die beste
              console.warn(`⚠️ Multi-Generator: Alle Varianten haben schlechte semantische Validierung (< ${semanticThreshold}%) - verwende trotzdem beste Variante`);
              // 🚨 WICHTIG: Markiere für spätere Korrektur
    ...
    if (message) {
      ...
      // 🆕 NEU: Prüfe, ob eine Frage fehlt (wenn targetAvgQuestions > 0)
      // 🚨 WICHTIG: Bei aggressiven/negativen Nachrichten KEINE unpassenden Fragen hinzufügen!
      if (targetAvgQuestions > 0 && questionCount === 0 && !isASA) {
        // Prüfe, ob die Kunden-Nachricht aggressiv/negativ ist
        const customerMsgLower = (customerMessage || '').toLowerCase();
        const aggressiveKeywords = [
          'schlampe', 'hure', 'arsch', 'fotze', 'scheiß', 'scheisse', 'drecks', 'hirntot', 'verflucht',
          'verrecken', 'nutzlos', 'dumm', 'blöd', 'idiot', 'wütend', 'aggressiv', 'beleidigend'
        ];
        const isAggressiveMessage = aggressiveKeywords.some(keyword => customerMsgLower.includes(keyword));
        
        // Bei aggressiven Nachrichten: Nur passende Fragen hinzufügen, keine generischen wie "Wonach suchst du hier?"
        if (isAggressiveMessage) {
          console.log(`ℹ️ Aggressive Nachricht erkannt - füge nur passende Frage hinzu (keine generischen Fragen)`);
        }
        
        console.warn(`⚠️ Nachricht enthält KEINE Frage, aber statistische Ziele erfordern ${targetAvgQuestions} Frage(n) - füge Frage hinzu...`);
        
        let questionToAdd = null;
        
        // 1. Versuche Frage aus Training-Daten zu extrahieren
        if (trainingDataQuestions.length > 0) {
          // Wähle eine passende Frage basierend auf Kontext
          // Priorisiere Fragen, die zum Kontext passen
          ...
          trainingDataQuestions.forEach(q => {
            ...
            // 🚨 WICHTIG: Bei aggressiven Nachrichten: Filtere unpassende Fragen raus
            if (isAggressiveMessage) {
              // Filtere "Wonach suchst du?" und ähnliche unpassende Fragen bei aggressiven Nachrichten
              if (qLower.includes('wonach suchst') || qLower.includes('was suchst') || qLower.includes('wonach suchst du hier')) {
                score -= 50; // Starke Abwertung
              }
            }
            
            // Wenn Kunde über Treffen spricht und Frage auch Treffen erwähnt
            if ((customerMsgLower.includes('treffen') || customerMsgLower.includes('kennenlernen')) && 
                (qLower.includes('treffen') || qLower.includes('kennenlernen') || qLower.includes('vorstellen'))) {
              score += 10;
            }
            
            // Wenn Kunde über Kosten/Geld spricht
            if ((customerMsgLower.includes('teuer') || customerMsgLower.includes('kosten') || customerMsgLower.includes('geld')) &&
                (qLower.includes('vorstellen') || qLower.includes('machen') || qLower.includes('suchen'))) {
              score += 10;
            }
            
            // Bei aggressiven Nachrichten: Priorisiere Fragen, die auf die Situation eingehen
            if (isAggressiveMessage) {
              if (qLower.includes('warum') || qLower.includes('wieso') || qLower.includes('was ist') || qLower.includes('was los')) {
                score += 15; // Starke Priorisierung
              }
            }
            
            // Allgemeine Passung
            if (qLower.includes('vorstellen') || qLower.includes('machen') || qLower.includes('suchen') || qLower.includes('interessiert')) {
              score += 5;
            }
          ...
          // Nur verwenden, wenn Score positiv ist (bei aggressiven Nachrichten)
          if (!isAggressiveMessage || bestScore > 0) {
            questionToAdd = bestQuestion || trainingDataQuestions[0];
            console.log(`✅ Frage aus Training-Daten extrahiert: "${questionToAdd.substring(0, 80)}..."`);
          } else {
            console.log(`⚠️ Keine passende Frage aus Training-Daten für aggressive Nachricht gefunden`);
          }
        }
        
        // 2. Wenn keine Training-Daten-Frage gefunden: Generiere logische Frage basierend auf Kontext
        if (!questionToAdd) {
          // Bei aggressiven Nachrichten: Generiere passende Frage
          if (isAggressiveMessage) {
            questionToAdd = "Was ist denn los?";
          } else if (customerMsgLower.includes('treffen') || customerMsgLower.includes('kennenlernen')) {
            questionToAdd = "Wie würdest du dir das denn vorstellen?";
          } else if (customerMsgLower.includes('teuer') || customerMsgLower.includes('kosten') || customerMsgLower.includes('geld')) {
            questionToAdd = "Was würdest du denn gerne machen?";
          } else if (customerMsgLower.includes('arbeit') || customerMsgLower.includes('arbeiten')) {
            questionToAdd = "Was machst du denn beruflich?";
          } else {
            // Generische, aber kontextuelle Frage
            questionToAdd = "Was würdest du denn gerne machen?";
          }
          
          console.log(`✅ Logische Frage generiert (keine Training-Daten): "${questionToAdd}"`);
        }
        
        // 3. Füge Frage zur Nachricht hinzu (nur wenn eine passende Frage gefunden wurde)
        if (questionToAdd) {
          ...
          console.log(`✅ Frage hinzugefügt: "${questionToAdd.substring(0, 80)}..."`);
        } else {
          console.warn(`⚠️ Keine passende Frage gefunden - Nachricht bleibt ohne zusätzliche Frage`);
        }
      }
      ...
      const shouldAddProactiveSexualAddition = hasExplicitSexualInCurrentMessage && !isMeetingWithoutExplicitSexual;
