...
async function agentMessageGenerator(multiAgentResults, {
  ...
  // Normale Reply-Nachrichten verwenden LoRA (wenn aktiviert)
  ...
  if (isASA) {
    console.log("🔍 ASA-Nachricht - verwende OpenAI (nicht LoRA)");
  } else {
    console.log(`🔍 Normale Reply-Nachricht - ${isLocal ? "LoRA-KI" : "OpenAI"} wird verwendet`);
...
async function runMultiStagePipeline({
  ...
  if (skipStages12) {
    ...
    if (!isASA) {
      ...
      const { agentSituationDetector } = require('./multi-agent');
