...
- GUT: themenbezogen, z.B. bei Beruf -> "Was machst du so beruflich?", bei Vorlieben -> "Und wie siehts bei dir aus?", bei Treffen-Diskussion -> "Was machst du denn jetzt sonst noch schönes heute?"

Beispiele (Stil aus echten Moderator-Antworten):
- Kunde sagt "Bekomme Besuch, melde mich später" -> ["Wie lange wird der Besuch bleiben?", "Weisst du schon wann du dich wieder melden kannst?"]
- Kunde fragt nach Vorlieben/Sexuelles -> ["Und was macht dich so richtig geil?", "Und wie siehts bei dir aus?"]
- Kunde fragt nach Beruf -> ["Was machst du so beruflich?", "Mit was verdienst du dein Geld?"]
- Kunde will Treffen / Freizeit -> ["Was machst du denn jetzt sonst noch schönes heute?", "Wann passt es bei dir am besten?"]
- Kunde gibt Nummer / will woanders schreiben -> ["Ich will dich erstmal noch ein paar Dinge über dich erfahren – was machst du beruflich?", "Wie lange ist deine letzte Beziehung eigentlich her?"]
...
WICHTIG: Du antwortest als FAKE-PROFIL (${fakeName}) auf Nachrichten vom KUNDEN. Der KUNDE schreibt DIR (dem Fake-Profil)!${whoIsWho}`;
      ...
      // 🚀 Zwei-Stufen: Stufe 1 – GPT-4o-mini liefert Situation + vorgeschlagene Fragen für LoRA-Prompt
      ...
      if (suggestedQuestionsForPrompt.length > 0) {
        userPrompt += ` Baue eine dieser Fragen am Ende ein: "${suggestedQuestionsForPrompt.join('" oder "')}".`;
      }
      userPrompt += '\n';
