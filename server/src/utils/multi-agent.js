...
/**
 * Wählt 2–3 ASA-Beispiele: erstes per Round-Robin + Diversität, dann 1–2 weitere möglichst unterschiedliche (für Mix).
 ...
 */
function selectTwoOrThreeASAs(pool, lastUsedText) {
  ...
  const firstSig = (first.asaMessage || first.moderatorResponse || '').trim().slice(0, 120);
  if (N === 1) return [first];
  const rest = [];
  for (let k = 1; k < N; k++) {
  ...
  rest.sort((a, b) => asaTextDistance((b.ex.asaMessage || b.ex.moderatorResponse || '').trim().slice(0, 120), firstSig) - asaTextDistance((a.ex.asaMessage || a.ex.moderatorResponse || '').trim().slice(0, 120), firstSig));
  const take = Math.min(2, rest.length);
  const selected = [first];
  for (let t = 0; t < take; t++) {
    selected.push(rest[t].ex);
    asaLastUsedIndices.unshift(rest[t].index);
  }
  if (asaLastUsedIndices.length > ASA_LAST_USED_MAX) asaLastUsedIndices.length = ASA_LAST_USED_MAX;
...
/**
 * Kombiniert 2–3 ASA-Texte zu EINER kohärenten Nachricht (Einstieg von einer, Teilsatz von anderer, Frage von dritter usw.).
 * Nur Neu-Zusammensetzung aus den gegebenen Texten – nichts erfinden.
 * @param {string[]} asaTexts - 2 oder 3 ASA-Nachrichten
 * @returns {Promise<string>} kombinierte Nachricht oder '' bei Fehler
 */
async function combineASAExamples(asaTexts) {
  if (!asaTexts || asaTexts.length <= 1) return (asaTexts && asaTexts[0]) ? String(asaTexts[0]).trim() : '';
  const client = getClient();
  if (!client) return '';
  const list = asaTexts.map((t, i) => `ASA ${i + 1}: ${(t || '').trim()}`).filter(s => s.length > 10).slice(0, 3);
  if (list.length < 2) return (asaTexts[0] || '').trim();
  const systemContent = `Du kombinierst 2–3 Reaktivierungsnachrichten (ASA) zu EINER kohärenten Nachricht.
- Nimm Teile von jeder: z.B. Einstieg (Hey/Oh/Na) von einer, einen Satz von einer anderen, Schlussfrage von einer dritten.
- Erfinde NICHTS dazu – nur aus den gegebenen Texten neu zusammensetzen. Es muss flüssig und natürlich klingen.
...
- Ausgabe: NUR die eine kombinierte Nachricht, sonst nichts.`;
  const userContent = `Kombiniere diese Nachrichten zu einer:\n\n${list.join('\n\n')}`;
  try {
...
async function runMultiStagePipeline({
  ...
  if (isASA) {
    ...
    if (asaExamples.length > 0) {
      ...
      const selected = selectTwoOrThreeASAs(pool, lastUsed);
      if (selected.length >= 2) {
        ...
        if (!asaMessageText) {
          ...
          console.log(`🎲 ASA Mix (${selected.length} Beispiele kombiniert): ${asaMessageText.length} Zeichen`);
        }
      } else {
