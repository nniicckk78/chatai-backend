...
- KEINE Meta-Kommentare
- KEINE bewertenden Einstiegs-Phrasen${situationalRules}
WICHTIG: Du antwortest als FAKE-PROFIL (${fakeName}) auf Nachrichten vom KUNDEN. Der KUNDE schreibt DIR (dem Fake-Profil)!`;
      ...
      // 🚨 Minimaler Kontext (nur wenn wirklich nötig)
      if (structuredContext.lastModeratorMessage && structuredContext.lastModeratorMessage.length > 0) {
        // Nur für laufende Gespräche - kompakt
        contextInfo = `Kontext: Der Kunde antwortet auf deine letzte Nachricht: "${structuredContext.lastModeratorMessage.substring(0, 150)}${structuredContext.lastModeratorMessage.length > 150 ? '...' : ''}"\n\n`;
      }
      
      // 🚨 NEU: Neue Informationen (nur wenn vorhanden)
      ...
      // 🚨 NEU: Bild-Kontext (nur wenn vorhanden)
      if (imageType && imageDescription) {
        if (imageType === 'penis' || imageType === 'dildo' || imageType === 'nude') {
      ...
      // 🚨🚨🚨 USER-PROMPT: GENAU wie im Training - einfach "Kunde: '...' Antworte..."
      userPrompt = `${contextInfo}Kunde: "${customerMessage}"\n\nAntworte als Chat-Moderator.`;

      // Generiere Nachricht mit vereinfachtem Prompt
      // 🚨🚨🚨 KRITISCH: Temperature 0.1 für maximale Orientierung am Training!
      const timeoutMs = 30000; // 30s für Together.ai
      const temperature = 0.1; // 🚨🚨🚨 KRITISCH: 0.1 für fine-tuned Models (maximale Orientierung am Training!)
