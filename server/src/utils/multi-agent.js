...
${stage1Plan.isOngoingConversation ? '🚨 KRITISCH: Dies ist ein LAUFENDES Gespräch - KEINE Begrüßung mit "Hey" oder "Hallo"! 🚨' : ''}`;
      ...
      // User-Prompt: VEREINFACHT - Kontext + Beispiele + Kundennachricht + Bild-Kontext
      let userPrompt = '';
      
      // 🚨 NEU: ConversationHistory + Logik-Analyse (was wurde besprochen, was nicht)
      if (conversationHistory && conversationHistory.trim().length > 0) {
        // Extrahiere letzte Nachrichten aus conversationHistory
        const historyLines = conversationHistory.split('\n').filter(line => line.trim().length > 0);
        const recentHistory = historyLines.slice(-6).join('\n'); // Letzte 6 Zeilen für mehr Kontext
        if (recentHistory && recentHistory.trim().length > 0) {
          userPrompt += `📜 GESPRÄCHSVERLAUF (was vorher besprochen wurde):\n${recentHistory.substring(0, 400)}\n\n`;
          
          // 🚨 NEU: Logik-Anweisungen basierend auf Kontext
          userPrompt += `🧠 LOGIK-REGELN:\n`;
          userPrompt += `- Prüfe: Was wurde bereits besprochen? (siehe Gesprächsverlauf oben)\n`;
          userPrompt += `- Vermeide: Wiederholungen von bereits besprochenen Themen\n`;
          userPrompt += `- Gehe auf: Neue Informationen in der aktuellen Kundennachricht ein\n`;
          userPrompt += `- Stelle: Eine neue, passende Frage (nicht bereits gestellt/beantwortet)\n\n`;
        }
      }
      
      // 🚨 NEU: Neue Informationen aus MINIMALEM STAGE 1 (wenn vorhanden)
      if (stage1Plan && stage1Plan.newInformation && stage1Plan.newInformation.hasNewInfo) {
        userPrompt += `🚨🚨🚨 NEUE INFORMATIONEN (HÖCHSTE PRIORITÄT!) 🚨🚨🚨\n\n`;
        ...
        if (stage1Plan.newInformation.workMentioned) {
          userPrompt += `- Arbeit erwähnt: "${stage1Plan.newInformation.workMentioned}" → GEHE DARAUF EIN! (z.B. "Dann viel Erfolg bei der Arbeit!", "Dann melde dich, wenn du Zeit hast")\n`;
        }
        userPrompt += `\n`;
      }
      
      // 🚨 NEU: Offene Fragen aus MINIMALEM STAGE 1 (wenn vorhanden)
      if (stage1Plan && stage1Plan.openQuestions && stage1Plan.openQuestions.length > 0) {
        userPrompt += `❓ OFFENE FRAGEN (MÜSSEN BEANTWORTET WERDEN):\n`;
        stage1Plan.openQuestions.forEach((q, idx) => {
          userPrompt += `${idx + 1}. ${q}\n`;
        });
        userPrompt += `\n🚨 WICHTIG: Beantworte diese Fragen in deiner Antwort!\n\n`;
      }
      
      // 🚨 NEU: Kritische Anweisungen aus MINIMALEM STAGE 1 (wenn vorhanden)
      if (stage1Plan && stage1Plan.criticalInstructions && stage1Plan.criticalInstructions.trim().length > 0) {
        userPrompt += `🚨 KRITISCHE ANWEISUNGEN:\n${stage1Plan.criticalInstructions}\n\n`;
      }
      ...
      // 🚨 ENTFERNT: Beispiele - Fine-Tuned Model hat bereits gelernt, braucht keine Beispiele im Prompt
      
      // 🚨 NEU: Spezielle Anweisungen für Situationen
      if (situations && situations.length > 0) {
        const hasMeetingRequest = situations.some(s => s.includes('Treffen') || s.includes('Termin'));
        const hasBotAccusation = situations.some(s => s.includes('Bot') || s.includes('KI'));
        const hasJobQuestion = situations.some(s => s.includes('Beruf') || s.includes('beruf'));
        
        if (hasMeetingRequest) {
          userPrompt += `🚨🚨🚨 KRITISCH: TREFFEN-ANFRAGE ERKANNT! 🚨🚨🚨\n\n`;
          userPrompt += `Der Kunde schlägt ein Treffen vor oder fragt nach einem Termin!\n\n`;
          userPrompt += `🚨 ABSOLUT KRITISCH:\n`;
          userPrompt += `- Du MUSST das Treffen höflich ablehnen (NIEMALS zustimmen!)\n`;
          userPrompt += `- Sage ZUERST, dass du an diesem Tag/Termin leider schon was vor hast\n`;
          userPrompt += `- DANN frage, wie er es sich vorstellen würde (nur EINE Frage!)\n`;
          userPrompt += `- Formulierungen: "Ich möchte dich erst noch besser kennenlernen, bevor wir uns treffen. Wie stellst du dir das denn vor?"\n`;
          userPrompt += `- ODER: "Heute habe ich leider schon was vor. Wie würdest du dir das denn vorstellen?"\n`;
          userPrompt += `- ODER: "Ich würde dich gerne erst noch besser kennenlernen. Wie stellst du dir das denn vor?"\n`;
          userPrompt += `- 🚨 WICHTIG: Begründe die Ablehnung (z.B. "erst besser kennenlernen", "leider schon was vor")\n\n`;
        }
        
        if (hasBotAccusation) {
          // Prüfe ob direkter oder indirekter Bot-Vorwurf
          const isDirectBotAccusation = customerMessage.toLowerCase().includes('ki') || 
                                       customerMessage.toLowerCase().includes('bot') ||
                                       customerMessage.toLowerCase().includes('künstliche intelligenz');
          
          if (!isDirectBotAccusation) {
            userPrompt += `ℹ️ HINWEIS: Der Kunde erwähnt KI-Profile allgemein, aber nicht direkt dich. Du musst NICHT darauf eingehen.\n\n`;
          } else {
            userPrompt += `🚨🚨🚨 KRITISCH: BOT-VORWURF ERKANNT! 🚨🚨🚨\n\n`;
            userPrompt += `Der Kunde wirft dir vor, eine KI zu sein!\n\n`;
            userPrompt += `🚨 ABSOLUT KRITISCH:\n`;
            userPrompt += `- Reagiere natürlich und menschlich (nicht defensiv!)\n`;
            userPrompt += `- Zeige, dass du echt bist (z.B. "Ich bin echt, versprochen!")\n`;
            userPrompt += `- Stelle eine Frage, um das Gespräch umzulenken\n\n`;
          }
        }
        
        if (hasJobQuestion && fakeWork) {
          userPrompt += `🚨🚨🚨 KRITISCH: BERUFSFRAGE ERKANNT! 🚨🚨🚨\n\n`;
          userPrompt += `Der Kunde fragt nach deinem Beruf oder erwähnt Arbeit/Beruf!\n\n`;
          userPrompt += `🚨 ABSOLUT KRITISCH:\n`;
          userPrompt += `- Dein Beruf ist: "${fakeWork}"\n`;
          userPrompt += `- Verwende IMMER diesen Beruf, NIEMALS Platzhalter wie "[dein Beruf]" oder "[Beruf]"!\n`;
          userPrompt += `- Wenn du über deinen Beruf sprichst, sage: "Ich arbeite als ${fakeWork}" oder "Ich bin ${fakeWork}"!\n`;
          userPrompt += `- Beispiel: "Ich arbeite als ${fakeWork}. Was machst du denn beruflich?"\n\n`;
        }
      }
      
      // Verbotene Wörter (wichtig für Validierung)
      if (allRules.forbiddenWords && allRules.forbiddenWords.length > 0) {
        userPrompt += `❌ VERBOTENE WÖRTER (NICHT verwenden): ${allRules.forbiddenWords.slice(0, 15).join(', ')}\n\n`;
      }
      
      // Aktuelle Kundennachricht
      userPrompt += `Kundennachricht: "${customerMessage}"\n\n`;
      
      // 🚨 NEU: Finale Anweisungen (was wichtig ist)
      userPrompt += `🚨 FINALE ANWEISUNGEN:\n`;
      userPrompt += `1. Gehe auf die Kundennachricht ein (was hat er gesagt?)\n`;
      userPrompt += `2. Reagiere auf alle Themen/Anfragen in der Nachricht\n`;
      userPrompt += `3. Stelle eine passende Frage (bezogen auf das, was der Kunde geschrieben hat)\n`;
      userPrompt += `4. Sei natürlich, authentisch und menschlich (nicht nach KI klingen)\n`;
      userPrompt += `5. Prägnant: 120-250 Zeichen (bei Erstnachrichten 150-300 Zeichen)\n`;
      if (situations && situations.length > 0) {
        userPrompt += `6. 🚨 KRITISCH: Beachte die Situations-Regeln oben (${situations.join(', ')})\n`;
      }
      userPrompt += `\nAntworte natürlich und authentisch auf die Kundennachricht.`;

      // Generiere Nachricht mit vereinfachtem Prompt
      const timeoutMs = 30000; // 30s für Together.ai
      console.log(`🚀 STAGE 3: Sende Anfrage an Together.ai (Modell: ${messageModel}, Timeout: ${timeoutMs/1000}s)...`);
