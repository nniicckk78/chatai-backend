...
🚨🚨🚨 KRITISCH: ERKENNE SITUATIONEN 🚨🚨🚨
Erkenne ALLE relevanten Situationen aus dieser Liste (nur die, die wirklich zutreffen):
- "Berufsfrage": Wenn nach Beruf/Arbeit gefragt wird (z.B. "was machst du so", "was arbeitest du", "was machst du beruflich", "was machst du so schönes")
- "Sexuelle Themen": Wenn über Sex, Vorlieben, Fantasien gesprochen wird
- "Treffen/Termine": Wenn nach einem Treffen gefragt wird (nicht "auf der Suche nach der richtigen fürs Leben")
- "Bilder Anfrage": Wenn nach Bildern/Fotos gefragt wird
- "Kontaktdaten außerhalb der Plattform": Wenn nach Telefonnummer, Email, Telegram, Instagram gefragt wird
- "Geld/Coins": Wenn über Geld, Coins, Aufladen, Kosten gesprochen wird
...
WICHTIG:
- "situations" ist ein Array - füge ALLE zutreffenden Situationen hinzu (kann leer sein)
- Erkenne Situationen SEMANTISCH - nicht nur Keywords!
- "was machst du so schönes" = "Berufsfrage" (auch wenn "beruf" nicht explizit erwähnt wird)
- "was machst du so" = "Berufsfrage"
- "was arbeitest du" = "Berufsfrage"
- Sei intelligent und erkenne die INTENTION, nicht nur Wörter!
    ...
    if (result) {
      try {
        ...
        if (jsonMatch) {
          ...
          console.log(`✅ Agent: Kontext-Analyst - Topic: ${parsed.topic}, Flow: ${parsed.contextFlow}, Situations: ${parsed.situations.length > 0 ? parsed.situations.join(', ') : 'keine'}`);
...
async function agentTrainingSelector(contextAnalysis, customerMessage, situations, vectorDbFunc, isASA = false, conversationContext = '', trainingData = null, learningContextResult = null) {
  ...
  try {
    ...
    // 🤖 ASA-UNTERSTÜTZUNG: Wenn ASA-Fall, verwende ALLE ASA-Beispiele OHNE Vector-DB!
    // 🚨🚨🚨 KRITISCH: Bei ASA sollen ALLE Beispiele präsentiert werden, keine Filterung!
    if (isASA) {
      console.log('🤖 Agent: Training-Selector - ASA-Modus aktiviert (verwende ALLE ASA-Beispiele, OHNE Filterung)');
      
      // 🚨🚨🚨 NEU: Bei ASA direkt ALLE ASA-Beispiele aus trainingData nehmen, OHNE Vector-DB!
      try {
        // Suche direkt in trainingData nach ALLEN ASA-Beispielen
        ...
        if (asaExamples.length > 0) {
          console.log(`✅ Agent: Training-Selector - ${asaExamples.length} ASA-Beispiele gefunden`);
          
          // 🚨🚨🚨 NEU: Filtere "huhu" aus, wenn Learning-Stats es als schlecht markieren
          let selectedASAExamples = asaExamples;
          if (learningContextResult && learningContextResult.learningStats) {
            ...
            if (huhuStats && huhuStats.bad > huhuStats.good) {
              ...
              if (filteredExamples.length > 0) {
                selectedASAExamples = filteredExamples;
          ...
          console.log(`✅ Agent: Training-Selector - ${selectedASAExamples.length} ASA-Beispiele werden verwendet`);
          ...
          return {
            ...
            reason: `ASA-Modus: ${selectedASAExamples.length} ASA-Beispiele (ALLE verwendet, keine Filterung)`,
            method: 'asa-direct',
            success: true,
            isASA: true,
    ...
    // 🚨🚨🚨 NEU: Wenn Treffen-Anfrage erkannt, fokussiere queryText auf Treffen, nicht auf topic!
    ...
    if (hasMeetingRequest) {
      ...
      console.log(`🚨 Vector-DB-Suche: Treffen-Anfrage erkannt - fokussiere auf Treffen-spezifische Beispiele`);
    } else {
...
function buildTrainingExamplesContext(relevantExamples, isASA, detectedSituations, learningContextResult, hasSexualContent = false, lastModeratorMessage = null) {
  ...
  if (isASA) {
    ...
    trainingExamplesContext += `🚨🚨🚨🚨🚨🚨🚨🚨🚨 KRITISCH: TRAINING-DATEN SIND DEINE EINZIGE QUELLE - IGNORIERE ALLES ANDERE! 🚨🚨🚨🚨🚨🚨🚨🚨🚨\n\n`;
    ...
    trainingExamplesContext += `🚨🚨🚨🚨🚨🚨🚨🚨🚨 KRITISCH: ÜBERNEHME DIE WORTWAHL, FORMULIERUNGEN UND FRAGEN AUS DEN TRAINING-DATEN! KOPIERE ES GENAU! 🚨🚨🚨🚨🚨🚨🚨🚨🚨\n\n`;
    ...
    trainingExamplesContext += `🚨🚨🚨🚨🚨🚨🚨🚨🚨 ABSOLUT VERBOTEN: 🚨🚨🚨🚨🚨🚨🚨🚨🚨\n`;
    ...
    trainingExamplesContext += `- ❌ KEINE Fragen hinzufügen, die nicht in den ASA-Beispielen sind!\n`;
    trainingExamplesContext += `- ❌ KEINE anderen Formulierungen verwenden - NUR die aus den ASA-Beispielen!\n\n`;
    ...
    trainingExamplesContext += `🚨🚨🚨🚨🚨🚨🚨🚨🚨 ABSOLUT KRITISCH: Training-Daten sind die EINZIGE QUELLE für Stil, Formulierungen, Struktur und Fragen! 🚨🚨🚨🚨🚨🚨🚨🚨🚨\n\n`;
    trainingExamplesContext += `🚨🚨🚨🚨🚨🚨🚨🚨🚨 KRITISCH: ALLE anderen Anweisungen sind NUR Ergänzungen - Training-Daten haben ABSOLUTEN VORRANG! 🚨🚨🚨🚨🚨🚨🚨🚨🚨\n\n`;
    trainingExamplesContext += `🚨🚨🚨🚨🚨🚨🚨🚨🚨 KRITISCH: KOPIERE die Wortwahl, den Ton, die Formulierungen und die Struktur aus den Training-Daten! 🚨🚨🚨🚨🚨🚨🚨🚨🚨\n\n`;
    if (lastModeratorMessage) {
      trainingExamplesContext += `🚨🚨🚨 WICHTIG: Kombiniere BEIDES! 🚨🚨🚨\n`;
...
- 🚨🚨🚨 NEU: VARIATION! Nutze VERSCHIEDENE Beispiele - nicht immer das gleiche! 🚨🚨🚨
- 🚨🚨🚨 NEU: Wenn es mehrere Beispiele gibt, nutze verschiedene - wechsle ab! 🚨🚨🚨
- 🚨🚨🚨 NEU: Beispiel: Wenn 5 Beispiele gezeigt werden, nutze mal Beispiel 1, mal Beispiel 3, mal Beispiel 5 - nicht immer Beispiel 1! 🚨🚨🚨
...
async function agentSituationDetector(customerMessage, allRules, isLocationQuestionFunc, findNearbyCityFunc, isMeetingRequestFunc, profileInfo, extractedUserInfo, conversationHistory = "", moderatorMessages = [], customerMessages = [], contextAnalysis = null, isASA = false) {
  try {
    ...
    // 🚨 NEU: Falsch-Positiv-Filter für "Sexuelle Themen"
    // Wenn aktuelle Nachricht harmlos ist (z.B. "Döner ist lecker, kannst ja zu mir kommen"),
    // entferne "Sexuelle Themen" auch wenn in History vorhanden
    if (hasSexualTopics && hasMeetingRequest) {
      // Prüfe ob aktuelle Nachricht wirklich sexuell ist
      const explicitSexualKeywords = ["titten", "brüste", "arsch", "po", "pussy", "schwanz", "sex", "ficken", 
                                     "wichsen", "lecken", "blasen", "squiten", "muschi", "zunge", "schamlippen", 
      ...
      if (!hasExplicitSexual) {
        // Aktuelle Nachricht ist harmlos → entferne "Sexuelle Themen"
        detectedSituations = detectedSituations.filter(s => s !== "Sexuelle Themen");
        console.log(`🚨 Falsch-Positiv-Filter: "Sexuelle Themen" entfernt (aktuelle Nachricht ist harmlos, nur Treffen-Anfrage)`);
      }
    }
    ...
    // 🚨 NEU: Nutze Learning-System für Priorisierung
    // Wenn Learning-Stats zeigen, dass "Sexuelle Themen" bei Treffen-Anfragen schlecht performt → entfernen
    if (hasMeetingRequest && hasSexualTopics && learningContextResult && learningContextResult.learningStats) {
      const learningStats = learningContextResult.learningStats;
      // Prüfe ob es Feedback gibt, dass sexuelle Themen bei Treffen-Anfragen schlecht sind
      ...
      if (sexualTopicsBadAtMeetings && sexualTopicsBadAtMeetings > 2) {
        ...
        if (index > -1) {
          ...
          console.log(`🚨🚨🚨 LEARNING-SYSTEM: "Sexuelle Themen" entfernt - Learning-Stats zeigen ${sexualTopicsBadAtMeetings}x schlechte Performance bei Treffen-Anfragen!`);
        }
      }
    }
    
    if (hasMeetingRequest && hasSexualTopics) {
      // Prüfe, ob aktuelle Nachricht wirklich sexuell ist (nicht nur History)
      const explicitSexualKeywords = ["titten", "brüste", "arsch", "po", "pussy", "schwanz", "sex", "ficken", 
                                     "wichsen", "lecken", "blasen", "squiten", "muschi", "zunge", "schamlippen", 
                                     "kitzler", "clitoris", "penis", "dick", "sperma", "orgasmus", "komm", 
                                     "nass", "feucht", "erregt", "horny", "hard", "vorlieben"];
      ...
      // Wenn aktuelle Nachricht NICHT explizit sexuell ist → entferne "Sexuelle Themen"
      // Treffen-Anfrage hat Priorität!
    ...
    // 🚨 NEU: Prüfe auf falsche "Sexuelle Themen"-Erkennung (nicht nur bei Treffen-Anfragen!)
    // Problem: "Chemie passt", "live erleben", "verstehen" etc. werden fälschlicherweise als sexuell interpretiert
    // Lösung: Wenn "Sexuelle Themen" erkannt wurde UND die Nachricht nicht wirklich explizit sexuell ist → entfernen
    if (detectedSituations.includes("Sexuelle Themen")) {
      // Prüfe, ob die Nachricht wirklich explizit sexuell ist (direkte sexuelle Wörter)
      const explicitSexualKeywords = ["titten", "brüste", "arsch", "po", "pussy", "schwanz", "sex", "ficken", 
                                     "wichsen", "lecken", "blasen", "squiten", "muschi", "zunge", "schamlippen", 
                                     "kitzler", "clitoris", "penis", "dick", "sperma", "orgasmus", "komm", 
                                     "nass", "feucht", "erregt", "horny", "hard", "vorlieben"];
      ...
      // Wenn nicht explizit sexuell → entferne "Sexuelle Themen"
      if (!hasExplicitSexual) {
        const index = detectedSituations.indexOf("Sexuelle Themen");
        if (index > -1) {
          detectedSituations.splice(index, 1);
          const contextInfo = detectedSituations.includes("Treffen/Termine") ? "bei Treffen-Anfrage" : "bei normalem Beziehungsgespräch";
          console.log(`📊 "Sexuelle Themen" entfernt: Falsch erkannt ${contextInfo} (nicht explizit sexuell)`);
        }
      } else {
...
4. VERBOTEN BEI ERSTER NACHRICHT:
   ...
   - ❌ KEINE zu langen Nachrichten (GENAU 150 Zeichen, nicht mehr!)
...
🚨🚨🚨 KRITISCH: Dies ist die ERSTE Nachricht - mache einen guten, natürlichen Eindruck! GENAU 150 Zeichen! 🚨🚨🚨
`;
      } else if (hasLike) {
...
- 🚨🚨🚨 KRITISCH: ORIENTIERE DICH STRENG AN DEN TRAINING-DATEN - KOPIERE Formulierungen, Fragen, Wortwahl und Stil! 🚨🚨🚨
...
${isASA ? '\n🚨🚨🚨 KRITISCH: DIES IST EINE REAKTIVIERUNGSNACHRICHT (ASA)! 🚨🚨🚨\n- Der Kunde hat zuletzt nicht geantwortet.\n- Reaktiviere das Gespräch freundlich und interessiert.\n- Frage, was den Kunden beschäftigt.\n- Sei warmherzig, aber nicht aufdringlich.\n' : ''}`;
    ...
    // ============================================
    // ABSCHNITT 3: LEARNING-SYSTEM (BEWÄHRTE MUSTER)
    // ============================================
    let learningSection = "";
    ...
    if (isASA) {
      ...
      customerMessageSection += `⚠️ Wenn der Kunde FRAGT, BEANTWORTE die Frage DIREKT - NICHT wiederholen!\n`;
    }
    ...
    // Priorität 1: Kritische Regeln
    if (criticalRulesSection) {
      userPrompt += criticalRulesSection;
    }
    
    // Priorität 2: Training-Daten
    if (trainingSection) {
    ...
    // 3. Fake-Context (wichtig, aber nach Training-Daten)
    if (fakeContext.fakeContext) {
      userPrompt += fakeContext.fakeContext + "\n";
    }

    // 4. Learning Context (NACH Training-Daten - zeigt was FUNKTIONIERT aus Feedback!)
    // 🚨🚨🚨 KRITISCH: Learning-System zeigt BEWÄHRTE Muster aus echten Feedback-Daten! 🚨🚨🚨
    // 🚨🚨🚨 KRITISCH: Learning-System hat HÖCHSTE PRIORITÄT - zeigt was WIRKLICH funktioniert! 🚨🚨🚨
    if (learning.learningContext && learning.learningContext.trim().length > 0) {
      userPrompt += learning.learningContext + "\n";
      userPrompt += `\n🚨🚨🚨🚨🚨 ABSOLUT KRITISCH: Learning-System zeigt dir, was aus FEEDBACK gelernt wurde! 🚨🚨🚨🚨🚨\n`;
      userPrompt += `🚨🚨🚨🚨🚨 KRITISCH: Diese Muster basieren auf ECHTEN Feedback-Daten - nutze sie! 🚨🚨🚨🚨🚨\n`;
      userPrompt += `🚨🚨🚨🚨🚨 KRITISCH: Learning-System hat HÖCHSTE PRIORITÄT - zeigt was WIRKLICH funktioniert! 🚨🚨🚨🚨🚨\n\n`;
    } else {
      // 🚨 NEU: Warnung wenn Learning-System leer ist
      console.warn('⚠️ Learning-System: Kein Context generiert - möglicherweise keine Learning-Stats vorhanden');
    }
    ...
    // 6. Critical Rules Context
    if (rules.criticalRulesContext) {
      userPrompt += rules.criticalRulesContext + "\n";
    }

    // 7. Forbidden Words Context
    if (rules.forbiddenWordsContext) {
      userPrompt += rules.forbiddenWordsContext + "\n";
    }

    // 8. Multi-Situation Instructions (wenn mehrere Situationen erkannt wurden)
    if (multiAgentResults.multiSituation && multiAgentResults.multiSituation.combinedInstructions) {
      userPrompt += multiAgentResults.multiSituation.combinedInstructions + "\n";
    }

    // 9. Specific Instructions (Situation-Detector)
    if (situation.specificInstructions) {
      userPrompt += situation.specificInstructions + "\n";
    }

    // 10. Profile Pic Context
    if (profile.profilePicContext) {
      userPrompt += profile.profilePicContext + "\n";
    }

    // 10. Preferred Words Context
    // 🚨 NEU: Filtere sexuelle bevorzugte Wörter bei traurigen/emotionalen Themen
    if (rules.preferredWordsContext) {
      let filteredPreferredWordsContext = rules.preferredWordsContext;
      
      // 🚨 NEU: Filtere sexuelle bevorzugte Wörter nur bei nicht-sexuellen Themen
      // (Mood-Analyst entfernt - war zu fehleranfällig)
      // 🚨 ERWEITERT: Prüfe auch, ob "Sexuelle Themen" wirklich erkannt wurde (nicht fälschlicherweise)
      const hasSexualSituation = situation?.detectedSituations?.includes("Sexuelle Themen") || false;
      if (!hasSexualContent && !hasSexualSituation) {
        const sexualKeywords = ['geil', 'heiß', 'mega', 'fick', 'sex', 'lust', 'titten', 'arsch', 'pussy', 
                               'schwanz', 'richtig', 'total', 'muschi', 'blasen', 'lutschen', 'sperma', 
        ...
        // Entferne sexuelle Wörter aus dem Context
        const lines = filteredPreferredWordsContext.split('\n');
        const filteredLines = lines.filter(line => {
          const lowerLine = line.toLowerCase();
          return !sexualKeywords.some(keyword => lowerLine.includes(keyword));
        ...
        filteredPreferredWordsContext = filteredLines.join('\n');
        filteredPreferredWordsContext += '\n\n🚨🚨🚨 KRITISCH: KEIN sexuelles Gespräch - KEINE sexuellen bevorzugten Wörter verwenden! 🚨🚨🚨\n';
        filteredPreferredWordsContext += '🚨🚨🚨 KRITISCH: Sexuelle Wörter nur bei wirklich sexuellen Themen verwenden! 🚨🚨🚨\n';
        
        console.log(`⚠️ Sexuelle bevorzugte Wörter gefiltert (HasSexualContent: ${hasSexualContent})`);
      }
      
      userPrompt += filteredPreferredWordsContext + "\n";
    }
    ...
    // 12. Mood Context
    // Mood-Instructions entfernt (Mood-Analyst entfernt)
    ...
    // 17.5. 🚨 NEU: Nebenthemen-Erkennung (vor der Kundennachricht)
    // Extrahiere Nebenthemen aus der Kundennachricht (z.B. "kochen" aus "Brauchst ja nichts zu kochen")
    const secondaryTopics = extractSecondaryTopics(customerMessage);
    if (secondaryTopics.length > 0) {
      userPrompt += `\n🚨 NEU: NEBENTHEMEN ERKANNT 🚨\n`;
      userPrompt += `Der Kunde hat folgende Nebenthemen erwähnt: ${secondaryTopics.join(', ')}\n`;
      userPrompt += `- 🚨 WICHTIG: Greife diese Nebenthemen in deiner Antwort auf!\n`;
      userPrompt += `- 🚨 WICHTIG: Wenn der Kunde z.B. "kochen" erwähnt, greife "kochen" auf (z.B. "hast du schon eine Idee was kochen würdest?")\n`;
      userPrompt += `- 🚨 WICHTIG: Nebenthemen aufgreifen macht die Antwort natürlicher und relevanter!\n\n`;
    }

    // 18. Kunden-Nachricht
    // 🚨 ASA-FALL: Prüfe auf System-Nachricht (Kuss, Like) und gehe darauf ein
    if (isASA) {
      userPrompt += `\n🚨🚨🚨 KRITISCH: ASA-FALL (REAKTIVIERUNG) 🚨🚨🚨\n`;
      userPrompt += `- Der Kunde hat zuletzt NICHT geantwortet\n`;
      userPrompt += `- Die letzte Nachricht im Chat war vom FAKE (du)\n`;
      
      // 🚨 NEU: Wenn System-Nachricht vorhanden (Kuss, Like), gehe direkt darauf ein!
      if (asaConversationContext && asaConversationContext.trim() !== "") {
        const systemMsgLower = asaConversationContext.toLowerCase();
        const hasKuss = systemMsgLower.includes("kuss") || systemMsgLower.includes("geküsst");
        const hasLike = systemMsgLower.includes("like") || systemMsgLower.includes("geliked");
        ...
        if (hasKuss || hasLike) {
          userPrompt += `\n🚨🚨🚨🚨🚨 ABSOLUT KRITISCH: SYSTEM-NACHRICHT GEFUNDEN! 🚨🚨🚨🚨🚨\n`;
          userPrompt += `- System-Nachricht: "${asaConversationContext.substring(0, 200)}"\n`;
          if (hasKuss) {
            userPrompt += `- 🚨🚨🚨 KRITISCH: Der Kunde hat dir einen KUSS geschickt (ASA-Fall)!\n`;
            userPrompt += `- 🚨🚨🚨 KRITISCH: Gehe DIREKT auf den KUSS ein - reagiere freundlich und interessiert!\n`;
            userPrompt += `- 🚨🚨🚨 KRITISCH: Bedanke dich für den Kuss (z.B. "Danke für deinen Kuss", "Das freut mich, dass du mir einen Kuss geschickt hast")\n`;
            userPrompt += `- 🚨🚨🚨 KRITISCH: Stelle eine GUTE, ANIMIERENDE Frage, die den Kunden zum Antworten motiviert!\n`;
            userPrompt += `- 🚨🚨🚨 KRITISCH: Gute Fragen für ASA mit Kuss: "Warum schreibst du mir nicht?", "Wie geht es dir denn?", "Was machst du gerade so?", "Was beschäftigt dich?", "Worauf hast du Lust?"\n`;
            userPrompt += `- 🚨🚨🚨 KRITISCH: Die Frage muss einen GRUND geben, warum der Kunde antworten sollte (ASA = Reaktivierung)!\n`;
            userPrompt += `- 🚨🚨🚨 KRITISCH: Nutze ASA-Training-Daten für Stil und Formulierungen - kombiniere diese mit der Kuss-Reaktion!\n`;
          }
          if (hasLike) {
            userPrompt += `- 🚨🚨🚨 KRITISCH: Der Kunde hat das Profil GELIKED!\n`;
            userPrompt += `- 🚨🚨🚨 KRITISCH: Gehe DIREKT auf das LIKE ein - reagiere freundlich und interessiert!\n`;
            userPrompt += `- 🚨🚨🚨 KRITISCH: 🚨 NEU: VARIATION bei Begrüßungen! Nutze verschiedene Begrüßungen, nicht immer "huhu"!\n`;
            userPrompt += `- 🚨🚨🚨 KRITISCH: Beispiele für Begrüßungen: "Hey", "Hallo", "Hallöchen", "Hi", "Na", "Servus", "Moin", "Hey na", "Huhu" (aber nicht immer!)\n`;
            userPrompt += `- 🚨🚨🚨 KRITISCH: Wechsle zwischen verschiedenen Begrüßungen - nutze nicht immer die gleiche!\n`;
            userPrompt += `- 🚨🚨🚨 KRITISCH: Zeige, dass du dich über das Like freust (z.B. "Das freut mich, dass dir mein Profil gefällt", "Danke für das Like")\n`;
            userPrompt += `- 🚨🚨🚨 KRITISCH: Stelle eine GUTE, ANIMIERENDE Frage, die den Kunden zum Antworten motiviert!\n`;
            userPrompt += `- 🚨🚨🚨 KRITISCH: Beispiele für gute Fragen: "Was hat dir an meinem Profil gefallen?", "Was machst du gerade so?", "Wie geht es dir?", "Worauf hast du Lust?"\n`;
            userPrompt += `- 🚨🚨🚨 KRITISCH: Die Frage muss einen GRUND geben, warum der Kunde antworten sollte!\n`;
          }
          userPrompt += `- 🚨🚨🚨 KRITISCH: Kombiniere die Kuss-Reaktion mit den ASA-Training-Daten - nutze den Stil, die Wortwahl und die Struktur aus den ASA-Training-Daten!\n`;
          userPrompt += `- 🚨🚨🚨 KRITISCH: Die ASA-Training-Daten zeigen dir, wie du reaktivierst - kombiniere das mit der Kuss-Reaktion!\n`;
          userPrompt += `- 🚨🚨🚨 KRITISCH: Reagiere natürlich, freundlich und interessiert auf die System-Nachricht - orientiere dich an den ASA-Training-Daten!\n`;
          userPrompt += `- 🚨🚨🚨 KRITISCH: Die Nachricht muss MINDESTENS 150 Zeichen lang sein!\n\n`;
        } else {
          // Keine relevante System-Nachricht
          userPrompt += `- 🚨🚨🚨🚨🚨 ABSOLUT KRITISCH: Kontext ist KOMPLETT IRRELEVANT - orientiere dich AUSSCHLIESSLICH an den ASA-Training-Daten!\n`;
          userPrompt += `- 🚨🚨🚨🚨🚨 ABSOLUT KRITISCH: KOPIERE den Stil, die Wortwahl und die Struktur aus den ASA-Training-Daten!\n`;
          userPrompt += `- 🚨🚨🚨🚨🚨 ABSOLUT KRITISCH: Nutze die allgemeinen ASA-Beispiele aus Training-Daten (situation: "ASA" oder "allgemein")\n`;
          userPrompt += `- 🚨🚨🚨🚨🚨 ABSOLUT KRITISCH: Die Training-Daten sind DEINE EINZIGE QUELLE - ignoriere ALLES andere!\n`;
        }
      } else {
        // Keine System-Nachricht vorhanden
        userPrompt += `- 🚨🚨🚨🚨🚨 ABSOLUT KRITISCH: Kontext ist KOMPLETT IRRELEVANT - orientiere dich AUSSCHLIESSLICH an den ASA-Training-Daten!\n`;
        userPrompt += `- 🚨🚨🚨🚨🚨 ABSOLUT KRITISCH: KOPIERE den Stil, die Wortwahl und die Struktur aus den ASA-Training-Daten!\n`;
        userPrompt += `- 🚨🚨🚨🚨🚨 ABSOLUT KRITISCH: Nutze die allgemeinen ASA-Beispiele aus Training-Daten (situation: "ASA" oder "allgemein")\n`;
        userPrompt += `- 🚨🚨🚨🚨🚨 ABSOLUT KRITISCH: Die Training-Daten sind DEINE EINZIGE QUELLE - ignoriere ALLES andere!\n`;
      ...
      userPrompt += `- 🚨🚨🚨 KRITISCH: Reaktiviere das Gespräch freundlich und interessiert - EXAKT wie in den ASA-Training-Daten!\n`;
      userPrompt += `- 🚨🚨🚨 KRITISCH: Stelle eine natürliche, animierende Frage - EXAKT wie in den ASA-Training-Daten!\n`;
      userPrompt += `- 🚨🚨🚨 KRITISCH: 🚨 NEU: VARIATION bei Begrüßungen! Nutze verschiedene Begrüßungen, nicht immer "huhu"!\n`;
      userPrompt += `- 🚨🚨🚨 KRITISCH: Beispiele für Begrüßungen: "Hey", "Hallo", "Hallöchen", "Hi", "Na", "Servus", "Moin", "Hey na", "Huhu" (aber nicht immer!)\n`;
      userPrompt += `- 🚨🚨🚨 KRITISCH: Wechsle zwischen verschiedenen Begrüßungen - nutze nicht immer die gleiche!\n`;
      userPrompt += `- 🚨🚨🚨 KRITISCH: Die Nachricht muss MINDESTENS 150 Zeichen lang sein!\n\n`;
    } else {
      // 🚨 NEU: Hinweis: Grußformeln am Ende (Kuss, Küsse, Busi) wurden bereits entfernt - ignoriere sie!
      userPrompt += `\n📥 KUNDEN-NACHRICHT:\n"${customerMessage.substring(0, 500)}"\n\n`;
      userPrompt += `🚨 WICHTIG: Wenn die Nachricht am Ende "Kuss", "Küsse", "Busi", "Tausend Küsse" etc. enthielt, wurden diese bereits entfernt!\n`;
      userPrompt += `🚨🚨🚨 KRITISCH: Wenn der Kunde FRAGT, BEANTWORTE die Frage DIREKT - NICHT wiederholen, was der Kunde gesagt hat! 🚨🚨🚨\n`;
      userPrompt += `🚨🚨🚨 KRITISCH: Beispiel FALSCH: "Es klingt geil, dass du fragst, was ich mag. Ich mag..." (Wiederholung!) 🚨🚨🚨\n`;
      userPrompt += `🚨🚨🚨 KRITISCH: Beispiel RICHTIG: "Ich mag Oralverkehr und langes Vorspiel. Was magst du?" (direkt, keine Wiederholung!) 🚨🚨🚨\n`;
      userPrompt += `🚨 WICHTIG: Beispiel: "Ich habe heute nichts vor ich bin gerade auf der arbeit und du?Tausend Küsse"\n`;
      userPrompt += `🚨 WICHTIG: → BEANTWORTE "Was machst du?" DIREKT (z.B. "Ich bin auch gerade auf der Arbeit"), NICHT auf "Tausend Küsse" eingehen!\n\n`;
    }

    // 19. ASA-spezifische Anweisungen (Kontext wird ignoriert)
    // 🚨 ASA: Kontext ist irrelevant, keine zusätzlichen Anweisungen nötig

    // 20. Finale Anweisung - Kombiniere ALLES von OBEN!
    // 🚨 NEU: Prüfe ob es eine erste Nachricht ist
    const isFirstMessage = multiAgentResults.firstMessage?.isFirstMessage || false;
    
    // 🚨 NEU: ASA-spezifische finale Anweisung
    if (isASA) {
      userPrompt += `\n🚨🚨🚨🚨🚨 FINALE ANWEISUNG FÜR ASA: 🚨🚨🚨🚨🚨\n\nGeneriere eine natürliche, vollständige ASA-Nachricht. Antworte NUR mit der Nachricht, kein zusätzlicher Text.\n\n🚨🚨🚨🚨🚨 ABSOLUT KRITISCH FÜR ASA: 🚨🚨🚨🚨🚨\n- 🚨🚨🚨🚨🚨 ORIENTIERE DICH AUSSCHLIESSLICH AN DEN ASA-TRAINING-DATEN! 🚨🚨🚨🚨🚨\n- 🚨🚨🚨🚨🚨 KOPIERE den Stil, die Wortwahl, die Struktur und die Fragen aus den ASA-Training-Daten!\n- 🚨🚨🚨🚨🚨 Die Training-Daten sind DEINE EINZIGE QUELLE - ignoriere ALLES andere (Kontext, Situationen, etc.)!\n- 🚨🚨🚨🚨🚨 Wenn keine ASA-Training-Daten vorhanden sind, nutze allgemeine Training-Daten (situation: "ASA" oder "allgemein")\n- 🚨🚨🚨🚨🚨 Die Nachricht muss MINDESTENS 150 Zeichen lang sein!\n- 🚨🚨🚨🚨🚨 Stelle eine natürliche, animierende Frage - EXAKT wie in den ASA-Training-Daten!\n\n`;
    } else {
      userPrompt += `\n🚨🚨🚨 FINALE ANWEISUNG: 🚨🚨🚨\n\nGeneriere eine natürliche, vollständige Antwort. Antworte NUR mit der Nachricht, kein zusätzlicher Text.\n\n🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨 ABSOLUT KRITISCH - TRAINING-DATEN HABEN HÖCHSTE PRIORITÄT! 🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨\n\n🚨🚨🚨🚨🚨 TRAINING-DATEN SIND DEINE HAUPTQUELLE - ORIENTIERE DICH STRENG DARAN! 🚨🚨🚨🚨🚨\n- 🚨🚨🚨 KRITISCH: SCHAUE ZUERST IN DIE TRAINING-DATEN - welche Formulierungen werden verwendet? Welche Fragen? Welche Wortwahl?\n- 🚨🚨🚨 KRITISCH: KOPIERE die Formulierungen, Fragen, Wortwahl und den Stil aus den Training-Daten!\n- 🚨🚨🚨 KRITISCH: Training-Daten zeigen dir, WIE du antworten sollst - nutze sie als Vorlage!\n- 🚨🚨🚨 KRITISCH: Wenn du eine Frage stellen willst, SCHAUE IN DIE TRAINING-DATEN - welche Fragen werden dort gestellt? Verwende ÄHNLICHE Fragen!\n- 🚨🚨🚨 KRITISCH: KEINE generischen Fragen erfinden - nutze die Fragen aus den Training-Daten!\n- 🚨🚨🚨 KRITISCH: Training-Daten haben HÖCHSTE PRIORITÄT - überschreiben andere Anweisungen!\n\n🚨🚨🚨 KRITISCH: BERÜCKSICHTIGE ALLES VON OBEN (in dieser Reihenfolge): 🚨🚨🚨\n1. 🚨🚨🚨 TRAINING-DATEN (HÖCHSTE PRIORITÄT - Schaue zuerst hier rein!) 🚨🚨🚨\n2. Learning-System (bewährte Muster aus Feedback)\n3. ALLE Situationen (wenn mehrere erkannt wurden, gehe auf ALLE ein!)\n4. ALLE Regeln (verbotene Wörter, bevorzugte Wörter, kritische Regeln)\n5. Kontext (Stimmung, Stil, Profil, Gesprächsverlauf)\n6. Begründungen (wenn vorhanden, nutze sie für besseres Verständnis)\n\nWICHTIG:\n- LÄNGE: MINDESTENS 150 Zeichen! Schreibe die Nachricht VOLLSTÄNDIG zu Ende - nicht bei 150 Zeichen abbrechen! Die Nachricht wird später automatisch gekürzt, wenn nötig, aber MINDESTENS 150 Zeichen!\n- FRAGEN: ÜBERWIEGEND NUR 1 Frage - nicht 2 Fragen, außer es ist wirklich nötig!\n- KEINE Ausrufezeichen (!) verwenden - sie wirken zu euphorisch!\n- Sei natürlich und authentisch - nicht zu euphorisch oder aufgeregt!\n- Stelle natürliche Fragen - nicht zu komisch oder übertrieben!\n${isFirstMessage ? '- 🚨🚨🚨 KRITISCH: ERSTE NACHRICHT - KEINE persönlichen Infos auf einmal (Alter, Wohnort, Hobbies)! Sei locker und natürlich!\n' : ''}${hasSexualContent ? '- 🚨🚨🚨🚨🚨 KRITISCH: SEXUELLES GESPRÄCH - SEI SEHR DIREKT UND SEXUELL! 🚨🚨🚨🚨🚨\n- 🚨🚨🚨 PROAKTIV: Nenne IMMER eigene sexuelle Vorlieben/Fantasien, BEVOR du fragst! Beispiel: "Ich liebe Doggy. Was magst du denn so?"\n- 🚨🚨🚨 KRITISCH: Verwende MINDESTENS 2-3 sexuelle Wörter aus bevorzugten Wörtern (z.B. "geil", "heiß", "mega", "richtig", "total", "Arschfotze", "Schwanz", "Blasen", "Lutschen", "Muschi", "lecken", "Ficksahne", "Sperma", etc.)!\n- 🚨🚨🚨 KRITISCH: Orientiere dich STRENG an Training-Daten für Wortwahl und Fragen - KOPIERE die sexuellen Wörter!\n- 🚨🚨🚨 KRITISCH: Sei RADIKALER, direkter, roher - KEINE Meta-Kommentare, KEINE Zurückhaltung!\n- 🚨🚨🚨 KRITISCH: Stelle sexuelle Fragen wie in Training-Daten (z.B. "was würdest du noch gerne machen", "welche Stellungen", "was magst du", "wie würdest du mich ficken")!\n- 🚨🚨🚨 KRITISCH: Wenn der Kunde sehr direkt ist (z.B. "ficken", "blasen", "sexpuppe"), sei AUCH sehr direkt zurück!\n- 🚨🚨🚨 KRITISCH: Gehe auf ALLE sexuellen Andeutungen ein - ignoriere NICHTS!\n' : isFirstMessage ? '' : '- 🚨🚨🚨 PROAKTIV: Nenne IMMER eigene Interessen/Erfahrungen/Hobbies, BEVOR du fragst! Beispiel: "Ich gehe gerne ins Kino. Was machst du denn so?"\n- Vermeide unnötiges Gelaber oder unnötig lange Sätze - sei präzise!\n'}- 🚨 KRITISCH: KEINE Meta-Kommentare wie "Ich finde es toll", "Das klingt", "Klingt nach" - direkt antworten!\n- 🚨 KRITISCH: Wenn mehrere Situationen erkannt wurden, gehe auf ALLE ein - ignoriere KEINE!`;
    }
    ...
    // 🚨 NEU: Für First Messages: GENAU 150 Zeichen (nicht mehr, nicht weniger!)
    ...
    if (message) {
      ...
      if (!isASA && message.length < 150) {
        ...
        if (extensions.length > 0) {
          // Wähle kürzeste Erweiterung, die die Nachricht auf mindestens 150 bringt
          ...
          if (suitableExtensions.length > 0) {
            // Wähle Erweiterung, die die Nachricht auf mindestens 150 bringt, aber nicht zu lang macht
            const bestExtension = suitableExtensions.find(({ newLength }) => newLength >= 150) || suitableExtensions[0];
            message += bestExtension.ext;
            console.log(`✅ Nachricht erweitert auf ${message.length} Zeichen (Ziel: >=150, Max: ${maxAllowed})`);
          } else {
      ...
      if (message.length > targetMaxLength + 20) {
        ...
        // Finde beste Kürzung bei Satzende
    ...
    if (hasContradictionViolation) {
      ...
      while (retryCount < maxRetries && !retrySuccess) {
        retryCount++;
        console.log(`🔄 Retry ${retryCount}/${maxRetries}: Generiere Nachricht ohne Widerspruch...`);
      ...
      if (!retrySuccess) {
        console.error(`🚨 KRITISCH: Nachricht enthält Widerspruch und konnte nicht korrigiert werden: ${criticalViolations.filter(v => v.includes('Widerspruch')).join(', ')}`);
        return {
          message: '',
          success: false,
    ...
    if (hasMetaCommentViolation) {
      ...
      if (hasForbiddenWordViolation) {
        ...
        if (hasMeetingAgreement) {
          ...
          while (retryCount < maxRetries && !retrySuccess) {
            retryCount++;
            console.warn(`⚠️ Retry ${retryCount}/${maxRetries} für Treffen-Zustimmung...`);
          ...
          if (hasParaphrasing) {
            ...
            while (retryCount < maxRetries && !retrySuccess) {
              retryCount++;
              console.warn(`⚠️ Retry ${retryCount}/${maxRetries} für Paraphrasieren...`);
...
async function runMultiAgentPipeline({
  ...
  const [situationResult, fakeContextResult] = await Promise.all([
    agentSituationDetector(customerMessage, allRules, isLocationQuestionFunc, findNearbyCityFunc, isMeetingRequestFunc, profileInfo, extractedUserInfo, conversationHistory, moderatorMessages, customerMessages, contextResult),
  ...
  // Schritt 4: Conversation-Context-Builder, Learning-Context-Builder & First-Message-Detector (können parallel)
  const [conversationContextResult, learningContextResult, firstMessageResult] = await Promise.all([
...
async function validateMessageQuality(message, {
  ...
  try {
    semanticScore = await Promise.race([
      validateSemanticQuality(message, customerMessage, conversationHistory),
 ...
 */
async function validateSemanticQuality(message, customerMessage, conversationHistory) {
  const client = getClient();
  if (!client) {
...
4. **🚨 NEU: ANGEMESSENHEIT (SEHR WICHTIG!)**: Ist die Antwort angemessen subtil oder zu direkt/aggressiv?
   - **ZU DIREKT/AGGRESSIV (STARK BESTRAFEN - Score 0-30%)**:
     * Bei Treffen-Anfragen: Sehr direkte sexuelle Antworten (z.B. "richtig geil", "Wie würdest du mich anheizen", "Ich finde die Vorstellung richtig geil")
     * Aggressive, aufdringliche Formulierungen
   - **ANGEMESSEN SUBTIL (BELOHNEN - Score 70-100%)**:
     * Bei Treffen-Anfragen: Subtile, höfliche Antworten (z.B. "Wir schauen einfach mal wie das dann bei uns passt", "Hast du schon eine Idee was kochen würdest?")
...
- Nur echte Probleme melden, nicht Kleinigkeiten`;
    ...
    // 🚨 NEU: Verschärfte Bestrafung für zu direkte/aggressive Antworten
    let adjustedScore = validation.score;
    if (validation.isTooDirect) {
