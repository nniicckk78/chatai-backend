...
async function runStage3Generation({
  ...
  const isLocal = !isASA && (process.env.USE_LOCAL_LLM === "true" || process.env.USE_LOCAL_LLM === "1");
  if (isASA) {
    console.log("🔍 STAGE 3: ASA-Nachricht - verwende OpenAI (nicht LoRA)");
  } else {
    console.log(`🔍 STAGE 3: Normale Reply-Nachricht - ${isLocal ? "LoRA-KI" : "OpenAI"} wird verwendet`);
...
${stage1Plan.isOngoingConversation ? '🚨🚨🚨 KRITISCH: Dies ist ein LAUFENDES Gespräch - KEINE Begrüßung mit "Hey" oder "Hallo"! 🚨🚨🚨' : ''}`;
    ...
    // Generiere Nachricht
    // 🚨 WICHTIG: LoRA-KI braucht länger (30-60 Sekunden für erste Anfrage)
    // Timeout: 120 Sekunden für LoRA-KI, 10 Sekunden für OpenAI
    const timeoutMs = isLocal ? 120000 : 10000; // 120s für LoRA, 10s für OpenAI
    console.log(`🚀 STAGE 3: Sende Anfrage an ${isLocal ? "LoRA-KI" : "OpenAI"} (Modell: ${messageModel}, Timeout: ${timeoutMs/1000}s)...`);
    ...
    const response = await Promise.race([
      ...
      console.error(`❌ STAGE 3: Fehler bei ${isLocal ? "LoRA-KI" : "OpenAI"}-Anfrage:`, error.message);
      if (isLocal) {
        ...
        console.error(`❌ STAGE 3: Erste Anfrage kann 30-60 Sekunden dauern - Timeout war ${timeoutMs/1000}s`);
      }
      throw error;
    });
...
async function runMultiStagePipeline({
  ...
  const stage1Plan = await runStage1Planning({
    customerMessage,
    conversationHistory,
    profileInfo,
    extractedUserInfo,
  ...
  const stage2Examples = await runStage2ExampleSelection({
    customerMessage,
    conversationHistory,
    stage1Plan,
    trainingData,
