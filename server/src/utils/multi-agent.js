...
async function agentLanguageDetector(customerMessage) {
  try {
    ...
    // 🚨 KRITISCH: Prüfe ob Nachricht auf Deutsch ist – NIE nur wegen eines einzelnen Wortes als Fremdsprache werten
...
- Wenn die Nachricht überwiegend (mehrere Sätze oder Großteil der Wörter) in einer anderen Sprache ist → markiere als "nicht deutsch"
- Wenn die Nachricht auf Deutsch ist (auch mit Rechtschreibfehlern oder Umgangssprache) → markiere als "deutsch"

WICHTIG – NIE wegen eines einzelnen Wortes als Fremdsprache werten:
- Ein einzelnes fremdsprachiges oder eingedeutschtes Wort in ansonsten deutscher Nachricht = DEUTSCH (z.B. Minijob, Job, okay, cool, Team, Chat, Date, Story, Like, Check, Feedback, Meeting, etc.)
- Deutsch mit Rechtschreibfehlern = DEUTSCH
- Deutsch in Umgangssprache = DEUTSCH
- Deutsch mit Emojis = DEUTSCH
- Nur wenn die Nachricht hauptsächlich in einer anderen Sprache ist (mehrere Sätze oder der Großteil der Wörter, z.B. Serbisch, Kroatisch, Englisch, Türkisch) = NICHT DEUTSCH
...
2. Danach 1–2 kurze Fragen stellen, um ein Gespräch zu starten:
   - Was machst du gerade? / Wie geht es dir? / Bist du noch auf der Arbeit? / Wie läuft dein Tag?
   - Passend zur Tageszeit/Wochentag (z.B. "Was hast du heute so vor?", "Schon Feierabend?").

REGELN:
- Keine Vorstellung (kein Name, kein Alter, kein Wohnort von dir – das kommt erst spaeter).
