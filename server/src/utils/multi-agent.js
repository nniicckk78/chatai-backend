 ...
 * @param {{ last10Messages: Array, mode: 'strong'|'soft', profileInfo: object, extractedUserInfo: object, noRealConversation?: boolean }} opts
 * @returns {Promise<string>} Nachricht mind. ASA_MIN_LENGTH Zeichen
 */
async function generateContextASA(opts) {
  const { last10Messages, mode, profileInfo = {}, extractedUserInfo = {}, noRealConversation = false } = opts;
...
FORMAT (wie beim normalen Generator): Immer ss, nie ß. KEINE Bindestriche (z. B. Hip Hop, kennenlernen, nicht Hip-Hop oder kennen-lernen). Keine Anführungszeichen am Anfang/Ende.`;
  ...
  const variationRule = `
VARIATION: Einstieg jedes Mal ANDERS (mal Hey, mal Oh, mal Na, mal direkt Frage) – NICHT immer „Hey, ich habe gerade an unser Gespräch über X gedacht“. Abwechslung bei Formulierung und Frage.`;
  ...
  let strongInstructions = `Schreibe EINE kurze Reaktivierungsnachricht, die DIREKT am letzten Thema des Kunden anknüpft (z. B. Sex, Arbeit, Schlafen, Hobbys).
Beispiele: Wenn das letzte Thema Sex war → auf das GESPRÄCH/Fantasie im Chat Bezug nehmen (nicht auf ein reales Treffen). Wenn der Kunde sagte, er geht arbeiten → "Hoffe bei der Arbeit läuft alles gut, denkst du auch an mich?"
Wenn er schlafen geht → "Hoffe du träumst was Schönes, hast du schonmal von mir geträumt?" oder "Erzähl mir wenn du wach bist, was du geträumt hast."
WICHTIG: Persönlich, zum letzten Thema passend, einladend. Keine generischen Floskeln. ${lengthHint} Nur die Nachricht, kein Prefix.`;
...
Stattdessen: kurze, freundliche Reaktivierung (z. B. Danke für die Anfrage / dass du dich gemeldet hast), dann 1–2 offene Fragen (wie geht es dir, was machst du, was magst du). Du darfst auf Profilinfos (Hobbys, Musik) eingehen, aber NICHT als „Gespräch“ bezeichnen – eher „ich seh du magst X“ oder „wie geht es dir“. ${lengthHint} Nur die Nachricht, kein Prefix.`;
  ...
  let softInstructions = `Schreibe EINE kurze Reaktivierungsnachricht, die zeigt, dass du an ihn denkst/ihn vermisst, mit EIN BISSCHEN Kontext aus dem Verlauf (z. B. "nach unserem Gespräch", "seit du weg bist").
Tonalität: "Wo bist du geblieben?", "Ich musste an dich denken", "Du hast mir gefehlt" – plus leichter Bezug zum letzten Thema, aber nicht das genaue Thema wiederholen.
...
Antworte NUR mit der einen Nachricht, nichts anderes.`;
  ...
  const userContent = `Letzte Nachrichten im Chat (Kunde / Fake):\n${conversationSnippet}\n\nSchreibe die Reaktivierungsnachricht (mind. ${ASA_MIN_LENGTH}, am besten 150–${ASA_IDEAL_MAX_CHARS} Zeichen). Denk an: Ihr habt euch nie getroffen – nur Bezug aufs Chat-Gespräch/Fantasie, nie "unser erstes Mal" o.ä.:`;
...
async function runMultiStagePipeline({
  ...
  if (isASA) {
    ...
    if (canUseContext) {
      ...
      if (useStrong) {
        asaMessageText = await generateContextASA({ last10Messages: last10, mode: 'strong', profileInfo, extractedUserInfo, noRealConversation: asaNoRealCustomerMessage });
        ...
        asaMessageText = await generateContextASA({ last10Messages: last10, mode: 'soft', profileInfo, extractedUserInfo, noRealConversation: asaNoRealCustomerMessage });
        useContextASA = asaMessageText.length > 0;
