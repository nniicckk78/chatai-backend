  ...
  try {
    ...
    if (isASA) {
      ...
      try {
        ...
        if (asaExamples.length > 0) {
          console.log(`✅ Agent: Training-Selector - ${asaExamples.length} ASA-Beispiele gefunden (ALLE werden verwendet, keine Filterung)`);
          
          // 🚨🚨🚨 KRITISCH: Verwende ALLE ASA-Beispiele, nicht nur 20!
          const selectedASAExamples = asaExamples; // Alle, keine Begrenzung!
    ...
    if (vectorDbFunc && typeof vectorDbFunc === 'function') {
      try {
        ...
        // Erhöhe topK für bessere Auswahl (mehr Beispiele = bessere Qualität)
        ...
        if (vectorResults && vectorResults.length > 0) {
          ...
          // 🚨 OPTION 1: Sortiere NUR nach semantischer Similarity (höher = besser)
          // Keine Situation-Priorisierung mehr - die Vector-DB hat bereits die besten Beispiele gefunden
...
async function agentSituationDetector(customerMessage, allRules, isLocationQuestionFunc, findNearbyCityFunc, isMeetingRequestFunc, profileInfo, extractedUserInfo, conversationHistory = "", moderatorMessages = [], customerMessages = [], contextAnalysis = null, isASA = false) {
  try {
    ...
    // 🚨 NEU: Nutze LLM-basierte Situation-Erkennung aus Context-Analyst (HÖCHSTE PRIORITÄT)
    if (contextAnalysis && contextAnalysis.situations && Array.isArray(contextAnalysis.situations) && contextAnalysis.situations.length > 0) {
      detectedSituations = [...contextAnalysis.situations];
      console.log(`✅ LLM-basierte Situation-Erkennung: ${detectedSituations.join(', ')}`);
    }
...
${isASA ? '\n🚨🚨🚨 KRITISCH: DIES IST EINE REAKTIVIERUNGSNACHRICHT (ASA)! 🚨🚨🚨\n- Der Kunde hat zuletzt nicht geantwortet.\n- Reaktiviere das Gespräch freundlich und interessiert.\n- Frage, was den Kunden beschäftigt.\n- Sei warmherzig, aber nicht aufdringlich.\n' : ''}`;
    ...
    // 🚨🚨🚨 NEUE REIHENFOLGE: Training-Daten haben ABSOLUT HÖCHSTE PRIORITÄT! 🚨🚨🚨
    let userPrompt = "";

    // 1. First-Message-Context (HÖCHSTE PRIORITÄT - wenn erste Nachricht!)
    ...
    // 2. 🚨🚨🚨 TRAINING EXAMPLES CONTEXT (ABSOLUT HÖCHSTE PRIORITÄT - direkt nach First-Message!) 🚨🚨🚨
    // 🚨🚨🚨 KRITISCH: Training-Daten sind die EINZIGE QUELLE für Stil, Formulierungen, Struktur und Fragen! 🚨🚨🚨
    // 🚨🚨🚨 KRITISCH: Alle anderen Anweisungen sind NUR Ergänzungen - Training-Daten haben VORRANG! 🚨🚨🚨
