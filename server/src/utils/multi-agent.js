...
/**
 * Wählt ein ASA-Beispiel per Round-Robin; optional unter den nächsten 5 Kandidaten den zum zuletzt
 * verwendeten Text diversesten (damit aufeinanderfolgende ASAs sich unterscheiden).
...
function selectOneASARoundRobinWithDiversity(pool, lastUsedText) {
  ...
  const CANDIDATES_FOR_DIVERSITY = 5;
  ...
  if (!useDiversity) {
    return { chosen: pool[idx], index: idx };
  }

  const candidates = [];
  for (let k = 0; k < CANDIDATES_FOR_DIVERSITY; k++) {
    candidates.push(pool[(idx + k) % N]);
  }
  let best = candidates[0];
  let bestDist = -1;
  for (const ex of candidates) {
    const sig = (ex.asaMessage || ex.moderatorResponse || '').trim().slice(0, 120).toLowerCase().replace(/\s+/g, ' ');
    const d = asaTextDistance(sig, lastSig);
    if (d > bestDist) {
      bestDist = d;
      best = ex;
    }
  }
  const chosenIndex = pool.indexOf(best);
  return { chosen: best, index: chosenIndex >= 0 ? chosenIndex : idx };
}
...
async function runMultiStagePipeline({
  ...
  if (isASA) {
    ...
    if (asaExamples.length > 0) {
      ...
      const chosenIdx = pool.indexOf(chosen);
      console.log(`🎲 ASA-Auswahl: Beispiel Index ${chosenIdx} von ${pool.length} (Round-Robin${lastUsed ? ' + Diversität' : ''})`);
