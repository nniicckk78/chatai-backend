...
// Zeitgrenzen (ohne Lücken):
// - Kein Kunde in den letzten 10 Nachrichten → POOL-ASA.
// - Letzte Kundennachricht ≥ 23 Std → POOL-ASA.
// - 30 Min ≤ Alter < 4 Std → KONTEXT-ASA STARK (am letzten Thema anknüpfen).
// - 4 Std ≤ Alter < 23 Std → KONTEXT-ASA WEICH („wo bist du“, „vermisst“ + etwas Kontext).
// - Unter 30 Min → POOL-ASA.
...
function getLast10MessagesAndCustomerAge(allMessages) {
  ...
  const customerInLast10 = last10.filter(m => isReceived(m) && isRealCustomerMessage(m));
...
async function runMultiStagePipeline({
  ...
  if (isASA) {
    ...
    const last10Info = getLast10MessagesAndCustomerAge(allMessages || []);
    const { last10, hasCustomerInLast10, ageHours } = last10Info;
    ...
    // Kontext-ASA nur wenn Kunde in den letzten 10 Nachrichten vorkommt und 50min–15h seit letzter Kundennachricht
    if (hasCustomerInLast10 && ageHours != null && ageMinutes >= ASA_CONTEXT_MIN_MINUTES && ageHours < ASA_POOL_FROM_HOURS) {
      if (ageHours <= ASA_STRONG_CONTEXT_MAX_HOURS) {
        ...
        if (useContextASA) console.log(`✅ ASA Kontext (stark, ~${Math.round(ageHours * 60)}min): ${asaMessageText.length} Zeichen`);
      } else {
