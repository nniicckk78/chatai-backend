...
Aufgabe:
1. situation: Kurz die Hauptsituation (z.B. "Besuch/Melde mich später", "Sexuelle Themen", "Treffen-Anfrage", "allgemein").
2. suggestedQuestions: 1-2 konkrete Fragen, die die Moderator-Antwort am Ende enthalten SOLL (zum Thema des Kunden). Beispiele:
   - Bei "Bekomme Besuch, melde mich später" -> ["Wie lange wird der Besuch bleiben?", "Weisst du schon wann du dich wieder melden kannst?"]
   - Bei Vorlieben/Sexuelles -> ["Was magst du am liebsten?", "Und du?"]
   - Bei Treffen-Diskussion (ohne konkrete Anfrage) -> ["Wann passt es bei dir am besten?", "Was machst du gern in deiner Freizeit?"]
  ...
  try {
    const response = await Promise.race([
      client.chat.completions.create({
        model: AGENT_MODEL,
        messages: [
          { role: 'system', content: 'Du bist eine Analyse-KI. Antworte NUR mit gültigem JSON, kein anderer Text.' },
    ...
    if (parsed && Array.isArray(parsed.suggestedQuestions) && parsed.suggestedQuestions.length > 0) {
      return {
        ...
        suggestedQuestions: parsed.suggestedQuestions.slice(0, 2),
        success: true
      };
    }
  } catch (err) {
