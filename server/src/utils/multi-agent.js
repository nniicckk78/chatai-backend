...
async function runMinimalStage1Planning({
  ...
  if (!client) {
    return {
      ...
      summary: '',
      keyPoints: [],
      isOngoingConversation: conversationHistory && conversationHistory.length > 0,
      criticalInstructions: '',
...
${stage1Plan.isOngoingConversation ? '🚨 KRITISCH: Dies ist ein LAUFENDES Gespräch - KEINE Begrüßung mit "Hey" oder "Hallo"! 🚨' : ''}`;
      ...
      // 🚨 NEU: Kompakter Kontext (nur wenn wichtig)
      let contextInfo = '';
      if (conversationHistory && conversationHistory.trim().length > 0) {
        const historyLines = conversationHistory.split('\n').filter(line => line.trim().length > 0);
        const recentHistory = historyLines.slice(-3).join(' '); // Nur letzte 3 Nachrichten, kompakt
        if (recentHistory && recentHistory.trim().length > 0) {
          contextInfo = `Kontext: ${recentHistory.substring(0, 150)}. `;
        }
      }
...
Antworte NUR als JSON:
{
  ...
  "changes": ["Änderung 1", "Änderung 2"]
      ...
      if (correctionParsed.correctedMessage) {
        console.log(`✅ STAGE 4: Nachricht korrigiert (${correctionParsed.changes?.length || 0} Änderungen)`);
        return {
          isValid: true,
          correctedMessage: correctionParsed.correctedMessage,
          changes: correctionParsed.changes || [],
          success: true
        };
      }
    }
