...
function selectOneASARoundRobinWithDiversity(pool, lastUsedText) {
  ...
  const CANDIDATES = Math.min(16, N); // Größeres Fenster → mehr Variation bei der Auswahl
  ...
  for (const { ex, index } of candidates) {
    ...
    // Frische UND Diversität: beide stark gewichten, damit wir Abwechslung haben (nicht immer ähnliche ASAs)
...
/**
 * Wählt genau 2 ASA-Beispiele: erstes per Round-Robin + Diversität, zweites möglichst unterschiedlich zum ersten (für Mix).
 * @param {Array<{ asaMessage?: string, moderatorResponse?: string }>} pool
 * @param {string|null} lastUsedText
 * @returns {Array<{ asaMessage?: string, moderatorResponse?: string }>}
 */
function selectTwoASAs(pool, lastUsedText) {
  if (!pool || pool.length === 0) return [];
  const N = pool.length;
  const firstResult = selectOneASARoundRobinWithDiversity(pool, lastUsedText);
  if (!firstResult) return [];
  const first = firstResult.chosen;
  const firstIdx = firstResult.index;
  const firstSig = (first.asaMessage || first.moderatorResponse || '').trim().slice(0, 150);
  if (N === 1) return [first];
  const rest = [];
  for (let k = 1; k < N; k++) {
    const i = (firstIdx + k) % N;
    rest.push({ ex: pool[i], index: i });
  }
  rest.sort((a, b) => asaTextDistance((b.ex.asaMessage || b.ex.moderatorResponse || '').trim().slice(0, 150), firstSig) - asaTextDistance((a.ex.asaMessage || a.ex.moderatorResponse || '').trim().slice(0, 150), firstSig));
  const selected = [first, rest[0].ex];
  asaLastUsedIndices.unshift(rest[0].index);
  if (asaLastUsedIndices.length > ASA_LAST_USED_MAX) asaLastUsedIndices.length = ASA_LAST_USED_MAX;
  return selected;
}
