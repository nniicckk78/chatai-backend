...
// - Unter 30 Min seit letzter Kundennachricht → POOL-ASA.
// - 30 Min ≤ Alter < 4 Std → KONTEXT-ASA STARK (direkt am letzten Thema/Satz des Kunden anknüpfen).
// - 4 Std ≤ Alter < 23 Std → KONTEXT-ASA WEICH („wo bist du“, „vermisst“, etwas Kontext aus Verlauf).
// - Kein Timestamp an letzter Kundennachricht (Extension sendet keine Zeit) → KONTEXT-ASA STARK (Fallback, damit nicht unnötig Pool).
const ASA_MIN_LENGTH = 150;
const ASA_IDEAL_MAX_CHARS = 250;
const ASA_CONTEXT_MIN_MINUTES = 30;
...
/** Prüft, ob eine Nachricht eine echte Kundennachricht ist (nicht System/Info wie Kuss, Like, Reaktivierungshinweis). */
function isRealCustomerMessage(msg) {
  if (!msg || typeof msg?.text !== 'string') return false;
  const t = (msg.text || '').trim();
  if (t.length < 2) return false;
  const lower = t.toLowerCase();
  if (/der (benutzer|kunde|user) hat (dich )?geküsst|like erhalten|bitte motiviere|reaktivierung|ein bild wurde übertragen/i.test(lower)) return false;
 ...
 * @returns {{ last10: Array, lastCustomerMsg: object|null, lastCustomerTime: number|null, ageHours: number|null, hasCustomerInLast10: boolean }}
 */
function getLast10MessagesAndCustomerAge(allMessages) {
  const out = { last10: [], lastCustomerMsg: null, lastCustomerTime: null, ageHours: null, hasCustomerInLast10: false };
  ...
  if (ts != null) out.ageHours = (Date.now() - ts) / (1000 * 60 * 60);
  return out;
}
...
- VERBOTEN: So tun, als hättet ihr euch schon getroffen oder etwas gemeinsam erlebt. Z. B. NIEMALS: „unser erstes Mal“, „erinnerst du dich an unser Treffen“, „als wir uns getroffen haben“, „beim letzten Mal“, „wie beim letzten Mal“, „wenn wir uns das nächste Mal sehen“ (ihr habt euch nie gesehen). Erlaubt: „unser Gespräch“, „als wir darüber geschrieben haben“, „was wir uns geschrieben haben“, Fantasie über „wenn es mal soweit wäre“.`;
  ...
  const strongInstructions = `Schreibe EINE kurze Reaktivierungsnachricht, die DIREKT am letzten Thema des Kunden anknüpft (z. B. Sex, Arbeit, Schlafen, Hobbys).
...
WICHTIG: Persönlich, zum letzten Thema passend, einladend. Keine generischen Floskeln. ${lengthHint} Nur die Nachricht, kein Prefix.`;
  ...
  const softInstructions = `Schreibe EINE kurze Reaktivierungsnachricht, die zeigt, dass du an ihn denkst/ihn vermisst, mit EIN BISSCHEN Kontext aus dem Verlauf (z. B. "nach unserem Gespräch", "seit du weg bist").
...
async function runMultiStagePipeline({
  ...
  if (isASA) {
    ...
    let { last10, hasCustomerInLast10, ageHours } = last10Info;
    const ageMinutes = ageHours != null ? ageHours * 60 : null;
    // Fallback: Wenn getLast10 aus allMessages keinen Kunden fand, aber wir Fallback-Messages haben → Kontext trotzdem nutzen
    if (!hasCustomerInLast10 && messagesForASA.length > 0 && (customerMessages?.length > 0 || messagesForASA.some(m => (m.type || m.messageType || '').toString().toLowerCase() === 'customer' || (m.type || m.messageType || '').toString().toLowerCase() === 'incoming'))) {
      ...
      console.log('🤖 ASA: Kunde in Fallback-Kontext erkannt → Kontext-ASA verwenden');
    }
    ...
    // Kontext-ASA: Kunde in letzten 10 Nachrichten + Alter der letzten Kundennachricht im Fenster 30min–23h
    // Wenn ageHours null (z. B. keine Timestamps von der Extension): trotzdem Kontext-ASA versuchen (stark), sonst unnötig Pool
    const canUseContext = hasCustomerInLast10 && (ageHours == null || (ageMinutes >= ASA_CONTEXT_MIN_MINUTES && ageHours < ASA_POOL_FROM_HOURS));
    if (canUseContext) {
      ...
      if (useStrong) {
        ...
        if (useContextASA) console.log(`✅ ASA Kontext (stark, ${ageHours == null ? 'kein Timestamp' : '~' + Math.round(ageHours * 60) + 'min'}): ${asaMessageText.length} Zeichen`);
      } else if (useSoft) {
    ...
    if (hasCustomerInLast10 && ageHours != null && (ageMinutes < ASA_CONTEXT_MIN_MINUTES || ageHours >= ASA_POOL_FROM_HOURS)) console.log(`🤖 ASA: Alter letzte Kundenmsg ${ageHours < 1 ? Math.round(ageMinutes) + 'min' : Math.round(ageHours) + 'h'} außerhalb 30min–23h → Pool-ASA`);
