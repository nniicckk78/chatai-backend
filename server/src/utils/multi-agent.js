...
// 🎲 ASA-Auswahl: Rotation durch alle Beispiele – zuletzt verwendete vermeiden, damit alle ~42 fair drankommen
...
const ASA_RECENTLY_USED_SIZE = 25; // Zuletzt verwendete werden vermieden → nicht immer dieselben paar
let asaRecentlyUsedSignatures = []; // Zuletzt verwendete Signaturen (neueste vorn)

/** Wählt ein ASA-Beispiel aus dem Pool; vermeidet die zuletzt verwendeten, damit alle Beispiele vorkommen. */
function selectOneASAWithRotation(pool) {
  if (!pool || pool.length === 0) return null;
  const recentSet = new Set(asaRecentlyUsedSignatures);
  const notRecentlyUsed = pool.filter(ex => !recentSet.has(getASASignature(ex)));
  const chooseFrom = notRecentlyUsed.length > 0 ? notRecentlyUsed : pool;
  const chosen = chooseFrom[Math.floor(Math.random() * chooseFrom.length)];
  const chosenSig = getASASignature(chosen);
  asaRecentlyUsedSignatures = [chosenSig, ...asaRecentlyUsedSignatures].slice(0, ASA_RECENTLY_USED_SIZE);
  return chosen;
}
    ...
    if (isASA) {
      ...
      try {
        ...
        if (asaExamplesRaw.length > 0) {
          ...
          // 🎲 ASA-Auswahl: Rotation – zuletzt verwendete vermeiden, damit alle Beispiele fair drankommen
          const pool = filteredASAExamples;
          const chosen = selectOneASAWithRotation(pool) || pool[0];
          const selectedASAExamples = [chosen];
          console.log(`✅ Agent: ASA-Beispiel Index ${pool.indexOf(chosen)} von ${pool.length} (Rotation durch alle Beispiele)`);
...
async function runMultiStagePipeline({
  ...
  if (isASA) {
    ...
    if (asaExamples.length > 0) {
      // 🎲 ASA-Auswahl: Rotation – zuletzt verwendete vermeiden, damit alle Beispiele vorkommen
      const pool = asaExamples;
      const chosen = selectOneASAWithRotation(pool) || pool[0];
      const selectedASAs = [chosen];
      console.log(`🎲 ASA-Auswahl: Beispiel Index ${asaExamples.indexOf(chosen)} von ${asaExamples.length} (Rotation)`);
...
async function runSimplifiedPipeline({
  ...
  if (isASA) {
    ...
    if (asaExamples.length > 0) {
      ...
      console.log(`✅ 1 ASA-Beispiel ausgewählt (von ${asaExamples.length} verfügbar, Rotation)`);
