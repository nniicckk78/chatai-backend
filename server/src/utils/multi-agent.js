...
${isASA ? '\n🚨🚨🚨 KRITISCH: DIES IST EINE REAKTIVIERUNGSNACHRICHT (ASA)! 🚨🚨🚨\n- Der Kunde hat zuletzt nicht geantwortet.\n- Reaktiviere das Gespräch freundlich und interessiert.\n- Frage, was den Kunden beschäftigt.\n- Sei warmherzig, aber nicht aufdringlich.\n' : ''}`;
    ...
    if (rules.preferredWordsContext) {
      ...
      // 🚨 NEU: Filtere sexuelle bevorzugte Wörter bei:
      // 1. Traurigen/frustrierten Moods
      // 2. Nicht-sexuellen Themen (z.B. Kaffee, Wetter, etc.)
      const shouldFilterSexualWords = (mood.mood === 'traurig' || mood.mood === 'frustriert') || !hasSexualContent;
      
      if (shouldFilterSexualWords) {
        const sexualKeywords = ['geil', 'heiß', 'mega', 'fick', 'sex', 'lust', 'titten', 'arsch', 'pussy', 
                               'schwanz', 'richtig', 'total', 'muschi', 'blasen', 'lutschen', 'sperma', 
                               'lecken', 'kitzler', 'vagina', 'penis', 'oral', 'anal', 'doggy', 'horny', 
                               'feucht', 'vorlieben', 'maulfotze', 'fotze', 'ficksahne', 'muschisaft',
        ...
        // Füge Warnung hinzu
        if (mood.mood === 'traurig' || mood.mood === 'frustriert') {
          filteredPreferredWordsContext += '\n\n🚨🚨🚨 KRITISCH: Kunde ist TRAURIG/FRUSTRIERT - KEINE sexuellen bevorzugten Wörter verwenden! 🚨🚨🚨\n';
          filteredPreferredWordsContext += '🚨🚨🚨 KRITISCH: Sei einfühlsam und unterstützend, NICHT sexuell! 🚨🚨🚨\n';
        } else if (!hasSexualContent) {
        ...
        console.log(`⚠️ Sexuelle bevorzugte Wörter gefiltert (Mood: ${mood.mood}, HasSexualContent: ${hasSexualContent})`);
      }
