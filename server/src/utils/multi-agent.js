...
async function agentSituationDetector(customerMessage, allRules, isLocationQuestionFunc, findNearbyCityFunc, isMeetingRequestFunc, profileInfo, extractedUserInfo, conversationHistory = "", moderatorMessages = [], customerMessages = [], contextAnalysis = null, isASA = false) {
  try {
    ...
    // 🚨 NEU: Prüfe auf falsche "Sexuelle Themen"-Erkennung bei Treffen-Anfragen
    // Problem: "Chemie passt", "live erleben" etc. werden fälschlicherweise als sexuell interpretiert
    // Lösung: Wenn "Treffen/Termine" erkannt wurde UND die Nachricht nicht wirklich explizit sexuell ist → "Sexuelle Themen" entfernen
    if (detectedSituations.includes("Treffen/Termine") && detectedSituations.includes("Sexuelle Themen")) {
      ...
      if (!hasExplicitSexual) {
        ...
        if (index > -1) {
          ...
          console.log(`📊 "Sexuelle Themen" entfernt: Falsch erkannt bei Treffen-Anfrage (nicht explizit sexuell)`);
        }
      } else {
        console.log(`📊 "Sexuelle Themen" behalten: Explizit sexuelle Wörter gefunden bei Treffen-Anfrage`);
      }
    }
