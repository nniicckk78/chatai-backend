...
function buildSimplifiedPrompt(multiAgentResults, {
  ...
  // 🧠 NEU: Few-Shot Examples mit Logik
  ...
  userPrompt += `🚨 KRITISCH: Denke LOGISCH wie in Beispiel 1 und 3, NICHT wie in Beispiel 2!\n\n`;
 ...
 */
async function aiSelfValidate(message, customerMessage, context, understoodRules, understoodLearning) {
  const client = getClient();
  if (!client) {
...
2. Gibt es Meta-Kommentare? (z.B. "Ich finde es spannend, mehr zu erfahren")
 ...
 */
async function aiSelfCorrect(message, issues, customerMessage, context, understoodRules, understoodLearning) {
  const client = getClient();
  if (!client) {
...
KORRIGIERE:
- Entferne Meta-Kommentare (z.B. "Ich finde es spannend, mehr zu erfahren")
