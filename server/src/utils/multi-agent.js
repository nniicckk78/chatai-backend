...
async function validateSemanticQuality(message, customerMessage, conversationHistory) {
  ...
  try {
    const validationPrompt = `Prüfe diese Chat-Antwort auf semantische/logische Korrektheit.
...
WICHTIG:
- Sei streng bei logischen Inkonsistenzen (z.B. "Salat" + "Belag" = falsch)
...
- Nur echte Probleme melden, nicht Kleinigkeiten`;
    ...
    if (!validation.isValid || validation.score < 50) {
      console.warn(`⚠️ Semantische Validierung: Probleme erkannt (Score: ${validation.score}%): ${validation.issues?.join(', ') || validation.reasoning || 'Unbekannt'}`);
    }
    
    // Score: 0-25% (validation.score ist 0-100, also /4)
    const semanticScore = Math.max(0, Math.min(25, (validation.score / 4)));
