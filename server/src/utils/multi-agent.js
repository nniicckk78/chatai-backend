...
Format:
- Länge: 150–250 Zeichen. Immer ss, nie ß. Keine Bindestriche. Keine Anführungszeichen am Anfang/Ende.
- Ausgabe: NUR die eine kombinierte Nachricht, sonst nichts.`;
  const userContent = `Nachricht A:\n${a}\n\nNachricht B:\n${b}\n\nKombiniere zu EINER flüssigen, paraphrasierten Reaktivierungsnachricht (Ideen aus A und B, aber deutlich andere Formulierung und Satzanfänge als in den Vorlagen):`;
  try {
...
async function runMultiStagePipeline({
  ...
  if (isASA) {
    ...
    if (asaMessageText) {
      asaMessageText = ensureASAMinLength(asaMessageText);
      asaMessageText = asaMessageText.replace(/-/g, ' ').replace(/ß/g, 'ss').replace(/\s+/g, ' ').trim();
      asaMessageText = ensureASAMinLength(asaMessageText);
