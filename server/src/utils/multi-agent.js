...
// 🎲 ASA-Variation: Zuletzt verwendete ASA-Beispiele (Signaturen), um Wiederholungen in kurzer Zeit zu reduzieren
const RECENTLY_USED_ASA_SIZE = 20; // Dieselbe ASA erst wieder nach mind. ~20 anderen Auswahlen
const recentlyUsedASASignatures = [];
    ...
    if (isASA) {
      ...
      try {
        ...
        if (asaExamplesRaw.length > 0) {
          ...
          // 🎲 ASA-Variation: Kürzlich verwendete Beispiele ausschließen (gleiche Logik wie runMultiStagePipeline)
          const recentSet = new Set(recentlyUsedASASignatures);
          let pool = filteredASAExamples.filter(ex => !recentSet.has(getASASignature(ex)));
          if (pool.length < 3) {
            pool = filteredASAExamples;
            recentlyUsedASASignatures.length = 0;
            console.log(`🎲 Agent: ASA-Pool - Alle ${filteredASAExamples.length} Beispiele („zuletzt verwendet“ zurückgesetzt)`);
          } else {
            console.log(`🎲 Agent: ASA-Pool - ${pool.length} von ${filteredASAExamples.length} Beispiele (${recentlyUsedASASignatures.length} kürzlich verwendet ausgeschlossen)`);
          }
          ...
          const chosenSig = getASASignature(selectedASAExamples[0]);
          if (chosenSig) {
            recentlyUsedASASignatures.push(chosenSig);
            if (recentlyUsedASASignatures.length > RECENTLY_USED_ASA_SIZE) {
              recentlyUsedASASignatures.shift();
            }
          }
          console.log(`✅ Agent: Training-Selector - 1 ASA-Beispiel zufällig ausgewählt (von ${pool.length} im Pool, ${filteredASAExamples.length} gesamt)`);
...
async function agentLanguageDetector(customerMessage) {
  try {
    ...
    if (msgTrim.length <= 200) {
      ...
      if (msgTrim.length <= 80 && !/[а-яА-ЯёЁ\u0400-\u04FF]/.test(msgTrim) && /\b(fkk|sauna|was|von|mit|und|oder|mag|magst|nackt)\b/i.test(msgLower)) {
        return {
          isGerman: true,
          detectedLanguage: 'de',
          needsGermanResponse: false,
...
- Enthält die Nachricht typisch deutsche Wörter (z.B. ich, du, nicht, und, ist, mit, der/die/das, war, habe, kann, sag, mal, vielleicht, weil, oder, auch, etwas, schon, noch, dann, wenn, dass, ob, keinen, grad, beruflich, auf, fall) oder Umlaute (ä, ö, ü, ß) → "isGerman": true.
- Deutsche Abkürzungen/Begriffe wie FKK, Sauna, Strand, nackt, Freikörperkultur, Textil, Nacktbereich, oder Formulierungen wie "was hältst du von FKK" = DEUTSCH (FKK ist ein typisch deutscher Begriff).
...
Antworte NUR als JSON, kein zusätzlicher Text.`;
    ...
    // Heuristik: Typisch deutsche Zeichen/Wörter → nicht als "nicht deutsch" werten (vermindert False-Positives)
    const msg = (customerMessage || '').trim().toLowerCase();
    const hasGermanChars = /[äöüß]/.test(customerMessage || '');
    const germanWordPattern = /\b(ich|du|nicht|und|ist|mit|der|die|das|war|habe|kann|sag|mal|vielleicht|weil|oder|auch|etwas|schon|noch|dann|wenn|dass|ob|meine|meiner|deine|frage|antwort|nachricht|schreiben|verstehen|keinen|keine|grad|beruflich|angespannt|auf|fall|nur|gerade|einfach|kommen|wollen|müssen|sollen|werden|werde|wird|hatte|hätte|können|möchte|will|passt|gut|gerne|lieber|besser|bestimmt|absicht|direkt|nahe|getreten|fkk|sauna|strand|nackt|freikörperkultur|nacktbereich|textil|hältst|von|magst|machst|gehst|öfter)\b/i;
    const hasLikelyGerman = hasGermanChars || germanWordPattern.test(msg) || (msg.length >= 8 && /\b(und|ist|nicht|mit|der|die|das|auf|nur|grad|fkk|was|von)\b/i.test(msg));
...
1. Kurz bedanken für den Kuss (z.B. "Danke für deinen Kuss" oder "Schön, danke für deinen Kuss").
2. Danach 1–2 kurze Fragen stellen, um ein Gespräch zu starten:
   ...
   - Passend zur Tageszeit/Wochentag (z.B. "Was hast du heute so vor?", "Schon Feierabend?").
...
DEINE ANTWORT – GENAU DIESE STRUKTUR:
1. Kurz bedanken für das Like (z.B. "Danke für dein Like" oder "Schön, danke dass du mich geliket hast").
2. Danach 1–2 kurze Fragen stellen, um ein Gespräch zu starten:
   - Wie geht es dir? / Was machst du gerade? / Wie läuft dein Tag? / Bist du noch auf der Arbeit?
...
async function runMultiStagePipeline({
  ...
  if (isASA) {
    ...
    if (asaExamples.length > 0) {
      // 🎲 ASA-Variation: Beispiele, die kürzlich verwendet wurden, von der Auswahl ausschließen
      const recentSet = new Set(recentlyUsedASASignatures);
      let pool = asaExamples.filter(ex => !recentSet.has(getASASignature(ex)));
      if (pool.length < 3) {
        pool = asaExamples;
        recentlyUsedASASignatures.length = 0;
        console.log(`🎲 ASA-Pool: Alle ${asaExamples.length} Beispiele („zuletzt verwendet“ zurückgesetzt)`);
      } else {
        console.log(`🎲 ASA-Pool: ${pool.length} von ${asaExamples.length} Beispiele (${recentlyUsedASASignatures.length} kürzlich verwendet ausgeschlossen)`);
      }

      // Seed-basierte Zufallsauswahl aus dem Pool
      ...
      // Ausgewähltes Beispiel in „zuletzt verwendet“ eintragen
      const chosenSig = getASASignature(selectedASAs[0]);
      if (chosenSig) {
        recentlyUsedASASignatures.push(chosenSig);
        if (recentlyUsedASASignatures.length > RECENTLY_USED_ASA_SIZE) {
          recentlyUsedASASignatures.shift();
      ...
      console.log(`🎲 Seed für ASA-Auswahl: chatId=${chatId}, ausgewähltes Beispiel Index: ${asaExamples.indexOf(selectedASAs[0])}`);
  ...
  if (isASA) {
    ...
    if (asaExamples.length > 0) {
      ...
      if (pool.length < 3) {
        pool = asaExamples;
        recentlyUsedASASignatures.length = 0;
        console.log(`🎲 ASA-Pool (Simplified): Alle ${asaExamples.length} Beispiele („zuletzt verwendet“ zurückgesetzt)`);
      } else {
        console.log(`🎲 ASA-Pool (Simplified): ${pool.length} von ${asaExamples.length} Beispiele (${recentlyUsedASASignatures.length} kürzlich verwendet ausgeschlossen)`);
      }
      const chatId = profileInfo?.chatId || extractedUserInfo?.chatId || 'unknown';
      const timestamp = Date.now();
      const randomComponent = Math.random() * 1000000;
