...
${isASA ? '\n🚨🚨🚨 KRITISCH: DIES IST EINE REAKTIVIERUNGSNACHRICHT (ASA)! 🚨🚨🚨\n- Der Kunde hat zuletzt nicht geantwortet.\n- Reaktiviere das Gespräch freundlich und interessiert.\n- Frage, was den Kunden beschäftigt.\n- Sei warmherzig, aber nicht aufdringlich.\n' : ''}`;
    ...
    if (message) {
      ...
      if (!proactiveCheck || !proactiveCheck.hasOwnExperience) {
        ...
        if (hasSexualContent) {
          ...
          // 🚨 NEU: Thematisch passende proaktive Ergänzungen
          // Prüfe auf spezifische Themen in der Nachricht UND conversationHistory
    ...
    // 🚨 NEU: Übergebe auch customerMessage und conversationHistory für Treffen-Zustimmung und Paraphrasieren-Erkennung
...
function validateCriticalRules(message, allRules, situation, isMeetingRequestFunc = null, customerMessage = "", conversationHistory = "") {
  ...
  // 🚨 NEU: Prüfe auf Paraphrasieren/Wiederholen der Kundennachricht
  if (customerMessage && customerMessage.trim().length > 0) {
    const similarity = calculateMessageSimilarity(message, customerMessage);
    if (similarity > 0.3) { // Mehr als 30% Ähnlichkeit = Paraphrasieren
      violations.push(`Paraphrasieren erkannt (${(similarity * 100).toFixed(0)}% Ähnlichkeit zur Kundennachricht) - blockiert`);
    }
  }
...
function calculateMessageSimilarity(message1, message2) {
