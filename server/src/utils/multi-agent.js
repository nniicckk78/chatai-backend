        ...
        if (messageEmbedding) {
          try {
            ...
            if (situationEmbedding) {
              ...
              // Schwellwert: 0.65 = hohe semantische Ähnlichkeit
              if (normalizedSimilarity > 0.65) {
...
${isASA ? '\n🚨🚨🚨 KRITISCH: DIES IST EINE REAKTIVIERUNGSNACHRICHT (ASA)! 🚨🚨🚨\n- Der Kunde hat zuletzt nicht geantwortet.\n- Reaktiviere das Gespräch freundlich und interessiert.\n- Frage, was den Kunden beschäftigt.\n- Sei warmherzig, aber nicht aufdringlich.\n' : ''}`;
            ...
            if (contextEmbedding) {
              ...
              for (const [topic, topicEmbedding] of Object.entries(topicEmbeddings)) {
                if (topicEmbedding) {
                  ...
                  if (normalizedSimilarity > bestSimilarity && normalizedSimilarity > 0.6) {
    ...
    if (customerMessage && customerMessage.trim().length > 0) {
      try {
        ...
        if (similarity > 0.65) { // 🚨 NEU: Höherer Schwellwert für semantische Ähnlichkeit (0.65 = sehr ähnlich)
