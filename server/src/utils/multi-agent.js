...
${stage1Plan.isOngoingConversation ? '🚨 KRITISCH: Dies ist ein LAUFENDES Gespräch - KEINE Begrüßung mit "Hey" oder "Hallo"! 🚨' : ''}`;
      ...
      // User-Prompt: VEREINFACHT - nur Kundennachricht + optional 1-2 Beispiele + Bild-Kontext
      let userPrompt = '';
      ...
      // Optional: 1-2 Beispiele (wenn gefunden)
      if (stage2Examples.selectedExamples && stage2Examples.selectedExamples.length > 0 && stage2Examples.selectedExamples.length <= 2) {
        userPrompt += `Beispiele wie du antworten sollst:\n\n`;
        stage2Examples.selectedExamples.forEach((ex, idx) => {
          userPrompt += `Beispiel ${idx + 1}:\n`;
          userPrompt += `Kunde: "${(ex.customerMessage || '').substring(0, 200)}"\n`;
          userPrompt += `Antwort: "${(ex.moderatorResponse || '').substring(0, 300)}"\n\n`;
        });
      }
      
      // 🚨 NEU: Spezielle Anweisungen für Berufsfragen (wenn Beruf bekannt ist)
      const hasJobQuestion = situations && situations.some(s => s.includes('Beruf') || s.includes('beruf'));
      ...
      userPrompt += `Antworte natürlich und authentisch auf die Kundennachricht.`;
...
async function runMultiStagePipeline({
  ...
  if (skipStages12) {
    ...
    // Minimaler Plan für STAGE 3 (nur für isOngoingConversation)
    stage1Plan = {
      topic: 'allgemein',
      summary: '',
      keyPoints: [],
    ...
    // Leere Beispiele (werden optional in STAGE 3 verwendet)
    stage2Examples = {
      selectedExamples: [],
      totalFound: 0
    };
