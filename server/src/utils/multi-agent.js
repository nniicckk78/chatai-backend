  ...
  try {
    ...
    if (isTogether) {
      ...
      if (situations && situations.length > 0) {
        ...
        // 🚨🚨🚨 KRITISCH: Stelle sicher, dass ALLE Situationen aus `situations` in `relevantSituations` sind!
        // Problem vorher: "Sexuelle Themen" war in `situations`, aber nicht in `relevantSituations`
        // Lösung: Füge ALLE Situationen hinzu, die in `situations` sind, auch wenn keine Regeln existieren!
        situations.forEach(s => {
          if (!relevantSituations.includes(s)) {
            // Wenn Regeln existieren, füge hinzu
            if (situationalResponses[s]) {
              relevantSituations.push(s);
              console.log(`✅ Situation "${s}" hinzugefügt (Regeln vorhanden)`);
            } else {
              // 🚨 KRITISCH: Auch wenn keine Regeln existieren, füge wichtige Situationen hinzu!
              // "Sexuelle Themen" und "Treffen/Termine" MÜSSEN immer übergeben werden!
              if (s.includes('Sexuell') || s.includes('sexuell') || s.includes('Treffen') || s.includes('Termine')) {
                relevantSituations.push(s);
                console.log(`🚨 KRITISCH: Wichtige Situation "${s}" hinzugefügt (auch ohne Regeln in rules.json)`);
              }
            }
          }
        });
...
async function runMultiStagePipeline({
  ...
  if (isASA) {
    ...
    if (asaExamples.length > 0) {
      // 🚨🚨🚨 FIX: WICHTIG - Nur 1 zufälliges ASA-Beispiel auswählen für maximale Variation!
      // 🚨 KRITISCH: Seed-basierte Zufallsauswahl mit chatId + timestamp + random für echte Variation!
      // Jede ASA-Anfrage muss ein ANDERES Beispiel bekommen!
      const chatId = profileInfo?.chatId || extractedUserInfo?.chatId || 'unknown';
      const timestamp = Date.now();
      const randomComponent = Math.random() * 1000000; // Zusätzliche Zufälligkeit
      const seed = (chatId.toString() + timestamp.toString() + randomComponent.toString()).split('').reduce((acc, char) => acc + char.charCodeAt(0), 0);
      ...
      console.log(`🎲 Seed für ASA-Auswahl: chatId=${chatId}, timestamp=${timestamp}, seed=${seed}, ausgewähltes Beispiel Index: ${asaExamples.indexOf(selectedASAs[0])}`);
