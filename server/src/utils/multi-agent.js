    ...
    if (isASA) {
      ...
      try {
        ...
        if (asaExamplesRaw.length > 0) {
          ...
          // 🚨 NEU: Seed-basierte Zufallsauswahl mit chatId + timestamp für echte Variation!
          const chatId = extractedUserInfo?.chatId || Date.now().toString();
          const timestamp = Date.now();
          const seed = (chatId + timestamp).split('').reduce((acc, char) => acc + char.charCodeAt(0), 0);
...
async function agentSituationDetector(customerMessage, allRules, isLocationQuestionFunc, findNearbyCityFunc, isMeetingRequestFunc, profileInfo, extractedUserInfo, conversationHistory = "", moderatorMessages = [], customerMessages = [], contextAnalysis = null, isASA = false, learningContextResult = null) {
  try {
    ...
    // 🚨 NEU: LLM-Erkennungen haben HÖCHSTE PRIORITÄT und werden IMMER behalten!
    let llmSituationsToKeep = [];
    if (llmDetectedSituationsWithConfidence && llmDetectedSituationsWithConfidence.confidence > 0.7) {
      // Wenn LLM hohe Confidence hat (>70%), behalte diese Situationen IMMER
      llmSituationsToKeep = llmDetectedSituationsWithConfidence.situations.filter(s => 
        detectedSituations.includes(s)
      );
      console.log(`🚨 LLM-Erkennung (Confidence: ${(llmDetectedSituationsWithConfidence.confidence * 100).toFixed(0)}%): Behalte Situationen: ${llmSituationsToKeep.join(', ')}`);
    }
    ...
    if (detectedSituations.length > 3) {
      ...
      if (messageEmbedding) {
        for (const situationName of detectedSituations) {
          // 🚨 NEU: LLM-Erkennungen bekommen höchsten Score
          const isLLMDetected = llmSituationsToKeep.includes(situationName);
          const baseScore = isLLMDetected ? 1.0 : 0.5; // LLM-Erkennungen = 1.0, andere = 0.5
          ...
          try {
            ...
            if (situationEmbedding) {
              const semanticSimilarity = cosineSimilarity(messageEmbedding, situationEmbedding);
              const normalizedSimilarity = (semanticSimilarity + 1) / 2;
              // Kombiniere LLM-Priorität mit semantischer Ähnlichkeit
              const finalScore = isLLMDetected ? Math.max(baseScore, normalizedSimilarity) : normalizedSimilarity;
              situationScores.push({ situation: situationName, score: finalScore, isLLM: isLLMDetected });
            } else {
              situationScores.push({ situation: situationName, score: baseScore, isLLM: isLLMDetected });
            }
          } catch (err) {
            situationScores.push({ situation: situationName, score: baseScore, isLLM: isLLMDetected });
          }
        ...
        // Fallback: Wenn kein messageEmbedding, LLM-Erkennungen = 1.0, andere = 0.5
        detectedSituations.forEach(s => {
          const isLLMDetected = llmSituationsToKeep.includes(s);
          situationScores.push({ situation: s, score: isLLMDetected ? 1.0 : 0.5, isLLM: isLLMDetected });
        });
      }
      
      // Sortiere nach Score (höchste zuerst) und nimm nur Top 2
      // 🚨 NEU: Aber behalte IMMER LLM-Erkennungen mit hoher Confidence!
      if (situationScores.length > 0) {
        situationScores.sort((a, b) => b.score - a.score);
        let topSituations = situationScores.slice(0, 2).map(s => s.situation);
        
        // 🚨 NEU: Füge LLM-Erkennungen hinzu, wenn sie nicht in Top 2 sind
        llmSituationsToKeep.forEach(llmSituation => {
          if (!topSituations.includes(llmSituation)) {
            topSituations.push(llmSituation);
            console.log(`🚨 LLM-Erkennung hinzugefügt (nicht in Top 2): ${llmSituation}`);
          }
        });
        
        console.log(`📊 Reduziere Situationen von ${detectedSituations.length} auf ${topSituations.length}: ${topSituations.join(', ')}`);
        detectedSituations = topSituations;
      } else {
        // Fallback: Wenn keine Scores, nimm einfach die ersten 2 + LLM-Erkennungen
        let topSituations = detectedSituations.slice(0, 2);
        llmSituationsToKeep.forEach(llmSituation => {
          if (!topSituations.includes(llmSituation)) {
            topSituations.push(llmSituation);
          }
        });
        console.log(`📊 Reduziere Situationen von ${detectedSituations.length} auf ${topSituations.length} (keine Scores verfügbar, LLM-Erkennungen behalten)`);
        detectedSituations = topSituations;
      }
    }
...
2. STELLE FRAGEN - KEINE INFOS PREISGEBEN:
   - 🚨🚨🚨 KRITISCH: Stelle FRAGEN (z.B. "Wie geht es dir?", "Was machst du so?", "Was suchst du?", "Wie läuft dein Tag?")
   - Frage nach Tageszeit/Wochentag (z.B. "Was machst du heute so?", "Was hast du an einem [Wochentag] vor?")
   - Stelle 1-2 natürliche Fragen, die den Kunden zum Antworten motivieren
   - ❌ KEINE persönlichen Infos über dich (Name, Alter, Wohnort, Beruf, Hobbies) - das kommt später, wenn der Kunde danach fragt!
...
1. NORMALE BEGRÜSSUNG MIT ZEITKONTEXT:
   - Begrüße locker und natürlich (z.B. "Hey na", "Hey", "Hallo")
   - Bedanke dich kurz für das Like (z.B. "danke dass du mich geliket hast" oder "schön dass du mich geliket hast")
   - 🚨 KRITISCH: Verwende "geliket" - NICHT "geschrieben hast"! Es war ein Like, keine geschriebene Nachricht!
   - Nutze Zeitkontext (Wochentag, Tageszeit) für natürliche Fragen
   - ❌ KEINE Vorstellung (kein Name, kein Alter, kein Wohnort - das kommt später!)
...
function buildSimplifiedPrompt(multiAgentResults, {
  ...
  if (structuredContext.lastModeratorMessage && structuredContext.lastModeratorMessage.length > 0) {
    // 🚨 KRITISCH: Zeige letzte Moderator-Nachricht EXPLIZIT!
    userPrompt += `💬💬💬 CHAT-VERLAUF (laufendes Gespräch) 💬💬💬\n\n`;
    ...
    userPrompt += `- Der Kunde antwortet IMMER auf deine letzte Nachricht\n`;
    userPrompt += `- Du musst den KONTEXT verstehen: Was wurde besprochen? Was ist das Thema?\n\n`;
    
    userPrompt += `🚨🚨🚨 KRITISCH: Der Kunde antwortet auf DEINE LETZTE NACHRICHT! 🚨🚨🚨\n\n`;
    ...
    userPrompt += `🚨🚨🚨 WICHTIG: Der Kunde reagiert auf DIESE Nachricht - gehe DIREKT darauf ein! 🚨🚨🚨\n`;
    userPrompt += `- Dies ist ein LAUFENDES GESPRÄCH - verstehe den Chat-Verlauf!\n`;
    userPrompt += `- Wenn du gefragt hast, dann hat der Kunde darauf geantwortet!\n`;
    userPrompt += `- Wenn du etwas gesagt hast, dann reagiert der Kunde darauf!\n`;
    userPrompt += `- Reagiere SPEZIFISCH auf die Kunden-Antwort, nicht generisch!\n`;
    userPrompt += `- Stelle KEINE Fragen, die bereits im Chat-Verlauf gestellt wurden!\n`;
    userPrompt += `- Wiederhole KEINE Informationen, die bereits im Chat-Verlauf besprochen wurden!\n\n`;
  }
  
  // Zeige letzten 3-4 Nachrichten für Kontext
  if (structuredContext.recentMessages.length > 0) {
    userPrompt += `📋 CHAT-VERLAUF (letzte Nachrichten, chronologisch - älteste zuerst, neueste zuletzt):\n\n`;
    ...
    userPrompt += `\n🚨 LOGIK-PRÜFUNG: Prüfe, welche Fragen bereits gestellt/beantwortet wurden!\n`;
    userPrompt += `🚨 WICHTIG: Verstehe den KONTEXT - was wurde besprochen? Was ist das Thema?\n`;
    userPrompt += `🚨 WICHTIG: Dies ist ein LAUFENDES GESPRÄCH - baue auf dem Chat-Verlauf auf!\n\n`;
    ...
    if (recentMessages.length > 0) {
      userPrompt += `📋 KONVERSATIONS-VERLAUF (letzte Nachrichten):\n\n`;
  ...
  userPrompt += `- 🚨 KRITISCH: Auf welche NACHRICHT antwortet der Kunde? (Meistens deine letzte Nachricht!)\n`;
  ...
  userPrompt += `- 🚨 KRITISCH: Auf welche NACHRICHT antwortet der Kunde? → Gehe DIREKT darauf ein!\n\n`;
  
  userPrompt += `SCHRITT 3: ANTWORT-GENERIERUNG (WIE ANTWORTE ICH?)\n`;
  userPrompt += `- 🚨 KRITISCH: Gehe DIREKT auf die aktuelle Kundennachricht ein (sie antwortet auf deine letzte Nachricht!)\n`;
  ...
  userPrompt += `7. LOGISCH: Wenn Kunde etwas sagt, gehe darauf ein (nicht ignorieren!)\n`;
  ...
  userPrompt += `5. GEHE DIREKT auf die Kundennachricht ein (sie antwortet auf deine letzte Nachricht!)\n`;
  ...
  try {
    ...
    if (useSimplifiedPrompt) {
      ...
      if (!message.includes('?')) {
        ...
        // Füge Frage hinzu
        const questionPrompt = `\n\nDie Nachricht enthält keine Frage. Füge eine passende Frage am Ende hinzu.`;
...
${isASA ? '\n🚨🚨🚨🚨🚨🚨🚨🚨🚨 KRITISCH: DIES IST EINE REAKTIVIERUNGSNACHRICHT (ASA)! 🚨🚨🚨🚨🚨🚨🚨🚨🚨\n- Der Kunde hat zuletzt nicht geantwortet.\n- Du hast ~40 ASA-Beispiele zur Verfügung - ein zufälliges wurde ausgewählt.\n- 🚨🚨🚨 DEINE AUFGABE: KOPIERE DAS ASA-BEISPIEL FAST 1:1 - NUR KLEINE VARIATIONEN ERLAUBT! 🚨🚨🚨\n- Ignoriere ALLE Situationen, Kontext, andere Analysen - NUR das ASA-Beispiel zählt!\n- Verwende die GLEICHEN Wörter, Formulierungen, Fragen wie im Beispiel (fast 1:1).\n- Reaktiviere das Gespräch freundlich und interessiert - ORIENTIERE DICH AM BEISPIEL!\n- Stelle die FRAGE aus dem Beispiel (fast 1:1).\n- Sei warmherzig, aber nicht aufdringlich - WIE IM BEISPIEL!\n- 🚨🚨🚨 DEINE NACHRICHT SOLLTE SICH ANFÜHLEN WIE EINE KOPIE DES BEISPIELS! 🚨🚨🚨\n' : ''}`;
    ...
    // Extrahiere offene Fragen (MUSS beantwortet werden!)
    ...
    // Extrahiere bereits beantwortete Fragen (DARF NICHT erneut gefragt werden!)
    ...
    const generateMessage = async (promptVariation = '', customTemperature = null) => {
      ...
      const temperature = customTemperature !== null ? customTemperature : (isASA ? 0.6 : 0.7); // 🚨 ASA: Niedrigere Temperature (0.6 statt 0.8) für stärkere Orientierung an Beispielen
      ...
      if (questionCount === 0 && !isASA) {
        ...
        if (!questionToAdd) {
          ...
          if (isAggressiveMessage || isNegativeFlow) {
            ...
            questionToAdd = "Was magst du denn so?";
...
async function runStage3Generation({
  ...
  try {
    ...
    // 🚀 VEREINFACHT FÜR FINE-TUNED MODELS (Together.ai)
    // Fine-Tuned Models haben Regeln/Stil bereits gelernt → einfacher Prompt
    if (isTogether) {
      // 🚨 NEU: Situations-spezifische Regeln aus rules.json (wie im Training)
      let situationalRules = '';
      const situationalResponses = allRules.situationalResponses || {};
      
      // 🚀 WICHTIG: Fine-Tuned Model hat Situationen bereits gelernt!
      // Füge NUR hinzu, wenn MEHRERE Situationen erkannt wurden (z.B. "Sexuelle Themen" + "Bilder Anfrage")
      // Dann müssen BEIDE Regeln hinzugefügt werden, damit das Model auf ALLES eingeht!
      if (situations && situations.length > 1) {
        // 🚨 KRITISCH: Wenn mehrere Situationen, füge ALLE relevanten Regeln hinzu!
        const relevantSituations = [];
        
        // Priorisiere "Sexuelle Themen" wenn vorhanden
        const sexualSituation = situations.find(s => s.includes('Sexuell') || s.includes('sexuell'));
        if (sexualSituation) relevantSituations.push(sexualSituation);
        
        // Füge "Bilder Anfrage" hinzu wenn vorhanden (wichtig für Foto-Anfragen!)
        const imageSituation = situations.find(s => s.includes('Bilder') || s.includes('Bild') || s.includes('Foto'));
        if (imageSituation && !relevantSituations.includes(imageSituation)) relevantSituations.push(imageSituation);
        
        // Füge andere Situationen hinzu
        situations.forEach(s => {
          if (!relevantSituations.includes(s) && situationalResponses[s]) {
            relevantSituations.push(s);
          }
        });
        
        // Füge Regeln für ALLE relevanten Situationen hinzu
        if (relevantSituations.length > 0) {
          situationalRules = `\n\n🚨🚨🚨 SITUATIONEN ERKANNT: ${relevantSituations.join(', ')} 🚨🚨🚨\n\n`;
          relevantSituations.forEach(sit => {
            if (situationalResponses[sit]) {
              situationalRules += `[${sit}]\n${situationalResponses[sit]}\n\n`;
            }
          });
          situationalRules += `🚨 KRITISCH: Gehe auf ALLE erkannten Situationen ein!`;
          console.log(`📋 Situations-spezifische Regeln für Together.ai hinzugefügt: ${relevantSituations.join(', ')}`);
        }
      } else if (situations && situations.length === 1) {
        // Nur eine Situation - füge sie hinzu (wie vorher)
        const primarySituation = situations[0];
        if (situationalResponses[primarySituation]) {
          situationalRules = `\n\n🚨🚨🚨 SITUATION: ${primarySituation} 🚨🚨🚨\n`;
          situationalRules += `${situationalResponses[primarySituation]}\n`;
          situationalRules += `\n🚨 KRITISCH: Diese Situation hat HÖCHSTE PRIORITÄT! Reagiere genau wie oben beschrieben!`;
          console.log(`📋 Situations-spezifische Regeln für Together.ai hinzugefügt: ${primarySituation}`);
        }
      }
      
      // 🚨 NEU: Extrahiere Profil-Informationen (wie in agentFakeContextBuilder)
      const fakeName = extractedUserInfo?.assistant?.Name || profileInfo?.moderatorInfo?.name || 'Du';
      const fakeAge = extractedUserInfo?.assistant?.Age || profileInfo?.moderatorInfo?.birthDate?.age || null;
      const fakeCity = extractedUserInfo?.assistant?.Wohnort || profileInfo?.moderatorInfo?.city || null;
      const fakeWork = extractedUserInfo?.assistant?.Work || profileInfo?.moderatorInfo?.occupation || null;
      
      // 🚨 NEU: Profil-Informationen für System-Prompt
      let profileInfoText = '';
      if (fakeName || fakeAge || fakeCity || fakeWork) {
        profileInfoText = `\n\n🚨🚨🚨 KRITISCH: DEINE FAKE-PROFIL INFORMATIONEN 🚨🚨🚨\n`;
        profileInfoText += `- Du bist das FAKE-PROFIL - der KUNDE schreibt DIR!\n`;
        if (fakeName) profileInfoText += `- Dein Name: ${fakeName}\n`;
        if (fakeAge) profileInfoText += `- Dein Alter: ${fakeAge} Jahre\n`;
        if (fakeCity) profileInfoText += `- Dein Wohnort: ${fakeCity}\n`;
        if (fakeWork) profileInfoText += `- Dein Beruf: ${fakeWork}\n`;
        profileInfoText += `- 🚨 KRITISCH: Wenn der Kunde nach deinem Alter, Wohnort, Beruf oder Namen fragt, MUSST du diese Informationen verwenden!\n`;
        profileInfoText += `- 🚨 KRITISCH: NIEMALS Platzhalter wie "[dein Beruf]" oder "[dein Name]" verwenden - verwende die ECHTEN Informationen!\n`;
        profileInfoText += `- 🚨 KRITISCH: Wenn der Kunde über dich redet, dann redet er über DICH - die Fake-Person!\n`;
      }
      
      // System-Prompt wie im Training (aus convert_to_together_ai.js)
      const systemPrompt = `Du bist ein Chat-Moderator auf einer Dating-Plattform. Du verwaltest verschiedene Fake-Profile und antwortest als diese Profile auf Kunden-Nachrichten.
...
${situationalRules}${profileInfoText}
WICHTIG: Du antwortest als FAKE-PROFIL (${fakeName}) auf Nachrichten vom KUNDEN. Der KUNDE schreibt DIR (dem Fake-Profil)!
${stage1Plan.isOngoingConversation ? '🚨 KRITISCH: Dies ist ein LAUFENDES Gespräch - KEINE Begrüßung mit "Hey" oder "Hallo"! 🚨' : ''}`;
      ...
      // 🚨🚨🚨 FIX: Verwende strukturierte Kontext-Extraktion für besseres Verständnis!
      const structuredContext = extractStructuredContext(conversationHistory);
      let contextInfo = '';
      
      // 🚨🚨🚨 KRITISCH: Wenn letzte Moderator-Nachricht gefunden, zeige sie EXPLIZIT!
      if (structuredContext.lastModeratorMessage && structuredContext.lastModeratorMessage.length > 0) {
        userPrompt = `💬💬💬 CHAT-VERLAUF (laufendes Gespräch) 💬💬💬\n\n`;
        userPrompt += `🚨🚨🚨 WICHTIG: Dies ist ein CHAT-VERLAUF - die Nachrichten sind in CHRONOLOGISCHER REIHENFOLGE! 🚨🚨🚨\n`;
        userPrompt += `- Älteste Nachricht steht OBEN, neueste Nachricht steht UNTEN\n`;
        ...
        userPrompt += `🚨🚨🚨 KRITISCH: Der Kunde antwortet auf DEINE LETZTE NACHRICHT! 🚨🚨🚨\n\n`;
        userPrompt += `DEINE LETZTE NACHRICHT war: "${structuredContext.lastModeratorMessage.substring(0, 400)}${structuredContext.lastModeratorMessage.length > 400 ? '...' : ''}"\n\n`;
        
        // Zeige letzten 2-3 Nachrichten für Kontext (wenn vorhanden)
        if (structuredContext.recentMessages.length > 1) {
          userPrompt += `📋 CHAT-VERLAUF (letzte Nachrichten, chronologisch - älteste zuerst, neueste zuletzt):\n\n`;
          structuredContext.recentMessages.slice(-3).forEach((msg, idx) => {
            const role = msg.type === 'moderator' ? 'Du' : 'Kunde';
            const position = idx === structuredContext.recentMessages.length - 1 ? 'NEUESTE' : 
                           idx === structuredContext.recentMessages.length - 2 ? 'VORLETZTE' : 'DRITTLETZTE';
            userPrompt += `${position} ${role}: "${msg.text.substring(0, 200)}${msg.text.length > 200 ? '...' : ''}"\n`;
          });
          userPrompt += `\n`;
        }
        
        userPrompt += `🚨🚨🚨 WICHTIG: Der Kunde reagiert auf DEINE LETZTE NACHRICHT - gehe DIREKT darauf ein! 🚨🚨🚨\n`;
        userPrompt += `- Dies ist ein LAUFENDES GESPRÄCH - verstehe den Chat-Verlauf!\n`;
        userPrompt += `- Wenn du gefragt hast, dann hat der Kunde darauf geantwortet!\n`;
        userPrompt += `- Wenn du etwas gesagt hast, dann reagiert der Kunde darauf!\n`;
        userPrompt += `- Reagiere SPEZIFISCH auf die Kunden-Antwort, nicht generisch!\n`;
        userPrompt += `- Verstehe den KONTEXT: Was war das Thema? Was wurde besprochen?\n`;
        userPrompt += `- Gehe auf das ein, was der Kunde gesagt hat, basierend auf DEINER letzten Nachricht!\n`;
        userPrompt += `- Stelle KEINE Fragen, die bereits im Chat-Verlauf gestellt wurden!\n`;
        userPrompt += `- Wiederhole KEINE Informationen, die bereits im Chat-Verlauf besprochen wurden!\n\n`;
      } else if (structuredContext.contextSummary && structuredContext.contextSummary.length > 0) {
        // Fallback: Wenn keine letzte Moderator-Nachricht, aber Kontext vorhanden
        contextInfo = `Kontext (laufendes Gespräch):\n${structuredContext.contextSummary}\n\n`;
      } else if (conversationHistory && conversationHistory.trim().length > 0) {
        // Fallback: Verwende conversationHistory direkt (kompakt)
        const historyLines = conversationHistory.split('\n').filter(line => line.trim().length > 0);
        const recentHistory = historyLines.slice(-5).join(' '); // Letzte 5 Nachrichten
        if (recentHistory && recentHistory.trim().length > 0) {
          contextInfo = `Kontext (laufendes Gespräch): ${recentHistory.substring(0, 300)}. `;
        }
      }
      
      // 🚨 NEU: Neue Informationen kompakt (nur wenn vorhanden)
      if (stage1Plan && stage1Plan.newInformation && stage1Plan.newInformation.hasNewInfo) {
        if (stage1Plan.newInformation.availability) {
          contextInfo += `WICHTIG: Kunde hat Verfügbarkeit erwähnt: "${stage1Plan.newInformation.availability}". `;
        }
        if (stage1Plan.newInformation.emotionalStatement) {
          contextInfo += `WICHTIG: Kunde hat emotionale Aussage gemacht: "${stage1Plan.newInformation.emotionalStatement}". `;
        }
        if (stage1Plan.newInformation.workMentioned) {
          contextInfo += `WICHTIG: Kunde hat Arbeit erwähnt: "${stage1Plan.newInformation.workMentioned}". `;
        }
      }
      
      // 🚨 NEU: Offene Fragen kompakt (nur wenn vorhanden)
      if (stage1Plan && stage1Plan.openQuestions && stage1Plan.openQuestions.length > 0) {
        contextInfo += `WICHTIG: Beantworte diese Frage: "${stage1Plan.openQuestions[0]}". `;
      }
      
      // 🚨 NEU: Bild-Kontext kompakt (nur wenn vorhanden)
      if (imageType && imageDescription) {
        if (imageType === 'penis' || imageType === 'dildo' || imageType === 'nude') {
          contextInfo += `KRITISCH: Kunde hat sexuelles Bild geschickt (${imageType}). Reagiere SEHR POSITIV und EXPLIZIT! `;
        } else {
          contextInfo += `Kunde hat Bild geschickt: "${imageDescription}". `;
        }
      }
      
      // 🚨 NEU: Prompt-Struktur wie im Training: "Kunde: '...', Antworte..."
      // Kompakt, fokussiert, ohne überladene Anweisungen
      if (!userPrompt) {
        // Fallback: Wenn keine letzte Moderator-Nachricht gefunden, verwende normalen Kontext
        userPrompt = `${contextInfo}Kunde: "${customerMessage}"\n\nAntworte als Chat-Moderator.`;
      } else {
        // Wenn letzte Moderator-Nachricht gefunden, füge Kontext und Kundennachricht hinzu
        userPrompt += `${contextInfo}Kunde: "${customerMessage}"\n\nAntworte als Chat-Moderator.`;
      }
      
      // 🚨 NEU: Bild-Kontext (wenn vorhanden)
      if (imageType && imageDescription) {
        if (imageType === 'penis') {
          userPrompt += `🖼️🚨🚨🚨 KRITISCH: SCHWANZ-BILD ERKANNT! 🚨🚨🚨\n\n`;
          userPrompt += `Der Kunde hat ein Bild mit seinem Schwanz/Penis geschickt!\n\n`;
          userPrompt += `🚨🚨🚨 ABSOLUT KRITISCH - EXPLIZITE REAKTION ERFORDERLICH! 🚨🚨🚨\n`;
          userPrompt += `- Du MUSST SEHR POSITIV und EXPLIZIT reagieren!\n`;
          userPrompt += `- Sage, wie GEIL der Schwanz aussieht!\n`;
          userPrompt += `- Sage, was du gerne mit ihm machen würdest (z.B. Blowjob, Handjob, etc.)!\n`;
          userPrompt += `- Zeige BEGEISTERUNG und INTERESSE!\n`;
          userPrompt += `- Sei DIREKT, FLIRTY und SEXUELL begeistert!\n`;
          userPrompt += `- KEINE neutrale Beschreibung - sei EMOTIONAL und ENTHUSIASTISCH!\n`;
          userPrompt += `- Stelle eine flirty, sexuelle Frage wie "In welches Loch würdest du ihn dann gerne als erstes schieben bei mir?" oder ähnlich!\n`;
          userPrompt += `- Die Bildbeschreibung ist: "${imageDescription}"\n\n`;
          userPrompt += `🚨🚨🚨 WICHTIG: Reagiere JETZT auf das Bild - es ist ein Schwanz-Bild, also sei sehr explizit und begeistert! Stelle eine flirty Frage! 🚨🚨🚨\n\n`;
        } else if (imageType === 'dildo' || imageType === 'nude') {
          userPrompt += `🖼️🚨 KRITISCH: SEXUELLES BILD ERKANNT (Typ: ${imageType})! 🚨\n\n`;
          userPrompt += `Der Kunde hat ein sexuelles Bild geschickt!\n\n`;
          userPrompt += `🚨 KRITISCH - EXPLIZITE REAKTION ERFORDERLICH! 🚨\n`;
          userPrompt += `- Du MUSST POSITIV und EXPLIZIT reagieren!\n`;
          userPrompt += `- Sage, wie GEIL es aussieht!\n`;
          userPrompt += `- Sage, was du gerne machen würdest!\n`;
          userPrompt += `- Zeige BEGEISTERUNG und INTERESSE!\n`;
          userPrompt += `- Sei DIREKT, FLIRTY und SEXUELL begeistert!\n`;
          userPrompt += `- Die Bildbeschreibung ist: "${imageDescription}"\n\n`;
          userPrompt += `🚨 WICHTIG: Reagiere JETZT auf das Bild - sei explizit und begeistert! 🚨\n\n`;
        } else {
          userPrompt += `🖼️ WICHTIG: Der Kunde hat ein Bild geschickt!\n\n`;
          userPrompt += `Die Bildbeschreibung ist: "${imageDescription}"\n\n`;
          userPrompt += `Du MUSST auf das Bild eingehen! Reagiere auf das, was im Bild zu sehen ist. Sei spezifisch und beziehe dich auf Details aus dem Bild.\n\n`;
        }
      }
      
      // 🚨 NEU: Kritische Anweisungen nur wenn absolut notwendig (kompakt)
      // Situations-spezifische Regeln sind bereits im System-Prompt!
      // Nur zusätzliche, kritische Anweisungen hier hinzufügen
      
      // 🚨 NEU: Treffen-Verbote (auch in sexuellen Konversationen) - kompakt
      const hasMeetingRequest = situations && situations.some(s => s.includes('Treffen') || s.includes('Termin'));
      const hasSexualSituation = situations && situations.some(s => s.includes('Sexuell') || s.includes('sexuell'));
      if (hasMeetingRequest || hasSexualSituation) {
        userPrompt += `\n\n🚨 WICHTIG: NIEMALS Treffen vorschlagen (auch nicht "besuchen", "vorbeikommen", "zu dir/mir kommen")!`;
      }
      
      // 🚨 NEU: Kombinierte Situation "Bilder + Kontaktdaten" - kompakt
      const hasImageRequest = situations && situations.some(s => s.includes('Bilder') || s.includes('Bild'));
      const hasContactData = situations && situations.some(s => s.includes('Kontaktdaten') || s.includes('Kontakt'));
      if (hasImageRequest && hasContactData) {
        userPrompt += `\n\n🚨 WICHTIG: Kunde will Bild schicken, hat vorher Telegram erwähnt. Reagiere SPIELERISCH (nicht technisch), lenke auf Beschreibung um!`;
      }

      // Generiere Nachricht mit vereinfachtem Prompt
      // 🚨 NEU: Optimierte Parameter für Fine-Tuned Models
      const timeoutMs = 30000; // 30s für Together.ai
      const temperature = 0.4; // 🚨 NEU: Niedrigere Temperature für konsistentere Antworten (Fine-Tuned Models)
      const maxTokens = 450; // 🚨 NEU: Mehr Tokens, damit Model nicht zu früh abbricht
...
Antworte NUR als JSON:
{
  ...
  "reasoning": "Warum ist die Nachricht gültig/ungültig?"
    ...
    // 🚨 NEU: Bei sexuellen Situationen KEINE Korrektur (Model soll sexuell antworten!)
    // ABER: Prüfe trotzdem auf Treffen-Vorschläge!
    ...
    if (isSexualSituation && !hasMeetingSuggestion) {
      console.log(`🚨 STAGE 4: Sexuelle Situation erkannt - überspringe Validierung/Korrektur (Model soll sexuell antworten!)`);
      return {
        isValid: true,
        correctedMessage: message, // Verwende Original-Nachricht ohne Korrektur
        changes: [],
        success: true
      };
    }
    
    // 🚨 NEU: Wenn Treffen-Vorschlag in sexueller Konversation → entferne ihn!
    if (isSexualSituation && hasMeetingSuggestion) {
      console.log(`🚨 STAGE 4: Sexuelle Situation + Treffen-Vorschlag erkannt - entferne Treffen-Vorschlag!`);
      // Entferne Treffen-Vorschläge aus der Nachricht
      ...
      // Wenn die Nachricht zu kurz wird, füge eine alternative Frage hinzu
      if (correctedMessage.length < 50) {
        correctedMessage = message.replace(/würdest du mich gerne.*\?/gi, 'Was würdest du denn gerne mit mir machen?');
        correctedMessage = correctedMessage.replace(/besuchen.*\?/gi, '');
        correctedMessage = correctedMessage.replace(/vorbeikommen.*\?/gi, '');
        correctedMessage = correctedMessage.replace(/\s+/g, ' ').trim();
      }
      ...
      return {
        ...
        changes: ['Treffen-Vorschlag entfernt'],
        success: true
      };
    }
    ...
    if (hasCriticalIssues && parsed.issues) {
...
Antworte NUR als JSON:
{
  ...
  "changes": ["Änderung 1", "Änderung 2"]
      ...
      if (correctionParsed.correctedMessage) {
        ...
        const changes = [...(correctionParsed.changes || [])];
...
async function runMultiStagePipeline({
  ...
  if (isASA) {
    ...
    if (asaExamples.length > 0) {
      // 🚨🚨🚨 FIX: WICHTIG - Nur 1 zufälliges ASA-Beispiel auswählen für maximale Variation!
      // Verwende Fisher-Yates Shuffle für echte Zufälligkeit (besser als sort(() => Math.random()))
      ...
      // 🚨 WICHTIG: Nur 1 Beispiel auswählen - das sorgt für maximale Variation zwischen ASA-Anfragen!
      const selectedASAs = shuffled.slice(0, 1);
  ...
  if (skipStages12) {
    ...
    // 🚨 ENTFERNT: Beispiele aus Vector-DB - Fine-Tuned Model braucht keine Beispiele im Prompt
    // Minimaler Plan ist bereits von runMinimalStage1Planning erstellt
    stage2Examples = {
      selectedExamples: [], // Keine Beispiele - Model hat bereits gelernt
      totalFound: 0
    };
  } else {
    // ============================================
  ...
  // 🚨 NEU: Wenn STAGE 1/2 übersprungen wurde, verwende erkannte Situationen
  const situationsForStage4 = skipStages12 ? detectedSituationsForTogether : situations;
  
  const stage4Validated = await runStage4Validation({
    ...
    allRules,
    isASA,
    situations: situationsForStage4 // 🚨 NEU: Situationen übergeben (für sexuelle Situationen)
  });
  
  console.log(`✅ STAGE 4 abgeschlossen: ${stage4Validated.isValid ? 'Nachricht ist gültig' : 'Nachricht wurde korrigiert'}`);
  
  // 🚨🚨🚨 WICHTIG: Verwende korrigierte Nachricht aus Stage 4!
  const finalMessage = stage4Validated.correctedMessage || stage3Generated.message;
...
async function runSimplifiedPipeline({
  ...
  if (isASA) {
    ...
    if (asaExamples.length > 0) {
      // 🚨🚨🚨 FIX: WICHTIG - Nur 1 zufälliges ASA-Beispiel auswählen für maximale Variation!
      // 🚨 NEU: Seed-basierte Zufallsauswahl mit chatId + timestamp für echte Variation!
      const chatId = profileInfo?.chatId || extractedUserInfo?.chatId || Date.now().toString();
      const timestamp = Date.now();
      const seed = (chatId + timestamp).split('').reduce((acc, char) => acc + char.charCodeAt(0), 0);
