...
// - 50 Min ≤ Alter < 4 Std → KONTEXT-ASA STARK (am letzten Thema anknüpfen).
// - 4 Std ≤ Alter < 23 Std → KONTEXT-ASA WEICH („wo bist du“, „vermisst“ + etwas Kontext).
// - Unter 50 Min → POOL-ASA.
const ASA_MIN_LENGTH = 150;
const ASA_CONTEXT_MIN_MINUTES = 50;
...
WICHTIG: Persönlich, zum letzten Thema passend, einladend. Keine generischen Floskeln. Mindestens ${ASA_MIN_LENGTH} Zeichen. Nur die Nachricht, kein Prefix.`;
...
Mindestens ${ASA_MIN_LENGTH} Zeichen. Nur die Nachricht, kein Prefix.`;
...
Antworte NUR mit der einen Nachricht, nichts anderes.`;
  ...
  const userContent = `Letzte Nachrichten im Chat (Kunde / Fake):\n${conversationSnippet}\n\nSchreibe die Reaktivierungsnachricht (mind. ${ASA_MIN_LENGTH} Zeichen). Denk an: Ihr habt euch nie getroffen – nur Bezug aufs Chat-Gespräch/Fantasie, nie "unser erstes Mal" o.ä.:`;
...
async function runMultiStagePipeline({
  ...
  // 🤖🤖🤖 ASA-FALL - Kontext-ASA (50min–15h) oder Pool-ASA (nie geantwortet / 23+ h), mind. 150 Zeichen
  if (isASA) {
