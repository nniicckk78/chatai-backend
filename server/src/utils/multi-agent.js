...
/**
 * Wählt genau 2 ASA-Beispiele: erstes aus ganzem Pool (LRU + Diversität), zweites möglichst
 * frisch UND unterschiedlich zum ersten – mehr Streuung, weniger gleiche Kombis.
 */
function selectTwoASAs(pool, lastUsedText) {
  ...
  for (let i = 0; i < N; i++) {
    ...
    const ex = pool[i];
    const sig = (ex.asaMessage || ex.moderatorResponse || '').trim().slice(0, 150);
    const distToFirst = asaTextDistance(sig, firstSig);
    const fresh = freshness(i);
    rest.push({ ex, index: i, distToFirst, fresh });
  }
  rest.sort((a, b) => {
    const scoreA = a.fresh * 1.2 + a.distToFirst * 2;
    const scoreB = b.fresh * 1.2 + b.distToFirst * 2;
    return scoreB - scoreA;
  });
  const second = rest[0];
  asaLastUsedIndices.unshift(second.index);
  if (asaLastUsedIndices.length > ASA_LAST_USED_MAX) asaLastUsedIndices.length = ASA_LAST_USED_MAX;
  return [first, second.ex];
}
