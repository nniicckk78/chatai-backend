...
${isASA ? '\n🚨🚨🚨🚨🚨🚨🚨🚨🚨 KRITISCH: DIES IST EINE REAKTIVIERUNGSNACHRICHT (ASA)! 🚨🚨🚨🚨🚨🚨🚨🚨🚨\n- Der Kunde hat zuletzt nicht geantwortet.\n- Du hast ~40 ASA-Beispiele zur Verfügung - ein zufälliges wurde ausgewählt.\n- 🚨🚨🚨 DEINE AUFGABE: KOPIERE DAS ASA-BEISPIEL FAST 1:1 - NUR KLEINE VARIATIONEN ERLAUBT! 🚨🚨🚨\n- Ignoriere ALLE Situationen, Kontext, andere Analysen - NUR das ASA-Beispiel zählt!\n- Verwende die GLEICHEN Wörter, Formulierungen, Fragen wie im Beispiel (fast 1:1).\n- Reaktiviere das Gespräch freundlich und interessiert - ORIENTIERE DICH AM BEISPIEL!\n- Stelle die FRAGE aus dem Beispiel (fast 1:1).\n- Sei warmherzig, aber nicht aufdringlich - WIE IM BEISPIEL!\n- 🚨🚨🚨 DEINE NACHRICHT SOLLTE SICH ANFÜHLEN WIE EINE KOPIE DES BEISPIELS! 🚨🚨🚨\n' : ''}`;
          ...
          if (processedVariants.length > 0) {
            ...
            const scoredVariants = await Promise.all(
              processedVariants.map(async (variant) => {
                ...
                // 🚨 OPTIMIERUNG: Quality-Validierung mit kürzerem Timeout (nicht kritisch für Hauptpfad)
                ...
                try {
                  qualityResultVariant = await Promise.race([
                    ...
                    }, 3000)) // 🚨 REDUZIERT: 3 Sekunden - Hauptpfad soll schnell sein
                  ]);
                } catch (err) {
    ...
    if (message) {
      ...
      // 🚨 KRITISCH: Proaktive Ergänzung nur wenn:
      // 1. AKTUELLE Situation "Sexuelle Themen" ist ODER aktuelle Nachricht explizit sexuell ist
      // 2. KEINE Treffen-Anfrage ohne explizit sexuellen Inhalt
      const shouldAddProactiveSexualAddition = hasSexualContent && !isMeetingWithoutExplicitSexual;
...
async function runMultiAgentPipeline({
  ...
  if (hasTrainingExamples) {
    ...
    // 🚨 OPTIMIERUNG: Max 5 Sekunden warten (schneller für Hauptpfad)
...
async function validateMessageQuality(message, {
  ...
  try {
    semanticScore = await Promise.race([
      ...
      new Promise((resolve) => setTimeout(() => {
        console.warn('⚠️ Semantische Validierung: Timeout nach 5 Sekunden - verwende Fallback');
        resolve(25); // Fallback: Volle Punkte
      }, 5000)) // 🚨 ERHÖHT: 5 Sekunden (statt 3) - Hauptpfad sollte funktionieren
    ]);
...
- Nur echte Probleme melden, nicht Kleinigkeiten`;
    ...
    const response = await Promise.race([
      ...
      new Promise((_, reject) => setTimeout(() => reject(new Error('Semantic validation timeout')), 5000)) // 🚨 ERHÖHT: 5 Sekunden (statt 3) - Hauptpfad sollte funktionieren
    ]);
...
{"hasConflict": true/false, "priority": "rules"|"examples", "guidance": "kurze Anleitung (max 100 Zeichen)"}`;
    ...
    const response = await Promise.race([
      ...
      new Promise((_, reject) => setTimeout(() => reject(new Error('Timeout')), 4000)) // 🚨 REDUZIERT: 4 Sekunden - Hauptpfad soll schnell sein
    ]);
