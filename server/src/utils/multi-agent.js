...
async function agentTrainingSelector(contextAnalysis, customerMessage, situations, vectorDbFunc, isASA = false, conversationContext = '', trainingData = null, learningContextResult = null) {
  ...
  try {
    ...
    // 🤖 ASA-UNTERSTÜTZUNG: Wenn ASA-Fall, suche NUR nach ASA-Beispielen (ohne Kontext!)
    if (isASA) {
      queryText = `ASA reaktivierung`;
      console.log('🤖 Agent: Training-Selector - ASA-Modus aktiviert (Kontext wird ignoriert, nur ASA-Beispiele)');
    }
    ...
    if (vectorDbFunc && typeof vectorDbFunc === 'function') {
      try {
        ...
        vectorResults = await vectorDbFunc(queryText, {
          ...
          situation: isASA ? null : primarySituation // 🚨 ASA: Keine Situation-Filterung (findet alle ASA-Varianten wie "Generische ASA (Neukunde)")
        });
        ...
        if (vectorResults && vectorResults.length > 0) {
          ...
          // 🚨 ASA: Ignoriere Situationen komplett - nutze nur allgemeine ASA-Beispiele
...
${isASA ? '\n🚨🚨🚨 KRITISCH: DIES IST EINE REAKTIVIERUNGSNACHRICHT (ASA)! 🚨🚨🚨\n- Der Kunde hat zuletzt nicht geantwortet.\n- Reaktiviere das Gespräch freundlich und interessiert.\n- Frage, was den Kunden beschäftigt.\n- Sei warmherzig, aber nicht aufdringlich.\n' : ''}`;
    ...
    // 4. Learning Context (NACH Training-Daten - nur als Ergänzung, nicht als Ersatz!)
    // 🚨 WICHTIG: Learning-Context ergänzt Training-Daten, überschreibt sie aber NICHT!
    ...
    // 🚨 NEU: Prüfe, ob eine Frage vorhanden ist (wenn targetAvgQuestions >= 1)
    // Problem: KI generiert manchmal Nachrichten ohne Frage, obwohl das Ziel 1 Frage ist
    // Lösung: Wenn keine Frage vorhanden → passende Frage hinzufügen
    // 🚨🚨🚨 KRITISCH FÜR ASA: KEINE generischen Fragen hinzufügen - NUR wenn Training-Daten zeigen, dass eine Frage fehlt!
    if (targetAvgQuestions >= 1 && !isASA) {
      // 🚨 NEU: Für ASA: KEINE generischen Fragen hinzufügen - Training-Daten sind die EINZIGE Quelle!
      const questionMatches = message.match(/\?/g);
      const questionCount = questionMatches ? questionMatches.length : 0;
      
      if (questionCount === 0) {
        console.warn(`⚠️ Nachricht enthält keine Frage (Ziel: ${targetAvgQuestions} Frage) - füge Frage hinzu...`);
        
        // 🚨 NEU: Wähle passende Frage basierend auf KUNDENNACHRICHT (kontextuell!), nicht generisch!
        let questionToAdd = "";
        const primarySituation = situation?.detectedSituations?.[0] || situation?.detectedSituations || 'allgemein';
        const situationName = Array.isArray(primarySituation) ? primarySituation[0] : primarySituation;
        
        // 🚨 NEU: Generiere kontextuelle Fragen basierend auf Kundennachricht
        ...
        if (hasSexualContent) {
          ...
          questionToAdd = sexualQuestions[Math.floor(Math.random() * sexualQuestions.length)];
        } else if (situationName === "Treffen/Termine") {
          // Treffen-Fragen
          questionToAdd = " Wie stellst du dir das denn vor?";
        } else if (situationName === "Berufsfrage" || customerMsgLower.includes('arbeit') || customerMsgLower.includes('beruf') || customerMsgLower.includes('job')) {
          // Berufs-Fragen
          questionToAdd = " Was machst du denn sonst noch so?";
        } else if (customerMsgLower.includes('umzug') || customerMsgLower.includes('räum') || customerMsgLower.includes('packen') || customerMsgLower.includes('kisten')) {
          // 🚨 NEU: Kontextuelle Fragen für Umzug/Räumen
          const umzugQuestions = [
            " Hast du denn wirklich niemanden der dir da helfen kann?",
            " Hast du da noch viel arbeit vor dir oder hast du das meiste schon geschafft?",
            " Wie lange brauchst du denn noch dafür?"
          ];
          questionToAdd = umzugQuestions[Math.floor(Math.random() * umzugQuestions.length)];
        } else if (customerMsgLower.includes('alleine') || customerMsgLower.includes('selbst')) {
          // 🚨 NEU: Kontextuelle Fragen für "alleine machen"
          const alleineQuestions = [
            " Hast du denn wirklich niemanden der dir da helfen kann?",
            " Wie schaffst du das alles alleine?",
            " Wie lange brauchst du denn noch dafür?"
          ];
          questionToAdd = alleineQuestions[Math.floor(Math.random() * alleineQuestions.length)];
        } else {
          // Allgemeine Fragen (fallback, aber seltener verwendet)
          const generalQuestions = [
            " Wie geht es dir denn so?",
            " Was beschäftigt dich denn gerade?"
          ];
          questionToAdd = generalQuestions[Math.floor(Math.random() * generalQuestions.length)];
        }
        
        // Füge Frage hinzu (natürlich, nicht einfach anhängen)
        if (questionToAdd) {
          const sentences = message.split(/(?<=[.!?])\s+/);
          if (sentences.length > 0) {
            const lastSentence = sentences[sentences.length - 1].trim();
            const trimmedQuestion = questionToAdd.trim();
            
            // Wenn letzter Satz mit Punkt endet, füge Frage als neuen Satz hinzu (natürlich)
            if (lastSentence.endsWith('.')) {
              // Frage als neuen Satz hinzufügen (natürlich)
              message = sentences.slice(0, -1).join(' ') + ' ' + lastSentence + ' ' + trimmedQuestion;
            } else if (lastSentence.endsWith('!')) {
              // Bei Ausrufezeichen: Frage als neuen Satz hinzufügen
              message = sentences.slice(0, -1).join(' ') + ' ' + lastSentence + ' ' + trimmedQuestion;
            } else {
              // Wenn kein Punkt/Ausrufezeichen (unwahrscheinlich), einfach anfügen
              message = message.trim() + ' ' + trimmedQuestion;
            }
            console.log(`✅ Frage hinzugefügt: "${trimmedQuestion}"`);
          } else {
            // Wenn keine Sätze, füge einfach am Ende hinzu
            message = message.trim() + ' ' + questionToAdd.trim();
            console.log(`✅ Frage hinzugefügt: "${questionToAdd.trim()}"`);
          }
        }
      }
    }
