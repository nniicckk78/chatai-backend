      ...
      try {
        ...
        if (learningDataForScoring) {
          ...
          if (processedVariants.length > 0) {
            // 🚨 OPTIMIERUNG: Bewerte Varianten - semantische Validierung optional (nicht blockierend)
            ...
            const scoredVariants = await Promise.all(
              processedVariants.map(async (variant) => {
                ...
                // Quality-Validierung: validateMessageQuality macht Embedding-Calls, daher ausreichend Zeit
                let qualityResultVariant;
                try {
                  qualityResultVariant = await Promise.race([
                    ...
                    }, 10000)) // 10 Sekunden (Embedding-Calls brauchen Zeit)
                  ]);
                } catch (err) {
                  console.warn('⚠️ Quality-Validierung Fehler für Variante - verwende Fallback-Score:', err.message);
                  qualityResultVariant = { overallScore: 75, styleScore: 50, trainingDataUsage: 50, contextUsage: 50, rulesCompliance: 100, learningSystemUsage: 50, semanticValidation: 25 };
                }
            ...
            console.log(`✅ Multi-Generator: Beste Variante ausgewählt (Stil: ${bestVariant.styleScore.toFixed(1)}%, Learning: ${bestVariant.learningScore.toFixed(1)}%, Quality: ${bestVariant.qualityScore.toFixed(1)}%, Semantic: ${(bestVariant.qualityResult?.scores?.semanticValidation || 25).toFixed(1)}%, Combined: ${bestVariant.combinedScore.toFixed(1)}%)`);
    ...
    if (message) {
      ...
      if (targetAvgQuestions > 0 && questionCount === 0 && !isASA) {
        ...
        if (!questionToAdd) {
          ...
          // Logische Fragen basierend auf Kontext
          if (customerMsgLower.includes('treffen') || customerMsgLower.includes('kennenlernen')) {
        ...
        // 3. Füge Frage zur Nachricht hinzu
        if (questionToAdd) {
...
async function runMultiAgentPipeline({
  ...
  if (hasTrainingExamples) {
    ...
    // 🚨 ERHÖHT: 10 Sekunden Timeout (besser für Stabilität)
    ruleInterpreterResult = await Promise.race([
      ...
      }, 5000)) // 🚨 REDUZIERT: 5 Sekunden (statt 8) - Hauptpfad soll schneller sein
    ]);
  } else {
...
async function validateMessageQuality(message, {
  ...
  if (hasTrainingData && !shouldUseFallbackMode) {
    try {
      ...
      if (messageEmbedding) {
        ...
        // 🚨 OPTIMIERUNG: Adaptive Anzahl - mehr Beispiele wenn verfügbar, aber max 5
  ...
  try {
    ...
    if (learningStats?.responsePatterns && learningStats.responsePatterns.length > 0) {
      ...
      if (messageEmbedding) {
        ...
        // 🚨 OPTIMIERUNG: Adaptive Anzahl - mehr Patterns wenn verfügbar, aber max 5
  ...
  try {
    // Timeout: 7 Sekunden (validateSemanticQuality hat 6s Timeout, plus 1s Buffer)
    semanticScore = await Promise.race([
      validateSemanticQuality(message, customerMessage, conversationHistory, hasSexualContent),
      new Promise((resolve) => setTimeout(() => {
        console.warn('⚠️ Semantische Validierung: Timeout nach 7 Sekunden - verwende Fallback');
        resolve(25); // Fallback: Volle Punkte
      }, 7000)) // 7 Sekunden (6s + 1s Buffer)
    ]);
...
{"isValid": true/false, "score": 0-100, "issues": [], "reasoning": "max 100 Zeichen", "isTooDirect": true/false, "isAppropriatelySubtle": true/false}`;
    ...
    // Timeout: 6 Sekunden (ausreichend für Validierung, aber nicht zu lang)
    const response = await Promise.race([
      client.chat.completions.create({
        model: AGENT_MODEL,
        messages: [
      ...
      new Promise((_, reject) => setTimeout(() => reject(new Error('Semantic validation timeout')), 6000)) // 6 Sekunden
    ]);
      ...
      try {
        ...
        if (jsonMatch) {
          ...
          if (hasPreferencesInMessage) {
            ...
            contextGuidance += `- 🚨 NICHT nochmal fragen "Was sind deine Vorlieben?" - das hat er bereits gesagt!\n`;
