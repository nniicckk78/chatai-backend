...
async function agentSituationDetector(customerMessage, allRules, isLocationQuestionFunc, findNearbyCityFunc, isMeetingRequestFunc, profileInfo, extractedUserInfo, conversationHistory = "", moderatorMessages = [], customerMessages = [], contextAnalysis = null, isASA = false) {
  try {
    ...
    if (detectedSituations.length > 3) {
      ...
      // Sortiere nach Score (höchste zuerst) und nimm nur Top 3
      if (situationScores.length > 0) {
        situationScores.sort((a, b) => b.score - a.score);
        const topSituations = situationScores.slice(0, 3).map(s => s.situation);
        
        console.log(`📊 Reduziere Situationen von ${detectedSituations.length} auf Top 3: ${topSituations.join(', ')}`);
        detectedSituations = topSituations;
      } else {
        // Fallback: Wenn keine Scores, nimm einfach die ersten 3
        console.log(`📊 Reduziere Situationen von ${detectedSituations.length} auf Top 3 (keine Scores verfügbar)`);
        detectedSituations = detectedSituations.slice(0, 3);
      }
    }
...
${isASA ? '\n🚨🚨🚨 KRITISCH: DIES IST EINE REAKTIVIERUNGSNACHRICHT (ASA)! 🚨🚨🚨\n- Der Kunde hat zuletzt nicht geantwortet.\n- Reaktiviere das Gespräch freundlich und interessiert.\n- Frage, was den Kunden beschäftigt.\n- Sei warmherzig, aber nicht aufdringlich.\n' : ''}`;
    ...
    if (isASA) {
      ...
      if (asaConversationContext && asaConversationContext.trim() !== "") {
        ...
        if (hasKuss || hasLike) {
          ...
          userPrompt += `- 🚨🚨🚨 WICHTIG: Kontext ist IRRELEVANT - orientiere dich NUR an den ASA-Training-Daten!\n`;
          userPrompt += `- 🚨🚨🚨 WICHTIG: Nutze die allgemeinen ASA-Beispiele aus Training-Daten (situation: "ASA" oder "allgemein")\n`;
        }
      } else {
      ...
      userPrompt += `- Stelle eine natürliche, animierende Frage\n\n`;
    } else {
    ...
    if (message) {
      ...
      // Wenn keine eigenen Erfahrungen/Interessen genannt wurden, ergänze sie THEMATISCH PASSEND
      // ABER: Bei ersten Nachrichten NICHT (zu viele Infos auf einmal!)
      if (!isFirstMessage && (!proactiveCheck || !proactiveCheck.hasOwnExperience)) {
        console.warn(`⚠️ Nachricht enthält keine eigenen Erfahrungen/Interessen - ergänze proaktive Elemente...`);
        
        // 🚨 NEU: Erkenne aktuelles Thema aus Nachricht und Kontext
        const lowerMessage = message.toLowerCase();
        const lowerCustomerMessage = (customerMessage || '').toLowerCase();
        const combinedText = (lowerMessage + ' ' + lowerCustomerMessage).toLowerCase();
        
        // 🚨 NEU: Prüfe auch conversationHistory für Kontext (falls verfügbar)
        // conversationHistory ist in params verfügbar
        const lowerHistory = (conversationHistory || '').toLowerCase();
        const fullContext = (combinedText + ' ' + lowerHistory).toLowerCase();
        
        // Kontextabhängige proaktive Ergänzungen - THEMATISCH PASSEND
        let proactiveAddition = '';
        let topicDetected = false;
        
        // 🚨 NEU: Proaktive Ergänzungen nur bei sexuellen Themen UND wenn es wirklich passt
        // (Mood-Analyst entfernt - keine Mood-Filterung mehr nötig)
        if (hasSexualContent) {
          // Sexuelle proaktive Ergänzungen
          ...
          proactiveAddition = sexualAdditions[Math.floor(Math.random() * sexualAdditions.length)];
          topicDetected = true;
        } else {
          // 🚨 NEU: Semantische Themen-Erkennung mit Embeddings (nutzt gecachte Embeddings!)
          try {
            const { getEmbedding, cosineSimilarity, getAllTopicEmbeddings } = require('./embeddings');
            const contextEmbedding = await getEmbedding(fullContext || combinedText);
            
            if (contextEmbedding) {
              // 🚨 WICHTIG: Nutze gecachte Themen-Embeddings statt neu zu generieren!
              const topicEmbeddings = getAllTopicEmbeddings();
              
              let bestTopic = null;
              let bestSimilarity = 0;
              
              for (const [topic, topicEmbedding] of Object.entries(topicEmbeddings)) {
                if (topicEmbedding) {
                  const similarity = cosineSimilarity(contextEmbedding, topicEmbedding);
                  const normalizedSimilarity = (similarity + 1) / 2;
                  
                  // 🚨 NEU: Schwellwert erhöht auf 0.75 (75%) für präzisere Themen-Erkennung
                  // Vorher: 0.6 war zu niedrig → unpassende Themen wurden erkannt
                  if (normalizedSimilarity > bestSimilarity && normalizedSimilarity > 0.75) {
                    bestSimilarity = normalizedSimilarity;
                    bestTopic = topic;
                  }
                }
              }
              ...
              if (bestTopic) {
                console.log(`📊 Semantische Themen-Erkennung: "${bestTopic}" (Ähnlichkeit: ${(bestSimilarity * 100).toFixed(1)}%)`);
                
                // Thematisch passende proaktive Ergänzungen
                const topicAdditions = {
                  'kaffee': "Ich trinke meinen Kaffee gerne mit Milch. ",
                  'essen': "Ich koche gerne italienisch. ",
                  'kino': "Ich gehe gerne ins Kino. ",
                  'musik': "Ich höre gerne Musik. ",
                  'sport': "Ich mache gerne Sport. ",
                  'buch': "Ich lese gerne Bücher. ",
                  'reisen': "Ich reise gerne. "
                };
                
                proactiveAddition = topicAdditions[bestTopic];
                topicDetected = true;
              }
            }
          } catch (err) {
            console.warn('⚠️ Fehler bei semantischer Themen-Erkennung, verwende Fallback:', err.message);
          }
          
          // Fallback: Keyword-basierte Erkennung (falls Embeddings fehlschlagen)
          if (!topicDetected) {
            if (fullContext.includes('kaffee') || fullContext.includes('kaffe') || fullContext.includes('coffee')) {
              proactiveAddition = "Ich trinke meinen Kaffee gerne mit Milch. ";
              topicDetected = true;
            } else if (fullContext.includes('essen') || fullContext.includes('kochen') || fullContext.includes('küche')) {
              proactiveAddition = "Ich koche gerne italienisch. ";
              topicDetected = true;
            } else if (fullContext.includes('kino') || fullContext.includes('film') || fullContext.includes('serie')) {
              proactiveAddition = "Ich gehe gerne ins Kino. ";
              topicDetected = true;
            } else if (fullContext.includes('musik') || fullContext.includes('hören') || fullContext.includes('song')) {
              proactiveAddition = "Ich höre gerne Musik. ";
              topicDetected = true;
            } else if (fullContext.includes('sport') || fullContext.includes('fitness') || fullContext.includes('training')) {
              proactiveAddition = "Ich mache gerne Sport. ";
              topicDetected = true;
            } else if (fullContext.includes('buch') || fullContext.includes('lesen') || fullContext.includes('lese')) {
              proactiveAddition = "Ich lese gerne Bücher. ";
              topicDetected = true;
            } else if (fullContext.includes('reisen') || fullContext.includes('urlaub') || fullContext.includes('reise')) {
              proactiveAddition = "Ich reise gerne. ";
              topicDetected = true;
            }
          }
          
          // Fallback: Nur wenn kein spezifisches Thema erkannt wurde
          if (!topicDetected) {
            // Generische proaktive Ergänzungen (nur wenn kein Thema erkannt)
            ...
            proactiveAddition = generalAdditions[Math.floor(Math.random() * generalAdditions.length)];
          }
        }
        ...
        console.log(`✅ Proaktive Ergänzung hinzugefügt (${topicDetected ? 'thematisch passend' : 'generisch'}): "${proactiveAddition.trim()}"`);
      }
      ...
      if (message.length < targetMinLength) {
        ...
        if (message.length < targetMinLength) {
          message += hasSexualContent ? " Was würdest du gerne machen?" : " Ich würde gerne mehr darüber erfahren.";
        }
...
async function runMultiAgentPipeline({
  ...
  // Schritt 6: Proactive (Mood-Analyst entfernt - verursachte mehr Probleme als Nutzen)
  const proactiveResult = await agentProactiveAnalyst(allMessages || [], customerMessage, proactiveAnalysisFunc);
