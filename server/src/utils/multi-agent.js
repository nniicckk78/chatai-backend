...
${isASA ? '\n🚨🚨🚨 KRITISCH: DIES IST EINE REAKTIVIERUNGSNACHRICHT (ASA)! 🚨🚨🚨\n- Der Kunde hat zuletzt nicht geantwortet.\n- Reaktiviere das Gespräch freundlich und interessiert.\n- Frage, was den Kunden beschäftigt.\n- Sei warmherzig, aber nicht aufdringlich.\n' : ''}`;
    ...
    if (message) {
      ...
      // 🚨 KRITISCH: ALLE Nachrichten müssen >=150 Zeichen sein (laut Memories)
      if (message.length < 150) {
        ...
        // Füge Erweiterungen hinzu, bis >=150 Zeichen
        while (message.length < 150 && extensions.length > 0) {
          const randomExtension = extensions[Math.floor(Math.random() * extensions.length)];
          message += randomExtension;
          // Entferne verwendete Erweiterung, um Wiederholungen zu vermeiden
          extensions = extensions.filter(e => e !== randomExtension);
        }
        
        // Falls immer noch zu kurz, füge generische Erweiterung hinzu
        if (message.length < 150) {
          if (hasSexualContent) {
            message += " Was würdest du denn gerne machen?";
          } else {
            message += " Erzähl mir doch mehr davon.";
          }
        }
        
        console.log(`✅ Nachricht erweitert auf ${message.length} Zeichen (Ziel: >=150)`);
      }
