            ...
            const scoredVariants = await Promise.all(
              processedVariants.map(async (variant) => {
                ...
                const [learningScore, qualityResultVariant] = await Promise.all([
                  ...
                  // Quality-Validierung mit optimiertem Timeout
                  Promise.race([
                    ...
                    }, 15000)) // 🚨 ERHÖHT: 15 Sekunden (Embedding-Calls brauchen Zeit, aber parallel = schneller)
                  ]).catch(err => {
...
async function validateMessageQuality(message, {
  ...
  try {
    // 🚨 OPTIMIERUNG: Timeout erhöht auf 8 Sekunden (validateSemanticQuality hat 6s Timeout, plus 2s Buffer)
    semanticScore = await Promise.race([
      validateSemanticQuality(message, customerMessage, conversationHistory, hasSexualContent),
      new Promise((resolve) => setTimeout(() => {
        console.warn('⚠️ Semantische Validierung: Timeout nach 8 Sekunden - verwende Fallback');
        resolve(25); // Fallback: Volle Punkte
      }, 8000)) // 8 Sekunden (6s + 2s Buffer für Stabilität)
    ]);
...
{"isValid": true/false, "score": 0-100, "issues": [], "reasoning": "max 100 Zeichen", "isTooDirect": true/false, "isAppropriatelySubtle": true/false}`;
    ...
    const response = await Promise.race([
      ...
      new Promise((_, reject) => setTimeout(() => reject(new Error('Semantic validation timeout')), 8000)) // 🚨 ERHÖHT: 8 Sekunden
    ]);
