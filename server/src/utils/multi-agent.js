      ...
      try {
        ...
        if (learningDataForScoring) {
          ...
          if (processedVariants.length > 0) {
            // 🚨 ROOT CAUSE FIX: Pre-Cache Training/Pattern-Embeddings (einmalig, nicht pro Variante)
            ...
            if (training?.selectedExamples && training.selectedExamples.length > 0 && !multiAgentResults?.shouldUseFallbackMode) {
              try {
                ...
                console.warn('⚠️ Fehler beim Cachen von Training-Embeddings:', err.message);
              }
            }
                ...
                const [learningScore, qualityResultVariant] = await Promise.all([
                  ...
                  Promise.race([
                    validateMessageQuality(variant, {
                      ...
                      allRules,
                      situation,
                      // 🚨 ROOT CAUSE FIX: Übergebe gecachte Embeddings + messageEmbedding
                    ...
                    }, 10000)) // 🚨 REDUZIERT: 10 Sekunden (weil Embeddings jetzt gecacht sind!)
                  ]).catch(err => {
...
async function validateMessageQuality(message, {
  ...
  allRules,
  situation,
  // 🚨 ROOT CAUSE FIX: Gecachte Embeddings übergeben (optional, für Performance)
