...
Antworte NUR als JSON, kein zusätzlicher Text.`;
    ...
    const hasLikelyGerman = hasGermanChars || germanWordPattern.test(msg) || (msg.length >= 10 && /\b(und|ist|nicht|mit|der|die|das|auf|nur|grad)\b/i.test(msg));
    ...
    // Nur bei sehr hoher Sicherheit als "nicht deutsch" werten (Schwelle 0.98 – weniger False-Positives bei deutschen Nachrichten)
    if (!isGerman && confidence > 0.98) {
