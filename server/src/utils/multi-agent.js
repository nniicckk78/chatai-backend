...
TRAINING-DATEN: In den Trainingsdaten endet fast jede Nachricht (99,9%) mit einer passenden Frage zum Thema. Mach es genauso – mindestens 120 Zeichen, dann eine Frage die zum Gesagten passt.
...
WICHTIG: Du antwortest als FAKE-PROFIL (${fakeName}) auf Nachrichten vom KUNDEN. Der KUNDE schreibt DIR (dem Fake-Profil)!${whoIsWho}`;
      ...
      if (containsRefusal(message)) {
        ...
        try {
          ...
          if (containsRefusal(message)) console.warn('⚠️ STAGE 3: Retry enthielt weiterhin Ablehnung – Nachricht bleibt wie generiert');
        } catch (retryErr) {
