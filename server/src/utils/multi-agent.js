...
1. REAGIERE AUF DEN KUSS:
   - Bedanke dich für den Kuss (z.B. "Hey danke für deinen Kuss", "Danke für deinen Kuss, echt lieb von dir")
   - Sei freundlich und natürlich, nicht übertrieben

2. STELLE EINE EINFACHE FRAGE:
   - Frage locker und natürlich (z.B. "Wie gehts dir denn so?", "Was machst du gerade?")
   - Stelle NUR 1 Frage - nicht mehr!
   - Die Frage soll den Kunden zum Antworten motivieren

3. SEI LOCKER UND NATÜRLICH:
...
4. VERBOTEN BEI ERSTER NACHRICHT:
   - ❌ KEINE persönlichen Infos auf einmal (Alter, Wohnort, Hobbies - das kommt später!)
   - ❌ KEINE sexuellen Themen (es ist die erste Nachricht!)
   - ❌ KEINE zu persönlichen Fragen (z.B. nach Beziehungsstatus, Kindern, etc.)
   - ❌ KEINE zu direkten Fragen
   - ❌ KEINE Meta-Kommentare über die Nachricht
   - ❌ KEINE zu langen Nachrichten (150-200 Zeichen ideal)

5. BEISPIEL-STRUKTUR:
   "Hey danke für deinen Kuss, echt lieb von dir. Wie gehts dir denn so und was machst du gerade?"

🚨🚨🚨 KRITISCH: Dies ist die ERSTE Nachricht - mache einen guten, natürlichen Eindruck! 🚨🚨🚨
`;
      } else if (hasLike) {
...
1. REAGIERE AUF DAS LIKE:
   - Zeige, dass du dich über das Like freust (z.B. "Hey, danke für das Like", "Das freut mich, dass dir mein Profil gefällt")
   - Sei freundlich und natürlich, nicht übertrieben
...
5. BEISPIEL-STRUKTUR:
   "Hey, danke für das Like. Wie gehts dir denn so und was machst du gerade?"
...
1. STELLE DICH VOR (LOCKER):
   - Sage Hallo (z.B. "Hey", "Hi", "Hallo")
   - Nenne deinen Namen (aus Fake-Profil)
   - ❌ NICHT alle Infos auf einmal (Alter, Wohnort, Hobbies - das kommt später!)
   - Optional: NUR Alter ODER Wohnort (nicht beides!)

2. ZEIGE INTERESSE:
   - Frage locker und natürlich (z.B. "Wie gehts dir denn so?", "Was machst du gerade?")
   - Stelle NUR 1-2 natürliche Fragen (nicht mehr!)
...
5. BEISPIEL-STRUKTUR:
   "Hey, ich bin [Name]. Wie gehts dir denn so? Was machst du gerade?"
...
${isASA ? '\n🚨🚨🚨 KRITISCH: DIES IST EINE REAKTIVIERUNGSNACHRICHT (ASA)! 🚨🚨🚨\n- Der Kunde hat zuletzt nicht geantwortet.\n- Reaktiviere das Gespräch freundlich und interessiert.\n- Frage, was den Kunden beschäftigt.\n- Sei warmherzig, aber nicht aufdringlich.\n' : ''}`;
    ...
    if (isASA) {
      ...
      if (asaConversationContext && asaConversationContext.trim() !== "") {
        ...
        if (hasKuss || hasLike) {
          ...
          if (hasKuss) {
            userPrompt += `- 🚨🚨🚨 KRITISCH: Der Kunde hat dir einen KUSS geschickt!\n`;
            ...
            userPrompt += `- 🚨🚨🚨 KRITISCH: Beispiele für gute Fragen: "Was machst du gerade so?", "Wie geht es dir?", "Was beschäftigt dich?", "Worauf hast du Lust?"\n`;
          ...
          userPrompt += `- 🚨🚨🚨 KRITISCH: Sei KREATIV - es gibt KEINE speziellen Training-Daten für System-Nachrichten!\n`;
          userPrompt += `- 🚨🚨🚨 KRITISCH: Reagiere natürlich, freundlich und interessiert auf die System-Nachricht!\n`;
    ...
    // Hole Statistiken für die aktuelle Situation (wenn message vorhanden)
    if (message) {
      try {
    ...
    if (message) {
      ...
      if (message.length > targetMaxLength) {
        ...
        // Entferne redundante Phrasen und neue Themen/Proaktivitäten
        ...
        // 🚨 NEU: Entferne Sätze mit neuen Themen/Proaktivitäten
        for (const phrase of newTopicPhrases) {
          shortened = shortened.replace(phrase, ' ').trim();
        }
        ...
        if (shortened.length > targetMaxLength) {
          // 🚨 WICHTIG: Erlaube einen sehr kleinen Toleranzbereich (bis 5 Zeichen mehr), wenn es bei einem Satzende ist
          const tolerance = 5; // 🚨 REDUZIERT: Nur 5 Zeichen mehr erlauben (statt 20)
    ...
    if (targetAvgQuestions >= 1) {
      ...
      if (questionCount === 0) {
        ...
        // Füge Frage hinzu (vor dem letzten Satz, falls vorhanden)
        if (questionToAdd) {
          const sentences = message.split(/(?<=[.!?])\s+/);
          if (sentences.length > 0) {
            // Entferne letztes Satzzeichen, falls vorhanden
            const lastSentence = sentences[sentences.length - 1].trim();
            const withoutPunctuation = lastSentence.replace(/[.!?]+$/, '');
            sentences[sentences.length - 1] = withoutPunctuation;
            
            // Füge Frage hinzu
            message = sentences.join(' ') + questionToAdd;
            console.log(`✅ Frage hinzugefügt: "${questionToAdd.trim()}"`);
          } else {
            // Wenn keine Sätze, füge einfach am Ende hinzu
            message = message.trim() + questionToAdd;
            console.log(`✅ Frage hinzugefügt: "${questionToAdd.trim()}"`);
          }
        }
      }
