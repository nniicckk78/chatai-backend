...
- reasoning: Kurze Begründung, warum diese Bewertung`;
    ...
    const response = await Promise.race([
      client.chat.completions.create({
        ...
        max_tokens: 150 // 🚨 ROOT CAUSE FIX: Reduziert von 200 auf 150 (schneller)
      }),
      new Promise((_, reject) => setTimeout(() => reject(new Error('ML-API Timeout')), 4000)) // 🚨 ROOT CAUSE FIX: Reduziert von 5s auf 4s
    ]);
