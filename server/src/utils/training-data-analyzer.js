...
function getAllowedQuestionCount(context, analysis) {
  ...
  if (context) {
    // 🚨 FIX: context kann ein Objekt (von analyzeContextForValidation) oder ein String sein
    let ctxStr = '';
    let topics = [];
    let isSexual = false;
    ...
    if (typeof context === 'string') {
      // Fallback: Wenn context ein String ist (alte API)
      ctxStr = context.toLowerCase();
    } else if (typeof context === 'object' && context !== null) {
      // Neue API: context ist ein Objekt mit properties
      topics = context.topics || [];
      isSexual = context.isSexual || false;
      hasHobbies = context.hasHobbies || false;
      hasPreferences = context.hasPreferences || false;
      isGeneral = context.isGeneralQuestion || false;
      // Erstelle String-Repräsentation für Kompatibilität
      ctxStr = topics.join(' ').toLowerCase();
    }
    
    // Wenn Kontext zeigt: Hobbies + Vorlieben, erlaube 2 Fragen
    if ((hasHobbies || ctxStr.includes('hobbies') || ctxStr.includes('hobby')) && 
        (hasPreferences || ctxStr.includes('vorlieb') || ctxStr.includes('preference'))) {
      maxQuestions = 2;
    }
    
    // Wenn Kontext zeigt: Sexuell + Allgemein, erlaube 2 Fragen
    if ((isSexual || ctxStr.includes('sexual')) && 
        (isGeneral || ctxStr.includes('general'))) {
      maxQuestions = 2;
    }
    
    // Wenn Kontext zeigt: Sexuell + Hobbies, erlaube 2 Fragen
    if ((isSexual || ctxStr.includes('sexual')) && 
        (hasHobbies || ctxStr.includes('hobbies') || ctxStr.includes('hobby'))) {
      maxQuestions = 2;
    }
  }
