...
async function filterWithModeration(examples) {
  ...
  for (let i = 0; i < examples.length; i++) {
    ...
    // ABER: Wir müssen harmlose Wörter wie "kuss", "küssen" nicht herausfiltern
    const sexualScore = categoryScores.sexual || 0;
    const sexualMinorsScore = categoryScores['sexual/minors'] || 0;
    
    // 🚨 WICHTIG: Harmlose Wörter wie "kuss", "küssen" haben oft niedrige Scores (0.1-0.3)
    // Wir filtern nur bei höheren Scores (> 0.3), um harmlose Inhalte nicht zu blockieren
    // Aber: sexual/minors wird IMMER gefiltert, auch bei niedrigsten Scores
    const SEXUAL_THRESHOLD = 0.3; // Threshold für sexuelle Inhalte (harmlose Wörter wie "kuss" haben ~0.1-0.2)
    const hasSexualByScore = sexualScore > SEXUAL_THRESHOLD; // Nur bei höheren Scores filtern
    const hasSexualMinorsByScore = sexualMinorsScore > 0.0; // Jeder Score für sexual/minors ist kritisch
    
    // 🚨 ZUSÄTZLICH: Prüfe auf harmlose Wörter, die fälschlicherweise als sexual eingestuft werden
    const textLower = combinedText.toLowerCase();
    const harmlessWords = ['kuss', 'küssen', 'küsse', 'küsst', 'umarmung', 'umarmen', 'liebe', 'lieben', 'verliebt'];
    const hasOnlyHarmlessWords = harmlessWords.some(word => textLower.includes(word)) && 
                                  sexualScore < 0.5; // Wenn nur harmlose Wörter + niedriger Score, dann nicht filtern
    
    // Wenn es nur harmlose Wörter sind, nicht filtern (auch wenn Score > 0.0)
    if (hasOnlyHarmlessWords && sexualScore < 0.5) {
      // Harmlose Wörter mit niedrigem Score -> nicht filtern
    ...
    const hasSexual = flaggedCategories.includes('sexual') || hasSexualByScore; // 🚨 NEU: Auch nach Score prüfen!
