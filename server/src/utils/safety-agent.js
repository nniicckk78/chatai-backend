...
/**
 * Szenarien mit Tieren in eindeutig sexuellem Kontext (ohne die Wörter Zoophilie/Bestialität).
 * z. B. Hund/Katze leckt an Intim-/Brustbereich, Sex mit Tier.
 */
function detectSexualAnimalExploitationScenario(text) {
  if (!text || typeof text !== 'string') return false;
  const lower = text.toLowerCase();

  const animalWord =
    /\b(hund|hunde|hündin|hündchen|welpe|welpen|katze|katzen|kater|kätzchen|pferd|pony|esel|schaf|ziege|kuh|rind|schwein|tier|haustier)\b/i.test(
      lower
    );
  if (!animalWord) return false;

  const intimateBody =
    /\b(nippel|nippeln|brust|brüste|titten|muschi|schwanz|vagina|penis|after|scheide|intim|po|popo|arsch)\b/i.test(lower);
  const oralOrSexAct = /\b(leck|leckt|lecken|schleck|schleckt|blas|blasen|lutschen|lutscht)\b/i.test(lower);

  if (intimateBody && oralOrSexAct) return true;

  // Reihenfolge egal: Tier … lecken … Nippel o. ä.
  if (
    /\b(hund|hunde|hündin|welpe|katze|kater|pferd|pony|tier)\b[\s\S]{0,220}\b(leck|leckt|lecken)\b[\s\S]{0,220}\b(nippel|nippeln|brust|brüste|titten|muschi|schwanz)\b/i.test(
      lower
    )
  ) {
    return true;
  ...
  if (
    /\b(leck|leckt|lecken)\b[\s\S]{0,220}\b(nippel|nippeln|brust|brüste|titten|muschi|schwanz)\b[\s\S]{0,220}\b(hund|hunde|hündin|katze|kater|pferd|pony|tier)\b/i.test(
      lower
    )
  ) {
    return true;
  ...
  // Explizit: Sex/Fick (mit) Tier
  if (/\bsex\s+mit\s+(meinem|deinem|einem|dem|ihrem)?\s*(hund|hunde|katze|katzen|tier|pferd|pony)\b/i.test(lower)) return true;
  if (/\b(fick|ficken|fickt)\s+(meinen|deinen|den|das)?\s*(hund|katze|tier|pferd|pony)\b/i.test(lower)) return true;
...
function checkMinorMention(text) {
  ...
  const agePatterns = [
    ...
    // Wichtig: "ich bin 17" soll auch OHNE "Jahre/alt" blockieren (kein isAgeContext-Gate).
    /\bich bin\s+(1[0-7])\b/i,
    /\bbin\s+(1[0-7])\b/i,
    // Explizite Varianten mit Alters-Wörtern
  ...
  for (const pattern of agePatterns) {
    if (pattern.test(lower)) {
      ...
      if (match) {
        ...
        // "ich bin 17" / "bin 17" ist per se eine Altersangabe (auch ohne "jahre/alt")
        const isDirectIAmAge = /^\s*(ich\s+bin|bin)\s+1[0-7]\b/i.test(match[0]);
        const isAgeContext = isDirectIAmAge || context.includes("alt") || context.includes("jahr") ||
  ...
  if (detectSexualAnimalExploitationScenario(text)) {
    return { isBlocked: true, reason: "zoophilia", errorMessage: "🚨 BLOCKIERT: Zoophilie-Themen erkannt!" };
  }
...
function checkKICheckMessage(text) {
  ...
  const kiCheckPatterns = [
    // FPC/Plattform: "Validierungscheck: Antworte mit 337075 in dieser Nachricht."
    /\bvalidierungscheck\b/i,
    /validierung\s*check/i,
    /--\s*validierungscheck/i,
    /antworte\s+mit\s+\d{4,8}\s+in\s+dieser\s+nachricht/i,
    /mit\s+\d{4,8}\s+in\s+dieser\s+nachricht/i,
    /nach\s+bestätigung.*(lädt|ladet).*chat/i,
    /bestätigung.*(lädt|ladet).*(chat|neu)/i,
  ...
  // Code + "in dieser Nachricht" + Validierungs-/Prüfungs-Kontext (oft am Ende langer FPC-Texte)
  if (/\d{5,8}/.test(text) && /in\s+dieser\s+nachricht/i.test(lower)) {
    if (/\b(validierung|bestätigung|sicherheit|prüfung|verifizierung)\b/i.test(lower) ||
        /\b(ki[-\s]?check|antworte\s+mit|validierungscheck)\b/i.test(lower)) {
      return true;
    }
  }
...
/**
 * Alle Texte vollständig sammeln, die auf Plattform-KI-/Validierungs-Checks geprüft werden sollen
 * (keine Kürzung — Ende der Bubble muss mit erfasst werden).
 * @param {object} [reqBody]
 * @param {string} [foundMessageText]
 * @returns {string[]}
 */
function collectKICheckCandidateTexts(reqBody, foundMessageText) {
  const out = [];
  const seen = new Set();
  function add(s) {
  ...
  add(foundMessageText);
  if (reqBody && typeof reqBody === 'object') {
    add(reqBody.messageText);
    add(reqBody.message);
    if (reqBody.userProfile && typeof reqBody.userProfile === 'object') {
      add(reqBody.userProfile.lastMessage);
      add(reqBody.userProfile.messageText);
    }
    const msgs = reqBody.siteInfos && Array.isArray(reqBody.siteInfos.messages) ? reqBody.siteInfos.messages : null;
    if (msgs) {
      const tail = msgs.length > 15 ? msgs.slice(-15) : msgs;
      for (const m of tail) {
        if (!m || typeof m !== 'object') continue;
        if (typeof m.text === 'string') add(m.text);
        if (typeof m.message === 'string') add(m.message);
        if (m.message && typeof m.message === 'object' && typeof m.message.text === 'string') add(m.message.text);
...
function runSafetyCheck(customerMessage) {
  ...
  if (checkKICheckMessage(customerMessage)) {
    return {
      isBlocked: true,
      reason: "ki_check",
      errorMessage: "🚨 BLOCKIERT: KI-/Validierungs-Check erkannt!\n\nDie Kundennachricht verlangt eine Zahlencode-Antwort (z. B. Validierungscheck). Bitte die Zahl(en) manuell eintippen und selbst absenden.\n\nEs wird KEINE automatische Antwort generiert."
    };
  }
...
module.exports = {
  checkMinorMention,
  detectSexualAnimalExploitationScenario,
  checkKICheckMessage,
  collectKICheckCandidateTexts,
