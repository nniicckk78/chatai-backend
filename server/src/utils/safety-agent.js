...
function checkForeignLanguage(text) {
  ...
  // 1. Andere Schriften: Polnisch/Tschechisch/Slawisch (ąęćłńóśźż...) oder Kyrillisch – nur blockieren wenn mehrere Wörter betroffen (nicht wegen eines einzelnen)
  const nonGermanScript = /[ąćęłńóśźżĄĆĘŁŃÓŚŹŻěščřžýáíéúůďťňĚŠČŘŽÝÁÍÉÚŮĎŤŇ]/;
  const cyrillic = /[\u0400-\u04FF]/;
  if (nonGermanScript.test(trimmed) || cyrillic.test(trimmed)) {
    ...
    if (wordsWithForeignScript.length >= 2) {
      return {
        isBlocked: true,
  ...
  // 2. Sehr häufige deutsche Wörter – wenn Nachricht mehrere Wörter hat, aber kaum deutsche
  ...
  if (words.length >= 4) {
    ...
    const germanCount = lowerWords.filter(w => w.length > 1 && germanCommon.has(w)).length;
