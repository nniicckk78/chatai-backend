...
async function calculateFeedbackScore(example, situation = 'allgemein') {
  ...
  // 🚨 NEU: Finde Beispiel-ID (Priorität: feedbackId > training_index > customerMessage|moderatorResponse)
  let exampleId = null;
  if (example.feedbackId) {
    exampleId = example.feedbackId;
  } else if (example.index !== null && example.index !== undefined) {
    exampleId = `training_${example.index}`;
  } else {
    exampleId = `${example.customerMessage || ''}|${example.moderatorResponse || ''}`;
  }
...
async function selectSmartExamples(customerMessage, options = {}) {
  ...
  // Bei mehreren Situationen: Feedback-Score für jede passende Situation (Anfrage + Beispiel) berechnen, besten nehmen – so werden Beispiele mit mehreren Tags (z.B. "Sexuelle Themen" + "Allgemein") fair bewertet.
  const requestedSituationList = Array.isArray(situation) && situation.length > 0
    ? situation.filter(s => s && String(s).trim())
  ...
  const scoredExamples = await Promise.all(
    similarExamples.map(async (example) => {
      ...
      const raw = example.situation || example.situations;
      if (raw) {
        if (Array.isArray(raw)) exampleSituations.push(...raw);
        else if (String(raw).includes(',')) exampleSituations.push(...String(raw).split(',').map(s => s.trim()).filter(Boolean));
        else exampleSituations.push(String(raw).trim());
      }
      const situationsToTry = [...new Set([...requestedSituationList, ...exampleSituations, 'allgemein'])];
      const scores = await Promise.all(situationsToTry.map(s => calculateFeedbackScore(example, s)));
      const feedbackScore = scores.length > 0 ? Math.max(...scores) : 0.5;
...
function formatExamplesForPrompt(examples) {
  ...
  let prompt = '\n\n📚 RELEVANTE BEISPIELE (antworte im GLEICHEN STIL wie diese Beispiele, aber variiere den Inhalt!):\n\n';
