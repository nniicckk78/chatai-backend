...
async function selectSmartExamples(customerMessage, options = {}) {
  ...
  const situationForFeedback = Array.isArray(situation) && situation.length > 0 ? situation[0] : (situation || 'allgemein');
  const scoredExamples = await Promise.all(
    similarExamples.map(async (example) => {
      const feedbackScore = await calculateFeedbackScore(example, situationForFeedback);
