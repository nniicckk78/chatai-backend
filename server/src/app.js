...
// Statische Dateien aus public-Ordner servieren (für Dashboard/Webseiten)
const publicDir = path.join(__dirname, "../public");
app.use(express.static(publicDir));

// Robuste Website-Route: akzeptiert beide bekannten Schreibweisen im URL-Pfad
app.get(["/omperiavision", "/omperiavision.html", "/omperiavsion", "/omperiavsion.html"], (_req, res) => {
  const candidates = ["omperiavision.html", "omperiavsion.html"];
  ...
  return res.status(404).send("Omperia Vision page not found in /public.");
});
...
async function start() {
  ...
  try {
    ...
    try {
      ...
      const { getGitHubClient, getFileFromGitHub, GITHUB_JSON_LONG_CACHE_TTL_MS } = require('./utils/github');
      const fs = require('fs');
      const path = require('path');
        ...
        loadTrainingDataForVectorDb().then(trainingData => {
          ...
          (async () => {
            try {
              ...
              console.log('⏳ Warte 10 Sekunden, damit PostgreSQL bereit ist (Vektor-DB-Init)...');
