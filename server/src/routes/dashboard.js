...
// Middleware für Fine-Tuning-Routen (extrahiert Token aus Header)
function fineTuningAuth(req, res, next) {
  const auth = req.headers.authorization;
  if (!auth || !auth.toLowerCase().startsWith("bearer ")) {
    return res.status(401).json({ error: "Kein Token" });
  }
...
router.get('/fine-tuning/status', fineTuningAuth, async (req, res) => {
  try {
...
router.post('/fine-tuning/retrain', fineTuningAuth, async (req, res) => {
  try {
    ...
    config.trainingExamplesCount = examples.length;
    await saveFineTuningConfig(config);
    
    console.log(`✅ Fine-Tuning gestartet: Job ${jobId} mit ${examples.length} Beispielen`);
    ...
    res.json({
      ...
      status: 'running',
      message: `Fine-Tuning gestartet mit ${examples.length} Beispielen. Job-ID: ${jobId}`
    });
  } catch (error) {
...
router.get('/fine-tuning/stats', fineTuningAuth, async (req, res) => {
  try {
    const config = await getFineTuningConfig();
    const examples = await collectPerfectExamples();
