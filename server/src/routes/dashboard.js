...
async function getTrainingData() {
  ...
  if (githubClient) {
    try {
      const { getFileFromGitHub, GITHUB_JSON_LONG_CACHE_TTL_MS } = require("../utils/github");
      const possiblePaths = [
...
async function getFeedbackData() {
  ...
  // GitHub ist die HAUPTQUELLE; getFileFromGitHub nutzt 24h In-Memory-Cache pro Prozess (entlastet API)
  const githubClient = getGitHubClient();
  if (githubClient) {
    repo = getRepoInfo();
    try {
