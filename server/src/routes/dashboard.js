    ...
    try {
      ...
      if (conversation.feedbackId) {
        ...
        if (feedbackIndex !== -1) {
          ...
          const oldSituation = feedback.situation; // Speichere alte Situation
          ...
          if (oldSituation !== finalSituation && (feedback.status === 'good' || feedback.status === 'edited')) {
            try {
      ...
      if (!feedbackUpdated && conversation.customerMessage) {
        ...
        for (const feedback of matchingFeedbacks) {
          const oldSituation = feedback.situation;
          ...
          feedback.updatedAt = new Date().toISOString();
          feedbackUpdated = true;
          console.log(`✅ Situation im Feedback synchronisiert (via Kundennachricht-Matching: ${feedback.id}, Situation: ${oldSituation || 'keine'} → ${finalSituation})`);
          
          // 🚨 NEU: Wenn Situation geändert wurde, analysiere Learning-Stats neu
          if (oldSituation !== finalSituation && (feedback.status === 'good' || feedback.status === 'edited')) {
            try {
              const { analyzeFeedback } = require('../utils/learning-system');
              console.log(`🔄 Analysiere Learning-Stats neu mit korrigierter Situation (${oldSituation || 'keine'} → ${finalSituation})...`);
