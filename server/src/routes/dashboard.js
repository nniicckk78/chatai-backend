...
router.post('/test-chat', async (req, res) => {
  try {
    ...
    // Treffen/Termine - WICHTIG: Auch Uhrzeiten erkennen!
    ...
    if (rules && rules.situationalResponses && typeof rules.situationalResponses === 'object') {
      for (const [situationName, situationResponse] of Object.entries(rules.situationalResponses)) {
        ...
        if (!matchesSituation) {
          ...
          // Treffen/Termine - WICHTIG: Explizit betonen!
          if ((situationLower.includes("treffen") || situationLower.includes("termin")) &&
              (isMeetingRequest || lowerMessage.includes("uhr") || lowerMessage.includes("20 uhr") || 
               lowerMessage.includes("20:00") || lowerMessage.includes("19 uhr") ||
               lowerMessage.includes("18 uhr") || lowerMessage.includes("21 uhr"))) {
            matchesSituation = true;
          }
    ...
    // Baue Context mit verbotenen und bevorzugten Wörtern (verstärkt wie in reply.js)
    ...
    if (rules) {
      ...
      if (rules.preferredWords && Array.isArray(rules.preferredWords) && rules.preferredWords.length > 0) {
        preferredWordsContext = `\n\n✅ BEVORZUGTE WÖRTER (VERWENDE DIESE WÖRTER REGELMÄSSIG und NATÜRLICH in deinen Antworten, wo es passt!):\n${rules.preferredWords.map(w => `- ${w}`).join('\n')}\n\nWICHTIG: Integriere diese Wörter natürlich in deine Antworten, wo sie thematisch passen! Verwende sie regelmäßig, aber nicht gezwungen!`;
        console.log(`⭐ ${rules.preferredWords.length} bevorzugte Wörter geladen`);
      }
    }

    // Füge allgemeine Regeln hinzu (verstärkt wie in reply.js)
    if (rules && rules.generalRules && rules.generalRules.trim()) {
      specificInstructions += `\n\n📝 ALLGEMEINE REGELN (STRENG BEACHTEN - diese Regeln haben höchste Priorität!):\n${rules.generalRules}`;
      console.log("📝 Allgemeine Regeln geladen");
    }
    ...
    if (trainingData && trainingData.conversations && Array.isArray(trainingData.conversations) && trainingData.conversations.length > 0) {
      ...
      // 3. Falls keine passenden gefunden, nimm die letzten 5 Beispiele als allgemeine Referenz
      ...
      if (relevantExamples.length > 0) {
        ...
        trainingExamplesContext += `\nWICHTIG: Orientiere dich STRENG an diesen Beispielen! Passe deinen Schreibstil, Ton, Länge und Formulierungen EXAKT an die Beispiel-Antworten an!`;
        console.log(`📚 ${relevantExamples.length} Training-Beispiele geladen`);
      }
    }
