...
2. TRAINING-DATEN (siehe unten) - ABSOLUT HÖCHSTE PRIORITÄT für Schreibstil, Wortwahl und Ton!
...
5. SPEZIFISCHE ANTWORTEN (siehe unten) - für bestimmte Situationen
...
– 🚨🚨🚨 KRITISCH: Wenn der Kunde etwas KONKRETES sagt (z.B. "bei deiner muschi", "deine muschi lecken"), gehe DARAUF ein – ignoriere es NICHT und frage NICHT allgemein! 🚨🚨🚨
– 🚨🚨🚨 Bei Sex-Talk: ESKALIERE mit dem Kunden – zeige Progression, nicht Stagnation! Wenn der Kunde eskaliert, eskaliere auch! 🚨🚨🚨
...
📋 BEACHTE: Grundregeln (Absolute Basis) + Zusätzliche Regeln (aus Dashboard) + Spezifische Antworten (für Situationen) + Training-Daten (für Stil) = Perfekte Antwort!`;
    ...
    if (foundForbiddenWords.length > 0 || foundMetaComments.length > 0 || foundFormalPhrases.length > 0 || hasRepetitiveQuestion || foundRepetitions.length > 0) {
      ...
      try {
        ...
        if (hasRepetitiveQuestion) {
          retryReason += `WIEDERHOLUNGSFRAGE (Echo-Loop) ERKANNT! Der Kunde hat bereits eine klare Antwort gegeben (z.B. "lecken"), aber du fragst trotzdem erneut "wo würdest du anfangen?" oder "was würdest du machen?" - ABSOLUT VERBOTEN! Gehe auf das ein, was der Kunde GESCHRIEBEN hat, frage NICHT erneut! Füge etwas NEUES hinzu oder führe das Gespräch in eine NEUE Richtung! `;
        }
        if (foundRepetitions.length > 0) {
