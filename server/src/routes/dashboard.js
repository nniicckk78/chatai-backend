...
async function getFeedbackData() {
  ...
  if (githubClient) {
    try {
      ...
      // 🚨 ROOT CAUSE FIX: Für feedback.json IMMER ref='HEAD' nutzen (umgeht GitHub Cache komplett)
      // Das Problem: getFileFromGitHub() nutzt einen Cache mit 1h TTL, aber nach Push ist die Datei sofort neu
      // Lösung: Nutze API direkt mit ref='HEAD' für feedback.json (umgeht Cache)
      let content = null;
      let usedPath = null;
      ...
      // 🚨 ROOT CAUSE FIX: Für feedback.json IMMER ref='HEAD' nutzen (umgeht Cache)
      if (client) {
        for (const filePath of possiblePaths) {
          try {
            console.log(`🔍 Lade Feedback-Daten von GitHub (Cache-Bypass): ${filePath}`);
            // Nutze API direkt mit ref='HEAD' (umgeht Cache komplett)
            const { data } = await client.repos.getContent({
              owner: repo.owner,
              repo: repo.repo,
              path: filePath,
              ref: 'HEAD' // HEAD = neueste Version, umgeht Cache komplett
            });
            if (data.encoding === 'base64' && data.content) {
              content = Buffer.from(data.content, 'base64').toString('utf8');
              usedPath = filePath;
              console.log(`✅ Feedback-Daten von GitHub geladen (Cache-Bypass): ${filePath}`);
              break;
            }
          } catch (err) {
            if (err.status === 404) {
...
async function saveFeedbackData(data) {
  ...
  if (githubClient) {
    try {
      ...
      for (const filePath of possiblePaths) {
        try {
          // 🚨 ROOT CAUSE FIX: Parse content, um Feedback-Count zu loggen
          let feedbackCount = 0;
          let asaCount = 0;
          try {
            const parsed = JSON.parse(content);
          ...
          console.log(`🚀 Pushe Feedback-Daten auf GitHub: ${filePath} (${feedbackCount} Feedbacks, ${asaCount} ASA)`);
          await pushFileToGitHub(filePath, content, 'Update feedback data via Dashboard');
          pushed = true;
          console.log(`✅ Feedback-Daten erfolgreich auf GitHub gepusht: ${filePath} (${feedbackCount} Feedbacks, ${asaCount} ASA)`);
          ...
          console.log(`✅ GitHub-Cache invalidiert für: ${filePath}`);
          break;
        } catch (err) {
          if (err.status === 404) {
