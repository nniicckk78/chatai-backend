...
async function getFeedbackData() {
  ...
  if (githubClient) {
    ...
    console.warn('⚠️ GitHub Client nicht verfügbar - lade nur lokale Feedback-Daten');
  }

  // Fallback: Lade von lokaler Datei (nur wenn GitHub fehlschlägt)
  ...
  if (localResult && localResult.data && Array.isArray(localResult.data.feedbacks) && localResult.data.feedbacks.length > 0) {
    console.log(`⚠️ Verwende lokale Feedback-Daten als Fallback (${localResult.data.feedbacks.length} Feedbacks) - GitHub war nicht verfügbar`);
    return localResult.data;
  }

  console.warn('⚠️ Keine Feedback-Daten gefunden (weder GitHub noch lokal) - gebe leeres Array zurück');
  return { feedbacks: [] };
}
