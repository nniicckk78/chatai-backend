...
router.get('/fine-tuning/status', verifyToken, async (req, res) => {
  try {
...
router.post('/fine-tuning/retrain', verifyToken, async (req, res) => {
  try {
...
router.get('/fine-tuning/stats', verifyToken, async (req, res) => {
  try {
    const config = await getFineTuningConfig();
    const examples = await collectPerfectExamples();
