...
router.post('/training-data', async (req, res) => {
  try {
    ...
    if (Array.isArray(situation) && situation.length > 0) {
      // Mehrere Situationen: Speichere als String mit Komma-Trennung (für Kompatibilität)
      // Oder als Array (wenn das System es unterstützt)
    ...
    data.conversations.push({
      ...
      situations: Array.isArray(situation) ? situation : (situation ? [situation] : ['allgemein']), // 🚨 NEU: Array für bessere Verarbeitung
...
// PUT /api/v1/training-data/:index - Aktualisiere Situation(en) eines Gesprächs
