...
router.put('/training-data/:index', async (req, res) => {
  try {
    ...
    try {
      ...
      if (conversation.feedbackId) {
        ...
        if (feedbackIndex !== -1) {
          ...
          // 🚨 NEU: Wenn Situation geändert wurde, analysiere Learning-Stats neu
          // Prüfe, ob Situation wirklich geändert wurde (auch wenn oldSituation 'keine' war)
          const situationChanged = oldSituation !== finalSituation && oldSituation !== finalSituation.split(',')[0];
          if (situationChanged && (feedback.status === 'good' || feedback.status === 'edited')) {
            try {
              ...
              console.log(`✅ Learning-Stats mit korrigierter Situation aktualisiert (${finalSituation})`);
            } catch (err) {
    ...
    // Wenn Situation geändert wurde, analysiere Training-Daten direkt für Learning-Stats
    if (situation !== undefined && conversation.customerMessage && conversation.moderatorResponse) {
      try {
        const { analyzeFeedback } = require('../utils/learning-system');
        const oldSituationInConv = conversation.situation || 'allgemein';
        const situationChanged = oldSituationInConv !== finalSituation;
        
        if (situationChanged) {
          console.log(`🔄 Analysiere Training-Daten direkt für Learning-Stats (Situation: ${oldSituationInConv} → ${finalSituation})...`);
          ...
          await analyzeFeedback({
            ...
            situation: finalSituation, // Verwende korrigierte Situation
