...
router.post('/fine-tuning/retrain', async (req, res) => {
  try {
    ...
    // 🚨 DEBUG: Prüfe ob filterWithModeration verfügbar ist
    if (!filterWithModeration || typeof filterWithModeration !== 'function') {
      console.error('❌ FEHLER: filterWithModeration ist nicht verfügbar!');
      const fineTuningUtils = require("../utils/fine-tuning");
      console.error('Verfügbare Funktionen:', Object.keys(fineTuningUtils));
      throw new Error('filterWithModeration is not a function. Verfügbare Funktionen: ' + Object.keys(fineTuningUtils).join(', '));
    }
    
    const { safe: safeExamples, flagged: flaggedExamples, warnings: warningExamples } = await filterWithModeration(examples);
