...
async function getFeedbackData() {
  ...
  const loadLocalFeedback = () => {
    try {
      ...
      console.error('Fehler beim Laden der lokalen Feedback-Daten:', err);
    }
    return null;
  };
  ...
  if (githubClient) {
    try {
      ...
      if (data && data.content) {
        const content = Buffer.from(data.content, 'base64').toString('utf8');
        const parsed = JSON.parse(content);
        const hasFeedbacks = Array.isArray(parsed.feedbacks) && parsed.feedbacks.length > 0;
        ...
        console.warn('⚠️ Feedback-Daten von GitHub sind leer, nutze lokale Datei als Fallback');
      }
    } catch (err) {
      if (err.status !== 404) {
