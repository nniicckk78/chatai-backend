...
router.post("/", asyncHandler(async (req, res, next) => {
  ...
  try {
    ...
    // AVZ/FPC: Auch URLs ohne Dateiendung akzeptieren (viele CDNs), gleiche Nachricht wie für Text nutzen
    if (imageUrls.length === 0 && req.body?.siteInfos?.messages) {
      ...
      if (receivedWithImages.length > 0) {
        ...
        const imageUrl = msgWithImage.image || msgWithImage.imageUrl || msgWithImage.imageSrc || msgWithImage.src ||
                        ...
                        (msgWithImage.url && (msgWithImage.url.match(/https?:\/\/.*\.(png|jpg|jpeg|gif|webp)/i) || msgWithImage.url.startsWith('http')) ? msgWithImage.url : null) ||
        ...
        if (imageUrl && isAcceptableImageUrl(imageUrl)) {
          imageUrls = [imageUrl];
          console.log("✅ Bild-URL aus siteInfos.messages extrahiert:", imageUrl.startsWith('data:') ? 'data:...' : imageUrl.substring(0, 100));
        }
      }
    }
    
    // 3. Prüfe metaData (lastImageUrl etc.; auch data: und http ohne Endung für AVZ/FPC)
    ...
    // 4. Prüfe auch direkt in req.body nach Bild-Feldern
