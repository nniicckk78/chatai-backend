...
router.post("/", asyncHandler(async (req, res, next) => {
  ...
  const thankYouTriggerType = getThankYouTriggerType(newestMsgByTime, foundMessageText);
  ...
  // Blenny/DF: Wenn ASA nur wegen "Gruß gesendet"-Systemnachricht erkannt wurde (keine echte received-Nachricht), Like/Kuss-Pfad nutzen (Danke + Fragen), nicht ASA.
  if (!thankYouTriggerType && asaSystemMessage && typeof asaSystemMessage === 'string') {
    ...
    if (asaLower.includes('hat dir einen gruß gesendet') || asaLower.includes('hat dir ein gruß gesendet') || (asaLower.includes('gruß') && asaLower.includes('gesendet'))) {
      ...
      console.log('✅ Blenny/DF: Gruß-Systemnachricht als ASA erkannt – nutze Like/Kuss-Pfad (Danke + gesprächseröffnende Fragen).');
    }
  }
