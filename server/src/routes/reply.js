...
async function generateFirstMessage({
  ...
  try {
    ...
    const fakeAge = extractedInfo?.assistant?.Age ?? profileInfo?.moderatorInfo?.birthDate?.age ?? null;
...
WICHTIG:
- Dein Alter (FAKE-PROFIL) ist ${fakeAge != null ? fakeAge + ' Jahre. Verwende NUR dieses Alter fuer dich – das Alter des Kunden ist NICHT dein Alter!' : 'nicht im Kontext – gib bei der ersten Nachricht KEIN Alter an (kommt spaeter).'}
    ...
    if (isASACalculated && asaFromKussOrLike && isFPC) {
      try {
        ...
        const firstMessageInstructions = isKuss
          ? "Der Kunde hat dir einen KUSS geschickt (Kuss-Systemnachricht). Bedanke dich mit Danke fuer den Kuss (NICHT fuer den Like!). Stelle 1-2 gespraechsoeffnende Fragen. Keine Vorstellung (kein Name, kein Alter, kein Wohnort)."
          : isLike
            ? "Der Kunde hat dich GELIKED (Like-Systemnachricht). Bedanke dich mit Danke fuer den Like (NICHT fuer den Kuss!). Stelle 1-2 gespraechsoeffnende Fragen. Keine Vorstellung (kein Name, kein Alter, kein Wohnort)."
            : "Der Kunde hat dir einen Kuss oder Like geschickt (System-Nachricht). Bedanke dich kurz dafuer und stelle 1-2 gespraechsoeffnende Fragen. Keine Vorstellung (kein Name, kein Alter, kein Wohnort).";
