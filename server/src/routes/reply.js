...
const { getRules, getTrainingData } = require("../config/config-loader");
const fs = require("fs");
...
- WICHTIG: Übernehme den Schreibstil der letzten Moderator-Nachrichten (siehe userPrompt)! Passe Länge, Emojis, Ton und Formulierungen an, damit der Kunde nicht merkt, dass verschiedene Moderatoren schreiben!`;
    ...
    // Füge allgemeine Regeln aus Config hinzu (vor der späteren rules-Deklaration)
    const generalRulesConfig = getRules();
    if (generalRulesConfig.generalRules && Array.isArray(generalRulesConfig.generalRules) && generalRulesConfig.generalRules.length > 0) {
      const generalRulesText = generalRulesConfig.generalRules.map(rule => `- WICHTIG: ${rule}`).join('\n');
...
- Beispiel-Nachrichten: ${writingStyle.sampleTexts ? writingStyle.sampleTexts.substring(0, 300) : ""}` : "";
    ...
    // WICHTIG: Nutze Config-Dateien für Regeln
    let rules = {};
    try {
      rules = getRules() || {};
    } catch (err) {
      console.error("❌ Fehler beim Laden der Regeln:", err.message);
      rules = {}; // Fallback auf leeres Objekt
    }
    
    // Stelle sicher, dass rules ein Objekt ist
    if (!rules || typeof rules !== "object") {
      console.warn("⚠️ Regeln sind kein Objekt, verwende leeres Objekt");
      rules = {};
    }
    
    let specificInstructions = "";
    
    // Prüfe auf verbotene Wörter/Phrasen
    if (rules.forbiddenWords && Array.isArray(rules.forbiddenWords) && rules.forbiddenWords.length > 0) {
      const hasForbiddenWord = rules.forbiddenWords.some(word => 
        lowerMessage.includes(word.toLowerCase())
      );
      if (hasForbiddenWord) {
        console.warn("⚠️ Verbotenes Wort erkannt in Nachricht");
        // Optional: Blockieren oder Warnung
      }
    }
    
    if (rules.forbiddenPhrases && Array.isArray(rules.forbiddenPhrases) && rules.forbiddenPhrases.length > 0) {
      const hasForbiddenPhrase = rules.forbiddenPhrases.some(phrase => 
        lowerMessage.includes(phrase.toLowerCase())
      );
      if (hasForbiddenPhrase) {
        console.warn("⚠️ Verbotene Phrase erkannt in Nachricht");
      }
    }
    
    // ============================================
    // PRIORITÄTS-REIHENFOLGE FÜR REGELN:
    // 1. Allgemeine Regeln (immer aktiv)
    // 2. Situational Responses (dynamisch, basierend auf Keywords)
    // 3. Bevorzugte Wörter (nur bei relevanten Situationen)
    // 4. Spezielle Legacy-Situationen (nur wenn nicht bereits durch dynamische Regeln abgedeckt)
    // ============================================
    
    // Track welche Situationen bereits angewendet wurden (verhindert Duplikate)
    const appliedSituations = new Set();
    
    // PHASE 1: Dynamische Situational Responses aus Config (PRIORITÄT 1)
    if (rules.situationalResponses && typeof rules.situationalResponses === "object" && !Array.isArray(rules.situationalResponses)) {
      try {
        Object.entries(rules.situationalResponses).forEach(([situationKey, situationRule]) => {
          if (!situationRule || typeof situationRule !== "object" || !situationRule.enabled) return;
          
          // Prüfe Keywords (falls vorhanden)
        let matches = false;
        if (situationRule.keywords && Array.isArray(situationRule.keywords) && situationRule.keywords.length > 0) {
          ...
          // Fallback: Prüfe auf bekannte Situationen (für Rückwärtskompatibilität)
          if (situationKey === "botAccusation" && isBotAccusation) matches = true;
          if (situationKey === "sexualTopics" && isSexualTopic) matches = true;
          if (situationKey === "meetingRequest" && isMeetingRequest) matches = true;
          if (situationKey === "moneyTopics") {
            const moneyKeywords = ["coin", "coins", "geld", "aufladen", "kredit", "bezahlen", "kostenlos", "kostenfrei", "gratis"];
            matches = moneyKeywords.some(k => lowerMessage.includes(k));
          }
        }
        ...
        if (matches && situationRule.instructions) {
          ...
          appliedSituations.add(situationKey);
          
          // PHASE 2: Bevorzugte Wörter für sexuelle Themen (nur wenn sexualTopics aktiv)
          if (situationKey === "sexualTopics" && rules.preferredWords) {
            if (rules.preferredWords.sexual && Array.isArray(rules.preferredWords.sexual) && rules.preferredWords.sexual.length > 0) {
              specificInstructions += `\n\n🔥 BEVORZUGTE WÖRTER (sexuell) - Diese Wörter SOLLST du verwenden, wenn passend:\n${rules.preferredWords.sexual.join(", ")}\n\nNutze diese Wörter natürlich und passend im Kontext, besonders bei expliziten sexuellen Gesprächen. Sei nicht zurückhaltend - diese Wörter sind ausdrücklich erlaubt und sollen verwendet werden!`;
            }
            
            if (rules.preferredWords.flirty && Array.isArray(rules.preferredWords.flirty) && rules.preferredWords.flirty.length > 0) {
              specificInstructions += `\n\n😘 BEVORZUGTE WÖRTER (flirty) - Kannst du auch verwenden:\n${rules.preferredWords.flirty.join(", ")}`;
            }
            
            if (rules.preferredWords.general && Array.isArray(rules.preferredWords.general) && rules.preferredWords.general.length > 0) {
              specificInstructions += `\n\n💬 BEVORZUGTE WÖRTER (allgemein):\n${rules.preferredWords.general.join(", ")}`;
            }
          }
        }
        });
      } catch (err) {
        console.error("❌ Fehler beim Verarbeiten der Situational Responses:", err.message);
        // Weiter mit Legacy-Logik
      }
    }
    
    // PHASE 3: Legacy-Spezialfälle (nur wenn nicht bereits durch dynamische Regeln abgedeckt)
    // Bot-Vorwurf (nur wenn noch nicht angewendet)
    try {
      if (isBotAccusation && !appliedSituations.has("botAccusation") && rules.situationalResponses?.botAccusation?.enabled && rules.situationalResponses.botAccusation?.instructions) {
        const botRule = rules.situationalResponses.botAccusation;
        specificInstructions += `\n\n🚨 BESONDERE SITUATION: Der Kunde wirft dir vor, ein Bot/KI/Fake zu sein!\n${botRule.instructions}`;
        appliedSituations.add("botAccusation");
      }
      
      // Sexuelle Themen (nur wenn noch nicht angewendet)
      if (isSexualTopic && !appliedSituations.has("sexualTopics") && rules.situationalResponses?.sexualTopics?.enabled && rules.situationalResponses.sexualTopics?.instructions) {
        const sexualRule = rules.situationalResponses.sexualTopics;
        specificInstructions += `\n\n💋 SEXUELLE THEMEN:\n${sexualRule.instructions}`;
        appliedSituations.add("sexualTopics");
        
        // Bevorzugte Wörter für sexuelle Themen
        if (rules.preferredWords && typeof rules.preferredWords === "object") {
          if (rules.preferredWords.sexual && Array.isArray(rules.preferredWords.sexual) && rules.preferredWords.sexual.length > 0) {
            specificInstructions += `\n\n🔥 BEVORZUGTE WÖRTER (sexuell) - Diese Wörter SOLLST du verwenden, wenn passend:\n${rules.preferredWords.sexual.join(", ")}\n\nNutze diese Wörter natürlich und passend im Kontext, besonders bei expliziten sexuellen Gesprächen. Sei nicht zurückhaltend - diese Wörter sind ausdrücklich erlaubt und sollen verwendet werden!`;
          }
      ...
      console.error("❌ Fehler beim Verarbeiten der Legacy-Spezialfälle:", err.message);
      // Weiter ohne diese Regeln
    }
    
    // PHASE 4: Allgemeine bevorzugte Wörter (nur bei flirty/heiß, wenn noch nicht hinzugefügt)
    try {
      if (rules.preferredWords && typeof rules.preferredWords === "object" && !appliedSituations.has("sexualTopics") && (lowerMessage.includes("flirt") || lowerMessage.includes("heiß") || lowerMessage.includes("sexy"))) {
        if (rules.preferredWords.general && Array.isArray(rules.preferredWords.general) && rules.preferredWords.general.length > 0) {
          specificInstructions += `\n\n💬 BEVORZUGTE WÖRTER (allgemein):\n${rules.preferredWords.general.join(", ")}`;
        }
      }
    } catch (err) {
      console.error("❌ Fehler beim Verarbeiten der bevorzugten Wörter:", err.message);
      // Weiter ohne bevorzugte Wörter
    }
...
- Keine Flugzeugbegleiter, Zugkontrolleur oder ähnliche Berufe (zu spezifisch, könnte zu Treffen führen)`;
    ...
    if (isMeetingRequest && rules.situationalResponses?.meetingRequest?.enabled && !specificInstructions.includes("TREFFEN")) {
      const meetingRule = rules.situationalResponses.meetingRequest;
      specificInstructions += `\n\n🚫 TREFFEN/TERMINE:\n${meetingRule.instructions}`;
    }
...
Antworte NUR mit der vollständigen Nachricht inklusive Frage am Ende, keine Erklärungen.`;
  ...
  // Speichere Nachricht für Statistiken
  try {
    ...
    const platformId = req.body?.platformId || req.body?.siteInfos?.origin || 'unknown';
    const messageEntry = {
      id: Date.now().toString(),
      timestamp: new Date().toISOString(),
      platform: platformId,
      customerMessage: foundMessageText?.substring(0, 200) || '',
      aiResponse: replyText?.substring(0, 500) || '',
      chatId: responseChatId,
      isASA: isLastMessageFromFake || false
    };
    
    messages.push(messageEntry);
    
    // Behalte nur die letzten 10000 Nachrichten
    ...
    console.error('Fehler beim Speichern der Nachricht:', logErr.message);
  }
