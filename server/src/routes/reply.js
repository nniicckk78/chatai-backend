...
- Beispiel-Nachrichten: ${writingStyle.sampleTexts ? writingStyle.sampleTexts.substring(0, 300) : ""}` : "";
    ...
    // Nutze Situations-Regeln aus Config - DYNAMISCH für ALLE Situationen
    ...
    // Legacy: Spezielle Situationen (für Rückwärtskompatibilität)
    if (isBotAccusation && rules.situationalResponses?.botAccusation?.enabled && !specificInstructions.includes("BOT")) {
    ...
    if (isSexualTopic && rules.situationalResponses?.sexualTopics?.enabled && !specificInstructions.includes("SEXUELLE")) {
      const sexualRule = rules.situationalResponses.sexualTopics;
      let sexualInstructions = `\n\n💋 SEXUELLE THEMEN:\n${sexualRule.instructions}`;
      ...
      if (rules.preferredWords?.sexual && Array.isArray(rules.preferredWords.sexual) && rules.preferredWords.sexual.length > 0) {
        sexualInstructions += `\n\n🔥 WICHTIG - Bevorzugte Wörter für sexuelle Gespräche (diese Wörter SOLLST du verwenden, wenn passend):\n${rules.preferredWords.sexual.join(", ")}\n\nNutze diese Wörter natürlich und passend im Kontext, besonders bei expliziten sexuellen Gesprächen. Sei nicht zurückhaltend - diese Wörter sind ausdrücklich erlaubt und sollen verwendet werden!`;
      }
      
      if (rules.preferredWords?.flirty && Array.isArray(rules.preferredWords.flirty) && rules.preferredWords.flirty.length > 0) {
        sexualInstructions += `\n\n😘 Flirty Wörter (kannst du auch verwenden):\n${rules.preferredWords.flirty.join(", ")}`;
      }
      
      if (rules.preferredWords?.general && Array.isArray(rules.preferredWords.general) && rules.preferredWords.general.length > 0) {
        sexualInstructions += `\n\n💬 Allgemeine bevorzugte Wörter:\n${rules.preferredWords.general.join(", ")}`;
      }
      
      specificInstructions += sexualInstructions;
    }
    
    // Bevorzugte Wörter auch bei anderen Situationen verfügbar machen (wenn relevant)
    if (rules.preferredWords && (isSexualTopic || lowerMessage.includes("flirt") || lowerMessage.includes("heiß"))) {
      if (rules.preferredWords.general && Array.isArray(rules.preferredWords.general) && rules.preferredWords.general.length > 0 && !specificInstructions.includes("bevorzugte Wörter")) {
        specificInstructions += `\n\n💬 Bevorzugte Wörter (kannst du verwenden):\n${rules.preferredWords.general.join(", ")}`;
      }
    }
