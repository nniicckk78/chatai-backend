...
// Helper: akzeptiere nur "stabile" chatIds (keine Datumsstrings/Spaces, die sich ständig ändern)
function isStableChatId(val) {
  if (!val) return false;
  const s = String(val).trim();
  if (!s) return false;
  // Wenn Leerzeichen enthalten sind, ist es sehr wahrscheinlich ein dynamischer "timestamp - snippet" String.
  if (/\s/.test(s)) return false;
  // Numeric dialogue/chat ids (typisch)
...
function deriveStableChatId({ chatId, pageUrl, platformId, siteInfos }) {
  const candidates = [
    chatId,
    siteInfos?.chatId,
    siteInfos?.metaData?.customerInfo?.username,
    siteInfos?.metaData?.moderatorInfo?.username
  ].filter(Boolean);

  // Best case: stabile ID direkt vorhanden
  for (const c of candidates) {
    if (isStableChatId(c)) return String(c).trim();
  }

  // Nächster Versuch: aus URL extrahieren (z.B. dialogue/chatId Parameter)
  if (pageUrl) {
    try {
      const u = new URL(pageUrl);
      const q =
        u.searchParams.get("dialogue") ||
        u.searchParams.get("chatId") ||
        u.searchParams.get("id");
      if (isStableChatId(q)) return String(q).trim();
      const m = String(pageUrl).match(/[Dd]ialogue[#\s]*(\d+)/);
      if (m && isStableChatId(m[1])) return m[1];
    } catch (e) {
      // ignore
    }
  }

  // Letzter Fallback: deterministische kurze ID aus stabilen Kontextfeldern (keine Leerzeichen)
  const seed = [
    platformId || siteInfos?.origin || "unknown",
    String(pageUrl || "").slice(0, 200),
    String(siteInfos?.metaData?.customerInfo?.username || ""),
    String(siteInfos?.metaData?.moderatorInfo?.username || "")
  ].join("|");
  const hex = crypto.createHash("sha1").update(seed).digest("hex").slice(0, 12);
  return `c_${hex}`;
}
...
router.post("/", asyncHandler(async (req, res, next) => {
  ...
  // #endregion
  // Schutz: Manche Clients/Extensions feuern POSTs ohne Body (oder mit leerem JSON).
  // Diese Requests dürfen NICHT den aktuellen Eingabetext in der Extension leeren/überschreiben.
  if (!req.body || typeof req.body !== "object" || Array.isArray(req.body) || Object.keys(req.body).length === 0) {
    console.warn("⚠️ Leerer Request-Body erhalten - ignoriere Request (noop), um UI nicht zu beeinflussen");
    return res.status(200).json({
      chatId: "00000000",
      actions: [],
      flags: { blocked: false, noop: true, reason: "empty_body", noReload: true, skipReload: true },
      noReload: true,
  ...
  if (!pageUrl && req.body?.siteInfos?.url) {
    pageUrl = req.body.siteInfos.url;
  }
  ...
  // Fallback auf metaData.chatId NUR wenn es stabil ist (iluvo liefert hier oft "Datum - Snippet" -> führt zu Reloads)
  if (!foundChatId && req.body?.siteInfos?.metaData?.chatId) {
    const candidate = req.body.siteInfos.metaData.chatId;
    if (isStableChatId(candidate)) {
      foundChatId = candidate;
      console.log("✅ chatId aus siteInfos.metaData.chatId (FALLBACK):", foundChatId);
    } else {
      console.warn("⚠️ metaData.chatId ist nicht stabil (enthält z.B. Leerzeichen/Datum) – ignoriert, um Reloads zu vermeiden");
    }
  }
  
  // WICHTIG: KEIN ungeprüfter "username-lastMessage" chatId-Fallback, weil iluvo hier oft
  // dynamische Datumsstrings/Message-Snippets liefert -> Reload-Schleifen. Nur stabile IDs akzeptieren.
  if (!foundChatId && typeof chatId === 'string' && isStableChatId(chatId)) {
    foundChatId = chatId;
    console.log(`✅ Stabiler chatId (String) gefunden:`, foundChatId);
  }
  ...
  // FINAL FALLBACK: deterministische, stabile chatId ableiten (pro Chat konstant, keine Reloads)
  if (!finalChatId || !isStableChatId(finalChatId)) {
    const derived = deriveStableChatId({
      chatId,
      pageUrl,
      platformId,
    ...
    if (!finalChatId) {
      console.warn("⚠️ Kein chatId gefunden - leite stabilen Wert ab:", derived);
    } else if (!isStableChatId(finalChatId)) {
      console.warn("⚠️ chatId ist nicht stabil - ersetze durch abgeleiteten stabilen Wert:", derived);
    }
    finalChatId = derived;
  }
  ...
  if (isKICheckMessage(foundMessageText)) {
    ...
    const safeChatId = deriveStableChatId({ chatId, pageUrl, platformId, siteInfos: req.body?.siteInfos });
    return res.status(200).json({
      error: errorMessage,
  ...
  if (!foundMessageText || foundMessageText.trim() === "") {
    ...
    // WICHTIG: Verwende eine stabile, deterministische chatId, damit die Extension nicht neu laedt
    const safeChatId = deriveStableChatId({ chatId, pageUrl, platformId, siteInfos: req.body?.siteInfos });
    return res.status(200).json({
      summary: {},
      ...
      flags: { 
        ...
        noop: true, // WICHTIG: Extension soll nichts in die UI schreiben/überschreiben
  ...
  if (!client) {
    ...
    return res.status(200).json({
      ...
      // blocked=false um UI/Reload-Schleifen zu vermeiden, aber trotzdem als Fehler markieren
      flags: { blocked: false, reason: "no_client", isError: true, showError: true, noReload: true, skipReload: true }
    });
  }
  ...
  try {
    console.log("🔍 DEBUG: Starte getTrainingData()...");
    trainingData = await getTrainingData();
    console.log("🔍 DEBUG: getTrainingData() abgeschlossen");
    console.log(`✅ Training Data geladen: ${trainingData?.conversations?.length || 0} Gespräche, ${trainingData?.asaExamples?.length || 0} generische ASAs`);
  } catch (err) {
    console.error('⚠️ Fehler beim Laden der Training Data:', err.message);
    console.error('⚠️ Stack:', err.stack?.substring(0, 200));
  }
  
  console.log(`🔍 DEBUG: isLastMessageFromFake = ${isLastMessageFromFake}, foundMessageText = "${foundMessageText?.substring(0, 50) || 'leer'}..."`);
  console.log(`🔍 DEBUG: Nach Debug-Log, fahre fort...`);

  // (Kein HOTFIX-Return hier) – normale Antworten muessen durch Regeln + Training-Daten laufen.
  ...
  console.log(`🔍 DEBUG: Prüfe ASA-Fall, isLastMessageFromFake = ${isLastMessageFromFake}`);
  console.log(`🔍 DEBUG: Vor if-Block, typeof isLastMessageFromFake = ${typeof isLastMessageFromFake}, Wert = ${isLastMessageFromFake}`);
  console.log(`🔍 DEBUG: Direkt vor if-Statement, isLastMessageFromFake = ${isLastMessageFromFake}`);
  if (isLastMessageFromFake) {
      console.log("🔍 DEBUG: ASA-Fall erkannt, betreten ASA-Block...");
      ...
      // 🚨 WICHTIG: Definiere auch filteredGenericASAs außerhalb, damit sie im Training-Context verwendet werden können
      let filteredGenericASAs = [];
          ...
          if (filteredASAExamples.length > 0 && !asaMessage) {
            ...
            if (!asaMessage || asaMessage.trim() === "") {
              ...
              if (customerMsg && asaMessage.toLowerCase().includes(customerMsg.toLowerCase().substring(0, 50))) {
                ...
                console.log(
                  `📚 ASA-Beispiel aus Training-Daten gefunden (${filteredASAExamples.length} von ${asaTrainingExamples.length} verfügbar nach Filterung):`,
                  asaMessage.substring(0, 100) + "..."
                );
              }
            }
          }
      ...
      // 🚨 FALLBACK 1: Verwende generische ASAs aus asaExamples (wenn keine situationsspezifischen gefunden)
      if (!asaMessage && trainingData && trainingData.asaExamples && Array.isArray(trainingData.asaExamples) && trainingData.asaExamples.length > 0) {
        console.log(`📚 Verwende generische ASAs aus asaExamples (${trainingData.asaExamples.length} verfügbar)`);
        
        // Filtere nach customerType (neukunde vs. langzeitkunde)
        filteredGenericASAs = trainingData.asaExamples.filter(example => {
          const exampleType = (example.customerType || "").toLowerCase();
          const targetType = isLongTermCustomer ? "langzeitkunde" : "neukunde";
          
          // Wenn kein customerType angegeben, verwende für alle
          if (!example.customerType || example.customerType === "") {
            return true;
          }
          
          // Wenn customerType passt, verwende es
          return exampleType === targetType || exampleType === "neukunde"; // Neukunde-Templates auch für Langzeitkunde verwenden, falls keine spezifischen vorhanden
        });
        
        // Falls keine nach customerType gefilterten gefunden, verwende alle
        if (filteredGenericASAs.length === 0) {
          console.log("⚠️ Keine ASAs für customerType gefunden, verwende alle generischen ASAs");
          filteredGenericASAs = trainingData.asaExamples;
        }
        
        // Filtere nach Nummern/Treffen (wenn Kunde diese nicht will)
        if (filteredGenericASAs.length > 0) {
          filteredGenericASAs = filteredGenericASAs.filter(example => {
            const message = (example.asaMessage || "").toLowerCase();
            
            // 🚨 KRITISCH: Wenn der Kunde KEINE Nummer will, filtere ALLE Nummern-Beispiele raus!
            if (!customerWantsNumber && (message.includes("nummer") || message.includes("telefon") || message.includes("handy"))) {
              console.log(`🚫 Filtere generisches ASA-Beispiel raus (Kunde will keine Nummer): ${message.substring(0, 80)}...`);
              return false;
            }
            
            // 🚨 KRITISCH: Wenn der Kunde KEIN Treffen will, filtere ALLE Treffen-Beispiele raus!
            if (!customerWantsMeeting && (message.includes("treffen") || message.includes("sehen") || message.includes("kennenlernen"))) {
              console.log(`🚫 Filtere generisches ASA-Beispiel raus (Kunde will kein Treffen): ${message.substring(0, 80)}...`);
              return false;
            }
          ...
          if (filteredGenericASAs.length === 0) {
            console.warn("⚠️ Alle generischen ASAs wurden gefiltert, verwende nur solche ohne Nummern/Treffen...");
            filteredGenericASAs = trainingData.asaExamples.filter(example => {
              const message = (example.asaMessage || "").toLowerCase();
              return !message.includes("nummer") && 
                     !message.includes("telefon") && 
                     !message.includes("handy") &&
                     !message.includes("treffen") && 
                     !message.includes("sehen") && 
                     !message.includes("kennenlernen");
            });
          }
          ...
          if (filteredGenericASAs.length > 0) {
            ...
            asaMessage = randomGenericASA.asaMessage || null;
            
            if (asaMessage && asaMessage.trim() !== "") {
              console.log(`✅ Generisches ASA-Beispiel aus asaExamples gefunden (${filteredGenericASAs.length} von ${trainingData.asaExamples.length} verfügbar):`, asaMessage.substring(0, 100) + "...");
            } else {
              console.warn("⚠️ Generisches ASA-Beispiel hat keine asaMessage, überspringe...");
              asaMessage = null;
            }
          }
        }
      ...
      // 🚨 FALLBACK 2: Verschiedene ASA-Nachrichten je nach Kunden-Typ (nur wenn auch keine generischen ASAs gefunden)
      if (!asaMessage) {
...
- KEINE generischen Fragen wie "Was denkst du?" - verwende Fragen aus den Beispielen!
- Übernehme den TON und die FORMULIERUNGEN aus den ASA-Beispielen!\n\n`;
          } else if (filteredGenericASAs && filteredGenericASAs.length > 0) {
            // Fallback: Verwende generische ASAs aus asaExamples
            console.log("📚 Verwende generische ASAs aus asaExamples für Training-Context");
            asaTrainingContext = `\n\n📚📚📚 GENERISCHE ASA-BEISPIELE AUS TRAINING-DATEN (ORIENTIERE DICH DARAN!): 📚📚📚\n`;
            filteredGenericASAs.slice(0, 5).forEach((example, idx) => {
              asaTrainingContext += `\nASA-Beispiel ${idx + 1}:\n`;
              asaTrainingContext += `Moderator/Fake Antwort: "${example.asaMessage || ''}"\n`;
            });
            asaTrainingContext += `\n\n⚠️⚠️⚠️ WICHTIG: ⚠️⚠️⚠️
- Orientiere dich am SCHREIBSTIL und der WORTWAHL aus diesen generischen ASA-Beispielen!
- Verwende ähnliche Formulierungen und Fragen wie in den Beispielen!
- KEINE generischen Fragen wie "Was denkst du?" - verwende Fragen aus den Beispielen!
- Übernehme den TON und die FORMULIERUNGEN aus den ASA-Beispielen!\n\n`;
          }
...
- ${(filteredASAExamples && filteredASAExamples.length > 0) || asaTrainingExamples.length > 0 || (filteredGenericASAs && filteredGenericASAs.length > 0) ? "Orientiere dich am SCHREIBSTIL und der WORTWAHL aus den ASA-Beispielen oben! Verwende ähnliche Formulierungen und Fragen!" : ""}
...
Antworte NUR mit der vollständigen, erweiterten Reaktivierungsnachricht (mindestens 150 Zeichen), keine Erklärungen. Die Nachricht soll fragen, warum der Kunde nicht mehr schreibt!`;
  ...
  // KRITISCH: Nur stabile chatId verwenden, sonst lädt die Extension neu
  const asaChatId = deriveStableChatId({ chatId, pageUrl, platformId, siteInfos: req.body?.siteInfos });
  ...
  // Ab hier: normaler Reply-Pfad (kein ASA)
  console.log("🔄 Starte normale Antwort-Generierung (kein ASA-Fall)...");
  
  // Normale Antwort-Generierung (wenn KEIN ASA-Fall)
  // WICHTIG: Gesamter normaler Reply-Pfad in try-catch, damit IMMER eine Response gesendet wird
  try {
    console.log("🔍 DEBUG: Nach if-Block, isLastMessageFromFake war false, fahre fort...");
    console.log("🔍 DEBUG: Nach if-Block geschlossen, isLastMessageFromFake war false");
    console.log("🔍 DEBUG: Nach if-Block, isLastMessageFromFake war false, starte normale Antwort-Generierung...");
    console.log("🔍 DEBUG: ASA-Block übersprungen, starte normale Antwort-Generierung...");
    console.log("🔄 Starte normale Antwort-Generierung (kein ASA-Fall)...");
    
    // 1. Informationen extrahieren (nur wenn Nachricht vom Kunden vorhanden)
    console.log("📝 Starte Extraktion von Informationen aus Nachricht...");
...
📝 ERINNERUNG: Die GRUNDREGELN (im System-Prompt) sind die BASIS für alle Antworten. Training-Daten sind deine HAUPTREFERENZ für Stil, Wortwahl und Ton.`;
    ...
    try {
      ...
      // Fallback: Gib IMMER eine nutzbare Antwort zurück (sonst wirkt es wie "keine Nachricht generiert").
      console.error("❌ OpenAI Fehler", err.message);
      const fallbackChatId = deriveStableChatId({ chatId, pageUrl, platformId, siteInfos: req.body?.siteInfos });
      const minWait = 40;
      const maxWait = 60;
      const waitTime = Math.floor(Math.random() * (maxWait - minWait + 1)) + minWait;
      const fallbackReply = isMeetingRequest
        ? "Ich verstehe dich, aber lass uns bitte noch kurz hier weiter schreiben. Was genau suchst du denn gerade und was ist dir wichtig?"
        : "Hey du. Erzaehl mir mal ein bisschen mehr, was hast du gerade so vor und worauf hast du heute Lust?";

      return res.status(200).json({
        resText: fallbackReply,
        replyText: fallbackReply,
        summary: extractedInfo,
        chatId: fallbackChatId,
        actions: [{ type: "insert_and_send", delay: waitTime }],
        flags: { blocked: false, noReload: true, skipReload: true, reason: "openai_fallback" },
        disableAutoSend: true,
        waitTime,
        noReload: true
      });
    ...
    if (!replyText || replyText.trim() === "") {
      console.error("❌ Antwort ist leer - verwende Fallback-Antwort, damit UI nicht hängen bleibt");
      const fallbackChatId = deriveStableChatId({ chatId, pageUrl, platformId, siteInfos: req.body?.siteInfos });
      const minWait = 40;
      const maxWait = 60;
      const waitTime = Math.floor(Math.random() * (maxWait - minWait + 1)) + minWait;
      const fallbackReply = isMeetingRequest
        ? "Hier im Chat passt mir besser. Was genau suchst du denn, eher was Lockeres oder was Ernstes?"
        : "Hey. Was machst du gerade so? Erzaehl mal ein bisschen.";
      return res.status(200).json({
        resText: fallbackReply,
        replyText: fallbackReply,
        summary: extractedInfo,
        chatId: fallbackChatId,
        actions: [{ type: "insert_and_send", delay: waitTime }],
        flags: { blocked: false, noReload: true, skipReload: true, reason: "empty_reply_fallback" },
        disableAutoSend: true,
        waitTime,
        noReload: true
      });
...
Antworte NUR mit der vollständigen Nachricht inklusive Frage am Ende, keine Erklärungen.`;
      ...
      try {
        ...
        const withQuestion = await withTimeout(
          client.chat.completions.create({
            model: "gpt-4o-mini",
            messages: [
    ...
    // WICHTIG: Die Extension braucht in der Praxis eine chatId, sonst werden actions (insert_and_send) oft ignoriert.
    // Daher IMMER eine stabile chatId zurückgeben:
    // PRIORITÄT: chatId aus Request > siteInfos.chatId > finalChatId > Default
    const responseChatId = deriveStableChatId({ chatId, pageUrl, platformId, siteInfos: req.body?.siteInfos });
    ...
    // KRITISCH: Fehler im normalen Reply-Pfad - sende IMMER eine Response, damit Extension nicht hängt
    console.error("❌ FEHLER im normalen Reply-Pfad:", err.message);
    console.error("❌ Stack:", err.stack?.substring(0, 500));
    
    const errorChatId = deriveStableChatId({ chatId, pageUrl, platformId, siteInfos: req.body?.siteInfos });
    const minWait = 40;
    const maxWait = 60;
    const waitTime = Math.floor(Math.random() * (maxWait - minWait + 1)) + minWait;
    const errorReply = "Hey, tut mir leid, ich hatte gerade einen kleinen Fehler. Kannst du mir nochmal schreiben, was du meintest?";
    
    return res.status(200).json({
      resText: errorReply,
      replyText: errorReply,
      summary: extractedInfo || { user: {}, assistant: {} },
      chatId: errorChatId,
      actions: [{ type: "insert_and_send", delay: waitTime }],
      flags: { blocked: false, noReload: true, skipReload: true, reason: "normal_reply_error", isError: true },
      disableAutoSend: true,
      waitTime,
      noReload: true
    });
