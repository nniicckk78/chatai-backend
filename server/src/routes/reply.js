...
router.post("/", asyncHandler(async (req, res, next) => {
  ...
  let hasKussSystemMessage = newestMsgByTime && isKussSystemMessage(newestMsgByTime);
  ...
  let hasLikeSystemMessage = newestMsgByTime && isLikeSystemMessage(newestMsgByTime);
  // Like-Benachrichtigung (z. B. "geliked") hat Vorrang, falls beides fälschlich true wäre
  if (hasLikeSystemMessage && hasKussSystemMessage && newestMsgByTime && typeof newestMsgByTime.text === "string") {
    const nt = newestMsgByTime.text.toLowerCase();
  ...
  // Cherry-Hard-Guard (final):
  // Wenn es eine echte received-Nachricht NACH der letzten sent/Fake gibt,
  // darf NIE ASA gesendet werden – unabhängig von späteren/globalen ASA-Indikatoren.
  ...
  if (isCherryRuntime && hasRecentCustomerTurnAfterLastSent) {
    if (isLastMessageFromFake) {
      console.log("⚠️ Cherry final guard: ASA auf false erzwungen (aktuelle Kundennachricht nach letzter Fake vorhanden).");
    }
    isLastMessageFromFake = false;
  }
  ...
  // Cherry-Notbremse (wirklich final):
  // Wenn ein echter Kundentext vorliegt, darf Cherry NICHT in ASA laufen.
  // So verhindern wir Fehl-ASA durch generische Reaktivierungs-/Heuristikpfade.
  const cherryTextMsg = { text: foundTrimmed, type: "received", messageType: "received" };
  const cherryHasRealCustomerText = foundTrimmed.length > 0 &&
    foundTrimmed !== "Der Kunde hat ein Bild geschickt." &&
    !isInfoMessage(cherryTextMsg, { isBlenny: false });
  if (isCherryRuntime && (hasRecentCustomerTurnAfterLastSent || cherryHasRealCustomerText)) {
    if (isLastMessageFromFake) {
      console.log("⚠️ Cherry final override: ASA deaktiviert, weil aktuelle Kundennachricht erkannt wurde.");
    }
    isLastMessageFromFake = false;
  }
