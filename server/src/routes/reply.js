...
router.post("/", asyncHandler(async (req, res, next) => {
  ...
  const newestMsg = newestMsgByTime || (msgsAll.length > 0 ? msgsAll[msgsAll.length - 1] : null);
  const isGrußTheNewestMessage = newestMsg && isGrußMessage(newestMsg.text);
    ...
    try {
      if (triggerGrußResponse) {
        ...
        if (generatedGrußMessage && generatedGrußMessage.trim() !== "") {
          ...
          return res.status(200).json({
            resText: generatedGrußMessage,
            replyText: generatedGrußMessage,
            summary: extractedInfo,
            chatId: chatId || finalChatId || "00000000",
