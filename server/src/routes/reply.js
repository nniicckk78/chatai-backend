...
router.post("/", asyncHandler(async (req, res, next) => {
  ...
  // 🤖 SCHRITT 11: INTELLIGENTE GENERIERUNG - GPT-4o-mini + Smart Example Selection
  const isASACalculated = isLastMessageFromFake || false;
  
  // Lade System-Prompt (ohne Fine-Tuning)
  let baseSystemPrompt = null;
  try {
    baseSystemPrompt = await buildSystemPrompt(getRules);
    console.log(`✅ System-Prompt geladen`);
  } catch (err) {
    console.error('⚠️ Fehler beim Laden des System-Prompts:', err.message);
  }
  
  // Verwende Standard-Model (kein Fine-Tuning)
