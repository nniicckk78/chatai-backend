...
router.post("/", asyncHandler(async (req, res, next) => {
  ...
  try {
    ...
    if (newestSystemTriggerMsg && tsRaw) {
      ...
      // Gold/S69/Mods: fehlende Timestamps + ASA-Kontext (Fake zuletzt, kein Kundenturn im Turn-Filter) →
      // alte „Kuss Nachricht!“-Zeilen sonst als „frisch“ behandelt (Sortierung 0 === 0).
  ...
  function getThankYouTriggerType(newestMsg, foundMsg) {
    ...
    // ASA-Kontext: letzte relevante Zeile = Fake, danach kein Kundenturn → kein Danke auf Kuss/Like/Gruß aus dem Verlauf
    // (häufig Gold/S69: alte „Kuss Nachricht!“-Bubble + fehlende Timestamps).
    if (suppressCustomerMessageFallbackForAsa) {
      if (newestMsg && isKussSystemMessage(newestMsg)) return null;
      if (newestMsg && isLikeSystemMessage(newestMsg)) return null;
      if (newestMsg && isGefaelltSystemMessage(newestMsg)) return null;
      if (newestMsg && isFavoritenSystemMessage(newestMsg)) return null;
