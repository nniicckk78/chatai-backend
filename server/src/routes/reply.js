...
router.post("/", asyncHandler(async (req, res, next) => {
  ...
  const lastMessage = friendRequestMessages.length > 0 ? friendRequestMessages[friendRequestMessages.length - 1] : null;
  const isFriendRequest = lastMessage && isInfoMessage(lastMessage) && (
    (lastMessage.text || "").toLowerCase().includes("freund") && 
    ((lastMessage.text || "").toLowerCase().includes("hinzufügen") || 
     (lastMessage.text || "").toLowerCase().includes("freundschaftsanfrage"))
  );
