...
router.post("/", asyncHandler(async (req, res, next) => {
  ...
  // FPC: Extra-Sicherung – wenn die Extraktion bereits einen Wohnort/Stadt fuer den Kunden erkannt hat,
  // stoppe Auto-Send auch ohne explizite Trigger-Worte (z.B. "Berlin." / "aus Berlin" / kurze Aussagen).
  try {
    if (isFPC && !isASACalculated && !fpcLogbookEntryDoNotSend && !manualReviewNoAutoSend && !fpcFriendRequestDoNotSend) {
      const city = (extractedInfo?.user?.Wohnort || extractedInfo?.user?.Stadt || "").toString().trim();
      if (city) {
        fpcLogbookEntryDoNotSend = true;
        fpcLogbookSide = "customer";
        fpcLogbookConfidence = 1;
        fpcLogbookReasons = ["Wohnort vom Kunden genannt (Logbuch/Notizen aktualisieren)"];
        console.warn("🛑 FPC Logbuch: Wohnort aus Extraktion erkannt (" + city + ") – Auto-Send gestoppt (manueller Eintrag).");
      }
    }
  } catch (_) {}

  if (isFPC && !isASACalculated && !fpcLogbookEntryDoNotSend && safeMessage && safeMessage.trim() && !manualReviewNoAutoSend && !fpcFriendRequestDoNotSend) {
    try {
      ...
      if (enable) {
        ...
        if (logbookClient && model) {
          const system = [
            ...
            "",
            "ALARM (Beispiele):",
            "- Wohnort/Ort/PLZ/Adresse des Kunden, wenn er ueber SICH sagt wo er wohnt/lebt/woher er kommt (auch wenn er nur den Ort nennt). NICHT alarmieren bei rein generischem 'zu Hause/daheim' ohne Ort.",
