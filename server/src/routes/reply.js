...
Antworte NUR mit gültigem JSON, kein zusätzlicher Text. Format:
{
  ...
  "Work": "Beruf/Arbeit falls erwähnt, sonst null",
  "Beruf": "wie Work",
  "Sport and Hobbies": "Hobbies/Sport falls erwähnt, sonst null",
  ...
  "Other": "Sonstige Infos über die Person (Termine wie 'morgen Zahnarzt', 'heute was mit Freundin', Vorlieben, Wohnsituation, etc.), sonst null"
}
...
// options.origin === 'iluvo': Zeitstempel ignorieren (Iluvo zeigt neueste oben, Zeitstempel oft falsch).
function hasRecentReceivedImageInMessages(messages, options) {
  if (!Array.isArray(messages) || messages.length === 0) return false;
  const now = Date.now();
  const maxAge = 10 * 60 * 1000; // 10 Minuten
  const ignoreTimestamp = (options?.origin || "").toLowerCase() === "iluvo";
  return messages.some(m => {
    if (m?.type !== "received" && m?.messageType !== "received") return false;
    if (!ignoreTimestamp && m.timestamp) {
      try {
...
[FAKE-PROFIL-INFO] (NUR diese Daten sind DEINE – Kunden-Daten gehoeren dem Kunden!)
Name: ${fakeName}
...
router.post("/", asyncHandler(async (req, res, next) => {
  ...
  if ((!foundMessageText || foundMessageText.trim() === "") && req.body?.siteInfos?.messages) {
    ...
    // AVZ: Alle System-/Info-Nachrichten ignorieren – nur echte Kunden-/Fake-Nachrichten verwenden
    const pageUrlForAVZ = pageUrlFromBody || req.body?.pageUrl || '';
    if ((req.body?.siteInfos?.origin && (String(req.body.siteInfos.origin).toLowerCase().includes('avz') || String(req.body.siteInfos.origin).toLowerCase().includes('chathomebase'))) ||
        ...
        (pageUrlForAVZ && (String(pageUrlForAVZ).toLowerCase().includes('chathomebase') || String(pageUrlForAVZ).toLowerCase().includes('avz')))) {
      ...
      if (lastAny) {
        ...
        if (newestReceived) {
          ...
          if (receivedAge <= maxAgeForMessage) {
            ...
            // Iluvo: Extension sendet Nachrichten in REVERSE-DOM-Reihenfolge → Array = [älteste … neueste].
            // Zeitstempel sind oft falsch, daher NICHT newestFirst vertrauen. Immer letzte received-Nachricht im Roh-Array = neueste.
            const origin = (req.body?.siteInfos?.origin || "").toLowerCase();
            if (origin === "iluvo" && msgs.length > 0) {
              const receivedList = msgs.filter(m => (m?.type === "received" || m?.messageType === "received") && !isInfoMessage(m));
              const lastReceivedByPosition = receivedList.length > 0 ? receivedList[receivedList.length - 1] : null;
              if (lastReceivedByPosition) {
                const hasImage = !!(lastReceivedByPosition.image || lastReceivedByPosition.imageUrl || lastReceivedByPosition.imageSrc || lastReceivedByPosition.image_url ||
                  (lastReceivedByPosition.url && /\.(jpg|jpeg|png|gif|webp)/i.test(String(lastReceivedByPosition.url))) ||
                  lastReceivedByPosition.attachment || lastReceivedByPosition.attachments || lastReceivedByPosition.media || lastReceivedByPosition.mediaUrl);
                const txt = (lastReceivedByPosition.text || "").trim();
                ...
                const noRealCustomerText = !txt.length || onlySystemImage(txt.toLowerCase());
                if (hasImage && noRealCustomerText) {
                  foundMessageText = "Der Kunde hat ein Bild geschickt.";
                  console.log("✅ Iluvo: Letzte Nachricht (neueste) nach Position ist nur Bild – antworte auf Bild.");
                } else if (txt.length > 0) {
                  foundMessageText = txt;
                  console.log("✅ Iluvo: Letzte Nachricht (neueste) nach Position verwendet (Extension sendet [älteste…neueste]):", foundMessageText.substring(0, 80) + "...");
                }
              }
            }
            if (!foundMessageText || foundMessageText.trim() === "") {
  ...
  // 🚨 Kunde hat Bild geschickt (ohne Kuss/Like): Kein ASA – antworte auf Bild (flirty/positiv)
  const originForImage = (req.body?.siteInfos?.origin || "").toLowerCase();
  if (isLastMessageFromFake && !asaFromKussOrLike && msgsAll.length && hasRecentReceivedImageInMessages(msgsAll, { origin: originForImage })) {
    isLastMessageFromFake = false;
    foundMessageText = "Der Kunde hat ein Bild geschickt.";
    console.log("✅ Kunde hat Bild geschickt – keine Kuss/Like-Systemnachricht, antworte auf Bild (kein ASA).");
  }
  ...
  try {
    ...
    // Grok-Pipeline: normale Reply (ASA wie FPC über Multi-Stage/Simplified/Multi-Agent)
    if (USE_GROK_PIPELINE && !isASACalculated) {
