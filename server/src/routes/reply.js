...
- Beispiel-Nachrichten: ${writingStyle.sampleTexts ? writingStyle.sampleTexts.substring(0, 300) : ""}` : "";
    ...
    if (rules && rules.situationalResponses && typeof rules.situationalResponses === 'object') {
      for (const [situationName, situationResponse] of Object.entries(rules.situationalResponses)) {
        // Prüfe ob die Situation in der Nachricht erwähnt wird (einfache Keyword-Suche)
        const situationLower = situationName.toLowerCase();
        const situationKeywords = situationLower.split(/[\s\-_]+/);
        const matchesSituation = situationKeywords.some(keyword => 
          keyword.length > 2 && lowerMessage.includes(keyword)
        ) || lowerMessage.includes(situationLower);
    ...
    // Hardcodierte Standard-Situationen (werden nur verwendet, wenn keine benutzerdefinierte Situation passt)
    if (isBotAccusation && !specificInstructions.includes("Bot-Vorwurf") && !specificInstructions.includes("bot")) {
...
- Zeige, dass dich diese Unterstellung verletzt`;
    ...
    if (isSexualTopic && !specificInstructions.includes("Sexuelle") && !specificInstructions.includes("sexuell")) {
      specificInstructions += `\n\n💋 SEXUELLE THEMEN:
...
- Keine Flugzeugbegleiter, Zugkontrolleur oder ähnliche Berufe (zu spezifisch, könnte zu Treffen führen)`;
    ...
    if (isMeetingRequest && !specificInstructions.includes("Treffen") && !specificInstructions.includes("treffen")) {
      specificInstructions += `\n\n🚫 TREFFEN/TERMINE:
...
- Passe deinen Schreibstil entsprechend an (Frau zu Mann oder Mann zu Frau)`;
    ...
    if (touchesMoney && !specificInstructions.includes("Geld") && !specificInstructions.includes("geld")) {
      specificInstructions += `\n\n💸 GELD/COINS:
