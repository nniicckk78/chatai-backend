...
Antworte NUR mit der vollständigen, erweiterten Reaktivierungsnachricht (mindestens 150 Zeichen), keine Erklärungen. Die Nachricht soll fragen, warum der Kunde nicht mehr schreibt!`;
  ...
  const asaChatId = chatId || req.body?.siteInfos?.chatId || (isStableChatId(finalChatId) ? finalChatId : "00000000");
...
📝 ERINNERUNG: Die GRUNDREGELN (im System-Prompt) sind die BASIS für alle Antworten. Training-Daten sind deine HAUPTREFERENZ für Stil, Wortwahl und Ton.`;
    ...
    try {
      ...
      const fallbackChatId = chatId || req.body?.siteInfos?.chatId || (isStableChatId(finalChatId) ? finalChatId : "00000000");
      const minWait = 40;
      const maxWait = 60;
      const waitTime = Math.floor(Math.random() * (maxWait - minWait + 1)) + minWait;
      const fallbackReply = isMeetingRequest
        ? "Ich versteh dich, aber lass uns bitte noch kurz hier weiter schreiben 🙂 Was genau suchst du denn gerade und was ist dir wichtig?"
        : "Hey du 🙂 Erzähl mir mal bisschen mehr – was hast du gerade so vor und worauf hast du heute Lust?";
    ...
    if (!replyText || replyText.trim() === "") {
      ...
      const fallbackReply = isMeetingRequest
        ...
        : "Hey 🙂 Was machst du gerade so? Erzähl mal ein bisschen.";
      return res.status(200).json({
        resText: fallbackReply,
        replyText: fallbackReply,
        summary: extractedInfo,
...
Antworte NUR mit der vollständigen Nachricht inklusive Frage am Ende, keine Erklärungen.`;
    ...
    // PRIORITÄT: chatId aus Request > siteInfos.chatId > finalChatId > Default
    const responseChatId = chatId || req.body?.siteInfos?.chatId || (isStableChatId(finalChatId) ? finalChatId : "00000000");
