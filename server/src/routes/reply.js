...
WICHTIG: Wirke NICHT zu schlau! Du bist eine normale Person, keine Wissensdatenbank! Weiche IMMER aus, auch wenn der Kunde explizit danach fragt! NIEMALS sachlich oder wissenschaftlich antworten!`;
    ...
    if (trainingData && trainingData.conversations && Array.isArray(trainingData.conversations) && trainingData.conversations.length > 0) {
      ...
      if (relevantExamples.length === 0) {
        // Verwende zufällig ausgewählte Beispiele für maximale Variation
        const allAvailable = trainingData.conversations
          .filter(conv => conv.customerMessage)
          .sort(() => Math.random() - 0.5)
          .slice(0, 30);
        allAvailable.forEach(ex => {
          relevantExamples.push(ex);
          usedMessages.add(ex.customerMessage);
        });
        console.log(`📚 Fallback: Verwende ${allAvailable.length} zufällig ausgewählte Beispiele (von ${trainingData.conversations.length} gesamt)`);
        ...
        const remainingExamples = trainingData.conversations
          .filter(conv => conv.customerMessage && !usedMessages.has(conv.customerMessage))
          .sort(() => Math.random() - 0.5)
          .slice(0, 20 - relevantExamples.length);
        
        remainingExamples.forEach(ex => {
            relevantExamples.push(ex);
            usedMessages.add(ex.customerMessage);
          });
          
        console.log(`📚 ${remainingExamples.length} zusätzliche zufällige Beispiele hinzugefügt (Gesamt: ${relevantExamples.length})`);
      }
