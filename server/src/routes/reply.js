...
router.post("/", asyncHandler(async (req, res, next) => {
  ...
  try {
    ...
    if (isASACalculated && asaFromKussOrLike && isFPC) {
      ...
      const meta = req.body?.siteInfos?.metaData || {};
      const lastCustomerMessageAgeMs = meta.lastCustomerMessageAgeMs != null ? Number(meta.lastCustomerMessageAgeMs) : null;
      const lastCustomerMessageTimestamp = meta.lastCustomerMessageTimestamp != null ? meta.lastCustomerMessageTimestamp : null;
      if (isASACalculated && (lastCustomerMessageAgeMs != null || lastCustomerMessageTimestamp != null)) {
        console.log('🤖 ASA: Explizites Alter der letzten Kunden-Nachricht von Extension:', lastCustomerMessageAgeMs != null ? (Math.round(lastCustomerMessageAgeMs / 60000) + ' Min') : ('Timestamp ' + lastCustomerMessageTimestamp));
      }
