...
const { writeToGoogleSheets, ensureHeaders } = require("../utils/google-sheets");
const fs = require("fs");
...
- Beispiel-Nachrichten: ${writingStyle.sampleTexts ? writingStyle.sampleTexts.substring(0, 300) : ""}` : "";
    ...
    // WICHTIG: Nutze Config-Dateien für Regeln (EINMALIG laden)
    let rules = {};
    ...
    try {
      ...
      // Füge allgemeine Regeln zum System-Prompt hinzu
      ...
      try {
        Object.entries(rules.situationalResponses).forEach(([situationKey, situationRule]) => {
          ...
          if (matches && situationRule.instructions) {
            ...
            if (situationKey === "sexualTopics" && rules.preferredWords && typeof rules.preferredWords === "object") {
...
Antworte NUR mit der vollständigen Nachricht inklusive Frage am Ende, keine Erklärungen.`;
  ...
  try {
    ...
    // Optional: Schreibe auch in Google Sheets (wenn konfiguriert)
    try {
      await ensureHeaders(); // Stelle sicher, dass Header existieren
      await writeToGoogleSheets(messageEntry);
    } catch (sheetsErr) {
      // Nicht kritisch - Google Sheets ist optional
      console.error('⚠️ Google Sheets Fehler (nicht kritisch):', sheetsErr.message);
    }
  } catch (logErr) {
