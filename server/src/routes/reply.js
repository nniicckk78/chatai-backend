...
router.post("/", asyncHandler(async (req, res, next) => {
  ...
  if ((!foundMessageText || foundMessageText.trim() === "") && req.body?.siteInfos?.messages) {
    ...
    let cherryNewestAtTop = false;
    if (isCherryOrigin) {
      // Cherry kann je nach View/Render als [neueste..älteste] ODER [älteste..neueste] kommen.
      // Richtung pro Request dynamisch aus den Rand-Zeitstempeln ableiten.
      let firstTs = null;
      let lastTs = null;
      for (let i = 0; i < orderedMsgs.length; i++) {
        ...
        if (!isNaN(t) && t > 0) { firstTs = t; break; }
      }
      for (let i = orderedMsgs.length - 1; i >= 0; i--) {
        ...
        if (!v) continue;
        const t = new Date(v).getTime();
        if (!isNaN(t) && t > 0) { lastTs = t; break; }
      }
      if (firstTs != null && lastTs != null) {
        cherryNewestAtTop = firstTs > lastTs;
      } else {
        cherryNewestAtTop = false; // Fallback: konservativ [älteste..neueste]
      }
      console.log("✅ Cherry Richtungs-Erkennung:", cherryNewestAtTop ? "[neueste..älteste]" : "[älteste..neueste]");
    }
