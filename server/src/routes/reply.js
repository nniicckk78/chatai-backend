...
router.post("/", asyncHandler(async (req, res, next) => {
      ...
      if (dataUrl) {
        ...
        if (desc) {
          ...
          if ((imageType === 'penis' || imageType === 'dildo' || imageType === 'nude') && desc) {
            ...
            if (isNegative) {
              console.warn(`⚠️ Bildbeschreibung ist negativ/ablehnend (z.B. API verweigert) – setze positiven Kontext für Pipeline.`);
              // Immer flirty/positiv auf Kundenbilder reagieren (auch nackt/Penis) – festen positiven Kontext setzen
              ...
              console.log(`✅ Positiver Bild-Kontext gesetzt (Typ: ${imageType}):`, desc.substring(0, 80) + '...');
            }
          }
  ...
  // Extrahiere Situation(en) – kontextbasiert per LLM, Fallback auf Stichwörter (für Grok-Pipeline + Multi-Stage)
  ...
  if (customerMessage && !isASACalculated) {
    ...
    if (llmSituations && llmSituations.length > 0) {
      ...
      if (msgLower.includes('wohnort') || msgLower.includes('wo wohnst') || msgLower.includes('woher')) {
        detectedSituations.push('Wohnort-Frage');
      }
      if (msgLower.includes('geld') || msgLower.includes('coins') || msgLower.includes('credits')) {
        detectedSituations.push('Geld/Coins');
      }
      if (msgLower.includes('bot') || msgLower.includes('ki') || msgLower.includes('fake')) {
        detectedSituations.push('Bot-Vorwurf');
      }
      if (msgLower.includes('beruf') || msgLower.includes('arbeit')) {
        detectedSituations.push('Berufsfrage');
      }
      if (msgLower.includes('kontakt') || msgLower.includes('whatsapp') || msgLower.includes('telegram') || msgLower.includes('instagram') || msgLower.includes('nummer')) {
        detectedSituations.push('Kontaktdaten außerhalb der Plattform');
      }
      const bildAnfrageMarkers = ['bild von dir', 'foto von dir', 'zeig mir ein bild', 'schick mir ein foto', 'bild von mir', 'foto von mir', 'bild von uns', 'foto von uns', 'hast du ein bild', 'hast du ein foto', 'kannst du mir ein bild', 'kannst du mir ein foto', 'möchte ein bild', 'moechte ein bild', 'will ein bild', 'will ein foto'];
      if (bildAnfrageMarkers.some(term => msgLower.includes(term))) {
        detectedSituations.push('Bilder Anfrage');
      }
      const sexualIndicators = ['sex', 'ficken', 'geil', 'heiß', 'kuss', 'kusse', 'liebe', 'flirt', 'date'];
      if (sexualIndicators.some(term => msgLower.includes(term))) {
        detectedSituations.push('Sexuelle Themen');
      }
      if (detectedSituations.length > 0) detectedSituation = detectedSituations[0];
    }
  ...
  try {
    ...
    if (USE_GROK_PIPELINE && !isASACalculated) {
      ...
      if (profileInfo && profileInfo.moderatorInfo && profileInfo.moderatorInfo.occupation) {
        extractedInfo.assistant = extractedInfo.assistant || {};
