...
router.post("/", asyncHandler(async (req, res, next) => {
  ...
  return res.status(200).json({
    ...
    flags: { 
      ...
      ...(isFPC ? { reloadPageAfterSend: true } : {}) // FPC: Nach Ablauf des Timers und erfolgreichem Send-Klick Seite neuladen (vermeidet falschen Chat bei kurzem Einblenden)
    },
    disableAutoSend: true,
    waitTime: waitTime,
    noReload: true,
    ...(isFPC ? { reloadPageAfterSend: true } : {}) // FPC: Extension soll nach erfolgreichem Senden die Seite neuladen
  });
}));
