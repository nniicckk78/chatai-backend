...
const { runSimplifiedPipeline, runMultiAgentPipeline, runMultiStagePipeline, agentMessageGenerator, agentFirstMessageDetector } = require("../utils/multi-agent"); // 🚀 NEU: Multi-Stage Pipeline + alte Pipelines
...
function isKICheckMessage(text) {
  ...
  const kiCheckPatterns = [
    ...
    /bitte\s+nur\s+den\s+code\s+\d+\s+in\s+diese\s+nachricht/i, // "Bitte nur den Code 334895 in diese Nachricht"
    /bitte\s+nur\s+den\s+code\s+\d+\s+eintragen/i, // "Bitte nur den Code 334895 eintragen"
    /bitte\s+nur\s+den\s+code\s+\d+\s+in\s+diese\s+nachricht\s+eintragen/i, // "Bitte nur den Code 334895 in diese Nachricht eintragen"
    /kontrollfrage.*bitte\s+nur\s+den\s+code/i, // "Kontrollfrage: Bitte nur den Code..."
    /kontrollfrage.*bitte\s+nur.*code.*\d+/i, // "Kontrollfrage: Bitte nur den Code 334895..."
    ...
    /(\d{4,6})\s+eintragen/i, // "717078 eintragen"
  ...
  if (codeMatch) {
    ...
    if (contextLower.includes("ki") || 
        ...
        contextLower.includes("eintragen") ||
  ...
  if ((lower.includes("ki") && lower.includes("prüfung")) || 
    ...
    // Oder ob "eintragen" und eine Zahl vorhanden sind (z.B. "334895 eintragen")
    if (lower.includes("eintragen") && /\d{4,6}/.test(text)) {
      return true;
    }
  }
  ...
  // 🚨 ZUSÄTZLICH: Erkenne "Bitte nur den Code [Zahl] in diese Nachricht eintragen" Pattern direkt
  if (/bitte\s+nur\s+den\s+code\s+\d{4,6}\s+in\s+diese\s+nachricht\s+eintragen/i.test(text)) {
    return true;
  }
  
  // 🚨 ZUSÄTZLICH: Erkenne "Kontrollfrage: Bitte nur den Code [Zahl] in diese Nachricht eintragen" Pattern direkt
  if (/kontrollfrage.*bitte\s+nur\s+den\s+code\s+\d{4,6}\s+in\s+diese\s+nachricht\s+eintragen/i.test(text)) {
    return true;
  }
...
// Hilfsfunktion: Finde eine Stadt im 50km Umkreis
// 🆕 NEU: OpenStreetMap API als HAUPTLÖSUNG, statische Liste als Fallback
async function findNearbyCity(customerCity) {
  ...
  // 🆕 NEU: PRIORITÄT 1: OpenStreetMap API als HAUPTLÖSUNG (funktioniert für ALLE Städte)
  // Nur wenn API fehlschlägt, verwende statische Liste als Fallback
  console.log(`🔍 Suche nahegelegene Stadt für "${customerCity}" via OpenStreetMap API (HAUPTLÖSUNG)...`);
  try {
    const nearbyCity = await findNearbyCityViaAPI(customerCity);
    if (nearbyCity) {
      console.log(`✅ Stadt via OpenStreetMap API gefunden: "${nearbyCity}" (für "${customerCity}")`);
      return nearbyCity;
    }
  } catch (apiErr) {
    console.warn(`⚠️ OpenStreetMap API Fehler: ${apiErr.message} - verwende statische Liste als Fallback`);
  }
  
  // PRIORITÄT 2: Fallback: Statische Liste (nur wenn API fehlgeschlagen ist)
  console.log(`🔍 OpenStreetMap API fehlgeschlagen - versuche statische Liste...`);
  ...
  for (const searchTerm of searchTerms) {
    for (const [key, cities] of Object.entries(nearbyCities)) {
      ...
      if (searchTerm === key || searchTerm.includes(key) || key.includes(searchTerm)) {
        ...
        console.log(`✅ Stadt in statischer Liste gefunden: "${selectedCity}" (für "${customerCity}", Suchbegriff: "${searchTerm}")`);
        return selectedCity;
      }
    }
  }
  
  // Letzter Fallback: Wenn alles fehlgeschlagen ist
  console.warn(`⚠️ Keine nahegelegene Stadt gefunden für: "${customerCity}" (weder API noch statische Liste)`);
  return null;
}
...
async function findNearbyCityViaAPI(customerCity) {
  ...
  try {
    ...
    // WICHTIG: User-Agent ist ERFORDERLICH (ohne wird man geblockt!)
    const geocodeUrl = `https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(customerCity)}&limit=1&addressdetails=1&countrycodes=de,at,ch`;
    ...
    // Berechne 4-5 nahegelegene Punkte (in verschiedene Richtungen, ~30-50km entfernt)
    const nearbyPoints = [];
    const directions = [
      { lat: 0.3, lon: 0.0 },   // Norden
      { lat: -0.3, lon: 0.0 },  // Süden
      { lat: 0.0, lon: 0.4 },    // Osten
      { lat: 0.0, lon: -0.4 },   // Westen
      { lat: 0.2, lon: 0.3 }     // Nord-Osten
    ];
...
function isInfoMessage(msg) {
  ...
  // "User hat ein Bild an Assistant gesendet" ist eine System-Nachricht der Plattform
  // "User hat ein Bild gesendet" ist auch eine System-Nachricht
  // WICHTIG: Diese Meldungen kommen von der PLATTFORM, nicht vom Kunden!
...
function compressConversation(messages, limit = 30) {
  ...
  // 🚨 KRITISCH: Finde die letzte Moderator-Nachricht (sent) und die neueste Kunden-Nachricht (received)
  // Diese müssen IMMER einbezogen werden, auch wenn sie alt sind!
  let lastModeratorMessage = null;
  let newestCustomerMessage = null;
  
  // Finde letzte Moderator-Nachricht (auch wenn alt)
  for (let i = messages.length - 1; i >= 0; i--) {
    const m = messages[i];
    if (!isInfoMessage(m) && (m.type === "sent" || m.messageType === "sent") && 
        typeof m?.text === "string" && m.text.trim() !== "") {
      lastModeratorMessage = m;
      break;
    }
  }
  
  // Finde neueste Kunden-Nachricht (auch wenn alt)
  for (let i = messages.length - 1; i >= 0; i--) {
    const m = messages[i];
    if (!isInfoMessage(m) && (m.type === "received" || m.messageType === "received") && 
        typeof m?.text === "string" && m.text.trim() !== "") {
      newestCustomerMessage = m;
      break;
    }
  }
  
  // Filtere Info-Messages UND zu alte Nachrichten (aber behalte letzte Moderator + neueste Kunden-Nachricht!)
  const nonInfo = messages.filter(m => {
    if (isInfoMessage(m)) return false;
    if (typeof m?.text !== "string" || m.text.trim() === "") return false;
    
    // 🚨 KRITISCH: Letzte Moderator-Nachricht und neueste Kunden-Nachricht IMMER behalten, auch wenn alt!
    if (m === lastModeratorMessage || m === newestCustomerMessage) {
...
function isMeetingRequest(text, customerMessage = "") {
  ...
  // Diese sind ERLAUBT, da sie nur darüber sprechen, wie toll es wäre, nicht vorschlagen:
  ...
  // Diese Phrasen zeigen, dass die KI aktiv ein Treffen vorschlägt/vereinbart/zustimmt:
...
async function generateFirstMessage({
  ...
  try {
    ...
    // Extrahiere Fake-Profil-Info
    const fakeName = extractedInfo?.assistant?.Name || profileInfo?.moderatorInfo?.name || "Sandy";
    const fakeCity = extractedInfo?.assistant?.Stadt || profileInfo?.moderatorInfo?.city || "";

    // Vereinfachter System-Prompt für Erstnachrichten
    const systemPrompt = `Du bist ein Chat-Moderator auf einer Dating-Plattform.
Du antwortest als FAKE-PROFIL namens ${fakeName}${fakeCity ? ` aus ${fakeCity}` : ''}.
...
Antworte NATÜRLICH und AUTHENTISCH - nicht wie eine KI!`;
    ...
    // Vereinfachter User-Prompt für Erstnachrichten
...
[DEINE AUFGABE]
Generiere eine natürliche, lockere Erstnachricht an den Kunden.
...
router.post("/", asyncHandler(async (req, res, next) => {
      ...
      if (lastAny) {
        ...
        if (newestReceived) {
          ...
          if (receivedAge <= maxAgeForMessage) {
            ...
            // Nachricht ist zu alt (>30 Minuten) - könnte ASA-Fall sein
  ...
  // Wenn ja, behandle sie als leer, damit die ASA-Erkennung funktioniert
  if (foundMessageText && foundMessageText.trim() !== "") {
    // Erstelle ein temporäres Message-Objekt für die Info-Message-Prüfung
  ...
  if (req.body?.siteInfos) {
    ...
    const alertBoxStr = JSON.stringify(alertBoxMessages).toLowerCase();
    
    // Prüfe auf "Reaktivierung" in alertBoxMessages oder siteInfos
    if (siteInfosStr.includes('reaktivierung') || alertBoxStr.includes('reaktivierung') ||
        siteInfosStr.includes('motiviere den kunden') || alertBoxStr.includes('motiviere den kunden') ||
        siteInfosStr.includes('bitte motiviere') || alertBoxStr.includes('bitte motiviere')) {
      isLastMessageFromFake = true; // 🚨 KRITISCH: Setze isLastMessageFromFake auf true, damit die Pipeline ASA erkennt!
      foundMessageText = ""; // Behandle als leer, damit ASA-Erkennung funktioniert
      console.log("✅ ASA-Indikator gefunden: 'Reaktivierung' in siteInfos/alertBoxMessages - setze isLastMessageFromFake = true");
    }
  }
  ...
  // 🚨 NEU: Prüfe auf Kuss/Like-Systemnachricht BEVOR Backup-Prüfung
  // 🚨 FIX: Erstnachricht vs. ASA unterscheiden!
  // Erstnachricht = keine Nachrichten vom Fake ODER Kunde, nur System-Nachrichten
  // ASA = Es gibt bereits Nachrichten vom Fake, aber keine neue vom Kunden
  const msgsAll = req.body?.siteInfos?.messages || [];
  if (!isLastMessageFromFake && (!foundMessageText || foundMessageText.trim() === "") && msgsAll.length) {
    // 🚨 KRITISCH: Prüfe ZUERST, ob es überhaupt Nachrichten vom Fake gibt (sent-Messages)
    // Wenn KEINE sent-Messages vorhanden sind, ist es eine Erstnachricht, KEIN ASA!
    const sentMsgs = msgsAll.filter(m => (m.type === "sent" || m.messageType === "sent") && !isInfoMessage(m));
    ...
    if (sentMsgs.length === 0) {
      ...
      // Es gibt Nachrichten vom Fake - prüfe auf Like/Kuss-Systemnachricht für ASA
      ...
      if (hasKussSystemMessage || hasLikeSystemMessage) {
        isLastMessageFromFake = true; // 🚨 KRITISCH: Setze isLastMessageFromFake auf true, damit ASA erkannt wird!
        console.log(`✅ ASA erkannt: Kuss/Like-Systemnachricht gefunden (${hasKussSystemMessage ? 'Kuss' : 'Like'}) UND es gibt bereits Nachrichten vom Fake - setze isLastMessageFromFake = true`);
      }
    }
  }
  ...
  if (!isLastMessageFromFake && (!foundMessageText || foundMessageText.trim() === "") && msgsAll.length) {
    ...
    if (sentMsgs.length === 0) {
      ...
      newestMsg = newestFirst ? list[0] : list[list.length - 1];
  ...
  if (list.length > 0) {
    ...
    // 🚨 NEU: Prüfe auf ASA-Indikatoren in siteInfos (z.B. "Reaktivierung", "alertBoxMessages")
    // Diese Indikatoren zeigen an, dass es ein ASA-Fall ist, unabhängig von foundMessageText
    if (!isLastMessageFromFake && req.body?.siteInfos) {
      const siteInfosStr = JSON.stringify(req.body.siteInfos).toLowerCase();
      const alertBoxMessages = req.body.siteInfos.alertBoxMessages || [];
      const alertBoxStr = JSON.stringify(alertBoxMessages).toLowerCase();
      ...
      if (siteInfosStr.includes('reaktivierung') || alertBoxStr.includes('reaktivierung') ||
          ...
          siteInfosStr.includes('bitte motiviere') || alertBoxStr.includes('bitte motiviere')) {
        isLastMessageFromFake = true;
        console.log("✅ ASA erkannt über 'Reaktivierung' in siteInfos/alertBoxMessages.");
      }
    }
    ...
    if (imageUrls.length > 0) {
      ...
      if (dataUrl) {
        ...
        const vision = await client.chat.completions.create({
          ...
          max_tokens: imageType === 'penis' || imageType === 'dildo' || imageType === 'nude' ? 250 : 120, // 🚨 ERHÖHT: Mehr Tokens für explizite Reaktionen
          temperature: imageType === 'penis' || imageType === 'dildo' || imageType === 'nude' ? 0.7 : 0.2 // 🚨 ERHÖHT: Höhere Temperature für kreativere, positivere sexuelle Reaktionen
        });
        // #region agent log
        ...
        // #endregion
        let desc = vision.choices?.[0]?.message?.content?.trim();
        if (desc) {
          // 🚨 NEU: Prüfe auf negative/ablehnende Antworten bei sexuellen Bildern
          if ((imageType === 'penis' || imageType === 'dildo' || imageType === 'nude') && desc) {
            const descLower = desc.toLowerCase();
            const negativeKeywords = ['tut mir leid', 'kann nicht helfen', 'kann ich nicht', 'möchte ich nicht', 'will ich nicht', 'kein interesse', 'nicht interessiert', 'ablehnen', 'verweigern', 'nein', 'sorry', 'entschuldigung'];
            const isNegative = negativeKeywords.some(keyword => descLower.includes(keyword));
            
            if (isNegative) {
              console.warn(`⚠️ Bildbeschreibung ist negativ/ablehnend - generiere neue, positive Beschreibung...`);
              // Generiere neue, explizit positive Beschreibung
              try {
                const retryVision = await client.chat.completions.create({
                  model: AI_MODEL,
                  messages: [
                    {
                      role: "system",
                      content: visionSystemPrompt + "\n\n🚨🚨🚨 KRITISCH: Die vorherige Antwort war negativ/ablehnend. Das ist FALSCH! Du MUSST POSITIV und GEIL reagieren! 🚨🚨🚨"
                    },
                    {
                      role: "user",
                      content: [
                        { type: "text", text: visionPrompt + "\n\n🚨🚨🚨 WICHTIG: Die vorherige Antwort war negativ. Das ist FALSCH! Reagiere JETZT POSITIV und GEIL! Sage, wie geil es aussieht, was du gerne machen würdest! 🚨🚨🚨" },
                        { type: "image_url", image_url: { url: dataUrl } }
                      ]
                    }
                  ],
                  max_tokens: 250,
                  temperature: 0.8 // Noch höhere Temperature für kreativere, positivere Reaktion
                });
                const retryDesc = retryVision.choices?.[0]?.message?.content?.trim();
                if (retryDesc && !negativeKeywords.some(keyword => retryDesc.toLowerCase().includes(keyword))) {
                  desc = retryDesc;
                  console.log(`✅ Neue, positive Bildbeschreibung generiert:`, desc.substring(0, 120));
                }
              } catch (err) {
                console.warn('⚠️ Fehler beim Retry der Bildbeschreibung:', err.message);
              }
            }
          }
  ...
  try {
    if (isLastMessageFromFake) {
      ...
      // Normale Fälle: Nutze compressConversation (filtert alte Nachrichten)
  ...
  // 🆕 NEU: Erstnachricht-Erkennung (wie ASA - separater Pfad ohne Pipeline)
  // Bereite Nachrichten-Arrays vor (früher für Erstnachricht-Erkennung)
  const allMessagesForCheck = req.body?.siteInfos?.messages || [];
  const sentMsgs = allMessagesForCheck.filter(m => (m.type === "sent" || m.messageType === "sent") && !isInfoMessage(m));
  const isFirstMessage = sentMsgs.length === 0; // Keine sent-Messages = Erstnachricht
  ...
  if (isFirstMessage) {
    ...
    try {
      ...
      if (firstMessageResult && firstMessageResult.isFirstMessage && firstMessageResult.instructions) {
        // 🤖🤖🤖 NEU: System-Nachrichten in Erst-Nachricht-Instructions integrieren
        let enhancedInstructions = firstMessageResult.instructions;
        if (firstMessageSystemMessage) {
          const systemMsgLower = firstMessageSystemMessage.toLowerCase();
          if (systemMsgLower.includes('credits') || systemMsgLower.includes('nicht ausreichend')) {
            enhancedInstructions += `\n\n🚨🚨🚨 SYSTEM-NACHRICHT: Der Kunde hat nicht ausreichend Credits für eine Antwort. 🚨🚨🚨\n- Beachte dies in deiner Antwort (aber erwähne Credits NICHT direkt!)\n- Reagiere natürlich, als ob du einfach eine normale Erstnachricht schreibst\n- Keine Hinweise auf Credits oder Aufladen geben\n\n`;
          }
        }
        
        // Generiere Erstnachricht (IMMER OpenAI - zu komplex für LoRA)
        const generatedFirstMessage = await generateFirstMessage({
          client: client, // OpenAI für Erstnachrichten (bessere Qualität)
          model: AI_MODEL,
  ...
  // Extrahiere Situation (für Multi-Agent-Pipeline)
  let detectedSituation = 'allgemein';
  if (customerMessage && !isASACalculated) {
    const msgLower = customerMessage.toLowerCase();
    if (msgLower.includes('treffen') || msgLower.includes('termine') || msgLower.includes('kennenlernen')) {
      detectedSituation = 'Treffen/Termine';
    } else if (msgLower.includes('wohnort') || msgLower.includes('wo wohnst') || msgLower.includes('woher')) {
      detectedSituation = 'Wohnort-Frage';
    } else if (msgLower.includes('geld') || msgLower.includes('coins') || msgLower.includes('credits')) {
      detectedSituation = 'Geld/Coins';
    } else if (msgLower.includes('bot') || msgLower.includes('ki') || msgLower.includes('fake')) {
      detectedSituation = 'Bot-Vorwurf';
    } else if (msgLower.includes('beruf') || msgLower.includes('arbeit')) {
      detectedSituation = 'Berufsfrage';
    }
  ...
  try {
    ...
    if (USE_MULTI_STAGE_PIPELINE) {
      ...
      multiAgentResults = await runMultiStagePipeline({
        ...
        findNearbyCityFunc: findNearbyCity // 🚨 NEU: Für Stadt-Suche
      });
      ...
      multiAgentResults = await runMultiAgentPipeline({
        ...
        findNearbyCityFunc: findNearbyCity,
        isMeetingRequestFunc: (msg, context) => isMeetingRequest(msg, context || customerMessage)
      });
    }
    ...
    if (multiAgentResults && multiAgentResults.needsGermanResponse && multiAgentResults.germanResponse) {
      ...
      // 🚀🚀🚀 NEU: Multi-Stage Pipeline - Nachricht ist bereits generiert!
      if (USE_MULTI_STAGE_PIPELINE && multiAgentResults.finalMessage) {
        generatedMessage = multiAgentResults.finalMessage;
        console.log(`✅ Multi-Stage Pipeline: Nachricht erfolgreich generiert (${generatedMessage.length} Zeichen)`);
        ...
        // Fallback: Verwende agentMessageGenerator (für alte Pipelines)
        console.log('📝 Rufe agentMessageGenerator auf, um Nachricht zu generieren...');
        try {
          ...
          if (messageResult && messageResult.success && messageResult.message) {
            ...
            console.log(`✅ Multi-Agent-Pipeline: Nachricht erfolgreich generiert (${generatedMessage.length} Zeichen)`);
            
            // Extrahiere selectedExamples für Feedback-System
            if (multiAgentResults.training && multiAgentResults.training.selectedExamples) {
              selectedExamples = multiAgentResults.training.selectedExamples;
            }
          } else {
            console.error('❌ agentMessageGenerator: Keine Nachricht generiert', messageResult?.error || 'Unbekannter Fehler');
            generatedMessage = "";
          }
        } catch (err) {
          console.error('❌ Fehler in agentMessageGenerator:', err.message);
          console.error('❌ Stack:', err.stack);
          generatedMessage = "";
        }
      }
  ...
  try {
    ...
    if (!generatedMessage || generatedMessage.trim() === "") {
      ...
      const feedbackPayload = {
        ...
        context: {
          detectedSituations: [detectedSituation].filter(s => s && s !== 'allgemein'), // 🚨 NEU: Speichere erkannte Situation
      ...
      try {
        ...
        if (response.ok) {
          ...
          console.warn(`⚠️ Feedback-Eintrag konnte nicht erstellt werden: ${response.status} - ${errorText} (isASA: ${feedbackPayload.isASA}, chatId: ${feedbackPayload.chatId})`);
          
          // 🚨 ROOT CAUSE FIX: Bei Fehler auch Details loggen
          if (feedbackPayload.isASA) {
            console.warn(`❌ ASA-Feedback konnte NICHT gespeichert werden!`);
          } else {
            console.warn(`❌ Normales Feedback konnte NICHT gespeichert werden!`);
          }
        }
      } catch (err) {
