...
router.post("/", asyncHandler(async (req, res, next) => {
  ...
  if ((!foundMessageText || foundMessageText.trim() === "") && req.body?.siteInfos?.messages) {
    ...
    // 🚨 NEU: Wenn neueste received-Nachricht (inkl. Info) NUR ein Bild ist (kein Kundentext dazu), darauf antworten – nicht auf älteren Text-Fallback. Bild + Text → Text behalten!
    if (foundMessageText && foundMessageText.trim() !== "" && req.body?.siteInfos?.messages) {
      const msgsForNewest = req.body.siteInfos.messages;
      const allReceived = msgsForNewest.filter(m => (m?.type === "received" || m?.messageType === "received") && typeof m?.text === "string");
      ...
      if (newestAny) {
        ...
        const isImageOnly = hasImage && (txt.includes("ein bild wurde übertragen") || txt.includes("bild wurde übertragen")) && onlySystemPhrase(txt);
        if (isImageOnly) {
          foundMessageText = "Der Kunde hat ein Bild geschickt.";
          console.log("✅ Neueste Nachricht ist nur Bild – überschreibe Fallback-Text, antworte auf Bild.");
        }
      }
    }
  }
