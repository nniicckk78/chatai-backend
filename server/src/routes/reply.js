...
Antworte NUR mit gültigem JSON, kein zusätzlicher Text. Format:
{
  ...
  "Other": "NUR Sonstige Infos, die die Fake-Person über SICH SELBST sagt (Termine wie 'morgen Zahnarzt', 'heute was mit Freundin'). NICHT: was der KUNDE über sich sagt. NICHT: Formulierungen die den Kunden meinen (z.B. 'viel Spass beim Zahnarzt' = Wunsch an den Kunden). Bei Unsicherheit: null."
}
...
router.post("/", asyncHandler(async (req, res, next) => {
  ...
  if ((!foundMessageText || foundMessageText.trim() === "") && req.body?.siteInfos?.messages) {
    ...
    // Iluvo/Blenny: Konversationsverlauf – nur Kundennachrichten NACH der letzten Moderator-Nachricht verwenden.
    // Sonst würde z.B. "Kuss" + "Was verstehst du unter verwöhnen?" kombiniert, obwohl auf den Kuss schon geantwortet wurde.
    let messagesToUse = validReceivedMessages;
    if ((isIluvo || isBlennyOrigin) && orderedMsgs.length > 0 && validReceivedMessages.length > 0) {
      let lastSentIdx = -1;
      ...
      if (afterLastMod.length > 0) {
        messagesToUse = afterLastMod;
        console.log("✅ Iluvo/Blenny: Nur Kunden-Nachrichten nach letzter Moderator-Nachricht (Konversationsverlauf):", messagesToUse.length, "Nachricht(en)");
      }
    }

    if (messagesToUse.length > 0) {
      // Bevorzugung der substanziellen Nachricht: unter den letzten 5 Min die längste als Hauptnachricht
      const fiveMinWindow = messagesToUse
      ...
      const lastReceived = (mainByLength && getMessageText(mainByLength.message).length > 80)
        ...
        : messagesToUse[0].message;
      if (mainByLength && mainByLength !== messagesToUse[0] && getMessageText(mainByLength.message).length > 80) {
        console.log('✅ Substanzielle Nachricht gewählt (längere unter letzten 5 Min):', getMessageText(lastReceived).substring(0, 80) + '...');
      }
      ...
      // 🚨 Iluvo/Blenny: Nur kombinieren, wenn ZWISCHEN den Kunden-Nachrichten KEINE Moderator-Nachricht liegt.
      // Liegt dazwischen schon eine Fake-Antwort (z.B. auf Kuss), dann nur auf die neueste Kunden-Nachricht eingehen.
      const recentMessages = messagesToUse
        .filter(m => {
      ...
      let shouldCombine = recentMessages.length > 1;
      if (shouldCombine && (isIluvo || isBlennyOrigin) && orderedMsgs.length > 0) {
        const minIdx = Math.min(...recentMessages.map(r => r.index));
        const maxIdx = Math.max(...recentMessages.map(r => r.index));
        for (let i = minIdx + 1; i < maxIdx; i++) {
          if (orderedMsgs[i] && isSentMessage(orderedMsgs[i])) {
            shouldCombine = false;
            console.log("✅ Iluvo/Blenny: Moderator-Nachricht zwischen Kunden-Nachrichten – nur neueste Kunden-Nachricht verwenden (keine Kombination).");
            break;
          }
        }
      }
      
      if (shouldCombine && recentMessages.length > 1) {
        ...
        if (isInfoMessage(lastReceived)) {
          ...
          const nonInfoMessages = messagesToUse
            .filter(m => m.isValid && !isInfoMessage(m.message))
  ...
  if (!alreadyThankedForFriendRequest && (isFriendRequest || isBefreundetSeinInFoundText)) {
    ...
    else console.log("✅ Freundschaftsanfrage erkannt - generiere Standardantwort");
