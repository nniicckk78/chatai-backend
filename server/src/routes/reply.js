...
// 🚨 NEU: Multi-Agent-Pipeline importieren
...
const AI_MODEL = process.env.AI_MODEL || "gpt-4o-mini"; // Standard: GPT-4o-mini (kann über Umgebungsvariable geändert werden)
...
router.post("/", asyncHandler(async (req, res, next) => {
  ...
  // 🚨 KRITISCH: Prüfe IMMER, ob die letzte echte Nachricht vom Moderator kam - auch wenn foundMessageText vorhanden ist!
  // Das Problem: Eine alte Kunden-Nachricht kann gefunden werden, während die neueste Nachricht vom Moderator ist!
  ...
  // Versuche Nachricht zu generieren
    ...
    if (isLastMessageFromFake) {
      console.log("🔄 ASA-Fall erkannt: Letzte Nachricht vom Fake, generiere Reaktivierungsnachricht über Multi-Agent-Pipeline...");
      ...
      // 🚨 NEU: Verwende Multi-Agent-Pipeline für ASA-Nachrichten
      // Sammle alle nötigen Daten für Multi-Agent-Pipeline
      ...
      // 🚨 KRITISCH: Definiere fakeName, fakeAge, etc. VOR ihrer Verwendung (sie werden normalerweise später definiert)
      ...
      // Sammle Profile-Info für Pipeline
      ...
      // Sammle extractedUserInfo für Pipeline
      const extractedUserInfoForPipeline = extractedInfo;
      
      // 🚨 KRITISCH: Definiere imageUrls VOR ihrer Verwendung (sie wird normalerweise später definiert)
      // Bei ASA gibt es normalerweise keine Bilder, aber prüfe trotzdem assetsToSend
      let imageUrls = [];
      ...
      const imageAnalysisFunc = null; // TODO: Falls vorhanden, hier übergeben
      const proactiveAnalysisFunc = null; // TODO: Falls vorhanden, hier übergeben
      // 🚨 KRITISCH: analyzeWritingStyle ist möglicherweise nicht definiert - verwende null als Fallback
      ...
      // ASA-Kontext für Pipeline (wird aus Chat-Verlauf extrahiert)
    ...
    const asaConversationContext = (compressConversation(req.body?.siteInfos?.messages || [], 10) || "").toLowerCase();
          ...
          if (filteredASAExamples.length > 0 && !asaMessage) {
            ...
            if (!asaMessage || asaMessage.trim() === "") {
              ...
              if (customerMsg && typeof customerMsg === "string" && asaMessage && typeof asaMessage === "string" && asaMessage.toLowerCase().includes(customerMsg.toLowerCase().substring(0, 50))) {
                ...
                console.log(`🔍 DEBUG ASA: asaMessage gesetzt, Wert:`, asaMessage ? `"${asaMessage.substring(0, 50)}..." (${asaMessage.length} Zeichen)` : "NULL");
              }
            }
          }
        }
      ...
      // 🔍 DEBUG: Prüfe asaMessage nach conversations-Suche
      if (asaMessage) {
        console.log(`✅ DEBUG ASA: asaMessage vorhanden nach conversations-Suche: "${asaMessage.substring(0, 50)}..." (${asaMessage.length} Zeichen)`);
        // 🚨 WICHTIG: Wenn asaMessage bereits gesetzt ist, SKIPPE den generischen ASA-Block und gehe direkt zu Post-Processing!
        console.log(`⏭️ DEBUG ASA: Überspringe generischen ASA-Block, da asaMessage bereits gesetzt ist`);
        console.log(`🔍 DEBUG ASA: Code-Flow: Überspringe generischen ASA-Block, gehe direkt zu Zeile 2711 (if (!asaMessage) Prüfung)`);
      } else {
        console.log(`⚠️ DEBUG ASA: asaMessage ist NULL nach conversations-Suche`);
      ...
      if (trainingData && trainingData.asaExamples && Array.isArray(trainingData.asaExamples) && trainingData.asaExamples.length > 0) {
        ...
        // 🔍 DEBUG: Prüfe asaMessage nach generischen ASA-Beispielen
        if (asaMessage) {
          console.log(`✅ DEBUG ASA: asaMessage vorhanden nach generischen ASA-Beispielen: "${asaMessage.substring(0, 50)}..." (${asaMessage.length} Zeichen)`);
        } else {
          console.log(`⚠️ DEBUG ASA: asaMessage ist NULL nach generischen ASA-Beispielen`);
        }
      } // Ende: if (!asaMessage) - generischer ASA-Block
      
      // 🔍 DEBUG: Prüfe asaMessage vor finaler Prüfung
      console.log(`🔍 DEBUG ASA: Vor finaler if (!asaMessage) Prüfung (Zeile 2711), asaMessage=`, asaMessage ? `"${asaMessage.substring(0, 30)}..." (${asaMessage.length} Zeichen)` : "NULL");
      ...
      if (!asaMessage) {
        ...
        // 🚨🚨🚨 KRITISCH: KEIN Fallback über Multi-Agent-Pipeline! ASA-Nachrichten MÜSSEN aus Training-Daten kommen!
        // Fallback-Nachrichten können zu Sperrungen führen und sind nicht gewünscht!
        // 🚨 KRITISCH: Verwende chatId aus Request, damit er sich NICHT ändert!
        const safeChatId = chatId || req.body?.siteInfos?.chatId || req.body?.siteInfos?.metaData?.chatId || finalChatId || "00000000";
        return res.status(200).json({
          error: "❌ FEHLER: Keine ASA-Beispiele in Training-Daten gefunden - keine ASA-Nachricht generiert (verhindert Fallback-Nachrichten)",
          resText: "",
          replyText: "",
          summary: extractedInfo || {},
          chatId: safeChatId, // 🚨 WICHTIG: Verwende chatId aus Request, damit er sich NICHT ändert!
          actions: [],
          flags: { 
            blocked: false, // 🚨 NICHT blocked, damit Extension nicht neu lädt!
            noReload: true, // Explizites Flag: Nicht neu laden
            skipReload: true // Zusätzliches Flag
          },
          noReload: true, // Explizites Flag auf oberster Ebene
          disableAutoSend: true, // Verhindere Auto-Send
          isError: true, 
          showError: false // showError: false, damit Extension nicht blockiert
        });
      }
      
      // ✅ ASA-Nachricht erfolgreich aus Training-Daten generiert - verwende diese!
      console.log("✅ ASA-Nachricht aus Training-Daten generiert:", asaMessage.substring(0, 100) + "...", `(${asaMessage.length} Zeichen)`);
      console.log(`🔍 DEBUG ASA: Code-Flow: if (!asaMessage) Prüfung übersprungen, gehe zu Post-Processing`);
      console.log(`🔍 DEBUG ASA: Vor Post-Processing, asaMessage="${asaMessage.substring(0, 100)}..."`);
      ...
      if (asaMessage && asaMessage.length < asaMinLen) {
        console.warn(`⚠️ ASA-Nachricht zu kurz (${asaMessage.length} Zeichen, benötigt >=150) - füge zusätzlichen Text hinzu...`);
      ...
      if (!asaMessage || asaMessage.trim() === "") {
        console.error("❌ FEHLER: ASA-Nachricht ist leer nach Post-Processing!");
        // 🚨 KRITISCH: Verwende chatId aus Request, damit er sich NICHT ändert!
        const safeChatId = chatId || req.body?.siteInfos?.chatId || req.body?.siteInfos?.metaData?.chatId || finalChatId || "00000000";
        return res.status(200).json({
          error: "❌ FEHLER: ASA-Nachricht ist leer nach Post-Processing",
          resText: "",
          replyText: "",
          summary: extractedInfo || {},
          chatId: safeChatId, // 🚨 WICHTIG: Verwende chatId aus Request, damit er sich NICHT ändert!
      ...
      try {
        ...
        // Für ASA: Verwende die letzte Kundennachricht aus dem Chat-Verlauf (oder leeren String)
        ...
        console.log(`✅ ASA-Feedback-Eintrag wird erstellt (Chat-ID: ${chatIdForFeedback}, Platform: ${detectedPlatform})`);
      } catch (err) {
        console.warn('⚠️ Fehler beim Erstellen des ASA-Feedback-Eintrags (nicht kritisch):', err.message);
      }

      // ASA-Antwort jetzt direkt zurückgeben (Multi-Agent-Pipeline bleibt deaktiviert)
      console.log(`🔍 DEBUG ASA: Vor return, asaMessage="${asaMessage.substring(0, 100)}..." (${asaMessage.length} Zeichen)`);
      // 🚨 KRITISCH: Verwende IMMER den chatId aus dem Request, damit er sich NICHT ändert!
      // Wenn sich chatId ändert, lädt die Extension die Seite neu!
      ...
      console.log(`🚀 DEBUG ASA: Returniere ASA-Nachricht, chatId=${asaChatId}, waitTime=${asaWaitTime}`);
      console.log(`🔍 DEBUG ASA: chatId-Quellen - chatId=${chatId}, siteInfos.chatId=${req.body?.siteInfos?.chatId}, metaData.chatId=${req.body?.siteInfos?.metaData?.chatId}, finalChatId=${finalChatId}`);

      // 🚨 KRITISCH: ASA-Antwort MUSS alle Flags haben, damit Extension NICHT neu lädt!
      return res.json({
        resText: asaMessage,
        replyText: asaMessage,
        summary: {},
        chatId: asaChatId, // 🚨 WICHTIG: Verwende chatId aus Request, damit er sich NICHT ändert!
        actions: [
          {
      ...
      // 🚨🚨🚨 ENTFERNT: Multi-Agent-Pipeline für ASA-Nachrichten!
      // ASA-Nachrichten sollen NUR aus trainingData.asaExamples kommen, NICHT über Multi-Agent-Pipeline generiert werden!
      // Die Multi-Agent-Pipeline ist für normale Konversationen gedacht, nicht für Reaktivierungsnachrichten!
      // Dies verhindert auch Fallback-Nachrichten, die zu Sperrungen führen können!
    }
  ...
  // Diese Prüfung ist redundant, da der ASA-Block bereits alle Fälle abdeckt
  // Entfernt, um zu vermeiden, dass der Code fälschlicherweise hier ankommt
    
    // Prüfe auf Bot/KI-Vorwürfe - NUR bei ECHTEM Vorwurf, nicht bei Verneinung!
    // 🚨 KRITISCH: Null-Check für foundMessageText (kann null/undefined/leer sein bei ASA)
    // 🚨 KRITISCH: lowerMessage muss außerhalb des if-Blocks definiert werden, da es später verwendet wird
    const lowerMessage = (foundMessageText && typeof foundMessageText === 'string' && foundMessageText.trim() !== '') 
      ? foundMessageText.toLowerCase() 
      : '';
    
    let isBotAccusation = false;
    if (lowerMessage) {
      ...
      isBotAccusation = hasBotKeyword && !hasNegation;
    }
    
    // Prüfe auf allgemeine Wissensfragen (die KI soll NICHT zu schlau wirken!)
    // 🚨 KRITISCH: Nur prüfen wenn lowerMessage vorhanden ist
    ...
    const isKnowledgeQuestion = lowerMessage ? knowledgeQuestionKeywords.some(keyword => lowerMessage.includes(keyword)) : false;
    
    // Prüfe auf sexuelle Themen (nur wenn lowerMessage vorhanden)
    const isSexualTopic = lowerMessage ? (
      lowerMessage.includes("titten") || lowerMessage.includes("brüste") ||
    ...
    // Prüfe auf Berufsfragen (nur wenn lowerMessage vorhanden)
    const isJobQuestion = lowerMessage ? (
    ...
    // 🚨 NEU: Validiere Kontext-Objekt für sichere Verwendung
    function validateContext(context) {
      ...
      return {
        hasSexualContent: Boolean(context.hasSexualContent),
        detectedSituations: Array.isArray(context.detectedSituations) 
          ? context.detectedSituations 
          : []
      };
    }
    ...
    // 🚨 NEU: Kontext-bewusste Validierung - erhält Kontext-Informationen
    function isMeetingRequest(text, customerMessage = "", context = {}) {
      // 🚨 NEU: Validiere Kontext vor Verwendung
      ...
      // 🚨 NEU: Kontext-bewusste Validierung - extrahiere Kontext-Informationen (verwende validierten Kontext)
      const hasSexualContent = safeContext.hasSexualContent || false;
      const detectedSituations = safeContext.detectedSituations || [];
      const isSexualConversation = hasSexualContent || detectedSituations.some(s => s.toLowerCase().includes('sexuell'));
      
      // 🚨 NEU: Bei sexuellen Gesprächen: Strengere Prüfung - nur ECHTE Treffen blockieren, nicht sexuelle Fantasien
      if (isSexualConversation) {
        // Bei sexuellen Gesprächen: Prüfe, ob es wirklich um ein REALES Treffen geht (nicht nur sexuelle Fantasie)
        // Beispiel: "deinen Kopf zwischen meine Schenkel" → KEINE Treffen-Anfrage, auch wenn "bei mir" vorkommt
        // Beispiel: "wann können wir uns treffen" → ECHTE Treffen-Anfrage
        
        // Prüfe auf explizite sexuelle Wörter in der Nachricht
        const explicitSexualKeywords = ["ficken", "sex", "blasen", "lecken", "kopf zwischen", "schenkel", "pussy", "schwanz", "titten", "arsch"];
        const hasExplicitSexualInText = explicitSexualKeywords.some(keyword => combinedLower.includes(keyword));
        
        // Wenn explizit sexuelle Wörter vorhanden → KEINE Treffen-Anfrage (nur sexuelle Fantasie)
        if (hasExplicitSexualInText) {
          console.log(`🚨 isMeetingRequest: Sexuelles Gespräch erkannt - explizit sexuelle Wörter gefunden, KEINE Treffen-Anfrage`);
          return false;
        }
        
        // Bei sexuellen Gesprächen: Nur blockieren, wenn es ECHTE Treffen-Anfragen sind (ohne sexuelle Wörter)
        // Prüfe auf ECHTE Treffen-Anfragen (z.B. "wann können wir uns treffen", "lass uns treffen")
        const realMeetingPatterns = [
          /\b(lass|lass uns|wollen|können|sollen|möchten|möchtest)\s+(wir|uns)\s+(treffen|sehen|kennenlernen)\b/i,
          /\bwann\s+(können|wollen|sollen|möchten)\s+(wir|uns)\s+(treffen|sehen|kennenlernen)\b/i,
          /\bwann\s+(hättest|hast|hättest du|hast du)\s+(du|die)\s+(zeit|möglichkeit|gelegenheit)\b/i,
          /\b(hättest|hast|hättest du|hast du)\s+(du|die)\s+(zeit|möglichkeit|gelegenheit)\s+(für|für uns|für mich)\b/i
        ];
        
        const isRealMeetingRequest = realMeetingPatterns.some(pattern => pattern.test(combinedLower));
        
        if (isRealMeetingRequest) {
          console.log(`🚨 isMeetingRequest: ECHTE Treffen-Anfrage erkannt (trotz sexuellem Gespräch)`);
          return true;
        }
        
        // Ansonsten: KEINE Treffen-Anfrage (nur sexuelle Fantasie)
        return false;
      }
      
      // 🚨 NEU: Bei NICHT-sexuellen Gesprächen: Normale Prüfung (wie bisher)
    ...
    // 🚨 NEU: Kontext-Objekt wird später zentral erstellt (nach detectedSituations)
    // Vorläufig: Leeres Kontext-Objekt für ersten Aufruf
    ...
    const isCustomerMeetingRequest = isMeetingRequest(foundMessageText, "", validationContext);
...
- Bei sensiblen Themen (z.B. Krankheit): Du kannst geschickt ausweichen ohne direkt darauf eingehen zu müssen (z.B. "Mir geht es schon etwas besser" statt "Ja, ich bin noch krank geschrieben")` : "";
    ...
    // 🚨 NEU: Aktualisiere Kontext-Objekt SOFORT nach detectedSituations-Erstellung
    validationContext.detectedSituations = detectedSituations;
...
🚨🚨🚨 KRITISCH: DER KUNDE WILL EIN TREFFEN - DU DARFST KEIN TREFFEN AUSMACHEN! 🚨🚨🚨

🚨 ABSOLUT VERBOTEN - DIESE WÖRTER DÜRFEN IN DEINER ANTWORT NIEMALS VORKOMMEN:
- "treffen" / "Treffen" / "treffen können" / "können wir treffen" / "wollen wir treffen"
- "Date" / "date"
- "Café" / "Cafe" / "café" / "cafe"
- "Park" / "park"
- "Spaziergang" / "spaziergang"
- "abholen" / "abhole" / "abholen können"
- "vorbeikommen" / "besuchen" / "vorbeikommen können"
- "bei dir" / "bei mir" / "zu dir" / "zu mir"
- "sehen wir uns" / "können wir uns sehen" / "wollen wir uns sehen"
- "echtes Leben" / "real life" / "im echten Leben"
- "kennenlernen" / "kennen lernen" (wenn es um ein Treffen geht)
- "wann" + "können" + "wir" (z.B. "wann können wir...")
- "wann" + "hättest" + "Zeit" (z.B. "wann hättest du Zeit")
- "wann" + "passt" + "es" (z.B. "wann passt es dir")
- "wann" + "klappt" + "es" (z.B. "wann klappt es bei dir")

🚨 ABSOLUT VERBOTEN - DIESE FORMULIERUNGEN DARFST DU NIEMALS VERWENDEN:
- "Ich wäre gerne bereit" / "Ich bin bereit" / "Ich würde gerne kommen"
- "Lass uns treffen" / "Lass uns uns treffen"
- "Wir können uns treffen" / "Wir könnten uns treffen"
- "Wann können wir uns treffen" / "Wann wollen wir uns treffen"
- "Wann hättest du Zeit" / "Wann passt es dir"
- "Ich komme zu dir" / "Du kannst mich abholen"
- "Bei dir oder bei mir" / "Bei mir oder bei dir"
- "Wir machen ein Date aus" / "Wir verabreden uns"
- JEDE Formulierung, die ein Treffen vorschlägt, zusagt oder andeutet!

✅ ERLAUBT - SO SOLLST DU REAGIEREN:
...
- "Das schauen wir noch, aber erst möchte ich dich noch besser kennenlernen"
- "Ich finde es schön, dass du das möchtest, aber ich möchte dich erst noch besser kennenlernen"
- Lenke das Thema zurück auf Chat, Gefühle, Persönlichkeit oder Fantasie
- Stelle eine Frage über seine Hobbys, Vorlieben, etc.

❌ FALSCH (macht ein Treffen aus): "Lass uns treffen! Wann hättest du Zeit?"
❌ FALSCH (macht ein Treffen aus): "Ich würde gerne zu dir kommen"
❌ FALSCH (macht ein Treffen aus): "Wir können uns im Café treffen"
✅ RICHTIG (reagiert, macht aber kein Treffen aus): "Ich würde gerne wissen, wie du es dir vorstellst, aber ich möchte dich erst noch besser kennenlernen. Erzähl mir doch mal mehr über dich - was machst du denn so in deiner Freizeit?"
...
- ABER: Verwende KEINE der verbotenen Wörter oder Formulierungen!

WICHTIG: Diese Regel hat HÖCHSTE PRIORITÄT und überschreibt JEDE andere Anweisung!
Wenn du diese Regel nicht befolgst, wird deine Antwort BLOCKIERT!`;
...
WICHTIG: Wirke NICHT zu schlau! Du bist eine normale Person, keine Wissensdatenbank! Weiche IMMER aus, auch wenn der Kunde explizit danach fragt! NIEMALS sachlich oder wissenschaftlich antworten!`;
    ...
    // 🚨 KRITISCH: Bei ASA-Fall: Überspringe normalen Code-Pfad komplett (ASA-Block hat bereits return)
    // Diese Prüfung verhindert, dass der normale Code-Pfad ausgeführt wird, wenn ASA erkannt wurde
    // Der ASA-Block sollte bereits ein return haben, aber diese Prüfung ist eine zusätzliche Sicherheit
    if (isLastMessageFromFake) {
      console.log("⚠️ ASA-Fall erkannt - normaler Code-Pfad wird übersprungen (ASA-Block sollte bereits return haben)");
      // Der ASA-Block sollte bereits ein return haben, aber falls nicht, überspringen wir hier
      // Diese Prüfung ist redundant, aber sichert ab
    }
    ...
    // 🚨 KRITISCH: Wenn ASA-Fall, überspringe Training-Daten-Suche (ASA-Block hat bereits gehandelt)
    if (!isLastMessageFromFake && trainingData && trainingData.conversations && Array.isArray(trainingData.conversations) && trainingData.conversations.length > 0) {
      ...
      // 🚨 NEU: Aktualisiere Kontext-Objekt SOFORT, wenn hasSexualContent verfügbar ist
      validationContext.hasSexualContent = hasSexualContent;
      validationContext.detectedSituations = detectedSituations; // Falls sich geändert hat
      ...
      try {
        ...
        // 🚨 KRITISCH: Bei ASA oder leerer Nachricht: Überspringe Vektor-Suche oder verwende Fallback-Query
        if (!foundMessageText || foundMessageText.trim() === '') {
          console.warn('⚠️ Vektor-Suche: foundMessageText ist leer - überspringe Vektor-Suche (ASA-Fall oder keine Nachricht)');
          vectorSearchResults = []; // Leere Ergebnisse, Fallback zu Keyword-Matching wird verwendet
        } else {
          // 🚨 NEU: Verbesserte Query-Generierung mit Antwort-Patterns
          let queryText = foundMessageText;
          const lowerMessageForQuery = foundMessageText.toLowerCase();
        
          // 🚨 NEU: Funktion die erwartete Antwort-Patterns basierend auf Situationen extrahiert
          ...
          // Extrahiere Antwort-Patterns
          const answerPatterns = extractAnswerPatterns(detectedSituations, foundMessageText);
          ...
          if (isJobAnswer) {
            ...
            if (answerPatterns.length > 0) {
              queryText += ` Antwort: ${answerPatterns.join(' ')}`;
            }
            ...
            // 🚨 NEU: Füge Antwort-Patterns zur Query hinzu
            const situationPart = `${detectedSituations.join(' + ')}`;
            const customerPart = foundMessageText;
            const patternPart = answerPatterns.length > 0 ? ` Antwort: ${answerPatterns.join(' ')}` : '';
            queryText = `${situationPart}: ${customerPart}${patternPart}`;
          }
          ...
          vectorSearchResults = await findSimilarExamples(queryText, {
            ...
            minSimilarity: 0.3, // 🚨 REDUZIERT von 0.4 auf 0.3 für mehr relevante Beispiele (weniger restriktiv)
          ...
          if (vectorSearchResults.length > 0) {
            ...
            console.warn(`⚠️ Vektor-Suche: KEINE ähnlichen Beispiele gefunden (minSimilarity: 0.3, Query: "${queryText.substring(0, 100)}...")`);
          }
        }
      } catch (err) {
      ...
      if (vectorSearchResults.length > 0) {
        ...
        // Fallback: Keyword-Matching (wenn Vektor-Suche keine Ergebnisse hat ODER foundMessageText leer ist)
        // 🚨 KRITISCH: Bei ASA oder leerer Nachricht: Überspringe Keyword-Matching oder verwende generische Beispiele
        if (!foundMessageText || foundMessageText.trim() === '' || !lowerMessage) {
          console.warn('⚠️ Keyword-Matching: foundMessageText ist leer - verwende ALLE verfügbaren Training-Daten als Fallback');
          // Bei ASA oder leerer Nachricht: Verwende ALLE Training-Daten (besser als nichts)
          const allTrainingExamples = trainingData.conversations
            .filter(conv => {
              if (usedMessages.has(conv.customerMessage)) return false;
              if (!conv.moderatorResponse || typeof conv.moderatorResponse !== 'string') return false;
              return true;
            })
            .slice(0, 15); // Maximal 15 Beispiele
          
          allTrainingExamples.forEach(conv => {
            if (!usedMessages.has(conv.customerMessage)) {
              relevantExamples.push(conv);
              usedMessages.add(conv.customerMessage);
            }
          });
          
          console.log(`📚 ${allTrainingExamples.length} generische Training-Daten-Beispiele verwendet (Fallback für leere Nachricht/ASA)`);
        } else {
      ...
      // 🚨🚨🚨 KRITISCH: IMMER mindestens 10-15 Beispiele verwenden für bessere Qualität! 🚨🚨🚨
      // Problem: Wenn zu wenige Beispiele gefunden wurden, müssen wir MEHR nehmen!
      // Lösung: Wenn weniger als 10 Beispiele gefunden, füge ALLE verfügbaren hinzu (bis 15)
      let minRequired = 10; // Mindestanzahl für gute Qualität
      const maxTotalExamples = 15; // Maximum für beste Qualität
      
      // Wenn zu wenige Beispiele gefunden wurden, füge ALLE verfügbaren hinzu
      if (relevantExamples.length < minRequired) {
        console.warn(`⚠️ WARNUNG: Nur ${relevantExamples.length} relevante Beispiele gefunden - füge ALLE verfügbaren Training-Daten hinzu (bis ${maxTotalExamples})!`);
        
        // Füge ALLE Training-Daten hinzu (priorisiert nach Feedback, dann zufällig)
        const allTrainingExamples = trainingData.conversations
          .filter(conv => {
            // Überspringe, wenn bereits verwendet
            if (usedMessages.has(conv.customerMessage)) return false;
            // Überspringe, wenn keine moderatorResponse
            if (!conv.moderatorResponse || typeof conv.moderatorResponse !== "string" || conv.moderatorResponse.trim().length === 0) return false;
            return true;
          })
          .sort((a, b) => {
            // Priorisiere Feedback-Einträge
            const aIsFeedback = a.priority === true || a.source === 'feedback_good' || a.source === 'feedback_edited';
            const bIsFeedback = b.priority === true || b.source === 'feedback_good' || b.source === 'feedback_edited';
            if (aIsFeedback && !bIsFeedback) return -1;
            if (!aIsFeedback && bIsFeedback) return 1;
        ...
        // Füge so viele hinzu, bis wir minRequired haben
        ...
        console.log(`✅ ${relevantExamples.length} Training-Beispiele werden verwendet (erweitert von ${currentCount} auf ${relevantExamples.length})`);
      }
      ...
      console.log(`✅ Insgesamt ${finalExamples.length} Training-Beispiele werden verwendet (von ${trainingData.conversations.length} verfügbaren, Ziel: ${minRequired}-${maxTotalExamples} für beste Qualität)`);
...
ZIEL: Deine Antwort soll sich anfühlen wie eine echte Chat-Reply aus den Training-Daten - nicht generisch oder "KI-mäßig"!`;
    ...
    } // Ende: if (!isLastMessageFromFake && trainingData...) - normaler Code-Pfad nur wenn KEIN ASA
    ...
    // 🚨 KRITISCH: Bei ASA sollte der normale Code-Pfad bereits übersprungen worden sein
    // Diese Prüfung ist eine zusätzliche Sicherheit, falls der ASA-Block nicht richtig geendet hat
...
🚨🚨🚨 KRITISCH: PROFIL-INFOS NUR VERWENDEN, WENN ZUM KONTEXT PASSEND! 🚨🚨🚨
- Diese Infos sind HINTERGRUND-Informationen, NICHT der Hauptkontext!
- Der CHAT-VERLAUF hat HÖCHSTE PRIORITÄT - was wurde VORHER besprochen?
- Verwende Hobbies/Interessen NUR, wenn sie zum aktuellen Gespräch passen!
- BEISPIEL FALSCH: Gespräch über Licht/Schatten/Sonne → "Was kochst du gerne?" → FALSCH! Nicht zum Kontext passend!
- BEISPIEL RICHTIG: Gespräch über Essen → "Was kochst du gerne?" → RICHTIG! Zum Kontext passend!
- Wenn der Kontext philosophisch/abstrakt ist (z.B. Licht/Schatten), dann bleibe beim Thema, verwende NICHT plötzlich Hobbies!
- Profil-Infos sind nur für Hintergrund-Wissen - der CHAT-VERLAUF bestimmt das Thema!\n` : ''}
...
- Sei menschlich, emotional, verständnisvoll - nicht mechanisch oder oberflächlich!`;
    ...
    // 🚨🚨🚨 MULTI-AGENT-PIPELINE: Verwende Multi-Agent-System statt direkt chat.completions.create 🚨🚨🚨
    // 🚨 WICHTIG: Nur für normale Nachrichten (NICHT ASA - ASA wird separat gehandhabt)
    
    if (!isLastMessageFromFake) {
      // 🚨 NEU: Sammle alle nötigen Daten für Multi-Agent-Pipeline
      const messages = req.body?.siteInfos?.messages || [];
      const allMessages = messages; // Alle Nachrichten für Pipeline
      
      // Extrahiere moderatorMessages und customerMessages (bereits oben gemacht, hier nochmal für Pipeline)
      const totalMessages = messages.filter(m => !isInfoMessage(m) && typeof m?.text === "string" && m.text.trim() !== "").length;
      const contextSize = totalMessages > 20 ? 20 : totalMessages > 10 ? 15 : 10;
      ...
      // 🚨 KRITISCH: Definiere imageUrls VOR ihrer Verwendung (sie wird normalerweise später definiert)
      // Prüfe assetsToSend (falls Extension Bilder dort sendet)
      let imageUrls = [];
      if (assetsToSend && Array.isArray(assetsToSend) && assetsToSend.length > 0) {
        for (const asset of assetsToSend) {
          const assetUrl = asset.url || asset.imageUrl || asset.src || asset.image_url;
      ...
      // Helper-Funktionen für Pipeline
      const isLocationQuestionFunc = null; // TODO: Falls vorhanden, hier übergeben
      const findNearbyCityFunc = findNearbyCity;
      // 🚨 KRITISCH: isMeetingRequest ist möglicherweise nicht definiert - verwende null als Fallback
      const isMeetingRequestFuncForPipeline = typeof isMeetingRequest !== 'undefined' ? isMeetingRequest : null;
      const vectorDbFunc = async (query, options) => {
      ...
      // ASA-Kontext für Pipeline (leer bei normalen Nachrichten)
      const asaConversationContextForPipeline = '';
      
      try {
        console.log('🤖 Multi-Agent-Pipeline wird gestartet...');
        
        // 🚨 NEU: Rufe Multi-Agent-Pipeline auf
        const multiAgentResults = await runMultiAgentPipeline({
          ...
          feedbackData: null, // TODO: Falls verfügbar, hier übergeben
        ...
        // Prüfe, ob Pipeline blockiert wurde (Safety-Check)
        if (multiAgentResults.blocked) {
          console.error(`🚨 Multi-Agent-Pipeline: BLOCKIERT - ${multiAgentResults.error || 'Unbekannter Fehler'}`);
          return res.status(200).json({
            error: multiAgentResults.error || "❌ FEHLER: Nachricht wurde blockiert",
            resText: multiAgentResults.error || "❌ FEHLER: Nachricht wurde blockiert",
            replyText: "",
            summary: extractedInfo,
            chatId: finalChatId,
            actions: [],
        ...
        console.log('✅ Multi-Agent-Pipeline erfolgreich abgeschlossen');
        
        // 🚀 NEU: Response Validator Agent - prüfe geplante Struktur VOR Generierung
        // 🚨 WICHTIG: Dieser Validator gibt nur Hinweise - blockiert nicht, da er nur eine "geplante Struktur" prüft
        // Die echte Validierung passiert NACH der Generierung in agentMessageGenerator
        const { agentResponseValidator } = require("../utils/multi-agent");
        const plannedStructure = `Antwort auf: "${validatedMessage.substring(0, 100)}..."`;
        
        // 🚨 NEU: Kontext-bewusste Validierung - übergebe Kontext-Informationen
        ...
        const validationResult = await agentResponseValidator(
          plannedStructure,
          validatedMessage,
          isCustomerMeetingRequest,
          multiAgentResults.meetingResponse,
          validationContext
        );
        
        if (!validationResult.isValid && validationResult.issues && validationResult.issues.length > 0) {
          console.warn(`⚠️ Response Validator: Struktur-Probleme erkannt: ${validationResult.issues.join(', ')}`);
          console.log(`💡 Vorschläge: ${validationResult.suggestions.join(', ')}`);
          // 🚨 NEU: Füge Validator-Empfehlungen zur Shared Knowledge Base hinzu
          if (multiAgentResults.sharedKnowledgeBase) {
            validationResult.suggestions.forEach(suggestion => {
              multiAgentResults.sharedKnowledgeBase.addPriorityGuidance(
                suggestion,
                'high',
                'responseValidator'
              );
        ...
        // 🚨 NEU: Rufe agentMessageGenerator auf statt direkt chat.completions.create
        ...
        console.log('✅ Nachricht von Multi-Agent-Pipeline generiert:', replyText ? replyText.substring(0, 100) + '...' : '(leer)');
        
      } catch (err) {
        console.error('❌ Fehler in Multi-Agent-Pipeline:', err.message);
        console.error('❌ Stack:', err.stack);
        
        // Fallback: Versuche direkt mit chat.completions.create (falls Pipeline fehlschlägt)
        console.warn('⚠️ Fallback: Verwende direkt chat.completions.create');
        try {
          ...
          console.log('✅ Fallback: Nachricht direkt generiert');
        } catch (fallbackErr) {
          errorMessage = `❌ FEHLER: Beim Generieren der Nachricht ist ein Fehler aufgetreten: ${fallbackErr.message}`;
          console.error("❌ OpenAI Fehler (Fallback):", fallbackErr.message);
          return res.status(200).json({
            error: errorMessage,
            resText: errorMessage,
            replyText: errorMessage,
      ...
      // 🚨 KRITISCH: Wenn wir hier ankommen, bedeutet das, dass der ASA-Block NICHT korrekt mit return geendet hat!
      // Das sollte NIE passieren - wenn es passiert, gibt es einen kritischen Fehler im ASA-Block!
      console.error("❌ FEHLER: ASA-Fall sollte bereits oben behandelt worden sein! Der ASA-Block hat nicht korrekt mit return geendet!");
      console.error("❌ DEBUG: isLastMessageFromFake=", isLastMessageFromFake, ", asaMessage sollte oben gesetzt worden sein");
      
      // 🚨 KRITISCH: Gebe eine Fehler-Antwort zurück, die KEINE Reloads auslöst!
      // Verwende den chatId aus dem Request, damit er sich nicht ändert
      const safeChatId = chatId || req.body?.siteInfos?.chatId || finalChatId || "00000000";
      return res.status(200).json({
        resText: "",
        replyText: "",
        summary: extractedInfo || {},
    ...
    // 🚨 NEU: Lade Training-Daten-Analyse EINMAL (global initialisiert beim Start)
    const trainingAnalysis = global.trainingDataAnalysis || null;
    ...
    if (rules && rules.forbiddenWords && Array.isArray(rules.forbiddenWords) && rules.forbiddenWords.length > 0) {
      ...
      for (const forbiddenWord of filteredForbiddenWords) {
        ...
        // 🚨 NEU: Training-Daten-basierte kontext-bewusste Prüfung für verbotene Wörter
        // trainingAnalysis wurde bereits oben deklariert
        if (trainingAnalysis) {
          const { checkForbiddenWordInContext } = require('../utils/training-data-analyzer');
          const checkResult = checkForbiddenWordInContext(forbiddenWord, replyText, trainingAnalysis);
          
          if (checkResult.isForbidden) {
            foundForbiddenWords.push(forbiddenWord);
            console.log(`🚨 Training-Daten-Analyse: Verbotenes Wort "${forbiddenWord}" gefunden: ${checkResult.reason}`);
          } else if (checkResult.reason && checkResult.reason.includes('Allowed')) {
            console.log(`✅ Training-Daten-Analyse: "${forbiddenWord}" erlaubt: ${checkResult.reason}`);
            continue; // Überspringe - Wort ist in erlaubtem Kontext
          }
          // Wenn kein spezieller Kontext, falle zurück auf Standard-Prüfung
        }
        
        // 🚨 FALLBACK: Standard-Prüfung (wenn Training-Daten-Analyse nicht verfügbar)
        // Präzisere Prüfung für "reiz" - nur "reizvoll" blockieren, nicht "reizt" oder "reiz"
        if (forbiddenLower === "reiz" || forbiddenLower.includes("reiz")) {
          // Prüfe ob es "reizvoll" ist (verboten) oder nur "reizt" (erlaubt)
          const reizvollMatch = /\breizvoll\b/i.test(replyText);
          const reiztMatch = /\breizt\b/i.test(replyText);
          
          // Nur blockieren wenn "reizvoll" gefunden, nicht "reizt"
          if (reizvollMatch) {
            foundForbiddenWords.push(forbiddenWord);
          } else if (!reiztMatch) {
            // Wenn weder "reizvoll" noch "reizt" gefunden, aber "reiz" als Wort, prüfe ob es als Wort steht
            const reizAsWord = new RegExp(`\\b${forbiddenLower}\\b`, 'i').test(replyText);
            if (reizAsWord && !reizvollMatch) {
              // "reiz" als eigenständiges Wort ist auch verboten
              foundForbiddenWords.push(forbiddenWord);
            }
          }
          // "reizt" ist erlaubt (z.B. "was dich reizt" = "was dich interessiert")
          continue;
        }
        ...
        // 🚨 AUSNAHME: "treffen" ist erlaubt, wenn der Kunde eine Treffen-Anfrage gestellt hat
        // Die KI MUSS darauf reagieren können (z.B. "das schauen wir noch wann es bei uns am besten klappt")
        // ABER: Die KI soll NICHT selbst ein Treffen vorschlagen!
        if (forbiddenLower.includes("treffen")) {
          // Prüfe, ob der Kunde eine Treffen-Anfrage gestellt hat
          ...
          if (typeof isCustomerMeetingRequest !== 'undefined' && isCustomerMeetingRequest) {
            // Prüfe, ob die Antwort eine Reaktion auf die Treffen-Anfrage ist (nicht selbst ein Vorschlag)
            ...
            const allowedMeetingContexts = [
              ...
              /bereit (für|für dich|für euch|für uns)/i // "ich bin bereit für dich"
            ];
            const isAllowedMeetingContext = allowedMeetingContexts.some(pattern => pattern.test(replyLower));
            if (isAllowedMeetingContext) {
              console.log(`✅ "treffen" erkannt, aber im erlaubten Kontext (Reaktion auf Treffen-Anfrage) - NICHT blockiert`);
              continue; // Überspringe diese Prüfung - "treffen" ist hier erlaubt
            }
          }
        }
    ...
    // 🚨 NEU: Training-Daten-basierte Meta-Kommentar-Erkennung
    // trainingAnalysis wurde bereits oben deklariert
    let metaCommentPatterns = [
    ...
    // 🚨 NEU: Füge erlaubte Patterns aus Training-Daten hinzu (diese werden NICHT als Meta-Kommentare erkannt)
    const allowedMetaCommentPatterns = [];
    if (trainingAnalysis && trainingAnalysis.metaComments && trainingAnalysis.metaComments.patterns) {
      // Patterns die in Training-Daten vorkommen und erlaubt sind (z.B. "Das finde ich geil" ohne "dass")
      allowedMetaCommentPatterns.push(...trainingAnalysis.metaComments.patterns);
    }
    
    // Prüfe zuerst, ob es ein erlaubtes Pattern ist (aus Training-Daten)
    let isAllowedMetaPattern = false;
    if (allowedMetaCommentPatterns.length > 0) {
      for (const allowedPattern of allowedMetaCommentPatterns) {
        if (allowedPattern.test(replyText)) {
          // Prüfe, ob es wirklich ein Meta-Kommentar ist oder eine Reaktion auf Inhalt
          const match = replyText.match(allowedPattern);
          if (match) {
            const matchText = match[0];
            const afterMatch = replyText.substring(replyText.indexOf(matchText) + matchText.length);
            
            // Wenn nach dem Match "dass", "wenn", "wie" kommt = Meta-Kommentar (verboten)
            // Wenn direkt auf Inhalt reagiert = erlaubt (aus Training-Daten gelernt)
            if (!/^\s*(dass|wenn|wie|das)/i.test(afterMatch)) {
              isAllowedMetaPattern = true;
              console.log(`✅ Training-Daten-Analyse: Meta-Kommentar-Pattern erkannt, aber erlaubt (Reaktion auf Inhalt, nicht Meta-Kommentar)`);
              break;
            }
          }
        }
    ...
    // Nur prüfen, wenn es kein erlaubtes Pattern ist
    if (!isAllowedMetaPattern) {
    ...
    // 🚨 NEU: Training-Daten-basierte Frage-Anzahl-Validierung
    let hasMultipleQuestions = false;
    
    // trainingAnalysis wurde bereits oben deklariert (beim verbotene-Wörter-Check)
    // Ermittle erlaubte Frage-Anzahl aus Training-Daten (nur wenn Training-Analyse verfügbar)
    const { getAllowedQuestionCount, questionsAreOnDifferentTopics, analyzeContextForValidation } = require('../utils/training-data-analyzer');
    
    // Analysiere Kontext
    const conversationHistory = compressConversation(req.body?.siteInfos?.messages || [], 20);
    const context = trainingAnalysis ? analyzeContextForValidation(validatedMessage, conversationHistory, extractedInfo?.user || {}) : null;
    const allowedQuestionCount = trainingAnalysis && context ? getAllowedQuestionCount(context, trainingAnalysis) : 1;
    
    if (questionCount > allowedQuestionCount) {
      // Wenn mehr Fragen als erlaubt
      if (questionCount === 2 && allowedQuestionCount >= 2) {
        // Spezielle Prüfung für 2 Fragen: Sind sie zu unterschiedlichen Themen?
        const questionMatches = replyText.match(/[^.!?]+\?/g);
        if (questionMatches && questionMatches.length === 2) {
          const areDifferent = questionsAreOnDifferentTopics(questionMatches, conversationHistory);
          if (!areDifferent) {
            // Fragen sind redundant = blockieren
            hasMultipleQuestions = true;
            console.log(`⚠️ Training-Daten-Analyse: 2 Fragen erkannt, aber redundant - blockiere`);
          } else {
            // Fragen sind unterschiedlich = OK
            console.log(`✅ Training-Daten-Analyse: 2 Fragen erkannt, aber unterschiedliche Themen - erlaube`);
          }
        } else {
          hasMultipleQuestions = true;
        }
      } else {
        // 3+ Fragen oder mehr als erlaubt
        hasMultipleQuestions = true;
      }
    }
    ...
    // 🚨🚨🚨 KRITISCH: Wenn verbotene Wörter, Meta-Kommentare, mehrere Fragen, etc. gefunden wurden, MUSS neu generiert werden! 🚨🚨🚨
    // Diese Fehler dürfen NICHT durchgehen - die Antwort ist sonst UNBRAUCHBAR!
    if (foundForbiddenWords.length > 0 || foundMetaComments.length > 0 || foundFormalPhrases.length > 0 || foundGreetings.length > 0 || hasRepetitiveQuestion || hasBlockingResponse || foundRepetitions.length > 0 || hasTooManyExclamations || hasMultipleQuestions || startsWithIch) {
      ...
      // 🚨🚨🚨 STAGE 4: Multi-Retry-Logik (reduziert auf 3 Versuche für weniger Blockierungen)
      const MAX_RETRIES = 3; // 🚨 REDUZIERT von 5 auf 3 für weniger Blockierungen, mehr natürliche Antworten
      let retryAttempt = 0;
...
Blockiere NUR bei: Minderjährige (10-17 Jahre), Inzest, Pädophilie, Zoophilie, Vergewaltigung! `;
        ...
        if (hasMultipleQuestions) {
          if (questionCount === 2) {
            retryReason += `MEHRERE FRAGEN ERKANNT: Die Antwort enthält 2 Fragen. Wenn beide Fragen zum gleichen Thema/redundant sind, stelle nur EINE Frage! Wenn die Fragen zu UNTERSCHIEDLICHEN Themen gehören (z.B. eine Frage zu Hobbies, eine zu Vorlieben), können 2 Fragen OK sein, aber prüfe ob beide wirklich nötig sind. `;
          } else {
            retryReason += `MEHRERE FRAGEN ERKANNT: Die Antwort enthält ${questionCount} Fragen, aber maximal 2 Fragen sind erlaubt (und nur wenn sie zu unterschiedlichen Themen gehören)! Reduziere auf 1-2 Fragen! `;
          }
        }
        if (startsWithIch) {
...
Antworte NUR mit der neuen Antwort, keine Erklärungen.`;
          ...
          if (retryText) {
            ...
            for (const forbiddenWord of filteredForbiddenWords) {
              ...
              // 🚨 NEU: Training-Daten-basierte kontext-bewusste Prüfung auch im Retry
              const retryTrainingAnalysis = global.trainingDataAnalysis;
              if (retryTrainingAnalysis) {
                const { checkForbiddenWordInContext } = require('../utils/training-data-analyzer');
                const retryCheckResult = checkForbiddenWordInContext(forbiddenWord, cleanedRetry, retryTrainingAnalysis);
                
                if (retryCheckResult.isForbidden) {
                  stillForbidden.push(forbiddenWord);
                } else if (retryCheckResult.reason && retryCheckResult.reason.includes('Allowed')) {
                  continue; // Überspringe - Wort ist in erlaubtem Kontext
                }
                // Wenn kein spezieller Kontext, falle zurück auf Standard-Prüfung
              }
              
              // 🚨 FALLBACK: Standard-Prüfung für "reiz" auch im Retry
              if (forbiddenLower === "reiz" || forbiddenLower.includes("reiz")) {
                const reizvollMatch = /\breizvoll\b/i.test(cleanedRetry);
                const reiztMatch = /\breizt\b/i.test(cleanedRetry);
                
                if (reizvollMatch) {
                  stillForbidden.push(forbiddenWord);
                } else if (!reiztMatch) {
                  const reizAsWord = new RegExp(`\\b${forbiddenLower}\\b`, 'i').test(cleanedRetry);
                  if (reizAsWord && !reizvollMatch) {
                    stillForbidden.push(forbiddenWord);
                  }
                }
            ...
            // 🚨🚨🚨 KRITISCH: Prüfe AUCH auf mehrere Fragen im Retry! 🚨🚨🚨
            // 🚨 NEU: Training-Daten-basierte Frage-Anzahl-Validierung auch im Retry
            const retryQuestionMarks = (cleanedRetry.match(/\?/g) || []).length;
            let hasMultipleQuestionsRetry = false;
            
            // Lade Training-Daten-Analyse (global initialisiert beim Start)
            const retryTrainingAnalysis = global.trainingDataAnalysis;
            if (retryTrainingAnalysis) {
              const { getAllowedQuestionCount, questionsAreOnDifferentTopics, analyzeContextForValidation } = require('../utils/training-data-analyzer');
              
              // Analysiere Kontext für Retry
              const retryConversationHistory = compressConversation(req.body?.siteInfos?.messages || [], 20);
              const retryContext = analyzeContextForValidation(validatedMessage, retryConversationHistory, extractedInfo?.user || {});
              
              // Ermittle erlaubte Frage-Anzahl aus Training-Daten
              const retryAllowedQuestionCount = getAllowedQuestionCount(retryContext, retryTrainingAnalysis);
              
              if (retryQuestionMarks > retryAllowedQuestionCount) {
                // Wenn mehr Fragen als erlaubt
                if (retryQuestionMarks === 2 && retryAllowedQuestionCount >= 2) {
                  // Spezielle Prüfung für 2 Fragen: Sind sie zu unterschiedlichen Themen?
                  const retryQuestionMatches = cleanedRetry.match(/[^.!?]+\?/g);
                  if (retryQuestionMatches && retryQuestionMatches.length === 2) {
                    const areDifferent = questionsAreOnDifferentTopics(retryQuestionMatches, retryConversationHistory);
                    if (!areDifferent) {
                      // Fragen sind redundant = blockieren
                      hasMultipleQuestionsRetry = true;
                    }
                  } else {
                    hasMultipleQuestionsRetry = true;
                  }
              ...
              // Fallback: Standard-Logik wenn Training-Daten-Analyse nicht verfügbar
              if (retryQuestionMarks > 2) {
                hasMultipleQuestionsRetry = true;
              } else if (retryQuestionMarks === 2) {
                const retryQuestionMatches = cleanedRetry.match(/[^.!?]+\?/g);
                if (retryQuestionMatches && retryQuestionMatches.length === 2) {
                  const rq1 = retryQuestionMatches[0].toLowerCase().trim();
                  const rq2 = retryQuestionMatches[1].toLowerCase().trim();
                  
                  const similarQuestionWords = ['was machst', 'was tust', 'wie geht', 'was machst du', 'was tust du', 'wie geht es'];
                  const rq1HasSimilar = similarQuestionWords.some(w => rq1.includes(w));
                  const rq2HasSimilar = similarQuestionWords.some(w => rq2.includes(w));
                  
                  if (rq1HasSimilar && rq2HasSimilar) {
                    hasMultipleQuestionsRetry = true;
                  }
                } else {
                  hasMultipleQuestionsRetry = true;
            ...
            if (stillForbidden.length === 0 && !hasMetaComments && !startsWithIchRetry && !hasMultipleQuestionsRetry) {
              ...
              if (hasMultipleQuestionsRetry) {
                console.error(`🚨 Retry ${retryAttempt}: Antwort enthält noch mehrere Fragen (${retryQuestionMarks} Fragen, nur 1 erlaubt!)`);
              }
...
Antworte NUR mit der erweiterten Version (mindestens ${minLength} Zeichen), keine Erklärungen.`;
      ...
      try {
        ...
        if (extendedText && extendedText.length >= minLength) {
          ...
          // 🚨 NEU: Kontext-bewusste Validierung - übergebe validierten Kontext
          if (isMeetingRequest(replyText, foundMessageText, validationContext)) {
            console.error("🚨🚨🚨 KRITISCH: Verlängerte Antwort enthält Treffen-Anfrage - versuche Antwort ohne Treffen zu generieren! 🚨🚨🚨");
            console.error("🚨 Blockierte Antwort:", replyText.substring(0, 200));
            // Versuche eine alternative Antwort zu generieren, die auf die Treffen-Anfrage reagiert, aber kein Treffen ausmacht
            errorMessage = "❌ FEHLER: Die KI hat versucht, ein Treffen auszumachen. Das ist nicht erlaubt. Bitte versuche es erneut.";
            return res.status(200).json({
              error: errorMessage,
              resText: "",
              replyText: "",
              ...
              chatId: finalChatId,
              actions: [],
              flags: { blocked: true, reason: "meeting_request", isError: true, showError: true, retry: true }
            });
          }
