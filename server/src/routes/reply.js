...
router.post("/", asyncHandler(async (req, res, next) => {
  ...
  if ((!foundMessageText || foundMessageText.trim() === "") && req.body?.siteInfos?.messages) {
    ...
    // Cherry: gesamte folgende Logik erwartet [neueste … älteste] (kleinerer Index = neuer), wie im Panel oben.
    // Nach dem Schritt oben liegt orderedMsgs aber fast immer als [älteste … neueste] vor → End-Timestamp erste < letzte → umdrehen.
    if (isCherryOrigin && orderedMsgs.length > 1) {
      try {
        const t0 = orderedMsgs[0]?.timestamp != null ? new Date(orderedMsgs[0].timestamp).getTime() : null;
        const tLast = orderedMsgs[orderedMsgs.length - 1]?.timestamp != null ? new Date(orderedMsgs[orderedMsgs.length - 1].timestamp).getTime() : null;
        if (t0 != null && tLast != null && !isNaN(t0) && !isNaN(tLast) && t0 < tLast) {
          orderedMsgs = [...orderedMsgs].reverse();
          console.log("✅ Cherry: Verlauf war [älteste…neueste] → intern auf [neueste…älteste] gedreht (End-Timestamps).");
        }
      } catch (e) { /* ignore */ }
    }
  ...
  function isWishlistSystemNoticeText(text) {
    ...
    return hasWishOrFav && hasAddVerb && (hasSystemLead || t.includes("zu seinen/ihren"));
  }
  const wishlistSystemCandidates = [
    foundMessageText,
    req.body?._customerMessageTextForDedup,
    req.body?.messageText,
    req.body?.lastMessage,
    req.body?.lastCustomerMessage
