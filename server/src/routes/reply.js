  ...
  if ((!foundMessageText || foundMessageText.trim() === "") && req.body?.siteInfos?.messages) {
    ...
    if (!foundMessageText || foundMessageText.trim() === "") {
      const lastAny = orderedMsgs.find(
        m => typeof m.text === "string" && m.text.trim() !== "" && m?.type !== "sent" && m?.messageType !== "sent"
      );
      if (lastAny) {
        foundMessageText = lastAny.text.trim();
        console.log("✅ Nachricht aus siteInfos.messages (any, nicht sent):", foundMessageText.substring(0, 100) + "...");
      }
    }
  }
...
- Beispiel-Nachrichten: ${writingStyle.sampleTexts ? writingStyle.sampleTexts.substring(0, 200) : ""}` : "";
