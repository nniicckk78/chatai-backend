...
router.post("/", asyncHandler(async (req, res, next) => {
  ...
  if ((!foundMessageText || foundMessageText.trim() === "") && req.body?.siteInfos?.messages) {
    ...
    if (isCherryOrigin && cherryNewestAtTop) {
      ...
      for (let i = 0; i < orderedMsgs.length; i++) {
        const m = orderedMsgs[i];
        if (!m || !isSentMessage(m)) continue;
        const v = (m.timestamp != null ? m.timestamp : m.time);
        if (!v) continue;
        const t = new Date(v).getTime();
        if (!isNaN(t) && t > 0 && (cherryLatestSentTs == null || t > cherryLatestSentTs)) cherryLatestSentTs = t;
      }
    } else {
      for (let i = 0; i < orderedMsgs.length; i++) {
        if (orderedMsgs[i] && isSentMessage(orderedMsgs[i])) lastSentIdx = i;
    ...
    const afterLastMod = validReceivedMessages.filter(m => {
      ...
      if (isCherryOrigin && cherryLatestSentTs != null) {
        const rv = m && m.message ? (m.message.timestamp != null ? m.message.timestamp : m.message.time) : null;
        const rt = rv ? new Date(rv).getTime() : NaN;
        if (!isNaN(rt) && rt > 0) return rt > cherryLatestSentTs;
      }
      if (isCherryOrigin && cherryNewestAtTop) return m.index < lastSentIdx;
