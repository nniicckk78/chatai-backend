...
- Beispiel-Nachrichten: ${writingStyle.sampleTexts ? writingStyle.sampleTexts.substring(0, 300) : ""}` : "";
    ...
    // Nutze Situations-Regeln aus Config
    if (isBotAccusation && rules.situationalResponses?.botAccusation?.enabled) {
    ...
    if (isSexualTopic && rules.situationalResponses?.sexualTopics?.enabled) {
...
- Keine Flugzeugbegleiter, Zugkontrolleur oder ähnliche Berufe (zu spezifisch, könnte zu Treffen führen)`;
    ...
    if (isMeetingRequest && rules.situationalResponses?.meetingRequest?.enabled) {
...
- Passe deinen Schreibstil entsprechend an (Frau zu Mann oder Mann zu Frau)`;
    ...
    if (touchesMoney && rules.situationalResponses?.moneyTopics?.enabled) {
      const moneyRule = rules.situationalResponses.moneyTopics;
      specificInstructions += `\n\n💸 GELD/COINS:\n${moneyRule.instructions}`;
    }
