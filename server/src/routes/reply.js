...
- Passe deinen Schreibstil entsprechend an (Frau zu Mann oder Mann zu Frau)`;
    ...
    if (moderatorProfilePicInfo && moderatorProfilePicInfo.personCount === 2) {
      profilePicContext += `\n\n📸 PROFILBILD-INFO (FAKE/MODERATOR):
- Dein Profilbild zeigt ZWEI Personen (z.B. zwei Frauen)
- Du kannst in deinen Antworten darauf Bezug nehmen (z.B. "wir" statt "ich")
...
${rules && rules.generalRules && rules.generalRules.trim() ? `\n⚠️⚠️⚠️ KRITISCH: Die oben genannten ALLGEMEINEN REGELN haben HÖCHSTE PRIORITÄT! Befolge sie STRENG! Sie überschreiben andere Anweisungen! ⚠️⚠️⚠️` : ''}`;
    ...
    if (rules && rules.forbiddenWords && Array.isArray(rules.forbiddenWords) && rules.forbiddenWords.length > 0) {
      ...
      if (foundForbiddenWords.length > 0) {
        ...
        try {
          const retryPrompt = `Die vorherige Antwort enthielt VERBOTENE WÖRTER: ${foundForbiddenWords.map(w => `"${w}"`).join(', ')}.
...
2. Natürlich und passend klingt
3. Mindestens 80 Zeichen lang ist
4. Mit einer Frage endet

VERBOTENE WÖRTER (ABSOLUT NICHT VERWENDEN): ${rules.forbiddenWords.map(w => `"${w}"`).join(', ')}
