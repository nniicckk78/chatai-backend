...
// Schneller Pfad: keine Neu-Generierung/Verlängerung/Nachfrage-Anhängung
const FAST_REPLY = true;
      ...
      for (const filePath of possiblePaths) {
        try {
          const response = await Promise.race([
            ...
            new Promise((_, reject) => setTimeout(() => reject(new Error("GitHub API timeout")), 10000))
          ]);
          if (response.data && response.data.content) {
            const content = Buffer.from(response.data.content, 'base64').toString('utf8');
            console.log(`✅ Training Data geladen: ${JSON.parse(content).conversations?.length || 0} Gespräche, ${JSON.parse(content).asaExamples?.length || 0} generische ASAs`);
...
Nachricht: ${messageText}`;
    ...
    console.error("Fehler beim Extrahieren von Informationen:", err.message);
    // #region agent log
...
Antworte NUR mit der vollständigen, erweiterten Reaktivierungsnachricht (mindestens 150 Zeichen), keine Erklärungen. Die Nachricht soll fragen, warum der Kunde nicht mehr schreibt!`;
      ...
      // Stabilisiere chatId: verwende nur stabile Quellen, vermeide wechselnde Zeitstempel aus metaData
      const asaChatId =
        chatId ||
        req.body?.siteInfos?.chatId ||
        (typeof finalChatId === "string" && !finalChatId.includes("GMT") ? finalChatId : undefined);
      ...
      return res.json({
        ...
        summary: {},
        ...(asaChatId ? { chatId: asaChatId } : {}), // Nur setzen, wenn stabil vorhanden
        actions: [
          {
  ...
  // WICHTIG: extractInfoFromMessage ist optional - überspringe es, wenn client nicht verfügbar ist
...
🚨🚨🚨 KONVERSATIONS-INTENTION FÜR DIE KI (HÖCHSTE PRIORITÄT) 🚨🚨🚨

Du bist auf einer Dating-Plattform.
Dein Ziel ist NICHT Informationssammlung, sondern emotionale Nähe, Anziehung und Dynamik.

Regeln:
- Stelle KEINE offensichtlichen Folgefragen auf die letzte Antwort.
- Jede Frage muss ENTWEDER:
  • Spannung aufbauen
  • eine Entscheidung provozieren
  • Nähe oder Fantasie auslösen
- Wenn der Nutzer kurz antwortet, darfst du:
  • interpretieren
  • zuspitzen
  • eine Richtung vorgeben
- Wiederhole NIEMALS dieselbe Entscheidungsfrage in anderer Form.
- Wenn ein Thema „ausgereizt" ist, springe weich weiter, statt nachzubohren.
- Handle wie eine Person, die den anderen kennenlernen UND begehrenswert wirken will.

---

🚨🚨🚨 FRAGEN-REGELN (HÖCHSTE PRIORITÄT) 🚨🚨🚨

Du bist auf einer Dating-Plattform.

WICHTIG:
- In JEDER Nachricht MUSS genau EINE Frage enthalten sein.

Frage-Regeln:
- Stelle KEINE Analyse-, Erklär- oder Vorgehensfragen.
- Vermeide Fragen wie:
  "wie genau", "warum genau", "wie würdest du", "was machst du dabei".

Erlaubt sind NUR diese Frage-Typen:
1. Persönliche Vorliebe („Magst du eher…?")
2. Stimmung / Gefühl („Fühlt sich das für dich eher… an?")
3. Auswahl-Fragen („eher A oder B?")
4. Leichte Fantasie („Was wäre dir lieber…?")
5. Anschluss-Fragen, die das Thema erweitern (nicht vertiefen).

Weitere Regeln:
- Erzähle IMMER auch etwas von dir selbst.
- Die Frage darf maximal EINEN Satz haben.
- Die Frage soll natürlich klingen, wie im echten Chat.
- Bleibe thematisch nahe, aber erweitere das Thema sanft.
...
📝 ERINNERUNG: Die GRUNDREGELN (im System-Prompt) sind die BASIS für alle Antworten. Training-Daten sind deine HAUPTREFERENZ für Stil, Wortwahl und Ton.`;
    ...
    if (!FAST_REPLY && (foundForbiddenWords.length > 0 || foundMetaComments.length > 0 || foundFormalPhrases.length > 0 || hasRepetitiveQuestion || hasBlockingResponse || foundRepetitions.length > 0)) {
...
Antworte NUR mit der neuen Antwort, keine Erklärungen.`;
    ...
    if (!FAST_REPLY && replyText.length < 80) {
