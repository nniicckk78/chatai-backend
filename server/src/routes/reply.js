...
const { getFineTuningConfig, buildSystemPrompt } = require("../utils/fine-tuning");
...
function findNearbyCity(customerCity) {
  ...
  // 🚨 NEU: Suche zuerst mit Original-Name, dann mit Name ohne Präfix
  ...
  for (const searchTerm of searchTerms) {
    for (const [key, cities] of Object.entries(nearbyCities)) {
      ...
      if (searchTerm === key || searchTerm.includes(key) || key.includes(searchTerm)) {
        ...
        console.log(`✅ Stadt gefunden: "${selectedCity}" (für "${customerCity}", Suchbegriff: "${searchTerm}")`);
        return selectedCity;
      }
    }
  }
  ...
  console.warn(`⚠️ Keine nahegelegene Stadt gefunden für: "${customerCity}" (auch nicht ohne Präfix: "${cityWithoutPrefix}")`);
  return null;
}
...
router.post("/", asyncHandler(async (req, res, next) => {
  ...
  // 🤖 SCHRITT 11: INTELLIGENTE GENERIERUNG - Fine-Tuned Model + Smart Example Selection
  const isASACalculated = isLastMessageFromFake || false;
  
  // Lade Fine-Tuning-Config
  let fineTuningConfig = null;
  let baseSystemPrompt = null;
  try {
    fineTuningConfig = await getFineTuningConfig();
    baseSystemPrompt = await buildSystemPrompt();
    console.log(`✅ Fine-Tuning-Config geladen: Model ${fineTuningConfig.modelId || 'NICHT TRAINIERT'}`);
  } catch (err) {
    console.error('⚠️ Fehler beim Laden der Fine-Tuning-Config:', err.message);
  }
  
  // Bestimme zu verwendendes Model
  const modelToUse = fineTuningConfig?.modelId || AI_MODEL;
  console.log(`🤖 Verwende Model: ${modelToUse}`);
  ...
  // 🚨 NEU: Extrahiere Situation (für intelligente Beispiel-Auswahl)
  ...
  // 🚨 NEU: Intelligente Beispiel-Auswahl (nutzt Embeddings + Feedback-Score)
  let selectedExamples = [];
  let examplesPrompt = '';
  try {
    if (isASACalculated && trainingData && trainingData.asaExamples && trainingData.asaExamples.length > 0) {
      // 🚨 ASA-FALL: Verwende ASA-Beispiele aus trainingData.asaExamples
      console.log(`📋 ASA-Fall erkannt - lade ${trainingData.asaExamples.length} ASA-Beispiele...`);
      
      // Wähle 3-5 zufällige ASA-Beispiele für Variation
      const asaExamples = trainingData.asaExamples.filter(ex => ex.asaMessage && ex.asaMessage.trim().length >= 120);
      const shuffled = [...asaExamples].sort(() => Math.random() - 0.5); // Zufällig mischen
      const selectedASAs = shuffled.slice(0, Math.min(5, shuffled.length));
      
      selectedExamples = selectedASAs.map((ex, idx) => ({
        customerMessage: "ASA Reaktivierung", // Einheitliche Kundennachricht für ASA
        moderatorResponse: ex.asaMessage,
        situation: "ASA Reaktivierung",
        source: ex.source || 'asa-example',
        similarity: 1.0, // ASA-Beispiele haben immer maximale Relevanz
        feedbackScore: 0.5, // Neutraler Score (könnte aus Learning-Stats kommen)
        combinedScore: 1.0
      }));
      
      if (selectedExamples.length > 0) {
        examplesPrompt = formatExamplesForPrompt(selectedExamples);
        console.log(`✅ ${selectedExamples.length} ASA-Beispiele ausgewählt für Prompt`);
      }
    } else if (customerMessage && customerMessage.trim() !== "" && trainingData) {
      // Normale Fälle: Verwende intelligente Beispiel-Auswahl
      selectedExamples = await selectSmartExamples(customerMessage, {
        topK: 12, // 🚨 ERHÖHT: 12 beste Beispiele (statt 5) für bessere Variation
        situation: detectedSituation !== 'allgemein' ? detectedSituation : null,
        conversationHistory: conversationContextForPipeline,
        includeSexual: true // Sexuelle Beispiele für Prompt verwenden
      });
      
      if (selectedExamples.length > 0) {
        examplesPrompt = formatExamplesForPrompt(selectedExamples);
        console.log(`✅ ${selectedExamples.length} intelligente Beispiele ausgewählt für Prompt`);
      }
    }
  } catch (err) {
    console.warn('⚠️ Fehler bei intelligenter Beispiel-Auswahl:', err.message);
  }
  
  // 🚨 NEU: Lade Learning-Context (kompakte Regeln aus Feedback)
  let learningContext = '';
  try {
    const learningStats = await getLearningStats();
    if (learningStats && Object.keys(learningStats).length > 0) {
      learningContext = await generateCompactLearningContext(customerMessage, detectedSituation, learningStats);
      if (learningContext) {
        console.log(`✅ Learning-Context geladen (${learningContext.length} Zeichen)`);
      }
    }
  } catch (err) {
    console.warn('⚠️ Fehler beim Laden des Learning-Context:', err.message);
  }
  
  // 🚨 NEU: Erweitere System-Prompt mit Learning-Context + ausgewählten Beispielen
  const systemPrompt = (baseSystemPrompt || "Du bist ein Chat-Moderator auf einer Dating-Plattform.") + learningContext + examplesPrompt;
  
  // Erstelle User-Prompt
  let userContent = '';
  if (isASACalculated) {
    userContent = `Situation: ASA Reaktivierung\n`;
    if (asaSystemMessage) {
      userContent += `System-Nachricht: "${asaSystemMessage}"\n\n`;
    }
    userContent += `Antworte als Chat-Moderator.`;
  } else {
    if (detectedSituation !== 'allgemein') {
      userContent += `Situation: ${detectedSituation}\n`;
    }
    userContent += `Kunde: "${customerMessage}"\n\n`;
    userContent += `Antworte als Chat-Moderator.`;
  }
  ...
  // Generiere Nachricht mit Fine-Tuned Model + intelligenten Beispielen
  ...
  try {
    const response = await client.chat.completions.create({
      ...
      temperature: isASACalculated ? 0.9 : 0.8, // 🚨 ERHÖHT: Mehr Variation (0.8 normal, 0.9 für ASA)
      max_tokens: 500
    });
    
    generatedMessage = response.choices?.[0]?.message?.content?.trim() || "";
    console.log(`✅ Nachricht generiert (${generatedMessage.length} Zeichen)`);
  } catch (err) {
    console.error('❌ Fehler bei Nachrichtengenerierung:', err.message);
    generatedMessage = "";
  }
