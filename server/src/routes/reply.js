...
Antworte NUR mit gültigem JSON, kein zusätzlicher Text. Format:
{
  ...
  "Health": "Gesundheit falls erwähnt, sonst null",
  "Other": "NUR Sonstige Infos, die die Fake-Person über SICH SELBST sagt (Termine wie 'morgen Zahnarzt', 'heute was mit Freundin', eigene Vorlieben). NICHT eintragen: was der KUNDE über sich sagt (z.B. sein Wohnmobil, seine Reise, 'ich bin bis 20.02 unterwegs') – das gehört nur ins Kunden-Logbuch, sonst null"
}

WICHTIG:
- Nur Infos über den ASSISTENTEN/die Fake-Person extrahieren, nicht über den Kunden.
- \"Name\": Immer NUR Vorname/Kosename (z.B. Alana). Kein Benutzername, keine Plattform-ID, kein Alter/Status – der Kunde darf das nicht sehen.
- \"Other\": NUR was die Fake-Person über SICH mitteilt. Wenn die Nachricht nur ECHO/Zitat von dem ist, was der Kunde gesagt hat (z.B. "du bist mit deinem Wohnmobil am Atlantik?"), ist das Kunden-Info – unter assistant.Other NICHT eintragen.
- Wohnort/Stadt NUR unter \"Stadt\" und \"Wohnort\" eintragen, NICHT unter \"Other\".
- \"Sport and Hobbies\": NUR echte Sportarten/Hobbys. KEINE Essensvorlieben (z.B. \"mag es würzig\", \"scharf\", \"Knoblauch\") – weglassen oder unter Other, nie unter Sport and Hobbies.
- Auch kurze Hinweise extrahieren: "hab morgen Zahnarzt", "bin heute mit Freundin unterwegs", "wohne in Berlin", "arbeite als Lehrerin".
- Wenn nichts Relevantes über die Person steht, gib leeres Objekt {} oder alle null zurück.
...
Nachricht des Assistenten:\n${text.substring(0, 3000)}`;
    ...
    const out = {};
    const keys = ["Name", "Stadt", "Wohnort", "Work", "Beruf", "Sport and Hobbies", "Beziehungsstatus", "Family", "Health", "Other"];
...
/** True, wenn der String wie ein Plattform-Benutzername wirkt (z.B. "SchwarzeDom"), nicht wie ein Vorname/Anzeigename. */
...
function buildSummaryFromMeta(metaData) {
  ...
  // Fake/Moderator – Name nur Anzeigename (z.B. Alana), nie Benutzername
  if (moderator.name) summary.assistant["Name"] = toFakeDisplayNameOnly(moderator.name);
  ...
  try {
    const res = await client.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: 'Du bist ein Helfer für Städte in Deutschland, Österreich und der Schweiz. Gegeben der Wohnort oder Stadtteil eines Kunden, nenne genau EINE Stadt in DACH, die maximal ca. 20–30 km entfernt liegt (für ein plausibles Dating-Profil). Antworte NUR mit dem Städtenamen, kein Satz, keine Anführungszeichen, keine Erklärung. Beispiel: Bei "Hamburg" oder "Niendorf" → z.B. Lübeck oder Kiel.'
        },
...
async function findNearbyCity(customerCity, opts = {}) {
  ...
  // PRIORITÄT 1: OpenAI/LLM zuerst (wie Nutzer es mit ChatGPT machen – versteht Stadtteile, Schreibweisen, Kontext)
  if (opts?.client && (opts?.model || AI_MODEL)) {
    console.log(`🔍 Suche nahegelegene Stadt für "${customerCity}" via OpenAI (zuerst)...`);
    const llmCity = await findNearbyCityViaLLM(opts.client, opts.model || AI_MODEL, customerCity);
    if (llmCity) return llmCity;
  }

  // PRIORITÄT 2: OpenStreetMap API (kostenlos, keine Tokens)
  console.log(`🔍 OpenAI keine Stadt – versuche OpenStreetMap für "${customerCity}"${city !== customerCity.trim().toLowerCase() ? ` (normalisiert: ${cityForApi})` : ''}...`);
  try {
    ...
    console.warn(`⚠️ OpenStreetMap Fehler: ${apiErr.message} – statische Liste...`);
  }

  // PRIORITÄT 3: Statische Liste (schnell, kein Netz)
  ...
  for (const searchTerm of searchTerms) {
    for (const [key, cities] of Object.entries(nearbyCities)) {
      if (searchTerm === key || searchTerm.includes(key) || key.includes(searchTerm)) {
        ...
        console.log(`✅ Stadt in statischer Liste: "${selectedCity}" (für "${customerCity}")`);
        return selectedCity;
      }
    }
  }

  console.warn(`⚠️ Keine nahegelegene Stadt gefunden für: "${customerCity}"`);
  return null;
}
...
async function findNearbyCityViaAPI(customerCity) {
  ...
  try {
    ...
    // Präzision: ", Deutschland" für eindeutige Treffer (z. B. Stendal → Sachsen-Anhalt, nicht Berlin/anderes)
    ...
    if (!geocodeData || geocodeData.length === 0) {
      // Retry ohne ", Deutschland" (z. B. für AT/CH oder andere Schreibweisen)
    ...
    // Berechne 4-5 nahegelegene Punkte (in verschiedene Richtungen, ~20km entfernt; 1° ≈ 111km → 0.18° ≈ 20km)
    const nearbyPoints = [];
    const directions = [
      { lat: 0.18, lon: 0.0 },   // Norden
      { lat: -0.18, lon: 0.0 },  // Süden
    ...
    // Stadtsuche: Nur Städte aus der gleichen Region zulassen (verhindert z. B. Berlin bei Kunde aus Stendal)
    ...
    if (toPick.length > 0) {
      ...
      if (nearbyInRegion.length === 0) {
        console.log(`⚠️ OpenStreetMap: Gefundene Stadt "${selectedCity}" außerhalb Region – nutze Fallback.`);
        return findNearbyCityByCountry(customerCity, customerCountry, customerLat, customerLon);
      }
      console.log(`✅ OpenStreetMap: Nahegelegene Stadt gefunden: "${selectedCity}" (für "${customerCity}")`);
...
function findNearbyCityByCountry(customerCity, country, lat, lon) {
  // Bekannte Städte in verschiedenen Regionen (als Fallback wenn API zu langsam ist)
  const regionCities = {
    'DE': {
      'nord': ['Hamburg', 'Bremen', 'Hannover', 'Kiel', 'Lübeck'],
      'nordost': ['Magdeburg', 'Halle', 'Potsdam', 'Stendal', 'Brandenburg'], // Sachsen-Anhalt, Brandenburg (z. B. Stendal)
  ...
  if (cities.length > 0) {
    // Filtere die ursprüngliche Stadt raus
...
function isInfoMessage(msg) {
  ...
  // "Der Benutzer möchte dich als Freund hinzufügen" ist eine System-Nachricht der Plattform
  // WICHTIG: Diese Meldungen kommen von der PLATTFORM, nicht vom Kunden!
...
// options.origin === 'iluvo': Nur die NEUESTE Nachricht (letzte im Array) prüfen – Array = [älteste … neueste].
function hasRecentReceivedImageInMessages(messages, options) {
  ...
  // Iluvo: nur letzte received = neueste Nachricht prüfen (kein .some() über alte Bilder)
  const toCheck = isIluvo ? [received[received.length - 1]] : received;
  for (const m of toCheck) {
    if (!m) continue;
    if (!isIluvo && m.timestamp) {
      try {
...
function compressConversation(messages, limit = 30, options = {}) {
  ...
  // Iluvo: Extension sendet seit Fix die letzten 20 Nachrichten (slice(-20)) in Reihenfolge [älteste … neueste]. Kein Reverse nötig.
...
- Bei Like/Kuss oder Erstnachricht: In der Nachricht die du JETZT schreibst duerfen NIEMALS dein Name, dein Alter oder dein Wohnort vorkommen. Nur Danke (bei Like/Kuss) + 1-2 gespraechsoeffnende Fragen; bei reiner Erstnachricht nur 1-2 gespraechsoeffnende Fragen. Keine Selbstvorstellung!
...
Antworte NATÜRLICH und AUTHENTISCH - nicht wie eine KI!`;
    ...
    // Bei Like/Kuss: Abwechslung bei den Folgefragen (nicht immer Winter/Getraenk etc.)
    const likeKussVarietyThemes = [
      'Wie geht es dir? / Was machst du gerade?',
      'Bist du noch auf der Arbeit? / Schon Feierabend?',
      'Was hast du heute Abend vor? / Wie laeuft dein Tag?',
      'Was hast du am Wochenende vor? (wenn Freitag/Samstag/Sonntag)',
      'Wie laeuft dein Tag bisher? / Was steht bei dir so an?',
      'Hast du heute was Schoenes vor? / Entspannst du dich gerade?'
    ];
    ...
    const likeKussVarietyHint = (hasLike || hasKuss)
      ? `\n[ABWECHSLUNG – WICHTIG] Stelle 1-2 gespraechsoeffnende Fragen. Nutze DIESMAL z.B. eine dieser Richtungen: ${varietyPick}. Wechsle ab – nicht immer Winter, Getraenk, Lieblingsgetraenk oder dieselbe Formulierung. Moeglich: Wie geht es dir? Was machst du gerade? Bist du noch auf der Arbeit? Was hast du heute Abend vor? Was hast du am Wochenende vor? Wie laeuft dein Tag? Schon Feierabend?\n`
      : '';
...
- VERBOTEN: Dich vorstellen (kein "Ich bin X", kein Alter, kein Wohnort in der Nachricht). Bei Like/Kuss oder Erstnachricht NUR Danke (wenn Like/Kuss) + 1-2 gespraechsoeffnende Fragen, sonst nur 1-2 Fragen.
...
router.post("/", asyncHandler(async (req, res, next) => {
  ...
  if ((!foundMessageText || foundMessageText.trim() === "") && req.body?.siteInfos?.messages) {
    ...
    // AVZ + Iluvo: wie FPC – System-/Info-Nachrichten rausfiltern
    ...
    if (isAVZOrigin || isIluvo) {
      msgs = msgs.filter(m => m && !isInfoMessage(m));
      if (msgs.length === 0) msgs = req.body.siteInfos.messages;
    }
    // Einheitlich [älteste … neueste]: Iluvo sendet schon so; FPC/AVZ ggf. per Zeitstempel umdrehen (wie bisher)
    let newestFirst = false;
    if (!isIluvo && msgs.length > 1) {
      try {
    ...
    const orderedMsgs = isIluvo ? msgs : (newestFirst ? [...msgs].reverse() : msgs);
    ...
    // Iluvo: wie AVZ/FPC – neueste Kunden-Nachricht = letzte received in orderedMsgs (Reihenfolge ist nur bei Iluvo umgekehrt geliefert, hier schon korrekt)
    if (isIluvo && orderedMsgs.length > 0) {
      ...
      if (lastReceived) {
        ...
        if (hasImg && (!txt.length || onlySystemImage(txt.toLowerCase()))) {
          ...
          console.log("✅ Iluvo: Neueste Nachricht (letzte im Verlauf) = nur Bild.");
        } else if (txt.length > 0) {
          foundMessageText = txt;
          console.log("✅ Iluvo: Neueste Kunden-Nachricht (letzte im Verlauf, wie AVZ/FPC):", foundMessageText.substring(0, 80) + "...");
        }
      }
    }
    ...
    // 🚨 NEU: Wenn neueste received-Nachricht NUR ein Bild ist (kein Kundentext), darauf antworten. Gilt für: leeren Text, oder nur "Ein Bild wurde übertragen". Bild + eigener Text → Text behalten!
  ...
  // 🚨 WICHTIG: isFPC / isAVZ müssen FRÜH definiert werden, damit sie im gesamten Router-Handler verfügbar sind
  ...
  if (!isLastMessageFromFake && (!foundMessageText || foundMessageText.trim() === "") && msgsAll.length) {
    ...
    // 🚨 FIX: Kuss/Like nur als ASA, wenn der Fake BEREITS im Chat geschrieben hat. Leerer Chat (nur System-Nachricht) = Erstnachricht!
    if (hasKussSystemMessage || hasLikeSystemMessage) {
      if (sentMsgs.length > 0) {
        ...
        console.log(`✅ ASA erkannt: Kuss/Like-Systemnachricht + Fake hat bereits geschrieben – schreibe Antwort auf Kuss/Like (OpenAI ASA-Pfad).`);
      } else {
        // Chat komplett leer, nur Like/Kuss-Systemnachricht → Erstnachricht: Danke + erste Frage (KEIN ASA!)
        isLastMessageFromFake = false;
        asaFromKussOrLike = false;
        console.log(`✅ Erstnachricht erkannt: Nur Like/Kuss-Systemnachricht, kein Fake-Text im Chat – nutze First-Message-Pfad (Danke + erste Frage).`);
      }
  ...
  if (!isLastMessageFromFake && ((!foundMessageText || foundMessageText.trim() === "") || isIluvoOrigin) && msgsAll.length) {
    ...
    if (sentMsgs.length === 0) {
      ...
      // Iluvo: Extension sendet [älteste … neueste] – neueste Nachricht ist immer list[list.length - 1], Zeitstempel ignorieren
      newestMsg = (isIluvoOrigin && list.length > 0) ? list[list.length - 1] : (newestFirst ? list[0] : list[list.length - 1]);
      if (newestMsg && (newestMsg.type === "received" || newestMsg.messageType === "received") && !isInfoMessage(newestMsg)) {
        isLastMessageFromFake = false;
        console.log(isIluvoOrigin ? "✅ Iluvo: Letzte Nachricht (neueste) ist vom Kunden – kein ASA." : "✅ Erste/neueste Nachricht ist vom Kunden – kein ASA.");
      } else {
      ...
      // Iluvo: Array = [älteste … neueste], also letzte = neueste
      const newestReceived = receivedMsgs.length > 0 ? (isIluvoOrigin ? receivedMsgs[receivedMsgs.length - 1] : (newestFirst ? receivedMsgs[0] : receivedMsgs[receivedMsgs.length - 1])) : null;
  ...
  if (list.length > 0 && !noASABecauseReceivedNewer) {
    // Iluvo: list = [älteste … neueste], also reverse für neueste zuerst
    const ordered = isIluvoOrigin ? [...list].reverse() : (newestFirst ? list : [...list].reverse());
  ...
  if (list.length > 0) {
    // Iluvo: Erkennung über siteInfos.origin ODER platformId (Extension sendet oft nur origin)
    ...
    if (isIluvo) {
      ...
      if (noASABecauseReceivedNewer) {
        ...
        // Bei iluvo: Wenn die letzte Nachricht "sent" ist, ist es ASA (wie FPC/AVZ)
        if (isSentMessage(newestMsg)) {
          isLastMessageFromFake = true;
          foundMessageText = "";
          console.log("✅ ASA erkannt für Iluvo: neueste Nachricht ist sent (wie FPC/AVZ).");
        }
      }
    }
    ...
    if ((isFPC || isAVZ) && !isLastMessageFromFake) {
  ...
  if (hasBild && (originForImage !== "iluvo" || isOnlySystemOrBild)) {
    isLastMessageFromFake = false;
    foundMessageText = "Der Kunde hat ein Bild geschickt.";
    console.log("✅ Kunde hat Bild geschickt – antworte auf Bild (kein ASA), auch wenn Kuss/Like im Verlauf war.");
  }

  // Iluvo: Plattform meldet ASA (z. B. Banner "Der Dialog ist eine ASA Stufe 1") → immer ASA, wie FPC/AVZ (Extension sendet oft nur siteInfos.origin, kein platformId)
  if (originForImage === "iluvo" && (req.body?.siteInfos || {})) {
    const siteInfosStr = JSON.stringify(req.body.siteInfos).toLowerCase();
    if (siteInfosStr.includes("asa stufe") || siteInfosStr.includes("asa-stufe") || siteInfosStr.includes("der dialog ist eine asa")) {
      isLastMessageFromFake = true;
      foundMessageText = "";
      console.log("✅ ASA erkannt für Iluvo: Plattform meldet ASA (z. B. 'ASA Stufe' in siteInfos/html) – nutze ASA-Pipeline.");
    }
  }
  ...
  // Iluvo: Kein Fallback – nur antworten, wenn neueste Kunden-Nachricht vorhanden; sonst gar keine Antwort.
  const isIluvoReply = (req.body?.siteInfos?.origin || "").toLowerCase() === "iluvo";
  if (isIluvoReply && (!foundMessageText || foundMessageText.trim() === "") && !isLastMessageFromFake) {
    console.log("⚠️ Iluvo: Keine neueste Kunden-Nachricht im Request – keine Antwort (Extension muss neueste Nachrichten senden).");
    return res.status(200).json({
      resText: "",
      replyText: "",
      summary: {},
  ...
  try {
    ...
    if (dataUrl) {
        ...
        try {
          const typeAnalysis = await client.chat.completions.create({
            ...
            messages: [
              {
                role: "system",
                content: "Du analysierst Bilder und kategorisierst sie. Antworte NUR als JSON im Format: {\"type\": \"penis\" | \"dildo\" | \"nude\" | \"face\" | \"body\" | \"other\", \"confidence\": 0.0-1.0, \"description\": \"Kurze Beschreibung\"}"
              },
              {
                role: "user",
                content: [
                  { type: "text", text: "Analysiere dieses Bild und kategorisiere es:\n- \"penis\": Wenn männliche Genitalien (Schwanz/Penis) sichtbar sind\n- \"dildo\": Wenn ein Dildo oder Sexspielzeug sichtbar ist (auch wenn es in einem Körper steckt, z.B. im Arsch, in der Muschi, etc.)\n- \"nude\": Wenn nackter Körper (aber keine Genitalien oder Sexspielzeug) sichtbar ist\n- \"face\": Wenn nur Gesicht/Portrait sichtbar ist\n- \"body\": Wenn bekleideter Körper sichtbar ist\n- \"other\": Alles andere (Urlaub, Haustier, etc.)\n\nWICHTIG: Wenn ein Dildo oder Sexspielzeug sichtbar ist (auch wenn es in einem Körper steckt), kategorisiere es als \"dildo\"!\n\nAntworte NUR als JSON, kein zusätzlicher Text." },
        ...
        let desc = null;
        // Bei penis/dildo/nude: Keinen zweiten Vision-Call – API lehnt oft ab. Direkt festen positiven Kontext nutzen (Bildtyp wurde bereits erkannt).
        if (imageType === 'penis' || imageType === 'dildo' || imageType === 'nude') {
          desc = positiveContextByType[imageType] || 'Der Kunde hat ein Bild von sich geschickt. Reagiere flirty und positiv – lehne nie ab.';
          console.log(`✅ Bild erkannt und Kontext gesetzt (Typ: ${imageType}) – kein zweiter Vision-Call (API würde oft ablehnen).`);
        } else {
          const vision = await client.chat.completions.create({
            model: AI_MODEL,
            messages: [
        ...
        if (desc) {
          ...
          imageDescriptions.push("Der Kunde hat mit dieser Nachricht ein Bild geschickt (Inhalt konnte nicht analysiert werden, z.B. Datei zu groß oder Abruf fehlgeschlagen). Reagiere positiv und flirty darauf: zeige Begeisterung, sag z.B. wie geil du das findest, was du damit machen würdest oder frag wo er es hinstecken möchte. Keine Ablehnung, kein 'ich sende keine Fotos' – nur positive, flirty Reaktion; frage NICHT erneut nach einem Foto.");
      ...
      // Bild-URL bekannt oder Text deutet auf Bild hin, aber Abruf fehlgeschlagen (z.B. 403) – Fallback
      imageDescriptions.push("Der Kunde hat mit dieser Nachricht ein Bild geschickt (Inhalt konnte nicht analysiert werden, z.B. Datei zu groß oder Abruf fehlgeschlagen). Reagiere positiv und flirty darauf: zeige Begeisterung, sag z.B. wie geil du das findest, was du damit machen würdest oder frag wo er es hinstecken möchte. Keine Ablehnung, kein 'ich sende keine Fotos' – nur positive, flirty Reaktion; frage NICHT erneut nach einem Foto.");
      console.log("📸 Bild zu groß/fehlgeschlagen – Fallback-Beschreibung für Pipeline gesetzt.");
    }
  } catch (err) {
    ...
    if (isLastMessageFromFake) {
      ...
      const relevantSystemMessages = messages
        .filter(m => {
          ...
          const text = m.text.toLowerCase();
          // Prüfe auf Kuss-System-Nachrichten
      ...
      // Normale Fälle: Nutze compressConversation (filtert alte Nachrichten). Iluvo: Reihenfolge normalisieren (neueste zuerst → älteste zuerst)
      const origin = (req.body?.siteInfos?.origin || "").toLowerCase();
      conversationContextForPipeline = compressConversation(req.body?.siteInfos?.messages || [], 50, { origin });
  ...
  // 🚨 NEU: SCHRITT 10.5: Freundschaftsanfrage-Behandlung
  // Prüfe, ob die letzte Nachricht eine Freundschaftsanfrage ist
  ...
  const isFriendRequest = lastMessageForFriendRequest && isInfoMessage(lastMessageForFriendRequest) && (
    (lastMessageForFriendRequest.text || "").toLowerCase().includes("freund") && 
    ((lastMessageForFriendRequest.text || "").toLowerCase().includes("hinzufügen") || 
     (lastMessageForFriendRequest.text || "").toLowerCase().includes("freundschaftsanfrage"))
  );
  
  if (isFriendRequest) {
    console.log("✅ Freundschaftsanfrage erkannt - generiere Standardantwort");
    // Profil-Info aus metaData (für Kundennamen)
    ...
    const friendRequestMessage = customerName 
  ...
  if (isFirstMessage && !isAVZ) {
    ...
    try {
      // 🚨 Like/Kuss hat VORRANG: Bei Erstnachricht + Like/Kuss-Systemnachricht immer "Danke für Like/Kuss" + Fragen (nicht Grok/Erstnachricht-Stil)
      if (hasLikeSystemMessage || hasKussSystemMessage) {
        ...
        const firstMessageInstructions = isKuss
          ? "Der Kunde hat dir einen KUSS geschickt (Kuss-Systemnachricht). NUR: 1) Kurz mit Danke fuer den Kuss bedanken (NICHT fuer den Like!). 2) 1-2 gespraechsoeffnende Fragen stellen. VERBOTEN: Dich vorstellen – weder Name noch Alter noch Wohnort nennen (kommt spaeter)."
          : isLike
            ? "Der Kunde hat dich GELIKED (Like-Systemnachricht). NUR: 1) Kurz mit Danke fuer den Like bedanken (NICHT fuer den Kuss!). 2) 1-2 gespraechsoeffnende Fragen stellen. VERBOTEN: Dich vorstellen – weder Name noch Alter noch Wohnort nennen (kommt spaeter)."
            : "Der Kunde hat dir einen Kuss oder Like geschickt (System-Nachricht). NUR: Danke + 1-2 gespraechsoeffnende Fragen. VERBOTEN: Dich vorstellen (kein Name, kein Alter, kein Wohnort).";
  ...
  // Like/Kuss OHNE Kundentext (Kunde hat nur geliked/geküsst, keine Nachricht): Immer "Danke + Gesprächsstarter", nie ASA
  // Trifft zu wenn wir z.B. ein Bild geschickt haben und der Kunde nur mit Like reagiert – dann kein ASA, sondern Danke + Fragen
  if (!isAVZ && !isFirstMessage && (hasLikeSystemMessage || hasKussSystemMessage) && (!foundMessageText || foundMessageText.trim() === "")) {
    console.log("✅ Like/Kuss ohne Kundentext – generiere Danke + Gesprächsstarter (kein ASA).");
    try {
      ...
      console.error('❌ Like/Kuss-Antwort fehlgeschlagen:', err.message);
    }
  }
  ...
  try {
    ...
    // FPC: ASA-Fall nur wegen Like/Kuss-Systemnachricht → Danke + gesprächsöffnende Fragen statt Reaktivierungs-ASA
    if (isASACalculated && asaFromKussOrLike && isFPC) {
      ...
      // 🚀 Grok-Pipeline: normale Reply + Stadtauswahl (ASA/Erstnachricht laufen über anderen Pfad)
      ...
      multiAgentResults = await runGrokPipeline({
        ...
        findNearbyCityFunc: (city) => findNearbyCity(city, { client, model: getMessageModel() || AI_MODEL }),
      ...
      // FPC/AVZ Logbuch: Wohnort nur übernehmen, wenn der Kunde tatsächlich nach dem Wohnort gefragt hat (Wohnort-Frage)
      const isWohnortFrage = detectedSituations && detectedSituations.some(s => s === 'Wohnort-Frage');
      if (isWohnortFrage && multiAgentResults && multiAgentResults.locationContext && multiAgentResults.locationContext.fakeCity) {
      ...
      multiAgentResults = await runMultiStagePipeline({
        ...
        findNearbyCityFunc: (city) => findNearbyCity(city, { client, model: getMessageModel() || AI_MODEL }) // 🚨 LLM-Fallback für Stadt
      });
  ...
  // Wohnort/Stadt nur übernehmen, wenn der Kunde tatsächlich nach dem Wohnort gefragt hat (kein Eintrag ohne Grund)
    ...
    try {
      ...
      if (assistantFromReply && typeof assistantFromReply === "object" && Object.keys(assistantFromReply).length > 0) {
        ...
        Object.keys(assistantFromReply).forEach((key) => {
          ...
          if ((key === "Stadt" || key === "Wohnort") && !isWohnortFrageForMerge) return;
          extractedInfo.assistant[key] = (key === "Name") ? toFakeDisplayNameOnly(assistantFromReply[key]) : assistantFromReply[key];
        });
        // Profil-Name nur übernehmen, wenn er wie Anzeigename wirkt – nie Benutzername (z.B. "SchwarzeDom") ins Logbuch
        if (profileModName) {
          const profileDisplay = toFakeDisplayNameOnly(profileModName);
          if (!looksLikeUsername(profileDisplay)) extractedInfo.assistant["Name"] = profileDisplay;
        }
  ...
  // AVZ/FPC/Iluvo: Name im Fake-Logbuch nur Anzeigename (z. B. "Dominika", "Alana"), nie Benutzername (z. B. "SchwarzeDom")
  if ((isFPC || isAVZ) && (profileInfo?.moderatorInfo?.name || profileInfo?.moderatorInfo?.firstName)) {
    extractedInfo.assistant = extractedInfo.assistant || {};
  ...
  // Extension (FPC/AVZ): "Detected prompt" = erkannte Kundennachricht anzeigen – promptType + prompt immer mitsenden
