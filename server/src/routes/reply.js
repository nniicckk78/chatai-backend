    ...
    if (!generatedMessage || generatedMessage.trim() === "") {
      ...
      try {
        ...
        if (response.ok) {
          ...
          console.log(`✅ Feedback-Eintrag erfolgreich erstellt: ${result.feedback?.id || 'unbekannt'} (isASA: ${feedbackPayload.isASA})`);
        } else {
          const errorText = await response.text();
          console.warn(`⚠️ Feedback-Eintrag konnte nicht erstellt werden: ${response.status} - ${errorText} (isASA: ${feedbackPayload.isASA})`);
        }
      } catch (err) {
