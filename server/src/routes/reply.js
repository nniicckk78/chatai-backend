...
router.post("/", asyncHandler(async (req, res, next) => {
  ...
  if ((!foundMessageText || foundMessageText.trim() === "") && req.body?.siteInfos?.messages) {
    ...
    if (isIluvo && orderedMsgs.length > 0) {
      const receivedList = orderedMsgs.filter(m => (m?.type === "received" || m?.messageType === "received") && !isInfoMessage(m));
      const lastReceived = receivedList.length > 0 ? receivedList[receivedList.length - 1] : null;
      if (lastReceived) {
        const txt = (lastReceived.text || "").trim();
  ...
  try {
    ...
    if (imageUrls.length > 0) {
      ...
      if (dataUrl) {
        ...
        // Bild erkannt, aber zu groß oder fehlgeschlagen – KI trotzdem informieren, damit sie nicht erneut nach Foto fragt
        imageDescriptions.push("Der Kunde hat mit dieser Nachricht ein Bild geschickt (Inhalt konnte nicht analysiert werden, z.B. Datei zu groß). Reagiere positiv darauf; frage NICHT erneut nach einem Foto.");
