...
Antworte NUR mit der neuen Antwort, keine Erklärungen.`;
                      ...
                      console.error("⚠️ Fehler beim dritten Retry:", err.message);
                      // Verwende Fallback statt Fehler zu werfen
                      replyText = "Danke für deine Nachricht! Lass uns weiter schreiben. 😊";
                      console.log("✅ Fallback-Antwort verwendet (nach Retry-Fehler)");
                    }
                  }
                } else {
                  // Keine Antwort generiert - verwende Fallback
                  console.warn("⚠️ Finale Retry-Antwort ist leer - verwende Fallback");
                  replyText = "Danke für deine Nachricht! Lass uns weiter schreiben. 😊";
                }
              } catch (err) {
                console.error("⚠️ Fehler beim finalen Retry:", err.message);
                // Verwende Fallback statt Fehler zu werfen
                replyText = "Danke für deine Nachricht! Lass uns weiter schreiben. 😊";
                console.log("✅ Fallback-Antwort verwendet (nach finalem Retry-Fehler)");
              }
            }
          }
      } catch (err) {
        console.error("⚠️ Fehler beim Neu-Generieren der Antwort:", err.message);
        // Verwende Fallback statt Fehler zu werfen
        replyText = "Danke für deine Nachricht! Lass uns weiter schreiben. 😊";
        console.log("✅ Fallback-Antwort verwendet (nach Neu-Generieren-Fehler)");
      }
    }
