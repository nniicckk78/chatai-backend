...
router.post("/", asyncHandler(async (req, res, next) => {
  ...
  // 🚨 NEU: Bei ASA mit System-Nachricht (Kuss, Like) nutze diese als Kontext
  const asaConversationContext = isASACalculated && asaSystemMessage 
    ? asaSystemMessage.toLowerCase() 
    : (isASACalculated ? (compressConversation(req.body?.siteInfos?.messages || [], 10) || "").toLowerCase() : '');
  ...
  try {
    ...
    // 🚨 NEU: Entferne Grußformeln am Ende (Kuss, Küsse, Busi, etc.) - diese sind wie Emojis, nicht der Hauptinhalt
    const customerMessage = removeGreetingWordsAtEnd(foundMessageText || "");
    ...
    const messagesForPipeline = req.body?.siteInfos?.messages || [];
    const moderatorMessagesForPipeline = messagesForPipeline
      .filter(m => !isInfoMessage(m) && (m.type === "sent" || m.messageType === "sent") && typeof m?.text === "string" && m.text.trim() !== "")
      .slice(-20)
      .map(m => ({ text: m.text.trim() }));
    
    // Sammle Kunden-Nachrichten für Style-Analyse
    const customerMessagesForPipeline = messagesForPipeline
      .filter(m => !isInfoMessage(m) && (m.type === "received" || m.messageType === "received") && typeof m?.text === "string" && m.text.trim() !== "")
      .slice(-20)
      .map(m => ({ text: m.text.trim() }));
    ...
    multiAgentResults = await runMultiAgentPipeline({
      ...
      allMessages: messagesForPipeline,
  ...
  if (multiAgentResults.situation?.locationQuestionError) {
    ...
    return res.status(400).json({
      ...
      resText: locationError.message,
      replyText: locationError.message,
      ...
      flags: {
        blocked: true,
        reason: "location_question_error",
        isError: true,
        showError: true,
        requiresAttention: true,
        errorType: "critical",
  ...
  try {
    const messageResult = await agentMessageGenerator(multiAgentResults, {
      ...
      isMeetingRequestFunc: isMeetingRequest // 🚨 WICHTIG: Übergebe isMeetingRequest, damit "treffen" nur blockiert wird, wenn es wirklich eine Treffen-Anfrage ist
    });
  ...
  try {
    ...
    const feedbackPayload = {
      ...
      platform: platformId || 'viluu',
      isASA: isASACalculated || false,
      // 🚨 NEU: Speichere Beispiel-IDs für Performance-Tracking
      usedExampleIds: multiAgentResults?.training?.exampleIds || [],
      context: {
