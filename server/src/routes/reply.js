...
router.post("/", asyncHandler(async (req, res, next) => {
    ...
    try {
      ...
      if (assistantFromReply && typeof assistantFromReply === "object" && Object.keys(assistantFromReply).length > 0) {
        ...
        Object.keys(assistantFromReply).forEach((key) => {
          ...
          if (key === "Name" && profileModName) return;
          extractedInfo.assistant[key] = (key === "Name") ? toFakeDisplayNameOnly(assistantFromReply[key]) : assistantFromReply[key];
        });
        if (profileModName) extractedInfo.assistant["Name"] = toFakeDisplayNameOnly(profileModName);
  ...
  // AVZ/FPC: Name im Fake-Logbuch immer nur Anzeigename (z. B. "Alana"), nie Benutzername/Plattform-ID (z. B. "3176intrigieren")
  // – auch wenn Extraktion nicht lief oder keinen Name lieferte; der Kunde sieht das Logbuch.
  if ((isFPC || isAVZ) && profileInfo?.moderatorInfo?.name) {
    extractedInfo.assistant = extractedInfo.assistant || {};
    extractedInfo.assistant["Name"] = toFakeDisplayNameOnly(String(profileInfo.moderatorInfo.name).trim());
  }
