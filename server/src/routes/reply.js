...
const { runSimplifiedPipeline, runMultiAgentPipeline, agentMessageGenerator, agentFirstMessageDetector } = require("../utils/multi-agent"); // 🎨 NEU: Vereinfachte Pipeline + alte Pipeline
...
router.post("/", asyncHandler(async (req, res, next) => {
  ...
  // 🎨 NEU: Verwende vereinfachte Pipeline (Style-Extraction + Intelligent Few-Shot Learning)
  // 🚨 FALLBACK: Alte Pipeline bleibt verfügbar für Kompatibilität
  const USE_SIMPLIFIED_PIPELINE = true; // 🎨 Flag: Vereinfachte Pipeline aktivieren
  
  console.log(`🚀 Starte ${USE_SIMPLIFIED_PIPELINE ? 'vereinfachte' : 'Multi-Agent'}-Pipeline${isASACalculated ? ' (ASA-Modus)' : ''}...`);
  ...
  try {
    ...
    // 🎨 NEU: Verwende vereinfachte Pipeline wenn aktiviert
    ...
    if (multiAgentResults && multiAgentResults.needsGermanResponse && multiAgentResults.germanResponse) {
      ...
      // 🚨 FIX: Generiere Nachricht mit agentMessageGenerator, wenn Pipeline erfolgreich war
