...
const { getGitHubClient, getRepoInfo, getFileFromGitHub, GITHUB_JSON_LONG_CACHE_TTL_MS } = require("../utils/github");
...
const { runGrokPipeline, detectSituationsWithLLM, stripInternalPlaceholders, hasObviousMetaOutingPatterns, hasSuspiciousFormatPatterns } = require("../utils/grok-pipeline"); // 🚀 Grok-Pipeline (ASA, Erstnachricht, Stadt, normale Reply + LLM-Situationserkennung)
const { logChataiFakeNameResolution, moderatorInfoSnapshotForDebug } = require("../utils/fake-name-debug-log");
const { mergeViluuMeetingRulesIntoRules } = require("../utils/viluu-meeting-rules");
...
async function getTrainingData() {
  // Versuche zuerst von GitHub zu laden (24h In-Memory-Cache in getFileFromGitHub)
    ...
    try {
      ...
      for (const filePath of possiblePaths) {
        try {
          ...
          if (err.status !== 404) {
            console.warn('⚠️ Training-Daten GitHub:', err.message);
          }
        }
      }
    } catch (err) {
...
const { checkKICheckMessage, collectKICheckCandidateTexts, checkMinorMention } = require('../utils/safety-agent');
...
{
  "user": {
    "Name": "NUR Vorname oder voller Name des KUNDEN, wenn er WÖRTLICH in dieser Nachricht vorkommt (z.B. 'ich heisse Max', 'LG Thomas', Signatur). NIEMALS aus Plattform-Profil, Username oder Vermutung – wenn kein Name im Text steht: null",
    ...
    "Updates": "Tages-/Ereignisnotiz fuer das Logbuch OHNE Datum (Datum traegt die Extension als DD.MM.YY davor). NUR bei klarer Aussage zu aktuellem Geschehen oder Plan (z.B. 'beim Putzen etwas gebrochen', 'ist im KH', 'geht gleich einkaufen', 'hat neuen Job', 'ist krank'). Keine Stammdaten (Beruf, Dauer-Wohnort). 'Hat Auto gekauft' NUR bei expliziter Kaufaussage – vage Auto-Erwähnungen NICHT als Updates, sonst null",
...
KRITISCH - NUR WAS IM TEXT STEHT:
- Name: NUR extrahieren, wenn der Name (Vorname oder klarer Eigenname) DIREKT in dieser Nachricht als Name des Schreibenden vorkommt (z.B. "ich bin der Tom", "heisse Stefan", "LG Max", Signatur am Ende). NICHT erraten, NICHT aus Kontext/Profil/Username. Wenn die Nachricht keinen solchen Namen enthält: "Name": null.
...
Nachricht: ${messageText}`;
    ...
    if (extractedText) {
      ...
      // Kundennachricht-Extraktion: niemals assistant füllen (Schema: assistant: {}). Sonst landet z.B. Kundenalter fälschlich unter assistant.Age im Logbuch.
      return { user: cleanUser, assistant: {} };
    }
  } catch (err) {
...
Antworte NUR mit gültigem JSON, kein zusätzlicher Text. Format:
{
  ...
  "Updates": "NUR eine kurze Tages-/Aktivitätsnotiz fuer das Logbuch OHNE Datum (Datum setzt die Extension). NUR wenn die Fake-Person in ICH-Form konkret sagt, was sie HEUTE/später noch tut, erlebt oder geplant hat (z.B. 'Heute noch Haushalt machen und einkaufen', 'muss gleich noch kochen', 'bin heute noch unterwegs'). KEINE Stammdaten (kein dauerhafter Beruf, keine Hobby-Liste wie 'mag Wandern', kein Wohnort, keine reine Befindlichkeitsfloskel ohne konkretes Ereignis/Plan). Health/Arzttermine: lieber unter Health/Other, NICHT doppelt unter Updates. Bei Unsicherheit: null.",
...
- \"Updates\": NUR Tages-/Aktivitätslog (was heute/später noch ansteht) – siehe Feldbeschreibung. Nicht fuer Stammdaten missbrauchen.
...
Nachricht des Assistenten:\n${text.substring(0, 3000)}`;
    ...
    const out = {};
    const keys = ["Name", "Stadt", "Wohnort", "Work", "Beruf", "Sport and Hobbies", "Beziehungsstatus", "Family", "Health", "Sexual Preferences", "Updates", "Other"];
...
/** Erstes explizites "ich bin XX" in der generierten Fake-Antwort = genanntes Persona-Alter (nicht "48 passt zu mir" ohne ich-bin). */
function parseFakeStatedAgeFromGeneratedMessage(text) {
  if (!text || typeof text !== "string") return null;
  const m = text.match(/\bich\s+bin\s+(\d{1,2})\b/i);
  if (!m) return null;
  const n = parseInt(m[1], 10);
  return n >= 18 && n <= 120 ? n : null;
}

/**
 * Verhindert, dass das Kundenalter im Fake-Notizfeld landet: Extraktion aus Kundennachricht darf kein assistant.Age setzen;
 * wenn doch gleich user.Age → Profil-Alter oder aus generierter Antwort ("Ich bin 34").
 */
function reconcileFakeAgeForLogbookSummary(extractedInfo, profileInfo, generatedMessage) {
  if (!extractedInfo || typeof extractedInfo !== "object") return;
  extractedInfo.assistant = extractedInfo.assistant && typeof extractedInfo.assistant === "object" ? extractedInfo.assistant : {};
  const a = extractedInfo.assistant;
  const userAgeRaw = extractedInfo.user?.Age;
  const userAge = userAgeRaw != null && userAgeRaw !== "" ? Number(userAgeRaw) : null;
  const profAgeRaw = profileInfo?.moderatorInfo?.birthDate?.age;
  const profAge = profAgeRaw != null && profAgeRaw !== "" && !Number.isNaN(Number(profAgeRaw)) ? Number(profAgeRaw) : null;
  const fromMsg = parseFakeStatedAgeFromGeneratedMessage(generatedMessage || "");
  if (fromMsg != null) {
    a.Age = fromMsg;
    return;
  }
  if (userAge != null && a.Age != null && Number(a.Age) === userAge) {
    delete a.Age;
  }
  if ((a.Age == null || a.Age === "") && profAge != null && (userAge == null || profAge !== userAge)) {
    a.Age = profAge;
  }
}

/** Erzwingt: Wenn Fake-Alter im Profil bekannt ist, darf die Antwort kein anderes "ich bin X (Jahre)" enthalten. */
function enforceFakeAgeInGeneratedMessage(message, profileInfo) {
  if (!message || typeof message !== 'string') return message || '';
  const ageRaw = profileInfo?.moderatorInfo?.birthDate?.age;
  const ageNum = ageRaw != null && ageRaw !== '' && !Number.isNaN(Number(ageRaw)) ? Number(ageRaw) : null;
  const txt = message;
  if (ageNum == null || ageNum < 18 || ageNum > 120) return txt;

  // Korrigiere nur klare Selbst-Altersformeln (nicht jede Zahl im Text).
  // Beispiele: "ich bin 34", "ich bin 34 jahre", "bin 34 jahre alt"
  let out = txt;
  out = out.replace(/\bich\s+bin\s+(\d{1,2})\b(?!\s*(?:km|kilometer|€|euro|x|mal)\b)/gi, `ich bin ${ageNum}`);
  out = out.replace(/\bich\s+bin\s+(\d{1,2})\s*(jahre|jahr)\b/gi, `ich bin ${ageNum} $2`);
  out = out.replace(/\b(bin)\s+(\d{1,2})\s*(jahre|jahr)\s+alt\b/gi, `bin ${ageNum} $3 alt`);
  // Wenn er "wie alt bist du" gefragt hat, ist die häufigste Form "ich bin X jahre" – ebenfalls sichern:
  out = out.replace(/\bich\s+bin\s+(\d{1,2})\s*(jahre|jahr)\s+alt\b/gi, `ich bin ${ageNum} $2 alt`);

  return out;
}

/** Cherry/Plattform: dialogLanguage aus metaData – jede explizit nicht-deutsche UI-Sprache (en, it, es, …); leer/unbekannt = kein Block. */
function isDialogLanguageExplicitlyNonGerman(profileInfo) {
  const raw = (profileInfo?.dialogLanguage || '').toString().trim().toLowerCase();
  if (!raw) return false;
  if (raw === 'de' || raw === 'deutsch' || raw === 'german' || raw.startsWith('de-')) return false;
  // Plattform meldet keine feste Sprache → nicht blockieren (wie ohne Feld)
  ...
  if (ambiguous.has(raw)) return false;
  // Alles andere mit gesetztem Wert = konkrete Fremdsprache (en, it, es, fr, pt, english, italiano, …)
  return true;
}
...
/** Darf der Kunden-Name ins Logbuch (z. B. Gold „- Name:“) – nie Plattform-Login, nie Username-Muster. */
function customerNameAllowedForLogbook(name, username) {
  if (!name || typeof name !== "string") return false;
  const s = name.trim();
  if (!s) return false;
  const lower = s.toLowerCase();
  if (GENERIC_OR_OFFENSIVE_FOR_ADDRESS.has(lower)) return false;
  const login = username != null ? String(username).trim().toLowerCase() : "";
  if (login && lower === login) return false;
  const first = s.split(/\s+/)[0] || s;
  if (login && first.toLowerCase() === login) return false;
  if (looksLikeUsername(s) || looksLikeUsername(first)) return false;
  return true;
}

/** Mindest-Check: extrahierter Kundenname kommt im Nachrichtentext vor (gegen LLM-Halluzination). */
function extractedCustomerNamePlausibleInMessage(nameRaw, msgRaw) {
  if (!nameRaw || !msgRaw || typeof nameRaw !== "string" || typeof msgRaw !== "string") return false;
  const norm = (s) => s.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "");
  const msg = norm(msgRaw);
  const name = nameRaw.trim();
  if (!name) return false;
  for (const token of name.split(/\s+/)) {
    const t = norm(token).replace(/[^a-z0-9äöüß]/gi, "");
...
function enrichCustomerFamilyWithRelationshipHistory(userInfo, messageText) {
  if (!userInfo || typeof userInfo !== "object" || !messageText || typeof messageText !== "string") return;
  const txt = messageText.replace(/\s+/g, " ").trim();
  if (!txt) return;
  const hasRelationshipContext = /\b(beziehung|ex[-\s]?freund(?:in)?|ex[-\s]?frau|ex[-\s]?mann|partner(?:in)?)\b/i.test(txt);
  if (!hasRelationshipContext) return;
  const m = txt.match(/\bvor\s+([0-9]{1,2}|einem|einer|ein|eins|zwei|drei|vier|f[üu]nf|sechs|sieben|acht|neun|zehn)\s+jahr(?:en)?\b/i);
  if (!m) return;
  const span = m[0].replace(/\s+/g, " ").trim();
  const existing = (userInfo.Family || "").toString().trim();
  if (/beziehung\s+vor/i.test(existing)) return;
  const addition = `letzte Beziehung ${span}`;
  userInfo.Family = existing ? `${existing}, ${addition}` : addition;
}

/** Entfernt Name/Kosename aus summary.user wenn es nur der Plattform-Login ist (Extension soll nicht „- Name: ReMa76“ schreiben). */
function stripInappropriateCustomerNameFromSummary(summary, metaData) {
  if (!summary || typeof summary !== "object") return;
  const u = summary.user;
  if (!u || typeof u !== "object") return;
  const username = metaData?.customerInfo?.username;
  for (const key of ["Name", "Kosename"]) {
    const v = u[key];
    if (typeof v !== "string" || !v.trim()) continue;
    if (!customerNameAllowedForLogbook(v, username)) delete u[key];
  }
}

/** Eine durchsuchbare Log-Zeile (z. B. Render): welche Logbuch-Felder in summary.user (Kunde) / summary.assistant (Fake) mitgegeben werden. */
function logLogbookSummaryLine(chatId, extractedInfo, maxValLen = 200) {
  if (!extractedInfo || typeof extractedInfo !== "object") return;
  const shrink = (val) => {
    ...
    if (typeof val === "string") {
      const t = val.trim();
      if (!t) return null;
      return t.length > maxValLen ? `${t.slice(0, maxValLen)}…` : t;
    }
    if (typeof val === "number" && !Number.isNaN(val)) return val;
    if (typeof val === "boolean") return val;
    const s = String(val).trim();
    if (!s) return null;
    return s.length > maxValLen ? `${s.slice(0, maxValLen)}…` : s;
  };
  const pack = (obj) => {
    ...
    for (const [k, v] of Object.entries(obj)) {
      const sv = shrink(v);
      if (sv != null && sv !== "") o[k] = sv;
    }
    return Object.keys(o).length ? o : null;
  };
  const kunde = pack(extractedInfo.user);
  const fake = pack(extractedInfo.assistant);
  if (!kunde && !fake) return;
  const id = chatId != null && String(chatId).trim() !== "" ? String(chatId).trim() : "unknown";
  const payload = { chatId: id };
  if (kunde) payload.kunde = kunde;
  if (fake) payload.fake = fake;
  console.log(`LOGBOOK_SUMMARY ${JSON.stringify(payload)}`);
}
...
/** Liest z. B. "- Name: Heinz" / "-Name: Heinz" aus Gold-/Mod-Notizen (mehrzeilig). */
function parseGoldNotesLabelValue(notesRaw, label) {
...
/** Vorname für Anrede: nie Plattform-Login (name===username), Reihenfolge realer Quellen. Nur erster Vorname (kein Nachname in der Anrede). */
function resolveCustomerFirstNameForAddress(customerInfo, customerNotesRaw, extractedUserName) {
  const login = (customerInfo?.username || "").trim().toLowerCase();
  const tryCandidate = (raw) => {
    if (!raw || typeof raw !== "string") return "";
    const s = raw.trim();
    if (!s) return "";
    const lower = s.toLowerCase();
    if (GENERIC_OR_OFFENSIVE_FOR_ADDRESS.has(lower)) return "";
    if (login && lower === login) return "";
    if (looksLikeUsername(s)) return "";
    const first = s.split(/\s+/)[0] || s;
    if (!first) return "";
    if (login && first.toLowerCase() === login) return "";
    if (looksLikeUsername(first)) return "";
    return first;
  };
  const fromNotes = parseGoldNotesLabelValue(customerNotesRaw || "", "Name");
  const candidates = [
    extractedUserName,
  ...
  for (const c of candidates) {
    const ok = tryCandidate(typeof c === "string" ? c : (c != null ? String(c) : ""));
    if (ok) return ok;
  }
  return "";
}

/** Sicherer Fake-Vorname: niemals Plattform-Username (z.B. vollwertigRachel), nur echter Vorname-Token. */
function resolveSafeFakeFirstNameForPrompt(moderatorInfo, extractedAssistantName, fallbackName = "Sandy", debugContext) {
  const tryCandidate = (raw) => {
    if (!raw || typeof raw !== "string") return { ok: "", reason: "empty_or_not_string" };
    const s = raw.trim();
    if (!s) return { ok: "", reason: "trim_empty" };
    if (looksLikeUsername(s)) return { ok: "", reason: "username_whole_string" };
    const first = s.split(/\s+/)[0] || s;
    if (!first) return { ok: "", reason: "no_first_token" };
    if (looksLikeUsername(first)) return { ok: "", reason: "username_first_token" };
  ...
  for (let i = 0; i < candidates.length; i++) {
    ...
    const { ok, reason } = tryCandidate(str);
    steps.push({
    ...
    if (ok) {
      logChataiFakeNameResolution("resolveSafeFakeFirstNameForPrompt", {
        ...debugContext,
  ...
  logChataiFakeNameResolution("resolveSafeFakeFirstNameForPrompt", {
    ...debugContext,
    resolvedFirstName: fallbackName,
...
/**
 * End-of-Pipeline Guard: Kunde darf NICHT mit Fake-Namen angesprochen werden.
 * Extrem eng, um False-Positives zu vermeiden: nur wenn die Antwort am Anfang eine Anrede + Name enthält
 * (Hey/Hi/Hallo/Na ...) und dieser Name exakt dem Fake-Vornamen entspricht.
 *
 * @returns {{ ok: true } | { ok: false, error: string, details?: object }}
 */
function checkWrongCustomerAddressingWithFakeName(generatedMessage, fakeFirstName) {
  if (!generatedMessage || typeof generatedMessage !== "string") return { ok: true };
  const fn = (fakeFirstName || "").toString().trim();
  if (!fn) return { ok: true };

  // Nur Start prüfen (erste Zeile / erste ~80 Zeichen)
  const head = generatedMessage.replace(/\r/g, "").split("\n")[0].slice(0, 120).trim();
  if (!head) return { ok: true };

  // Unicode tolerant (u-Flag), Name max ~40 Zeichen
  const m = head.match(/^(?:hey|hi|hallo|na|servus|guten\s+(?:morgen|tag|abend))\s+([A-Za-zÄÖÜäöüß][A-Za-zÄÖÜäöüß\-]{1,40})\b/i);
  ...
  const addressed = normalize(m[1]);
  const fake = normalize(fn.split(/\s+/)[0] || fn);
  if (!addressed || !fake) return { ok: true };

  if (addressed === fake) {
    return {
      ok: false,
      error: `❌ FEHLER: Falsche Anrede – der Kunde wurde mit dem Fake-Namen („${fn}“) angesprochen.`,
      details: { addressed: m[1], fakeFirstName: fn }
    };
  }
  return { ok: true };
}
...
function ageFromBirthDateString(dateStr) {
  ...
  // DD.MM.YYYY / DD-MM-YYYY / DD/MM/YYYY (oder 2-stelliges Jahr)
...
function parseFakeAgeFromProfileText(text) {
  ...
  // 0) Cherry/Panel: Zeile wie "Maria, 56" oder "Maria, 56 Deutschland …" (Überschrift in Notizen/Profiltext)
  const lines = trimmed.split(/\r?\n/);
  for (const rawLine of lines) {
    const line = rawLine.replace(/\s+/g, " ").trim();
    const mEnd = line.match(/^[A-Za-zÄÖÜäöüß][A-Za-zÄÖÜäöüß.\-\s]{0,40},\s*(\d{1,3})\s*$/);
    if (mEnd) {
      const a0 = parseInt(mEnd[1], 10);
      if (a0 >= 18 && a0 <= 120) return a0;
    }
    const mDe = line.match(
      /^[A-Za-zÄÖÜäöüß][A-Za-zÄÖÜäöüß.\-\s]{0,40},\s*(\d{1,3})\s+(?:Deutschland|Schweiz|Österreich)\b/
    );
    if (mDe) {
      const a1 = parseInt(mDe[1], 10);
      if (a1 >= 18 && a1 <= 120) return a1;
    }
  }
...
/** Nur bei unsicherer Extraktion (HTML/Rohtext, kein explizites moderatorInfo-Feld): wenn „Fake“-Ort = Kundenort, verwerfen (Panel-Verwechslung). Nicht anwenden, wenn im Profil bereits ein Wohnort steht – gleiche Stadt wie Kunde ist dann erlaubt. */
function discardFakeCityIfMatchesCustomer(fakeCity, metaData, extractedUser) {
  if (!fakeCity || typeof fakeCity !== "string") return "";
  const f = fakeCity.trim();
  if (!f) return "";
  const cust = (
    metaData?.customerInfo?.city ||
    metaData?.customerInfo?.Wohnort ||
    extractedUser?.Wohnort ||
    extractedUser?.city ||
    ""
  )
    .toString()
    .trim()
    .toLowerCase();
  if (cust && f.toLowerCase() === cust) {
    console.warn("⚠️ Extrahierter Fake-Ort = Kundenort – verworfen (Verwechslung Panel/HTML).");
    return "";
  }
  return f;
}

/** Vorgegebenen Fake-Ort aus metaData oder HTML extrahieren (Blenny: "Stadt: …", FPC: "Wohnort: …"). FPC: nur Fake-Profilblock (.profilefake / f_detail), nicht letztes Vorkommen auf der ganzen Seite (sonst Kunden-Panel). */
function extractFakeCityFromMetaOrHtml(metaData, siteInfos) {
  ...
  if (existing && existing.toLowerCase() !== "sag ich später") {
    return existing;
  }
  ...
  if (fromRaw) return discardFakeCityIfMatchesCustomer(fromRaw, metaData, null);

  const html = (siteInfos?.html || siteInfos?.pageHtml || "").toString();
  if (html) {
    const fakeBlock =
      (html.match(/class="[^"]*card-body[^"]*profilefake[^"]*"[^>]*>[\s\S]{0,80000}/i)?.[0]) ||
      (html.match(/id="f_detail"[\s\S]{0,80000}/i)?.[0]) ||
      (html.match(/profilefake[\s\S]{0,80000}/i)?.[0]) ||
      "";
    if (fakeBlock) {
      const fromFakeHtml = collectLast(fakeBlock);
      if (fromFakeHtml) return discardFakeCityIfMatchesCustomer(fromFakeHtml, metaData, null);
      return "";
    }
    const fromHtml = collectLast(html);
    if (fromHtml) return discardFakeCityIfMatchesCustomer(fromHtml, metaData, null);
  }
  return "";
}
...
function buildSummaryFromMeta(metaData) {
  ...
  // Name/Kosename NICHT aus Profil/metaData – nur wenn der Kunde ihn in der aktuellen Kundennachricht nennt (extractInfoFromMessage).
...
// Lovado: Summary für Extension-Logbuch stabilisieren (keine Bundesland->Wohnort-Fehler, fehlende Kernfelder aus metaData/profil ergänzen).
function normalizeLovadoSummary(summary, profileInfo, metaData) {
  ...
  const bundeslaenderLikelyOnly = new Set([
    "baden-württemberg","baden wuerttemberg","bayern","sachsen","sachsen-anhalt","thüringen","thueringen",
  ...
  // Kunden-Name nicht aus Profil nachziehen – nur aus Extraktion der Kundennachricht (Logbuch „- Name:“).
...
// FPC: Backend-Summary auf stabile Alias-Felder normalisieren, damit Extension-FPC-Mapping
// unabhängig von Schreibweise/Altformat konsistent arbeitet.
function normalizeFpcSummary(summary) {
  const out = summary && typeof summary === "object" ? summary : {};
  out.user = out.user && typeof out.user === "object" ? out.user : {};
  out.assistant = out.assistant && typeof out.assistant === "object" ? out.assistant : {};
  const clean = (v) => (v == null ? "" : String(v).trim());
  const normKey = (k) => String(k || "").toLowerCase().replace(/[^a-z0-9äöüß]/g, "");
  const pickAny = (obj, keys) => {
    if (!obj || typeof obj !== "object") return "";
    const direct = (k) => clean(obj[k]);
    for (const k of keys) {
      const v = direct(k);
      if (v) return v;
    }
    // Fallback: case-/separator-insensitive lookup ("bodyType", "Body Type", "body_type", ...)
  ...
  const normalizePerson = (p, preferStadtForCity) => {
    const pref = pickAny(p, [
      "Sexual Preferences", "SexualPreferences", "sexual_preferences",
      "Sexuell Preferences", "Sexuell Prefrences", "Sexuelle Vorlieben",
      "SexuelleVorlieben", "Vorlieben"
    ]);
    if (pref) {
      if (!clean(p["Sexual Preferences"])) p["Sexual Preferences"] = pref;
      if (!clean(p["Vorlieben"])) p["Vorlieben"] = pref;
    }
    const work = pickAny(p, ["Work", "Beruf"]);
    if (work) {
      if (!clean(p["Work"])) p["Work"] = work;
      if (!clean(p["Beruf"])) p["Beruf"] = work;
    }
    const cityKeys = preferStadtForCity ? ["Stadt", "Wohnort", "city"] : ["Wohnort", "Stadt", "city"];
    let city = pickAny(p, cityKeys);
    if (preferStadtForCity) {
      const st = clean(p["Stadt"]);
      const wo = clean(p["Wohnort"]);
      if (st && wo && st.toLowerCase() !== wo.toLowerCase()) {
        city = st;
        p["Wohnort"] = st;
      }
    }
    if (city) {
      if (!clean(p["Wohnort"])) p["Wohnort"] = city;
      if (!clean(p["Stadt"])) p["Stadt"] = city;
    }
    const age = pickAny(p, ["Age", "Alter"]);
    if (age) {
      if (!clean(p["Age"])) p["Age"] = age;
      if (!clean(p["Alter"])) p["Alter"] = age;
    }
    const fam = pickAny(p, ["Family", "Beziehungsstatus", "Status"]);
    if (fam) {
      if (!clean(p["Family"])) p["Family"] = fam;
      if (!clean(p["Beziehungsstatus"])) p["Beziehungsstatus"] = fam;
      if (!clean(p["Status"])) p["Status"] = fam;
    }
    const height = pickAny(p, ["Height", "Körpergröße", "Koerpergroesse", "Groesse", "Größe"]);
    const weight = pickAny(p, ["Weight", "Gewicht", "kg"]);
    const heightWeight = pickAny(p, ["Height / Weight", "HeightWeight", "Height_Weight", "Größe / kg", "Groesse / kg"]);
    if (heightWeight) {
      if (!clean(p["Height / Weight"])) p["Height / Weight"] = heightWeight;
      if (!clean(p["Größe / kg"])) p["Größe / kg"] = heightWeight;
    } else if (height || weight) {
      const merged = [height, weight].filter(Boolean).join(" / ");
      if (merged) {
        if (!clean(p["Height / Weight"])) p["Height / Weight"] = merged;
        if (!clean(p["Größe / kg"])) p["Größe / kg"] = merged;
      }
    }
    const penisSize = pickAny(p, ["Penisgröße", "Penisgroesse", "Penis Size", "PenisSize", "Schwanzgröße", "Schwanzgroesse"]);
    if (penisSize) {
      const other = clean(p["Other"]);
      const marker = `Penisgröße: ${penisSize}`;
      if (!other) p["Other"] = marker;
      else if (!new RegExp(`penisgr(ö|oe)sse\\s*:\\s*${String(penisSize).replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}`, "i").test(other)) {
        p["Other"] = `${other}, ${marker}`;
      }
    }
  };
  normalizePerson(out.user, false);
  normalizePerson(out.assistant, true);
  return out;
}
...
const _normOrt = (s) => String(s || "")
  .trim()
  .toLowerCase()
  .normalize("NFD")
  .replace(/[\u0300-\u036f]/g, "");

/** Profil hat schon Stadt (z. B. Rahden), KI nennt Stadtteil/Ortsteil (z. B. Hoppe) → trotzdem ins Fake-Logbuch. */
function fakeWohnortExtractRefinesProfile(profileInfo, extractedVal) {
  const v = _normOrt(extractedVal);
  if (!v) return false;
  let mod = _normOrt(profileInfo?.moderatorInfo?.city || profileInfo?.moderatorInfo?.Wohnort || "");
  mod = mod.replace(/^\d{4,5}\s+/, "").replace(/\s*\([^)]*\)\s*$/, "").trim();
  if (!mod) return true;
...
/** Zielband 10–30 km, harte Obergrenze 50 km (Hilfskonstanten + Haversine). */
const FAKE_WOHNORT_IDEAL_MIN_KM = 10;
const FAKE_WOHNORT_IDEAL_MAX_KM = 30;
const FAKE_WOHNORT_ABS_MAX_KM = 50;
...
async function nominatimSleep(ms = 1100) {
  await new Promise((r) => setTimeout(r, ms));
}

async function geocodePlaceInDach(placeName, preferredCountryUpper) {
  if (!placeName || typeof placeName !== 'string') return null;
  const q = placeName.trim();
  if (!q || q.length < 2) return null;
  const cc = String(preferredCountryUpper || '').toLowerCase();
  const countrycodes = cc && ['de', 'at', 'ch'].includes(cc) ? cc : 'de,at,ch';
  const url = `https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(q)}&limit=1&addressdetails=1&countrycodes=${countrycodes}`;
  const res = await fetch(url, { headers: NOMINATIM_HEADERS });
  if (!res.ok) return null;
  const data = await res.json();
  if (!data || !data[0]) return null;
  const country = (data[0].address?.country_code || '').toUpperCase();
  if (!DACH_COUNTRIES.includes(country)) return null;
  return {
...
/**
 * Prüft Entfernung Kunde → vorgeschlagener Ort (Nominatim). optionally minKmDistinct: Mindestabstand für Fake-Wohnort (nicht dieselbe Gemeinde).
 */
async function validateCityDistanceFromCustomer(customerCity, proposedCity, opts = {}) {
  const maxKm = opts.maxKm != null ? opts.maxKm : FAKE_WOHNORT_ABS_MAX_KM;
  const minKmDistinct = opts.minKmDistinct != null ? opts.minKmDistinct : 0;
  if (!customerCity || !proposedCity) return { ok: false, reason: 'missing' };
  await nominatimSleep(1100);
  const cust = await geocodePlaceInDach(customerCity, null);
  if (!cust) return { ok: false, reason: 'customer_geocode' };
  await nominatimSleep(1100);
  const prop = await geocodePlaceInDach(proposedCity, cust.country);
  if (!prop) return { ok: false, reason: 'proposed_geocode' };
  if (prop.country !== cust.country) return { ok: false, reason: 'country_mismatch' };
  const km = haversineKm(cust.lat, cust.lon, prop.lat, prop.lon);
  if (minKmDistinct > 0 && km < minKmDistinct) return { ok: false, reason: 'too_close', km };
  if (km > maxKm) return { ok: false, reason: 'too_far', km };
  return { ok: true, km };
}

/** Kundenherkunft aus letzter Kundennachricht (z. B. "ich komme aus Schwindegg") – zuverlässiger als PLZ-Fragment im Profil. */
function extractCustomerOriginCityFromMessage(text) {
  if (!text || typeof text !== 'string') return null;
  const t = text.replace(/\s+/g, ' ').trim();
  const patterns = [
    /\bich\s+komme\s+aus\s+([A-Za-zäöüÄÖÜß][A-Za-zäöüÄÖÜß\-\s]{1,45}?)(?=[\s,!.?]|$)/iu,
    /\bkomm(?:e|st)\s+aus\s+([A-Za-zäöüÄÖÜß][A-Za-zäöüÄÖÜß\-\s]{1,45}?)(?=[\s,!.?]|$)/iu,
    /\bwohn(?:e|t)\s+in\s+([A-Za-zäöüÄÖÜß][A-Za-zäöüÄÖÜß\-\s]{1,45}?)(?=[\s,!.?]|$)/iu
  ];
  for (const p of patterns) {
    const m = t.match(p);
    if (m && m[1]) {
      let raw = m[1].trim().replace(/\s+/g, ' ');
      raw = raw.split(/\s+und\s+/i)[0].trim();
...
/** Profil-/Meta-Strings wie "8442... (Bayern)" für Nominatim unbrauchbar → Kundenort aus Nachricht bevorzugen. */
function sanitizeLocationQueryForGeocode(raw, fallbackFromCustomerMessage) {
  let s = String(raw || '').trim();
  if (!s) return fallbackFromCustomerMessage || '';
  s = s.replace(/\s*\([^)]*\)\s*/g, ' ').trim();
  if (/\.\.\.|…/.test(s) || /^\d{1,4}\s*\.?\s*\.?\s*$/.test(s)) {
    return fallbackFromCustomerMessage || '';
  }
  if (/^\d{5}\b/.test(s) && s.length <= 12) {
    return fallbackFromCustomerMessage || s;
  }
  return s;
}

/** Ortsname aus "Ich komme aus …" / "ich wohne in …" (nur erste Person ich/wir) — keine KI, keine Freitext-Ortserkennung. */
function cleanModeratorOriginPlace(raw) {
  if (!raw || typeof raw !== 'string') return null;
  let s = raw.trim().replace(/\s+/g, ' ');
  s = s.split(/[,;]/)[0].trim();
  s = s.replace(/\s+und\s+.*$/i, '').trim();
  if (s.length < 2 || s.length > 50) return null;
  if (/^(dem|der|die|das|einem|einer|einen|eine)\s+(raum|nähe|gebiet|süden|norden|osten|westen|umland)/i.test(s)) return null;
  if (/^(dem|der)\s+/i.test(s)) return null;
  return s;
}

/**
 * Alle Orte, die der Moderator in der generierten Antwort als EIGENEN Wohnort/Herkunft nennt
 * (nur klare Muster: ich/wir + komme/kommen aus / wohne in / bin aus).
 */
function extractModeratorFirstPersonOriginPlaces(generatedMessage) {
  if (!generatedMessage || typeof generatedMessage !== 'string') return [];
  const t = generatedMessage.replace(/\s+/g, ' ');
  const patterns = [
    /\bich\s+komme\s+aus\s+([A-Za-zäöüÄÖÜß][A-Za-zäöüÄÖÜß\-\s]{1,50}?)(?=[\s,.!?;:]|$)/giu,
    /\bwir\s+kommen\s+aus\s+([A-Za-zäöüÄÖÜß][A-Za-zäöüÄÖÜß\-\s]{1,50}?)(?=[\s,.!?;:]|$)/giu,
    /\bich\s+wohne\s+in\s+([A-Za-zäöüÄÖÜß][A-Za-zäöüÄÖÜß\-\s]{1,50}?)(?=[\s,.!?;:]|$)/giu,
    /\bich\s+bin\s+aus\s+([A-Za-zäöüÄÖÜß][A-Za-zäöüÄÖÜß\-\s]{1,50}?)(?=[\s,.!?;:]|$)/giu
  ];
  const found = new Set();
  for (const p of patterns) {
    p.lastIndex = 0;
    ...
    while ((m = p.exec(t)) !== null) {
      const c = cleanModeratorOriginPlace(m[1]);
      if (c) found.add(c);
    }
  }
  return [...found];
...
/**
 * Prüft nur explizite Fake-Selbstangaben („Ich komme aus …“ o. ä.) in der generierten Antwort:
 * jeder solche Ort muss ≤50 km vom Kundenwohnort sein (Profil + Fallback aus Kundennachricht).
 * Kein LLM — erwähnte Orte ohne diese Muster werden ignoriert (keine Fehlermeldung).
 * Bei Geocoding-Fehler: kein Block (vermeidet False Positives).
 */
async function validateGeneratedMessageFakeWohnortDistance(customerCity, generatedMessage, opts = {}) {
  if (!generatedMessage || typeof generatedMessage !== 'string') return { ok: true, skipped: true };
  const customerMsg = opts.customerMessageText && String(opts.customerMessageText) ? String(opts.customerMessageText) : '';
  const declaredFromCustomer = extractCustomerOriginCityFromMessage(customerMsg);
  let baseCity = String(customerCity || '').trim();
  baseCity = sanitizeLocationQueryForGeocode(baseCity, declaredFromCustomer);
  if (!baseCity || baseCity.length < 2) return { ok: true, skipped: true };

  const places = extractModeratorFirstPersonOriginPlaces(generatedMessage);
  if (places.length === 0) return { ok: true, skipped: true };

  const seenFail = [];
  for (const place of places) {
    const v = await validateCityDistanceFromCustomer(baseCity, place, { maxKm: FAKE_WOHNORT_ABS_MAX_KM, minKmDistinct: 0 });
    if (v.ok) continue;
    if (v.reason === 'customer_geocode' || v.reason === 'proposed_geocode') {
      console.warn(`🌍 Wohnort-Check: Geocoding übersprungen für „${place}“ (${v.reason}) — kein Block.`);
      continue;
    }
    if (v.reason === 'too_far' || v.reason === 'country_mismatch') {
      const kmPart = v.km != null ? ` (${Math.round(v.km)} km)` : '';
      const detail = v.reason === 'country_mismatch' ? ' (Land/Region)' : '';
      seenFail.push({ place, kmPart, detail });
    }
  }
  if (seenFail.length === 0) return { ok: true, placesChecked: places };

  const first = seenFail[0];
  return {
    ok: false,
    error: `In der Antwort steht „Ich komme aus …“ / „ich wohne in …“ mit „${first.place}“ — das ist zu weit vom Kundenwohnort${first.kmPart}${first.detail}. Maximal ${FAKE_WOHNORT_ABS_MAX_KM} km erlaubt.`
  };
}

// Hilfsfunktion: Finde eine Stadt im Zielband per OpenAI (nur wenn OSM nichts liefert); Entfernung wird per Nominatim verifiziert.
async function findNearbyCityViaLLM(client, model, customerCity) {
  ...
  try {
      ...
      messages: [
        {
          role: 'system',
          content:
            'Du bist ein Helfer für Städte in Deutschland, Österreich und der Schweiz. Gegeben der Wohnort eines Kunden, nenne genau EINE andere Stadt im gleichen Land, ' +
            `die etwa ${FAKE_WOHNORT_IDEAL_MIN_KM}–${FAKE_WOHNORT_IDEAL_MAX_KM} km entfernt liegt (niemals weiter als ${FAKE_WOHNORT_ABS_MAX_KM} km). ` +
            'Antworte NUR mit dem Städtenamen, kein Satz, keine Anführungszeichen.'
        },
    ...
    const verified = await validateCityDistanceFromCustomer(customerCity, cityName, {
      maxKm: FAKE_WOHNORT_ABS_MAX_KM,
      minKmDistinct: 1
    });
    if (!verified.ok) {
      console.warn(`⚠️ OpenAI-Stadt verworfen (Distanz/Geocoding): "${cityName}" für "${customerCity}"`, verified.reason, verified.km);
      return null;
    }
    console.log(`✅ Stadt via OpenAI gefunden und verifiziert: "${cityName}" (~${Math.round(verified.km)} km, für "${customerCity}")`);
    return cityName;
  } catch (err) {
...
// Hilfsfunktion: Finde eine Stadt im Zielband 10–30 km, maximal 50 km (für Dating-Profil).
// Reihenfolge: 1) OpenStreetMap (Reverse-Punkte + Haversine-Validierung), 2) OpenAI nur mit Nominatim-Verifikation – keine statischen Regional-Fallbacks.
async function findNearbyCity(customerCity, opts = {}) {
  ...
  // Kundenort kann als "Bundesland Ort" kommen (z.B. "Brandenburg Schönborn") – für Geocoding ist das oft schlecht.
  // Wenn das erste Token ein Bundesland ist, verwende nur den Rest als eigentlichen Ort.
  {
    const cityClean = city.replace(/\s+/g, ' ').trim();
    const tokens = cityClean.split(' ').filter(Boolean);
    if (tokens.length >= 2) {
      const t0 = tokens[0];
      if (BUNDESLAENDER_NAMES.has(t0) || [...BUNDESLAENDER_NAMES].some(b => t0 === b)) {
        const rest = tokens.slice(1).join(' ').trim();
        if (rest.length >= 2) city = rest;
      }
    }
  }
  ...
  // PRIORITÄT 2: OpenAI/LLM (nur mit Nominatim-Distanzprüfung, s. findNearbyCityViaLLM)
  ...
  console.warn(`⚠️ Keine nahegelegene Stadt gefunden für: "${customerCity}" (OSM und OpenAI ohne gültigen Treffer ≤${FAKE_WOHNORT_ABS_MAX_KM} km)`);
  return null;
}

/**
 * Reverse-Geocoding an Punkten im Zielband; Kandidaten per Vorwärtssuche + Haversine auf ≤50 km prüfen.
 * Bevorzugt Städte im Band 10–30 km, sonst jede gültige ≤50 km (min. ~1 km vom Kundenort).
 */
async function findNearbyCityFromGeocodedCustomer(customerCity, customerLat, customerLon, customerCountry) {
  const cosLat = Math.max(0.25, Math.cos((customerLat * Math.PI) / 180));
  const offsetKmToLatLon = (eastKm, northKm) => {
    const dLat = northKm / 111;
    const dLon = eastKm / (111 * cosLat);
    return { lat: customerLat + dLat, lon: customerLon + dLon };
  };
  // Mehrere Richtungen im Zielband 15–28 km
  const probeOffsets = [
    [24, 0],
    [-22, 0],
    [0, 26],
  ...
  const rawNames = [];
  for (const [ek, nk] of probeOffsets) {
    const { lat: plat, lon: plon } = offsetKmToLatLon(ek, nk);
    await nominatimSleep(1100);
    const reverseUrl = `https://nominatim.openstreetmap.org/reverse?format=json&lat=${plat}&lon=${plon}&zoom=10&addressdetails=1`;
    try {
      const reverseResponse = await fetch(reverseUrl, { headers: NOMINATIM_HEADERS });
      if (!reverseResponse.ok) continue;
      const reverseData = await reverseResponse.json();
      if (reverseData && reverseData.address) {
        const placeName =
          reverseData.address.city || reverseData.address.town || reverseData.address.village || reverseData.address.municipality || '';
        const country = (reverseData.address.country_code || '').toUpperCase();
        if (
          placeName &&
          DACH_COUNTRIES.includes(country) &&
          country === customerCountry &&
          placeName.trim().toLowerCase() !== String(customerCity).trim().toLowerCase()
        ) {
          rawNames.push(placeName.trim());
        }
      }
    } catch (_) {
      continue;
  ...
  const validatedIdeal = [];
  const validatedAny = [];
  for (const name of unique) {
    await nominatimSleep(1100);
    const prop = await geocodePlaceInDach(name, customerCountry);
    if (!prop) continue;
    const km = haversineKm(customerLat, customerLon, prop.lat, prop.lon);
    if (km < 1 || km > FAKE_WOHNORT_ABS_MAX_KM) continue;
    if (km >= FAKE_WOHNORT_IDEAL_MIN_KM && km <= FAKE_WOHNORT_IDEAL_MAX_KM) validatedIdeal.push({ name, km });
    else validatedAny.push({ name, km });
  }
  const pool = validatedIdeal.length > 0 ? validatedIdeal : validatedAny;
  if (pool.length === 0) return null;
  const pick = pool[Math.floor(Math.random() * pool.length)];
  console.log(
    `✅ OpenStreetMap: Stadt verifiziert: "${pick.name}" (~${Math.round(pick.km)} km, Zielband bevorzugt: ${validatedIdeal.length > 0 ? 'ja' : 'nein'})`
  );
  return pick.name;
}

// OpenStreetMap Nominatim – keine statischen Länder-Fallbacks; nur verifizierte Distanzen.
async function findNearbyCityViaAPI(customerCity) {
  if (!customerCity || typeof customerCity !== 'string') return null;

  try {
    const q = customerCity.trim();
    const geocodeUrl = `https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(q)}&limit=3&addressdetails=1&countrycodes=de,at,ch`;
    let geocodeResponse = await fetch(geocodeUrl, { headers: NOMINATIM_HEADERS });

    if (!geocodeResponse.ok && geocodeResponse.status === 429) {
      console.warn('⚠️ OpenStreetMap Rate Limit – warte 2s und wiederhole Geocode...');
      await new Promise((r) => setTimeout(r, 2000));
      geocodeResponse = await fetch(geocodeUrl, { headers: NOMINATIM_HEADERS });
    }
    ...
    if (!geocodeData || geocodeData.length === 0) {
      await nominatimSleep(1100);
      const fallbackRes = await fetch(geocodeUrl, { headers: NOMINATIM_HEADERS });
      if (fallbackRes.ok) geocodeData = await fallbackRes.json();
    }
    if (!geocodeData || geocodeData.length === 0) {
    ...
    await nominatimSleep(1000);
    const picked = await findNearbyCityFromGeocodedCustomer(customerCity, customerLat, customerLon, customerCountry);
    return picked;
  } catch (err) {
...
function lastMessageFromFakeHadImage(messages) {
  ...
  if (text.includes('ein bild wurde übertragen') || text.includes('ein bild wurde uebertragen')) return true;
  return false;
}

// Gibt die Bild-URL der letzten vom Fake gesendeten Nachricht zurück (für optionale Analyse). Sonst null.
...
function isInfoMessage(msg, options) {
  ...
  //
  // Ausnahme (Cherry u.a.): „Reinhold hat dir einen Kuss geschickt 😘“ = Kunde hat ans Profil geküsst → NICHT als Info filtern,
  // sondern Danke-/Kuss-Pfad (nur wenn neueste relevante received, siehe newestSystemTriggerMsg + Alters-/Verbraucht-Logik).
  // „Du hast Moritz einen Kuss geschickt“ = Bestätigung Mod→Kunde → weiterhin System-Info (kein Danke als „Kunden-Kuss“).
  {
    const rawK = String(msg.text || "").trim();
    const lowK = rawK.toLowerCase();
    if (/\bhat\s+dir\s+einen\s+kuss\s+(geschickt|gesendet)\b/i.test(lowK) && !/^du\s+hast\s+/i.test(rawK)) {
      return false;
    }
  }
  if (t.includes("ich habe dir einen kuss geschickt") || 
      ...
      t.includes("kuss nachricht") || // Gold / S69: „Kuss Nachricht!💋“
      t.includes("hat einen kuss geschickt") ||
      t.includes("hat einen kuss gesendet") ||
      // Kurz-Kuss: nur Moderator-/Plattform-Formulierungen — nicht „Name hat dir …“ (oben ausgenommen)
      (t.length < 80 && t.includes("kuss") && t.includes("geschickt") && (/^du\s+hast\b/i.test(t.trim()) || /^ich\s+habe\s+dir\b/i.test(t.trim()) || t.includes("der benutzer"))) ||
      (t.length < 80 && t.includes("kuss") && t.includes("gesendet") && (/^du\s+hast\b/i.test(t.trim()) || /^ich\s+habe\s+dir\b/i.test(t.trim()) || t.includes("der benutzer"))) ||
  ...
  // 🚨 System-Nachrichten für Freundschaftsanfragen erkennen (strikt)
  // WICHTIG: NICHT bei normalen Kundensätzen auslösen, in denen nur das Wort
  // "freundschaftsanfrage" vorkommt.
  const isStandaloneFriendRequest = /^\s*friend\s*request\s*[!?.:]*\s*$/i.test(t);
  const hasFriendRequestSystemPattern =
    /der benutzer .*möchte dich als freund hinzuf(ü|ue)gen/i.test(t) ||
    /benutzer .*möchte dich als freund hinzuf(ü|ue)gen/i.test(t) ||
    /hat dir eine freundschaftsanfrage (gesendet|geschickt)/i.test(t) ||
    /freundschaftsanfrage (gesendet|geschickt|annehmen|annehmen\?|akzeptiert|abgelehnt|best(ä|a)tigt)/i.test(t) ||
    /(friend request)/i.test(t);
  if (isStandaloneFriendRequest || hasFriendRequestSystemPattern) {
...
/**
 * Texte für Erkennung des Favoriten-/Wunschzettel-Systemhinweises.
 * AVZ filtert diese Bubble als Info aus `foundMessageText`; Roh-`siteInfos.messages` muss mitgeprüft werden,
 * sonst fehlt die rote Extension-Meldung (flags.showError), obwohl die KI korrekt schweigt.
 */
function collectWishlistSystemCandidateTexts(body, foundMessageText) {
  const out = [];
  const push = (v) => {
    if (typeof v === "string" && v.trim() !== "") out.push(v);
  };
  push(foundMessageText);
  push(body?._customerMessageTextForDedup);
  push(body?.messageText);
  push(body?.lastMessage);
  push(body?.lastCustomerMessage);
  const msgs = body?.siteInfos?.messages;
  if (Array.isArray(msgs)) {
    const tail = 35;
    const start = Math.max(0, msgs.length - tail);
    for (let i = msgs.length - 1; i >= start; i--) {
      const m = msgs[i];
      if (!m) continue;
      const t = m.text || m.content || m.message || m.body;
      if (typeof t === "string" && t.trim() !== "") push(t);
    }
  }
  return out;
}
...
/**
 * Struktur-Check ohne Zeitstempel-Alter (10-Min-Filter): Liegt im Verlauf – gleiche Ordnung wie bei
 * siteInfos.messages (Iluvo/Blenny [älteste…neueste], sonst ggf. per Timestamp umdrehen) – nach der
 * letzten sent-Zeile noch eine received mit Kundensignal (Text oder Bild)?
 * Cherry nicht verwenden; dort greift die eigene Final-Logik.
 */
function structuralCustomerTurnAfterLastFakeFromRaw(rawMessages, opts) {
  const isBlennyOrigin = !!(opts && opts.isBlennyOrigin);
  const isIluvo = !!(opts && opts.isIluvo);
  const isAVZOrigin = !!(opts && opts.isAVZOrigin);
  const empty = { hasTurn: false, customerText: null, isImageOnly: false };
  if (!Array.isArray(rawMessages) || rawMessages.length === 0) return empty;

  let msgs = rawMessages;
  if (isAVZOrigin || isIluvo || isBlennyOrigin) {
    msgs = rawMessages.filter(m => m && !isInfoMessage(m, { isBlenny: isBlennyOrigin }));
    if (msgs.length === 0) msgs = rawMessages;
  }
  ...
  const hasImageSignal = (m) =>
    !!(m &&
      (m.image ||
        m.imageUrl ||
        m.imageSrc ||
        m.image_url ||
        (m.url && /\.(jpg|jpeg|png|gif|webp)/i.test(String(m.url))) ||
        m.attachment ||
        m.attachments ||
        m.media ||
        m.mediaUrl));
  ...
  if (lastSentIdx < 0) return empty;

  for (let i = lastSentIdx + 1; i < orderedMsgs.length; i++) {
    const m = orderedMsgs[i];
    if (!m || !isReceived(m)) continue;
    if (isInfoMessage(m, { isBlenny: isBlennyOrigin })) continue;
    const txt = getMessageText(m);
    if (txt.length > 0) {
      return { hasTurn: true, customerText: txt, isImageOnly: false };
    }
    if (hasImageSignal(m)) {
      return { hasTurn: true, customerText: null, isImageOnly: true };
    }
  }
  return empty;
}
...
// WICHTIG: Nur auslösen, wenn der KUNDE selbst den Wunsch äußert – nicht wenn er wiedergibt, was der Fake gesagt hat (z. B. "beeindruckt hat mich, dass du mich als freund haben willst").
function isBefreundetSeinMessage(text) {
  ...
  // Ausschluss: Kunde gibt wieder, was DU (Fake) gesagt/gewollt hast – dann ist "als freund haben" auf den Fake bezogen, keine Freundschaftsanfrage des Kunden
  ...
  const patterns = [
    ...
    // "als freund haben/hinzufügen/sein" nur wenn ICH/er/sie (Kunde) der Handelnde ist – nicht "dass du mich als freund haben willst"
    /\b(ich\s+)?(möchte|will)\s+.{0,30}als\s+freund(e)?\s+(hinzufügen|haben|sein)\b/,
    /\b(lass\s+uns|können\s+wir)\s+.{0,25}(befreundet|als\s+freund)\b/,
    /\bals\s+freund(e)?\s+hinzufügen\b/  // Plattform-Formulierung
  ];
  return patterns.some(p => p.test(t));
}
...
/** Parst aus Blenny/DF-Seiten-HTML relative Zeiten (z.B. "15 hours ago", "1 day ago", "vor 15 Stunden") und liefert das kleinste gefundene Alter in ms (entspricht der zuletzt sichtbaren/aktuellsten Nachricht). Für ASA: Alter der letzten Kunden-Nachricht, wenn Extension kein lastCustomerMessageAgeMs schickt. */
...
function isMeetingRequest(text, customerMessage = "") {
  ...
  // Zeitgebundene "Bock/Lust"-Fragen implizieren oft ein reales Treffen am gleichen Tag.
  // Beispiel (verboten): "Hast du heute Abend Bock auf etwas Spass?"
  const timeBoundedBockQuestionPatterns = [
    /\bhast\s+du\b.{0,40}\b(heute|heut|heute\s+abend|heute\s+nacht|morgen|morgen\s+abend|sp[aä]ter)\b.{0,40}\b(bock|lust)\b/i,
    /\b(bock|lust)\b.{0,40}\b(heute|heut|heute\s+abend|heute\s+nacht|morgen|morgen\s+abend|sp[aä]ter)\b/i
  ];
  ...
  // Prüfe auf zeitgebundene "Bock/Lust"-Formulierungen (impliziter Termin)
  if (timeBoundedBockQuestionPatterns.some((rx) => rx.test(lower))) {
    return true; // BLOCKIERT - klingt nach konkreter Zeit-Einladung
  }
...
async function generateFirstMessage({
  ...
  try {
    ...
    const fakeName = resolveSafeFakeFirstNameForPrompt(
      profileInfo?.moderatorInfo || {},
      extractedInfo?.assistant?.Name,
      "Sandy",
      { route: "generateFirstMessage", platformId }
    );
    ...
    const customerName = resolveCustomerFirstNameForAddress(
      customerInfo,
      profileInfo?.customerNotes,
...
Antworte NATÜRLICH und AUTHENTISCH - nicht wie eine KI!`;
    ...
    const likeKussExactHint = hasLike && !hasKuss
      ? `\n[PFLICHT – LIKE] Der Kunde hat das Profil geliked (LIKE), es gab keinen Kuss. Bedanke dich NUR fuer den Like / die Aufmerksamkeit (z.B. Danke fuer dein Like, freut mich ueber dein Like, schoen dass du vorbeischaust). VERBOTEN in der Antwort: das Wort "Kuss" oder "küssen" oder "Danke fuer den Kuss".\n`
      : hasKuss && !hasLike
        ? `\n[PFLICHT – KUSS] Der Ausloeser war ein KUSS. Bedanke dich fuer den Kuss. VERBOTEN: "Like", "geliked", "Danke fuer den Like" wenn es nur ein Kuss war.\n`
        : '';
...
router.post("/", asyncHandler(async (req, res, next) => {
  // 🚨 FIX: Ignoriere Extension-Reload-Requests mit reason "not_matching_chat_id"
  ...
  // Manche Clients hängen Text direkt an "not_matching_chat_id" an (z. B. "not_matching_chat_idHey ...")
  ...
  if (isNotMatchingChatId && (requestOriginLower.includes("fpc") || requestOriginLower.includes("iluvo") || requestOriginLower.includes("cherry") || requestOriginLower.includes("lovado") || requestOriginLower.includes("blenny") || requestOriginLower.includes("zumblenny") || requestOriginLower.includes("chathomebase") || requestOriginLower.includes("avz"))) {
    console.log("⚠️ Extension-Reload-Request erkannt (origin=" + requestOriginLower + ", reason=not_matching_chat_id) - IGNORIERE");
  ...
  /** True wenn laut siteInfos.messages die letzte relevante Zeile vom Fake ist und danach kein Kundenturn kam (ASA) – kein Fallback auf alte Kundenzeilen. */
  let suppressCustomerMessageFallbackForAsa = false;
  ...
  // Cherry: wie FPC – wenn siteInfos.messages gesetzt ist, Body-messageText ignorieren (Panel kann veralteten Text aus letzer Kundenzeile senden).
  const cherryUseMessagesOnly = isCherryReply && Array.isArray(req.body?.siteInfos?.messages) && req.body.siteInfos.messages.length > 0;
  let foundMessageText = cherryUseMessagesOnly ? "" : (messageText || possibleMessageFromBody);
  ...
  if (!isFPCRequest && !cherryUseMessagesOnly && messageText && messageText.trim() !== "") {
  ...
  if (!isFPCRequest && !cherryUseMessagesOnly && (!foundMessageText || foundMessageText.trim() === "") && userProfile && typeof userProfile === 'object') {
  ...
  if ((!foundMessageText || foundMessageText.trim() === "") && req.body?.siteInfos?.messages) {
    ...
    /** Gold/ChatsX/Diamond-Mods: Extension liefert oft keine Timestamps; Combine mehrerer Kundenzeilen erzeugt lange Texte → KI reagiert auf den Anfang (älteste Bubble). Nur neueste Nachricht im Turn verwenden (wie Cherry strict). */
    const siteUrlForGold = String(req.body?.siteInfos?.url || "");
    const isModsGoldOrigin =
      originMsg === "gold" ||
      originMsg === "diamond" ||
      originMsg === "s69" ||
      originMsg === "ml" ||
      /mods\.(gold-chat\.net|chatsx\.net)/i.test(siteUrlForGold) ||
      /mods\.(diamondchat\.net|platin-chat|onlydates69|mltestapp\.com)/i.test(siteUrlForGold);
    ...
    let newestFirst = false;
    // Cherry: Reihenfolge kommt aus der Extension (stableY = Panel oben→unten). Timestamp-Sort würde bei falsch zugeordnetem Datum „Heute“ vs. ältere Sektion ASA erzwingen.
    if (!isIluvo && !isBlennyOrigin && !isCherryOrigin && msgs.length > 1) {
      try {
        const firstTs = msgs[0]?.timestamp ? new Date(msgs[0].timestamp).getTime() : null;
        const lastTs = msgs[msgs.length - 1]?.timestamp ? new Date(msgs[msgs.length - 1].timestamp).getTime() : null;
        if (firstTs && lastTs && firstTs > lastTs) newestFirst = true;
    ...
    let orderedMsgs = (isIluvo || isBlennyOrigin) ? msgs : (newestFirst ? [...msgs].reverse() : msgs);
      ...
      .map(({ m, idx }) => {
        ...
        if (m.timestamp) {
          try {
            ...
            // Daher bei Cherry nicht über age verwerfen, sondern über Turn/Index entscheiden.
            if (isCherryOrigin) {
              if (isNaN(age)) age = null;
              isValid = true;
            } else if (age < -5 * 60 * 1000) {
              console.log(`⚠️ Nachricht liegt in der Zukunft (${Math.round(age / 1000)}s), überspringe:`, getMessageText(m).substring(0, 50));
              isValid = false;
            } else if (age > maxAge) {
    ...
    const cherryNewestAtTop = !!isCherryOrigin;
    if (isCherryOrigin) console.log("✅ Cherry: Index-Logik [neueste..älteste] (nach Timestamp-Normalisierung der Liste).");
    const validReceivedMessages = receivedMessages
      .filter(m => m.isValid && !isInfoMessage(m.message, { isBlenny: isBlennyOrigin }))
      .sort((a, b) => {
        if (isCherryOrigin) {
          // Cherry: Reihenfolge ist oben=neueste, also kleinerer Index = neuer.
          return cherryNewestAtTop ? (a.index - b.index) : (b.index - a.index);
        }
    ...
    let lastSentIdx = -1;
    if (isCherryOrigin && cherryNewestAtTop) {
      // Erste „sent“ von oben = chronologisch letzte Fake-Nachricht (Cherry = [neueste … älteste]).
      for (let i = 0; i < orderedMsgs.length; i++) {
        if (orderedMsgs[i] && isSentMessage(orderedMsgs[i])) { lastSentIdx = i; break; }
      }
    } else {
      for (let i = 0; i < orderedMsgs.length; i++) {
        if (orderedMsgs[i] && isSentMessage(orderedMsgs[i])) lastSentIdx = i;
    ...
    // Cherry: nur Index zur letzten Fake-Zeile von oben – kein globaler Max-Timestamp der Fakes.
    // Sonst schließt rt > max(sentTs) eine oben sichtbare Kundenzeile aus, wenn irgendein älterer Eintrag
    // einen neueren (kaputten) Zeitstempel hat oder der Kunde einen älteren TS trägt als ein Fake weiter unten.
    let afterLastMod = validReceivedMessages.filter(m => {
      if (lastSentIdx < 0) return true;
      if (isCherryOrigin) return m.index < lastSentIdx; // Cherry fest [neueste … älteste]: kleinerer Index = neuer
      return m.index > lastSentIdx;
    });
    // Cherry: DOM/Extension-Reihenfolge weicht oft von der echten Zeit ab (ältere Bubble oben, neueste unten im Payload).
    // Dann liefert nur Index < lastSentIdx die falsche „neueste“ Kundenzeile. Wenn Zeitstempel plausibel sind: strikt
    // alle received NACH dem spätesten Fake-Zeitpunkt (max.sent ts), neueste zuerst sortieren.
    if (isCherryOrigin && orderedMsgs.length > 0) {
      const cherryMsgTs = (msg) => {
        if (!msg) return NaN;
        const v = msg.timestamp != null ? msg.timestamp : msg.time;
        if (v == null) return NaN;
        const t = new Date(v).getTime();
        return isNaN(t) ? NaN : t;
      };
      let lastModMaxTs = null;
      for (let ci = 0; ci < orderedMsgs.length; ci++) {
        const om = orderedMsgs[ci];
        if (!om || !isSentMessage(om)) continue;
        const st = cherryMsgTs(om);
        if (!isNaN(st) && st > 0 && (lastModMaxTs == null || st > lastModMaxTs)) lastModMaxTs = st;
      }
      if (lastModMaxTs != null) {
        const tsAfterFake = validReceivedMessages.filter(m => {
          const rt = cherryMsgTs(m.message);
          return !isNaN(rt) && rt > lastModMaxTs;
        });
        if (tsAfterFake.length > 0) {
          tsAfterFake.sort((a, b) => {
            const ta = cherryMsgTs(a.message);
            const tb = cherryMsgTs(b.message);
            if (tb !== ta) return tb - ta;
            return a.index - b.index;
          });
          afterLastMod = tsAfterFake;
          console.log("✅ Cherry: Kundenturn per Timestamp (received nach letztem Fake-Zeitpunkt), Anzahl:", tsAfterFake.length);
        }
      }
      // Einzelne received mit kaputtem/neuem Timestamp können rt > max(sent) erfüllen, obwohl global zuletzt der Fake schrieb.
      // Abgleich: spätester Fake-Zeitpunkt vs. spätester Kunden-Zeitpunkt (nur sinnvolle TS, keine weit-in-der-Zukunft-Daten).
      const futureSlackMs = 15 * 60 * 1000;
      let maxSentTsAll = null;
      let maxRecvTsAll = null;
      for (let gi = 0; gi < orderedMsgs.length; gi++) {
        const om = orderedMsgs[gi];
        if (!om) continue;
        const t = cherryMsgTs(om);
        if (isNaN(t) || t <= 0) continue;
        if (isSentMessage(om)) {
          if (maxSentTsAll == null || t > maxSentTsAll) maxSentTsAll = t;
        } else if (isReceived(om) && getMessageText(om) && !isInfoMessage(om, { isBlenny: isBlennyOrigin })) {
          if (t > now + futureSlackMs) continue;
          if (maxRecvTsAll == null || t > maxRecvTsAll) maxRecvTsAll = t;
        }
      }
      // Wenn weiter unten im DOM noch Fakes mit neuerem Kalendertag/Zeit hängen (Chat-Wechsel, Scroll, virtuelle Liste),
      // ist max(Fake-Ts) > max(Kunden-Ts) trotzdem erfüllt – obwohl oben im Panel klar eine Kundenbubble steht (Index 0 = received).
      // Dann afterLastMod leeren würde foundMessageText killen und ASA erzwingen, obwohl Extension/UI „Kundenturn“ zeigen.
      let cherryFirstRecvFromTop = -1;
      let cherryFirstSentFromTop = -1;
      for (let fi = 0; fi < orderedMsgs.length; fi++) {
        const fm = orderedMsgs[fi];
        if (!fm) continue;
        if (cherryFirstRecvFromTop < 0 && isReceived(fm) && getMessageText(fm)) {
          cherryFirstRecvFromTop = fi;
        }
        if (cherryFirstSentFromTop < 0 && isSentMessage(fm)) {
          cherryFirstSentFromTop = fi;
        }
        if (cherryFirstRecvFromTop >= 0 && cherryFirstSentFromTop >= 0) break;
      }
      const cherryTrustViewportTopOverGlobalMaxTs =
        cherryFirstRecvFromTop >= 0 && (cherryFirstSentFromTop < 0 || cherryFirstRecvFromTop < cherryFirstSentFromTop);
      if (!cherryTrustViewportTopOverGlobalMaxTs && maxSentTsAll != null && maxRecvTsAll != null && maxSentTsAll > maxRecvTsAll) {
        afterLastMod = [];
        console.log("✅ Cherry: max(Fake-Ts) > max(Kunden-Ts) → chronologisch zuletzt Fake, kein Kundenturn (ASA).");
      } else if (cherryTrustViewportTopOverGlobalMaxTs && maxSentTsAll != null && maxRecvTsAll != null && maxSentTsAll > maxRecvTsAll) {
        console.log(
          "⚠️ Cherry: max(Fake-Ts) > max(Kunden-Ts), aber erste Zeile im Payload = Kunde vor erster Fake-Zeile – afterLastMod nicht leeren (vermutlich DOM-Mischung mehrerer Tage/Sessions)."
        );
      }
    }
    // Fake hat zuletzt geschrieben und es gibt keinen Kundenturn danach → kein Fallback auf ältere „received“ (sonst Pseudo-Reply statt ASA).
    if (!isCherryOrigin && lastSentIdx >= 0 && afterLastMod.length === 0) {
      suppressCustomerMessageFallbackForAsa = true;
    }
    const messagesToUse = isCherryOrigin
      ? afterLastMod
      : (afterLastMod.length > 0 ? afterLastMod : (suppressCustomerMessageFallbackForAsa ? [] : validReceivedMessages));
    ...
    if (isCherryOrigin) {
      ...
      const cherryCandidates = [...afterLastMod].sort((a, b) => cherryNewestAtTop ? (a.index - b.index) : (b.index - a.index));
      for (let i = 0; i < cherryCandidates.length; i++) {
        const cand = cherryCandidates[i];
        if (!cand || !cand.message) continue;
        if (isInfoMessage(cand.message, { isBlenny: false })) continue;
        const txt = getMessageText(cand.message);
        if (!txt) continue;
        cherryNewestReceived = cand;
        break;
      }
      if (cherryNewestReceived && cherryNewestReceived.message) {
        foundMessageText = getMessageText(cherryNewestReceived.message);
        console.log("✅ Cherry strict: Neueste einzelne received gewählt (kein Combine):", foundMessageText.substring(0, 80) + "...");
      }
      let newestSentIdx = -1;
      let newestReceivedIdx = -1;
      let newestSentTs = null;
      let newestReceivedTs = null;
      if (cherryNewestAtTop) {
        // Gesamten Verlauf scannen: max. Timestamp je sent/received (nicht nur die erste sent+received-Zeile).
        // Sonst: z. B. erste Fake-Zeile oben mit altem ts, darunter Kunde, weiter unten neuere Fakes → fälschlich „Kunde nach Fake“.
        for (let i = 0; i < orderedMsgs.length; i++) {
          const m = orderedMsgs[i];
          if (!m) continue;
          if (newestSentIdx < 0 && isSentMessage(m)) newestSentIdx = i;
          // Erste sichtbare Kundenzeile von oben: keine System-/Info-Bubbles (Kuss/Like/…)
          if (newestReceivedIdx < 0 && isReceived(m) && getMessageText(m) && !isInfoMessage(m, { isBlenny: isBlennyOrigin })) newestReceivedIdx = i;
          if (isSentMessage(m)) {
            const sv = (m.timestamp != null ? m.timestamp : m.time);
            const st = sv ? new Date(sv).getTime() : NaN;
            if (!isNaN(st) && st > 0 && (newestSentTs == null || st > newestSentTs)) newestSentTs = st;
          }
          // Wichtig: max(received-ts) darf Info-/Systemzeilen (z. B. „Du hast … einen Kuss geschickt“ mit abends TS)
          // nicht mitzählen — sonst newestReceivedTs > newestSentTs obwohl zuletzt der Moderator schrieb (ASA).
          if (isReceived(m) && getMessageText(m) && !isInfoMessage(m, { isBlenny: isBlennyOrigin })) {
            const rv = (m.timestamp != null ? m.timestamp : m.time);
            const rt = rv ? new Date(rv).getTime() : NaN;
            if (!isNaN(rt) && rt > 0 && (newestReceivedTs == null || rt > newestReceivedTs)) newestReceivedTs = rt;
          }
        }
      } else {
        for (let i = 0; i < orderedMsgs.length; i++) {
      ...
      if (newestSentTs != null && newestReceivedTs != null) {
        cherryComputedLastMessageFromFake = newestSentTs > newestReceivedTs;
        hasRecentCustomerTurnAfterLastSent = newestReceivedTs > newestSentTs;
      } else {
        cherryComputedLastMessageFromFake = cherryNewestAtTop ? (newestSentIdx < newestReceivedIdx) : (newestSentIdx > newestReceivedIdx);
        hasRecentCustomerTurnAfterLastSent = cherryNewestAtTop ? (newestReceivedIdx < newestSentIdx) : (newestReceivedIdx > newestSentIdx);
      }
      // Keine „sent“-Zeile im Payload (Erstkontakt / Extension liefert nur Kunde): Index-Vergleich wäre -1 vs 0 → fälschlich ASA.
      if (newestSentIdx < 0 && newestReceivedIdx >= 0) {
        cherryComputedLastMessageFromFake = false;
        hasRecentCustomerTurnAfterLastSent = true;
      }
      // Oberste sichtbare Kundenzeile (kleinerer Index als erste „sent“ von oben) schlägt globale Max-Timestamps:
      // verhindert ASA, obwohl die UI klar „Kunde zuletzt“ zeigt.
      if (cherryNewestAtTop && newestReceivedIdx >= 0 && newestSentIdx >= 0 && newestReceivedIdx < newestSentIdx) {
        cherryComputedLastMessageFromFake = false;
        hasRecentCustomerTurnAfterLastSent = true;
      }
      ...
      if (!hasRecentCustomerTurnAfterLastSent && (!foundMessageText || foundMessageText.trim() === "")) {
        console.log("✅ Cherry strict: Keine Kundennachricht nach letzter Fake gefunden -> ASA/Fallback auf alte received blockiert.");
      }
    }
    if (messagesToUse.length > 0) {
      // Cherry: Kundentext nur dann übernehmen, wenn vorher wirklich "received nach letzter sent"
      // über Zeitstempel/Index festgestellt wurde. Damit vermeiden wir falsche Non-ASA-Fälle
      // durch inkonsistente afterLastMod-Bildung.
      if (isCherryOrigin && hasRecentCustomerTurnAfterLastSent && afterLastMod.length > 0) {
        // Cherry: strikt auf die neueste Kunden-Nachricht NACH letzter Fake-Nachricht reagieren.
        // Keine Kombination mehrerer Nachrichten – das führte bei Cherry teils zu Antworten auf ältere Texte.
        const sortedByIndex = [...afterLastMod].sort((a, b) => cherryNewestAtTop ? (a.index - b.index) : (b.index - a.index));
        const newestAfterFake = sortedByIndex[0] ? sortedByIndex[0].message : null;
        if (newestAfterFake) {
          ...
          if (!isInfoMessage(newestAfterFake, { isBlenny: false }) && newestTxt) {
            ...
            console.log("✅ Cherry: Neueste Kundennachricht nach letzter Fake (strict single):", foundMessageText.substring(0, 80) + "...");
          }
        }
      }
      ...
      if (nonInfoInTurn.length > 1) {
        ...
        if (isModsGoldOrigin) {
          // messagesToUse ist bereits „neueste zuerst“ (Alter/Index); nonInfoInTurn behält diese Reihenfolge → [0] = neueste Bubble.
          const newestWrap = nonInfoInTurn[0];
          const newestTxt = newestWrap && newestWrap.message ? getMessageText(newestWrap.message) : "";
          if (newestTxt && newestTxt.trim() !== "") {
            foundMessageText = newestTxt.trim();
            console.log(
              "✅ Gold/Diamond/S69: nur neueste Kundennachricht im Turn (kein Combine; " + sortedByIndex.length + " Zeilen im Turn):",
              foundMessageText.substring(0, 80) + "..."
            );
          }
        } else {
        ...
        if (combinedLower.startsWith("ich habe dir einen kuss geschickt") || combinedLower.startsWith("der benutzer hat dich geküsst") || combinedLower.startsWith("benutzer hat dich geküsst") || combinedLower.startsWith("hat dir einen kuss geschickt") || combinedLower.startsWith("hat dir ein kuss geschickt") || combinedLower.startsWith("hat dir einen kuss gesendet") || combinedLower.startsWith("hat dir ein kuss gesendet") || /^\s*kuss nachricht\b/i.test(combinedLower)) {
          const systemEndPatterns = [
            ...
            /^\s*kuss nachricht\b[^\n]*/i // Gold/S69: „Kuss Nachricht!💋“ (+ ggf. Rest der Zeile)
          ];
        ...
        if (isInfoMessage(lastReceived, { isBlenny: isBlennyOrigin })) {
          ...
          if (nonInfoMessages.length > 0) {
            ...
            const isOnlyGruß = onlyMsgText && (onlyMsgText.toLowerCase().includes('hat dir einen gruß gesendet') || onlyMsgText.toLowerCase().includes('hat dir ein gruß gesendet') || (onlyMsgText.toLowerCase().includes('gruß') && onlyMsgText.toLowerCase().includes('gesendet')));
            foundMessageText = isOnlyGruß ? onlyMsgText : '';
            if (isOnlyGruß) console.log('✅ Blenny/DF: Nur Gruß-Systemnachricht im Kundenturn – setze foundMessageText für Danke-Pfad.');
            else console.log("⚠️ Nur Info-Messages in Kundenturn");
          }
        } else {
    ...
    if (!foundMessageText || foundMessageText.trim() === "") {
      if (isCherryOrigin) {
        // Cherry: NIEMALS den anyMessages-Fallback (10-Minuten-Sortierung nach age).
        // Kaputte Timestamps (Zukunft) erzeugen negatives Alter → alte Texte werden als „neueste“ gewählt.
        if (hasRecentCustomerTurnAfterLastSent) {
          let bestMsg = null;
          if (cherryNewestAtTop) {
            for (let ci = 0; ci < orderedMsgs.length; ci++) {
              const m = orderedMsgs[ci];
              if (!m || !isReceived(m) || !getMessageText(m)) continue;
              if (isInfoMessage(m, { isBlenny: isBlennyOrigin })) continue;
              bestMsg = m;
              break;
            }
          } else {
            for (let ci = orderedMsgs.length - 1; ci >= 0; ci--) {
              const m = orderedMsgs[ci];
              if (!m || !isReceived(m) || !getMessageText(m)) continue;
              if (isInfoMessage(m, { isBlenny: isBlennyOrigin })) continue;
              bestMsg = m;
          ...
          if (bestMsg) {
            foundMessageText = getMessageText(bestMsg);
            console.log("✅ Cherry: foundMessageText nachgezogen (ohne anyMessages-Fallback, DOM-Reihenfolge):", foundMessageText.substring(0, 80) + "...");
          }
        }
        if (!foundMessageText || foundMessageText.trim() === "") {
          foundMessageText = "";
          console.log("✅ Cherry strict: Kein anyMessages-Fallback auf alte Kundennachrichten.");
        }
      } else if (suppressCustomerMessageFallbackForAsa) {
        foundMessageText = "";
        console.log("✅ ASA (Nachrichtenreihenfolge): Letzte Zeile vom Fake, kein Kunde danach – kein Fallback auf ältere Kundennachrichten.");
      } else {
    ...
    if (!suppressCustomerMessageFallbackForAsa && req.body?.siteInfos?.messages) {
  ...
  // Wenn die Extension ein explizites ASA-Flag liefert, ist das für Cherry zuverlässiger als
  // eine nachträgliche Rekonstruktion aus siteInfos.messages (die bei Cherry gelegentlich unvollständig/mis-ordered sein kann).
  let hasExplicitAsaFlagFromExtension = false;
  ...
  if (lastMessageFromFake !== undefined) {
    hasExplicitAsaFlagFromExtension = true;
  ...
  if (isCherryReply && cherryComputedLastMessageFromFake !== null) {
    // Für Cherry: ein explizites ASA-Flag aus der Extension NICHT überschreiben.
    // Das verhindert seltene False-Negatives (ASA wird als normale Reply behandelt), wenn siteInfos.messages unvollständig ist.
    if (hasExplicitAsaFlagFromExtension) {
      console.log("✅ Cherry strict: Extension-ASA-Flag vorhanden – überschreibe nicht mit cherryComputedLastMessageFromFake.");
    } else if (!cherryHasAnySentMessage) {
      // Wenn im Payload keine sent-Nachricht erkannt wurde, ist die Cherry-Rekonstruktion unsicher.
      // Dann lieber NICHT aggressiv auf „Kunde zuletzt“ umschalten.
      console.log("⚠️ Cherry strict: Keine sent-Nachricht im Payload erkannt – cherryComputedLastMessageFromFake wird nicht angewendet (unsicher).");
    } else {
      ...
      if (oldFlag !== isLastMessageFromFake) {
        console.log("⚠️ Cherry strict: lastMessageFromFake überschrieben von", oldFlag, "auf", isLastMessageFromFake);
      }
    }
  }
  // Harte Konsistenzregel:
  // Wenn es eine gültige received-Nachricht NACH der letzten sent/Fake-Nachricht gibt,
  // darf das kein ASA-Fall sein, selbst wenn die Extension lastMessageFromFake=true meldet.
  if (hasRecentCustomerTurnAfterLastSent) {
    if (isLastMessageFromFake) {
      console.log("⚠️ ASA-Flag überschrieben: Es gibt eine aktuelle Kundennachricht nach letzter Fake-Nachricht.");
    }
    isLastMessageFromFake = false;
  }
  // Gold/FPC/AVZ/…: Extension sendet oft kein lastMessageFromFake – aus Verlauf: Fake zuletzt, kein Kunde danach → ASA
  if (suppressCustomerMessageFallbackForAsa && !hasRecentCustomerTurnAfterLastSent) {
    isLastMessageFromFake = true;
    foundMessageText = "";
    console.log("✅ ASA aus siteInfos.messages: Zuletzt Fake, kein Kundenturn danach → isLastMessageFromFake=true, foundMessageText leer.");
  }
  ...
  const siteUrlForModsTrigger = String(req.body?.siteInfos?.url || "");
  const isModsGoldChatsxForTrigger =
    requestOrigin === "gold" ||
    requestOrigin === "diamond" ||
    requestOrigin === "s69" ||
    requestOrigin === "ml" ||
    /mods\.(gold-chat\.net|chatsx\.net)/i.test(siteUrlForModsTrigger) ||
    /mods\.(diamondchat\.net|platin-chat|onlydates69|mltestapp\.com)/i.test(siteUrlForModsTrigger);
  /** FPC, Cherry, Gold/S69: Extension Post-Typing-Timer (content.js); JSON delay/waitTime hier angleichen für Logs/Konsistenz. WM/XL (Diamond/Platin) = 3s wie Extension. */
  const useFastInsertSendDelay = isFPC || isCherryReply || isModsGoldChatsxForTrigger;
  function insertSendDelaySecForPlatform() {
    if (useFastInsertSendDelay) {
      if (isFPC) return 3; // Extension: getChataiPostTypingSendDelaySec() FPC = 3s
      return 3; // Gold/S69/Chatsx inkl. WM/XL: 3s (vormals WM/XL 30s)
    }
    const minW = 40;
    const maxW = 60;
    return Math.floor(Math.random() * (maxW - minW + 1)) + minW;
  }
  function isKussSystemMessage(m) {
    if (!m || typeof m.text !== "string") return false;
    if (isSentMessage(m)) return false;
    const tr = m.text.trim();
    const text = tr.toLowerCase();
    // Bestätigung Mod→Kunde („Du hast … einen Kuss geschickt“): kein „Kunde hat geküsst“-Danke-Trigger
    if (/^du\s+hast\s+/i.test(tr)) return false;
    // Nur eindeutige Plattform-Systemtexte – NICHT "geküsst" + "schreib" (trifft auf normale Kundennachrichten mit Kuss + "schreib mir" zu und verwechselt Like mit Kuss).
    return text.includes("ich habe dir einen kuss") ||
           ...
           text.includes("kuss nachricht") || // Gold / S69: System-Bubble „Kuss Nachricht!…“
  ...
  // Trigger-Nachricht für Kuss/Like:
  ...
  // 🚨 FIX: Zu alte System-Trigger (Gefällt/Favoriten/Gruß/Freundschaft …) dürfen nicht ewig als Auslöser dienen.
  // Kuss/Like: kein hartes 10-Min-Limit — entscheidend ist, ob danach bereits Fake geantwortet hat (discardConsumedSystemTrigger).
  try {
    const maxSystemTriggerAgeMs = 10 * 60 * 1000; // konsistent mit "Nachricht zu alt" Filter (10 Min)
    const tsRaw = newestSystemTriggerMsg && (newestSystemTriggerMsg.timestamp != null ? newestSystemTriggerMsg.timestamp : newestSystemTriggerMsg.time);
    if (newestSystemTriggerMsg && tsRaw) {
      const tms = new Date(tsRaw).getTime();
      if (!isNaN(tms) && tms > 0) {
        const ageMs = Date.now() - tms;
        const skipAgeForKussLikeThankYou =
          typeof newestSystemTriggerMsg.text === 'string' &&
          (isKussSystemMessage(newestSystemTriggerMsg) || isLikeSystemMessage(newestSystemTriggerMsg));
        if (ageMs > maxSystemTriggerAgeMs && !skipAgeForKussLikeThankYou) {
          console.log(`⚠️ System-Trigger zu alt (${Math.round(ageMs / 1000)}s) – verwerfe für Danke/Trigger:`, (newestSystemTriggerMsg.text || '').substring(0, 50));
          newestSystemTriggerMsg = null;
        } else if (ageMs > maxSystemTriggerAgeMs && skipAgeForKussLikeThankYou) {
          console.log(
            `✅ Kuss/Like-Trigger älter als 10 min (${Math.round(ageMs / 1000)}s) – behalten; Verbraucht nur, wenn Fake nach Trigger geantwortet hat.`
          );
        }
      }
    } else if (newestSystemTriggerMsg && !tsRaw && suppressCustomerMessageFallbackForAsa) {
      // Fehlende Timestamps + ASA: ohne Sonderfall wirken alte Kuss-Zeilen „frisch“ (Sortierung 0 === 0).
      // FPC und Mods Gold/ChatsX/S69/Diamond: Extension liefert Timestamps oft nicht – Kuss/Like trotzdem behalten.
      const keepKussLikeNoTsAsa =
        typeof newestSystemTriggerMsg.text === 'string' &&
        (isKussSystemMessage(newestSystemTriggerMsg) || isLikeSystemMessage(newestSystemTriggerMsg)) &&
        (isFPC || isModsGoldChatsxForTrigger);
      if (!keepKussLikeNoTsAsa) {
        console.log('⚠️ System-Trigger ohne Zeitstempel verworfen (ASA-Kontext: Fake zuletzt, kein Kundenturn):', (newestSystemTriggerMsg.text || '').substring(0, 50));
        newestSystemTriggerMsg = null;
      } else {
        console.log('✅ Kuss/Like-Systemzeile ohne Timestamp behalten (FPC/Mods-Gold/ChatsX – Danke-/Trigger-Pfad).');
      }
    }
  } catch (eAgeTrig) { /* ignore */ }
  // Schutz gegen "alten Kuss/Like im Verlauf":
  // Wenn die neueste sent-Nachricht zeitlich NACH dem möglichen System-Trigger liegt,
  // ist der Trigger bereits "verbraucht" und darf nicht erneut die Pipeline auslösen.
  ...
  const newestSentTs = newestSentMsgByTime && (newestSentMsgByTime.timestamp != null || newestSentMsgByTime.time != null)
    ...
    : null;
  // Kuss/Like-Danke nur, wenn der System-Trigger noch nicht durch eine neuere Fake-Zeile „beantwortet“ wurde.
  // Standard: Timestamp-Vergleich. Cherry: laut Kommentar oben oft unzuverlässige Zeitstempel → zusätzlich
  // dieselbe UI-Logik wie cherryLastSentIdx / cherryLastReceivedIdx (neueste Nachricht oben = kleinerer Index).
  let discardConsumedSystemTrigger = false;
  let discardConsumedSystemTriggerReason = "";
  if (newestSystemTriggerMsg && triggerTs != null && !isNaN(triggerTs) && newestSentTs != null && !isNaN(newestSentTs) && newestSentTs > triggerTs) {
    discardConsumedSystemTrigger = true;
    discardConsumedSystemTriggerReason = "Zeitstempel";
  }
  // Gold / ChatsX / S69 / Diamond: Extension hat oft fehlende Fake-Zeitstempel → reiner Timestamp-Vergleich verwirft den Kuss nicht,
  // obwohl die chronologisch letzte Zeile im Thread bereits vom Fake ist (ASA). Gleiche Normalisierung wie siteInfos.messages [älteste…neueste].
  if (!discardConsumedSystemTrigger && newestSystemTriggerMsg && msgsAll.length > 0) {
    if (isModsGoldChatsxForTrigger) {
      let newestFirst = false;
      if (msgsAll.length > 1) {
        try {
          const firstTs = msgsAll[0]?.timestamp ? new Date(msgsAll[0].timestamp).getTime() : null;
          const lastTs = msgsAll[msgsAll.length - 1]?.timestamp ? new Date(msgsAll[msgsAll.length - 1].timestamp).getTime() : null;
          if (firstTs && lastTs && firstTs > lastTs) newestFirst = true;
        } catch (e) { /* ignore */ }
      }
      const orderedForTrigger = newestFirst ? [...msgsAll].reverse() : msgsAll;
      const lastInThread = orderedForTrigger[orderedForTrigger.length - 1];
      if (lastInThread && isSentMessage(lastInThread)) {
        discardConsumedSystemTrigger = true;
        discardConsumedSystemTriggerReason = "Mods/ChatsX: letzte Zeile im Verlauf ist Fake (sent)";
      }
    }
  }
  if (
    !discardConsumedSystemTrigger &&
    isCherryReply &&
    newestSystemTriggerMsg &&
    cherryLastSentIdxDebug >= 0 &&
    cherryLastReceivedIdxDebug >= 0
  ) {
    // Wenn der Kuss zeitlich eindeutig NACH der jüngsten bekannten Fake-Zeile liegt, Index-Heuristik nicht zum Verwerfen nutzen
    // (sonst kann ein frischer Kuss fälschlich „verbraucht“ wirken, wenn oben noch eine ältere Fake-Bubble steht).
    const kissStrictlyAfterLatestSentTs =
      triggerTs != null &&
      !isNaN(triggerTs) &&
      newestSentTs != null &&
      !isNaN(newestSentTs) &&
      triggerTs > newestSentTs;
    if (!kissStrictlyAfterLatestSentTs) {
      const cherryNewestAtTop = true; // wie cherryNewestAtTop bei isCherryOrigin (Cherry = neueste oben)
      const sentIsNewerInUiThanNewestReceived = cherryNewestAtTop
        ? cherryLastSentIdxDebug < cherryLastReceivedIdxDebug
        : cherryLastSentIdxDebug > cherryLastReceivedIdxDebug;
      if (sentIsNewerInUiThanNewestReceived) {
        discardConsumedSystemTrigger = true;
        discardConsumedSystemTriggerReason = "Cherry-Index (neueste oben)";
      }
    }
  }
  if (discardConsumedSystemTrigger && newestSystemTriggerMsg) {
    console.log(
      "⚠️ System-Trigger verworfen: neueste Fake-Nachricht nach Kuss/Like-Systemnachricht (" +
        discardConsumedSystemTriggerReason +
        ")."
    );
    newestSystemTriggerMsg = null;
  }
  // Nur wenn die AKTUELL relevante received-Nachricht eine Systemnachricht ist → Kuss/Like/Gefällt/Favoriten-Pipeline.
  ...
  // Like-Benachrichtigung (z. B. "geliked") hat Vorrang, falls beides fälschlich true wäre
  if (hasLikeSystemMessage && hasKussSystemMessage && newestSystemTriggerMsg && typeof newestSystemTriggerMsg.text === "string") {
    ...
    if (/\bgeliked\b|hat dich geliked|\bgelikt\b|like erhalten|ich habe dir einen like|der benutzer hat dich geliked/.test(nt)) {
      ...
      console.log("✅ Neueste Nachricht eindeutig Like – Kuss-Erkennung für diese Bubble deaktiviert.");
    }
  }
  ...
  function isFreundschaftsanfrageSystemText(text) {
    ...
    const isStandaloneFriendRequest = /^friend\s*request[!?.:]*$/.test(trimmed);
    // Reine Kundensätze wie "hast du meine freundschaftsanfrage gesehen?" dürfen KEINEN Danke-Trigger auslösen.
    const conversationalContext =
      /\b(ich|du|wir|mein|meine|dein|deine|mir|dir|hast du|habe ich)\b/.test(t) &&
      !/\b(der benutzer|hat dir|system|hinweis)\b/.test(t);
    if (conversationalContext && !isStandaloneFriendRequest) return false;
    return isStandaloneFriendRequest ||
      /der benutzer .*möchte dich als freund hinzuf(ü|ue)gen/.test(t) ||
      /benutzer .*möchte dich als freund hinzuf(ü|ue)gen/.test(t) ||
      /hat dir eine freundschaftsanfrage (gesendet|geschickt)/.test(t) ||
      /freundschaftsanfrage (gesendet|geschickt|annehmen|akzeptiert|abgelehnt|best(ä|a)tigt)/.test(t);
  }
  const sentMsgsForTriggerFallback = msgsAll.filter(m => isSentMessage(m) && !isInfoMessage(m, { isBlenny: isBlennyRequest })).length;
  function getThankYouTriggerType(newestMsg, foundMsg) {
    const found = (typeof foundMsg === 'string' ? foundMsg.trim() : '') || '';
    // ASA-Kontext (suppressCustomerMessageFallbackForAsa): Veraltete Trigger werden oben per Alter
    // und „Fake neuer als Trigger“ bereits verworfen (newestSystemTriggerMsg = null). Kuss/Like hier
    // NICHT pauschal nullen – sonst FPC/Gold: frischer Kuss/Like wird nie thankYouTriggerType.
    if (suppressCustomerMessageFallbackForAsa) {
      const preStr = newestMsg && (newestMsg.text || newestMsg.content || newestMsg.message || '');
      if (typeof preStr === 'string' && isGrußMessage(preStr)) return null;
      if (found && isGrußMessage(found)) return null;
      // NICHT „found“ mit Kuss-Zeile hier pauschal nullen – das lief VOR der newestMsg-Prüfung und blockierte Gold/Mods.
    }
    ...
    if (str) {
      ...
      // Like vor Kuss prüfen (FPC: "geliked" + "Schreibe …" – kein Kuss)
    ...
    // Blenny/DF: Gruß kann als foundMessageText ankommen (wenn in messages als Info gefiltert) – trotzdem erkennen
    if (found && isGrußMessage(found)) return 'gruß';
    ...
    // Gold/S69/ChatsX: "foundMessageText=Kuss Nachricht" darf alte Trigger NICHT reaktivieren.
    // Nur erlauben, wenn es ein echter Erstkontakt ohne Fake-Sendeverlauf ist.
    if (found && found.length < 220 && /^\s*kuss\s+nachricht\b/i.test(found)) {
      if (sentMsgsForTriggerFallback === 0) return 'kuss';
      return null;
    }
    // Cherry (und vereinzelt andere): System-Kuss kann als foundMessageText "wie echte Nachricht" ankommen.
    // Wenn das Message-Objekt nicht sauber klassifiziert werden kann, trotzdem Danke-/Kuss-Pfad nutzen,
    // damit die KI nicht "Oh X hat dir einen Kuss geschickt" paraphrasiert.
    if (found && found.length < 260) {
      const fl = found.toLowerCase();
      // Bestätigung Mod→Kunde („Du hast … einen Kuss geschickt“) darf KEIN Danke-Trigger sein.
      if (!/^\s*du\s+hast\s+/i.test(found)) {
        if (
          fl.includes("ich habe dir einen kuss") ||
          fl.includes("der benutzer hat dich geküsst") ||
          fl.includes("benutzer hat dich geküsst") ||
          fl.includes("hat dich geküsst") ||
          fl.includes("hat dir einen kuss geschickt") ||
          fl.includes("hat dir ein kuss geschickt") ||
          fl.includes("hat dir einen kuss gesendet") ||
          fl.includes("hat dir ein kuss gesendet")
        ) {
          return 'kuss';
        }
      }
  ...
  // ASA-Hartregel:
  // Wenn isLastMessageFromFake=true (ASA) und kein aktueller, gültiger System-Trigger vorhanden ist,
  // darf KEIN Danke-Trigger aus älterem Verlauf/newestMsgByTime/foundMessageText reaktiviert werden.
  const allowFallbackTriggerFromContext = !isLastMessageFromFake && !suppressCustomerMessageFallbackForAsa;
  const triggerCandidateMsg = newestSystemTriggerMsg || (allowFallbackTriggerFromContext ? newestMsgByTime : null);
  let thankYouTriggerType = getThankYouTriggerType(triggerCandidateMsg, foundMessageText);
  if (isLastMessageFromFake && !newestSystemTriggerMsg && thankYouTriggerType) {
    console.log("⚠️ Danke-Auslöser verworfen: ASA-Fall ohne gültigen aktuellen System-Trigger.");
    thankYouTriggerType = null;
  }
  if (thankYouTriggerType) console.log('✅ Danke-Auslöser (einheitlich):', thankYouTriggerType);
  // FPC: Grüner/gelber Hinweis („Der Benutzer hat dich geliked/geküsst…“) steht oft nur in alertBoxMessages, nicht in messages.
  if (!thankYouTriggerType && isFPC && isLastMessageFromFake) {
    const abRaw = req.body?.siteInfos?.metaData?.alertBoxMessages || req.body?.siteInfos?.alertBoxMessages;
    const abList = Array.isArray(abRaw) ? abRaw : [];
    for (let abi = abList.length - 1; abi >= 0; abi--) {
      const raw = typeof abList[abi] === 'string' ? abList[abi] : (abList[abi] && (abList[abi].text || abList[abi].message || abList[abi].content)) || '';
      const t = String(raw).toLowerCase();
      if (!t.trim()) continue;
      if (/\bgeliked\b|hat dich geliked|der benutzer hat dich geliked/.test(t)) {
        thankYouTriggerType = 'like';
        console.log('✅ FPC: Danke-Auslöser „like“ aus alertBoxMessages.');
        break;
      }
      if (/geküsst|ich habe dir einen kuss|der benutzer hat dich geküsst|benutzer hat dich geküsst|hat dich geküsst/.test(t)) {
        thankYouTriggerType = 'kuss';
        console.log('✅ FPC: Danke-Auslöser „kuss“ aus alertBoxMessages.');
        break;
      }
    }
  }
  ...
  // Struktur-Schutz gegen Falsch-ASA: Nach der letzten sent-Zeile gibt es noch eine relevante received (ohne 10-Min-Zeitfilter).
  // Cherry: eigene Final-Logik weiter unten – hier nicht eingreifen.
  const skipStructuralAsaGuard =
    isCherryReply ||
  ...
  if (!skipStructuralAsaGuard && isLastMessageFromFake && msgsAll.length > 0) {
    const pageUrlForStruct = pageUrlFromBody || req.body?.pageUrl || "";
    const isAVZOriginStruct =
      (req.body?.siteInfos?.origin &&
        (String(req.body.siteInfos.origin).toLowerCase().includes("avz") ||
          String(req.body.siteInfos.origin).toLowerCase().includes("chathomebase"))) ||
      (platformIdFromBody &&
        (String(platformIdFromBody).toLowerCase().includes("avz") ||
          String(platformIdFromBody).toLowerCase().includes("chathomebase"))) ||
      (pageUrlForStruct &&
        (String(pageUrlForStruct).toLowerCase().includes("chathomebase") ||
          String(pageUrlForStruct).toLowerCase().includes("avz")));
    const structTurn = structuralCustomerTurnAfterLastFakeFromRaw(msgsAll, {
      isBlennyOrigin: isBlennyRequest,
      isIluvo: originForImage === "iluvo",
      isAVZOrigin: isAVZOriginStruct,
    });
    if (structTurn.hasTurn) {
      isLastMessageFromFake = false;
      suppressCustomerMessageFallbackForAsa = false;
      if (!foundMessageText || foundMessageText.trim() === "") {
        if (structTurn.customerText && structTurn.customerText.trim() !== "") {
          foundMessageText = structTurn.customerText;
        } else if (structTurn.isImageOnly) {
          foundMessageText = "Der Kunde hat ein Bild geschickt.";
        }
      }
      console.log("✅ ASA-Schutz (Struktur): received nach letzter Fake – ASA abgebrochen (kein Falsch-ASA).");
    }
  }
  ...
  if (isCherryRuntime) {
    ...
    if (hasRecentCustomerTurnAfterLastSent) {
      if (isLastMessageFromFake) {
        console.log("⚠️ Cherry final: ASA auf false erzwungen (aktuelle Kundennachricht nach letzter Fake vorhanden).");
      }
      isLastMessageFromFake = false;
    } else if ((cherryHasAnySentMessage || cherryComputedLastMessageFromFake === true) && !thankYouTriggerType) {
      if (foundMessageText && foundMessageText.trim() !== "") {
        console.log("⚠️ Cherry final: Alter/inkonsistenter Kundentext verworfen, da keine received nach letzter Fake vorhanden ist.");
      }
      isLastMessageFromFake = true;
      foundMessageText = "";
    } else if (thankYouTriggerType) {
      // Kuss/Like/etc. hat Vorrang vor generischer Cherry-ASA-Erzwingung.
      if (isLastMessageFromFake) console.log("✅ Cherry final: Thank-You-Trigger aktiv, ASA-Erzwingung übersprungen.");
      isLastMessageFromFake = false;
    }
    try {
      ...
      console.warn('⚠️ Cherry decision file log fehlgeschlagen:', e.message);
    }
    // Immer eine einzelne, kurze Render-Log-Zeile ausgeben (einfach suchbar über chatId).
    console.log(
      "CHERRY_DECISION_RESULT chatId=" + cherryDebugChatId +
      " asa=" + (isLastMessageFromFake ? "1" : "0") +
      " trigger=" + String(thankYouTriggerType || "-") +
      " hasTurn=" + (hasRecentCustomerTurnAfterLastSent ? "1" : "0") +
      " sentIdx=" + String(cherryLastSentIdxDebug) +
      " recvIdx=" + String(cherryLastReceivedIdxDebug) +
      " text=" + JSON.stringify((foundMessageText || "").slice(0, 90))
    );
  }
  ...
  // ChatsX (S69): Extension/In() liefert oft noch "gold"; Sheets/Feedback sollen "S69" nutzen
  try {
    const urlS69 = String(pageUrl || req.body?.pageUrl || req.body?.siteInfos?.url || req.body?.url || "").toLowerCase();
    if (urlS69.includes("chatsx.net") && String(platformId || "").toLowerCase() === "gold") {
      platformId = "S69";
    }
    if (urlS69.includes("chatsx.net") && !platformId) {
      platformId = "S69";
    }
    if (urlS69.includes("chatsx.net") && req.body?.siteInfos && typeof req.body.siteInfos === "object") {
      req.body.siteInfos.origin = "S69";
    }
  } catch (_) {}
  // WM (Diamond) / XL (Platin): Extension liefert oft "gold"; Sheets/Feedback sollen "WM" bzw. "XL" (wie ChatsX → "S69")
  try {
    const urlWmXl = String(pageUrl || req.body?.pageUrl || req.body?.siteInfos?.url || req.body?.url || "").toLowerCase();
    if (urlWmXl.includes("diamondchat.net")) {
      platformId = "WM";
      if (req.body?.siteInfos && typeof req.body.siteInfos === "object") {
        req.body.siteInfos.origin = "WM";
      }
    } else if (urlWmXl.includes("platin-chat")) {
      platformId = "XL";
      if (req.body?.siteInfos && typeof req.body.siteInfos === "object") {
        req.body.siteInfos.origin = "XL";
      }
    } else if (urlWmXl.includes("mltestapp.com")) {
      platformId = "ML";
      if (req.body?.siteInfos && typeof req.body.siteInfos === "object") {
        req.body.siteInfos.origin = "ML";
      }
    }
  } catch (_) {}
  ...
  // Plattform-Systemhinweis (Wunschzettel/Favoriten) darf NICHT als Kundennachricht beantwortet werden.
  // Statt KI-Antwort: rote Meldung "Mensch eingreifen".
  function isWishlistSystemNoticeText(text) {
    if (!text || typeof text !== "string") return false;
    const t = text.toLowerCase().replace(/\s+/g, " ").trim();
    if (!t) return false;
    const hasWishOrFav = /wunschzettel|favoriten|favourites|favorites/.test(t);
    const hasSystemLead = /wir m(ö|oe)chten dir mitteilen|system|hinweis/.test(t);
    const hasAddVerb = /hinzugef(ü|ue)gt|zugef(ü|ue)gt|zuf(ü|ue)gen/.test(t);
    // ChatsX/S69-Template: "Wir möchten dir mitteilen, dass ... zu seinen/ihren Wunschzettel hinzugefügt hat ..."
    const hasZuSeinenIhren =
      t.includes("zu seinen/ihren") ||
      (t.includes("zu seinen") && t.includes("favoriten")) ||
      (t.includes("zu ihren") && t.includes("favoriten"));
    return hasWishOrFav && hasAddVerb && (hasSystemLead || hasZuSeinenIhren);
  }
  const wishlistSystemCandidates = collectWishlistSystemCandidateTexts(req.body, foundMessageText);
  const wishlistSystemDetectedText = wishlistSystemCandidates.find(v => isWishlistSystemNoticeText(v));
  if (wishlistSystemDetectedText) {
    console.warn("⛔ Systemhinweis (Wunschzettel/Favoriten) erkannt – keine KI-Antwort, Mensch eingreifen.");
    return res.status(200).json({
      error: "Mensch eingreifen: Systemhinweis (Wunschzettel/Favoriten) erkannt. Bitte manuell reagieren.",
      resText: "",
      replyText: "",
      summary: {},
      chatId: finalChatId,
      ...
      flags: {
        blocked: true,
        reason: "wishlist_system_notice",
        isError: true,
        showError: true,
        requiresAttention: true,
        errorType: "manual_review",
        errorColor: "red",
        errorStyle: "critical",
        errorMessage: "Mensch eingreifen: Diese Systemnachricht soll nicht automatisch beantwortet werden."
      }
    });
  }

  // Prüfe auf KI-/Validierungs-Check (HÖCHSTE PRIORITÄT) — volle Texte inkl. siteInfos.messages-Tail, nichts kürzen
  ...
  if (isKICheck) {
    ...
    const errorMessage = "🚨 BLOCKIERT: KI-/Validierungs-Check erkannt!\n\nDie Kundennachricht verlangt eine Zahlencode-Antwort (z. B. Validierungscheck). Das darf die KI nicht übernehmen.\n\nBitte die Zahl(en) manuell eintippen und selbst absenden.\n\nEs wird KEINE automatische Antwort generiert.";
  ...
  const minorSafety =
    foundMessageText && foundMessageText.trim() !== ""
      ? checkMinorMention(foundMessageText)
      : { isBlocked: false, reason: null, errorMessage: null };
  if (minorSafety.isBlocked) {
    const reason = minorSafety.reason || "minor_or_illegal";
    const errorMessage =
      minorSafety.errorMessage || "🚨 BLOCKIERT: Minderjährige oder strafrechtliche Themen erkannt!";
    console.error("🚨🚨🚨 BLOCKIERT: Minderjährige oder strafrechtliche Themen erkannt! Grund:", reason);
    console.error("🚨 Erkannte Nachricht:", foundMessageText.substring(0, 200));
  ...
  // Zusätzlicher KI-Safety-Check (semantisch): Minderjährige erkennen, auch bei Umschreibungen.
  // Wichtig: Regex bleibt als schneller Vorfilter; LLM schließt Lücken.
  async function checkMinorMentionViaLLM(client, text) {
    try {
      if (!client || !text || typeof text !== "string" || text.trim() === "") {
        return { isBlocked: false, reason: null, errorMessage: null };
      }
      const enableLLMMinorCheck = String(process.env.ENABLE_LLM_MINOR_CHECK || "true").toLowerCase() !== "false";
      if (!enableLLMMinorCheck) return { isBlocked: false, reason: null, errorMessage: null };

      // Trigger: Nur wenn es nach Alters-/Minderjährigen-Thema aussieht (Kosten sparen).
      const lower = text.toLowerCase();
      const hasAgeOrMinorCue =
        /\b(1[0-9]|[2-9][0-9])\b/.test(lower) ||
        /\b(minderj|vollj|unter\s*18|noch\s*nicht\s*18|jugend|teen|schul|klasse|abi|abschluss)\b/i.test(lower) ||
        /\b(ich\s+bin|bin|werde|wurd|wuerd)\b/.test(lower);
      if (!hasAgeOrMinorCue) return { isBlocked: false, reason: null, errorMessage: null };

      const prompt = [
        "Du bist ein Safety-Klassifikator fuer Dating-Chat-Nachrichten.",
        "Aufgabe: Erkenne, ob der Kunde SELBST unter 18 ist oder klar sagt, noch nicht volljaehrig zu sein.",
        "",
        "Wichtig:",
        '- Blockiere NUR wenn mit hoher Sicherheit gilt: Kunde ist unter 18 (minderjaehrig).',
        "- Wenn unklar/mehrdeutig: gib is_minor=false und confidence niedrig.",
        "- Nicht raten, nicht interpretieren als waere der Kunde minderjaehrig, wenn es nur um eine andere Person geht.",
        "",
        "Gib NUR JSON im Format:",
        '{"is_minor": boolean, "confidence": number, "evidence": string}',
      ].join("\n");
      ...
      try {
        ...
        // Fallback: versuche JSON aus Text zu ziehen
        const m = raw.match(/\{[\s\S]*\}/);
        if (m) obj = JSON.parse(m[0]);
      }
      const isMinor = !!obj?.is_minor;
      const conf = typeof obj?.confidence === "number" ? obj.confidence : 0;
      if (isMinor && conf >= 0.65) {
        return {
          isBlocked: true,
          reason: "minor_llm",
      ...
      console.warn("⚠️ LLM Minor-Check fehlgeschlagen (optional):", err?.message || err);
      return { isBlocked: false, reason: null, errorMessage: null };
    }
  }
  ...
  // LLM-Minor-Check nach Client-Init (falls Regex nichts gefunden hat)
  if (foundMessageText && foundMessageText.trim() !== "") {
    const llmMinor = await checkMinorMentionViaLLM(client, foundMessageText);
    if (llmMinor?.isBlocked) {
      const reason = llmMinor.reason || "minor_or_illegal";
      const errorMessage = llmMinor.errorMessage || "🚨 BLOCKIERT: Minderjährige oder strafrechtliche Themen erkannt!";
      console.error("🚨🚨🚨 BLOCKIERT (LLM): Minderjährige erkannt! Grund:", reason);
      console.error("🚨 Erkannte Nachricht:", foundMessageText.substring(0, 200));
      return res.status(200).json({
        error: errorMessage,
        resText: "",
  ...
  // isCherryReply wird bereits früh aus requestOrigin berechnet.
  ...
  /** AVZ/FPC: assetsToSend enthält Bild(er), aber keine URL konnte extrahiert werden – trotzdem als "Kunde hat Bild geschickt" behandeln */
  let customerSentImageFromAssetsNoUrl = false;
  let customerImageEvidenceThisTurn = false;
  let customerAssetCandidateSeen = false;
  try {
    // #region agent log
    ...
    if (imageUrls.length > 0) customerImageEvidenceThisTurn = true;
    const siteMsgsForImage = Array.isArray(req.body?.siteInfos?.messages) ? req.body.siteInfos.messages : [];
    const messageHasImageFieldStrict = (m) => !!(m?.image || m?.imageUrl || m?.imageSrc || m?.image_url || m?.mediaUrl || m?.src ||
      (m?.url && (String(m.url).match(/\.(png|jpg|jpeg|gif|webp)/i) || String(m.url).startsWith('http'))) ||
      m?.attachment || m?.attachments || m?.media);
    let latestReceivedMsg = null;
    for (let ri = siteMsgsForImage.length - 1; ri >= 0; ri--) {
      const mm = siteMsgsForImage[ri];
      if (mm?.type === "received" || mm?.messageType === "received") { latestReceivedMsg = mm; break; }
    }
    const avzCurrentCustomerHasImage = isAVZ ? !!(latestReceivedMsg && messageHasImageFieldStrict(latestReceivedMsg)) : true;
    ...
    // AVZ/FPC können andere Property-Namen nutzen (blobUrl, objectUrl, dataUrl, content, file)
    if (imageUrls.length === 0 && assetsToSend && Array.isArray(assetsToSend) && assetsToSend.length > 0) {
      if (isAVZ && !avzCurrentCustomerHasImage) {
        console.log("ℹ️ AVZ: Letzte Kundennachricht hat kein Bild – assetsToSend wird für Bildanalyse ignoriert.");
      } else {
      for (const asset of assetsToSend) {
        if (!asset || typeof asset !== 'object') continue;
        const senderMeta = String(asset.senderType || asset.messageType || asset.sourceType || asset.senderRole || asset.sender || "").toLowerCase().trim();
        const hasSenderMeta = senderMeta.length > 0;
        const isCustomerSender = senderMeta.includes("customer") || senderMeta === "received";
        const isNonCustomerSender = senderMeta.includes("moderator") || senderMeta.includes("assistant") || senderMeta === "sent" || senderMeta.includes("profile");
        if ((isAVZ || isFPC || isCherryReply) && hasSenderMeta && isNonCustomerSender) continue;
        if ((isAVZ || isFPC || isCherryReply) && hasSenderMeta && !isCustomerSender) continue;
        if ((isAVZ || isFPC || isCherryReply) && isCustomerSender) customerAssetCandidateSeen = true;
        ...
        if (assetUrl && typeof assetUrl === 'string') {
          ...
          if (isHttpImage || isDataImage) {
            ...
            customerImageEvidenceThisTurn = true;
      ...
      if (imageUrls.length === 0 && assetsToSend.length > 0) {
        console.log("🔍 assetsToSend hat " + assetsToSend.length + " Element(e), aber keine Bild-URL extrahiert. Erste Asset-Keys:", typeof assetsToSend[0] === 'object' && assetsToSend[0] ? Object.keys(assetsToSend[0]) : []);
      }
      }
    }
    ...
    if (imageUrls.length === 0 && req.body?.siteInfos?.messages) {
      ...
      if (isCherryReply && Array.isArray(msgs) && msgs.length > 0) {
        ...
        for (let j = lastSentIdx + 1; j < msgs.length; j++) {
          ...
          const t = getTxt(mm);
          if (t && !/^bild\s*undefined$/i.test(t) && !/^\s*(ein\s+bild\s+wurde\s+übertragen|bild\s+wurde\s+übertragen)\s*$/i.test(t)) {
      ...
      if (currentCustomerMessage && messageHasImageField(currentCustomerMessage)) {
        if (isCherryReply && cherryHasCustomerTextAfterLastSent && !getTxt(currentCustomerMessage)) {
          console.log("⚠️ Cherry: Bild ohne Text im Kundenturn, aber Text-Nachricht nach letzter Fake erkannt – Bild-Kontext wird ignoriert (vermutlich Reaktion auf Fake-Bild).");
        } else {
    ...
    // AVZ/FPC Fallback: assetsToSend enthält Bild(er), aber keine URL extrahiert (z. B. anderes Asset-Format) – trotzdem als "Bild mitgeschickt" behandeln
    if (imageUrls.length === 0 && assetsToSend && Array.isArray(assetsToSend) && assetsToSend.length > 0 && !isLastMessageFromFake && (isAVZ || isFPC)) {
      if (isAVZ && !avzCurrentCustomerHasImage) {
        console.log('ℹ️ AVZ: Kein assetsToSend-Fallback, da letzte Kundennachricht kein Bild enthält.');
      } else if (!customerAssetCandidateSeen) {
        console.log('⚠️ AVZ/FPC: assetsToSend ohne sicheren Kunden-Sender – kein Bild-Fallback für Kunde.');
      } else {
        ...
        console.log('✅ AVZ/FPC: assetsToSend(' + assetsToSend.length + ') ohne extrahierbare URL – Bild-Kontext gesetzt (Fallback).');
      }
    }
    // #region agent log
    ...
    let dataUrl = null;
    if ((isAVZ || isFPC || isCherryReply) && imageUrls.length > 0 && !customerImageEvidenceThisTurn) {
      console.log("⚠️ Bild im Request gefunden, aber kein sicherer Kunden-Nachweis (received/customer) – ignoriere Bild-Kontext.");
      imageUrls = [];
    }
    ...
    // Vision-API unterstützt nur PNG/JPEG/GIF/WebP – SVG (z. B. AVZ/chathomebase Platzhalter) führt zu 400
    ...
    if (isUnsupportedFormat) {
      console.log("📸 Bild-URL ist SVG oder anderes nicht unterstütztes Format – überspringe Vision-Analyse, setze Fallback.");
      imageDescriptions.push("Der Kunde hat mit dieser Nachricht ein Bild geschickt. (Format wird von der Analyse nicht unterstützt.) PFLICHT: In deiner Antwort ZWINGEND auf das Bild eingehen: Begeisterung zeigen, z.B. wie geil du das findest oder kurze Frage zum Bild. Auf BEIDES eingehen – Text und Bild.");
    }
    ...
    if (dataUrl) {
        ...
        if (isCherryReply && (imageType === 'face' || imageType === 'body' || imageType === 'nude')) {
          console.log(`⚠️ Cherry: Bildtyp ${imageType} erkannt – Bildkontext wird vorsorglich ignoriert (mögliche Reaktion auf Fake-Bild).`);
          imageUrls = [];
          dataUrl = null;
          imageType = 'unknown';
    ...
    // Trotzdem Fallback setzen, damit die Pipeline auf „Bild geschickt“ reagiert (z. B. bei 400 unsupported image/SVG)
    if (imageUrls.length > 0 && imageDescriptions.length === 0) {
      imageDescriptions.push("Der Kunde hat mit dieser Nachricht ein Bild geschickt. (Inhalt konnte nicht geladen werden – Format oder Abruf fehlgeschlagen.) PFLICHT: In deiner Antwort ZWINGEND auf das Bild eingehen: Begeisterung zeigen, z.B. wie geil du das findest oder kurze Frage zum Bild. Auf BEIDES eingehen – Text und Bild.");
      console.log("📸 Fallback-Bildbeschreibung nach Fehler gesetzt.");
    }
  }
  ...
  // 🚨 SCHRITT 2: KI-/Validierungs-Check (HÖCHSTE PRIORITÄT - vor allen anderen Checks!)
  ...
  if (isKICheck2) {
      console.error("🚨🚨🚨 BLOCKIERT: KI-/Validierungs-Check in Kundennachricht erkannt! 🚨🚨🚨");
      const errorMessage = "🚨 BLOCKIERT: KI-/Validierungs-Check erkannt!\n\nDie Kundennachricht verlangt eine Zahlencode-Antwort (z. B. Validierungscheck). Das darf die KI nicht übernehmen.\n\nBitte die Zahl(en) manuell eintippen und selbst absenden.\n\nEs wird KEINE automatische Antwort generiert.";
      return res.status(200).json({
        error: errorMessage,
        resText: "",
  ...
  if (rules) {
    const rulesViluu = mergeViluuMeetingRulesIntoRules(rules, {
      platformId,
      siteInfos: req.body?.siteInfos,
      pageUrl,
      bodyOrigin: req.body?.origin
    });
    if (rulesViluu !== rules) {
      rules = rulesViluu;
      console.log("✅ Viluu: strenge Treffen/Date/Zeit-Regeln in Regeln gemerged (nur diese Plattform)");
    }
  }
  ...
  try {
    ...
    if (trainingData && trainingData.conversations && Array.isArray(trainingData.conversations)) {
      ...
      // Vektor-DB: wird beim Server-Start initialisiert (app.js), nicht bei jedem Request (vermeidet Flattern/Races)
    } else {
  ...
  try {
    if (isLastMessageFromFake) {
      ...
      const nowAsa = Date.now();
      const maxSystemTriggerAgeMs = 10 * 60 * 1000; // konsistent mit "Nachricht zu alt" (10 Min)
      ...
      const relevantSystemMessages = messages
        .filter(m => {
          ...
          // 🚨 FIX: zu alte Systemnachrichten (z.B. alter Kuss) ignorieren
          try {
            const tsV = (m.timestamp != null ? m.timestamp : (m.time != null ? m.time : null));
            if (tsV) {
              const tms = new Date(tsV).getTime();
              if (!isNaN(tms) && tms > 0) {
                const ageMs = nowAsa - tms;
                if (ageMs > maxSystemTriggerAgeMs) return false;
              }
            }
          } catch (eTs) {}
          const text = m.text.toLowerCase();
          // Prüfe auf Kuss-System-Nachrichten (gleiche Logik wie isKussSystemMessage – keine "geküsst+schreib"-Heuristik auf Kundentext)
          const isKussMessage = text.includes("ich habe dir einen kuss") ||
                               ...
                               text.includes("hat dir einen kuss gesendet") ||
                               text.includes("hat dir ein kuss gesendet") ||
                               text.includes("kuss nachricht") || // Gold/S69: „Kuss Nachricht!…“ (wie isKussSystemMessage)
                               text.includes("hat dir einen gruß gesendet") ||
                               text.includes("hat dir ein gruß gesendet");
      ...
      if (relevantSystemMessages.length > 0) {
        const cand = relevantSystemMessages[0];
        const candTs = cand.timestamp != null ? new Date(cand.timestamp).getTime() : (cand.time != null ? new Date(cand.time).getTime() : null);
        const sentList = messages.filter(m => m && isSentMessage(m));
        let newestSentTs = null;
        for (let si = 0; si < sentList.length; si++) {
          const sm = sentList[si];
          const sv = sm.timestamp != null ? sm.timestamp : sm.time;
          if (!sv) continue;
          const st = new Date(sv).getTime();
          if (!isNaN(st) && st > 0 && (newestSentTs == null || st > newestSentTs)) newestSentTs = st;
        }
        // Wie bei Kuss/Like-Trigger: nur reagieren, wenn die Systemzeile noch „aktuell“ ist (keine neuere Fake-Nachricht danach)
        if (candTs != null && !isNaN(candTs) && newestSentTs != null && !isNaN(newestSentTs) && newestSentTs > candTs) {
          console.log('⚠️ ASA: Kuss/Like-Systemnachricht für Kontext verworfen – neuere Fake-Nachricht liegt zeitlich danach (veralteter Kuss/Like im Verlauf).');
          asaSystemMessage = "";
          conversationContextForPipeline = "";
        } else {
          asaSystemMessage = cand.text.trim();
          console.log(`✅ ASA-System-Nachricht gefunden: "${asaSystemMessage.substring(0, 100)}..."`);
          // Nutze die System-Nachricht als Kontext für ASA – explizit Like vs. Kuss (Multi-Agent/Grok verwechselt sonst leicht)
          ...
          if (/\bgeliked\b|hat dich geliked|\bgelikt\b|like erhalten|ich habe dir einen like|der benutzer hat dich geliked/.test(al)) {
            asaKindHint = "\n\n[Wichtig: Auslöser war ein LIKE (Profil-Like). Bedanke dich für den Like / die Aufmerksamkeit. Verboten: „Kuss“, „küssen“, „Danke für den Kuss“.]";
          } else if (/geküsst|ich habe dir einen kuss|hat dir (einen|ein) kuss (geschickt|gesendet)|der benutzer hat dich geküsst|kuss\s*nachricht/.test(al)) {
            asaKindHint = "\n\n[Wichtig: Auslöser war ein KUSS. Bedanke dich für den Kuss. Verboten: „Like“, „geliked“ als Dank.]";
          }
  ...
  if (client && foundMessageText && foundMessageText.trim() !== "") {
    try {
      ...
      if (extractedInfo.user && foundMessageText && foundMessageText.trim()) {
        const ft = foundMessageText.trim();
        if (extractedInfo.user.Name && !extractedCustomerNamePlausibleInMessage(String(extractedInfo.user.Name), ft)) {
          delete extractedInfo.user.Name;
          console.log("📝 Kunden-Name aus Extraktion verworfen (kommt nicht im Kundentext vor)");
        }
        if (extractedInfo.user.Kosename && !extractedCustomerNamePlausibleInMessage(String(extractedInfo.user.Kosename), ft)) {
          delete extractedInfo.user.Kosename;
        }
        enrichCustomerFamilyWithRelationshipHistory(extractedInfo.user, ft);
      }
  ...
  if (req.body?.siteInfos?.metaData) {
    ...
    const metaUserSansName = { ...(metaSummary.user || {}) };
    delete metaUserSansName.Name;
    delete metaUserSansName.Kosename;
    extractedInfo.user = extractedInfo.user && typeof extractedInfo.user === "object" ? extractedInfo.user : {};
    // Stammdaten aus Profil ergänzen, aber Name/Kosename nur aus Kundennachricht-Extraktion (oben).
    if (Object.keys(metaUserSansName).length > 0) {
      for (const k of Object.keys(metaUserSansName)) {
        const v = metaUserSansName[k];
        if (v == null || v === "") continue;
        const cur = extractedInfo.user[k];
        if (cur == null || cur === "") extractedInfo.user[k] = v;
      }
  ...
  if (isFPC && extractedInfo) extractedInfo = normalizeFpcSummary(extractedInfo);
  ...
  let profileInfo = {
    ...
    customerNotes: req.body?.siteInfos?.metaData?.customerNotes,
    ...(req.body?.siteInfos?.metaData?.customerInfo || {}) // Fallback für Kompatibilität
  };
  // Cherry/All: Profilbild-Flag aus metaData ableiten (ohne Bildanalyse)
  try {
    const md = req.body?.siteInfos?.metaData || {};
    const custInfo = profileInfo.customerInfo && typeof profileInfo.customerInfo === 'object' ? profileInfo.customerInfo : {};
    const hasPicAlready = custInfo.hasProfilePic === true || custInfo.hasProfilePic === false;
    if (!hasPicAlready) {
      const hasCustPicKey =
        Object.prototype.hasOwnProperty.call(md, 'customerProfilePic') ||
        Object.prototype.hasOwnProperty.call(md, 'customerProfilePicUrl') ||
        Object.prototype.hasOwnProperty.call(md, 'customerProfilePicURL');
      const rawCustPic = md.customerProfilePicUrl ?? md.customerProfilePicURL ?? md.customerProfilePic;
      const s = typeof rawCustPic === 'string' ? rawCustPic.trim() : '';
      // Nur setzen, wenn das Feld explizit im Request vorhanden ist (sonst unbekannt lassen)
      if (hasCustPicKey) {
        ...
        const isNoPic =
          ...
          normalized === 'none' ||
          normalized === '-' ||
          /no\s*profile\s*pic|no\s*profile\s*picture|kein\s*profilbild|kein\s*profile\s*bild/i.test(normalized);
        profileInfo = {
          ...profileInfo,
          customerInfo: {
            ...custInfo,
            hasProfilePic: !isNoPic
          }
        };
      }
    }
  } catch (ePic) {}
  const metaDialogLang = req.body?.siteInfos?.metaData?.dialogLanguage;
  if (metaDialogLang && String(metaDialogLang).trim()) {
    profileInfo = { ...profileInfo, dialogLanguage: String(metaDialogLang).trim() };
  }
  // Vor ASA / Erstnachricht / Thank-You / Grok: nur deutschsprachige Dialoge (Cherry: dialogLanguage)
  if (isDialogLanguageExplicitlyNonGerman(profileInfo)) {
    const errMsg =
      'Dieser Dialog ist nicht auf Deutsch eingestellt. Beschreibung und Moderation sind nur für deutschsprachige Chats möglich. Bitte einen anderen Chat wählen oder die Dialogsprache in der Plattform prüfen.';
    console.error('🌍 Dialogsprache nicht Deutsch – blockiert (vor ASA/Erstnachricht/Grok):', profileInfo.dialogLanguage);
    logLogbookSummaryLine(chatId || finalChatId || '00000000', extractedInfo);
    return res.status(200).json({
      error: errMsg,
      resText: '',
      replyText: '',
      ...
      flags: {
        blocked: true,
        reason: 'dialog_language_not_german',
        isError: true,
        showError: true,
  ...
  if (isBlennyRequest && !(profileInfo.moderatorInfo?.city && String(profileInfo.moderatorInfo.city).trim())) {
    const fakeCity = discardFakeCityIfMatchesCustomer(
      extractFakeCityFromMetaOrHtml(req.body?.siteInfos?.metaData || {}, req.body?.siteInfos || {}),
      req.body?.siteInfos?.metaData || {},
      extractedInfo?.user
    );
    if (fakeCity) {
      profileInfo = {
  ...
  // FPC: Vorgegebenen Fake-Ort nutzen (Profil "Wohnort: …" oder metaData.moderatorInfo) – KI soll NUR diesen Ort nennen, keinen anderen erfinden
  ...
  if (isFpcRequest) {
    ...
    const hasValidCity = existingCity && existingCity.toLowerCase() !== 'sag ich später';
    const metaFpc = req.body?.siteInfos?.metaData || {};
    const siteFpc = req.body?.siteInfos || {};
    const rawFakeCity = hasValidCity ? existingCity : extractFakeCityFromMetaOrHtml(metaFpc, siteFpc);
    const fakeCity = hasValidCity
      ? (rawFakeCity || "").trim()
      : discardFakeCityIfMatchesCustomer(rawFakeCity || "", metaFpc, extractedInfo?.user);
    if (fakeCity && fakeCity.trim()) {
      ...
      console.log('✅ FPC: Fake-Wohnort für Pipeline (keine Suche):', cityTrim);
    }
  }
  ...
  // Nur FPC: HTML-Banner (Screenshot) + Extension-DOM-Flag aus metaData (zuverlässig wenn keine Systemzeile im Chat)
  ...
  if (isFriendRequestByFpcButton) {
    console.log('✅ FPC: Freundschaftsanfrage über UI/HTML erkannt (Button/Banner-Text).');
  }
  if (isFriendRequestByFpcBannerMeta) {
    console.log('✅ FPC: Freundschaftsanfrage über Extension (fpcFriendRequestBannerVisible / DOM-Banner).');
  }
  ...
  if (thankYouTriggerType && !isAVZ) {
    ...
    const isOnlyKussNachrichtUnified = foundTrimmedUnified.length > 0 && foundTrimmedUnified.length < 220 && /^\s*kuss\s+nachricht\b/i.test(foundTrimmedUnified);
    const noRealCustomerTextUnified = !foundTrimmedUnified || isOnlyGrußUnified || isOnlyKussNachrichtUnified;
    ...
    if (!skipThankYou) {
      ...
      if (instructions) {
        ...
        const waitTime = insertSendDelaySecForPlatform();
    ...
    try {
      ...
      if (firstMessageResult && firstMessageResult.isFirstMessage && firstMessageResult.instructions) {
        ...
        if (generatedFirstMessage && generatedFirstMessage.trim() !== "") {
          ...
          const waitTimeFirst = insertSendDelaySecForPlatform();
            ...
            actions: [
              {
                ...
                delay: waitTimeFirst // FPC/Cherry/Gold/S69: 3s (Extension); sonst 40–60s variabel
              }
            ],
            assets: [],
            flags: { 
  ...
  const isOnlyKussNachrichtSystemText = foundTrimmedForLK.length > 0 && foundTrimmedForLK.length < 220 && /^\s*kuss\s+nachricht\b/i.test(foundTrimmedForLK);
  const noRealCustomerText = !foundTrimmedForLK || isOnlyGrußSystemText || isOnlyKussNachrichtSystemText;
  ...
  if (hasFavoritenSystemMessage && noRealCustomerTextForFavoriten && thankYouTriggerType !== 'favoriten') {
    ...
    try {
      ...
      if (generatedFavoritenMessage && generatedFavoritenMessage.trim() !== "") {
        const waitTimeFavoriten = insertSendDelaySecForPlatform();
  ...
  if (isIluvoReply && !isASACalculated && /\b(wie hei[ßs]t du|wie ist dein name|was ist dein name|wie hei[ßs]en sie|wie hei[ßs]t ihr)\b/i.test((customerMessage || ''))) {
    ...
    if (existingFakeName) {
      ...
      console.log('✅ Iluvo: Vorhandenen Fake-Namen konsistent gesetzt (kein Zufallsname):', existingFakeName);
    } else {
      console.warn('⚠️ Iluvo: Namensfrage, aber kein echter Fake-Name gefunden – kein Name wird erfunden/überschrieben.');
    }
  }
  ...
  // Kunde = received/customer/incoming (Blenny/DF können andere Felder nutzen – muss überall für ASA-Kontext funktionieren)
  ...
  try {
    ...
    if (!imageDescriptionForPipeline && (hasBildPlatzhalterInText || customerSentImageFromAssetsNoUrl)) {
      ...
      if (customerSentImageFromAssetsNoUrl) console.log('✅ AVZ/FPC: Fallback-Bildbeschreibung (assetsToSend ohne URL).');
    }
    ...
    const customerSentImageThisTurn = (customerImageEvidenceThisTurn && imageUrls.length > 0) || customerSentImageFromAssetsNoUrl || (customerImageEvidenceThisTurn && hasBildPlatzhalterInText && imageDescriptionForPipeline);
    ...
    // Kontext „letzte Fake-Nachricht mit Bild“ – für Grok- und Multi-Stage-Pipeline (gemeinsam, damit ASA-Pfad sie nutzen kann)
          ...
          if (modDataUrl && modDataUrl.length < 4 * 1024 * 1024) {
            const modVision = await client.chat.completions.create({
              ...
              messages: [
                {
                  role: 'system',
                  content: 'Du beschreibst Bilder kurz und sachlich. 1–2 Sätze: Was ist zu sehen (Inhalt, Stimmung)? Aus Sicht des Absenders: Dieses Bild hat der Moderator dem Kunden geschickt.'
                },
                {
                  role: 'user',
                  content: [
                    { type: 'text', text: 'Beschreibe dieses Bild in 1–2 Sätzen (Inhalt, Stimmung).' },
            ...
            if (desc) {
              ...
              console.log('✅ Moderator-Bild beschrieben für Kontext:', desc.substring(0, 60) + '…');
            }
          }
        } catch (err) {
          console.warn('⚠️ Optionale Analyse des von uns gesendeten Bildes fehlgeschlagen (nicht kritisch):', err.message);
        }
      }
    }
    if (imageAlreadySentToCustomer) console.log('✅ Letzte Fake-Nachricht enthielt ein Bild – setze imageAlreadySentToCustomer für Pipeline');
    ...
    if (isASACalculated && asaFromKussOrLike && isFPC) {
      ...
      // imageAlreadySentToCustomer + moderatorSentImageDescription werden weiter oben gemeinsam für Grok + Multi-Stage gesetzt
      ...
      const safeCustomerName = resolveCustomerFirstNameForAddress(
      ...
      if (isASACalculated && lastCustomerMessageAgeMs == null && lastCustomerMessageTimestamp == null) {
        ...
        if (fromHtml != null) {
          ...
          console.log('🤖 ASA: Alter der letzten Kunden-Nachricht aus HTML geparst:', Math.round(lastCustomerMessageAgeMs / 60000) + ' Min');
        } else if (lastPreviousCustomerMessageAgeMs != null) {
          lastCustomerMessageAgeMs = lastPreviousCustomerMessageAgeMs;
          console.log('🤖 ASA: Alter der letzten Kunden-Nachricht aus Messages (reply.js):', Math.round(lastCustomerMessageAgeMs / 60000) + ' Min');
        }
      }
      if (isASACalculated && (lastCustomerMessageAgeMs != null || lastCustomerMessageTimestamp != null)) {
        console.log('🤖 ASA: Alter für Kontext vs. Pool:', lastCustomerMessageAgeMs != null ? (Math.round(lastCustomerMessageAgeMs / 60000) + ' Min') : ('Timestamp ' + lastCustomerMessageTimestamp));
      }
      const asaNoRealCustomerMessage = isASACalculated && (!foundMessageText || String(foundMessageText).trim() === '');
      if (asaNoRealCustomerMessage) {
        console.log('🤖 ASA: Keine echte Kundennachricht (nur Freundschaftsanfrage/Like) – Kontext-ASA ohne „unser Gespräch über X“');
      }
    ...
    if (isASACalculated && generatedMessage && generatedMessage.trim()) {
      const asaCustomerName = resolveCustomerFirstNameForAddress(
        profileInfo?.customerInfo || {},
        profileInfo?.customerNotes,
        extractedInfo?.user?.Name
      );
    ...
    // Platzhalter [Vorname]/[dein Name]/[Stadt] durch echte Werte ersetzen (niemals an Kunden ausliefern) – für alle Plattformen (metaData + extractedInfo)
    if (generatedMessage && typeof generatedMessage === 'string') {
      const fakeFirst = resolveSafeFakeFirstNameForPrompt(
        profileInfo?.moderatorInfo || {},
        extractedInfo?.assistant?.Name,
        "",
        { route: "reply_placeholder_replace", chatId: chatId || finalChatId || null }
      );
  ...
  // Wohnort-Kontext: Nur bei explizitem „Ich komme aus …“ / „ich wohne in …“ in der generierten Antwort — Distanz zum Kundenwohnort (Profil, sonst Kundennachricht), kein LLM
  const isWohnortDistanceCheck =
    (detectedSituations && detectedSituations.some((s) => s === 'Wohnort-Frage')) ||
    !!(multiAgentResults && multiAgentResults.locationContext && multiAgentResults.locationContext.fakeCity);
  if (isWohnortDistanceCheck && generatedMessage && generatedMessage.trim()) {
    const declaredFromChat = extractCustomerOriginCityFromMessage(foundMessageText || '');
    const fromProfile = String(
      multiAgentResults?.locationContext?.customerCity ||
        extractedInfo?.user?.Stadt ||
        extractedInfo?.user?.Wohnort ||
        profileInfo?.customerInfo?.city ||
        profileInfo?.customerInfo?.Stadt ||
        req.body?.siteInfos?.metaData?.customerInfo?.city ||
        req.body?.siteInfos?.metaData?.customerInfo?.Stadt ||
        ''
    ).trim();
    const customerCityForDistance =
      sanitizeLocationQueryForGeocode(fromProfile, declaredFromChat) || declaredFromChat || fromProfile;
    if (customerCityForDistance && customerCityForDistance.length >= 2) {
      const wohnortCheck = await validateGeneratedMessageFakeWohnortDistance(customerCityForDistance, generatedMessage, {
        customerMessageText: foundMessageText || ''
      });
      if (!wohnortCheck.ok) {
        console.error('🌍 Wohnort-Distanz: Antwort blockiert –', wohnortCheck.error);
        logLogbookSummaryLine(chatId || finalChatId || '00000000', extractedInfo);
        return res.status(200).json({
          error: wohnortCheck.error,
          resText: '',
          replyText: '',
          message: '',
          summary: extractedInfo,
          ...
          flags: {
            blocked: true,
            reason: 'fake_wohnort_too_far',
            isError: true,
            showError: true,
            errorType: 'location_distance',
            errorColor: 'red',
            errorMessage: wohnortCheck.error
          }
        });
      }
    }
  ...
  // End-of-Pipeline Guard: Kunde darf NICHT mit Fake-Namen angesprochen werden (z.B. "Hey Jutta" obwohl Jutta der Fake ist).
  try {
    const fakeFirstName = resolveSafeFakeFirstNameForPrompt(
      profileInfo?.moderatorInfo || {},
      extractedInfo?.assistant?.Name || profileInfo?.moderatorInfo?.name || "",
      "",
      { route: "reply_name_swap_guard", chatId: chatId || finalChatId || null }
    );
    const nameSwapCheck = checkWrongCustomerAddressingWithFakeName(generatedMessage, fakeFirstName);
    if (nameSwapCheck && nameSwapCheck.ok === false) {
      console.error('🚨 Name-Swap Guard: Antwort blockiert –', nameSwapCheck.error, nameSwapCheck.details || '');
      logLogbookSummaryLine(chatId || finalChatId || '00000000', extractedInfo);
      return res.status(200).json({
        error: nameSwapCheck.error,
        resText: '',
        replyText: '',
        message: '',
        summary: extractedInfo,
        ...
        flags: {
          blocked: true,
          reason: 'wrong_customer_addressing',
          isError: true,
          showError: true,
          errorType: 'name_swap',
          errorColor: 'red',
          errorMessage: nameSwapCheck.error
        }
      });
    }
  } catch (eNameSwap) {
    console.warn('⚠️ Name-Swap Guard: Check fehlgeschlagen (ignoriert):', eNameSwap?.message || eNameSwap);
  }
  ...
  const siteUrlForAssistantMerge = String(req.body?.siteInfos?.url || "");
  const originForAssistantMerge = (req.body?.siteInfos?.origin || "").toLowerCase();
  const keepAssistantUpdatesForGoldMods =
    originForAssistantMerge === "gold" ||
    originForAssistantMerge === "diamond" ||
    originForAssistantMerge === "s69" ||
    originForAssistantMerge === "ml" ||
    /mods\.(gold-chat\.net|chatsx\.net)/i.test(siteUrlForAssistantMerge) ||
    /mods\.(diamondchat\.net|platin-chat|onlydates69|mltestapp\.com)/i.test(siteUrlForAssistantMerge);
    ...
    try {
      ...
      if (assistantFromReply && typeof assistantFromReply === "object" && Object.keys(assistantFromReply).length > 0) {
        ...
        Object.keys(assistantFromReply).forEach((key) => {
          ...
          if (key === "Updates" && !keepAssistantUpdatesForGoldMods) return;
          if (key === "Stadt" || key === "Wohnort") {
            if (!isWohnortFrageForMerge) return;
            const valStr = typeof val === "string" ? val.trim() : "";
            if (!valStr) return;
            if (fakeAlreadyHasWohnortForMerge && !fakeWohnortExtractRefinesProfile(profileInfo, valStr)) return;
          }
          if (key === "Name") {
      ...
      reconcileFakeAgeForLogbookSummary(extractedInfo, profileInfo, generatedMessage);
    } catch (err) {
  ...
  // 📊 Plattform-Label für Sheets/Feedback (nach URL-Normalisierung: WM/XL/S69/ML etc. in platformId).
  const platformLabelForSheets = platformId || "viluu";
    ...
    (async () => {
      try {
        await writeToGoogleSheets({
          ...
          platform: platformLabelForSheets,
          chatId: chatId || finalChatId || "00000000",
      ...
      const sendFeedback = async (attempt = 1) => {
        ...
        try {
          ...
          if (response.ok) {
            ...
            if (result.skipped) {
              console.log('ℹ️ Auto-Feedback ist im Dashboard deaktiviert – kein neuer Eintrag in feedback.json (bestehende Feedbacks unverändert).');
              return true;
            }
  ...
  let formatAnomalyDoNotSend = !!(multiAgentResults && multiAgentResults.formatAnomalyDoNotSend);
  if (isAVZ) {
  ...
  // Gold-Familie u. a.: Wenn Fake-Geburtsdatum/Alter im Profil steht, darf die Antwort kein anderes Alter nennen.
  if (generatedMessage && typeof generatedMessage === 'string') {
    generatedMessage = enforceFakeAgeInGeneratedMessage(generatedMessage, profileInfo);
  }
  ...
  // Zusatzsicherung (nur schnelle Patterns – kein zweiter LLM-Call): Pipeline-Check + alte Deploys + Ersetzungen o.ä. in reply.js
  if (!metaOutingDoNotSend && safeMessage && safeMessage.trim() && hasObviousMetaOutingPatterns(safeMessage)) {
    metaOutingDoNotSend = true;
    console.warn('🛡️ reply.js: Meta-Outing (Pattern-Zusatzcheck) – metaOutingDoNotSend nachträglich gesetzt');
  }
  if (!formatAnomalyDoNotSend && safeMessage && safeMessage.trim() && hasSuspiciousFormatPatterns(safeMessage)) {
    formatAnomalyDoNotSend = true;
    console.warn('🛡️ reply.js: Format-/Klammer-Auffälligkeit – manuelle Prüfung (kein Auto-Send)');
  }
  const manualReviewNoAutoSend = metaOutingDoNotSend || formatAnomalyDoNotSend;
  let manualReviewErrorMessage = '';
  if (manualReviewNoAutoSend) {
    if (metaOutingDoNotSend && formatAnomalyDoNotSend) {
      manualReviewErrorMessage = 'Die Antwort enthält KI-/Bot-/Moderator-Hinweise und auffällige Formatierung (z. B. Klammern). Nicht automatisch gesendet. Bitte prüfen und manuell senden.';
    } else if (formatAnomalyDoNotSend) {
      manualReviewErrorMessage = 'Die Antwort enthält Klammern oder ungewöhnliche Formatierung (z. B. interne Überlegungen). Nicht automatisch gesendet. Bitte prüfen und manuell senden.';
    } else {
      manualReviewErrorMessage = 'Die Antwort enthält KI-, Bot- oder Moderator-Hinweise – nicht automatisch gesendet. Bitte prüfen und ggf. anpassen.';
    }
  }
  if (metaOutingDoNotSend) {
    console.warn('🛡️ Meta-Outing: Extension soll Nachricht einfügen ohne Auto-Send (metaOutingDoNotSend)');
  }
  if (formatAnomalyDoNotSend) {
    console.warn('🛡️ Format-Anomalie: Extension soll Nachricht einfügen ohne Auto-Send (formatAnomalyDoNotSend)');
  }
  ...
  if (isFPC && extractedInfo) extractedInfo = normalizeFpcSummary(extractedInfo);
  stripInappropriateCustomerNameFromSummary(extractedInfo, req.body?.siteInfos?.metaData || {});
  logLogbookSummaryLine(responseChatIdForExtension, extractedInfo);

  // FPC-only: LLM-Check ob manuelle Logbuch/Notiz-Eintraege noetig sind.
  // Ziel: Automatisierung behalten, aber bei wirklich "notierwuerdigen" Infos (Beruf, Kinder, Gesundheit, dauerhafte Fakten, echte Updates)
  // den Auto-Send stoppen und eine rote Meldung anzeigen.
  let fpcLogbookEntryDoNotSend = false;
  let fpcLogbookReasons = [];
  let fpcLogbookSide = null; // "customer" | "fake" | "both"
  let fpcLogbookConfidence = null;

  // FPC: Wenn der Kunde in DIESEM Turn ein Bild geschickt hat, immer manuelle Logbuch-Notiz verlangen.
  // (FPC-Panels koennen nicht alles automatisch ins Logbuch uebernehmen; Bild ist immer notierwuerdig.)
  try {
    if (
      isFPC &&
      !manualReviewNoAutoSend &&
      !fpcFriendRequestDoNotSend &&
      typeof customerSentImageThisTurn !== "undefined" &&
      customerSentImageThisTurn === true
    ) {
      fpcLogbookEntryDoNotSend = true;
      fpcLogbookSide = "customer";
      fpcLogbookConfidence = 1;
      fpcLogbookReasons = ["Kundenbild erhalten (Logbuch/Notizen aktualisieren)"];
      console.warn("🛑 FPC Logbuch: Kundenbild erkannt – Auto-Send gestoppt (manueller Eintrag).");
    }
  } catch (_) {}

  // FPC: Wenn der Kunde seinen Wohnort nennt, immer Logbuch/Notiz verlangen (Auto-Send stoppen).
  // Wichtig: enges Pattern, damit "zu Hause" / "daheim" nicht dauernd alarmiert.
  try {
    if (isFPC && !fpcLogbookEntryDoNotSend && !manualReviewNoAutoSend && !fpcFriendRequestDoNotSend) {
      const t = String(foundMessageText || "").replace(/\s+/g, " ").trim().toLowerCase();
      if (t) {
        const hasExplicitWohnort =
          /\b(ich|wir)\s+(wohne|wohn)\b/.test(t) ||
          /\b(ich|wir)\s+komme(n)?\s+aus\b/.test(t) ||
          /\b(ich|wir)\s+bin\s+aus\b/.test(t) ||
          /\bwohnort\b/.test(t) ||
          /\bwohnhaft\b/.test(t) ||
          /\bplz\b/.test(t) ||
          /\bpostleitzahl\b/.test(t) ||
          /\b\d{5}\b/.test(t) ||
          /\b(straße|str\\.|weg|allee|platz|gasse|hausnummer|nr\\.)\b/.test(t);
        const isGenericHomeOnly =
          /(^|\\s)(zuhause|zu hause|daheim|bei mir|bei dir)(\\s|$)/i.test(t) &&
          !/\b(komme\s+aus|bin\s+aus|wohne|wohnort|wohnhaft|plz|postleitzahl|straße|str\\.|hausnummer|nr\\.)\b/i.test(t) &&
          !/\b\d{5}\b/.test(t);
        if (hasExplicitWohnort && !isGenericHomeOnly) {
          fpcLogbookEntryDoNotSend = true;
          fpcLogbookSide = "customer";
          fpcLogbookConfidence = 1;
          fpcLogbookReasons = ["Wohnort vom Kunden genannt (Logbuch/Notizen aktualisieren)"];
          console.warn("🛑 FPC Logbuch: Wohnort-Nennung erkannt – Auto-Send gestoppt (manueller Eintrag).");
        }
      }
    }
  } catch (_) {}

  if (isFPC && !fpcLogbookEntryDoNotSend && safeMessage && safeMessage.trim() && !manualReviewNoAutoSend && !fpcFriendRequestDoNotSend) {
    try {
      const enable = String(process.env.ENABLE_FPC_LOGBOOK_LLM_CHECK || "true").toLowerCase() !== "false";
      if (enable) {
        const model = (process.env.FPC_LOGBOOK_LLM_MODEL || "gpt-4o-mini").trim();
        const logbookClient = getClient();
        if (logbookClient && model) {
          const system = [
            "Du bist ein strenger Klassifikator fuer Moderations-Workflows (Dating-Chat).",
            "Aufgabe: Entscheide, ob aus (A) letzter Kundennachricht oder (B) der generierten Antwort der Persona",
            "wichtige, dauerhafte oder konkret notierwuerdige Fakten hervorgehen, die ein Moderator MANUELL ins Logbuch/Notizen eintragen muss.",
            "",
            "WICHTIG: Schlage NUR Alarm bei wirklich logbuchrelevanten Infos. Bei Unsicherheit: KEIN Alarm.",
            "",
            "KEIN Alarm fuer (explizit ignorieren):",
            "- Name / Alter (wird technisch bereits automatisch erfasst)",
            "- banale Tagesstimmung oder voruebergehende Zustände: 'muede', 'gestresst', 'busy', 'heute viel zu tun', 'bin beschaeftigt'",
            "- sexuelle Details / Dirty Talk / einzelne Vorlieben im Sex-Chat (lecken, anal, etc.)",
            "- reine Fragen oder Floskeln ohne neue Fakten",
            "",
            "ALARM (Beispiele):",
            "- Beruf/Job/Arbeitszeiten/Schichten als feste Info (z.B. 'ich arbeite als...', 'Nachtschicht im Krankenhaus')",
            "- Kinder / Familienverhaeltnisse (z.B. 'habe 2 Kinder', 'keine Kinder')",
            "- Gesundheit/Diagnose/Behinderung (Rollstuhl, OP, chronisch, Schwangerschaft etc.)",
            "- konkrete wichtige Updates/Ereignisse mit Datum/Termin (Arzttermin, Urlaub, Umzug, Dienstreise)",
            "- stabile persoenliche Fakten, die man spaeter wieder wissen sollte (z.B. 'ich trinke nie', 'bin verheiratet', 'will nur Beziehung')",
            "",
            "Gib NUR JSON im Format:",
            '{"needs_logbook_entry": boolean, "side": "customer"|"fake"|"both"|"none", "confidence": number, "reasons": string[]}',
          ].join("\n");

          const user = [
            "KUNDEN-NACHRICHT:",
            String(foundMessageText || "").slice(0, 1600),
            "",
            "GENERIERTE ANTWORT (FAKE):",
            String(safeMessage || "").slice(0, 1600),
          ].join("\n");

          const resp = await logbookClient.chat.completions.create({
            model,
            temperature: 0,
            messages: [
          ...
          const needs = !!obj?.needs_logbook_entry;
          const conf = typeof obj?.confidence === "number" ? obj.confidence : 0;
          const side = typeof obj?.side === "string" ? obj.side : "none";
          const reasons = Array.isArray(obj?.reasons) ? obj.reasons.filter(r => typeof r === "string" && r.trim()).slice(0, 4) : [];
          fpcLogbookConfidence = conf;
          if (needs && conf >= 0.72 && (side === "customer" || side === "fake" || side === "both")) {
            fpcLogbookEntryDoNotSend = true;
            fpcLogbookSide = side;
            fpcLogbookReasons = reasons.length ? reasons : ["Logbuch/Notizen aktualisieren"];
            console.warn("🛑 FPC Logbuch-LLM: Auto-Send gestoppt (side=" + side + ", conf=" + conf + ", reasons=" + JSON.stringify(fpcLogbookReasons) + ")");
          }
        }
      }
    } catch (e) {
      console.warn("⚠️ FPC Logbuch-LLM Check fehlgeschlagen (optional):", e?.message || e);
    }
  }
  ...
  return res.status(200).json({
    ...
    actions: [
      {
        type: "insert_and_send",
        delay: waitTime // FPC/Cherry/Gold/S69: 3s (Extension); sonst 40–60s variabel
      }
    ],
    assets: validAssets,
    flags: { 
      ...
      // Pipeline: Meta-Outing und/oder Klammer-/Format-Leak – wie FPC Freundschaftsanfrage: einfügen, rote Warnung, kein Auto-Send
      ...(manualReviewNoAutoSend ? {
        metaOutingDoNotSend: true,
        formatAnomalyDoNotSend: !!formatAnomalyDoNotSend,
        showError: true,
        errorType: metaOutingDoNotSend && formatAnomalyDoNotSend ? 'meta_and_format' : (formatAnomalyDoNotSend ? 'format_anomaly' : 'meta_outing'),
        errorColor: 'red',
        errorMessage: manualReviewErrorMessage
      } : {}),
      ...(fpcFriendRequestDoNotSend ? { fpcFriendRequestDoNotSend: true } : {}),
      ...(fpcLogbookEntryDoNotSend ? {
        fpcLogbookEntryDoNotSend: true,
        fpcLogbookSide: fpcLogbookSide,
        fpcLogbookConfidence: fpcLogbookConfidence,
        fpcLogbookReasons: fpcLogbookReasons,
        showError: true,
        errorType: 'fpc_logbook_entry',
        errorColor: 'red',
        errorMessage: 'Logbuch-Eintrag Kunde/Fake erforderlich – bitte manuell eintragen, dann senden.'
      } : {}),
      // FPC: Reload nach Send-Klick – gilt für normale Nachricht UND für ASA/Like-Kuss (Pipeline-Antwort kommt alle hier an)
      ...(isFPC ? { reloadPageAfterSend: true } : {})
    },
    disableAutoSend: true,
    waitTime: waitTime,
