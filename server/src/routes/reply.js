...
router.post("/", asyncHandler(async (req, res, next) => {
  ...
  if ((!foundMessageText || foundMessageText.trim() === "") && req.body?.siteInfos?.messages) {
    ...
    // Zusätzlich: ASA nur dann, wenn die neueste Nachricht im Verlauf wirklich sent ist.
    if (isCherryOrigin) {
      let cherryNewestReceived = null;
      for (let i = validReceivedMessages.length - 1; i >= 0; i--) {
        const cand = validReceivedMessages[i];
      ...
      const extensionFlag = isLastMessageFromFake;
      isLastMessageFromFake = newestSentIdx > newestReceivedIdx;
      if (extensionFlag !== isLastMessageFromFake) {
        console.log("⚠️ Cherry strict: lastMessageFromFake überschrieben von", extensionFlag, "auf", isLastMessageFromFake, "(sentIdx:", newestSentIdx, "receivedIdx:", newestReceivedIdx, ")");
      }
      ...
      if (isCherryOrigin && afterLastMod.length > 0) {
        ...
        if (newestAfterFake) {
          ...
          if (!isInfoMessage(newestAfterFake, { isBlenny: false }) && newestTxt) {
            ...
            // Cherry-Konsistenz: Wenn es eine neue received nach letzter sent gibt, ist es kein ASA-Fall.
  ...
  const isCherryReply = originReply.includes("cherry");
