...
router.post("/", asyncHandler(async (req, res, next) => {
  ...
  try {
    ...
    if (isASACalculated && asaFromKussOrLike && isFPC) {
      try {
        const firstMessageInstructions = "Der Kunde hat dir gerade einen Kuss oder Like geschickt (System-Nachricht). Bedanke dich kurz dafuer und stelle 1-2 gespraechsoeffnende Fragen (z.B. wie geht es dir, was machst du so). Keine Vorstellung (kein Name, kein Alter, kein Wohnort in der Nachricht).";
