...
Gib die Antwort NUR als JSON zurück, kein zusätzlicher Text. Format:
{
  "user": {
    ...
    "Updates": "Aktualisierungen/Neuigkeiten falls erwähnt (z.B. 'geht zum Friseur', 'hat neuen Job', 'ist umgezogen', 'wohnt bei Verwandten', 'hat bald eigene Wohnung'), sonst null",
    "Wohnsituation": "Wohnsituation falls erwähnt (z.B. 'wohnt bei Verwandten', 'hat eigene Wohnung', 'wohnt alleine', 'zieht bald um'), sonst null",
...
- Wichtige Lebensumstände (Familie, Gesundheit, Arbeit, Hobbies, Wohnsituation)
- Wohnsituation: Wenn erwähnt (z.B. "wohnt bei Verwandten", "hat eigene Wohnung", "wohnt alleine", "zieht bald um", "bald eigene Wohnung"), extrahiere es als "Wohnsituation"
...
🚨 KRITISCH: Orientiere dich STRENG an den Training-Daten oben für Stil, Wortwahl und Ton! Schreibe wie echte Chat-Replies - kurz, natürlich, locker, direkt, roh, spontan, leicht emotional! KEINE erklärenden/beschreibenden/kommentierenden Formulierungen! KEINE therapeutische/Coaching-Sprache! MEHR reagieren als formulieren! Du bist eine echte Person im Chat, nicht jemand der versucht alles richtig zu machen!`;
    ...
    if (foundForbiddenWords.length > 0 || foundMetaComments.length > 0 || foundFormalPhrases.length > 0 || hasRepetitiveQuestion || hasBlockingResponse || foundRepetitions.length > 0 || hasTooManyExclamations || hasMultipleQuestions) {
