...
router.post("/", asyncHandler(async (req, res, next) => {
  ...
  // Versuche Nachricht zu generieren
    ...
    try {
      ...
      console.error('❌ Fehler in Multi-Agent-Pipeline (ASA):', err.message);
      console.error('❌ Stack:', err.stack);
      
      // Fallback: Versuche direkt mit chat.completions.create (falls Pipeline fehlschlägt)
      console.warn('⚠️ Fallback: Verwende direkt chat.completions.create für ASA');
      try {
        // Einfacher Fallback für ASA
        const fallbackASA = "Hallo? Bist du noch da? Mega schade, dass du mir nicht zurückschreibst. Ich würde mich echt freuen, wenn du mir wieder antwortest. Wo steckst du denn gerade und was hält dich ab?";
        let asaMessage = fallbackASA;
        ...
        const errorMessage = `❌ FEHLER: Beim Generieren der ASA-Nachricht ist ein Fehler aufgetreten: ${fallbackErr.message}`;
        console.error("❌ OpenAI Fehler (ASA Fallback):", fallbackErr.message);
        return res.status(200).json({
          error: errorMessage,
          resText: errorMessage,
          replyText: errorMessage,
          ...
          chatId: finalChatId,
          actions: [],
          flags: { blocked: true, reason: "asa_generation_error", isError: true, showError: true }
        });
      }
    }
...
- Sei menschlich, emotional, verständnisvoll - nicht mechanisch oder oberflächlich!`;
    ...
    if (!isLastMessageFromFake) {
      ...
      const findNearbyCityFunc = findNearbyCity;
      const isMeetingRequestFuncForPipeline = isMeetingRequest;
      const vectorDbFunc = async (query, options) => {
        const { findSimilarExamples } = require('../utils/vector-db');
        return await findSimilarExamples(query, options);
      };
      const imageAnalysisFunc = null; // TODO: Falls vorhanden, hier übergeben
      const proactiveAnalysisFunc = null; // TODO: Falls vorhanden, hier übergeben
      const analyzeWritingStyleFunc = analyzeWritingStyle;
      const isInfoMessageFunc = isInfoMessage;
