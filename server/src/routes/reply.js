...
function looksLikeFakeOrAIContent(text) {
  ...
  if (/\b(danke\s+dass\s+du|mach\s+mich\s+(total\s+)?an|mein\s+Schatz|macht\s+mich|😘|Ich\s+bin\s+auch|Ich\s+habe\s+ein|Ich\s+wohne|Ich\s+arbeite|finde\s+mich\s+toll|super\s+toll)\b/i.test(t)) return true;
  return false;
}
  ...
  if (req.body?.siteInfos?.metaData) {
    ...
    if (Object.keys(extractedInfo.user).length === 0 && Object.keys(metaSummary.user).length > 0) {
      ...
      if (extractedInfo.user.Other && typeof extractedInfo.user.Other === 'string') {
        ...
        if (o.length > 180 || /\b(danke\s+dass\s+du|mach\s+mich\s+(total\s+)?an|mein\s+Schatz|😘|macht\s+mich)\b/i.test(o)) {
