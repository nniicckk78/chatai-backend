  ...
  if ((!foundMessageText || foundMessageText.trim() === "") && req.body?.siteInfos?.messages) {
    ...
    if (lastReceived) {
        ...
        if (isInfoMessage(lastReceived)) {
          console.log("⚠️ Gefundene Nachricht ist Info-Message (Like), ignoriere sie:", lastReceived.text.substring(0, 100) + "...");
          ...
          if (realReceivedMessages.length > 0) {
            ...
            console.log("✅ Echte Nachricht aus siteInfos.messages (received, NEU, Info-Message übersprungen):", foundMessageText.substring(0, 100) + "...");
          } else {
...
Antworte NUR mit der vollständigen, erweiterten Reaktivierungsnachricht (mindestens 150 Zeichen), keine Erklärungen. Die Nachricht soll fragen, warum der Kunde nicht mehr schreibt!`;
      ...
      try {
        const platform = req.body?.platformId || req.body?.siteInfos?.platform || 'unknown';
        const chatIdForFeedback = asaChatId || req.body?.chatId || req.body?.siteInfos?.chatId || null;
...
- 🚨🚨🚨 KRITISCH: NUR EINE FRAGE PRO NACHRICHT! 🚨🚨🚨
- Bei 90% der Antworten ist KEINE zweite Frage nötig - eine reicht völlig!
- Wenn du zwei Fragen stellst, ist das FALSCH - wähle die wichtigste Frage aus!
- 🚨 KRITISCH: ZEIGE EIGENINITIATIVE! Sage von dir aus etwas (Vorlieben, Fantasien, Gedanken, Erfahrungen), nicht nur fragen!
- Der Kunde soll nicht alles aus dir rausquetschen müssen - trage aktiv zum Gespräch bei!
- 🚨 WICHTIG: Wenn der Kunde sagt, dass er Sex sucht oder Freundschaft+ sucht, dann sage auch, dass DU Sex suchst oder Freundschaft+ suchst! Zeige Gemeinsamkeiten! Frage dann nach seinen Vorlieben, Fantasien oder wie er sich das vorstellt!
- 🚨 WICHTIG: Gehe tiefer auf die Kundennachricht ein! Wenn der Kunde detailliert schreibt, dann schreibe auch detailliert zurück! Nicht so trocken - sei emotional, menschlich, interessiert!
...
- 🚨 KEINE Begrüßungen in laufenden Gesprächen! ("Guten Morgen", "Guten Tag", "Hallo" etc. sind NUR am Anfang erlaubt, nicht mitten im Gespräch!)
- Nachrichtenlänge: FLEXIBEL basierend auf der Kundennachricht! NIEMALS unter 120 Zeichen! Alle Nachrichten müssen mindestens 120 Zeichen haben. Bei kurzen Kundennachrichten: 120-150 Zeichen ideal. Wenn der Kunde eine lange, detaillierte Nachricht schreibt (z.B. 200+ Zeichen), dann schreibe auch eine längere, detailliertere Antwort (150-250 Zeichen). Passe die Länge an die Qualität und Länge der Kundennachricht an!
- 🚨 KRITISCH: KONSISTENZ mit früheren Nachrichten! Wenn in früheren Nachrichten etwas über dich gesagt wurde (z.B. Beruf, Wohnsituation), dann sei KONSISTENT und gehe darauf ein, wenn der Kunde danach fragt!
- Bei Fragen: Verwende "wäre" statt "bin" (z.B. "wenn ich bei dir wäre" statt "wenn ich bei dir bin", "wenn wir uns sehen würden" statt "wenn wir uns sehen")
- 🚨 KRITISCH: VERSTEHE DEN KONTEXT! Wenn der Kunde sagt "nicht mehr hier zu schreiben" oder ähnliches, bedeutet das, dass er keine Lust hat weiter zu schreiben und lieber persönlich treffen will - reagiere darauf emotional und menschlich, nicht mit oberflächlichen Fragen!
- 🚨 EMOTIONALER SCHREIBEN: Verwende emotionale, persönliche Formulierungen wie "Das würde sich bestimmt sehr schön mit dir anfühlen" statt generische Phrasen wie "Das gibt einem ein gutes Gefühl"
- 🚨 BESSERE FRAGEN: Stelle konkrete, emotionale Fragen, die auf den Kontext eingehen (z.B. "Wenn wir uns sehen würden, würde es dann bei einer Umarmung bleiben oder würdest du mich dann auch direkt Küssen wollen?" statt oberflächliche Fragen wie "Was wäre dein Traumort?")
...
- 🚨🚨🚨 ABSOLUT KRITISCH: Bei sexuellen Themen - orientiere dich RADIKAL an den Training-Daten! Die Training-Daten zeigen, wie man auf sexuelle Nachrichten reagiert - RADIKAL, direkt, roh, spontan, ohne bewertende Phrasen! 🚨🚨🚨
- 🚨🚨🚨 Training-Daten haben HÖCHSTE PRIORITÄT bei Sex-Gesprächen - orientiere dich STRENG daran, nicht an generischen Formulierungen! 🚨🚨🚨
- 🚨🚨🚨 KRITISCH: ÜBERNEHME DIE WORTWAHL AUS DEN TRAINING-DATEN! Verwende die GLEICHEN Wörter und Formulierungen wie in den Moderator-Antworten aus den Training-Daten! 🚨🚨🚨
- 🚨🚨🚨 KRITISCH: ANALYSIERE JEDES TRAINING-DATEN-BEISPIEL GENAU - welche Wörter werden verwendet? Welche Formulierungen? Welcher Ton? 🚨🚨🚨
...
🚨🚨🚨🚨🚨 TRAINING-DATEN HABEN ABSOLUT HÖCHSTE PRIORITÄT - ÜBER ALLEM ANDEREN! 🚨🚨🚨🚨🚨
- 🚨🚨🚨 KRITISCH: Training-Daten sind DEINE HAUPTREFERENZ für ALLES - Stil, Wortwahl, Ton, Formulierungen! 🚨🚨🚨
...
- 🚨🚨🚨 ABSOLUT KRITISCH: Bei sexuellen Themen - orientiere dich RADIKAL an den Training-Daten! Die Training-Daten zeigen, wie man auf sexuelle Nachrichten reagiert - RADIKAL, direkt, roh, spontan, ohne bewertende Phrasen! 🚨🚨🚨
- 🚨🚨🚨 Training-Daten überschreiben ALLE anderen Stil-Anweisungen - wenn Training-Daten etwas zeigen, dann MACH ES SO! 🚨🚨🚨
- 🚨🚨🚨 KRITISCH: ÜBERNEHME DIE WORTWAHL AUS DEN TRAINING-DATEN! Analysiere JEDES Beispiel genau - welche Wörter werden verwendet? Welche Formulierungen? Welcher Ton? 🚨🚨🚨
- 🚨🚨🚨 KRITISCH: KEINE eigenen Formulierungen erfinden - NUR das verwenden, was in den Training-Daten steht! Die Moderator-Antworten in den Training-Daten zeigen dir GENAU, welche Wörter und Formulierungen du verwenden sollst! 🚨🚨🚨
...
- 🚨 KRITISCH: Zeige EIGENINITIATIVE! Wenn der Kunde sagt, dass er Sex sucht oder Freundschaft+ sucht, dann sage auch, dass DU das suchst! Zeige Gemeinsamkeiten! Frage dann nach seinen Vorlieben, Fantasien oder wie er sich das vorstellt!
- 🚨 KRITISCH: Gehe TIEFER auf die Kundennachricht ein! Wenn der Kunde detailliert schreibt (z.B. über Vorlieben, Fantasien, Erfahrungen), dann schreibe auch detailliert zurück! Nicht so trocken - sei emotional, menschlich, interessiert!
...
🚨 KRITISCH: BEI TREFFEN/SILVESTER-ANFRAGEN! 🚨
- Wenn der Kunde nach einem Treffen/Silvester fragt, sage ZUERST, dass du an diesem Tag/Termin leider schon was vor hast
- DANN frage, wie er es sich vorstellen würde (nur EINE Frage!)
- Beispiel: "An Silvester hab ich leider schon was vor. Wie würdest du dir den Abend denn vorstellen?"
- NICHT direkt fragen ohne zuerst zu sagen, dass du schon was vor hast!
- 🚨 WICHTIG: NIEMALS sagen "Ich wäre gerne bereit" oder "Ich bin bereit" oder "Ich würde gerne kommen" - das klingt wie eine Zusage!
- 🚨 WICHTIG: Sage stattdessen "Ich könnte es mir vorstellen" oder "Ich würde gerne wissen, wie du es dir vorstellst" - vorsichtiger formulieren!
- 🚨 WICHTIG: Frage nach, wie ER es sich vorstellt, aber mache KEINE Zusage!
...
- WICHTIG: Nutze aktuelles Datum/Zeit für DACH (Europe/Berlin): ${dateTimeInfo}
...
- Bei sensiblen Themen (z.B. Krankheit): Du kannst geschickt ausweichen ohne direkt darauf eingehen zu müssen (z.B. "Mir geht es schon etwas besser" statt "Ja, ich bin noch krank geschrieben")` : "";
    ...
    try {
      const { getChatVariation, generateStyleInstruction } = require('../utils/chat-variation');
      // Extrahiere Namen für stabilen Identifier
      ...
      const chatVariation = getChatVariation(
        responseChatId || req.body?.chatId || finalChatId,
        customerName,
        fakeName,
        platformId
      );
      if (chatVariation) {
        chatVariationContext = generateStyleInstruction(chatVariation);
        console.log(`🎨 Chat-Variation: ${chatVariation.name} (${chatVariation.tone})`);
      }
    } catch (err) {
      console.warn('⚠️ Fehler beim Generieren der Chat-Variation (nicht kritisch):', err.message);
    }
...
WICHTIG: Wirke NICHT zu schlau! Du bist eine normale Person, keine Wissensdatenbank! Weiche IMMER aus, auch wenn der Kunde explizit danach fragt! NIEMALS sachlich oder wissenschaftlich antworten!`;
    ...
    if (trainingData && trainingData.conversations && Array.isArray(trainingData.conversations) && trainingData.conversations.length > 0) {
      ...
      if (hasSexualContent) {
        ...
        sortedSexualExamples.slice(0, 20).forEach(ex => {
          if (!usedMessages.has(ex.customerMessage)) {
      ...
      if (detectedSituation) {
        ...
        // Verwende ALLE passenden Situation-Beispiele (Feedback-Einträge zuerst)!
        sortedSituationExamples.forEach(ex => {
          relevantExamples.push(ex);
          usedMessages.add(ex.customerMessage);
        });
        ...
        console.log(`📚 Situation "${detectedSituation}" erkannt: ${sortedSituationExamples.length} Beispiele gefunden (${feedbackCount} Feedback-Einträge mit höherer Priorität)`);
      }
      ...
      if (vectorSearchResults.length > 0) {
        ...
        sortedVectorResults.slice(0, 20).forEach(ex => {
          if (!usedMessages.has(ex.customerMessage)) {
            relevantExamples.push(ex);
            usedMessages.add(ex.customerMessage);
          }
        });
        console.log(`✅ ${sortedVectorResults.slice(0, 20).length} Vektor-Ergebnisse hinzugefügt (semantisch ähnlich)`);
      } else {
        ...
        const selectedRelevant = shuffledTopRelevant.slice(0, Math.min(20, shuffledTopRelevant.length));
      ...
      // 3. Falls keine passenden gefunden, nimm zufällige verfügbare Beispiele als Referenz
      // WICHTIG: Feedback-Einträge zuerst priorisieren!
      if (relevantExamples.length === 0) {
        // Trenne Feedback-Einträge von normalen Einträgen
        const allAvailable = trainingData.conversations.filter(conv => conv.customerMessage);
        const feedbackExamples = allAvailable.filter(conv => conv.priority === true || conv.source === 'feedback_good' || conv.source === 'feedback_edited');
        const normalExamples = allAvailable.filter(conv => !(conv.priority === true || conv.source === 'feedback_good' || conv.source === 'feedback_edited'));
        
        // Mische Feedback-Einträge und normale Einträge, aber Feedback zuerst
        const shuffledFeedback = [...feedbackExamples].sort(() => Math.random() - 0.5);
        const shuffledNormal = [...normalExamples].sort(() => Math.random() - 0.5);
        const combined = [...shuffledFeedback, ...shuffledNormal].slice(0, 30);
        
        combined.forEach(ex => {
          relevantExamples.push(ex);
          usedMessages.add(ex.customerMessage);
        });
        console.log(`📚 Fallback: Verwende ${combined.length} zufällig ausgewählte Beispiele (${shuffledFeedback.length} Feedback-Einträge mit höherer Priorität, ${shuffledNormal.length} normale)`);
      } else if (relevantExamples.length < 15) {
        // 4. Füge zusätzliche zufällige Beispiele hinzu, wenn zu wenige gefunden wurden (max. 20 insgesamt)
        // WICHTIG: Feedback-Einträge zuerst priorisieren!
        const remaining = trainingData.conversations.filter(conv => conv.customerMessage && !usedMessages.has(conv.customerMessage));
        const remainingFeedback = remaining.filter(conv => conv.priority === true || conv.source === 'feedback_good' || conv.source === 'feedback_edited');
        const remainingNormal = remaining.filter(conv => !(conv.priority === true || conv.source === 'feedback_good' || conv.source === 'feedback_edited'));
        
        // Mische Feedback-Einträge und normale Einträge, aber Feedback zuerst
        const shuffledRemainingFeedback = [...remainingFeedback].sort(() => Math.random() - 0.5);
        const shuffledRemainingNormal = [...remainingNormal].sort(() => Math.random() - 0.5);
        const combinedRemaining = [...shuffledRemainingFeedback, ...shuffledRemainingNormal].slice(0, 20 - relevantExamples.length);
        
        combinedRemaining.forEach(ex => {
            relevantExamples.push(ex);
            usedMessages.add(ex.customerMessage);
          });
          
        const feedbackCount = combinedRemaining.filter(ex => ex.priority === true || ex.source === 'feedback_good' || ex.source === 'feedback_edited').length;
        console.log(`📚 ${combinedRemaining.length} zusätzliche zufällige Beispiele hinzugefügt (${feedbackCount} Feedback-Einträge mit höherer Priorität, Gesamt: ${relevantExamples.length})`);
      }
        ...
        try {
          ...
          if (learningStats && Object.keys(learningStats).length > 0) {
            ...
            if (learningContext) {
              ...
              console.log(`🧠 Learning-System: Bewährte Muster für Situation "${detectedSituation || 'allgemein'}" hinzugefügt`);
            }
          }
        } catch (err) {
...
🚨🚨🚨🚨🚨 KRITISCH: TRAINING-DATEN HABEN ABSOLUT HÖCHSTE PRIORITÄT! 🚨🚨🚨🚨🚨
🚨🚨🚨 Orientiere dich STRENG an den Training-Daten oben für Stil, Wortwahl und Ton! 🚨🚨🚨
🚨🚨🚨 Training-Daten überschreiben ALLE anderen Stil-Anweisungen - wenn Training-Daten etwas zeigen, dann MACH ES SO! 🚨🚨🚨
...
- Sei menschlich, emotional, verständnisvoll - nicht mechanisch oder oberflächlich!`;
    ...
    // Wenn verbotene Wörter, Meta-Kommentare, formelle Formulierungen, Begrüßungen, Wiederholungsfragen, Blockierungen, Wiederholungen oder zu viele Ausrufezeichen gefunden wurden, versuche Neu-Generierung
...
Antworte NUR mit der neuen Antwort, keine Erklärungen.`;
                ...
                const finalRetryChat = await client.chat.completions.create({
                  ...
                  max_tokens: 200,
                  temperature: 0.7 // Etwas niedrigere Temperatur für konsistentere Ergebnisse
                });
...
Antworte NUR mit der neuen Antwort, keine Erklärungen.`;
                      ...
                      const thirdRetryChat = await client.chat.completions.create({
                        ...
                        max_tokens: 200,
                        temperature: 0.5 // Niedrigere Temperatur für konsistentere, regelkonforme Ergebnisse
                      });
                      ...
                      console.error("🚨🚨🚨 KRITISCHER FEHLER: KI konnte keine regelkonforme Antwort generieren:", err);
                ...
                console.error("🚨🚨🚨 KRITISCHER FEHLER beim finalen Retry:", err);
                throw err; // Wirf den Fehler weiter, keine Fallback-Antwort
              }
            }
          }
      } catch (err) {
        console.error("🚨🚨🚨 KRITISCHER FEHLER beim Neu-Generieren der Antwort:", err);
        // KEIN Fallback - wirf den Fehler weiter, damit der Benutzer sieht, dass etwas nicht stimmt
        // Die ursprüngliche Antwort wird NICHT verwendet, wenn sie Regelverstöße enthält!
...
Antworte NUR mit der vollständigen Nachricht inklusive Frage am Ende, keine Erklärungen.`;
    ...
    try {
      const platform = req.body?.platformId || req.body?.siteInfos?.platform || 'unknown';
      const chatIdForFeedback = responseChatId || req.body?.chatId || req.body?.siteInfos?.chatId || null;
      ...
      fetch(`${req.protocol}://${req.get('host')}/api/v1/feedback`, {
        ...
        console.warn('⚠️ Konnte Feedback-Eintrag nicht erstellen (nicht kritisch):', err.message);
      });
      
      console.log(`✅ Feedback-Eintrag wird erstellt (Chat-ID: ${chatIdForFeedback}, Platform: ${platform})`);
    } catch (err) {
