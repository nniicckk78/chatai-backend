...
Nachricht: ${messageText}`;
    ...
    // Timeout für extractInfoFromMessage: 15 Sekunden
    ...
    const timeoutPromise = new Promise((_, reject) => 
      setTimeout(() => reject(new Error("extractInfoFromMessage timeout nach 15 Sekunden")), 15000)
    );
...
Antworte NUR mit der vollständigen, erweiterten Reaktivierungsnachricht (mindestens 150 Zeichen), keine Erklärungen. Die Nachricht soll fragen, warum der Kunde nicht mehr schreibt!`;
    ...
    console.log("🔍 DEBUG: foundMessageText:", foundMessageText?.substring(0, 100));
    console.log("🔍 DEBUG: client vorhanden:", !!client);
    
    // 1. Informationen extrahieren (nur wenn Nachricht vom Kunden vorhanden)
    console.log("🔍 DEBUG: Rufe extractInfoFromMessage auf...");
    try {
      extractedInfo = await extractInfoFromMessage(client, foundMessageText);
      console.log("✅ extractInfoFromMessage erfolgreich abgeschlossen");
      ...
      // Setze extractedInfo auf leeres Objekt, damit der Code weiterläuft
...
${forbiddenWordsSystemNote}`;
    ...
    console.log("🔍 DEBUG: Prüfe ob rules vorhanden:", !!rules);
    if (!rules) {
      console.log("⚠️ rules fehlt - lade Regeln nach...");
      try {
        rules = await getRules();
        console.log("✅ Regeln nachgeladen:", !!rules);
      } catch (err) {
        console.error('⚠️ Fehler beim Laden der Regeln (Fallback):', err.message);
      }
    }
    
    console.log("🔍 DEBUG: Prüfe ob trainingData vorhanden:", !!trainingData);
    if (!trainingData) {
      console.log("⚠️ trainingData fehlt - lade Training Data nach...");
      try {
        trainingData = await getTrainingData();
        console.log("✅ Training Data nachgeladen:", !!trainingData);
      } catch (err) {
        console.error('⚠️ Fehler beim Laden der Training Data (Fallback):', err.message);
      }
    }
    
    console.log("✅ Weiter mit Context-Bau für verbotene/bevorzugte Wörter...");
