...
router.post("/", asyncHandler(async (req, res, next) => {
  ...
  // Wenn Kuss/Like-Systemnachricht vorhanden ist UND keine Kunden-Nachricht, dann ist es ASA!
  // 🚨 FIX: Definiere msgsAll außerhalb der if-Blöcke, damit es überall verfügbar ist
  const msgsAll = req.body?.siteInfos?.messages || [];
  if (!isLastMessageFromFake && (!foundMessageText || foundMessageText.trim() === "") && msgsAll.length) {
    // Suche nach Kuss/Like-Systemnachrichten
    ...
    if (hasKussSystemMessage || hasLikeSystemMessage) {
      ...
      console.log(`✅ Kuss/Like-Systemnachricht gefunden (${hasKussSystemMessage ? 'Kuss' : 'Like'}) UND keine Kunden-Nachricht - setze isLastMessageFromFake = true (ASA-Fall)`);
    }
  }
