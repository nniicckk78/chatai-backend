...
router.post("/", asyncHandler(async (req, res, next) => {
  ...
  if ((!foundMessageText || foundMessageText.trim() === "") && req.body?.siteInfos?.messages) {
    ...
    // Cherry: gesamte folgende Logik erwartet [neueste … älteste]. Nur erste/letzte zu vergleichen reicht nicht (Array kann „durcheinander“ sein).
    if (isCherryOrigin && orderedMsgs.length > 1) {
      try {
        const cherryTsMs = (m) => {
          if (!m) return NaN;
          const v = m.timestamp != null ? m.timestamp : m.time;
          if (v == null) return NaN;
          const t = new Date(v).getTime();
          return isNaN(t) ? NaN : t;
        };
        orderedMsgs = [...orderedMsgs].sort((a, b) => {
          const tb = cherryTsMs(b);
          const ta = cherryTsMs(a);
          if (!isNaN(tb) && !isNaN(ta) && tb !== ta) return tb - ta;
          if (isNaN(ta) && !isNaN(tb)) return 1;
          if (!isNaN(ta) && isNaN(tb)) return -1;
          return 0;
        });
        console.log("✅ Cherry: siteInfos.messages nach Timestamp sortiert ([neueste … älteste], Extension-Reihenfolge nur Fallback).");
      } catch (eCherrySort) { /* ignore */ }
    }
