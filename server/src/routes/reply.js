...
// Helper: Lade Regeln aus Datei oder GitHub (für situations-spezifische Antworten)
...
Gib die Antwort NUR als JSON zurück, kein zusätzlicher Text. Format:
{
  "user": {
    ...
    "Wohnort": "Stadt/Ort falls erwähnt (z.B. 'Köln', 'Nähe von Erfurt', 'aus Berlin'), sonst null",
    "Work": "Beruf/Arbeit falls erwähnt, sonst null",
    "Sport and Hobbies": "Sportarten und Hobbies falls erwähnt, sonst null",
    "Sexual Preferences": "Sexuelle Vorlieben falls erwähnt (z.B. 'OHNE GUMMI', 'mit Kondom', etc.), sonst null",
    "Family": "Familienstand falls erwähnt (z.B. 'Single', 'verheiratet', 'geschieden', 'ledig'), sonst null",
...
- Name: Extrahiere ALLE Namen, die erwähnt werden (z.B. "Ich heiße Tobi" → Name: "Tobi")
- Alter: Extrahiere ALLE Altersangaben (z.B. "50 Jahre" → Age: "50", "bin 30" → Age: "30")
- Wohnort: Extrahiere ALLE Ortsangaben, auch ungenaue (z.B. "komme aus der Nähe von Erfurt" → Wohnort: "Nähe von Erfurt", "aus Berlin" → Wohnort: "Berlin")
- Family: Extrahiere ALLE Beziehungsstatus (z.B. "bin Single" → Family: "Single", "ich bin verheiratet" → Family: "verheiratet")
- Sexual Preferences: Extrahiere ALLE sexuellen Vorlieben (z.B. "OHNE GUMMI" → Sexual Preferences: "OHNE GUMMI", "nur mit Kondom" → Sexual Preferences: "mit Kondom")
...
- "Ich heiße X" → Name: "X"
- "bin X Jahre alt" oder "X Jahre" → Age: "X"
- "komme aus X" oder "aus X" oder "wohne in X" → Wohnort: "X"
- "bin Single" oder "Single" → Family: "Single"
- "OHNE GUMMI" oder "ohne Kondom" → Sexual Preferences: "OHNE GUMMI"
...
Nachricht: ${messageText}`;
    ...
    if (extractedText) {
      ...
      // Logge extrahierte Informationen für Debugging
      if (Object.keys(cleanUser).length > 0) {
        console.log("✅ Extrahierte User-Informationen:", cleanUser);
      }
      if (Object.keys(cleanAssistant).length > 0) {
        console.log("✅ Extrahierte Assistant-Informationen:", cleanAssistant);
      }
...
// Extrahiere Erstkontakt-Datum aus Nachrichten (älteste Kunden-Nachricht)
...
// Erkenne letztes Gesprächsthema aus der letzten Kunden-Nachricht
function detectLastTopic(lastCustomerMessage) {
  ...
  if (msg.includes('treffen') || msg.includes('termin') || msg.includes('verabredung') || 
      msg.includes('wann sehen') || msg.includes('wann können wir') || msg.includes('können wir uns')) {
    // Prüfe ob abgelehnt (keine Zeit, kann nicht, etc.)
  ...
  // Sexuelle Themen
  if (msg.includes('sex') || msg.includes('intim') || msg.includes('körper') || 
  ...
  if (msg.includes('arbeit') || msg.includes('job') || msg.includes('beruf') || 
      msg.includes('kollege') || msg.includes('chef') || msg.includes('büro') ||
  ...
  // Allgemeines Gespräch (Smalltalk)
  if (msg.includes('hallo') || msg.includes('hey') || msg.includes('hi') ||
      msg.includes('wie geht') || msg.includes('was machst') || msg.includes('wie läuft')) {
    return 'allgemein';
  }
  
  // Wenn kein spezifisches Thema erkannt, null zurückgeben (generische ASA)
  return null;
}
...
router.post("/", asyncHandler(async (req, res, next) => {
  ...
  try {
    ...
    if (isLastMessageFromFake) {
      ...
      // Bestimme Kunden-Typ (berücksichtigt Nachrichtenanzahl UND Zeit seit Erstkontakt)
      ...
      console.log(`📊 Kunden-Analyse: ${customerMessageCount} Nachrichten, ${daysSinceFirstContact !== null ? daysSinceFirstContact + ' Tage' : 'unbekannt'} seit Erstkontakt -> Typ: ${customerType}`);
      
      // Finde letzte Kunden-Nachricht für Thema-Erkennung
      ...
      console.log(`📝 Letztes Gesprächsthema: ${lastTopic || 'kein spezifisches Thema (generische ASA)'}`);
      ...
      try {
        ...
        if (asaExamples.length > 0) {
          ...
          let matchingExamples = asaExamples.filter(asa => {
            ...
            // Wenn letztes Thema vorhanden, muss es übereinstimmen
          ...
          // Falls keine passenden Beispiele gefunden, nimm generische für diesen Kunden-Typ
          ...
          // Falls immer noch keine, nimm irgendwelche für diesen Kunden-Typ
          if (matchingExamples.length === 0) {
            matchingExamples = asaExamples.filter(asa => asa.customerType === customerType);
          }
          
          if (matchingExamples.length > 0) {
            // Wähle zufällig eine passende ASA
            const selectedASA = matchingExamples[Math.floor(Math.random() * matchingExamples.length)];
            asaMessage = selectedASA.asaMessage.trim();
            console.log(`✅ ASA aus Training Data verwendet (${matchingExamples.length} passende Beispiele gefunden)`);
          }
        }
      } catch (err) {
      ...
      if (!asaMessage) {
        console.log("⚠️ Keine ASA-Beispiele aus Training Data, verwende Fallback-Templates...");
...
- ${customerType === 'langzeit' ? "Sei persönlicher und emotionaler, da es ein Langzeitkunde ist." : customerType === 'schlechter_langzeit' ? "Sei verständnisvoll aber auch etwas traurig, da es ein langjähriger aber weniger aktiver Kunde ist." : "Sei freundlich und hoffnungsvoll, da es ein Neukunde ist."}
...
Antworte NUR mit der vollständigen, erweiterten Nachricht (mindestens 150 Zeichen), keine Erklärungen.`;
          ...
          const asaExtended = await client.chat.completions.create({
            ...
            messages: [
              { 
                role: "system", 
                content: `Du erweiterst Reaktivierungsnachrichten auf mindestens 150 Zeichen. Fokus auf Reaktivierung, natürlicher Ton, keine Bindestriche/Anführungszeichen/ß. ${customerType === 'langzeit' ? "Für Langzeitkunden: persönlicher, emotionaler Ton." : customerType === 'schlechter_langzeit' ? "Für langjährige aber weniger aktive Kunden: verständnisvoll aber auch etwas traurig." : "Für Neukunden: freundlich, hoffnungsvoll."} Übernehme den Schreibstil der letzten Moderator-Nachrichten.` 
              },
    ...
    // Baue System-Prompt mit verbotenen Wörtern (falls vorhanden)
    ...
    if (rules && rules.forbiddenWords && Array.isArray(rules.forbiddenWords) && rules.forbiddenWords.length > 0) {
      forbiddenWordsSystemNote = `\n- KRITISCH: Die folgenden Wörter/Phrasen sind ABSOLUT VERBOTEN und dürfen NIEMALS verwendet werden: ${rules.forbiddenWords.map(w => `"${w}"`).join(', ')}. Verwende stattdessen alternative Formulierungen!`;
    }
...
- Beispiel-Nachrichten: ${writingStyle.sampleTexts ? writingStyle.sampleTexts.substring(0, 300) : ""}` : "";
    ...
    try {
      ...
      if (trainingData && trainingData.conversations) {
        console.log(`📚 Training Data geladen: ${trainingData.conversations.length} Gespräche gefunden`);
      } else {
        console.log("⚠️ Training Data ist leer oder nicht verfügbar");
      }
    } catch (err) {
    ...
    if (rules) {
      if (rules.forbiddenWords && Array.isArray(rules.forbiddenWords) && rules.forbiddenWords.length > 0) {
        forbiddenWordsContext = `\n\n❌ VERBOTENE WÖRTER/PHRASEN (ABSOLUT NIEMALS VERWENDEN - KRITISCH!):\n${rules.forbiddenWords.map(w => `- "${w}"`).join('\n')}\n\nWICHTIG: Diese Wörter/Phrasen sind STRENG VERBOTEN! Verwende sie NIEMALS, auch nicht in ähnlicher Form oder als Variation! Wenn du dir unsicher bist, verwende stattdessen eine andere Formulierung!`;
        console.log(`🚫 ${rules.forbiddenWords.length} verbotene Wörter/Phrasen geladen`);
      }
...
- Keine Flugzeugbegleiter, Zugkontrolleur oder ähnliche Berufe (zu spezifisch, könnte zu Treffen führen)`;
    ...
    // Füge allgemeine Regeln hinzu (falls vorhanden)
    ...
    if (trainingData && trainingData.conversations && Array.isArray(trainingData.conversations) && trainingData.conversations.length > 0) {
      ...
      if (specificInstructions && specificInstructions.includes("BENUTZERDEFINIERTE SITUATION:")) {
        ...
        if (situationMatch) {
          ...
          console.log(`📋 Erkannte Situation aus Rules: "${detectedSituation}"`);
        }
      }
      ...
      if (!detectedSituation && rules && rules.situationalResponses) {
        ...
        if (detectedSituation) {
          console.log(`📋 Situation aus Nachricht erkannt: "${detectedSituation}"`);
        }
      }
      
      // Suche Training-Beispiele für die erkannte Situation
      if (detectedSituation) {
        const situationExamples = trainingData.conversations.filter(conv => {
          ...
          // Exakte Übereinstimmung oder Teilübereinstimmung (z.B. "Bot-Vorwurf" passt zu "Bot Vorwurf")
        ...
        if (situationExamples.length > 0) {
          relevantExamples.push(...situationExamples.slice(0, 5)); // Erhöht auf 5 Beispiele pro Situation
          console.log(`✅ ${situationExamples.length} Beispiele für Situation "${detectedSituation}" gefunden`);
        } else {
          console.log(`⚠️ Keine Training-Beispiele für Situation "${detectedSituation}" gefunden`);
        }
      }
      
      // 2. Finde Beispiele mit ähnlichen Kunden-Nachrichten (Keyword-Matching) - weniger restriktiv
      if (relevantExamples.length < 5) {
        ...
        const similarExamples = trainingData.conversations
          .filter(conv => {
            ...
            // Überspringe bereits hinzugefügte Beispiele
        ...
        if (similarExamples.length > 0) {
          console.log(`✅ ${similarExamples.length} ähnliche Beispiele gefunden (Keyword-Matching)`);
        }
      }
      
      // 3. Falls immer noch nicht genug, nimm zufällige Beispiele aus verschiedenen Situationen
      if (relevantExamples.length < 5) {
        ...
        if (randomExamples.length > 0) {
          console.log(`✅ ${randomExamples.length} zufällige Beispiele hinzugefügt (für allgemeine Referenz)`);
        }
      }
      ...
      if (relevantExamples.length === 0) {
        ...
        console.log(`⚠️ Keine passenden Beispiele gefunden, verwende letzte ${relevantExamples.length} Beispiele`);
      }
      
      // Baue Training Examples Context
      if (relevantExamples.length > 0) {
        console.log(`📚 Verwende ${relevantExamples.length} Training-Beispiele im Prompt`);
        ...
        trainingExamplesContext += `\n\nWICHTIG: Orientiere dich STRENG an diesen Beispielen! Passe deinen Schreibstil, Ton, Länge, Formulierungen und die Art der Antworten EXAKT an die Beispiel-Antworten an! Verwende ähnliche Wörter, ähnliche Satzstrukturen und ähnliche Emotionen wie in den Beispielen!`;
      } else {
        console.log("⚠️ Keine Training-Beispiele gefunden, obwohl Training Data vorhanden ist");
      }
    } else {
      console.log("⚠️ Keine Training Data verfügbar oder leer");
    }
