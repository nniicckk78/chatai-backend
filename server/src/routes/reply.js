...
router.post("/", asyncHandler(async (req, res, next) => {
  ...
  try {
    ...
    if (newestSystemTriggerMsg && tsRaw) {
      ...
      // Gold/S69/Mods: fehlende Timestamps + ASA → alte Kuss-Zeilen wirkten „frisch“ (Sortierung 0 === 0).
      // FPC: Timestamps fehlen oft – echte Kuss/Like-Banner trotzdem als Trigger behalten.
      const keepFpcKussLikeNoTs =
        isFPC &&
      ...
      if (!keepFpcKussLikeNoTs) {
        ...
        console.log('✅ FPC: Kuss/Like-Systemzeile ohne Timestamp behalten (Danke-/Trigger-Pfad).');
      }
    }
  ...
  function getThankYouTriggerType(newestMsg, foundMsg) {
    ...
    if (suppressCustomerMessageFallbackForAsa) {
      ...
      if (found && found.length < 220 && /^\s*kuss\s+nachricht\b/i.test(found)) return null;
    }
