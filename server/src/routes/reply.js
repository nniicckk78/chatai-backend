...
/**
 * LLM: Extrahiert einen vom Fake genannten Wohnort/Herkunft aus der Antwort (oder null).
 */
async function extractFakeWohnortFromMessageLLM(client, model, messageText) {
  if (!client || !messageText || typeof messageText !== 'string') return null;
  ...
  try {
      ...
      messages: [
        {
          role: 'system',
          content:
            'Du prüfst Chat-Antworten eines Dating-Moderators (Fake-Profil). Antworte NUR mit gültigem JSON: {"genannterWohnort": string|null}. ' +
            '"genannterWohnort" = genau EIN Ortsname in DE/AT/CH, wenn die Antwort explizit nennt, wo der Schreibende wohnt oder herkommt ' +
            '(z. B. "ich wohne in …", "komme aus …", "bin aus …"). Namen des Kunden oder vage Formulierungen ohne Ort = null. Kein Text außerhalb JSON.'
        },
        { role: 'user', content: messageText.slice(0, 4000) }
      ],
      max_tokens: 120,
      temperature: 0.1,
      response_format: { type: 'json_object' }
    ...
    if (!raw) return null;
    const parsed = JSON.parse(raw);
    const p = parsed.genannterWohnort;
    if (p == null || !String(p).trim()) return null;
    const name = String(p).trim();
    if (name.length < 2 || name.length > 80) return null;
    return name;
  } catch (e) {
    console.warn('⚠️ extractFakeWohnortFromMessageLLM:', e?.message || e);
    return null;
  }
}

/**
 * Nachgelagerte Sicherheitsprüfung: Genannter Wohnort in der Nachricht darf nicht >50 km vom Kunden entfernt sein.
 * Nur sinnvoll, wenn customerCity bekannt und Wohnort-Thema (extern über Flag gesteuert).
 */
async function validateGeneratedMessageFakeWohnortDistance(client, model, customerCity, messageText) {
  if (!client || !customerCity || String(customerCity).trim().length < 2 || !messageText) return { ok: true, skipped: true };
  const place = await extractFakeWohnortFromMessageLLM(client, model, messageText);
  if (!place) return { ok: true };
  const v = await validateCityDistanceFromCustomer(customerCity, place, { maxKm: FAKE_WOHNORT_ABS_MAX_KM, minKmDistinct: 0 });
  if (!v.ok) {
    const kmPart = v.km != null ? ` (${Math.round(v.km)} km)` : '';
    return {
      ok: false,
      error: `Der in der Nachricht genannte Wohnort „${place}“ ist zu weit vom Kunden entfernt${kmPart}. Maximal ${FAKE_WOHNORT_ABS_MAX_KM} km erlaubt.`
    };
  }
  return { ok: true, km: v.km, place };
}
...
router.post("/", asyncHandler(async (req, res, next) => {
  ...
  // Wohnort-Kontext: KI prüft, ob in der Antwort ein genannter Fake-Wohnort per Nominatim >50 km vom Kunden entfernt ist → keine Auslieferung ins Textfeld
  ...
  if (isWohnortDistanceCheck && client && generatedMessage && generatedMessage.trim()) {
    const customerCityForDistance = String(
    ...
    if (customerCityForDistance.length >= 2) {
      const wohnortCheck = await validateGeneratedMessageFakeWohnortDistance(client, AI_MODEL, customerCityForDistance, generatedMessage);
      if (!wohnortCheck.ok && !wohnortCheck.skipped) {
