...
router.post("/", asyncHandler(async (req, res, next) => {
  ...
  // Versuche Nachricht zu generieren
    ...
    console.log("🔍 DEBUG: Nach Training Data Loading - isLastMessageFromFake:", isLastMessageFromFake);
    ...
    if (isLastMessageFromFake) {
      console.log("🔍 DEBUG: Betrete ASA-Block");
...
Antworte NUR mit der vollständigen, erweiterten Reaktivierungsnachricht (mindestens 150 Zeichen), keine Erklärungen. Die Nachricht soll fragen, warum der Kunde nicht mehr schreibt!`;
    ...
    console.log("🔍 DEBUG: Nach ASA-Block - foundMessageText:", foundMessageText ? foundMessageText.substring(0, 50) : "(leer)");
  ...
  console.log("🔍 DEBUG: Prüfe auf leere messageText - foundMessageText:", foundMessageText ? "vorhanden" : "leer", "isLastMessageFromFake:", isLastMessageFromFake);
  ...
  console.log("🔍 DEBUG: Betrete normalen Reply-Pfad");
...
${forbiddenWordsSystemNote}`;
    ...
    // 🚨🚨🚨 MULTI-AGENT PIPELINE: Initialisiere multiAgentResults (wird später gefüllt)
    let multiAgentResults = null;
    
    // 🚨 WICHTIG: Initialisiere detectedSituations früh (wird später befüllt, aber Pipeline braucht es)
    ...
    // 🤖 MULTI-AGENT: Wird später durch Pipeline-Ergebnisse erweitert/ersetzt (siehe Zeile ~4665)
    let customerContext = [];
    ...
    // 🤖 MULTI-AGENT: Nutze Pipeline-Ergebnisse (PRIORITÄT), Fallback auf altes System
    let styleContext = "";
    let lastModeratorMessage = null;
    
    if (multiAgentResults && multiAgentResults.style && multiAgentResults.style.success && 
        multiAgentResults.style.styleContext && multiAgentResults.style.styleContext.length > 0) {
      // Nutze styleContext von Pipeline
      styleContext = multiAgentResults.style.styleContext;
      console.log(`🤖 Multi-Agent: Verwende styleContext von Pipeline (${styleContext.length} Zeichen)`);
      
      // Extrahiere lastModeratorMessage aus messages (für Logging)
      try {
      ...
      try {
        ...
        if (moderatorMessages.length > 0) {
          ...
          if (lastModeratorMessage && lastModeratorMessage.length > 0) {
            ...
            console.log(`✅ ${moderatorMessages.length} Moderator-Nachricht(en) und ${customerMessages.length} Kunden-Nachricht(en) analysiert (altes System, letzte Moderator: ${lastModeratorMessage.substring(0, 100)}...)`);
          }
        }
      } catch (err) {
...
- Bei sensiblen Themen (z.B. Krankheit): Du kannst geschickt ausweichen ohne direkt darauf eingehen zu müssen (z.B. "Mir geht es schon etwas besser" statt "Ja, ich bin noch krank geschrieben")` : "";
    ...
    if (imageDescriptions.length > 0) {
      ...
      // 🤖 MULTI-AGENT: Nutze Pipeline-Ergebnisse für Image-Analyse (PRIORITÄT)
      // Wenn die Pipeline erfolgreich war und Image-Analyse durchgeführt hat, verwende sie!
      if (multiAgentResults && multiAgentResults.image && multiAgentResults.image.success && 
          multiAgentResults.image.imageType) {
        console.log(`🤖 Multi-Agent: Verwende Image-Analyse von Pipeline (Typ: ${multiAgentResults.image.imageType})`);
        // Setze imageType aus Pipeline-Ergebnissen (überschreibt alte Analyse, wenn vorhanden)
        imageType = multiAgentResults.image.imageType;
        console.log(`📸 Image-Type aus Multi-Agent Pipeline übernommen: ${imageType}`);
      }
    ...
    // 🧠 EMOTIONALE INTELLIGENZ - Analysiere Stimmung des Kunden
    // 🤖 MULTI-AGENT: Nutze Pipeline-Ergebnisse (PRIORITÄT), Fallback auf altes System
    let moodContext = "";
    if (multiAgentResults && multiAgentResults.mood && multiAgentResults.mood.success && 
        multiAgentResults.mood.mood !== 'neutral' && multiAgentResults.mood.confidence > 0.5) {
      // Nutze Mood-Analyst von Pipeline
      moodContext = multiAgentResults.mood.instructions;
      console.log(`🤖 Multi-Agent: Verwende Mood-Analyse von Pipeline (Mood: ${multiAgentResults.mood.mood}, Confidence: ${(multiAgentResults.mood.confidence * 100).toFixed(0)}%)`);
    } else if (client && foundMessageText && foundMessageText.trim().length > 10) {
      // Fallback: Altes System
      try {
        ...
        if (moodAnalysis && moodAnalysis.mood !== 'neutral' && moodAnalysis.confidence > 0.5) {
          ...
          console.log(`🧠 Stimmungs-Analyse integriert (altes System): ${moodAnalysis.mood} (Confidence: ${(moodAnalysis.confidence * 100).toFixed(0)}%)`);
        }
      } catch (err) {
        console.warn('⚠️ Fehler bei Stimmungs-Analyse (nicht kritisch):', err.message);
      }
    ...
    // 🎯 PROAKTIVE GESPRÄCHSFÜHRUNG - Erkenne stagnierende Gespräche
    // 🤖 MULTI-AGENT: Nutze Pipeline-Ergebnisse (PRIORITÄT), Fallback auf altes System
    let proactiveContext = "";
    if (multiAgentResults && multiAgentResults.proactive && multiAgentResults.proactive.success && 
        multiAgentResults.proactive.isStagnant && multiAgentResults.proactive.suggestions.length > 0) {
      // Nutze Proactive-Analyst von Pipeline
      const stagnation = multiAgentResults.proactive;
      ...
      console.log(`🤖 Multi-Agent: Verwende Proactive-Analyse von Pipeline (Stagnant: ${stagnation.isStagnant}, Suggestions: ${stagnation.suggestions.length})`);
    } else if (foundMessageText && foundMessageText.trim().length > 0) {
      // Fallback: Altes System
      try {
        ...
        if (stagnation.isStagnant && stagnation.suggestions.length > 0) {
          proactiveContext = `\n\n🎯 PROAKTIVE GESPRÄCHSFÜHRUNG: Stagnation erkannt!\n\nDas Gespräch wirkt etwas langweilig/uninteressant (kurze, generische Antworten).\n\n🚨🚨🚨 WICHTIG: Sei PROAKTIV und BELEBE das Gespräch! 🚨🚨🚨\n- Stelle INTERESSANTE Fragen, die zum aktuellen Gespräch passen!\n- Wechsle NICHT abrupt das Thema - es muss zum Kontext passen!\n- Wenn das Gespräch über Kochen geht, kannst du flirty werden: "Was würdest du denn gerne mit mir kochen?"\n- Wenn das Gespräch über Arbeit geht, frage nach Details: "Was machst du denn da so?"\n- Wenn das Gespräch über Hobbies geht, zeige Interesse: "Erzähl mir mehr darüber!" (🚨 KRITISCH: NICHT "Das klingt spannend" - "spannend" ist VERBOTEN!)\n\nMögliche Themenvorschläge (NUR wenn sie zum Gespräch passen!):\n${stagnation.suggestions.map((s, i) => `${i + 1}. ${s}`).join('\n')}\n\n🚨🚨🚨 KRITISCH: Diese Vorschläge sind NUR Beispiele - verwende sie NUR wenn sie zum aktuellen Gespräch passen! 🚨🚨🚨\n`;
          console.log(`🎯 Proaktive Gesprächsführung aktiviert (altes System): ${stagnation.suggestions.length} Vorschläge`);
        }
      } catch (err) {
    ...
    // 🤖🤖🤖 MULTI-AGENT PIPELINE: Führe Multi-Agent-Analyse FRÜH durch (HAUPTWEG) 🤖🤖🤖
    // WICHTIG: Pipeline wird HIER aufgerufen, bevor der alte Reply-Pfad läuft!
    // Der alte Code wird NUR als Fallback verwendet, wenn die Pipeline fehlschlägt!
    console.log("🔍 DEBUG: Starte Multi-Agent Pipeline...");
    try {
      ...
      console.log("🔍 DEBUG: Rufe runMultiAgentPipeline auf...");
      ...
      console.log("🔍 DEBUG: Multi-Agent Pipeline Aufruf abgeschlossen");
    ...
    console.log("🔍 DEBUG: Nach Multi-Agent Pipeline Block");
    ...
    // 🤖 MULTI-AGENT: Nutze Pipeline-Ergebnisse für Regeln (PRIORITÄT)
    // Verwende Agent-Ergebnisse, falls verfügbar, sonst Fallback auf ursprüngliche Regeln
    let applicableForbiddenWords = rules?.forbiddenWords || [];
    let applicablePreferredWords = rules?.preferredWords || [];
    
    if (multiAgentResults && multiAgentResults.rules && multiAgentResults.rules.success) {
      if (multiAgentResults.rules.applicableForbiddenWords && multiAgentResults.rules.applicableForbiddenWords.length > 0) {
        applicableForbiddenWords = multiAgentResults.rules.applicableForbiddenWords;
        console.log(`🤖 Multi-Agent: Verwende ${applicableForbiddenWords.length} gefilterte verbotene Wörter von Pipeline (PRIORITÄT)`);
      }
      if (multiAgentResults.rules.applicablePreferredWords && multiAgentResults.rules.applicablePreferredWords.length > 0) {
        applicablePreferredWords = multiAgentResults.rules.applicablePreferredWords;
        console.log(`🤖 Multi-Agent: Verwende ${applicablePreferredWords.length} gefilterte bevorzugte Wörter von Pipeline (PRIORITÄT)`);
      }
    }
    
    if (rules || applicableForbiddenWords.length > 0 || applicablePreferredWords.length > 0) {
      if (applicableForbiddenWords && Array.isArray(applicableForbiddenWords) && applicableForbiddenWords.length > 0) {
        // 🚨 PROAKTIVE PRÄVENTION: Explizite Liste der verbotenen Wörter
        const forbiddenWordsList = applicableForbiddenWords.map(w => `"${w}"`).join(', ');
        forbiddenWordsContext = `\n\n❌❌❌❌❌❌❌❌❌ VERBOTENE WÖRTER/PHRASEN (ABSOLUT NIEMALS VERWENDEN!) ❌❌❌❌❌❌❌❌❌\n\nDie folgenden ${applicableForbiddenWords.length} Wörter/Phrasen sind ABSOLUT VERBOTEN:\n${applicableForbiddenWords.map(w => `- "${w}"`).join('\n')}\n\n🚨🚨🚨🚨🚨 PROAKTIVE PRÄVENTION - VERWENDE DIESE WÖRTER NIEMALS! 🚨🚨🚨🚨🚨\n\n🚨🚨🚨 KRITISCH: PRÜFE DEINE ANTWORT VOR DEM SCHREIBEN! 🚨🚨🚨\n- Enthält deine Antwort eines dieser Wörter? → DANN SCHREIBE SIE UM!\n- Verwende sie NIEMALS, auch nicht in ähnlicher Form (z.B. "spannend" verboten = auch NICHT "spannende", "spannendes", "spannend!", "spannend?", etc.)\n- Verwende sie NIEMALS als Teil eines anderen Wortes\n- Verwende sie NIEMALS als Variation oder Synonym\n- Verwende sie NIEMALS in Kombination mit anderen Wörtern\n\nBEISPIELE für VERBOTENE Verwendungen:\n- "spannend" verboten → VERBOTEN: "spannende", "spannendes", "spannendste", "spannend!", "das ist spannend", "wie spannend", "total spannend"\n- "Das klingt" verboten → VERBOTEN: "Das klingt gut", "klingt total", "klingt sehr", "klingt nach", "klingt interessant"\n- "reiz" verboten → VERBOTEN: "reiz", "Reiz", "reizvoll", "reizt", "reizende", "reizend"\n\n🚨🚨🚨 KRITISCH: Diese Regel hat HÖCHSTE PRIORITÄT und überschreibt ALLE anderen Anweisungen! 🚨🚨🚨\n🚨🚨🚨 KRITISCH: Wenn du eine Antwort generierst, die eines dieser Wörter enthält, dann ist die Antwort FALSCH und muss neu geschrieben werden! 🚨🚨🚨\n🚨🚨🚨 KRITISCH: Wenn du dir unsicher bist, ob ein Wort verboten ist, verwende IMMER eine andere Formulierung! 🚨🚨🚨`;
        console.log(`🚫 ${applicableForbiddenWords.length} verbotene Wörter/Phrasen geladen und aktiviert${multiAgentResults?.rules?.success ? ' (von Multi-Agent Pipeline gefiltert)' : ''}`);
      }
      if (applicablePreferredWords && Array.isArray(applicablePreferredWords) && applicablePreferredWords.length > 0) {
        preferredWordsContext = `\n\n🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨 KRITISCH: BEVORZUGTE WÖRTER - HÖCHSTE PRIORITÄT! 🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨\n\nDie folgenden ${applicablePreferredWords.length} Wörter sind BEVORZUGT und sollten REGELMÄSSIG verwendet werden:\n${applicablePreferredWords.map(w => `- "${w}"`).join('\n')}\n\n🚨🚨🚨🚨🚨🚨🚨🚨🚨 ABSOLUT KRITISCH - VERWENDE DIESE WÖRTER! 🚨🚨🚨🚨🚨🚨🚨🚨🚨\n\n🚨🚨🚨🚨🚨 KRITISCH: BEVORZUGTE WÖRTER HABEN HÖCHSTE PRIORITÄT NACH TRAINING-DATEN! 🚨🚨🚨🚨🚨\n⭐⭐⭐ WICHTIG: Diese Wörter sind NICHT ohne Grund da - sie wurden basierend auf Feedback und Qualität ausgewählt! ⭐⭐⭐\n⭐ WICHTIG: Integriere diese Wörter NATÜRLICH in deine Antworten, wo sie thematisch passen!\n⭐ Verwende sie REGELMÄSSIG - nicht nur bei Neukunden, sondern IMMER wenn es passt!\n⭐ Diese Wörter helfen dir, natürlicher und passender zu klingen!\n⭐ Orientiere dich an den Training-Daten - dort siehst du, wie diese Wörter verwendet werden!\n⭐ Kombiniere bevorzugte Wörter MIT Training-Daten - beide zusammen = BESTE Qualität!\n\n🚨 KRITISCH: KEIN "random" reinwerfen! NUR wenn es zur Message passt und nicht unnatürlich wirkt!\n🚨 ABER: Wenn ein Preferred Word thematisch passt, dann VERWENDE ES - nicht weglassen!\n🚨🚨🚨 WICHTIG: Bevorzugte Wörter + Training-Daten + Learning-System = BESTE Kombination! 🚨🚨🚨\n\n`;
        console.log(`⭐ ${applicablePreferredWords.length} bevorzugte Wörter geladen und aktiviert${multiAgentResults?.rules?.success ? ' (von Multi-Agent Pipeline gefiltert)' : ''}`);
      }
    }
    ...
    // detectedSituations wurde bereits oben initialisiert (bei Zeile 3463, für Multi-Agent Pipeline)
...
WICHTIG: Wirke NICHT zu schlau! Du bist eine normale Person, keine Wissensdatenbank! Weiche IMMER aus, auch wenn der Kunde explizit danach fragt! NIEMALS sachlich oder wissenschaftlich antworten!`;
    ...
    if (trainingData && trainingData.conversations && Array.isArray(trainingData.conversations) && trainingData.conversations.length > 0) {
      ...
      // WICHTIG: hasSexualContent wird hier deklariert (außerhalb des if-Blocks), damit es überall verfügbar ist
      let hasSexualContent = !isRejection && (
        lowerMessage.includes("titten") || lowerMessage.includes("brüste") || lowerMessage.includes("arsch") ||
        lowerMessage.includes("po") || lowerMessage.includes("pussy") || lowerMessage.includes("schwanz") ||
        lowerMessage.includes("sex") || lowerMessage.includes("ficken") || lowerMessage.includes("vorlieben") ||
        lowerMessage.includes("sexuell") || lowerMessage.includes("geil") || lowerMessage.includes("lust") ||
      ...
      // 🚨 ALTER PIPELINE-AUFRUF ENTFERNT - Pipeline wird jetzt früher aufgerufen (nach Zeile 3794)!
      // Die Pipeline-Ergebnisse werden weiter unten verwendet (ab Zeile ~4833)
      
      // 🤖 MULTI-AGENT: Nutze Pipeline-Ergebnisse für customerContext (PRIORITÄT)
      // Aktualisiere customerContext aus Pipeline-Ergebnissen (falls vorhanden)
      if (multiAgentResults && multiAgentResults.profile && multiAgentResults.profile.success && 
          multiAgentResults.profile.customerContext && multiAgentResults.profile.customerContext.length > 0) {
        customerContext = multiAgentResults.profile.customerContext;
        console.log(`🤖 Multi-Agent: Verwende customerContext von Pipeline (${customerContext.length} Einträge, PRIORITÄT)`);
      }
      
      // 🤖 MULTI-AGENT: Nutze Pipeline-Ergebnisse für Training-Daten (PRIORITÄT)
      // Wenn die Pipeline erfolgreich war und Training-Daten gefunden hat, verwende sie!
      let usePipelineTrainingData = false;
      if (multiAgentResults && multiAgentResults.training && multiAgentResults.training.success && 
          multiAgentResults.training.selectedExamples && multiAgentResults.training.selectedExamples.length > 0) {
        console.log(`🤖 Multi-Agent: Verwende ${multiAgentResults.training.selectedExamples.length} Training-Beispiele von Pipeline (PRIORITÄT)`);
        // Setze relevantExamples aus Pipeline-Ergebnissen
        relevantExamples = multiAgentResults.training.selectedExamples.slice(0, 15); // Max 15 Beispiele
        // Markiere diese Beispiele als verwendet
        multiAgentResults.training.selectedExamples.forEach(ex => {
          if (ex.customerMessage) usedMessages.add(ex.customerMessage);
        });
        usePipelineTrainingData = true; // Markiere, dass wir Pipeline-Daten verwenden
      }
      ...
      if (multiAgentResults && multiAgentResults.image && multiAgentResults.image.success && 
          multiAgentResults.image.imageType) {
        console.log(`🤖 Multi-Agent: Verwende Image-Analyse von Pipeline (Typ: ${multiAgentResults.image.imageType})`);
        // Setze imageType aus Pipeline-Ergebnissen (wird später für imageContext verwendet)
      ...
      // 🤖 MULTI-AGENT: Nur wenn Pipeline KEINE Training-Daten hatte, verwende bestehende Logik
      if (!usePipelineTrainingData) {
      ...
      // 🚨 NEU: Mit 200+ Training-Daten können wir mehr Beispiele verwenden (10-15 für bessere Abdeckung)
      // Priorität: 1. Sexuelle Themen (wenn vorhanden), 2. Situationen, 3. Vektor-Suche
      const maxTotalExamples = relevantExamples.length >= 10 ? 15 : relevantExamples.length >= 7 ? 12 : 10; // Dynamisch: 10-15 Beispiele
...
- Sei menschlich, emotional, verständnisvoll - nicht mechanisch oder oberflächlich!`;
    ...
    let chat;
    try {
      // 🚨 STAGE 3: Generation mit höherer Temperature für natürlichere Antworten
      // Temperature 0.7 für natürlichere, weniger steife Antworten (ähnlich wie das alte System)
      chat = await client.chat.completions.create({
        ...
        temperature: 0.7 // 🚨 ERHÖHT: Natürlichere, weniger steife Antworten (ähnlich wie das alte System mit GPT-4o-mini)
      });
    ...
    // WICHTIG: hasSexualContent wurde bereits oben deklariert (Zeile 4416) - hier nur aktualisieren, wenn noch nicht gesetzt
    // Falls hasSexualContent nicht verfügbar ist (z.B. wenn Pipeline verwendet wurde), dann hier setzen
    if (typeof hasSexualContent === 'undefined') {
      ...
      hasSexualContent = sexualKeywords.some(keyword => validatedMessageLower.includes(keyword));
    }
