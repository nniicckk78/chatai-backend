...
router.post("/", asyncHandler(async (req, res, next) => {
  ...
  if ((!foundMessageText || foundMessageText.trim() === "") && req.body?.siteInfos?.messages) {
    ...
    if (messagesToUse.length > 0) {
      ...
      } // Ende if (!usedImageOnlyPath) – Blenny/Iluvo „nur Bild“ überspringt Substanz-/Kombi-Logik
  ...
  if (isFirstMessage && !isAVZ) {
    ...
    try {
      // 🚨 Like/Kuss/Gefällt hat VORRANG: Bei Erstnachricht + Systemnachricht immer Freude/Danke + Fragen (nicht Grok/Erstnachricht-Stil)
      if (hasLikeSystemMessage || hasKussSystemMessage || hasGefaelltSystemMessage) {
