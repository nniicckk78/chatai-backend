...
router.post("/", asyncHandler(async (req, res, next) => {
  ...
  const hasKussSystemMessage = msgsAll.some(m => {
    ...
    if (isSentMessage(m)) return false; // nur echte Systemnachricht der Plattform, nicht Moderator-Text mit "Kuss"
  ...
  const hasLikeSystemMessage = msgsAll.some(m => {
    if (!m || typeof m.text !== "string") return false;
    if (isSentMessage(m)) return false; // nur echte Systemnachricht der Plattform, nicht Moderator-Text mit "like"
  ...
  const hasGefaelltSystemMessage = msgsAll.some(m => {
    if (!m || typeof m.text !== "string") return false;
    if (isSentMessage(m)) return false; // nur echte Systemnachricht der Plattform
    const text = m.text.toLowerCase();
  ...
  // AVZ/Plattform: "Benutzer X dich zu seinen/ihren Favoriten hinzugefügt" / "Hallo [Fake], Wir möchten dir mitteilen, dass … dich zu Favoriten hinzugefügt" – wie Like/Kuss: Danke + Fragen
  ...
  if (!alreadyThankedForFriendRequest && (isFriendRequest || isBefreundetSeinInFoundText) && !hasSubstantialCustomerMessage) {
    ...
    friendRequestMessage = stripInternalPlaceholders(friendRequestMessage);
  ...
  if (isFirstMessage && !isAVZ) {
    ...
    try {
      ...
      if (hasLikeSystemMessage || hasKussSystemMessage || hasGefaelltSystemMessage || hasFavoritenSystemMessage || hasGrußSystemMessage) {
        const isKuss = hasKussSystemMessage && !hasLikeSystemMessage && !hasGefaelltSystemMessage && !hasFavoritenSystemMessage && !(hasGrußSystemMessage && isGrußTheNewestMessage);
        const isGruß = hasGrußSystemMessage && isGrußTheNewestMessage;
        ...
        const firstMessageInstructions = isFavoriten
          ...
          : isGruß
          ? "Der Kunde hat dir einen GRUSS gesendet (Blenny/DF Systemnachricht). NUR: 1) Kurz fuer den Gruss bedanken – VARIIERE: z.B. 'Das freut mich!', 'Oh wie suess!', 'Super dass du dich meldest!', 'Danke fuer deinen Gruss!', 'Nice, danke!'. 2) 1-2 gespraechsoeffnende Fragen – ABWECHSLUNGSREICH (wie geht es dir, was machst du, Tag, Arbeit). KEINE Treffen-Vorschlaege, keine Anspielungen. VERBOTEN: Dich vorstellen (Name, Alter, Wohnort)."
          : isKuss
        ...
        if (likeKussFirstMessage && likeKussFirstMessage.trim() !== "") {
          ...
          const safeLikeKuss = stripInternalPlaceholders(likeKussFirstMessage || '');
          return res.status(200).json({
            resText: safeLikeKuss,
            replyText: safeLikeKuss,
            summary: extractedInfo,
            chatId: chatId || finalChatId || "00000000",
  ...
  if (!isAVZ && !isFirstMessage && (hasLikeSystemMessage || hasKussSystemMessage || hasGefaelltSystemMessage || (hasGrußSystemMessage && isGrußTheNewestMessage) || (hasGrußFromAsa && isGrußTheNewestMessage)) && noRealCustomerText) {
    console.log("✅ Like/Kuss/Gefällt/Gruß ohne echten Kundentext (oder nur Gruß-Systemnachricht) – generiere Danke/Freude + Gesprächsstarter (kein ASA).");
    try {
      if (triggerGrußResponse) {
        const grußInstructions = "Der Kunde hat dir einen GRUSS gesendet (Blenny/DF Systemnachricht). NUR: 1) Kurz fuer den Gruss bedanken – VARIIERE: z.B. 'Das freut mich!', 'Oh wie suess!', 'Super dass du dich meldest!', 'Danke fuer deinen Gruss!', 'Nice, danke!'. 2) 1-2 gespraechsoeffnende Fragen – ABWECHSLUNGSREICH (wie geht es dir, was machst du, Tag, Arbeit). KEINE Treffen-Vorschlaege, keine Anspielungen. VERBOTEN: Dich vorstellen (Name, Alter, Wohnort).";
        const generatedGrußMessage = await generateFirstMessage({
          client: client,
          model: AI_MODEL,
          firstMessageInstructions: grußInstructions,
          hasLike: false,
          hasKuss: false,
          profileInfo,
          extractedInfo,
        ...
        if (generatedGrußMessage && generatedGrußMessage.trim() !== "") {
          const minWait = 40;
          const maxWait = 60;
          const waitTimeGruß = Math.floor(Math.random() * (maxWait - minWait + 1)) + minWait;
          const safeGruß = stripInternalPlaceholders(generatedGrußMessage);
          return res.status(200).json({
            resText: safeGruß,
            replyText: safeGruß,
            summary: extractedInfo,
            chatId: chatId || finalChatId || "00000000",
            actions: [{ type: "insert_and_send", delay: waitTimeGruß }],
            assets: [],
            flags: { blocked: false, isFirstMessage: false, noReload: true, skipReload: true, preventReload: true },
            disableAutoSend: true,
            waitTime: waitTimeGruß
          });
        }
      }
      if (hasGefaelltSystemMessage && !hasLikeSystemMessage && !hasKussSystemMessage) {
        ...
        if (generatedGefaelltMessage && generatedGefaelltMessage.trim() !== "") {
          ...
          const safeGefaellt = stripInternalPlaceholders(generatedGefaelltMessage);
          return res.status(200).json({
            resText: safeGefaellt,
            replyText: safeGefaellt,
            summary: extractedInfo,
            chatId: chatId || finalChatId || "00000000",
      ...
      if (likeKussResult && (likeKussResult.hasLike || likeKussResult.hasKuss) && likeKussResult.instructions) {
        ...
        if (generatedLikeKussMessage && generatedLikeKussMessage.trim() !== "") {
          ...
          const safeLikeKussMsg = stripInternalPlaceholders(generatedLikeKussMessage);
          return res.status(200).json({
            resText: safeLikeKussMsg,
            replyText: safeLikeKussMsg,
            summary: extractedInfo,
            chatId: chatId || finalChatId || "00000000",
      ...
      // Fallback: Kuss/Like aus Systemnachricht, aber agentFirstMessageDetector hat nichts Brauchbares geliefert (z. B. Iluvo) – trotzdem Danke + Gesprächsstarter
      if (hasKussSystemMessage || hasLikeSystemMessage) {
        const fallbackLikeKussInstructions = hasKussSystemMessage
          ? "Der Kunde hat dir einen KUSS gesendet (Systemnachricht). NUR: 1) Kurz freuen und bedanken – VARIIERE: z.B. 'Das freut mich!', 'Oh wie suess!', 'Super dass du dich meldest!', 'Danke fuer den Kuss!', 'Nice, danke!'. 2) 1-2 gespraechsoeffnende Fragen – ABWECHSLUNGSREICH (wie geht es dir, was machst du, Tag, Arbeit). KEINE Treffen-Vorschlaege, keine Anspielungen. VERBOTEN: Dich vorstellen (Name, Alter, Wohnort)."
          : "Der Kunde hat dir ein LIKE/Gefaellt mir gesendet (Systemnachricht). NUR: 1) Kurz freuen und bedanken – VARIIERE: z.B. 'Das freut mich!', 'Oh wie suess!', 'Super dass du dich meldest!', 'Nice, danke!'. 2) 1-2 gespraechsoeffnende Fragen – ABWECHSLUNGSREICH (wie geht es dir, was machst du, Tag, Arbeit). KEINE Treffen-Vorschlaege, keine Anspielungen. VERBOTEN: Dich vorstellen (Name, Alter, Wohnort).";
        const generatedFallbackLikeKuss = await generateFirstMessage({
          client: client,
          model: AI_MODEL,
          firstMessageInstructions: fallbackLikeKussInstructions,
          hasLike: hasLikeSystemMessage || false,
          hasKuss: hasKussSystemMessage || false,
          profileInfo,
          extractedInfo,
          rules,
          platformId
        });
        if (generatedFallbackLikeKuss && generatedFallbackLikeKuss.trim() !== "") {
          const minWait = 40;
          const maxWait = 60;
          const waitTimeFallback = Math.floor(Math.random() * (maxWait - minWait + 1)) + minWait;
          const safeFallback = stripInternalPlaceholders(generatedFallbackLikeKuss);
          return res.status(200).json({
            resText: safeFallback,
            replyText: safeFallback,
            summary: extractedInfo,
            chatId: chatId || finalChatId || "00000000",
            actions: [{ type: "insert_and_send", delay: waitTimeFallback }],
            assets: [],
            flags: { blocked: false, isFirstMessage: false, noReload: true, skipReload: true, preventReload: true },
            disableAutoSend: true,
            waitTime: waitTimeFallback
          });
        }
      }
    } catch (err) {
  ...
  // Auch wenn foundMessageText inhaltlich die Favoriten-Systemnachricht ist (z. B. "Hallo Leben_frisch, … dich zu Favoriten hinzugefügt"), Like/Kuss-Pfad nutzen
  ...
  if (hasFavoritenSystemMessage && noRealCustomerTextForFavoriten) {
    console.log("✅ Favoriten-Systemnachricht (ohne Kundentext oder Nachricht = Favoriten-Text) – generiere Danke + Gesprächsstarter wie Like/Kuss (auch AVZ).");
    try {
