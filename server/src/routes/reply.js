    ...
    if (imageUrls.length > 0) {
      ...
      if (dataUrl) {
        ...
        const vision = await client.chat.completions.create({
          ...
          max_tokens: imageType === 'penis' || imageType === 'nude' ? 200 : 120, // Mehr Tokens für explizite Reaktionen
...
${forbiddenWordsSystemNote}`;
    ...
    try {
      ...
      // 🚨 WICHTIG: Finde die letzten 5 Moderator-Nachrichten UND die letzten 5 Kunden-Nachrichten!
      // Das hilft der KI, den vollen Kontext zu verstehen, besonders bei komplexen Themen
      const moderatorMessages = messages
        .filter(m => !isInfoMessage(m) && (m.type === "sent" || m.messageType === "sent") && typeof m?.text === "string" && m.text.trim() !== "")
        .slice(-5); // 🚨 NEU: Letzte 5 Moderator-Nachrichten für besseren Kontext!
      
      const customerMessages = messages
        .filter(m => !isInfoMessage(m) && (m.type === "received" || m.messageType === "received") && typeof m?.text === "string" && m.text.trim() !== "")
        .slice(-5); // 🚨 NEU: Letzte 5 Kunden-Nachrichten für besseren Kontext!
...
WICHTIG: Wirke NICHT zu schlau! Du bist eine normale Person, keine Wissensdatenbank! Weiche IMMER aus, auch wenn der Kunde explizit danach fragt! NIEMALS sachlich oder wissenschaftlich antworten!`;
    ...
    if (trainingData && trainingData.conversations && Array.isArray(trainingData.conversations) && trainingData.conversations.length > 0) {
      ...
      try {
        ...
        // 🚨 WICHTIG: Verbesserte Query für Vektor-Suche
        // Kombiniere Kundennachricht + ALLE erkannten Situationen für bessere Ähnlichkeit
        ...
        vectorSearchResults = await findSimilarExamples(queryText, {
          ...
          minSimilarity: 0.25, // 🚨 REDUZIERT von 0.3 auf 0.25 für mehr Ergebnisse
        ...
        if (vectorSearchResults.length > 0) {
          ...
          console.warn(`⚠️ Vektor-Suche: KEINE ähnlichen Beispiele gefunden (minSimilarity: 0.25, Query: "${queryText.substring(0, 100)}...")`);
        }
      } catch (err) {
        ...
        try {
          ...
          if (learningStats && Object.keys(learningStats).length > 0) {
            ...
            if (learningContext) {
              ...
              console.log(`🧠 Learning-System: Bewährte Muster für Situation "${detectedSituations.length > 0 ? detectedSituations.join(' + ') : 'allgemein'}" hinzugefügt (als Ergänzung)`);
            }
          }
        } catch (err) {
...
Antworte NUR mit der neuen Antwort, keine Erklärungen.`;
          ...
          if (retryText) {
            ...
            // 🚨 WICHTIG: Filtere "klingt" aus der Validierung
            ...
            if (stillForbidden.length === 0) {
              ...
              try {
                const finalRetryPrompt = `🚨🚨🚨🚨🚨🚨🚨🚨🚨 ABSOLUT KRITISCH - VERBOTENE WÖRTER! 🚨🚨🚨🚨🚨🚨🚨🚨🚨

Die vorherige Antwort enthielt VERBOTENE WÖRTER: ${stillForbidden.map(w => `"${w}"`).join(', ')}

🚨🚨🚨🚨🚨 DU DARFST DIESE WÖRTER ABSOLUT NIEMALS VERWENDEN! 🚨🚨🚨🚨🚨

ALLE VERBOTENEN WÖRTER:
${rules.forbiddenWords.map(w => `- "${w}"`).join('\n')}

🚨🚨🚨 KRITISCH: Verwende KEINE dieser Wörter, auch nicht in ähnlicher Form!
- "reiz" verboten = auch NICHT "reiz", "Reiz", "reizt", "reizende", "reizend", "reizvoll"
...
- Statt "reizt" → "geil", "heiß", "aufregend", "macht mich an"

🚨🚨🚨 ORIENTIERE DICH AN DEN TRAINING-DATEN OBEN - DORT SIEHST DU, WIE MAN ES RICHTIG MACHT! 🚨🚨🚨
...
Antworte NUR mit der neuen Antwort, keine Erklärungen.`;
                ...
                const finalRetryChat = await client.chat.completions.create({
                  model: AI_MODEL,
                  messages: [
                    { role: "system", content: systemPrompt + `\n\n🚨🚨🚨 KRITISCH: Die folgenden Wörter sind ABSOLUT VERBOTEN: ${rules.forbiddenWords.filter(w => !w.toLowerCase().includes("klingt")).map(w => `"${w}"`).join(', ')}. Verwende sie NIEMALS, auch nicht in ähnlicher Form! 🚨🚨🚨` },
                    { role: "user", content: finalRetryPrompt }
                  ],
                  max_tokens: 200,
                  temperature: 0.3 // 🚨 Sehr niedrige Temperatur für maximale Regelkonformität
                });
                ...
                if (finalRetryText) {
                  ...
                  const filteredForbiddenWords2 = rules.forbiddenWords.filter(w => !w.toLowerCase().includes("klingt"));
                  for (const forbiddenWord of filteredForbiddenWords2) {
                    const forbiddenLower = forbiddenWord.toLowerCase();
                    const escapedForbidden = forbiddenLower.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
                    const wordPattern = new RegExp(`\\b${escapedForbidden}[a-zäöü]*\\b`, 'i');
                  ...
                  if (stillForbiddenFinal.length === 0) {
                    ...
                    try {
                      const thirdRetryPrompt = `🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨 ABSOLUT KRITISCH - VERBOTENE WÖRTER! 🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨
...
🚨🚨🚨 KRITISCH: Verwende KEINE dieser Wörter, auch nicht in ähnlicher Form!
- "Das klingt" verboten = auch NICHT "das klingt", "klingt", "klingt ja", "klingt nach", "klingt gut", "klingt geil", "klingt richtig", "klingt heiss"
- "reiz" verboten = auch NICHT "reiz", "Reiz", "reizt", "reizende", "reizend", "reizvoll"
- "spannend" verboten = auch NICHT "spannende", "spannendes", "spannend!", "spannendste"
- "Vorstellung" verboten = auch NICHT "vorstellen", "vorstellst", "vorstellte"

🚨🚨🚨 STATTDESSEN VERWENDE DIESE ALTERNATIVEN:
- Statt "reizt" → "geil", "heiß", "aufregend", "macht mich an"
...
Antworte NUR mit der neuen Antwort, keine Erklärungen.`;
                      ...
                      const thirdRetryChat = await client.chat.completions.create({
                        model: AI_MODEL,
                        messages: [
                          { role: "system", content: systemPrompt + `\n\n🚨🚨🚨🚨🚨 ABSOLUT KRITISCH: Die folgenden Wörter sind ABSOLUT VERBOTEN: ${rules.forbiddenWords.filter(w => !w.toLowerCase().includes("klingt")).map(w => `"${w}"`).join(', ')}. Verwende sie NIEMALS, auch nicht in ähnlicher Form! Diese Regel hat HÖCHSTE PRIORITÄT und überschreibt ALLES! 🚨🚨🚨🚨🚨` },
                          { role: "user", content: thirdRetryPrompt }
                        ],
                        max_tokens: 200,
                        temperature: 0.2 // 🚨 EXTREM niedrige Temperatur für maximale Regelkonformität
                      });
                      ...
                      if (thirdRetryText) {
                        ...
                        const filteredForbiddenWords3 = rules.forbiddenWords.filter(w => !w.toLowerCase().includes("klingt"));
                        for (const forbiddenWord of filteredForbiddenWords3) {
                          const forbiddenLower = forbiddenWord.toLowerCase();
                          const escapedForbidden = forbiddenLower.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
                          const wordPattern = new RegExp(`\\b${escapedForbidden}[a-zäöü]*\\b`, 'i');
