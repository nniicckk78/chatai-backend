...
- Beispiel-Nachrichten: ${writingStyle.sampleTexts ? writingStyle.sampleTexts.substring(0, 300) : ""}` : "";
    ...
    if (rules.forbiddenPhrases && rules.forbiddenPhrases.length > 0) {
    ...
    if (isBotAccusation && !appliedSituations.has("botAccusation") && rules.situationalResponses?.botAccusation?.enabled) {
    ...
    if (isSexualTopic && !appliedSituations.has("sexualTopics") && rules.situationalResponses?.sexualTopics?.enabled) {
    ...
    if (rules.preferredWords && !appliedSituations.has("sexualTopics") && (lowerMessage.includes("flirt") || lowerMessage.includes("heiß") || lowerMessage.includes("sexy"))) {
      if (rules.preferredWords.general && Array.isArray(rules.preferredWords.general) && rules.preferredWords.general.length > 0) {
        specificInstructions += `\n\n💬 BEVORZUGTE WÖRTER (allgemein):\n${rules.preferredWords.general.join(", ")}`;
      }
    }
