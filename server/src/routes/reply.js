...
router.post("/", asyncHandler(async (req, res, next) => {
  ...
  if (!isLastMessageFromFake && req.body?.siteInfos?.messages?.length) {
    ...
    const isFPC = (platformId && platformId.toLowerCase().includes('fpc')) || 
                  (req.body?.siteInfos?.origin && req.body.siteInfos.origin.toLowerCase().includes('fpc')) ||
                  (pageUrl && pageUrl.includes('fpc'));
