...
router.post("/", asyncHandler(async (req, res, next) => {
  ...
  /** WM = Diamond, XL = Platin; 30s Insert-Delay wie content.js isModsWmOrXlHost(). */
  const isModsWmOrXlForTimer =
    requestOrigin === "diamond" ||
    requestOrigin === "platin" ||
    /diamondchat\.net/i.test(siteUrlForModsTrigger) ||
    /platin-chat/i.test(siteUrlForModsTrigger) ||
    /diamondchat\.net/i.test(String(pageUrl || "")) ||
    /platin-chat/i.test(String(pageUrl || "")) ||
    /diamondchat\.net/i.test(siteInfosUrl) ||
    /platin-chat/i.test(siteInfosUrl) ||
    (platformId && typeof platformId === "string" && (platformId.toLowerCase().includes("diamond") || platformId.toLowerCase().includes("platin")));
  /** FPC, Cherry, Gold/S69: Extension Post-Typing-Timer (content.js); JSON delay/waitTime hier angleichen für Logs/Konsistenz. */
  ...
  function insertSendDelaySecForPlatform() {
    if (useFastInsertSendDelay) {
      ...
      if (isModsWmOrXlForTimer) return 30; // WM/XL (Diamond + Platin)
      return 3;
    }
    const minW = 40;
    const maxW = 60;
