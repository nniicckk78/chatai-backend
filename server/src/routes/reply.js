...
const { runGrokPipeline } = require("../utils/grok-pipeline"); // 🚀 Grok-Pipeline (ASA, Erstnachricht, Stadt, normale Reply)
...
router.post("/", asyncHandler(async (req, res, next) => {
  ...
  if (!isLastMessageFromFake && (!foundMessageText || foundMessageText.trim() === "") && msgsAll.length) {
    ...
    // 🚨 KRITISCH: Kuss/Like-Systemnachricht = ASA (wir antworten auf den Kuss), auch wenn noch keine Fake-Nachricht im Chat ist
    if (hasKussSystemMessage || hasLikeSystemMessage) {
      isLastMessageFromFake = true;
      asaFromKussOrLike = true;
      console.log(`✅ ASA erkannt: Kuss/Like-Systemnachricht gefunden (${hasKussSystemMessage ? 'Kuss' : 'Like'}) – schreibe Antwort auf Kuss/Like (OpenAI ASA-Pfad).`);
  ...
  if (isFirstMessage) {
    ...
    try {
      ...
      if (firstMessageResult && firstMessageResult.isFirstMessage && firstMessageResult.instructions) {
        ...
        let generatedFirstMessage = "";
        if (USE_GROK_PIPELINE) {
          // 🚀 Grok-Pipeline: Erstnachricht mit Grok generieren
          try {
            const grokResult = await runGrokPipeline({
              conversationHistory: "",
              customerMessage: "",
              profileInfo,
            ...
            generatedFirstMessage = grokResult.finalMessage || "";
            if (grokResult.blocked) generatedFirstMessage = "";
          } catch (err) {
            console.error("❌ Grok Erstnachricht:", err.message);
          }
        } else {
          // Generiere Erstnachricht (OpenAI)
          generatedFirstMessage = await generateFirstMessage({
            client: client,
            model: AI_MODEL,
            firstMessageInstructions: enhancedInstructions,
            hasLike: firstMessageResult.hasLike || false,
  ...
  // Extrahiere Situation(en) – mehrere pro Nachricht möglich (für Grok-Pipeline + Multi-Stage)
  ...
  try {
    ...
    if (USE_GROK_PIPELINE) {
      // 🚀 Grok-Pipeline: ASA, Stadtauswahl, normale Reply mit Grok (inkl. Learning-Context aus Feedback)
