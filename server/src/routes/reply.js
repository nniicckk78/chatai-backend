 ...
 * @param {string} messageText - Kundennachricht
 * @param {{ moderatorName?: string, moderatorAge?: number }} [options] - Fake-/Moderator-Infos, damit Name/Alter des Gegenübers nicht als Kunde extrahiert werden
 */
async function extractInfoFromMessage(client, messageText, options = {}) {
  if (!client || !messageText) return { user: {}, assistant: {} };

  const { moderatorName, moderatorAge } = options;
  const hasModeratorContext = (moderatorName && moderatorName.trim() !== "") || (moderatorAge != null && moderatorAge !== "");

  try {
    let contextBlock = "";
    if (hasModeratorContext) {
      contextBlock = `
KONTEXT – WICHTIG FÜR DIE EXTRAKTION:
- Der Kunde schreibt an einen Chat-Partner (Fake-Profil/Moderator). Dieser heisst "${String(moderatorName || "").trim()}"${moderatorAge != null ? ` und ist ${moderatorAge} Jahre alt` : ""}.
- Wenn in der Nachricht NUR dieser Name oder dieses Alter in Bezug auf die ANDERE Person vorkommt (z.B. "Die 20 jährige Nancy will es haben", "Nancy ist süss"), extrahiere das NICHT als Kundendaten – das sind Bezugnahmen auf den Chat-Partner, nicht auf den Kunden (Autor der Nachricht).
- Extrahiere Name und Age für den Kunden NUR wenn eindeutig der Kunde selbst gemeint ist (z.B. "Ich bin der Thomas", "ich heisse Max", "ich bin 25").
`;
    }
...
Nachricht: ${messageText}`;
    ...
    if (extractedText) {
      ...
      // Sicherheitsnetz: Extrahierter Kunden-Name darf nicht der Fake-/Moderator-Name sein (z.B. "Die 20 jährige Nancy will es haben" -> Nancy = Fake, nicht Kunde)
      if (moderatorName && typeof moderatorName === "string" && cleanUser.Name) {
        const a = String(cleanUser.Name).trim().toLowerCase();
        const b = String(moderatorName).trim().toLowerCase();
        if (a === b || a.includes(b) || b.includes(a)) {
          delete cleanUser.Name;
          if (cleanUser.Age != null && moderatorAge != null && Number(cleanUser.Age) === Number(moderatorAge)) {
            delete cleanUser.Age;
          }
          console.log("📝 Extraktion: Name/Alter des Fake ignoriert (nicht als Kunde übernommen):", moderatorName);
        }
      }
...
// Prüft, ob in messages eine received-Nachricht mit Bild in den letzten 10 Min vorhanden ist (Kunde hat Bild geschickt)
function hasRecentReceivedImageInMessages(messages) {
  if (!Array.isArray(messages) || messages.length === 0) return false;
  const now = Date.now();
...
router.post("/", asyncHandler(async (req, res, next) => {
  ...
  if (isLastMessageFromFake && !asaFromKussOrLike && msgsAll.length && hasRecentReceivedImageInMessages(msgsAll)) {
    isLastMessageFromFake = false;
    foundMessageText = "Der Kunde hat ein Bild geschickt.";
  ...
  // Fake-Name/Alter mitgeben, damit "Die 20 jährige Nancy will es haben" nicht als Kunden-Name/Alter extrahiert wird
  if (client && foundMessageText && foundMessageText.trim() !== "") {
    try {
      ...
      const moderatorAge = moderatorInfo.birthDate?.age != null ? moderatorInfo.birthDate.age : null;
      extractedInfo = await extractInfoFromMessage(client, foundMessageText, { moderatorName: moderatorName || undefined, moderatorAge });
  ...
  try {
    ...
    if (USE_GROK_PIPELINE && !isASACalculated) {
      ...
      // FPC/AVZ Logbuch: gewählte Stadt nur übernehmen, wenn die Pipeline eine gewählt hat
      // (Pipeline wählt nur, wenn im Fake-Profil kein Ort steht oder "sag ich später" – siehe grok-pipeline.js hasLocation / fakeCity)
    ...
    try {
      ...
      if (assistantFromReply && typeof assistantFromReply === "object" && Object.keys(assistantFromReply).length > 0) {
        ...
        Object.keys(assistantFromReply).forEach((key) => {
          if (assistantFromReply[key] != null && assistantFromReply[key] !== "") {
