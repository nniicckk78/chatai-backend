...
router.post("/", asyncHandler(async (req, res, next) => {
  ...
  if ((!foundMessageText || foundMessageText.trim() === "") && req.body?.siteInfos?.messages) {
    ...
    // KRITISCH: Prüfe, ob die Nachricht wirklich NEU ist (innerhalb der letzten 30 Sekunden)
    // Das verhindert, dass alte Nachrichten fälschlicherweise als neue behandelt werden
    // WICHTIG: Prüfe auch, ob die Nachricht nicht ZU NEU ist (< 2 Sekunden) - das könnte bedeuten, dass sie noch nicht wirklich abgeschickt wurde!
    const now = Date.now();
    const maxAge = 30 * 1000; // 30 Sekunden in Millisekunden
    const minAge = 2 * 1000; // 2 Sekunden in Millisekunden - Nachricht muss mindestens 2 Sekunden alt sein, um wirklich abgeschickt zu sein
    
    const lastReceived = orderedMsgs.find(
      m => {
        if (m?.type !== "received" || typeof m.text !== "string" || m.text.trim() === "") {
          return false;
        }
        // Prüfe Zeitstempel, falls vorhanden
        if (m.timestamp) {
          try {
            ...
            if (age > maxAge) {
              ...
              return false; // Nachricht ist zu alt
            }
            if (age < minAge) {
              console.log(`⚠️⚠️⚠️ KRITISCH: Nachricht zu NEU (${Math.round(age * 100) / 100}s) - möglicherweise noch nicht wirklich abgeschickt! Überspringe:`, m.text.substring(0, 50));
              return false; // Nachricht ist zu neu - möglicherweise noch nicht wirklich abgeschickt
            }
            console.log(`✅ Nachricht-Alter: ${Math.round(age * 100) / 100}s - OK`);
          } catch (e) {
            // Wenn Zeitstempel ungültig, akzeptiere die Nachricht (Fallback)
            console.warn("⚠️ Zeitstempel ungültig, akzeptiere Nachricht als Fallback");
          }
        } else {
          // Wenn kein Zeitstempel vorhanden, akzeptiere die Nachricht (Fallback)
    ...
    // KRITISCH: Auch hier prüfen, ob die Nachricht wirklich NEU ist!
    if (!foundMessageText || foundMessageText.trim() === "") {
      const lastAny = orderedMsgs.find(
        m => {
          if (typeof m.text !== "string" || m.text.trim() === "" || m?.type === "sent" || m?.messageType === "sent") {
            return false;
          }
          // Prüfe Zeitstempel, falls vorhanden
          if (m.timestamp) {
        ...
        if (asaTrainingExamples.length > 0) {
          ...
          if (!asaMessage || asaMessage.trim() === "") {
            ...
            if (customerMsg && asaMessage.toLowerCase().includes(customerMsg.toLowerCase().substring(0, 50))) {
              ...
              console.log(`📚 ASA-Beispiel aus Training-Daten gefunden (${asaTrainingExamples.length} verfügbar):`, asaMessage.substring(0, 100) + "...");
            }
          }
        }
      }
...
- NIEMALS über Nummern, Treffen oder andere Themen sprechen - NUR über die fehlende Kommunikation!
- NIEMALS die Kunden-Nachricht echo'en oder wiederholen!
- NIEMALS denken, dass der Kunde deine Nummer will - das steht NICHT im Chat!
...
🚨🚨🚨🚨🚨🚨🚨🚨🚨 ABSOLUT KRITISCH: KEINE WIEDERHOLUNGSFRAGEN - PROGRESSION STATT LOOP! 🚨🚨🚨🚨🚨🚨🚨🚨🚨
– NIEMALS ähnliche Fragen wiederholen oder paraphrasieren!
– NIEMALS Fragen wiederholen wie "wo würdest du anfangen?", "was würdest du machen?", "wie tief", "wie schnell", "was wäre dein Plan", "was würdest du mit deiner Zunge machen?" - das ist ein Echo-Loop!
– Wenn der Kunde etwas KONKRETES sagt (z.B. "lecken", "bei deiner muschi", "in deinen arsch"), gehe DARAUF ein – frage NICHT erneut "wo" oder "was würdest du machen"!
– BEISPIEL FALSCH: Kunde: "lecken" → KI: "wo würdest du anfangen?" → FALSCH! Kunde hat es schon gesagt!
– BEISPIEL FALSCH: Kunde: "lecken" → KI: "was würdest du mit deiner Zunge machen?" → FALSCH! Zu ähnlich, Echo-Loop!
– BEISPIEL RICHTIG: Kunde: "lecken" → KI: "Mmm, das würde mir gefallen. Wie lange würdest du das machen?" → RICHTIG! Neue Richtung, Progression!
– Jede Antwort muss etwas NEUES hinzufügen – nicht erneut fragen!
...
Fragen
🚨🚨🚨 KRITISCH: FRAGEN-LOGIK - MAXIMAL EINE FRAGE, NUR WENN SIE NEUE INFORMATION BRINGT! 🚨🚨🚨
– PRO NACHRICHT MAXIMAL EINE FRAGE!
– Die Frage muss NEUE Information bringen – keine redundanten oder hypothetischen Folgefragen!
– KEINE hypothetischen Folgefragen, wenn bereits eine konkrete Antwort vorliegt!
– BEISPIEL FALSCH: Kunde: "Wurstbrot und Spaghetti" → KI: "Was isst du?" → dann "Was würdest du essen, wenn du mehr Zeit hättest?" → FALSCH! Redundant, hypothetisch, interview-artig!
– BEISPIEL FALSCH: Kunde: "Ich hatte einen stressigen Tag" → KI: "Hattest du einen stressigen Tag?" → FALSCH! Kunde hat es schon gesagt!
– BEISPIEL FALSCH: Kunde: "Ich mache mir gleich was zu essen" → KI: "Was kochst du dir?" → dann "Was würdest du dir sonst so zu essen machen, wenn du mehr Zeit hättest?" → FALSCH! Redundant, hypothetisch!
– Wenn der Nutzer etwas Konkretes nennt (z.B. Essen, Stress, Tagesablauf), reagiere DARAUF oder trage etwas Eigenes bei – stelle NICHT sofort eine weitere ähnliche Frage!
– Wenn die nächste Frage nur eine Umformulierung oder Erweiterung der vorherigen ist, lasse sie WEG und schreibe stattdessen eine Aussage oder einen eigenen Impuls!
– BEISPIEL RICHTIG: Kunde: "Wurstbrot und Spaghetti" → KI: "Klingt lecker! Ich mache mir gleich auch was zu essen." → RICHTIG! Reagiert darauf, kein redundantes Nachfragen!
– BEISPIEL RICHTIG: Kunde: "Ich hatte einen stressigen Tag" → KI: "Stressige Tage sind echt nervig. Was machst du normalerweise, um dich nach so einem Tag zu entspannen?" → RICHTIG! Neue Information, keine Wiederholung!
– Stelle KREATIVE, PROAKTIVE, VERSCHAUTE, DIREKTE Fragen – nicht wie ein Verhör oder Interview!
– Bei Sex-Gesprächen: Stelle VERSCHAUTE, DIREKTE, PROVOKANTE Fragen – nicht zahm oder zurückhaltend!
– BEISPIEL FALSCH: Kunde: "lecken" → KI: "Wie würdest du es machen?" → FALSCH! Zu offensichtlich, wie ein Verhör!
– BEISPIEL FALSCH: Kunde: "lecken" → KI: "Was würdest du dir denn wünschen, dass ich dafür mache?" → FALSCH! Zu zahm, nicht versaut genug!
– BEISPIEL FALSCH: Kunde: "lecken" → KI: "Was würdest du denn gerne mal ausprobieren?" → FALSCH! Zu zahm, nicht versaut genug!
– BEISPIEL RICHTIG: Kunde: "lecken" → KI: "Stehst du eigentlich auf Tittenfick?" → RICHTIG! Versaut, direkt, interessiert!
– BEISPIEL RICHTIG: Kunde: "lecken" → KI: "Wenn du mich schon so richtig geil leckst, soll ich dir dann einen blasen?" → RICHTIG! Versaut, proaktiv, direkt!
– BEISPIEL RICHTIG: Kunde: "fingern" → KI: "Mmm, das würde mir gefallen. Würdest du es eher sanft oder bestimmt angehen?" → RICHTIG! Kreativ, nicht offensichtlich!
– BEISPIEL RICHTIG: Kunde: "69" → KI: "Mmm, 69 ist echt geil. Da kann man sich richtig schön gegenseitig verwöhnen. Würdest du mir dann auch richtig tief in den Hals ficken?" → RICHTIG! Versaut, direkt, interessiert!
– Stelle VERSCHAUTE, DIREKTE Fragen, die echtes Interesse zeigen – nicht zahm oder zurückhaltend!
– Mache auch eigene VERSCHAUTE Vorschläge und zeige, dass du interessiert bist – nicht nur zahme Fragen!
...
– NIEMALS Fragen wiederholen wie "wie tief", "wie schnell", "was wäre dein Plan", "wie würdest du es machen", "was würdest du machen" – das ist ein Echo-Loop und wirkt wie ein Verhör!
– Wenn der Kunde etwas KONKRETES sagt (z.B. "bei deiner muschi", "in deinen arsch", "lecken"), gehe DARAUF ein – frage NICHT erneut "wo" oder "wie würdest du es machen"!
– BEISPIEL FALSCH: Kunde: "lecken" → KI: "Wie würdest du es machen?" → FALSCH! Zu offensichtlich, wie ein Verhör!
– BEISPIEL FALSCH: Kunde: "lecken" → KI: "Was würdest du mit deiner Zunge machen?" → FALSCH! Zu offensichtlich, wie ein Verhör!
– BEISPIEL RICHTIG: Kunde: "lecken" → KI: "Stehst du eigentlich auf Tittenfick?" → RICHTIG! Kreativ, proaktiv, interessiert!
– BEISPIEL RICHTIG: Kunde: "lecken" → KI: "Wenn du mich schon so richtig geil leckst, soll ich dir dann einen blasen?" → RICHTIG! Proaktiv, kreativ, interessiert!
– BEISPIEL RICHTIG: Kunde: "bei deiner muschi" → KI: "Mmm, das würde mir gefallen. Würdest du es eher sanft oder bestimmt angehen?" → RICHTIG! Kreativ, nicht offensichtlich!
...
– Stelle KREATIVE, PROAKTIVE Fragen, die echtes Interesse zeigen – nicht wie ein Verhör!
– Mache auch eigene Vorschläge und zeige, dass du interessiert bist – nicht nur fragen!
...
2. TRAINING-DATEN (im userPrompt) - ABSOLUT HÖCHSTE PRIORITÄT für Schreibstil, Wortwahl und Ton!
3. ZUSÄTZLICHE ALLGEMEINE REGELN (falls vorhanden) - ergänzen die Grundregeln
4. VERBOTENE WÖRTER/PHRASEN - HÖCHSTE PRIORITÄT bei Verwendung
5. SPEZIFISCHE ANTWORTEN (im userPrompt) - für bestimmte Situationen

⚠️ WICHTIG: ⚠️
- Die Grundregeln sind die ABSOLUTE BASIS - sie gelten IMMER
- 🚨🚨🚨 TRAINING-DATEN HABEN ABSOLUT HÖCHSTE PRIORITÄT für Schreibstil, Wortwahl und Ton - ÜBER ALLEN ANDEREN REGELN (AUSSER GRUNDREGELN)! 🚨🚨🚨
- 🚨🚨🚨 SCHREIBE SO, ALS HÄTTEST DU DIE TRAINING-DATEN SELBST GESCHRIEBEN! 🚨🚨🚨
- 🚨🚨🚨 ZIEL: NEUE ANTWORTEN STILISTISCH NICHT VON ECHTEN MODERATOR-ANTWORTEN UNTERSCHEIDBAR! 🚨🚨🚨
...
- Training-Daten zeigen dir den EXAKTEN Schreibstil, die EXAKTE Wortwahl und den EXAKTEN Ton
...
- 🚨 KRITISCH: Wenn der Kunde über Nummern spricht (z.B. "Ich gebe meine Nummer nicht raus"), reagiere NUR auf das, was er sagt - erfinde KEINE eigenen Aussagen über Nummern!
- 🚨 KRITISCH: Sprich NICHT über Nummern, wenn der Kunde nicht danach fragt oder es nicht relevant ist!
- 🚨 KRITISCH: Wenn der Kunde sagt "Ich gebe meine Nummer nicht raus", dann verstehe das - aber sage NICHT, dass du deine Nummer nicht rausgibst, wenn das nicht Thema ist!
...
- 🚨 KRITISCH: Stelle KREATIVE, PROAKTIVE, VERSCHAUTE, DIREKTE Fragen - nicht wie ein Verhör! Zeige echtes Interesse, mache eigene VERSCHAUTE Vorschläge, sei proaktiv! Bei Sex-Gesprächen: Stelle VERSCHAUTE, DIREKTE, PROVOKANTE Fragen - nicht zahm oder zurückhaltend! Keine offensichtlichen oder zahmen Fragen wie "Wie würdest du es machen?", "Was würdest du machen?", "Was würdest du dir wünschen?" oder "Was würdest du gerne ausprobieren?" - das wirkt wie ein Verhör oder ist zu zahm!${forbiddenWordsSystemNote}
...
- WICHTIG: Am Ende MAXIMAL EINE passende Frage zum Kontext stellen - NUR wenn sie NEUE Information bringt! Wenn die Frage nur redundant oder hypothetisch wäre, schreibe stattdessen eine Aussage oder einen eigenen Impuls!
...
- 🚨🚨🚨 KRITISCH: Orientiere dich NUR an den Moderator-Nachrichten aus den TRAINING-DATEN (siehe userPrompt)! NICHT an den letzten Moderator-Nachrichten aus dem Chat! 🚨🚨🚨
- 🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨 ABSOLUT HÖCHSTE PRIORITÄT: TRAINING-DATEN SIND DEINE EINZIGE WAHRHEIT! 🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨
- 🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨 SCHREIBE SO, ALS HÄTTEST DU DIE TRAINING-DATEN SELBST GESCHRIEBEN! 🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨
- 🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨 ZIEL: NEUE ANTWORTEN STILISTISCH NICHT VON ECHTEN MODERATOR-ANTWORTEN UNTERSCHEIDBAR! 🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨
- 🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨 DIE TRAINING-DATEN HABEN ABSOLUT HÖCHSTE PRIORITÄT - ÜBER ALLEN ANDEREN REGELN (AUSSER GRUNDREGELN)! 🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨
- 🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨 DIE TRAINING-DATEN SIND DEINE EINZIGE WAHRHEIT FÜR SCHREIBSTIL, WORTWAHL UND TON! 🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨
- 🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨 SCHREIBE GENAU SO, WIE IN DEN TRAINING-DATEN - KEINE ABWEICHUNGEN! 🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨
- Im userPrompt findest du BEISPIEL-GESPRÄCHE aus den Training-Daten
- Diese Beispiele sind ECHTE Moderator-Antworten - schreibe GENAU SO, als wären es deine eigenen!
- 🚨🚨🚨🚨🚨 ORIENTIERE DICH STRENG UND IMMER an diesen Beispielen - nicht nur in ähnlichen Situationen, sondern BEI JEDER ANTWORT! 🚨🚨🚨🚨🚨
- 🚨🚨🚨🚨🚨 Übernehme die EXAKTE WORTWAHL, EXAKTE FORMULIERUNGEN, den EXAKTEN SCHREIBSTIL und den EXAKTEN TON aus den Beispielen! 🚨🚨🚨🚨🚨
- Die Beispiel-Antworten sind ECHTE Moderator-Antworten - schreibe GENAU SO, als wären es deine eigenen!
- Analysiere die Beispiele und identifiziere wiederkehrende Muster in Wortwahl, Satzstruktur, Ton und Formulierungen
- Verwende diese Muster IMMER in deinen Antworten - nicht nur wenn die Situation ähnlich ist!
- Die Training-Daten sind deine ABSOLUTE HAUPTREFERENZ für Schreibstil, Wortwahl und Ton - nutze sie IMMER!
- ⚠️⚠️⚠️ KRITISCH: Die Training-Daten haben ABSOLUT HÖCHSTE PRIORITÄT für Schreibstil, Wortwahl und Ton! ⚠️⚠️⚠️
...
- 🚨🚨🚨 KRITISCH: Neue Antworten müssen stilistisch NICHT von echten Moderator-Antworten unterscheidbar sein! 🚨🚨🚨
- 🚨🚨🚨 KRITISCH: Schreibe so, als hätten die Training-Daten sie selbst geschrieben - KEINE eigenen Formulierungen, NUR aus den Beispielen! 🚨🚨🚨`;
...
⚠️⚠️⚠️ KRITISCH: Diese ${relevantExamples.length} Beispiele sind ALLE gleich wichtig - es gibt KEIN "Haupt-Beispiel"! ⚠️⚠️⚠️
⚠️⚠️⚠️ KRITISCH: Du MUSST aus ALLEN Beispielen einen GENERELLEN Schreibstil, Wortschatz und Ton bilden! ⚠️⚠️⚠️
⚠️⚠️⚠️ KRITISCH: Neue Beispiele, die später hinzukommen, werden automatisch mit einbezogen! ⚠️⚠️⚠️
⚠️⚠️⚠️ KRITISCH: Schreibe GENAU SO, wie in den Beispielen - KEINE Abweichungen, KEINE eigenen Formulierungen! ⚠️⚠️⚠️
...
🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨 ABSOLUT KRITISCH - HÖCHSTE PRIORITÄT: 🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨
- 🚨🚨🚨🚨🚨 SCHREIBE SO, ALS HÄTTEST DU DIE TRAINING-DATEN SELBST GESCHRIEBEN! 🚨🚨🚨🚨🚨
- 🚨🚨🚨🚨🚨 ZIEL: NEUE ANTWORTEN STILISTISCH NICHT VON ECHTEN MODERATOR-ANTWORTEN UNTERSCHEIDBAR! 🚨🚨🚨🚨🚨
- 🚨🚨🚨🚨🚨 DIE TRAINING-DATEN HABEN ABSOLUT HÖCHSTE PRIORITÄT - ÜBER ALLEN ANDEREN REGELN (AUSSER GRUNDREGELN)! 🚨🚨🚨🚨🚨
...
- 🚨🚨🚨 Schreibe GENAU SO, wie in den Beispielen - KEINE Abweichungen, KEINE eigenen Formulierungen! 🚨🚨🚨
...
- 🚨🚨🚨 KRITISCH: Wenn der Kunde über Nummern spricht (z.B. "Ich gebe meine Nummer nicht raus"), reagiere NUR darauf - erfinde KEINE eigenen Aussagen über Nummern, die nicht relevant sind!
- 🚨🚨🚨 KRITISCH: Sprich NICHT über Nummern, wenn der Kunde nicht danach fragt oder es nicht direkt relevant ist!
...
- Variiere deine Antworten: unterschiedliche Formulierungen, unterschiedliche Ansätze, unterschiedliche Fragen!
- Wenn der Kunde aufdringlich ist oder sich gegen dich stellt, reagiere VARIERT - nicht mit immer gleichen Formulierungen!
- 🚨🚨🚨 ABSOLUT VERBOTEN: NIEMALS die Nachricht des Kunden 1:1 wiederholen oder echo'en! 🚨🚨🚨
- 🚨🚨🚨 ABSOLUT VERBOTEN: Wenn der Kunde sagt "Ich gebe meine Nummer nicht raus", dann wiederhole NICHT "Ich gebe meine Nummer nicht raus" - das ist ein Echo! 🚨🚨🚨
- 🚨🚨🚨 ABSOLUT VERBOTEN: Paraphrasiere NICHT die Nachricht des Kunden - reagiere darauf mit einer EIGENEN Antwort! 🚨🚨🚨
- BEISPIEL FALSCH: Kunde: "Ich gebe meine Nummer nicht raus" → KI: "Ich gebe meine Nummer nicht raus..." → FALSCH! Echo!
- BEISPIEL RICHTIG: Kunde: "Ich gebe meine Nummer nicht raus" → KI: "Das kann ich total verstehen, das ist auch vorsichtig von dir. Was hält dich denn sonst noch ab?" → RICHTIG! Eigene Reaktion!
...
- Am Ende MAXIMAL EINE passende Frage zum Kontext stellen - NUR wenn sie NEUE Information bringt! Wenn die Frage nur redundant oder hypothetisch wäre, schreibe stattdessen eine Aussage oder einen eigenen Impuls!
...
📋 BEACHTE: Grundregeln (Absolute Basis) + Zusätzliche Regeln (aus Dashboard) + Spezifische Antworten (für Situationen) + Training-Daten (für Stil) = Perfekte Antwort!`;
    ...
    // Wenn verbotene Wörter, Meta-Kommentare, formelle Formulierungen, Wiederholungsfragen oder Wiederholungen gefunden wurden, versuche Neu-Generierung
    if (foundForbiddenWords.length > 0 || foundMetaComments.length > 0 || foundFormalPhrases.length > 0 || hasRepetitiveQuestion || foundRepetitions.length > 0) {
...
4. KEINE Wiederholungsfragen enthält (z.B. NICHT "wo würdest du anfangen?", "was würdest du machen?", "was würdest du mit deiner Zunge machen?" wenn der Kunde bereits "lecken" gesagt hat - gehe DARAUF ein, frage NICHT erneut!)
5. KEINE Wiederholungen von vorherigen Antworten enthält - die Antwort muss EINZIGARTIG sein!
