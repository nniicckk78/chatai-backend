...
router.post("/", asyncHandler(async (req, res, next) => {
      ...
      if (lastAny) {
        ...
        if (newestReceived) {
          ...
          if (receivedAge <= maxAgeForMessage) {
            ...
            // Iluvo: Chat ist von unten nach oben (neueste oben) – erste Nachricht in der Liste ist die neueste, unabhängig von Zeitstempel
            const origin = (req.body?.siteInfos?.origin || "").toLowerCase();
            if (origin === "iluvo" && newestFirst && orderedMsgs.length > 0) {
              const firstReceived = orderedMsgs.find(m => (m?.type === "received" || m?.messageType === "received") && !isInfoMessage(m));
              if (firstReceived) {
                const hasImage = !!(firstReceived.image || firstReceived.imageUrl || firstReceived.imageSrc || firstReceived.image_url ||
                  (firstReceived.url && /\.(jpg|jpeg|png|gif|webp)/i.test(String(firstReceived.url))) ||
                  firstReceived.attachment || firstReceived.attachments || firstReceived.media || firstReceived.mediaUrl);
                const txt = (firstReceived.text || "").trim();
                ...
                if (hasImage && noRealCustomerText) {
                  ...
                  console.log("✅ Iluvo: Erste Nachricht (neueste) ist nur Bild – antworte auf Bild (Position statt Zeitstempel).");
                } else if (txt.length > 0) {
                  foundMessageText = txt;
                  console.log("✅ Iluvo: Erste Nachricht (neueste) nach Position verwendet (Zeitstempel ignoriert):", foundMessageText.substring(0, 80) + "...");
                }
              }
            }
            if (!foundMessageText || foundMessageText.trim() === "") {
  ...
  if (!isLastMessageFromFake && ((!foundMessageText || foundMessageText.trim() === "") || isIluvoOrigin) && msgsAll.length) {
    ...
    if (sentMsgs.length === 0) {
      ...
      // Iluvo: Bei neueste-zuerst ist die erste Nachricht in der Liste die neueste (unabhängig von Zeitstempel)
      if (newestFirst && list.length > 0 && (list[0].type === "received" || list[0].messageType === "received") && !isInfoMessage(list[0])) {
        isLastMessageFromFake = false;
        console.log("✅ Iluvo: Erste Nachricht (neueste) ist vom Kunden – kein ASA.");
      } else {
