...
/** xAI Grok Client (OpenAI-kompatibel, baseURL https://api.x.ai/v1). */
...
/** Modellname für Grok-Pipeline (xAI). */
function getGrokModel() {
  return process.env.GROK_MODEL_ID || "grok-4-1-fast-non-reasoning";
}

/** Mistral-Client für Nachrichtengenerierung (Fine-Tune). OpenAI-kompatibel, baseURL https://api.mistral.ai/v1. */
function getMistralGeneratorClient() {
...
/** Modell-ID für Mistral-Generator (z. B. Fine-Tune: ft:mistral-small-latest:... oder mistral-small-latest). */
function getMistralGeneratorModel() {
  return process.env.MISTRAL_FINETUNED_GENERATOR_MODEL || process.env.MISTRAL_GENERATOR_MODEL_ID || "mistral-small-latest";
}
...
module.exports = { getClient, getLocalClient, getTogetherClient, getGrokClient, getMessageClient, getMessageModel, getGrokModel, getMistralGeneratorClient, getMistralGeneratorModel };
